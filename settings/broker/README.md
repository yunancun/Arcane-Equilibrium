...
`stock_etf_ibkr_readonly_probe_result_import_request.template.toml` records the default-blocked result import request envelope for future sanitized IBKR read-only probe outputs with exact `stock_etf_ibkr_readonly_probe_result_import_request_v1` / source-version fields plus read-only probe request, session attestation, allowlist, redaction, audit-event, downstream evidence, raw/redacted/source artifact, as-of, and idempotency lineage. It is secret-free and cannot contact IBKR, start a connector, import evidence, write scorecards, apply DB changes, route orders, or authorize live/tiny-live.
...
`ibkr_dual_engine_contract.template.toml` records the source-only local dual-engine shape for `ibkr_demo_engine` and `ibkr_live_engine`, including trade-core port reservations, broker Gateway/TWS port references, read/write API interface buckets, denied transfer/withdraw actions, and the session/admission epoch Phase2 seal model. It does not authorize IBKR contact, service startup, credential inspection, order routing, transfers, or withdrawals.

`stock_etf_broker_capability_registry.template.toml` records the default-blocked broker capability registry contract shape with exact `broker_capability_registry_v1` / source-version fields. Accepted read rows require the lane-scoped IPC contract and readonly probe request contract in addition to their operation-specific gates; scorecard derivation also requires readonly probe result import request lineage. It is source-only and does not contact IBKR, create secrets, create connectors, route orders, or change Bybit live execution behavior.

`stock_etf_lane_scoped_ipc.template.toml` records the default-blocked lane-scoped IPC method matrix contract shape with exact `lane_scoped_ipc_v1` / source-version fields, including the read-only `stock_etf.preview_readonly_probe` validation fixture. It does not start IPC, contact IBKR, create connectors, route paper orders, inspect secrets, or change Bybit live execution behavior.
...
`stock_etf_shadow_signal_request.template.toml` records the default-blocked typed request shape for `stock_etf.evaluate_shadow_signal` with exact `stock_etf_shadow_signal_request_v1` / source-version fields, signal identity, and lineage hashes. It does not start IPC, contact IBKR, create connectors, emit shadow signals, generate shadow fills, write scorecards, inspect secrets, apply DB changes, route orders, or change Bybit live execution behavior.

`stock_etf_paper_shadow_reconciliation.template.toml` records the default-blocked paper-vs-shadow reconciliation contract shape with exact `stock_etf_paper_shadow_reconciliation_v1` / source-version fields, paper lifecycle/fill identity, synthetic shadow-fill linkage, divergence thresholds, and source hashes. It does not contact IBKR, start connectors, import fills, generate shadow fills, start reconciliation or scorecard writers, apply DB changes, inspect secrets, route orders, or change Bybit live execution behavior.
...
`stock_etf_gui_lane_contract.template.toml` records the default-blocked GUI lane contract shape with exact `gui_lane_contract_v1` / source-version fields. It pins the current Stock/ETF display-only GET surfaces for readiness, lane, phase0 contract packet, data-foundation, policy/capability, authorization, evidence, universe, shadow, paper, reconciliation, account, scorecard, launch, release-packet, and disable-cleanup status. It is source-only and does not serve pages, contact IBKR, create secrets, route orders, or authorize lane selection.

`stock_etf_phase3_evidence_contracts.toml` records the default-blocked market-data provenance, collector-run, DQ manifest, frozen-input, and evidence-clock checker shape. It requires named contract fields, collector/DQ lineage hashes for evidence-clock days, and checker-side side-effect denials for the collector, DQ manifest, and evidence-clock day shapes and does not start a collector, market-data ingestion, a DQ writer, or the evidence clock.
...
`stock_etf_scorecard_derivation.template.toml` records the default-blocked derived scorecard artifact lineage with exact `stock_etf_scorecard_derivation_v1` / source-version fields, input bundle hash, paper-shadow reconciliation hash, verdict hash, formula/preregistration hashes, replay proof, and writer side-effect denials. It does not contact IBKR, import fills, generate shadow fills, start reconciliation or scorecard writers, apply DB changes, start the evidence clock, authorize tiny-live/live, or change Bybit behavior.
...
`stock_etf_tiny_live_adr_eligibility.template.toml` records the default-blocked future ADR discussion eligibility shape with exact `tiny_live_adr_eligibility_v1` / source-version fields, Phase 5 release packet lineage, scorecard derivation/verdict/manifest hashes, paper-shadow reconciliation hash, DQ/statistical preregistration hashes, and QC/MIT/QA review gates. It is secret-free and cannot authorize IBKR tiny-live or live even if a future paper/shadow scorecard is positive.
