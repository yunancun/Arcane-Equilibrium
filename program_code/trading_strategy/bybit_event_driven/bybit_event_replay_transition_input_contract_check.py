...
MODULE_NOTE = \'\'\'
[Maintainer Note]
Script: bybit_event_replay_transition_input_contract_check.py
...
OUT_LATEST = OUT_DIR / "bybit_event_replay_transition_input_contract_latest.json"
...
def main():
    ...
    add("input_exists", obj is not None, str(INPUT_PATH))

    if obj is not None:
        add("input_type_expected", obj.get("input_type") == "bybit_event_replay_transition_input", obj.get("input_type"))
        add("input_version_v1", obj.get("input_version") == "v1", obj.get("input_version"))
        add("exchange_bybit", obj.get("exchange") == "bybit", obj.get("exchange"))
        add("stage_g2_3", obj.get("stage") == "G2.3", obj.get("stage"))
        add("transition_readiness_positive", obj.get("transition_readiness") == "input_ready_for_transition_engine", obj.get("transition_readiness"))
        add("transition_allowed_true", obj.get("transition_allowed") is True, obj.get("transition_allowed"))

    failed = [c for c in checks if not c["ok"]]
    report = {
        "report_type": "bybit_event_replay_transition_input_contract_check",
        "report_version": "v1",
        "ts_ms": now_ms,
    ...
    dated = save_report(report, OUT_LATEST, "bybit_event_replay_transition_input_contract")
