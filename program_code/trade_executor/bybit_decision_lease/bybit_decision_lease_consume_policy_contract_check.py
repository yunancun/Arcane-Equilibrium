...
TARGET = BASE / "bybit_decision_lease_consume_policy_latest.json"
STEM = "bybit_decision_lease_consume_policy_contract"
...
def main() -> None:
    ...
    add("policy_type_expected", obj.get("policy_type") == "bybit_decision_lease_consume_policy", obj.get("policy_type"))
    add("policy_version_v1", obj.get("policy_version") == "v1", obj.get("policy_version"))
    add("stage_i3a", obj.get("stage") == "I3-A", obj.get("stage"))
    add("policy_ok_bool", isinstance(obj.get("policy_ok"), bool), obj.get("policy_ok"))
    add("consume_policy_view_dict", isinstance(obj.get("consume_policy_view"), dict), type(obj.get("consume_policy_view")).__name__ if obj.get("consume_policy_view") is not None else None)
    ...
    add("policy_state_known", obj.get("policy_state") in {
        "decision_lease_consume_policy_blocked",
        "decision_lease_consume_policy_ready_soft_warn",
        "decision_lease_consume_policy_ready",
    }, obj.get("policy_state"))
    add("allow_progress_bool", isinstance(obj.get("allow_progress_to_i3b_consume_gate"), bool), obj.get("allow_progress_to_i3b_consume_gate"))
