...
def main() -> None:
    ...
    response_state = obj.get("response_state")
    terminal_mode = obj.get("terminal_mode")
    ...
    checks = [
        ...
        make_check("report_version_supported", obj.get("report_version") in {"v1", "v2"}, obj.get("report_version")),
        ...
        make_check(
            "response_state_known",
            response_state in {
                "response_json_contract_satisfied",
                "response_invalid_or_incomplete",
                "legal_no_ai_call_terminal",
                "illegal_no_ai_call_state",
            },
            response_state,
        ),
        make_check(
            "terminal_mode_known",
            terminal_mode in {None, "provider_json_ready", "legal_no_ai_call"},
            terminal_mode,
        ),
        make_check(
            "allow_progress_bool",
            isinstance(obj.get("allow_progress_to_h1h_governed_decision"), bool),
            obj.get("allow_progress_to_h1h_governed_decision"),
        ),
    ]
