...
模組目的：
    ...
    + AI-E §9.3 規格：
      ...
      Case 1 — Full capability (22/22 true)：完整 Block B 含
...
# Full schema：22/22 capability all true
PROBE_FULL_SCHEMA = [
...
# ─── Case 1：Full capability (22/22 true) → 完整 Block B ──────────────────


def test_case1_full_capability_all_true_emits_full_block_b():
    """Case 1：22/22 capability 全 true → Block B 含 manifest_hash NOT NULL +
...
def test_case5_capability_re_probed_each_cycle():
    ...
    # cursor1 應收到 7 次 probe execute 呼叫 + 0 final SELECT (caller 沒 execute sql)
    # probe：MSR / replay / residual registry / hidden OOS registry
    assert cur1.probe_call_count == 7, (
        f"cycle 1 probe_call_count={cur1.probe_call_count}, expected 7"
    )
    ...
    assert cur2.probe_call_count == 7, (
        f"cycle 2 probe_call_count={cur2.probe_call_count}, expected 7"
    )
    ...
    # 同一 cursor 連跑兩 cycle：probe 必觸發 14 次（每 cycle 7 次，0 cache）
    ...
    assert cur3.probe_call_count == 14, (
        f"two cycles same cursor probe_call_count={cur3.probe_call_count}, "
        f"expected 14 (re-probe each cycle, no cache)"
    )
