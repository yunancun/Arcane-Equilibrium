...
MODULE_NOTE
模塊用途：掃 V137 `research.alpha_wealth_ledger` 的 pending debit（經
...
硬邊界：
  ...
  - double-refund / refund+fail 並存在 DB 層不可能（V137 partial unique
  ...
  - binding 缺 = 永 pending（attribution 紀律：不猜 cell）；stage0r verdict
    缺 = pending（證據缺席不渲染結論性裁決，不誤鑄 dead-mode）。
  - 三重 OFF：flag `OPENCLAW_ALPHA_WEALTH_RECONCILER` 預設 0 + cron job 不在
    DEFAULT_JOBS + V137 表 0 rows = 部署即行為中性。
...
OPERATOR RUNBOOK（P4 activation 開閘序；全部 operator-gated，本模組不替開）
  ...
  healthcheck [82]-[86]（checks_alpha_wealth_fdr.py）按此語義監測。
...
# pending debit：消費 V137 debit_state 視圖（M1「debit_state 在 PG」落點），
...
def _v137_deployed(cur: Any) -> bool:
    """V137 未部署 → 對帳器無事可做（SKIP 不 FAIL，部署順序容忍）。"""
    cur.execute(
...
def run_alpha_wealth_refund_reconciler(
    ...
    *,
    ...
    冪等性：refund / debit_failed 由 V137 awl_one_terminal_per_debit 在 DB 層
    ...
    with conn.cursor() as cur:
        if not _v137_deployed(cur):
            summary["skipped"] = "v137_not_deployed"
            logger.info("alpha_wealth_reconciler: V137 tables absent; nothing to do")
