...
def test_fee_path_finds_first_standard_vip_tier_that_clears_break_even():
    ...
    actionability = scorecard["business_path_actionability"]
    assert actionability["status"] == "STANDARD_FEE_TIER_CLEARS_BUT_SCALE_OR_CAPITAL_GATED"
    assert actionability["first_clearing_tier"] == "VIP5"
    assert actionability["volume_gap_usd"] == 249_159_700.59
    assert actionability["volume_multiplier_needed"] == 297.513
    assert actionability["asset_gap_usd"] == 2_000_000.0
    assert actionability["operator_action_required"] == (
...
def test_fee_path_reports_current_fee_clear_when_break_even_above_current_fee():
    ...
    assert scorecard["business_path_actionability"]["status"] == (
        "CURRENT_ACCOUNT_FEE_CLEARS_BREAK_EVEN"
    )
    assert scorecard["business_path_actionability"]["operator_action_required"] == (
...
def test_fee_path_preserves_no_break_even_cell_status():
    ...
    assert scorecard["business_path_actionability"]["status"] == (
        "NO_SAMPLE_GATED_BREAK_EVEN_CELL"
    )
    assert scorecard["business_path_actionability"]["operator_action_required"] == (
        "find_sample_gated_break_even_cell_first"
    )
