...
TARGET = BASE / "bybit_decision_lease_consume_gate_latest.json"
STEM = "bybit_decision_lease_consume_gate_contract"
...
def main() -> None:
    ...
    add("gate_type_expected", obj.get("gate_type") == "bybit_decision_lease_consume_gate", obj.get("gate_type"))
    add("gate_version_v1", obj.get("gate_version") == "v1", obj.get("gate_version"))
    add("stage_i3b", obj.get("stage") == "I3-B", obj.get("stage"))
    add("gate_ok_bool", isinstance(obj.get("gate_ok"), bool), obj.get("gate_ok"))
    add("consume_decision_dict", isinstance(obj.get("consume_decision"), dict), type(obj.get("consume_decision")).__name__ if obj.get("consume_decision") is not None else None)
    ...
    add("gate_state_known", obj.get("gate_state") in {
        "decision_lease_consume_gate_blocked",
        "decision_lease_consume_gate_ready_soft_warn",
        "decision_lease_consume_gate_ready",
    }, obj.get("gate_state"))
    add("allow_progress_bool", isinstance(obj.get("allow_progress_to_i3c_final_audit"), bool), obj.get("allow_progress_to_i3c_final_audit"))
