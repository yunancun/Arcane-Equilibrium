...
from .closed_pnl_pagination import (
    ...
    _closed_pnl_decode_cursor,
    _closed_pnl_history_cursor_payload,
    _fetch_pg_closed_pnl_fallback,
    _safe_float,
)
from .demo_snapshot_payloads import (
    _demo_snapshot_fills_payload,
    _demo_snapshot_metrics_payload,
    _demo_snapshot_orders_payload,
    _demo_snapshot_pnl_series_payload,
...
# Closed-PnL 降級/快取狀態 — 為 route 層 monkeypatch / 全域狀態縫，故留在本模塊；
# 純分頁/視窗/正規化邏輯已移至 closed_pnl_pagination。
...
async def get_demo_closed_pnl(
    ...
    if cursor_mode_enabled or cursor_token:
        payload = await _closed_pnl_history_cursor_payload(
            ...
            # 注入 route 層 monkeypatch / 全域狀態縫；lambda 於呼叫時解析當前綁定，
            # 保留測試 patch routes._engine_owner_strategy_map / failure-state 的語意。
            engine_owner_lookup=lambda engine: _engine_owner_strategy_map(engine),
            record_failure=_record_closed_pnl_bybit_failure,
            clear_failures=_clear_closed_pnl_bybit_failures,
        )
