...
(function _wireReplaySubtabNamespace() {
  ...
  function _injectReplayReadyCss() {
    ...
    s.textContent = ''
      + '.oc-replay-ready-card{background:rgba(var(--ov-panel-b-rgb),0.7);'
      ...
      + '.oc-replay-ready-title strong{display:block;color:var(--pos);'
      + 'font-size:14px;margin-bottom:3px}'
      + '.oc-replay-ready-meta{font-size:11px;color:var(--text-secondary);'
      + 'word-break:break-all;line-height:1.5}'
      + '.oc-replay-ready-banner{font-size:12px;color:var(--text-secondary);'
      + 'line-height:1.6;padding:8px 12px;background:rgba(var(--ov-panel-rgb),0.55);'
      + 'border-left:3px solid var(--text-secondary);border-radius:var(--r-1);margin:8px 0 14px}'
      + '.oc-replay-ready-banner .zh{display:block;color:var(--text-primary)}'
      + '.oc-replay-ready-banner .en{display:block;color:var(--text-secondary);'
      + 'font-size:11px;margin-top:2px}'
      + '.oc-replay-cells-grid{display:grid;gap:8px;margin-bottom:12px;'
      + 'grid-template-columns:repeat(auto-fill,minmax(180px,1fr))}'
      + '.oc-replay-cell{background:rgba(var(--ov-panel-rgb),0.4);border:1px solid var(--border-subtle);'
      ...
      + '.oc-replay-cell-val{font-size:13px;color:var(--text-primary);font-weight:600;'
      + 'margin-top:4px}'
      + '.oc-replay-cell.oc-cell-warn{border-color:rgba(248,81,73,0.5);'
      + 'background:rgba(248,81,73,0.06)}'
      + '.oc-replay-cell.oc-cell-warn .oc-replay-cell-val{color:var(--neg)}'
      + '.oc-replay-cell.oc-cell-ok{border-color:rgba(63,185,80,0.5);'
      + 'background:rgba(63,185,80,0.06)}'
      + '.oc-replay-cell.oc-cell-ok .oc-replay-cell-val{color:var(--pos)}'
      ...
      + '.oc-replay-badge{font-size:10px;color:var(--text-secondary);border:1px solid var(--border-subtle);'
      + 'border-radius:999px;padding:3px 8px;background:rgba(var(--ov-panel-rgb),0.45)}'
      + '.oc-replay-badge-warn{color:var(--neg);border-color:rgba(248,81,73,0.45);'
      + 'background:rgba(248,81,73,0.07)}'
      + '.oc-replay-quick-grid{display:grid;grid-template-columns:repeat(4,minmax(130px,1fr));'
      + 'gap:8px;padding:12px;background:rgba(var(--ov-panel-rgb),0.46);'
      + 'border:1px solid rgba(var(--ov-accent-rgb),0.22);border-radius:var(--r-1)}'
      + '.oc-replay-strategy-field{grid-column:span 4;display:flex;gap:8px;'
      + 'flex-wrap:wrap;border:1px dashed var(--border-subtle);border-radius:var(--r-1);'
      + 'padding:8px 10px;color:var(--text-secondary);font-size:10px}'
      + '.oc-replay-strategy-field legend{padding:0 4px;color:var(--text-secondary);'
      + 'text-transform:uppercase}'
      + '.oc-replay-check{display:flex;align-items:center;gap:5px;color:var(--text-primary);'
      ...
      + 'gap:8px;align-items:center;background:rgba(var(--ov-panel-rgb),0.42);'
      + 'border:1px solid var(--border-subtle);border-radius:var(--r-1);padding:7px 9px;'
      + 'font-size:11px;color:var(--text-primary)}'
      + '.oc-replay-run-row code{font-size:11px;color:var(--text-secondary);'
      + 'overflow:hidden;text-overflow:ellipsis;white-space:nowrap}'
      + '.oc-replay-run-row strong{color:var(--pos);font-size:11px}'
      + '.oc-replay-warning-line{margin-top:8px;font-size:11px;color:var(--neg);'
      + 'line-height:1.5}'
      + '.oc-replay-workflow-grid{display:grid;grid-template-columns:repeat(4,minmax(120px,1fr));'
      + 'gap:8px;padding:10px 12px;background:rgba(var(--ov-panel-rgb),0.4);'
      + 'border:1px dashed var(--border-subtle);border-radius:var(--r-1)}'
      + '.oc-replay-field,.oc-replay-json{display:flex;flex-direction:column;'
      + 'gap:5px;font-size:10px;color:var(--text-secondary);text-transform:uppercase}'
      + '.oc-replay-json{grid-column:span 4}'
      + '.oc-replay-field input,.oc-replay-field select,.oc-replay-json textarea{'
      + 'background:var(--bg-app);border:1px solid var(--border-subtle);border-radius:var(--r-1);'
      + 'padding:6px 8px;color:var(--text-primary);font-family:monospace;font-size:12px;'
      + 'min-width:0}'
      ...
      + 'align-items:center;padding:10px 12px;background:rgba(var(--ov-panel-rgb),0.4);'
      + 'border:1px dashed var(--border-subtle);border-radius:var(--r-1)}'
      + '.oc-replay-load-label{font-size:11px;color:var(--text-secondary)}'
      + '.oc-replay-load-input{flex:1;min-width:200px;background:var(--bg-app);'
      + 'border:1px solid var(--border-subtle);border-radius:var(--r-1);padding:6px 10px;'
      + 'color:var(--text-primary);font-family:monospace;font-size:12px}'
      + '.oc-replay-load-status{font-size:11px;color:var(--text-secondary);'
      + 'flex-basis:100%;line-height:1.5}'
      + '.oc-replay-load-status.oc-cell-warn{color:var(--neg)}'
      + '.oc-replay-load-status.oc-cell-ok{color:var(--pos)}'
