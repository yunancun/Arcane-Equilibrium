 ...
 *   保留 Pipeline Bridge、Telegram Alerter 與 System Health 的唯讀監控；已退役的
 *   外部 Gateway 與 Grafana 不再出現在 UI、輪詢或外部開啟入口。
 * 主要函數:renderMonitorView(建骨架,冪等)、resumeMonitorView(顯示→拉真值+啟輪詢)、
 *   pauseMonitorView(隱藏→停輪詢/停 fetch)、load / loadPipeline /
 *   loadTelegram / loadHealth(per-view fetch)、renderHealth / flatEntries。
 ...
 * 硬邊界(canon / LOOP §6):
 *   ① 零寫路徑——monitor 唯讀:4 個 ocApi GET(paper session status / strategy status /
 *      telegram status / system health),無 POST/order。
 ...
 *      .style.setProperty('--tag-tone', var(...)) scoped-var 正法。
 * 誠實邊界:靜態(node --check + ratchet + 5b 對齊 + registry/asset smoke)只證 source/路徑
 *   事實;**真渲染正確性 / 三態版式 / 真值 = NEEDS-LINUX runtime + operator 視覺**,
 *   不由本刀 attest。
 * ═══════════════════════════════════════════════════════════════════
 */
(function () {
      ...
      '<div class="panel">' +
        '<div class="row-between wrap gap-3">' +
          '<div>' +
            ...
            '<div class="note">管線、告警與系統 healthcheck 彙總(唯讀)。</div>' +
          '</div>' +
  ...
  async function load() {
    ...
    showError('');
    try {
      await Promise.allSettled([loadPipeline(), loadTelegram(), loadHealth()]);
    } finally {
      loading = false;
    }
    if (!built) return;
