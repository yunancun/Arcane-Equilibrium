...
class EdgeEstimatorScheduler:
    ...
    def _run_backfill(self, mode: str) -> dict:
        ...
        r = backfill_labels(
            pg_url=get_secret_value("OPENCLAW_DATABASE_URL"),
            engine_mode=mode,
            batch_limit=5000,
            dry_run=False,
        )
        ...
        if mode == "demo":
            try:
                ...
                statement_timeout_ms = int(
                    os.environ.get("OPENCLAW_MLDE_LINUCB_STATEMENT_TIMEOUT_MS", "5000")
                )
                max_age_days = int(
                    os.environ.get("OPENCLAW_MLDE_LINUCB_MAX_AGE_DAYS", "30")
                )
