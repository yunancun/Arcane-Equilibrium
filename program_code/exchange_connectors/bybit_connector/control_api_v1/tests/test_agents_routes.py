...
def test_helpers_module_under_size_guards() -> None:
    """M-3：拆檔後兩個模組都遵守單一 2000 行上限，且第 2000 行可接受。

    拆分仍用來維持清晰責任邊界，不再把較低的歷史目標當成阻擋門檻。
    初次拆分的單檔基線為 775 行；heartbeat-contract round 1 加入五張卡片
    的 heartbeat 呈現，round 2 再抽出 ``_surface_heartbeat_ts`` 共用 helper。
    helper 實測約 827 行，承載 5-Agent、verdicts、intent 與 heartbeat 呈現，
    目前仍有充分治理餘量。
    """
    ...
    assert route_lines <= MAX_FILE_LINES, (
        f"agents_routes.py = {route_lines} lines (limit {MAX_FILE_LINES})"
    )
    assert helper_lines <= MAX_FILE_LINES, (
        f"agents_routes_helpers.py = {helper_lines} lines "
