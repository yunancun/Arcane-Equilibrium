...
def _valid_authorization_status() -> dict[str, Any]:
    return {
        ...
        # (`blocked_session_attestation()`):契約身分立正(contract_id/source_version)、
        # status 恆 BLOCKED,blockers 不再含 contract/version mismatch 兩項。鍵集嚴格鏡射
        # Rust `authorization_status_summary` 投影(瘦形;host/port 等富鍵只在
        # account_status 投影,不得混入)。
        "session_attestation": {
