...
def _create_and_advance_to(sm, target: OrderState) -> str:
    """Helper: create order and advance to target state / 创建订单并推进到目标状态"""
    ...
    if target == OrderState.RECONCILING:
        return oid
...
class TestPreExecutionFlow:
    def test_created_to_pending(self, sm):
        oid = _create_and_advance_to(sm, OrderState.PENDING)
        assert sm.get(oid)["state"] == "PENDING"

    def test_pending_to_approved(self, sm):
        oid = _create_and_advance_to(sm, OrderState.APPROVED)
...
class TestExecutionFlow:
    def test_approved_to_submitted(self, sm):
        oid = _create_and_advance_to(sm, OrderState.SUBMITTED)
        assert sm.get(oid)["state"] == "SUBMITTED"

    def test_submitted_to_working(self, sm):
        oid = _create_and_advance_to(sm, OrderState.WORKING)
    ...
    def test_working_to_partial_fill(self, sm):
        oid = _create_and_advance_to(sm, OrderState.PARTIALLY_FILLED)
        assert sm.get(oid)["state"] == "PARTIALLY_FILLED"

    def test_working_to_filled(self, sm):
        oid = _create_and_advance_to(sm, OrderState.FILLED)
...
class TestPostExecutionFlow:
    def test_filled_to_reconciling(self, sm):
        oid = _create_and_advance_to(sm, OrderState.RECONCILING)
...
class TestForbiddenTransitions:
    ...
    def test_completed_cannot_exit(self, sm):
        ...
        oid = _create_and_advance_to(sm, OrderState.COMPLETED)
...
class TestQueries:
    def test_get_active_orders(self, sm):
        oid1 = _create_and_advance_to(sm, OrderState.WORKING)
        oid2 = _create_and_advance_to(sm, OrderState.PENDING)
        oid3 = _create_and_advance_to(sm, OrderState.COMPLETED)
    ...
    def test_status_summary(self, sm):
        _create_and_advance_to(sm, OrderState.WORKING)
        _create_and_advance_to(sm, OrderState.COMPLETED)
