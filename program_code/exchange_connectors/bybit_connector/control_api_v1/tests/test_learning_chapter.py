...
class TestAutoLearningPipeline:
    ...
    def test_ai_consult_response_includes_deprecation_notice(self):
        """
        廢棄端點應在回傳中包含 deprecation_notice 字段。
        Deprecated endpoint must include deprecation_notice field in response data.
        """
        import warnings
        client = build_client()
        rev = get_state_revision(client)
        ...
        packet_ids = scan_r.json()["data"]["packet_ids"]
        if not packet_ids:
            return  # No packets to consult — skip (not an error condition)

        packet_id = packet_ids[0]
        rev = get_state_revision(client)
        # Use catch_warnings to suppress DeprecationWarning from leaking into test output.
        # 用 catch_warnings 過濾 DeprecationWarning，避免干擾測試輸出。
        ...
        assert r.status_code == 200
        d = r.json()["data"]
        # deprecation_notice must be present and non-empty
        # deprecation_notice 必須存在且非空
        assert "deprecation_notice" in d, "deprecation_notice field missing from response"
        assert d["deprecation_notice"] is not None, "deprecation_notice must not be None"
        assert len(d["deprecation_notice"]) > 0, "deprecation_notice must not be empty"
        assert "intel-log" in d["deprecation_notice"], (
            "deprecation_notice should point to /phase2/strategist/intel-log"
        )
