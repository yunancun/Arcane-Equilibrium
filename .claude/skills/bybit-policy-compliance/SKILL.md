...
> ⚠️ **本節門檻 / rebate 數字為 reference snapshot；Bybit broker / MM 計劃條款動態變動**。**最終以 Bybit 官方 Broker / Institutional 頁面為準**（BB 可 WebFetch 查證）；申請評估 / eligibility 結論前必先官方核對，本 skill 不取代官方條款。

### 4.1 Broker partnership
- 客戶帶交易量給 Bybit → 享 fee rebate
- 申請門檻（snapshot）：30d 累計 volume ≥ $10M
- 收益（snapshot）：taker fee 15-30% rebate

### 4.2 Market Maker 計劃
- 條件（snapshot）：maker volume ≥ $50M / 30d + maker ratio ≥ 60%
- 收益（snapshot）：maker rebate 負費率提升
...
| Broker rebate | 30d volume vs 官方門檻（§4 snapshot disclaimer） | (sub-agent 填) |
...
9. **Broker rebate eligibility**（30d volume vs 官方門檻，先 WebFetch 核對 §4）
