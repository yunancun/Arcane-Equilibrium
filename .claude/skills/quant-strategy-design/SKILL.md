...
description: QC agent 主用：新策略提案、alpha hypothesis、信號設計、paper/KOL 異常評估、edge 衰減接班評估時先讀（三段鏈之首 → math-model-audit → walk-forward-validation-protocol，順序不可顛倒）。
...
- **Bybit funding 公式**：`F = clamp[ P + clamp(I − P, ±0.05%), upperFundingRate, lowerFundingRate ]`，I（interest rate baseline / floor）= 0.03% ÷ (24 / fundingInterval_h)；8h 結算時 I = +0.01%/8h = +0.0001 = +10.9% APR。低-premium 窗正側 max 全 = +0.0001 是 **IR floor 指紋，不是 cap**。
- **2026-05-31 教訓**（`docs/audits/2026-05-31--funding_short_v2_structural_infeasibility.md` ERRATUM）：funding_short_v2 審計把 66 天低-premium 窗的 +0.0001 max 誤當 +10.9% APR 結構 cap，錯誤宣布「永久結構性 NO-GO」。實情：Bybit 真 cap = `upperFundingRate`，比誤判值高 50–200×（**2026-05 snapshot 示例**：BTC/SOL +547% / DOGE +635% / 1000PEPE +1095% / WIF +2190% APR；實際以 per-symbol `upperFundingRate` 即時查，勿照抄）。正確定性 = **regime-dormant 低頻**（bull premium 下會 fire），真問題在策略 160% break-even 門檻過高，屬成本/門檻設計範疇。完整 funding 公式 + cap 欄位見 `docs/references/2026-04-04--bybit_api_reference.md` get_funding_history 後「Funding Rate 公式」段。
...
`engine_mode IN ('live','live_demo')` filter 必含兩者；edge_estimator JSON 結構 = `strategy::symbol` top-level key（不是 `cells{}` nested）。其餘 OpenClaw 特定情境跑命令查 SSOT（見頭部權威序）。
