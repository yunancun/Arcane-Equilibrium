...
### 1.1 結算機制
- **結算週期 per-symbol**：`fundingInterval` 因 symbol 而異（1h / 4h / 8h 不等），整點 snapshot；不可假設普適 8h cycle，以 `instruments-info` 即時查
- **funding cap/floor per-symbol**：上下限為 `upperFundingRate` / `lowerFundingRate`（`instruments-info` 即時查），不是固定 clamp 值
- **計算公式**（簡化）：`F = clamp(premium_index + interest_rate, lowerFundingRate, upperFundingRate)`
- **方向**：F > 0 → long 付 short；F < 0 → short 付 long
- **應用 / 收取時**：持倉跨越 settlement instant 才算
- 本節與 `quant-strategy-design` funding 機制專段保持一致；衝突以 Bybit `instruments-info` 即時值為準
...
- 在 thin orderbook 下幾分鐘可洗 -10% 到 -30%（2020-03-12、2021-05-19、2022-05 LUNA、2022-11 FTX）
...
- 費率以 Bybit 官方 fee schedule 即時查（BB 可 WebFetch）；本檔不寫死數字
- 結構通則：volume tier 越高 maker fee 越低，高 tier 可達 maker rebate（負費率）

> 帳戶真實當前 fee：API 查 `/v5/account/fee-rate` 或字典手冊 `docs/references/2026-04-04--bybit_api_reference.md`
