...
- **設計前必排查**（通用清單，不引特定 OpenClaw 過往實驗結果以避免誤導）：
  ...
  - OpenClaw 過往 funding_arb 實驗結論查 CLAUDE.md §三 + git log，**不引本 skill 內過期數字**
...
**穩定不變的平台結構 rule**：Bybit 為唯一交易所（CLAUDE.md §一；cross-CEX 策略 out of scope）；Bybit demo 不支援 spot lending（cash-and-carry 在 demo 不可行）；funding settlement 前後高波動是 perp 結構性現象（不是 OpenClaw 特有）。
