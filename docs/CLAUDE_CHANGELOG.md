...
`py_compile` PASS，`git diff --check` PASS。Linux `trade-core` fast-forward 到
`91379cd2` 後 targeted tests 215 PASS、`py_compile` PASS。Runtime row proof
仍待 MAG-013/014 啟用/驗證 `[52]`。
