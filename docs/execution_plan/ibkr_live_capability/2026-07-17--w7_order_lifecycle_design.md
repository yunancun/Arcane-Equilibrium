...
- **placeOrder = 定長位置編碼(非 head-prefix)**:encoder 必逐欄輸出到 band 末欄(sv≥145 省前導 VERSION);STK 現金天然塌縮 comboLegs/deltaNeutral/algo/conditions 變長塊;承載欄 = action / totalQuantity(sv≥101 float)/ orderType / lmtPrice / auxPrice / tif / account / transmit / outsideRth / cashQty(sv≥111 fractional)/ whatIf;sv≤157 骨架末欄 = `usePriceMgmtAlgo`(@151)。
- **cancelOrder ≤157 band** = `[4, VERSION=1, orderId]`,**無 `manualOrderCancelTime`**(10.x sv≥161 才加 → DIVERGENT,見 §11 BLOCK-ORDER-BAND-2)。
- **nextValidId 管理**:`REQ_IDS=8`(OUT)請求 → `NEXT_VALID_ID=9`(IN)回;order-id 本地遞增;冪等真源仍是 `idempotency_key`(§2.2.4)。
- **error code**:`10147` 官方查無 → 用 **`10148`**(order-verb 相關 typed reject)。
...
2. **preview(whatIf)**:`placeOrder` with `whatIf=true` 是**零效果預覽**(broker 回 margin/commission 預估,不成單)。whatIf frame 仍走 `send_order_framed`(它是 order-verb 訊息,受 INV-ORDER;**設計決策:whatIf 仍需 effect envelope**,因它觸 order channel,保守 fail-closed)。
   - **★R18 衝突 seam(IB 现勘)**:whatIf 的 **OrderState 尾在 `OPEN_ORDER=5` 訊息尾端**(whatIf/status/initMargin*/maintMargin*/equityWithLoan*/commission/minCommission/maxCommission/warningText,sv≥142)。**R18 openOrder head-prefix descope 讀不到此尾** → **preview 功能前置 = 擴充 openOrder decode 越過 head-prefix 邊界**。此擴充受 158-176 band UNVERIFIED 約束(§11 BLOCK-ORDER-BAND-3:whatIf 尾增長)。**歸片 S1**(preview builder + openOrder tail decode 同片);sv∈[145,157] band 骨架確定可先開發,真接觸待 10.x re-pin。
...
| IB 现勘決策點 | 設計處置 | 歸片 |
|---|---|---|
| **①openOrder tail 擴充(R18 head-prefix 衝突)** | whatIf OrderState 尾在 `OPEN_ORDER=5` 尾端;preview 功能前置=擴 openOrder decode 越 head-prefix;受 158-176 band UNVERIFIED 約束 | **S1**(§2.2.2) |
...
W7 可在 **sv∈[145,157] band 先開發 encoder+decoder(骨架確定)**;**真接觸待 10.x re-pin + envelope**。以下三項標為 **EA 前置 blocking**(承 §4.4 gap matrix UNVERIFIED 紀律,IB 定期政策巡檢):

- **BLOCK-ORDER-BAND-1**:placeOrder 158-176 尾欄 UNVERIFIED(官方位元組未 pin;10.x band decode/encode 佈局待現勘)。
- **BLOCK-ORDER-BAND-2**:cancelOrder 10.x `manualOrderCancelTime`(sv≥161)DIVERGENT(≤157 band 無此欄)。
- **BLOCK-ORDER-BAND-3**:whatIf OrderState 尾在 10.x 增長(openOrder tail 擴充受此約束)。

三項均 **transport ceiling guard 覆蓋**(sv>157 拒 encode/decode + audit,§1.5),故 W7 source-ready 不被 band 未證阻塞;10.x re-pin 是 **EA 活化前置**(operator follow-up + IB 现勘),非 W7 收口前置。error code 用 **10148**(10147 官方查無)。
