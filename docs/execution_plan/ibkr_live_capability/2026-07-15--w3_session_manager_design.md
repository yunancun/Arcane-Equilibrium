...
> **as-built 偏差(S4,2026-07-16;E2-F3/E3 認可)**:本設計原擬 fake crate 直接 `impl TransportFactory`(§API 形態 `build() -> (impl TransportFactory, ...)`)。**實作改為 fake crate engine-independent**——fake **不依賴 openclaw_engine**,只提供場景 DSL + tokio duplex spawn(`Scenario::spawn() -> (DuplexStream, FakeHandle)`)+ 自有 wire codec;`TransportFactory` trait 由 **engine 側 test-only adapter**(`OneShotTransport`/`ScriptedTransport`,`#[cfg(test)]` 於 `ibkr_tws_driver_tests.rs`)實作,包裝 fake 的 duplex。**理由**:若 fake `impl` engine trait,則 engine 的 `--lib` in-file 測試中,「engine-under-test」與「fake 所連結的 engine」是**不同編譯單元**(dev-dep cycle:engine dev-deps fake、fake deps engine),trait/型別**不 unify**(E0599:`FakeFactory: TransportFactory`(fake 眼中的 engine)≠ `crate::TransportFactory`(engine-test 眼中)),實測驗證不可行。engine-independent + engine adapter 令型別在 engine-test 單一編譯單元內 unify,`--lib` 端到端測試走通,且 fake 零 socket 更易獨立驗(自有 codec + in-crate 源守衛)。**場景 DSL 落地**:`Send/SendRaw/Delay/CloseAbruptly/SilentDrop`(`SilentDrop`=keep-open 不回心跳,driver 以心跳 miss 判 liveness → Degraded/HeartbeatDropped);`FakeHandle::{received_bytes, received_message_frames, assert_script_exhausted, steps_done}`。`AnswerHandshake/Expect/Duplicate/Reorder/InjectErr` 未做為 DSL primitive——改以 canned 場景(`version_too_old`/`reordered_handshake`/`serve_duplicate_current_time`/`serve_farm_blip`/`kick_duplicate_client`/`pacing_violation` 等)直接組 frame,較 primitive 更直白。**W4+ follow-up**:若 W10 E2E 需在 `tests/` 整合測試(獨立 crate,見 non-test engine lib,cycle 於彼可 unify)跨檔復用 fake,再提**共享 pub test-util adapter**(engine 側 `pub` TransportFactory adapter,非 `#[cfg(test)]`),使 fake 於整合測試亦 drop-in;W3 `--lib` 端到端已足,不提前做。
...
- **S4 收口(2026-07-16)**:交付 fake-TWS dev-crate(engine-independent,見 §4 as-built 偏差)+ engine `ibkr_tws_driver`(端到端讀迴圈:permit→connect→握手→Ready→心跳單一出口→故障→重連;INV-1 production 恆停 Disconnected、granting provider 僅測試域)+ F4(`send_framed` by-value 消費 `OutboundGrant`,出站編譯期必經 governor)+ F3(queued-heartbeat 簿記閉環 `resolve_pacing`,避免重複心跳/漂移)+ IBKR-CI-3 兩守衛(structure dev-dep-only + nm 缺席審計,皆正控實證有牙,掛 `rust-ibkr-tests`)。E2/E3 APPROVE/PASS_WITH_NOTES;E2-F1 補丁:serve loop 改「靜默 poll tick(Idle)非斷線」——liveness 靠心跳 miss 而非讀逾時(silent server → Degraded → HeartbeatDropped),配 `SilentDrop` 場景 + advanceable 注入時鐘整合覆蓋(`observed_degraded` 斷言真經 Degraded)。殘項:**W-CI 缺席審計的 CI-inline 正控**(仿 g4 範式對 fake-linked lib-test bin 跑一腿自證 pattern 有牙)未內嵌 ci.yml step,本地已實證(exit 1),留 E4 一腿實跑存證;及 §4 W4+「共享 pub test-util adapter」follow-up。
