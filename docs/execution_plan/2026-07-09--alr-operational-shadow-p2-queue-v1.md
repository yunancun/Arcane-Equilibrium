...
| P2-8 | ACTIVE_NATURAL_CYCLE_SOAK | PM -> E3 -> BB -> QA -> PM | Linux shadow soak consumes at least three real new scanner cycles, survives restart, suppresses duplicates, makes a target decision, and keeps authority counters at zero. Engine notifier remains dormant because flags make engine restart unsafe; use only natural scanner cycles plus an ALR-only fresh-gated restart/reconciliation. |
