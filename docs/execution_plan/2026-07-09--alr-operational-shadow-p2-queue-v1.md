...
| P2-3 | ACTIVE_E4_RETRY_SELECT_ONLY_FIX | PM -> E3 -> BB -> PM -> PA -> E1 -> E2 -> E4 -> QA -> PM | The first disposable listener reached real persistence but found repository `FOR SHARE` incompatible with SELECT/INSERT-only `alr_shadow`. Source removes both unneeded row locks and pins that constraint with a regression test; rerun Linux isolated LISTEN/append-only/duplicate-lock evidence, then QA, before a fresh exact prestart E3/BB recheck for any role/secret, engine restart, or service start. |
