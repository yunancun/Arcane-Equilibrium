...
| P2-3 | ACTIVE_FRESH_E3_BB_GATE | PM -> E3 -> BB -> PM -> PA -> E1 -> E2 -> E4 -> QA -> PM | Before any user-level service start, obtain a fresh exact-scope gate for event/backlog/novelty consumption with backpressure, resource budget, one-instance lock, graceful shutdown, fail-closed recovery, no cron/fixed scheduler, and verified service DB identity/privileges. |
