...
| P2-2 | READY_FOR_PREAPPLY_ALIGNMENT | PM -> E3 -> BB -> PM -> PA -> E1 -> E2 -> E4 -> QA -> PM | Source implementation, two-run regression, isolated Docker migration double-apply, append-only privilege probe, and real repository round trip pass. Commit/push then prove Mac/GitHub/Linux head alignment and a clean Linux checkout before V151-only existing-PG dry-run/apply and verification. |
