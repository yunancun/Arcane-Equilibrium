...
| P2-4 | ACTIVE | PM -> QC -> MIT -> AI-E -> PA -> E1 -> E2 -> E4 -> QA -> PM | LearningTarget -> PIT dataset -> existing training/statistical pipeline -> after-cost evaluation -> challenger artifact with no serving/promotion. Start from the durable P2-3 backlog; no engine notifier activation is required for this slice. |
...
| P2-8 | WAITING_SAFE_ENGINE_NOTIFIER_ACTIVATION_AND_P2-4..7 | PM -> E3 -> BB -> QA -> PM | Linux shadow soak consumes at least three real new scanner cycles, survives restart, suppresses duplicates, makes a target decision, and keeps authority counters at zero. Current engine flags make notifier activation unsafe under no-order scope. |
