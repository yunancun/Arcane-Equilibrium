...
- **M1 `GET /api/v1/paper/status`**(governance.js:93)= **真 drift(HIGH)**:後端無此路由(有 /paper/session/status·/metrics·/positions·/pnl);try/catch→404→paperState={}→govPostReconcile 靜默 POST 空 paper_state=**dead call**。**處置=待修**(P1.0-5-M1:核 govPostReconcile 需要的 paper_state 契約→repoint GUI 到正確路由;修好即刪此 allowlist 列)。暫 allowlist 使 5b 綠。
