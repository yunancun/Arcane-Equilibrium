...
5. **Bull data is allowed but must be labeled**: the operator does not reject bull-market data. The requirement is explicit provenance: every metric, chart, and verdict must say when its base sample is bull-data-heavy, rally-regime-only, or stale-year-dominated, so agents cannot silently treat bull data as general-market proof.
