...
| **Sprint 1A-γ 補完** | V103 EXTEND 完整 ALTER DDL（下游 sub-agent）+ ADR-0042（per leakage spec §6.2 建議）+ Linux PG dry-run | 8-12 hr | DDL + governance |
...
### §12.3 ADR-0042 建議（per leakage spec §6.2）

本 spec 對應 governance authority = ADR-0042（per leakage spec §6.2 建議）。M4 Hypothesis Discovery 全 scope（含 6 attribute minimum bar + leakage protocol + Decision Lease writeback + Cowork hybrid path）由 ADR-0042 統一落 governance。本 spec 不寫 ADR 本檔；待 PA Sprint 1A-γ dispatch 統一收口。
...
- ADR-0042（待建立）：M4 Hypothesis Discovery 統一 governance authority
...
| **PA 主會話** | **PENDING** | V103 EXTEND 完整 ALTER DDL（下游 sub-agent 補）+ ADR-0042 起草 + Sprint 1A-γ dispatch | 待主會話收口 |
