...
> **檔案大小政策投影（2026-07-22）**：下列 dated checkpoint 的實測行數與拆分結果仍是歷史事實；文中「現行 2000 行門檻」描述的是今天仍可重用的 guard，不代表當年的拆分由後來的政策觸發。舊制數字只保留在明確標示的歷史實測值中。
...
- 拆分後模組行數為 `57`、`63`、`482`、`629`、`364`，全部低於現行 2000 行
...
- 更新 `test_stock_etf_python_no_write_static_guard.py`：
  ...
  - 將 `tab-stock-etf.js` 與新 fallback 模組的 file-size cap 統一為現行 `<= 2000`。
...
- 更新 `test_stock_etf_python_no_write_static_guard.py`：
  ...
  - 將 `tab-stock-etf.js` file-size cap 對齊現行 `<= 2000`。
  - 將 `tab-stock-etf-data-policy.js` file-size cap 對齊現行 `<= 2000`。
- 本 checkpoint 不改 runtime behavior、不改 Bybit path、不改 IBKR boundary，只做
...
- 更新 `test_stock_etf_python_no_write_static_guard.py`：
  ...
  - 將 `tab-stock-etf.js` file-size cap 對齊現行 `<= 2000`。
  - 將 `tab-stock-etf-auth-account.js` file-size cap 對齊現行 `<= 2000`。
...
- 更新 `test_stock_etf_python_no_write_static_guard.py`：
  ...
  - 將 `tab-stock-etf.js` file-size cap 對齊現行 `<= 2000`。
  - 將 `tab-stock-etf-evidence-paper.js` file-size cap 對齊現行 `<= 2000`。
...
- 更新 `test_stock_etf_python_no_write_static_guard.py`：
  ...
  - 將 `tab-stock-etf.js` file-size cap 對齊現行 `<= 2000`。
  - 將 `tab-stock-etf-scorecard-launch.js` file-size cap 對齊現行 `<= 2000`。
...
- 更新 `test_stock_etf_python_no_write_static_guard.py`：
  ...
  - 將 `tab-stock-etf.js` 與 `tab-stock-etf-readiness.js` 的 file-size cap
    統一對齊現行 `<= 2000`。
...
- `stock_etf_phase3_evidence.rs` 從 982 行降到 742 行，低於現行 2000 行
...
- `status_summaries.rs` 保留 thin wrapper，對外 `scorecard_status_summary` import
  surface 不變。
- 父檔由 1006 行降至 785 行，低於現行 2000 行門檻；新子模組 228 行。
...
- 原 1022 行 guard 降至 152 行；共享 helper 522 行；新增 route/GUI guard 分別
  147/263 行，Stock/ETF guard files 皆低於現行 2000 行門檻。
...
本 checkpoint 將 Rust `stock_etf_scorecard_inputs.rs` 從實測 800 行的檔案拆成父
...
- 原父檔由實測 800 行降至 128 行；新 components/bundle 模組分別為 520/181 行（此處 800 是歷史實測值，不是現行門檻）。
...
IPC layer 的單檔審查風險，並把 split structure guard 對齊現行唯一 2000 行 cap。
...
- Rust IPC handler/test split static guards 的舊制較低 cap 已退役；現行 file cap 統一為 2000，並鎖住
...
- Guard 鎖住 `ibkr_phase2_policies.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_lane_scoped_ipc.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_lane.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `ibkr_phase2_gate.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `ibkr_phase2_runtime.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `ibkr_phase2_artifact.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `ibkr_feature_flag_secret_auth.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `ibkr_non_bybit_api_allowlist.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_broker_capability_registry.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_risk_policy.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 parent module 與 validation module 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `ibkr_paper_lifecycle.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_paper_fill_import_request.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_paper_shadow_reconciliation.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_shadow_signal_request.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 parent、components、bundle 三檔各自不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_scorecard_derivation.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_scorecard_verdict.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_tiny_live_eligibility.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_release_packet.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_phase0_manifest.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_audit_events.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_db_evidence_ddl.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_disable_cleanup_runbook.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_gui_lane_contract.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_ibkr_readonly_probe_request.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_ibkr_readonly_probe_result_import_request.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_instrument_identity.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_pit_universe.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_reference_data_sources.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_strategy_hypothesis.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 parent 與 market-data child 均不超過現行 2000 行 governance cap。
...
- Guard 鎖住 `stock_etf_paper_order_request/fixtures.rs` 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 scorecard summary child module 不超過現行 2000 行 governance cap。
...
- Guard 鎖住 template 不超過現行 2000 行 governance cap。
...
- Guard 要求 validation 子模組不超過現行 2000 行 governance cap，並維持現有 helper/function surface。
...
## 205. 2026-07-01 PM session source checkpoint：IBKR External Surface Gate Default Lineage Exact Guard

本 checkpoint 補強 `ibkr_phase2_gate` external surface gate 的 default 與 wrong-surface aggregate
fail-closed lineage，固定 default `IbkrExternalSurfaceGateV1`、wrong identity、retroactive call-side-effect、
wrong API baseline、wrong host/port policy cases 的 ordered blocker vectors，並用 source-static parser 鎖住
external surface gate validator blocker emit order。這不是 Rust production behavior change、不是 IPC/API route
change、不是 IBKR contact、不是 connector runtime、不是 socket/client construction、不是 secret lookup、
不是 paper order route enablement、不是 tiny-live/live gate；只把 Phase2 external surface source-only gate 的
fail-closed lineage 變成 exact-blocker acceptance test 與 source-static guard。

已完成：

- 在 `ibkr_phase2_gate_acceptance.rs` 將 default `IbkrExternalSurfaceGateV1` 固定為完整順序 blocker 向量，
  覆蓋 contract/source、blocked status、live-port denial proof、secret-contract/live-secret proof、allowlist、
  redaction/rate-limit/audit/paper-attestation/python guard prerequisites。
- 在同檔將 wrong identity、retroactive call-side-effect、wrong API baseline、wrong host/port policy cases
  固定為 exact blocker vectors。
- 在 `test_ibkr_phase2_gate_source_static.py` 新增 external surface gate validator blocker ordering parser。

驗證：

- Targeted rustfmt check：PASS。
- IBKR Phase2 gate source static pytest：`7 passed`。
...
## 209. 2026-07-01 PM session source checkpoint：Stock/ETF Scorecard Derivation Exact Blocker Guard

本 checkpoint 補強 `StockEtfScorecardDerivationV1` scorecard derivation acceptance 的 aggregate fail-closed
lineage，固定 default/template derivation artifacts、ID/hash-lineage aggregate drift、runtime side-effect aggregate
failures、atomic/replay/separation/Bybit/writer cross-wire cases 的 ordered blocker vectors 或 exact single-blocker
verdicts。這不是 Rust production behavior change、不是 IPC/API route change、不是 IBKR contact、不是 connector
runtime、不是 socket/client construction、不是 secret lookup、不是 paper order route enablement、不是
reconciliation/scorecard writer 啟動、不是 shadow-fill generation、不是 tiny-live/live gate；只把 scorecard
derivation source-only artifact 的 fail-closed lineage 變成 exact-blocker acceptance guard。

已完成：

- 在 `stock_etf_scorecard_derivation_acceptance.rs` 將 default 與 template `StockEtfScorecardDerivationV1` 固定為
  完整順序 blocker 向量，覆蓋 contract/source、StockEtfCash/IBKR lane、environment、ID lineage、hash lineage、
  atomic/replay/separation、Bybit protection 與 sealed posture。
- 在同檔將 ID/hash-lineage aggregate drift、runtime side-effect aggregate failures、writer/runtime aggregate
  failures 固定為 exact blocker vectors。
- 移除 scorecard derivation blocker 的 loose `blockers.contains` helper checks；atomic/replay/separation/Bybit/
  writer cross-wire cases 改為 single-blocker 或 complete-vector assertions。

驗證：

- Targeted rustfmt check：PASS。
- Stock/ETF scorecard derivation source static pytest：`7 passed`。
- Stock/ETF scorecard derivation Rust acceptance：`11 passed`。
- Full `cargo test -p openclaw_types`：PASS。
- `cargo fmt -p openclaw_types -- --check`：PASS。
- Dynamic docs trace pytest：PASS；主計畫與 Operator summary 保持 checkpoint title coverage。
- Diff check：PASS。
...
## 214. 2026-07-01 PM session source checkpoint：Stock/ETF Paper Order Request Exact Blocker Guard

本 checkpoint 補強 `StockEtfPaperOrderRequestEnvelopeV1` paper-order request envelope 的 aggregate fail-closed
lineage，固定 default envelope、method/authority/effect mismatches、effect lifecycle hash gaps、preview pollution、
submit order-intent failures、market-order price/TIF mismatch、cancel submit-shape pollution、replace
replacement-shape gaps、boundary regressions 的 ordered blocker vectors 或 exact single-blocker vectors。這不是
...
cancel/replace execution、不是 lifecycle writer、不是 fill import execution、不是 tiny-live/live gate；只把
Stock/ETF paper-order request source-only contract 的 fail-closed lineage 變成 exact-blocker acceptance guard。

已完成：

- 在 `stock_etf_paper_order_request_acceptance.rs` 將 default `StockEtfPaperOrderRequestEnvelopeV1` 固定為完整
  順序 blocker 向量，覆蓋 contract/source identity、lane/broker/env、request/account identity 與 unsupported
  method。
- 在同檔將 method/authority/effect mismatch、effect lifecycle hash gaps、preview pollution、submit
  order-intent failures、market-order price/TIF mismatch、cancel submit-shape pollution、replace
  replacement-shape gaps、boundary regressions 固定為 exact blocker vectors。
- 移除 paper-order request blocker 的 loose `has()` / `blockers.contains` helper；aggregate cases 改為完整
  ordered-vector assertions。
- 在 `test_stock_etf_paper_order_request_validation_source_static.py` 新增 validator blocker emit-order guard，
...
## 217. 2026-07-01 PM session source checkpoint：Stock/ETF Shadow Signal Request Exact Blocker Guard

本 checkpoint 補強 `StockEtfShadowSignalRequestV1` future shadow-signal evaluation request contract 的 aggregate
fail-closed lineage，固定 default request、method/operation/authority/effect cross-wire、lineage/hash aggregate
failures、no-side-effect boundary regressions 的 ordered blocker vectors 或 exact single-blocker vectors。這不是
Rust production behavior change、不是 IPC/API route change、不是 IBKR contact、不是 connector runtime、不是
socket/client construction、不是 secret lookup、不是 broker session、不是 paper order route enablement、不是 shadow
signal emission、不是 shadow fill generation、不是 scorecard writer、不是 DB/evidence writer、不是 tiny-live/live
gate；只把 Stock/ETF shadow-signal request source-only contract 的 fail-closed lineage 變成 exact-blocker
acceptance guard。

已完成：

- 在 `stock_etf_shadow_signal_request_acceptance.rs` 將 default `StockEtfShadowSignalRequestV1` 固定為完整
  順序 blocker 向量，覆蓋 contract/source identity、lane/broker/shadow environment、IPC method、operation、
  authority scope、request/evaluation/signal ids 與 evidence-clock/PIT universe/strategy hypothesis/instrument
  identity/market-data provenance/cost model/asset-lane events/source hashes。
- 在同檔將 method/operation/authority/effect cross-wire、lineage/hash aggregate、no-side-effect boundary
  regressions 固定為 exact blocker vectors。
- 移除 shadow-signal request blocker 的 loose `has()` / `blockers.contains` helper；aggregate cases 改為完整
  ordered-vector assertions。
- 在 `test_stock_etf_shadow_signal_request_source_static.py` 新增 validator blocker emit-order guard，pin
  top-level、required fields 與 boundary flags 的 source order。
...
## 219. 2026-07-01 PM session source checkpoint：Stock/ETF Readonly Probe Result Import Exact Blocker Guard

本 checkpoint 補強 `StockEtfIbkrReadonlyProbeResultImportRequestV1` sanitized read-only probe result-import request
contract 的 aggregate fail-closed lineage，固定 default request、read-action/operation cross-wire、common
lineage/hash/timestamp aggregate failures、kind-specific downstream lineage、duplicate/stale replay、no-side-effect
boundary regressions 的 ordered blocker vectors 或 exact single-blocker vectors。這不是 Rust production behavior
change、不是 IPC/API route change、不是 IBKR contact、不是 connector runtime、不是 socket/client construction、
不是 secret lookup、不是 broker session、不是 read-only probe execution、不是 result import execution、不是
paper order route enablement、不是 evidence/scorecard/DB writer、不是 tiny-live/live gate；只把 Stock/ETF IBKR
read-only probe result-import request source-only contract 的 fail-closed lineage 變成 exact-blocker acceptance
guard。

已完成：

- 在 `stock_etf_ibkr_readonly_probe_result_import_request_acceptance.rs` 將 default
  `StockEtfIbkrReadonlyProbeResultImportRequestV1` 固定為完整順序 blocker 向量，覆蓋 contract/source identity、
  lane/broker/environment、probe action、operation、authority scope、API read allowlist、result-import/request/
  probe ids、readonly probe request、session attestation、API allowlist、redaction/audit policies、payload/raw/
  redacted/source hashes、timestamps、idempotency 與 health snapshot hash。
- 在同檔將 read-action/operation cross-wire、common lineage/hash/timestamp aggregate、kind-specific downstream
  lineage、duplicate/stale replay、no-side-effect boundary regressions 固定為 exact blocker vectors。
- 保留 paper-order API action 與 missing import timestamp 的天然 aggregate 行為：paper action 必須同時命中
  ...
  `ImportRequestedAtMissing` 與 `ResultAsOfAfterImportRequested`。
- 移除 result-import request blocker 的 loose `has()` / `blockers.contains` helper；aggregate cases 改為完整
  ordered-vector assertions。
- 在 `test_stock_etf_ibkr_readonly_probe_result_import_request_source_static.py` 新增 validator blocker
  emit-order guard，pin top-level、required lineage、kind-specific lineage 與 boundary flags 的 source order。
...
## 228. 2026-07-01 PM session source checkpoint：Stock/ETF Readiness Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI readiness route 的 exact contract-violation lineage，固定 aggregate
side-effect、missing API allowlist、boolean `source_version` normalization 的 ordered violation vectors。這不是
FastAPI route behavior change、不是 connector production code change、不是 GUI/Rust IPC behavior change、不是
IBKR contact、不是 connector runtime、不是 secret lookup、不是 broker session、不是 read-only probe execution、
不是 paper order route enablement、不是 DB/evidence/scorecard writer、不是 tiny-live/live gate；只把 readiness
route source-only tests 從 membership/set checks 收緊成 exact-vector guard。

已完成：

- 在 `test_stock_etf_readiness_routes.py` 新增 readiness aggregate、missing API allowlist、boolean
  API allowlist version 的完整 ordered `contract_violations` expected vectors。
- 將 readiness route 的 loose `set(data["contract_violations"])` 與
  `in data["contract_violations"]` 檢查改為 exact ordered list assertions。
- 新增本檔內 source guard，防止 readiness route contract-violation assertions 退回 loose set/membership/subset
  檢查。

驗證：

- Readiness route focused pytest：`7 passed`。
- Stock/ETF no-write/surface/readiness focused pytest：`17 passed`。
- Full Stock/ETF Python route/static pytest：`128 passed`。
...
## 229. 2026-07-01 PM session source checkpoint：Stock/ETF Lane Status Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI lane-status route 的 exact contract-violation lineage，固定 lane-level
side-effect 與 asset/broker mismatch 的 ordered violation vector。這不是 FastAPI route behavior change、不是
connector production code change、不是 GUI/Rust IPC behavior change、不是 IBKR contact、不是 connector runtime、
不是 secret lookup、不是 broker session、不是 read-only probe execution、不是 paper order route enablement、不是
DB/evidence/scorecard writer、不是 tiny-live/live gate；只把 lane-status route source-only test 從 set comparison
收緊成 exact-vector guard。

已完成：

- 在 `test_stock_etf_lane_status_routes.py` 新增完整 ordered
  `EXPECTED_LANE_STATUS_CONTRACT_VIOLATIONS`。
- 將 lane-status route 的 loose `set(data["contract_violations"])` 檢查改為 exact ordered list assertion。
- 新增本檔內 source guard，防止 lane-status route contract-violation assertions 退回 loose set/membership/subset
  檢查。

驗證：

- Lane-status route focused pytest：`5 passed`。
- Stock/ETF no-write/surface/lane/readiness focused pytest：`22 passed`。
- Full Stock/ETF Python route/static pytest：`129 passed`。
- `python3 -m py_compile` for changed lane-status route test：PASS。
...
## 230. 2026-07-01 PM session source checkpoint：Stock/ETF Phase0 Status Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI Phase0 status route 的 exact contract-violation lineage，固定 runtime/status、
required-contract、API baseline、global-denial regression case 的 ordered violation vector。這不是 FastAPI route
...
route enablement、不是 DB/evidence/scorecard writer、不是 tiny-live/live gate；只把 Phase0 status route
source-only test 從 membership checks 收緊成 exact-vector guard。

已完成：

- 在 `test_stock_etf_phase0_status_routes.py` 新增完整 ordered
  `EXPECTED_PHASE0_CONTRACT_VIOLATIONS`。
- 將 Phase0 route 的 loose `in data["contract_violations"]` 檢查改為 exact ordered list assertion。
- 新增本檔內 source guard，防止 Phase0 route contract-violation assertions 退回 loose set/membership/subset
  檢查。

驗證：

- Phase0 status route focused pytest：`5 passed`。
- Stock/ETF no-write/surface/Phase0 focused pytest：`15 passed`。
- Full Stock/ETF Python route/static pytest：`130 passed`。
- `python3 -m py_compile` for changed Phase0 status route test：PASS。
...
## 231. 2026-07-01 PM session source checkpoint：Stock/ETF Data Foundation Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI data-foundation route 的 exact contract-violation lineage，固定 top-level
side-effect、phase/data collection flags、lane/environment drift、instrument identity drift、reference-data source
drift 的 ordered violation vector。這不是 FastAPI route behavior change、不是 connector production code change、
不是 GUI/Rust IPC behavior change、不是 IBKR contact、不是 connector runtime、不是 secret lookup、不是 broker
session、不是 read-only probe execution、不是 paper order route enablement、不是 DB/evidence/scorecard writer、
不是 tiny-live/live gate；只把 data-foundation route source-only test 從 subset check 收緊成 exact-vector guard。

已完成：

- 在 `test_stock_etf_data_foundation_status_routes.py` 新增完整 ordered
  `EXPECTED_DATA_FOUNDATION_CONTRACT_VIOLATIONS`。
- 將 data-foundation route 的 loose `issubset(set(data["contract_violations"]))` 檢查改為 exact ordered list
  assertion。
- 新增本檔內 source guard，防止 data-foundation route contract-violation assertions 退回 loose
  set/membership/subset 檢查。

驗證：

- Data-foundation route focused pytest：`6 passed`。
- Stock/ETF no-write/surface/data-foundation focused pytest：`16 passed`。
- Full Stock/ETF Python route/static pytest：`131 passed`。
- `python3 -m py_compile` for changed data-foundation route test：PASS。
...
## 232. 2026-07-01 PM session source checkpoint：Stock/ETF Authorization Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI authorization route 的 exact contract-violation lineage，固定 top-level
side-effect、runtime/rehearsal/order authority flags、lane/environment drift、authorization matrix drift、
secret-slot drift、Phase2 artifact drift、session attestation drift、authorization envelope drift 的 ordered
violation vector。這不是 FastAPI route behavior change、不是 connector production code change、不是 GUI/Rust IPC
behavior change、不是 IBKR contact、不是 connector runtime、不是 secret lookup、不是 broker session、不是
read-only probe execution、不是 paper order route enablement、不是 DB/evidence/scorecard writer、不是 tiny-live/live
gate；只把 authorization route source-only test 從 subset check 收緊成 exact-vector guard。

已完成：

- 在 `test_stock_etf_authorization_status_routes.py` 新增完整 ordered
  `EXPECTED_AUTHORIZATION_CONTRACT_VIOLATIONS`。
- 將 authorization route 的 loose `issubset(set(data["contract_violations"]))` 檢查改為 exact ordered list
  assertion。
- 新增本檔內 source guard，防止 authorization route contract-violation assertions 退回 loose set/membership/subset
  檢查。

驗證：

- Authorization route focused pytest：`6 passed`。
- Stock/ETF no-write/surface/authorization focused pytest：`16 passed`。
- Full Stock/ETF Python route/static pytest：`132 passed`。
- `python3 -m py_compile` for changed authorization route test：PASS。
- Authorization no-loose contract violation assertion scan：PASS。
- Diff check：PASS。
...
## 233. 2026-07-01 PM session source checkpoint：Stock/ETF Universe Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI universe route 的 exact contract-violation lineage，固定 top-level
side-effect、lane/environment drift、Phase3 collection flags、PIT universe contract drift 的 ordered violation
vector。這不是 FastAPI route behavior change、不是 connector production code change、不是 GUI/Rust IPC behavior
change、不是 IBKR contact、不是 connector runtime、不是 secret lookup、不是 broker session、不是 read-only probe
execution、不是 paper order route enablement、不是 DB/evidence/scorecard writer、不是 tiny-live/live gate；只把
universe route source-only test 從 subset check 收緊成 exact-vector guard。

已完成：

- 在 `test_stock_etf_universe_status_routes.py` 新增完整 ordered
  `EXPECTED_UNIVERSE_CONTRACT_VIOLATIONS`。
- 將 universe route 的 loose `issubset(set(data["contract_violations"]))` 檢查改為 exact ordered list assertion。
- 新增本檔內 source guard，防止 universe route contract-violation assertions 退回 loose set/membership/subset
  檢查。

驗證：

- Universe route focused pytest：`6 passed`。
- Stock/ETF no-write/surface/universe focused pytest：`16 passed`。
- Full Stock/ETF Python route/static pytest：`133 passed`。
- `python3 -m py_compile` for changed universe route test：PASS。
- Universe no-loose contract violation assertion scan：PASS。
- Diff check：PASS。
...
## 234. 2026-07-01 PM session source checkpoint：Stock/ETF Policy Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI policy route 的 exact contract-violation lineage，固定 top-level side-effect、
phase/runtime/order flags、lane/environment drift、risk-policy drift、broker capability registry lineage drift、denial
boundary drift 的 ordered violation vector。這不是 FastAPI route behavior change、不是 connector production code
change、不是 GUI/Rust IPC behavior change、不是 IBKR contact、不是 connector runtime、不是 secret lookup、不是
broker session、不是 read-only probe execution、不是 paper order route enablement、不是 DB/evidence/scorecard
writer、不是 tiny-live/live gate；只把 policy route source-only test 從 subset check 收緊成 exact-vector guard。

已完成：

- 在 `test_stock_etf_policy_status_routes.py` 新增完整 ordered
  `EXPECTED_POLICY_CONTRACT_VIOLATIONS`。
- 將 policy route 的 loose `issubset(set(data["contract_violations"]))` 檢查改為 exact ordered list assertion。
- 新增本檔內 source guard，防止 policy route contract-violation assertions 退回 loose set/membership/subset
  檢查。

驗證：

- Policy route focused pytest：`6 passed`。
- Stock/ETF no-write/surface/policy focused pytest：`16 passed`。
- Full Stock/ETF Python route/static pytest：`134 passed`。
- `python3 -m py_compile` for changed policy route test：PASS。
- Policy no-loose contract violation assertion scan：PASS。
- Diff check：PASS。
...
## 235. 2026-07-01 PM session source checkpoint：Stock/ETF Account Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI account route 的 exact contract-violation lineage，固定 top-level
side-effect、DB apply drift、lane/environment drift、account snapshot drift、session attestation drift、paper-attestation
policy drift 的 ordered violation vector。這不是 FastAPI route behavior change、不是 connector production code
change、不是 GUI/Rust IPC behavior change、不是 IBKR contact、不是 connector runtime、不是 secret lookup、不是
broker session、不是 read-only probe execution、不是 paper order route enablement、不是 DB/evidence/scorecard
writer、不是 tiny-live/live gate；只把 account route source-only test 從 subset check 收緊成 exact-vector guard。

已完成：

- 在 `test_stock_etf_account_status_routes.py` 新增完整 ordered
  `EXPECTED_ACCOUNT_CONTRACT_VIOLATIONS`。
- 將 account route 的 loose `issubset(set(data["contract_violations"]))` 檢查改為 exact ordered list assertion。
- 新增本檔內 source guard，防止 account route contract-violation assertions 退回 loose set/membership/subset
  檢查。

驗證：

- Account route focused pytest：`6 passed`。
- Stock/ETF no-write/surface/account focused pytest：`16 passed`。
- Full Stock/ETF Python route/static pytest：`135 passed`。
- `python3 -m py_compile` for changed account route test：PASS。
- Account no-loose contract violation assertion scan：PASS。
- Diff check：PASS。
...
## 236. 2026-07-01 PM session source checkpoint：Stock/ETF Shadow Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI shadow route 的 exact contract-violation lineage，固定 top-level side-effect、
lane/environment drift、Phase3 shadow collection flags、shadow-fill model drift、strategy hypothesis drift 的 ordered
violation vector。這不是 FastAPI route behavior change、不是 connector production code change、不是 GUI/Rust IPC
behavior change、不是 IBKR contact、不是 connector runtime、不是 secret lookup、不是 broker session、不是
read-only probe execution、不是 paper order route enablement、不是 DB/evidence/scorecard writer、不是 tiny-live/live
gate；只把 shadow route source-only test 從 subset check 收緊成 exact-vector guard。

已完成：

- 在 `test_stock_etf_shadow_status_routes.py` 新增完整 ordered
  `EXPECTED_SHADOW_CONTRACT_VIOLATIONS`。
- 將 shadow route 的 loose `issubset(set(data["contract_violations"]))` 檢查改為 exact ordered list assertion。
- 新增本檔內 source guard，防止 shadow route contract-violation assertions 退回 loose set/membership/subset
  檢查。

驗證：

- Shadow route focused pytest：`6 passed`。
- Stock/ETF no-write/surface/shadow focused pytest：`16 passed`。
- Full Stock/ETF Python route/static pytest：`136 passed`。
- `python3 -m py_compile` for changed shadow route test：PASS。
...
## 237. 2026-07-01 PM session source checkpoint：Stock/ETF Reconciliation Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI reconciliation route 的 exact contract-violation lineage，固定 top-level
side-effect、lane/environment drift、reconciliation readiness flags、lifecycle/event-log/shadow/reconciliation
contract drift、matching artifact drift 的 ordered violation vector。這不是 FastAPI route behavior change、不是
connector production code change、不是 GUI/Rust IPC behavior change、不是 IBKR contact、不是 connector runtime、
不是 secret lookup、不是 broker session、不是 read-only probe execution、不是 paper order route enablement、不是
DB/evidence/scorecard writer、不是 tiny-live/live gate；只把 reconciliation route source-only test 從 subset check
收緊成 exact-vector guard。

已完成：

- 在 `test_stock_etf_reconciliation_status_routes.py` 新增完整 ordered
  `EXPECTED_RECONCILIATION_CONTRACT_VIOLATIONS`。
- 將 reconciliation route 的 loose `issubset(set(data["contract_violations"]))` 檢查改為 exact ordered list
  assertion。
- 新增本檔內 source guard，防止 reconciliation route contract-violation assertions 退回 loose
  set/membership/subset 檢查。

驗證：

- Reconciliation route focused pytest：`6 passed`。
- Stock/ETF no-write/surface/reconciliation focused pytest：`16 passed`。
- Full Stock/ETF Python route/static pytest：`137 passed`。
- `python3 -m py_compile` for changed reconciliation route test：PASS。
...
## 238. 2026-07-01 PM session source checkpoint：Stock/ETF Disable Cleanup Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI disable-cleanup route 的 exact contract-violation lineage，固定 top-level
side-effect、disable/cleanup request flags、launch-denial flags、runbook identity/posture drift、runbook side-effect
drift、env-flag evidence drift、proof evidence/authority drift 的 ordered violation vector。這不是 FastAPI route
behavior change、不是 connector production code change、不是 GUI/Rust IPC behavior change、不是 IBKR contact、
不是 connector runtime、不是 secret lookup、不是 broker session、不是 read-only probe execution、不是 paper order
route enablement、不是 DB/evidence/scorecard writer、不是 tiny-live/live gate；只把 disable-cleanup route
source-only test 從 subset check 收緊成 exact-vector guard。

已完成：

- 在 `test_stock_etf_disable_cleanup_status_routes.py` 新增完整 ordered
  `EXPECTED_DISABLE_CLEANUP_CONTRACT_VIOLATIONS`。
- 將 disable-cleanup route 的 loose `issubset(set(data["contract_violations"]))` 檢查改為 exact ordered list
  assertion。
- 新增本檔內 source guard，防止 disable-cleanup route contract-violation assertions 退回 loose
  set/membership/subset 檢查。

驗證：

- Disable-cleanup route focused pytest：`5 passed`。
- Stock/ETF no-write/surface/disable-cleanup focused pytest：`15 passed`。
- Full Stock/ETF Python route/static pytest：`138 passed`。
- `python3 -m py_compile` for changed disable-cleanup route test：PASS。
...
## 239. 2026-07-01 PM session source checkpoint：Stock/ETF Evidence Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI evidence route 的 exact contract-violation lineage，固定 top-level
side-effect、lane/environment drift、Phase3 evidence flags、market-data provenance drift、collector-run drift、
DQ-manifest drift、evidence-clock linkage/side-effect drift、frozen-input scorecard regeneration drift、scorecard
writer drift 的 ordered violation vector。這不是 FastAPI route behavior change、不是 connector production code
change、不是 GUI/Rust IPC behavior change、不是 IBKR contact、不是 connector runtime、不是 secret lookup、不是
broker session、不是 read-only probe execution、不是 paper order route enablement、不是 DB/evidence/scorecard
writer、不是 tiny-live/live gate；只把 evidence route source-only test 從 subset check 收緊成 exact-vector guard。

已完成：

- 在 `test_stock_etf_evidence_status_routes.py` 新增完整 ordered
  `EXPECTED_EVIDENCE_CONTRACT_VIOLATIONS`。
- 將 evidence route 的 loose `issubset(set(data["contract_violations"]))` 檢查改為 exact ordered list
  assertion。
- 新增本檔內 source guard，防止 evidence route contract-violation assertions 退回 loose
  set/membership/subset 檢查。

驗證：

- Evidence route focused pytest：`5 passed`。
- Stock/ETF no-write/surface/evidence focused pytest：`15 passed`。
- Full Stock/ETF Python route/static pytest：`139 passed`。
- `python3 -m py_compile` for changed evidence route test：PASS。
- Evidence no-loose contract violation assertion scan：PASS。
- Diff check：PASS。
...
## 240. 2026-07-01 PM session source checkpoint：Stock/ETF Paper Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI paper route 的 exact contract-violation lineage，固定 top-level
side-effect、DB apply drift、lane/environment drift、Phase2/paper lifecycle flags、lifecycle/event-log/request
contract drift、lifecycle gate leakage、lifecycle identity/hash/redaction lineage、request-contract leakage、
reconstructability readiness/manual-review drift 的 ordered violation vectors。這不是 FastAPI route behavior
change、不是 connector production code change、不是 GUI/Rust IPC behavior change、不是 IBKR contact、不是
connector runtime、不是 secret lookup、不是 broker session、不是 read-only probe execution、不是 paper order
route enablement、不是 lifecycle writer、不是 fill import、不是 DB/evidence/scorecard writer、不是 tiny-live/live
gate；只把 paper route source-only tests 從 subset/membership checks 收緊成 exact-vector guards。

已完成：

- 在 `test_stock_etf_paper_status_routes.py` 新增完整 ordered
  `EXPECTED_PAPER_CONTRACT_VIOLATIONS`。
- 新增 stale lifecycle shape 專用的完整 ordered
  `EXPECTED_STALE_PAPER_CONTRACT_VIOLATIONS`。
- 將 paper route 的 loose `issubset(set(data["contract_violations"]))` 與 membership assertions 改為 exact
  ordered list assertions。
- 新增本檔內 source guard，防止 paper route contract-violation assertions 退回 loose set/membership/subset
  檢查。

驗證：

- Paper route focused pytest：`7 passed`。
- Stock/ETF no-write/surface/paper focused pytest：`17 passed`。
- Full Stock/ETF Python route/static pytest：`140 passed`。
- `python3 -m py_compile` for changed paper route test：PASS。
...
IPC 行為、不呼叫 IBKR、不導入 IBKR SDK、不讀/建/序列化 secret、不啟動 connector runtime、不做 socket/client
construction、不做 broker session、不執行 read-only probe、不執行 fill import、不做 paper order routing/cancel/
replace、不做 lifecycle writer、不做 DB/evidence/scorecard writer、不啟動 evidence clock、不做 release launch、不做
Linux runtime sync/restart、不授權 paper-shadow launch、tiny-live/live 或任何 Bybit behavior change。

## 241. 2026-07-01 PM session source checkpoint：Stock/ETF Scorecard Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI scorecard route 的 exact contract-violation lineage，固定 top-level
side-effect、live/tiny-live drift、phase/writer/evidence-window drift、lane/environment drift、scorecard input
bundle drift、derivation drift、scorecard verdict hash/review/label drift、nonzero scorecard metric leakage 的
ordered violation vector。這不是 FastAPI route behavior change、不是 connector production code change、不是
GUI/Rust IPC behavior change、不是 IBKR contact、不是 connector runtime、不是 secret lookup、不是 broker
session、不是 read-only probe execution、不是 broker fill import、不是 paper order route enablement、不是
reconciliation writer、不是 scorecard writer、不是 DB/evidence writer、不是 tiny-live/live gate；只把 scorecard
route source-only test 從 subset check 收緊成 exact-vector guard。

已完成：

- 在 `test_stock_etf_scorecard_status_routes.py` 新增完整 ordered
  `EXPECTED_SCORECARD_CONTRACT_VIOLATIONS`。
- 將 scorecard route 的 loose `issubset(set(data["contract_violations"]))` 檢查改為 exact ordered list
  assertion。
- 新增本檔內 source guard，防止 scorecard route contract-violation assertions 退回 loose
  set/membership/subset 檢查。
...
- `python3 -m py_compile` for changed scorecard route test：PASS。
- Scorecard no-loose contract violation assertion scan：PASS。
- Diff check：PASS。

PM 邊界不變：此 checkpoint 不改 FastAPI route 行為、不改 connector production code、不改 GUI runtime、不改 Rust
IPC 行為、不呼叫 IBKR、不導入 IBKR SDK、不讀/建/序列化 secret、不啟動 connector runtime、不做 socket/client
construction、不做 broker session、不執行 read-only probe、不執行 broker fill import、不做 paper order routing/
...
## 243. 2026-07-01 PM session source checkpoint：Stock/ETF Launch Route Exact Contract-Violation Guard

本 checkpoint 補強 Stock/ETF FastAPI launch route 的 exact contract-violation lineage，固定 top-level
side-effect、launch-authority drift、lane/environment drift、release/runbook/tiny-live identity drift、
accepted-before-audit drift、release readiness drift、runbook side-effect drift、tiny-live evidence/review/metric
drift、tiny-live decision drift 的 ordered violation vector。這不是 FastAPI route behavior change、不是 connector
production code change、不是 GUI/Rust IPC behavior change、不是 IBKR contact、不是 connector runtime、不是
secret lookup、不是 broker session、不是 read-only probe execution、不是 paper order route enablement、不是
release launch、不是 DB/evidence writer、不是 scorecard writer、不是 tiny-live/live gate；只把 launch route
source-only test 從 subset check 收緊成 exact-vector guard。

已完成：

- 在 `test_stock_etf_launch_status_routes.py` 新增完整 ordered
  `EXPECTED_LAUNCH_CONTRACT_VIOLATIONS`。
- 將 launch route 的 loose `issubset(set(data["contract_violations"]))` 檢查改為 exact ordered list
  assertion。
- 新增本檔內 source guard，防止 launch route contract-violation assertions 退回 loose
  set/membership/subset 檢查。

驗證：

- Launch route focused pytest：`6 passed`。
- Stock/ETF no-write/surface/launch focused pytest：`16 passed`。
- Full Stock/ETF Python route/static pytest：`143 passed`。
- `python3 -m py_compile` for changed launch route test：PASS。
...
## 244. 2026-07-01 PM session source checkpoint：Stock/ETF IBKR Connector Config Exact Blocker Guard

本 checkpoint 補強 inert IBKR connector skeleton 的 config blocker lineage，固定
`IbkrReadOnlyEndpointConfig.validate_source_boundary()` risky runtime/live/secret/Bybit-path blocker 的 ordered
vector。這不是 connector production code change、不是 FastAPI route behavior change、不是 GUI/Rust IPC behavior
change、不是 IBKR contact、不是 connector runtime、不是 secret lookup、不是 broker session、不是 read-only probe
execution、不是 paper order route enablement、不是 release launch、不是 DB/evidence writer、不是 scorecard writer、
不是 tiny-live/live gate；只把 connector skeleton source-only test 從 set/subset check 收緊成 exact-vector guard。

已完成：

- 將 `test_stock_etf_ibkr_connector_skeleton.py` 的 risky config blocker 檢查改為 exact ordered
  `RISKY_CONFIG_BLOCKERS` assertion。
- 新增本檔內 source guard，防止 connector config blocker assertions 退回 loose `issubset(blockers)` 或
...
## 245. 2026-07-01 PM session source checkpoint：Stock/ETF Rust IPC Status Exact Blocker Guard

本 checkpoint 補強 Stock/ETF Rust IPC status fixture 的 exact blocker lineage，固定 Phase0 manifest、Phase2
pre-contact readiness、data foundation、policy、authorization、evidence、universe、shadow、paper、account、
reconciliation、scorecard 等 source-fixture blocker vectors。這不是 Rust IPC behavior change、不是 FastAPI route
...
Rust IPC fixture tests 從 membership check 收緊成 exact ordered vector guard。

已完成：

- 將 `stock_etf.rs` parent IPC fixture tests 的 Phase0 contract manifest 與 status blockers 改為
  `assert_json_array_eq` exact ordered assertions。
- 將 `precontact_fixtures.rs`、`foundation_status_fixtures.rs`、`status_fixtures.rs` 的 remaining loose
  `json_array_contains` checks 改為完整 ordered blocker vectors。
- 新增 Rust source guard，跨 parent/submodule fixture 檔防止 `json_array_contains(...)` 與 loose
  `.iter().any(|item| item.as_str() == Some(...))` assertions 回流。

驗證：

- `rustfmt --edition 2021 --check` on changed Rust fixture files：PASS。
...
- 將 `status_fixtures.rs` 收斂成 account、reconciliation、scorecard fixture tests；所有 Rust Stock/ETF
  IPC fixture files 目前均低於現行 2000 行門檻。
...
- Rust fixture line-count guard：`stock_etf.rs` 127 行、`core_status_fixtures.rs` 759 行、
  `status_fixtures.rs` 571 行、`phase5_status_fixtures.rs` 308 行；全部低於現行 2000 行門檻。
...
- 維持 test 檔尺寸 hygiene：legacy acceptance 檔 746 行，新 IO matrix acceptance 檔 348 行，均低於現行 2000 行門檻。
...
## 250. 2026-07-02 PM session source checkpoint：Stock/ETF Broker Capability Gate Matrix Exact Guard

本 checkpoint 補強 Stock/ETF broker capability registry 的 operation gate matrix exact coverage，固定 15 個
accepted broker operations 的 `required_gates` 必須等於完整 ordered vectors，而不是只檢查局部 membership。
這不是 Rust production behavior change、不是 broker capability validator semantics change、不是 Rust IPC handler
behavior change、不是 FastAPI route behavior change、不是 GUI behavior change、不是 connector production code
change、不是 IBKR contact、不是 connector runtime、不是 secret lookup、不是 broker session、不是 read-only probe
execution、不是 paper order/fill route enablement；只把 broker capability acceptance coverage 從局部 gate
membership checks 收緊成獨立 exact-matrix guard。

已完成：

- 新增 `stock_etf_broker_capability_registry_gate_matrix_acceptance.rs`，逐一固定 15 個 broker capability
  operations 的完整 ordered gate vectors、authority scope、denial reason、Rust ownership、audit/source hash flags。
- 移除 `stock_etf_broker_capability_registry_acceptance.rs` 內 accepted registry 與 paper-fill-import row 的正向
  `required_gates.contains(...)` membership assertions，改由新 matrix acceptance test 驗證完整 vectors。
- 新增 source guard，防止 broker capability legacy acceptance 檔回流 `.required_gates.contains(...)`。
- 維持 test 檔尺寸 hygiene：legacy acceptance 檔 607 行，新 gate matrix acceptance 檔 237 行，均低於現行 2000 行門檻。
...
PM 邊界不變：此 checkpoint 不改 Rust production code、不改 broker capability validator semantics、不改 Rust IPC
handler behavior、不改 FastAPI route 行為、不改 GUI runtime、不改 connector production code、不呼叫 IBKR、不導入
IBKR SDK、不讀/建/序列化 secret、不啟動 connector runtime、不做 socket/client construction、不做 broker
...
- 維持 test 檔尺寸 hygiene：`stock_etf_db_evidence_ddl_acceptance.rs` 目前為 387 行，低於現行 2000 行門檻。
...
- 維持 test 檔尺寸 hygiene：`stock_etf_risk_policy_acceptance.rs` 目前為 393 行，低於現行 2000 行門檻。
...
## 253. 2026-07-02 PM session source checkpoint：Stock/ETF Lane Readiness Denial Reasons Exact Guard

本 checkpoint 補強 Stock/ETF lane default readiness fail-closed coverage，固定 default feature flags 下
`readiness.denial_reasons` 必須等於完整 ordered vector：`LaneDisabled`、`BrokerDisabled`、`ShadowOnly`，而不是
只檢查局部 membership。這不是 Rust production behavior change、不是 lane/readiness validator semantics
...
lookup、不是 broker session、不是 read-only probe execution、不是 paper order/fill route enablement；只把 lane
acceptance coverage 從局部 readiness denial membership check 收緊成 exact-vector guard。

已完成：

- 將 `stock_etf_lane_acceptance.rs` 的 default readiness `denial_reasons` 改為完整 ordered vector assertion。
- 新增 source guard，防止 default readiness denial assertions 回流 `.denial_reasons.contains(...)`。
- 維持 test 檔尺寸 hygiene：`stock_etf_lane_acceptance.rs` 目前為 666 行，低於現行 2000 行門檻。
...
PM 邊界不變：此 checkpoint 不改 Rust production code、不改 lane/readiness validator semantics、不改 source config/
runtime config、不改 Rust IPC handler behavior、不改 FastAPI route 行為、不改 GUI runtime、不改 connector
production code、不呼叫 IBKR、不導入 IBKR SDK、不讀/建/序列化 secret、不啟動 connector runtime、不做
socket/client construction、不做 broker session、不執行 read-only probe、不做 paper order routing/cancel/replace、
不做 fill import execution、不做 release launch、不做 DB/evidence writer、不做 scorecard writer、不啟動 evidence
...
## 254. 2026-07-02 PM session source checkpoint：IBKR Non-Bybit API Allowlist Bucket Exact Guard

本 checkpoint 補強 `ibkr_phase2_gate_acceptance.rs` 內嵌 `NonBybitApiAllowlistV1` accepted fixture 的 action bucket
coverage，固定 `read_actions`、`paper_write_actions` 與 `denied_actions` 必須等於完整 ordered vectors，而不是只
用三個 bucket 的 `.len()` 加總去對齊 `required_non_bybit_api_actions().len()`。這不是 Rust production behavior
change、不是 non-Bybit API allowlist validator semantics change、不是 external-surface gate PASS、不是
IBKR connector/runtime、不是 secret lookup、不是 broker session、不是 read-only probe execution、不是 paper
order/fill route enablement；只把 Phase2 gate acceptance 內的 IBKR non-Bybit API allowlist bucket coverage 從
aggregate count 收緊成 exact bucket guard。

已完成：

- 將 embedded accepted fixture `read_actions` 改為完整 10-item ordered vector assertion。
- 將 embedded accepted fixture `paper_write_actions` 改為完整 3-item ordered vector assertion。
- 將 embedded accepted fixture `denied_actions` 改為完整 10-item ordered vector assertion。
- 新增 source guard，防止 embedded accepted action bucket assertions 回流 `allowlist.read_actions.len()`、
  `allowlist.paper_write_actions.len()`、`allowlist.denied_actions.len()` 或
  `required_non_bybit_api_actions().len()` aggregate coverage。
- 維持 test 檔尺寸 hygiene：`ibkr_phase2_gate_acceptance.rs` 目前為 763 行，低於現行 2000 行門檻。
...
PM 邊界不變：此 checkpoint 不改 Rust production code、不改 non-Bybit API allowlist validator semantics、不改
external-surface/session gate semantics、不改 Rust IPC handler behavior、不改 FastAPI route 行為、不改 GUI runtime、
不改 connector production code、不呼叫 IBKR、不導入 IBKR SDK、不讀/建/序列化 secret、不啟動 connector runtime、
不做 socket/client construction、不做 broker session、不執行 read-only probe、不做 paper order routing/cancel/
...
- 維持 test 檔尺寸 hygiene：parent `stock_etf.rs` 128 行、`core_status_fixtures.rs` 759 行、
  `phase5_status_fixtures.rs` 401 行，均低於現行 2000 行門檻。
...
- 將原 `stock_etf_phase3_evidence_acceptance.rs` 收斂為 collector run、evidence-clock day 與全 Phase3
  template guard。
- 維持 test 檔尺寸 hygiene：原檔 640 行，market-data 檔 265 行，DQ 檔 173 行，均低於現行 2000 行門檻。
...
- Line-count guard：三個拆分檔均不超過現行 2000 行門檻。
...
## 257. 2026-07-02 PM session source checkpoint：Stock/ETF Paper Order Request Acceptance Split Guard

本 checkpoint 將 oversized `stock_etf_paper_order_request_acceptance.rs` 拆成 paper-order request 契約邊界更清楚
的兩個 integration test 檔：default/accepted fixture、aggregate cross-wire、method-specific shape 與 template
coverage 保留在原檔，逐欄位 gap matrix coverage 移到
`stock_etf_paper_order_request_gap_acceptance.rs`。這不是 Rust production behavior change、不是 paper-order
request validator semantics change、不是 Rust IPC handler behavior change、不是 FastAPI route behavior change、
不是 GUI behavior change、不是 connector production code change、不是 IBKR contact、不是 connector runtime、
不是 secret lookup、不是 broker session、不是 read-only probe execution、不是 paper order routing/cancel/replace
enablement；只降低 paper-order request acceptance test review blast radius 並保持原 17 個測試語義。

已完成：

- 新增 `stock_etf_paper_order_request_gap_acceptance.rs`，承接 common surface、method authority/effect、
  preview/order-intent、submit lifecycle、cancel/replace 與 boundary flag independent gap coverage。
- 將原 `stock_etf_paper_order_request_acceptance.rs` 收斂為 default/accepted fixture、aggregate method
  cross-wire、method-specific shape、boundary regression 與 blocked template coverage。
- 維持 test 檔尺寸 hygiene：原檔 378 行，gap 檔 506 行，均低於現行 2000 行門檻。
- PM 未派 PA/E1/E2/E4/QA subagent：本 checkpoint 為窄範圍純測試檔拆分，沒有 production/runtime/exchange-facing
  行為，也未改 validator semantics；以 PM 本地精準驗證與全 package 測試收斂。
...
- Changed-file rustfmt：PASS。
- Paper-order focused Rust acceptance：原檔 `11 passed`、gap 檔 `6 passed`。
- Full `cargo test -p openclaw_types`：PASS。
- Test count preservation scan：拆分後仍為 17 個 paper-order request tests。
- Line-count guard：兩個拆分檔均不超過現行 2000 行；Stock/ETF/IBKR Rust acceptance 檔目前無超過 2000 行的主要 oversized
  項。

PM 邊界不變：此 checkpoint 不改 Rust production code、不改 paper-order request validator semantics、不改
source config/runtime config、不改 Rust IPC handler behavior、不改 FastAPI route 行為、不改 GUI runtime、不改
connector production code、不呼叫 IBKR、不導入 IBKR SDK、不讀/建/序列化 secret、不啟動 connector runtime、不做
socket/client construction、不做 broker session、不執行 read-only probe、不做 paper order routing/cancel/replace、
不做 fill import execution、不做 release launch、不做 DB/evidence writer、不做 scorecard writer、不啟動 evidence
clock、不做 Linux runtime sync/restart、不做 destructive DB cleanup、不授權 paper-shadow launch、tiny-live/live 或
...
## 258. 2026-07-02 PM session source checkpoint：Stock/ETF IBKR Connector Action Matrix Preview Guard

本 checkpoint 將 Rust `non_bybit_api_allowlist_v1` 的 read / paper-write / denied action bucket
以 source-only、不可執行的方式顯示在 inert Python IBKR connector skeleton。這不是 IBKR connector runtime、
不是 FastAPI wiring、不是 read-only probe execution、不是 paper-order submit/cancel/replace enablement；只是讓
Python skeleton 自身也能被審計到完整 action matrix 與 side-effect false posture。

已完成：

- 新增 `IbkrApiActionMatrixPreview`，固定 `non_bybit_api_allowlist_v1` contract id、source version、10 個
  read actions、3 個 paper-write actions、10 個 denied actions，以及 `external_surface_gate_accepted=false`、
  `broker_write_authority=false`、`paper_write_actions_authorized=false`、`ibkr_contact_performed=false`、
  `network_contact_performed=false`、`secret_content_loaded=false`、`secret_content_serialized=false`、
  `bybit_path_reused=false`、`live_or_tiny_live_authorized=false`。
- 新增 `IbkrReadOnlyClient.api_action_matrix_preview()` 與 `blocked_api_action_matrix_fixture()`；payload 只回傳
  blocked dict/list/count，不提供 execute/import/apply/order capability。
- Connector package `__all__`、README source-only boundary、fixture export、client public surface freeze、payload
  shape guard、default/risky config blocker guard 全部同步。
- `test_stock_etf_ibkr_connector_skeleton.py` 保持 791 行，低於現行 2000 行門檻。
...
PM 邊界不變：此 checkpoint 不改 FastAPI route 行為、不改 GUI runtime、不改 Rust IPC handler behavior、不改 Rust
production code、不呼叫 IBKR、不導入 IBKR SDK、不讀/建/序列化 secret、不啟動 connector runtime、不做
socket/client construction、不做 broker session、不執行 read-only probe、不做 paper order routing/cancel/replace、
不做 fill import execution、不做 release launch、不做 DB/evidence writer、不做 scorecard writer、不啟動 evidence
clock、不做 Linux runtime sync/restart、不做 destructive DB cleanup、不授權 paper-shadow launch、tiny-live/live 或
...
## 259. 2026-07-02 PM session hygiene checkpoint：Stock/ETF IBKR Connector Action Matrix Test Split Guard

本 checkpoint 將剛新增的 action-matrix exact coverage 從
`test_stock_etf_ibkr_connector_skeleton.py` 拆到 focused
`test_stock_etf_ibkr_connector_action_matrix.py`。這不是 connector behavior change、不是 FastAPI wiring、不是
IBKR contact、不是 runtime/paper-order enablement；只降低 connector skeleton test review blast radius。

已完成：

- 新增 `test_stock_etf_ibkr_connector_action_matrix.py`，承接 `non_bybit_api_allowlist_v1` contract id/source
  version、read/paper-write/denied ordered buckets、fixture parity、risky config blocker expansion 與 dataclass
  inert posture coverage。
- 原 `test_stock_etf_ibkr_connector_skeleton.py` 保留 package export、README boundary、Bybit import separation、
  client public surface、readiness/session/result-import/paper preview payload shape 與 config blocker coverage。
- Line-count hygiene：skeleton test 由 791 行降到 701 行；action-matrix test 為 168 行。
...
PM 邊界不變：此 checkpoint 不改 connector production code、不改 FastAPI route 行為、不改 GUI runtime、不改 Rust
IPC handler behavior、不改 Rust production code、不呼叫 IBKR、不導入 IBKR SDK、不讀/建/序列化 secret、不啟動
connector runtime、不做 socket/client construction、不做 broker session、不執行 read-only probe、不做 paper order
...
## 262. 2026-07-02 PM session test checkpoint：Stock/ETF API Allowlist Route Surface Parity Guard

本 checkpoint 將 API allowlist bucket parity 從 connector/FastAPI fixture 延伸到所有 accepted
Stock/ETF status route surface。這不是 IBKR connector runtime、不是 FastAPI behavior change、不是
IBKR contact、不是 read-only probe execution、不是 paper-order enablement；只是保證 route normalization
不會丟失 `non_bybit_api_allowlist_v1` 的完整 action buckets。

已完成：

- 擴展 `test_stock_etf_api_allowlist_cross_surface_parity.py`。
- 參數化覆蓋 14 個 display-only status endpoints：lane、data foundation、policy、authorization、evidence、
  account、universe、shadow、paper、reconciliation、scorecard、launch、release-packet、disable-cleanup。
- 每個 endpoint 使用 accepted source fixture + mocked IPC，並要求 normalized `api_allowlist` 的
  `read_actions`、`paper_write_actions`、`denied_actions` 與 counts 完全匹配 shared constants。
...
## 263. 2026-07-02 PM session test checkpoint：Stock/ETF API Allowlist Fail-Closed Route Surface Parity Guard

本 checkpoint 將 API allowlist route-surface parity 補到 IPC unavailable / fail-closed path。
這不是 IBKR connector runtime、不是 FastAPI behavior change、不是 IBKR contact、不是 read-only
probe execution、不是 paper-order enablement；只是保證 degraded status response 也保留穩定的
`api_allowlist` bucket schema。

已完成：

- 擴展 `test_stock_etf_api_allowlist_cross_surface_parity.py`。
- 參數化覆蓋 14 個 display-only status endpoints 的 `client_fail_closed` response。
- 每個 endpoint 在 IPC unavailable 時必須輸出 empty `read_actions`、`paper_write_actions`、
  `denied_actions`、zero counts、`contract_id=""`、`source_version=0`、`accepted=false` 與
...
- Full Stock/ETF FastAPI/static pytest：`179 passed`。
- `git diff --check`：PASS。
- Dynamic docs trace pytest：PASS；主計畫與 Operator summary checkpoint title coverage 保持同步。
...
## 264. 2026-07-02 PM session test checkpoint：Stock/ETF Session Attestation Safety Shape Parity Guard

本 checkpoint 將 `ibkr_session_attestation_v1` 的 Python display-only safety shape
對齊 Rust Phase2 session attestation default lineage，並把 inert connector preview、
FastAPI account status surface、FastAPI authorization status surface 與 fail-closed route
path 鎖成同一組安全欄位。這不是 IBKR connector runtime、不是 IBKR contact、不是
secret lookup、不是 broker session、不是 read-only probe execution、不是 paper-order
enablement；只是避免 Phase2 前置 session/topology/secret-lineage 欄位在各 display surface
各自漂移。

已完成：

- `IbkrSessionAttestationPreview` 補齊 host/port、process identity、gateway mode、
  secret-slot mode、world-readable secret、live-secret absence、env credential fallback、
  attested/expires window 等 fail-closed preview 欄位，所有值仍為空、0、`unknown` 或
  `false`。
- Authorization status normalizer 的 session attestation fallback 與 normalizer 補齊同一組欄位。
- Authorization contract-violation guard 新增 session host/port/process/gateway/secret-slot/
  live-secret/env-fallback/API/data-tier/entitlement/startup/window/raw-artifact claim checks，
  防止 Phase2 gate 前任何 session identity 或 secret lineage claim 被信任。
- 新增 `test_stock_etf_session_attestation_cross_surface_parity.py`，要求 connector preview、
  connector fixture、account-status route、authorization-status route 以及 IPC-unavailable
  fail-closed path 都輸出相同 safety baseline。

驗證：

- Python changed files `py_compile`：PASS。
- Session parity + authorization + connector skeleton focused pytest：`20 passed`。
- Full Stock/ETF FastAPI/static pytest：`181 passed`。
- `git diff --check`：PASS。
- Dynamic docs trace pytest：`23 passed`。

PM 邊界不變：此 checkpoint 不改 Rust production code、不改 Rust IPC handler behavior、不改 GUI runtime、不呼叫
IBKR、不導入 IBKR SDK、不讀/建/序列化 secret、不啟動 connector runtime、不做 socket/client construction、不做
broker session、不執行 read-only probe、不做 paper order routing/cancel/replace、不做 fill import execution、不做
release launch、不做 DB/evidence writer、不做 scorecard writer、不啟動 evidence clock、不做 Linux runtime
sync/restart、不做 destructive DB cleanup、不授權 paper-shadow launch、tiny-live/live 或任何 Bybit behavior
change。
