...
- 維持 test 檔尺寸 hygiene：legacy acceptance 檔 607 行，新 gate matrix acceptance 檔 237 行，均低於 800 行。
...
- 維持 test 檔尺寸 hygiene：`stock_etf_lane_acceptance.rs` 目前為 666 行，低於 800 行。
...
- 維持 test 檔尺寸 hygiene：`ibkr_phase2_gate_acceptance.rs` 目前為 763 行，低於 800 行。
...
- 維持 test 檔尺寸 hygiene：原檔 378 行，gap 檔 506 行，均低於 800 行。
- PM 未派 PA/E1/E2/E4/QA subagent：本 checkpoint 為窄範圍純測試檔拆分，沒有 production/runtime/exchange-facing
  行為，也未改 validator semantics；以 PM 本地精準驗證與全 package 測試收斂。
...
- Line-count guard：兩個拆分檔均低於 800 行；Stock/ETF/IBKR Rust acceptance 檔目前無 800+ 行主要 oversized
  項。
...
- `test_stock_etf_ibkr_connector_skeleton.py` 保持 791 行，低於 800 行 review-attention threshold。
