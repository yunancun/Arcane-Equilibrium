...
- `deploy_V017.sh` and `deploy_V018.sh` build a `postgresql://user:password@...` DSN at line `35` and pass it to `psql "$DSN"` at `deploy_V017.sh:57`, `:75`, and `:79`.
- `restart_all.sh` launches the engine/API with `OPENCLAW_DATABASE_URL=postgresql://...:${pg_pass}@...` and `OPENCLAW_IPC_SECRET=...` in the process environment at `restart_all.sh:243-248` and `:275-279`.
