...
|------|------|------|
| FAIL | 6 | ✅ 全部修復 |
| WARN | 7 | 記錄（regime details JSONB、liquidation PK dedup、oi_value=0.0、Vec capacity、RwLock） |
| PASS | 5 | 10 表覆蓋、DDL match、REST 簽名正確、client Arc 共享 |

G2 FAIL 修復明細：
- F-1：liquidations qty/price NOT NULL → `sanitize_f64_or_zero()`
- F-2：fallback total_lines 改為跨文件累加器 `total_written`
- F-3：quality_writer phantom type → 改用 `Arc<AtomicU64>` last_tick_ms
- F-4：`spawn_rest_pollers()` 在 main.rs 中調用
- F-5：`run_quality_monitor()` 在 main.rs 中 spawn + last_tick_ms 傳遞
- F-6：FallbackWriter 接入 market_writer（3 次失敗切換 + 自動恢復）
