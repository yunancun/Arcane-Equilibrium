...
| `MAE-004` | P0 | `DONE` | All 22 custom agents are fixed to `high`, while direct spawns may inherit root `max`. | Derive every execution surface from the operator role tier: Codex T1=`Sol/high`, T2=`Sol/low`, T3=`Terra/medium`; Claude direct/saved surfaces exact-match `opus/high`, `opus/low`, `sonnet/medium`. | Native and saved-workflow identities resolve the same exact Registry role tier and reject caller or session inheritance. |
