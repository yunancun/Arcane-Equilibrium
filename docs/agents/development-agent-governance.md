...
Status: active Interface, 2026-07-11
...
direct interfaces、previous failure 與 verdict-relevant `claim_inputs` 正規化成
...
scope、acceptance、hard stops、baseline、direct interfaces、previous failure 與可選
`evidence_state` 供 Context Interface 使用；verdict-relevant prior/evidence 另由
`claim_inputs` 以 canonical digest 固定。`side_effect_class` 必須明示為 `none`、repo/test/
...
- runtime claim 或 operational change/deploy → `OPS preflight`；deploy 再經
  PM/operator exact intent → Deploy Adapter contract → `OPS postcheck`；目前 apply 因 trusted
  runtime probe 缺失而 fail closed。
...
exact native identity、node/task digest 與 path scope，只接受 `--` 後 argv 並以 `shell=false`
...
`DONE + FAIL` 合法：review 已完成，被審物失敗。`BLOCKED + PASS` 非法。
...
unverified scope、skipped role、consumption、next owner/action。
...
Linux read-only probe；拒 git/PG/service/filesystem mutation、private broker effect、
...
- `deploy_adapter_v1` contract：只接受 PM/operator 批准的 exact-SHA
  ...
  `effect_adapter_result_v1` receipt。**目前只有 intent validation 可執行**；repository 尚無
  能獨立重現 endpoint class、mainnet flag、mode、authorization scope 與 process identity 的
  trusted local probe，因此 intent-only 回 `INTENT_VALIDATED_APPLY_DISABLED`，即使提供
  `--apply`/env/attestation 也在呼叫 `build_then_restart_atomic.sh` 前 fail closed。未來若補齊
  probe，仍須 pre/post attestation 同 intent、safe environment 與 running binary identity，
  exit 0 + 唯一 verified marker 才能形成 closure-admissible receipt。
...
- Deploy apply 在 trusted runtime probe 缺失時不可執行；broker/external effect route 不可 PASS。
