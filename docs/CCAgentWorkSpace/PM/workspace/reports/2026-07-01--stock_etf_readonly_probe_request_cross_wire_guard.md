...
日期：2026-07-01
角色：PM
範圍：IBKR Stock/ETF paper/shadow source hygiene；readonly probe request mapping hardening

## 結論

已補強 `stock_etf_ibkr_readonly_probe_request` 對 probe kind / API action /
BrokerOperation mapping 的 coverage。這次只改 acceptance 與 source-static guard，不改 Rust
production code、IPC method、runtime、IBKR connector、secret、DB/evidence writer 或 paper order
route。

## 變更

- Rust acceptance 新增 `readonly_probe_request_rejects_probe_action_operation_cross_wire`。
- 證明 `MarketDataSnapshot` 搭配 `AccountSummarySnapshotRead` action 會被
  `ProbeActionMismatch` 擋下，且不誤報 operation 或 allowlist blocker。
- 證明 `MarketDataSnapshot` 搭配 `AccountSnapshotRead` operation 會被
  `OperationMismatch` 擋下，且不誤報 action 或 allowlist blocker。
- 證明 request envelope 混入 `PaperOrderSubmit` action 時，必須產生
  `ProbeActionMismatch` 與 `ApiActionNotReadAllowed`，且不被誤當作已提交 paper order。
...
- `python3 -B -m pytest -q tests/structure/test_stock_etf_ibkr_readonly_probe_request_source_static.py --tb=short`：`8 passed`。
- `cargo test -p openclaw_types --test stock_etf_ibkr_readonly_probe_request_acceptance`：`7 passed`。
