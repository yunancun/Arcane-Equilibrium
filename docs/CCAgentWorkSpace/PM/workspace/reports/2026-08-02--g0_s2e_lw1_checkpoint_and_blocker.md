...
checkpoint；剩餘阻塞是 Linux 固定 trust/attestor roots 與獨立 external WORM/provider/
...
10. `e68966670`：current-head Codex review 再抓到 1 個 P1：producer 與 dependencies 在 Git
    clean/head 檢查前已從 mutable checkout import，同 UID 程序可載入污染 bytes 後還原檔案，令
    不同程式碼產出的 claims 被簽成乾淨 `HEAD`。修復後 unprivileged producer 不再採集或組裝
    source/host/process/clock claims，也不能提交 signing payload；唯一 kernel exec point 只以
    零輸入呼叫 root-owned `attest-v2` capability，由 capability 從 immutable source view 自主
    產出完整 SSHSIG artifact，producer 只做 bounded parse、可信時鐘 freshness 與中央驗簽。
...
- observation：`2026-08-02T14:51:53Z`，evidence class 固定為
...
- 10 個固定 public trust roots 加一個 fixed attestor capability：全部 absent。
...
- source checkpoint：`e689666707be5fabb3434ca77ba11a9958449941`
- blocked prerequisites：14/14
- packet digest：`sha256:c13142b4be6b607176ea464e3c7780a8a6db74ae12d524496d0ed35e174d5809`
...
- `e68966670` attestor-owned delta：host-capture/action-packet `48 passed`；host kernel
  `211 passed, 9 skipped`；S2E/recovery adjacent set `416 passed, 9 skipped`。攻擊回歸釘住
  zero-input attestor invocation、producer 無 source/fact/signing-payload surface、duplicate/non-object
  JSON 與 signed identity drift 均 fail closed。
...
提出四項 finding，final provider delta review 再提出一項 P2，current-head review 又提出
mutable-import/signing P1；六項均已於 source checkpoint `e68966670` 閉合。publication 仍要求
current-head classified checks。

## 唯一後續序列

1. Operator 在帶外安全通道 provision fixed trust roots；不得把 private key/credential 寫入 packet。
2. provision root-owned fixed host-capture `attest-v2` capability；它必須從 immutable source
   view 自主採集完整 artifact，拒絕任何 caller-authored signing payload。
