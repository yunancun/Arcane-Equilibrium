# PM Checkpoint - IBKR Stock/ETF Rust IPC Test Split Guard

Date: 2026-06-30
Status: DONE_WITH_BOUNDARY - source-only Rust IPC test split guard

## Result

- Split tail Stock/ETF Rust IPC status fixture tests into
  `rust/openclaw_engine/src/ipc_server/tests/stock_etf/status_fixtures.rs`.
- Reduced parent `stock_etf.rs` from `2532` lines to `1852` lines; the child
  fixture module is `685` lines.
- Added `tests/structure/test_stock_etf_ipc_tests_split_static.py` so the parent
  and child Stock/ETF IPC test files stay below the 2000-line governance cap.
- The new guard also checks the moved status fixture methods remain present and
  blocks IBKR SDK / socket / HTTP client tokens in the moved fixture module.
...
- `python3 -m pytest -q tests/structure/test_stock_etf_ipc_tests_split_static.py`: `2 passed`
