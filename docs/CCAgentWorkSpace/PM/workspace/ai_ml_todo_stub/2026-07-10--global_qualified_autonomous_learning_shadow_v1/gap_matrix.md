...
| G2 Candidate selection | `PARTIAL_SOURCE_ACCEPTED` | B2.2b source acceptance `a7d8d5f8b` completes candidate-board v2/arbiter input cost provenance: count-weighted global mean and signed-cost bound are strict, malformed/rehashed artifacts fail closed, thin fallback is conservative, and publisher -> board -> adapter -> arbiter stays canonical/hash/causality bound. Final focused `4 passed`, final integrated `586 passed`, E2/QC/QA PASS, P0/P1 `0/0`. | Event-driven primary handoff, deployment, and natural-cycle runtime proof remain missing. | P0; blocks terminal | B2.2c restart-safe event-driven primary handoff with cron reconciliation only; later runtime requires fresh exact E3/BB. |
