...
WP4 handshake design reviewed head: `2189c996101da680a8ac9ec426d28c1028b3557d`
WP4 trusted-issuer / isolated-runner handshake source checkpoint: `c900d1ecb2eac495994b8715f09ad10bee6d9583`
WP4 V160-style atomic-consumption design reviewed head: `9a41c8d2abf34dbdce01fde010a500b4c19ba4f4`
WP4 trusted Ed25519 verifier source-preauthoring reviewed head: `75f3db2b55cd1d9737d83c811d291abecb67ad49`
WP4 trusted Ed25519 verifier source checkpoint: `0b9038c78a9e1a5256895901aa22376d645adbd4`, hosted run `29213814965` (#1101), `8/8 SUCCESS`
...
|---|---|---|---|---|---|
| G1 Collection | `PARTIAL` | Fresh raw/ALR identity and lag `0` remained healthy through WP1. Source `c900d1ecb` validates handshake identities; design `9a41c8d2a` freezes atomic acquisition; source `0b9038c78` now provides the isolated strict verifier with local `5+28`, scanner `10/10+10/10`, E2/E3/E4 `0/0/0`, and hosted CI `8/8`. | Candidate decision, order/fill, actual cost/funding, risk-context runtime evidence, real issuer/runner bytes, and a platform-attested production verifier remain absent. | P0; blocks terminal | Advance to the accepted V160 source coordinator; any external acquisition remains exact effect/runtime gated. |
| G2 Candidate selection | `PARTIAL_DISPOSABLE_PG_VERIFIED` | The WP2/WP3 lineage and WP4 V158/V159 chain are preserved. Run `29195892105` proved disposable V159 schema behavior; `0b9038c78` proves the source-only strict verifier and hosted offline gate. | Linux runtime proof, current candidate qualification, production V159 apply, production-attested verifier, V160-style request consumption, real durable row/readback, trusted issuer receipt, and real execution remain absent. | P0; blocks terminal | Author the fixed V160 coordinator under fresh collision/effect authorization, then run disposable PG; source PASS is not runtime readiness. |
| G3 Outcome chain | `FAIL_DISPOSABLE_PG_VERIFIED_NO_REAL_RECEIPT_OR_OUTCOME_CHAIN` | Last accepted WP1 runtime snapshot had 638 feedback rows, all DEFER; ProofPacket-present `0`; Reward total `0`; complete PIT->fill->cost->proof->reward->label chain `0`. Source verifier `0b9038c78` is `SOURCE_ONLY_UNATTESTED`; durable consumption and real evidence remain absent. Production proof/reward/model rows are `0/0/0`. | Production verifier attestation, V160 source/apply, one real durable receipt/readback, trusted issuer receipt, production caller, and one current candidate-matched controlled Rust evidence chain. | P0; blocks terminal | Build and verify V160, then acquire evidence only from a qualified candidate. With none, collect/ROTATE; never force training. |
| G4 Actual training | `FAIL_DISPOSABLE_PG_VERIFIED_EXECUTION_NOT_ESTABLISHED` | Last accepted WP1 runtime snapshot had 638 DEFER-only runs and `model_training_performed=0`. Handshake source remains synthetic-only and verifier `0b9038c78` is source-only. `model_training_performed=false`; no fit, model bytes/artifact, or registry row exists; production proof/reward/model rows are `0/0/0`. | Production-attested verifier, isolated runner, pre-fit claim, authenticated terminal receipt, atomic consumption, a qualified real fit, immutable model trio readback, production apply, and runtime registry rows. | P0; blocks terminal | Complete V160/disposable PG/production readiness first. Do not execute or claim a fit without a qualified candidate; source PASS is not training. |
...
| G6 Decisions | `FAIL` | Runtime distribution is DEFER only; V152/V153 constrain allowed run/feedback states. Disposable V159, pure handshake source, accepted coordinator design, and source-only verifier `0b9038c78` create no real execution or decision. | Durable DEFER/ROTATE/TRAIN/REJECT/CHALLENGER_ACCEPT/ROLLBACK/STOP reasons and hashes. | P0; blocks terminal | Complete V160 and separately gate attestation, runner, and any later trainer before WP5. `CHALLENGER_ACCEPT` remains no-authority. |
