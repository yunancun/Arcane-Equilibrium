...
|---|---|---|---|---|---|
| G1 Collection | `PARTIAL` | Fresh raw/ALR identity and lag `0` remained healthy through WP1. Runtime checkout and service pin now equal exact target `7d1c24794`; one session stayed stable with restart `0` and scanner INSERT denied. | Candidate decision, order/fill, actual cost/funding, and risk-context runtime evidence remain absent. | P0; blocks terminal | WP4 forward-schema/trainer gate, then later controlled acquisition. Any external acquisition remains exact E3/BB/Operator gated. |
| G2 Candidate selection | `PARTIAL_SOURCE_ACCEPTED` | The immutable handoff/replay checkpoint `328125a08` is preserved. Repair `03ef761b` keeps missing-policy READY decisions durable under exact handoff causality. Event checkpoint `1b85318f` replaces five-second candidate polling with bounded PG/inotify wakes. WP3 repository checkpoint `c2bdefbf` independently reconstructs the newest selected projection and exact bounded lineage, then rechecks head/lineage/bridge identities in one snapshot before returning any receipt. Full ML `1818 passed/36 skipped`; E2/QA/CC-FA P0/P1/P2 `0/0/0`. | The real Linux inotify test was skipped on Darwin. Deployment, service restart/recovery, natural-cycle proof, and candidate qualification remain absent. | P0; blocks terminal | Continue WP4 forward schema/isolated trainer under exact gates. Later B2.2c Linux/runtime proof requires fresh exact E3/BB. |
| G3 Outcome chain | `FAIL_SOURCE_REPOSITORY_ADAPTER_ACCEPTED` | Last accepted WP1 runtime snapshot had 638 feedback rows, all DEFER; ProofPacket-present `0`; Reward total `0`; complete PIT->fill->cost->proof->reward->label chain `0`. WP3 `8999aa2b`/`c2bdefbf` derives and rechecks exact repository inputs. WP4 `f36379b9` revalidates the receipt and binds it into a non-executable training identity without upgrading durability. | A dedicated forward schema for a durable receipt, plus one current candidate-matched controlled Rust evidence chain and runtime receipts. | P0; blocks terminal | Prepare exact E3/BB review for provisional forward schema; external acquisition remains E3/BB + Operator + same-window Rust gated. |
| G4 Actual training | `FAIL_SOURCE_CONTRACT_ACCEPTED` | Last accepted WP1 runtime snapshot had 638 DEFER-only runs and `model_training_performed=0`. WP4 contract `f36379b9` now binds exact projection/proof/reward/PIT/data/split/code/config/resource identities and requires actual rehash/fit/model bytes, but always returns `SCHEMA_REQUIRED` with training/model/registry false. Focused `32`; full ML `1850/36`; E2/QA P0/P1/P2 `0/0/0`. | Durable receipt schema, actual fitted matrix rehash, isolated no-symlink trainer, immutable model trio, and challenger registry. | P0; blocks terminal | Fresh exact E3/BB gate for provisional V158 reservation/creation and isolated trainer/repository design. Never train on one fill merely to check G3. |
