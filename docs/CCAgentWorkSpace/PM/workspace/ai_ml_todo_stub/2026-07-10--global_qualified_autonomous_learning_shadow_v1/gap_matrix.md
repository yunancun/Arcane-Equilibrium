...
|---|---|---|---|---|---|
| G1 Collection | `PARTIAL` | Fresh raw/ALR identity and lag `0` remained healthy through WP1. Runtime checkout and service pin now equal exact target `7d1c24794`; one session stayed stable with restart `0` and scanner INSERT denied. | Candidate decision, order/fill, actual cost/funding, and risk-context runtime evidence remain absent. | P0; blocks terminal | Build only the qualified training-result contract, then later controlled acquisition. Any external acquisition remains exact E3/BB/Operator gated. |
| G2 Candidate selection | `PARTIAL_SOURCE_ACCEPTED` | The immutable handoff/replay checkpoint `328125a08` is preserved. Event checkpoint `1b85318f` replaces five-second polling. WP3 repository `c2bdefbf` reconstructs current projection/lineage; writer `c0aec681` and reader `fb842a36` bind exact validated contract identity to V158 fixed APIs. Reader focused `61`, adjacent `130`, full ML `1911/36`, reviews `0/0/0`. | Linux runtime proof, candidate qualification, V158 apply, durable row creation/readback, and result contract remain absent. | P0; blocks terminal | Build only pure qualified training-result contract TDD. Later runtime proof requires fresh exact E3/BB. |
| G3 Outcome chain | `FAIL_SOURCE_RECEIPT_READER_ACCEPTED_SCHEMA_UNAPPLIED` | Last accepted WP1 runtime snapshot had 638 feedback rows, all DEFER; ProofPacket-present `0`; Reward total `0`; complete PIT->fill->cost->proof->reward->label chain `0`. WP3 derives inputs; input contract `f36379b9` binds them; V158 declares durability; writer `c0aec681` and reader `fb842a36` prove exact fake-DB mapping, full-row parity, and transaction ownership. | Separately gated V158 apply, one real durable receipt/readback, and one current candidate-matched controlled Rust evidence chain with runtime receipts. | P0; blocks terminal | Define the result contract source boundary first. External acquisition and PG apply remain separately gated. |
| G4 Actual training | `FAIL_SOURCE_RECEIPT_READER_ACCEPTED_RESULT_CONTRACT_PENDING` | Last accepted WP1 runtime snapshot had 638 DEFER-only runs and `model_training_performed=0`. V158, receipt writer, and receipt reader source are published; reader focused `61`, adjacent `130`, full ML `1911/36`, reviews `0/0/0`. None was executed against PG and no training fact was created. | Qualified result contract, later result repository/trainer, actual fitted-matrix and split/code/config rehash, separately gated real fit, immutable model trio bytes, V158 apply, and runtime registry rows. | P0; blocks terminal | Build only pure result-contract TDD first; result writer/reader, trainer, fit, and filesystem are later cycles. Never train on one fill merely to check G3. |
...
| G6 Decisions | `FAIL` | Runtime distribution is DEFER only; V152/V153 constrain allowed run/feedback states. | Durable DEFER/ROTATE/TRAIN/REJECT/CHALLENGER_ACCEPT/ROLLBACK/STOP reasons and hashes. | P0; blocks terminal | WP4 result contract and later trainer first, followed by the WP5 decision engine. `CHALLENGER_ACCEPT` remains no-authority. |
