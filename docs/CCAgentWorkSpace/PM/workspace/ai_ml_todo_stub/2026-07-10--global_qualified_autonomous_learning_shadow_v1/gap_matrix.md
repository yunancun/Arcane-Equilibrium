...
|---|---|---|---|---|---|
| G1 Collection | `PARTIAL` | Fresh raw/ALR identity and lag `0` remained healthy through WP1. Source `c900d1ecb` validates handshake identities; design `9a41c8d2a` now freezes phase-correct verifier evidence and append-only exact-byte acquisition without acquiring real evidence. | Candidate decision, order/fill, actual cost/funding, risk-context runtime evidence, real issuer/runner bytes, and a production verifier remain absent. | P0; blocks terminal | Preauthorize only the Rust verifier source/test seam; any external acquisition remains exact E3/BB/Operator gated. |
| G2 Candidate selection | `PARTIAL_DISPOSABLE_PG_VERIFIED` | The WP2/WP3 lineage and WP4 V158/V159 chain are preserved. Run `29195892105` proved disposable schema behavior; design `9a41c8d2a` closes coordinator lifecycle, replay, and ACL semantics on paper only. | Linux runtime proof, current candidate qualification, production V159 apply, Rust verifier, V160-style request consumption, real durable row/readback, trusted issuer receipt, and real execution remain absent. | P0; blocks terminal | Execute only the trusted-Ed25519 verifier source preauthoring gate. Production/runtime proof remains a later fresh exact E3/BB gate. |
| G3 Outcome chain | `FAIL_DISPOSABLE_PG_VERIFIED_NO_REAL_RECEIPT_OR_OUTCOME_CHAIN` | Last accepted WP1 runtime snapshot had 638 feedback rows, all DEFER; ProofPacket-present `0`; Reward total `0`; complete PIT->fill->cost->proof->reward->label chain `0`. V159 disposable behavior, pure handshake source, and accepted durability design exist, but no production verifier, durable consumption, or real evidence exists. | Rust verifier source and platform attestation, separately gated V160 source/apply, one real durable receipt/readback, trusted issuer receipt, production caller, and one current candidate-matched controlled Rust evidence chain. | P0; blocks terminal | Preauthorize only the verifier source seam; verifier implementation, schema, production apply, and real evidence remain separate gates. |
| G4 Actual training | `FAIL_DISPOSABLE_PG_VERIFIED_EXECUTION_NOT_ESTABLISHED` | Last accepted WP1 runtime snapshot had 638 DEFER-only runs and `model_training_performed=0`. Handshake source remains synthetic-only; atomic-consumption design `9a41c8d2a` is non-executable. No verifier, trainer/fit, model bytes, or production rows exist. | Production-attested verifier, isolated runner, pre-fit claim, authenticated terminal receipt, atomic consumption, separately gated real fit, immutable model trio readback, production apply, and runtime registry rows. | P0; blocks terminal | Preauthorize only verifier source/tests. Do not execute or claim a fit; never train on one fill merely to check G3. |
...
| G6 Decisions | `FAIL` | Runtime distribution is DEFER only; V152/V153 constrain allowed run/feedback states. Disposable V159, pure handshake source, and accepted atomic-consumption design create no real execution or decision. | Durable DEFER/ROTATE/TRAIN/REJECT/CHALLENGER_ACCEPT/ROLLBACK/STOP reasons and hashes. | P0; blocks terminal | Preauthorize verifier source, then separately gate verifier implementation, schema, runner, and any later trainer before WP5. `CHALLENGER_ACCEPT` remains no-authority. |
