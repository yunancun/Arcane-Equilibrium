...
|---|---|---|---|---|---|
| G1 Collection | `PARTIAL` | Fresh raw/ALR identity and lag `0` remained healthy through WP1. Runtime checkout and service pin now equal exact target `7d1c24794`; one session stayed stable with restart `0` and scanner INSERT denied. | Candidate decision, order/fill, actual cost/funding, and risk-context runtime evidence remain absent. | P0; blocks terminal | First implement the WP4 repository-only durable-receipt tracer, then later controlled acquisition. Any external acquisition remains exact E3/BB/Operator gated. |
| G2 Candidate selection | `PARTIAL_SOURCE_ACCEPTED` | The immutable handoff/replay checkpoint `328125a08` is preserved. Repair `03ef761b` keeps missing-policy READY decisions durable under exact handoff causality. Event checkpoint `1b85318f` replaces five-second candidate polling with bounded PG/inotify wakes. WP3 repository checkpoint `c2bdefbf` independently reconstructs the newest selected projection and exact bounded lineage, then rechecks head/lineage/bridge identities in one snapshot before returning any receipt. Full ML `1818 passed/36 skipped`; E2/QA/CC-FA P0/P1/P2 `0/0/0`. | The real Linux inotify test was skipped on Darwin. Deployment, service restart/recovery, natural-cycle proof, and candidate qualification remain absent. | P0; blocks terminal | Continue with the WP4 repository-only durable-receipt tracer. Later B2.2c Linux/runtime proof requires fresh exact E3/BB. |
| G3 Outcome chain | `FAIL_SOURCE_SCHEMA_AUTHORED_UNAPPLIED` | Last accepted WP1 runtime snapshot had 638 feedback rows, all DEFER; ProofPacket-present `0`; Reward total `0`; complete PIT->fill->cost->proof->reward->label chain `0`. WP3 `8999aa2b`/`c2bdefbf` derives and rechecks exact repository inputs. WP4 contract `f36379b9` binds them; V158 source `beeb77325` now declares a durable receipt boundary but was not applied or exercised against PG. | A repository implementation using the fixed V158 receipt API, separately gated migration apply, and one current candidate-matched controlled Rust evidence chain with runtime receipts. | P0; blocks terminal | Implement the isolated repository source seam first. External acquisition and PG apply remain separately gated. |
| G4 Actual training | `FAIL_SOURCE_SCHEMA_AUTHORED_TRAINER_PENDING` | Last accepted WP1 runtime snapshot had 638 DEFER-only runs and `model_training_performed=0`. WP4 contract `f36379b9` binds exact identities; V158 source `beeb77325` publishes receipt/run/q10-q50-q90/NOT_SERVING registry DDL and fixed APIs with focused `37`, full ML `1850/36`, Rust compile-only, and independent P0/P1/P2 `0/0/0`. It is unexecuted and creates no runtime training fact. | Durable-receipt repository integration, later isolated trainer, actual fitted-matrix and split/code/config rehash, a separately gated real fit, immutable model trio bytes, V158 apply, and runtime registry rows. | P0; blocks terminal | Build only the repository durable-receipt tracer first; trainer/fit/filesystem are later cycles. Never train on one fill merely to check G3; no PG/fit is authorized by source publication. |
...
| G6 Decisions | `FAIL` | Runtime distribution is DEFER only; V152/V153 constrain allowed run/feedback states. | Durable DEFER/ROTATE/TRAIN/REJECT/CHALLENGER_ACCEPT/ROLLBACK/STOP reasons and hashes. | P0; blocks terminal | WP4 durable-receipt repository tracer, then later trainer, followed by the WP5 decision engine. `CHALLENGER_ACCEPT` remains no-authority. |
