...
|---|---|---|---|---|---|
| G1 Collection | `PARTIAL` | Fresh raw/ALR identity and lag `0` remained healthy through WP1. Runtime checkout and service pin now equal exact target `7d1c24794`; one session stayed stable with restart `0` and scanner INSERT denied. | Candidate decision, order/fill, actual cost/funding, and risk-context runtime evidence remain absent. | P0; blocks terminal | Implement the WP4 fixed receipt reader, then later controlled acquisition. Any external acquisition remains exact E3/BB/Operator gated. |
| G2 Candidate selection | `PARTIAL_SOURCE_ACCEPTED` | The immutable handoff/replay checkpoint `328125a08` is preserved. Repair `03ef761b` keeps missing-policy READY decisions durable under exact handoff causality. Event checkpoint `1b85318f` replaces five-second candidate polling with bounded PG/inotify wakes. WP3 repository checkpoint `c2bdefbf` reconstructs and rechecks current projection/lineage; writer tracer `c0aec681` now maps the validated contract to V158's fixed receipt API. | Linux runtime proof, candidate qualification, V158 apply, durable row creation, and fixed reader source are absent. | P0; blocks terminal | Build only fixed receipt-reader `FOUND/NOT_FOUND` TDD. Later runtime proof requires fresh exact E3/BB. |
| G3 Outcome chain | `FAIL_SOURCE_WRITER_TRACER_AUTHORED_SCHEMA_UNAPPLIED` | Last accepted WP1 runtime snapshot had 638 feedback rows, all DEFER; ProofPacket-present `0`; Reward total `0`; complete PIT->fill->cost->proof->reward->label chain `0`. WP3 `8999aa2b`/`c2bdefbf` derives inputs; contract `f36379b9` binds them; V158 `beeb77325` declares durability; writer tracer `c0aec681` now proves exact fake-DB mapping and transaction ownership. | Fixed receipt reader, separately gated V158 apply, one real durable receipt, and one current candidate-matched controlled Rust evidence chain with runtime receipts. | P0; blocks terminal | Implement the fixed receipt reader source seam first. External acquisition and PG apply remain separately gated. |
| G4 Actual training | `FAIL_SOURCE_WRITER_TRACER_ACCEPTED_TRAINER_PENDING` | Last accepted WP1 runtime snapshot had 638 DEFER-only runs and `model_training_performed=0`. V158 source and writer tracer are published; tracer focused `31`, adjacent `121`, full ML `1920/28`, reviews `0/0/0`. Neither was executed against PG and no training fact exists. | Fixed receipt reader, later isolated trainer, actual fitted-matrix and split/code/config rehash, a separately gated real fit, immutable model trio bytes, V158 apply, and runtime registry rows. | P0; blocks terminal | Build only fixed receipt-reader TDD first; result/trainer/fit/filesystem are later cycles. Never train on one fill merely to check G3. |
...
| G6 Decisions | `FAIL` | Runtime distribution is DEFER only; V152/V153 constrain allowed run/feedback states. | Durable DEFER/ROTATE/TRAIN/REJECT/CHALLENGER_ACCEPT/ROLLBACK/STOP reasons and hashes. | P0; blocks terminal | WP4 fixed receipt reader, then later trainer, followed by the WP5 decision engine. `CHALLENGER_ACCEPT` remains no-authority. |
