...
| G3 Outcome chain | `FAIL_DESIGN_ACCEPTED_DURABLE_SCHEMA_UNAUTHORED_UNAPPLIED` | Last accepted WP1 runtime snapshot had 638 feedback rows, all DEFER; ProofPacket-present `0`; Reward total `0`; complete PIT->fill->cost->proof->reward->label chain `0`. Fit-capture `a09c70f2` remains externally unchecked and non-persistent. The durable schema design now has PA/CC/FA/MIT acceptance plus E3/BB source-authoring approval at reviewed head `0c90de9c2`, P0/P1/P2 `0/0/0`; no migration or durable row exists. | Forward durable attestation source/tests, separately gated migration apply, one real durable receipt/readback, platform/external trusted issuer receipt, and one current candidate-matched controlled Rust evidence chain. | P0; blocks terminal | Execute source-only `WP4-DURABLE-FIT-ATTESTATION-SCHEMA-SOURCE-TDD` under the frozen design; external acquisition and PG apply remain separately gated. |
| G4 Actual training | `FAIL_DESIGN_ACCEPTED_EXECUTION_NOT_ESTABLISHED` | Last accepted WP1 runtime snapshot had 638 DEFER-only runs and `model_training_performed=0`. Fit-capture `a09c70f2` still keeps execution/training `NOT_ESTABLISHED`, persistence false, and authenticity `EXTERNAL_HOST_UNCHECKED`. The accepted forward design closes that path only after later source authoring and migration application; this gate ran no trainer/fit and created no model or row. | Durable attestation/result source accepting only trusted, current issuer receipts; later isolated runner, separately gated real fit, immutable model trio readback, migration apply, and runtime registry rows. | P0; blocks terminal | Author only the forward migration/tests after a fresh collision scan. Do not contact PostgreSQL/runtime or execute fit in this source loop. Never train on one fill merely to check G3. |
