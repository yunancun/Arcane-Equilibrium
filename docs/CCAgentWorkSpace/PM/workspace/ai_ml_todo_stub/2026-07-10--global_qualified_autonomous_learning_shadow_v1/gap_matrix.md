...
|---|---|---|---|---|---|
| G1 Collection | `PARTIAL` | Fresh raw/ALR identity and lag `0` remained healthy through WP1. Exact head `74d8475e3` proves isolated V159 behavior; source `c900d1ecb` now validates bounded request/nonce/audience/policy-snapshot and terminal-receipt identities without acquiring real evidence. | Candidate decision, order/fill, actual cost/funding, risk-context runtime evidence, and real issuer/runner bytes remain absent. | P0; blocks terminal | Design only the V160-style atomic-consumption coordinator; any external acquisition remains exact E3/BB/Operator gated. |
| G2 Candidate selection | `PARTIAL_DISPOSABLE_PG_VERIFIED` | The WP2/WP3 lineage and WP4 V158/V159 chain are preserved. Run `29195892105` proved disposable schema behavior; source `c900d1ecb` implements the pure handshake while preserving V159 as containment. | Linux runtime proof, current candidate qualification, production V159 apply, V160-style request consumption, real durable row/readback, trusted issuer receipt, and real execution remain absent. | P0; blocks terminal | Execute only the V160-style atomic-consumption design preauthoring gate. Production/runtime proof remains a later fresh exact E3/BB gate. |
| G3 Outcome chain | `FAIL_DISPOSABLE_PG_VERIFIED_NO_REAL_RECEIPT_OR_OUTCOME_CHAIN` | Last accepted WP1 runtime snapshot had 638 feedback rows, all DEFER; ProofPacket-present `0`; Reward total `0`; complete PIT->fill->cost->proof->reward->label chain `0`. V159 disposable behavior and pure handshake source exist, but synthetic verification remains `EXTERNAL_HOST_UNCHECKED` with no durable consumption or real evidence. | Separately gated V160-style consumption, production apply, one real durable receipt/readback, platform/external trusted issuer receipt, production caller, and one current candidate-matched controlled Rust evidence chain. | P0; blocks terminal | Freeze the single fixed coordinator design; external acquisition, schema source, production apply, and real evidence remain separate gates. |
| G4 Actual training | `FAIL_DISPOSABLE_PG_VERIFIED_EXECUTION_NOT_ESTABLISHED` | Last accepted WP1 runtime snapshot had 638 DEFER-only runs and `model_training_performed=0`. Handshake source `c900d1ecb` passes synthetic contract tests, but fixture signatures are not production-valid, durable consumption is false, and training remains `NOT_ESTABLISHED`. No trainer/fit, model bytes, or production rows exist. | Isolated runner, pre-fit nonce claim, authenticated terminal receipt, future atomic consumption, separately gated real fit, immutable model trio readback, production apply, and runtime registry rows. | P0; blocks terminal | Design only the V160-style atomic coordinator. Do not execute or claim a fit; never train on one fill merely to check G3. |
...
| G6 Decisions | `FAIL` | Runtime distribution is DEFER only; V152/V153 constrain allowed run/feedback states. Disposable V159 verification and accepted pure handshake source create no real execution or decision. | Durable DEFER/ROTATE/TRAIN/REJECT/CHALLENGER_ACCEPT/ROLLBACK/STOP reasons and hashes. | P0; blocks terminal | Complete only the V160-style design gate, then separately gate schema, runner, and any later trainer before WP5. `CHALLENGER_ACCEPT` remains no-authority. |
