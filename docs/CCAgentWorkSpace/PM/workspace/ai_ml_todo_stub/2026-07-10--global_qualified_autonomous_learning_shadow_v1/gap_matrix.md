...
| G2 Candidate selection | `PARTIAL_SOURCE_ACCEPTED` | WP2-A `c84e14f5d` implements typed selection and the true source chain. WP2-B extraction `13d2b980c` preserved `199` tests/four golden hashes. Checkpoint `1afdf4231` now captures immutable Rust `candidate_event_context_v1` only at organic Cost Gate reject before `on_rejection`, preserves it through an optional ledger field, and validates cold Python `candidate_evaluation_context_v1` with shared canonical bytes/hash. Python E4 `220` unique tests and Rust E2/E4/QA passed with zero findings. | Python ledger/outcome propagation, explicit no-backfill treatment for historical PG/snapshot rows, cold enrichment into board compatibility, event-driven primary handoff, deployment, and natural-cycle runtime proof. | P0; blocks terminal | WP2-B B2.2 propagation via PA/E1/E2/E4/QA, then event-driven handoff; runtime later requires E3/BB. |
