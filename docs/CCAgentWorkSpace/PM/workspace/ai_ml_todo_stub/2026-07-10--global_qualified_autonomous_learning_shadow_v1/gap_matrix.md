...
|---|---|---|---|---|---|
| G1 Collection | `PARTIAL` | Fresh raw/ALR identity and lag `0` remained healthy through WP1. Runtime checkout and service pin now equal exact target `7d1c24794`; one session stayed stable with restart `0` and scanner INSERT denied. | Candidate decision, order/fill, actual cost/funding, and risk-context runtime evidence remain absent. | P0; blocks terminal | WP4 source contracts, then later controlled acquisition. Any external acquisition remains exact E3/BB/Operator gated. |
| G2 Candidate selection | `PARTIAL_SOURCE_ACCEPTED` | The immutable handoff/replay checkpoint `328125a08` is preserved. Repair `03ef761b` keeps missing-policy READY decisions durable under exact handoff causality. Event checkpoint `1b85318f` replaces five-second candidate polling with bounded PG/inotify wakes. WP3 repository checkpoint `c2bdefbf` independently reconstructs the newest selected projection and exact bounded lineage, then rechecks head/lineage/bridge identities in one snapshot before returning any receipt. Full ML `1818 passed/36 skipped`; E2/QA/CC-FA P0/P1/P2 `0/0/0`. | The real Linux inotify test was skipped on Darwin. Deployment, service restart/recovery, natural-cycle proof, and candidate qualification remain absent. | P0; blocks terminal | Continue WP4 source contracts. Later B2.2c Linux/runtime proof requires fresh exact E3/BB. |
| G3 Outcome chain | `FAIL_SOURCE_REPOSITORY_ADAPTER_ACCEPTED` | Last accepted WP1 runtime snapshot had 638 feedback rows, all DEFER; ProofPacket-present `0`; Reward total `0`; complete PIT->fill->cost->proof->reward->label chain `0`. WP3 pure adapter `8999aa2b` plus read-only repository adapter `c2bdefbf` now require the immutable selected handoff, derive candidate/context binding internally, validate exact projection/PIT/proof/reward causality, preserve raw bytes separately from canonical adapter inputs, enforce no-fill non-reward, bound lineage/bridge reads, and emit only non-persisted/non-runtime-attested receipts. | A dedicated forward schema for any durable projection receipt, plus one current candidate-matched controlled Rust evidence chain and runtime receipts. | P0; blocks terminal | Advance WP4 collision scan and typed receipt/training/registry contracts. Any external acquisition requires fresh exact E3/BB + Operator + same-window Rust gates. |
| G4 Actual training | `FAIL` | Last accepted WP1 runtime snapshot had 638 runs, all `scanner_novelty_statistical_baseline/DEFER_EVIDENCE`; `model_training_performed=0`; no model artifact/registry. WP3 wrote no rows and performed no fit. | Eligible multi-sample labels, actual fit, hashes, isolated registry, next versioned migration. | P0; blocks terminal | WP4 is active; first do source collision scan/contracts. Never train on one fill merely to check G3. V152/V153 stay immutable. |
