...
|---|---|---|---|---|---|
| G1 Collection | `PARTIAL` | Fresh raw/ALR identity and lag `0` remained healthy through WP1. Exact head `74d8475e3` proves isolated V159 behavior; handshake design `2189c9961` now freezes request/nonce/audience/policy-snapshot identity without acquiring evidence. | Candidate decision, order/fill, actual cost/funding, and risk-context runtime evidence remain absent. | P0; blocks terminal | Author only the pure handshake contract/tests; any external acquisition remains exact E3/BB/Operator gated. |
| G2 Candidate selection | `PARTIAL_DISPOSABLE_PG_VERIFIED` | The WP2/WP3 lineage and WP4 V158/V159 chain are preserved. Run `29195892105` proved disposable schema behavior; the handshake design now preserves V159 as containment and requires future atomic request consumption. | Linux runtime proof, current candidate qualification, production V159 apply, V160-style request consumption, real durable row/readback, trusted issuer receipt, and real execution remain absent. | P0; blocks terminal | Execute only handshake-contract source TDD with synthetic fixtures. Production/runtime proof remains a later fresh exact E3/BB gate. |
| G3 Outcome chain | `FAIL_DISPOSABLE_PG_VERIFIED_NO_REAL_RECEIPT_OR_OUTCOME_CHAIN` | Last accepted WP1 runtime snapshot had 638 feedback rows, all DEFER; ProofPacket-present `0`; Reward total `0`; complete PIT->fill->cost->proof->reward->label chain `0`. V159 disposable behavior and a no-effect handshake design exist, but no real evidence. | Separately gated V160-style consumption, production apply, one real durable receipt/readback, platform/external trusted issuer receipt, production caller, and one current candidate-matched controlled Rust evidence chain. | P0; blocks terminal | Author only request/receipt/verifier source contracts; external acquisition, schema, production apply, and real evidence remain separate gates. |
| G4 Actual training | `FAIL_DISPOSABLE_PG_VERIFIED_EXECUTION_NOT_ESTABLISHED` | Last accepted WP1 runtime snapshot had 638 DEFER-only runs and `model_training_performed=0`. Disposable signatures were synthetic; the handshake design's maximum pure verdict is `AUTHENTICATED_UNCONSUMED`. No trainer/fit, model bytes, or production rows exist. | Isolated runner, pre-fit nonce claim, authenticated terminal receipt, future atomic consumption, separately gated real fit, immutable model trio readback, production apply, and runtime registry rows. | P0; blocks terminal | Implement only the pure handshake contracts/tests. Do not execute or claim a fit; never train on one fill merely to check G3. |
...
| G6 Decisions | `FAIL` | Runtime distribution is DEFER only; V152/V153 constrain allowed run/feedback states. Disposable V159 verification and the handshake design create no real execution or decision. | Durable DEFER/ROTATE/TRAIN/REJECT/CHALLENGER_ACCEPT/ROLLBACK/STOP reasons and hashes. | P0; blocks terminal | Complete pure handshake source/tests, then separately gate schema, runner, and any later trainer before WP5. `CHALLENGER_ACCEPT` remains no-authority. |
