...
| `WP2-CANDIDATE-AWARE-ARBITER` | 0 | `ACTIVE_WP2B_PROSPECTIVE_EVENT_LINEAGE` | `PM -> QC -> MIT -> AI-E -> PA -> E1 -> E2 -> E4 -> QA -> PM` | WP1 `DONE_RUNTIME_ACCEPTED`; WP2-A `DONE_SOURCE_ACCEPTED_PROVISIONAL_BRIDGE` at `c84e14f5d` | Candidate identity includes strategy+version/config hash, symbol, side, horizon, regime, data/evidence context; ranks by distinct-entry `n_eff`, UTC-day/top-day/regime coverage, quality, proof gap, EVI, compute/storage cost, cooldown, portfolio exposure/correlation/overlap, and capital context; rotates globally.  Event-time identity must be captured prospectively and evaluation-time context must remain hash-bound. | `false` source/tests; `false`; `false` | WP2-A source acceptance: integrated `458`, cron static `17`; E2/AI-E/E4/QA PASS with P0/P1/P2 `0/0/0`; true review -> stamped publisher -> adapter -> active consumer -> projection -> repository chain; zero authority/training.  WP2-A did not inspect runtime; the last accepted WP1 pin was `7d1c24794`.  Cron is provisional reconciliation only; retry/RCA `0/0`. | Extract candidate-board construction behind a narrow Interface/Adapter with byte-identical hashes, then add immutable Rust `candidate_event_context_v1`, cold Python `candidate_evaluation_context_v1`, and event-driven primary handoff; no exchange contact. |
