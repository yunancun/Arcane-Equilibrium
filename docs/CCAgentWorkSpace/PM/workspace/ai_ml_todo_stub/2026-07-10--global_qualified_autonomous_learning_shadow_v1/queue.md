...
Updated: 2026-07-10T23:01:08Z
Goal status: `ACTIVE`
Current item: `WP2-CANDIDATE-AWARE-ARBITER`
...
outcome-writer, and price-observation production code is unchanged. B2.2b cold
evaluation attachment plus candidate-board v2/arbiter-input versioning is now
active, followed by event-driven primary handoff; no runtime was inspected or
changed.
...
| `WP2-CANDIDATE-AWARE-ARBITER` | 0 | `ACTIVE_WP2B_COLD_EVALUATION_BOARD_V2` | `PM -> QC -> MIT -> AI-E -> PA -> E1 -> E2 -> E4 -> QA -> PM` | WP1 `DONE_RUNTIME_ACCEPTED`; WP2-A accepted at `c84e14f5d`; WP2-B extraction `13d2b980c`; prospective lineage `1afdf4231`; B2.2a propagation `38ccd014c` | Candidate identity includes strategy+version/config hash, symbol, side, horizon, regime, data/evidence context; ranks by distinct-entry `n_eff`, UTC-day/top-day/regime coverage, quality, proof gap, EVI, compute/storage cost, cooldown, portfolio exposure/correlation/overlap, and capital context; rotates globally. Event-time identity must be captured prospectively and evaluation-time context must remain hash-bound. | `false` source/tests; `false`; `false` | B2.2a preserves validated raw context losslessly event -> decision -> ledger -> blocked outcome with exact strategy/symbol/side/context/signal/mode/timestamp binding and fail-closed hash/summary/graft checks. Only `explicit_source_rows` can carry prospective context; PG/snapshot/unmarked history is contextless and unqualified. No evaluation/projection synthesis; candidate-board/outcome-writer/price-observation production unchanged. E1 `220 passed, 1 skipped`; E2 closed a P1 provenance bypass then P0/P1/P2 `0/0/0`; E4 `303 passed, 1 skipped` plus Rust targeted `1`; root Rust `10/10`; QA `41/41`, `PASS_SOURCE_CHECKPOINT_TO_PM`. No Linux/runtime/PG/Bybit/training/authority action; last accepted WP1 pin remains `7d1c24794`. | Implement B2.2b explicit cold evaluation attachment and version candidate-board/arbiter input to v2, then implement restart-safe event-driven primary handoff. Cron remains reconciliation only. |
