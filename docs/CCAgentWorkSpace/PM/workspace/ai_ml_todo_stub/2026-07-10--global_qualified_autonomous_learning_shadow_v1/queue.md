...
| `WP2-CANDIDATE-AWARE-ARBITER` | 0 | `ACTIVE_CONTRACT_AND_FIXTURE_DISCOVERY` | `PM -> QC -> MIT -> AI-E -> PA -> E1 -> E2 -> E4 -> QA -> PM` | WP1 `DONE_RUNTIME_ACCEPTED` | Candidate identity includes strategy+version/config hash, symbol, side, horizon, regime, data/evidence context; ranks by distinct-entry `n_eff`, UTC-day/top-day/regime coverage, quality, proof gap, EVI, compute/storage cost, cooldown, portfolio exposure/correlation/overlap, and capital context; rotates globally | `false` source/tests; `false`; `false` | Runtime remains scanner-novelty/side `NONE` without horizon/regime; QC and PA source audits dispatched; retry/RCA `0/0` | Freeze arbiter input/output schema and adversarial fixtures, then dispatch MIT/AI-E review before implementation; no exchange contact. |
