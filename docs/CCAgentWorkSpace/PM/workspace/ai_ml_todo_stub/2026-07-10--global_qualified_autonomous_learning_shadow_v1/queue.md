...
| `WP4-ACTUAL-TRAINING-REGISTRY` | 0 | `ACTIVE_FORWARD_SCHEMA_GATE_AND_ISOLATED_TRAINER_DESIGN` | `PM -> QC -> MIT -> AI-E -> PA -> E1 -> E2 -> E4 -> QA -> E3 -> BB -> PM` | WP3 repository `c2bdefbf`; collision scan; source contract `f36379b9` | Provisional V158 forward schema must persist a durable receipt/training run/model trio/isolated challenger registry without editing V152/V153 or reusing V023/V157. Actual row/split/code/config bytes must be rehashed; `model_training_performed=true` only after real fit and immutable artifact hashing. | source contract complete; migration reservation/creation/apply and runtime `true`; Operator `false`; runtime mutation gated | Source contract is accepted: focused `32`, full ML `1850/36`, E2/QA `0/0/0`; current training/artifact/registry `0/0/0`; V158 remains unreserved; retry/RCA `0/0`. | Prepare fresh exact E3/BB review for provisional V158 reservation/creation and isolated no-symlink trainer/repository design. Do not reserve/create/apply before approval. |
