...
cross-language canonical fixture.  Python adapter/materializer/outcome
propagation and event-driven primary handoff remain active; no runtime was
...
| `WP2-CANDIDATE-AWARE-ARBITER` | 0 | `ACTIVE_WP2B_LINEAGE_PROPAGATION` | `PM -> QC -> MIT -> AI-E -> PA -> E1 -> E2 -> E4 -> QA -> PM` | WP1 `DONE_RUNTIME_ACCEPTED`; WP2-A accepted at `c84e14f5d`; WP2-B extraction `13d2b980c`; lineage checkpoint `1afdf4231` | Candidate identity includes strategy+version/config hash, symbol, side, horizon, regime, data/evidence context; ranks by distinct-entry `n_eff`, UTC-day/top-day/regime coverage, quality, proof gap, EVI, compute/storage cost, cooldown, portfolio exposure/correlation/overlap, and capital context; rotates globally. Event-time identity must be captured prospectively and evaluation-time context must remain hash-bound. | `false` source/tests; `false`; `false` | Candidate-board Module extraction preserved `199` tests and four golden hashes. Prospective lineage adds Rust organic-reject capture before `on_rejection`, optional old-row-compatible ledger propagation, pure Python evaluation validation, and one Rust/Python canonical fixture. Python E4 ran `220` unique tests; Rust E2/E4 and combined QA passed with P0/P1/P2 `0/0/0`. No Linux/runtime/PG/Bybit/training/authority action; last accepted WP1 pin remains `7d1c24794`. | Propagate only valid prospective context through `runtime_adapter` and outcomes, keep historical PG/snapshot materialization explicitly unqualified, project validated cold evaluation context into candidate-board compatibility, then implement restart-safe event-driven primary handoff. |
