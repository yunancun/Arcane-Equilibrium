...
Updated: 2026-07-12T14:55:12Z
Goal status: `ACTIVE`
Current item: `WP4-ACTUAL-TRAINING-REGISTRY`
Current state: `ACTIVE_WP4_TRUSTED_ISSUER_ISOLATED_RUNNER_HANDSHAKE_CONTRACT_SOURCE_TDD`
...
The active safe slice is now
`WP4-TRUSTED-ISSUER-ISOLATED-RUNNER-HANDSHAKE-CONTRACT-SOURCE-TDD`: author
only one pure contract module and synthetic mutation-biting tests. Real bytes,
external contact, PG/files/network/runtime, V158/V159 changes, V160
reservation, fit/model effects, and authority remain forbidden.
...
| `WP4-ACTUAL-TRAINING-REGISTRY` | 0 | `ACTIVE_WP4_TRUSTED_ISSUER_ISOLATED_RUNNER_HANDSHAKE_CONTRACT_SOURCE_TDD` | `PM -> QC -> MIT -> AI-E -> PA -> E1 -> E2 -> E4 -> QA -> E3 -> BB -> PM` | WP3 repository `c2bdefbf`; WP4 source chain through V159; disposable verification `74d8475e3` / run `29195892105`; handshake design `2189c9961` | Author one pure request/receipt/verifier contract module with request hash = attempt/invocation identity, issuer nonce/policy snapshot, pre-fit runner target, closed terminal failures, exact replay, Ed25519-only v1, bounded canonical bytes, and false/zero authority. Synthetic fixtures only. | source/tests `false`; PG/files/network/runtime, V158/V159/V160, external issuer/runner, real fit/model remain fresh gates; Operator `false` | `DONE_DESIGN_ACCEPTED_TRUSTED_ISSUER_ISOLATED_RUNNER_HANDSHAKE_PREAUTHORING_GATE`: V159 trust/consumption gap is explicit; pure verifier ceiling `AUTHENTICATED_UNCONSUMED`; later V160-style atomic seam required. Design effect bytes/source/PG/network/fit/model/authority `0`; G3/G4 still fail. | Execute only `WP4-TRUSTED-ISSUER-ISOLATED-RUNNER-HANDSHAKE-CONTRACT-SOURCE-TDD`. No real bytes, PG/files/network/runtime apply, V159/V160 changes, external contact, trainer/fit, model publication, serving/promotion, broker, order, risk, Cost Gate, or authority effect. |
