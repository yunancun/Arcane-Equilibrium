...
| `P0-AIML-ALR-LEARNING-TARGET-ARBITER` | 0 | `ACTIVE` | `PM -> PA -> E1/E1a -> E2 -> E4 -> QA -> PM` | `SOURCE_ONLY_OFFLINE_P0_P1`; single-shot CLI emits `learning_target_runtime_v1` under explicit `--out`; input snapshot manifest is hash-bound; stale `_latest` is rejected; scanner/no-order/artifact-count evidence cannot become proof; objective is `expected_value_of_information`. | Controller contracts are implemented and validated for source-only work-item/effect/state packets. | Implement after controller contracts. |
