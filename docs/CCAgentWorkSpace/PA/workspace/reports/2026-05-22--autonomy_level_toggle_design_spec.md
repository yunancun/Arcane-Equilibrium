...
Date: 2026-05-22
Status: **REVISED — operator 6 拍板 + E2 BLOCK 補丁 + FA/A3/MIT 補完整合（v2 patch）**
Owner: PA
Operator Source:
- 2026-05-22 directive Q2「PM 推薦 Path B 設一個自動等級，CC 設另外一個等級，可以在設置裡切換 autonomy level」
- 2026-05-22 operator 6 拍板：A3 U1（typed-confirm = `CONFIRM SWITCH`）+ A3 U2（14 path matrix 展開/折疊）+ E2 Q3（三路 fail → 1h wait → 最高保護模式）+ E2 Q4（emergency override rolling 30d + machine local time）+ FA U-FA-1（Level 2 GUI toggle disabled until evidence baseline）+ FA U-FA-2（emergency override 30% = freeze 24h + monthly PM review）+ MIT Q1（current_level PG ENUM）+ MIT Q2（`system` schema）

Revision History:
- v1 2026-05-22 初稿（已 land HEAD pre-patch）
- **v2 2026-05-22 本版 — operator 6 拍板整合 + E2 BLOCK B1-B5 補丁 + FA F1-F3 補完 + A3 8 anti-pattern 寫入 + MIT Q1/Q2 schema update**
Related:
...
**Level 2 啟用閘門（per FA U-FA-1，受 P0-EDGE-1 阻）**：Level 2 schema + DB row 可現在 land（V099 不變），但 **GUI toggle button 顯示但 disabled** 直到 evidence baseline 達標（§5.4 + §11 readiness checklist）。達標條件 = (1) 21d demo 穩定期 + (2) 5 textbook 策略各 N≥30 + (3) Wilson CI 95% lower bound 正向。Disabled state tooltip 顯示具體 unmet 條件 + 達標進度。達標後 toggle 自動 enable + Slack alert 通知 operator「Level 2 enable 條件達成，可考慮切換」。
...
| **(b)** | 5-gate live boundary toggle | v1 protected → v2 auto-eligible（但 5-gate 永鎖） | **operator approve manual**（5-gate 整體含 HMAC + live_reserved 軟邊界檢查 + Operator role + secret slot + authorization.json，**全部 manual**） | **operator approve manual**（同 Level 1 — 5-gate 整體永鎖永遠不可繞，含 HMAC 簽署 + 軟邊界檢查 + 全 5 條） | CLAUDE.md §四 / AMD v2 §Decision 1 (b) / **B2-CI-2 wording 修正**：以本表 PA spec 立場為準（更保守 = fail-closed 原則）；TW v2 §3.3 「5-gate 非 HMAC 部分可 auto」wording 需 TW patch 對齊本表 |
...
| **(e)** | Kill criteria 觸發 | v1 protected → v2 auto-trigger（既設 fail-closed） | **auto-trigger + auto-enforce**（既設 fail-closed 路徑，per CLAUDE.md §四）+ §Decision 2 fail-safe 全 PASS；detect + enforce **均 auto** 不需 operator click | **同 Level 1**（既設 fail-closed 兩 level 都 auto-trigger + auto-enforce）| AMD v2 §Decision 1 (e) + CLAUDE.md §四 fail-closed 紀律 + **B2-CI-3 wording 修正**：TW v2 §3.2 (e)「operator click」誤導需 TW 改齊本表 |
...
- **Schema 命名（per MIT Q2）**：新建 **`system`** schema（**不**塞既有 `governance`），預留 future system-wide policy 空間。DDL：`CREATE SCHEMA IF NOT EXISTS system;`（idempotent；本 V### migration 第一條 statement）
- **Type 設計（per MIT Q1）**：`current_level` 用 **PG ENUM type** `autonomy_level_enum AS ENUM ('CONSERVATIVE', 'STANDARD')`（readability + DB-level CHECK constraint 自動 + 應用層 mapping 至 Rust enum class）；不用 smallint 1/2
...
-- ── Step 0：新建 system schema（per MIT Q2）
CREATE SCHEMA IF NOT EXISTS system;

-- ── Step 1：新建 ENUM type（per MIT Q1；readability + DB-level CHECK constraint 自動）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'autonomy_level_enum') THEN
...
CREATE TABLE IF NOT EXISTS system.autonomy_level_config (
    ...
    -- 當前 Autonomy Level（per MIT Q1 PG ENUM 取代 smallint 1/2）
    ...
    switch_reason text NOT NULL DEFAULT 'cold_start_default_level_conservative',
...
INSERT INTO system.autonomy_level_config (id, current_level, switched_by, switch_reason)
    VALUES (1, 'CONSERVATIVE', 'system_default', 'cold_start_default_level_conservative')
    ON CONFLICT (id) DO NOTHING;

-- ── PG LISTEN/NOTIFY channel（per B4 R-2 cache invalidation 推薦 (b)）
-- 切換 commit 時 toggle handler emit NOTIFY autonomy_level_changed；
-- 引擎內 PG listener task subscribe 此 channel，收到 NOTIFY 後重讀 atomic store。
-- 配 fallback (a) polling 5s（容錯路徑；正常路徑 NOTIFY 觸發 ≤ 200ms latency）
COMMENT ON TABLE system.autonomy_level_config IS
    'Single-row autonomy level state. Toggle handler必走 PG transaction wrap + PG NOTIFY autonomy_level_changed channel for engine cache invalidation. See spec §4.3 + §B4 R-2.';
```

**Rust enum mapping**（per MIT Q1 應用層）：
```rust
// rust/openclaw_core/src/autonomy/level.rs（new file or 既有 governance module）
#[derive(Debug, Clone, Copy, PartialEq, Eq, sqlx::Type)]
#[sqlx(type_name = "autonomy_level_enum", rename_all = "UPPERCASE")]
pub enum AutonomyLevel {
    Conservative,  // Level 1 / CC stance / fail-closed default
    Standard,      // Level 2 / PM Path B / Auto with §Decision 2 fail-safe
}
```

**Guard 對齊**（per ADR-0010）：
- **Guard A**：`CREATE TABLE IF NOT EXISTS` + `CREATE SCHEMA IF NOT EXISTS` + ENUM 創建用 `DO $$ ... IF NOT EXISTS` pattern — table 重 create 不破壞既有 row；schema/ENUM 重 apply 安全
...
CREATE TABLE IF NOT EXISTS system.autonomy_level_switch_audit (
    ...
    -- 切換時間 雙時間戳（per E2 Q4 拍板）
    -- switched_at_utc：UTC ms 精度（跨 timezone 統一審計）
    -- switched_at_local：machine local timezone（per Q4「machine local time」for monthly 計數窗口計算）
    ...
    -- Before / after level（per MIT Q1 改用 ENUM；Guard against silent corruption）
    ...
    twofa_method text NULL,  -- 'TOTP' / 'backend_unreachable' / NULL；禁 'hardware_key' / 禁 'remember_device_30d'（per A3 anti-pattern）

    -- 切換理由（operator 自由文本必填；system_default 為固定字串；最少 30 字元 per A3 spec §5.2）
    ...
    result text NOT NULL DEFAULT 'success' CHECK (result IN (
        ...
        'freeze_active_block', -- §7.4 freeze 期間禁切換
    ...
    -- per B1：三路全 fail 後 1h wait 結果（NULL = 未觸發 escalation；'operator_responded' = operator 1h 內介入；'auto_escalated_to_sm04_defensive' = 1h 內無 response 自動進 SM-04 Defensive + active 鎖利）
...
-- Index for monthly 計數（per E2 Q4 rolling 30d machine local time）
...
| Dry-run 必驗 | (1) Guard A 雙 apply 不錯（含 `CREATE SCHEMA IF NOT EXISTS system` + ENUM 雙 apply）(2) Insert seed row 後 `current_level='CONSERVATIVE'`（per MIT Q1）(3) UPDATE 觸發 updated_at touch (4) Append-only constraint：trading_ai role 嘗試 UPDATE / DELETE 必拒 (5) ENUM constraint：嘗試 `INSERT current_level='LEVEL_3'` 必拒（PG ENUM 自動 reject）(6) Audit insert with twofa_verify_result='FAIL' 必成功（記錄 fail 嘗試）(7) switch_reason char_length < 30 必拒（per A3 spec ≥30 字元）(8) **AV-9 atomic**：模擬 INSERT audit 失敗（如違反 CHECK）後 UPDATE config 必 ROLLBACK（PG transaction wrap 驗證）(9) **AV-10 race**：兩 session 同時走 toggle handler，PG advisory lock `pg_try_advisory_xact_lock` win 1 lose 1，audit 兩 row（win = success / lose = race_lost）(10) **AV-11 fail-closed**：2FA backend mock unreachable 必寫 `twofa_verify_result='FAIL', twofa_method='backend_unreachable', result='twofa_backend_down'` (11) **B1 escalation**：notification_status 三路全 FAILED 必 trigger `notification_escalation_result` 寫入（mock 1h timeout 後 = `auto_escalated_to_sm04_defensive`） (12) **B4 PG NOTIFY**：toggle commit 後 `LISTEN autonomy_level_changed` channel 必收 notification（mock subscriber 驗 ≤ 200ms latency）(13) **雙時間戳一致性**：`switched_at_utc` 與 `switched_at_local AT TIME ZONE 'UTC'` 必差 < 1s（per Q4 雙時間戳設計） |
...
**Emergency override 月度計數窗口（per E2 Q4 拍板）**：
- **窗口類型**：**Rolling 30 天**（不是 calendar month — 防月初規避紀律）
- **時區**：**Machine local time**（不是 UTC — operator 月度認知對齊本地時區；query 用 `switched_at_local`）
- **雙時間戳記錄**：每次 emergency override audit row 同時記 `switched_at_utc`（跨 timezone 統一審計）+ `switched_at_local`（rolling 30d 計算）— per §3.3 audit table 雙時間戳設計
- **計算 query 範式**：
  ```sql
  SELECT COUNT(*) FROM system.autonomy_level_switch_audit
  WHERE emergency_override = true
    AND switched_at_local >= (now() AT TIME ZONE current_setting('TimeZone')) - INTERVAL '30 days';
  ```

**Emergency override 30% 比率達標 action（per FA U-FA-2 混合模式）**：
- 計算公式：`emergency_override_count / total_switch_count`（rolling 30d window）
- **30% 比率達標 trigger 雙 action**：
  1. **Active alert + freeze Level toggle 24h**（per FA active 路徑）：
     - 三路通知緊急 emit「emergency override rate exceeded 30% in rolling 30d window; Level toggle frozen 24h pending governance review」
     - 24h freeze 期間任何 toggle attempt 必拒 + audit `result='emergency_override_rate_freeze'`
     - 24h 後自動解凍（per FA「比 passive monthly review 強」設計）
  2. **加入 monthly PM review queue**（per FA passive 路徑）：
     - PM 月度 review session 必看本 30% trigger 詳細 audit chain
     - PM 決定是否走 ADR amendment / 更嚴格 cooldown / 其他 governance action
- **設計理由（FA U-FA-2）**：「比 passive monthly review 強，比 active 純 freeze 軟（保留 review 路徑）」
...
- (e) **Emergency override 月度計數用 calendar month**（per Q4 明禁；rolling 30d 才能防月初規避）
- (f) **Emergency override 月度計數用 UTC time**（per Q4 明禁；machine local time 才能對齊 operator 認知）
- (g) **30% 比率達標只走 monthly review 不 freeze 24h**（per FA U-FA-2 明禁 passive-only；必雙 action）
...
| In-flight lease（已 emit 未 acquired） | **走切換前 Level**（lease 已 emit 含 lal_level + lease 內也 snapshot 切換前的 autonomy_level 字段；retroactive change 違反 ADR-0008 lease 完整性） |
| In-flight lease（已 acquired 未 settled） | **走切換前 Level**（per ADR-0008 lease lifecycle integrity） |
| Engine restart | Cold start 從 DB 讀 `current_level`；DB row 不存在或 read fail → fail-closed default = Level 1 (CONSERVATIVE) |

**讀法**：Level 切換是 **forward-looking policy change**；不 retroactive 影響既有 lease。已 emit 的 lease 帶當時的 level snapshot，繼續走完 lifecycle。

**Cache invalidation 機制（per B4 R-2 IMPL risk patch）**：

引擎內部 `Arc<AtomicU8>` cache autonomy_level；lease emit 時 `lal_level` 字段 snapshot 必從該 atomic 讀（非 DB query — 防 hot path PG round-trip）。Toggle handler PG transaction COMMIT 後必 atomic store 新值。
...
```rust
// 偽碼（per §3.2 schema 配套）
pub struct AutonomyLevelCache {
    inner: Arc<AtomicU8>,  // 0=Conservative, 1=Standard
}

impl AutonomyLevelCache {
    /// 主路徑：PG LISTEN/NOTIFY subscriber task
    pub async fn listen_loop(&self, pg_pool: &PgPool) {
        let mut listener = PgListener::connect_with(pg_pool).await?;
        listener.listen("autonomy_level_changed").await?;
        loop {
            tokio::select! {
                Ok(_notification) = listener.recv() => {
                    self.reload_from_db(pg_pool).await;  // atomic store new value
                }
                _ = tokio::time::sleep(Duration::from_secs(5)) => {
                    // Fallback (a) polling 5s — 防 NOTIFY 漏觸發
                    self.reload_from_db(pg_pool).await;
                }
            }
        }
    }
...
**反模式（明禁）**：
- (a) Toggle handler 改 in-memory cache 但不寫 DB → engine restart 後丟
- (b) 只 polling 60s 不 NOTIFY → lease emit lag 60s
- (c) 引擎 lease emit 時直接 PG query current_level → hot path PG round-trip
- (d) 切換要求 engine 重啟 → UX 摩擦 + lease 中斷

### 4.4 Notification — 三路冗餘 + 三路全 fail Escalation Ladder（per E2 Q3 + B1 補丁）
...
**三路全 fail Escalation Ladder（per E2 Q3 operator 拍板，覆寫 v1「freeze + rollback」設計）**：

```
[Stage 0] 三路全 fail 偵測（all 3 status = 'FAILED' in audit row）
   ↓
[Stage 1] Freeze Level toggle + Emit Alert
   - Toggle handler 拒絕後續任何 Level 切換 attempt 直到 escalation clear
   - Audit `result='notification_3way_fail_escalated'`
   - Engine console emit red banner「Notification all 3 channels FAILED; Level toggle frozen; awaiting operator response (1h)」
   - Triple alarm 不依賴 Slack/email/banner — 直接走 OS-level local logger + (optional) hardware buzzer 物理 alarm
   ↓
[Stage 2] 1 小時 wait for operator response
   - Operator 任何時候可介入解除（從 console / direct DB 訂正紀律 / SSH 引擎指令）
   - Operator response 必走 ADR-0006 數據訂正紀律 audit
   ↓
[Stage 3a] 1h 內 operator 介入 → Audit `notification_escalation_result='operator_responded'`
   - Operator 顯式 unfreeze + 顯式選擇下一步（resume Level toggle / 維持 freeze / etc.）
   - 不自動進 SM-04 Defensive
   ↓
[Stage 3b] 1h 內 operator 無 response → 自動進入 SM-04 Defensive（per E2 Q3「最高保護模式」）
   - 觸發 SM-04 risk_gov state transition: current_level → Defensive
   - 觸發來源：`RiskEvent::NotificationFailsafeTimeout`（new event variant；E1 IMPL 補入 risk_gov.rs）
   - SM-04 Defensive 既有語義生效（per rust/openclaw_core/src/sm/risk_gov.rs line 180-187）：
     - `new_entries_allowed=false`（停止新倉 = 停止損失）
     - `position_size_multiplier=0.0`（零新倉位）
     - `reduce_only=true`（只能 reduce_only order = close-only）
     - `active_de_risking=true`（active 主動減倉 = 鎖利路徑）
     - `emergency_stops=false`（非 emergency 平倉 — 保留持倉 active 鎖利不強制平倉）
     - `requires_operator=false`（自動進入無需 operator click）
   - Active 鎖利擴充（per E2 Q3「保住盈利」語義 — Defensive 既有 active_de_risking hook 擴充實作）：
     - Each existing position：縮 SL 至 entry + 小幅 protective buffer（per ATR）
     - 同步 sync 至 exchange-side conditional protection（per CLAUDE.md §二 原則 9 雙重防線）
     - 觸發後 emit lease 紀錄「active_lock_profit_triggered_by_notification_failsafe」
   - Audit `notification_escalation_result='auto_escalated_to_sm04_defensive'`
   ↓
...
2. **不破壞既有 ladder transition rules**：SM-04 已有 6 級（Normal/Cautious/Reduced/Defensive/CircuitBreaker/ManualReview）；新增第 7 級需重 verify 所有 transition rules（35+ pair）成本高且無新需求
3. **不誤用 CircuitBreaker**：`CircuitBreaker.emergency_stops=true` 是強制平倉行為，會破壞「鎖定 unrealized PnL」設計意圖（per E2 Q3「保住盈利」）；不宜重 reuse
4. **「Active 鎖利」由 Defensive 既有 `active_de_risking` hook 擴充實作**：不需新 enum；E1 IMPL 在 active_de_risking 真路徑加「縮 SL 至 entry + 小幅 / sync to exchange conditional order」即可

**反模式（明示禁止）**：
- (a) 三路全 fail → freeze + rollback DB row 但不 escalate（v1 設計）— 違反 E2 Q3「等 1h 等 operator」原則
- (b) 三路全 fail → auto-recovery「通道恢復後自動 unfreeze」— per HC-5 違反原則 6（已知 CC 反模式）
- (c) 三路全 fail → 立即進 CircuitBreaker 強制平倉 — 破壞「保住盈利」語義
- (d) 1h timeout → 無 escalation 繼續 wait — 違反 E2 Q3 「無 operator response → 自動最高保護模式」
...
### 5.2 UI Flow（切換）— v2 patch 整合 A3 U1+U2 / AV-9/10/11 / B1 escalation

```
[1] Operator 在 Governance tab / Autonomy Posture sub-section 點「Switch Autonomy Level」 button
    └─ Console 顯示當前 Level → 提議切到目標 Level（雙向）
    └─ 14 path × 2 level matrix 預設顯示（per A3 U2）：
        - Level 1→Level 2 升級：matrix **預設展開**（operator 需看 13 條 path 新增 auto 行為）
        - Level 2→Level 1 降級：matrix **預設折疊**（降級行為更保守，operator 已熟悉 Level 1）

[2] 系統顯式 warning modal（中文 copy per feedback_chinese_output；雙向 differential warning）：

    升級（Level 1→2）warning：
    "切換至 Level 2 (Standard / PM Path B) 將開啟 13/14 條 protected/opt-in
     governance path 的自動執行（AMD-2026-05-21-01 v2 §Decision 1）。
     5-gate operator role HMAC 簽署 + venue change 兩條維持 manual。
     此變更影響整個系統的 autonomy posture 設置。
     請於下方輸入 'CONFIRM SWITCH' 以確認切換。"

    降級（Level 2→1）warning：
    "切換至 Level 1 (Conservative / CC stance) 將回收 4 條既有 auto path
     (a)/(c)/(d)/(f) 至 operator manual approve；已 in-flight lease 不受影響
     繼續走 Level 2 行為。此變更影響整個系統的 autonomy posture 設置。
     請於下方輸入 'CONFIRM SWITCH' 以確認切換。"

    └─ Operator 輸入確認字串：**`CONFIRM SWITCH`**（per A3 U1 拍板；兩方向統一 phrase；
       與既有 governance bulkAudit `CONFIRM` 區隔但簡潔）
    └─ 字串 case-sensitive 比對；錯字串 → 切換 abort + 寫 audit `result='typed_confirm_mismatch'`（informational）

[3] Reason 輸入（per A3 spec ≥30 字元，覆寫 v1「≥20 字元」）：
    - **Dropdown** 預設 reason 選項（per A3 spec）：
      [evidence baseline 達標] / [PM 月度 review 決議] / [emergency operational] / [其他自由文本]
    - **Free text** 補充說明 ≥30 字元必填（即使選 dropdown 也需補充）

[4] 2FA confirm modal：
    "請輸入 TOTP 6-digit code"
    - **僅接受 TOTP**（per A3 anti-pattern：禁 hardware key with no 6-digit fallback；禁 'Remember device 30d'）
    - Verify 結果寫 audit twofa_verify_result
    - **AV-11 fail-mode**：2FA backend timeout/unreachable → **fail-closed**
      - audit `twofa_verify_result='FAIL', twofa_method='backend_unreachable', result='twofa_backend_down'`
      - 切換 abort + emit alert + freeze 操作 5 min

[5] System verify（按順序）：
    (a) Operator role check（CLAUDE.md §四 5-gate B）
    (b) 2FA result PASS
    (c) **PG advisory lock 取得**（per AV-10 race patch）：
        - `BEGIN; SELECT pg_try_advisory_xact_lock('autonomy_level_switch'::regtype::int);`
        - 取得失敗（另 session 持鎖）→ audit `result='race_lost'` + 切換 abort
        - 取得成功 → 繼續
    (d) 24h cooldown check（query last_switched_at FOR UPDATE）：
        - `SELECT last_switched_at FROM system.autonomy_level_config WHERE id=1 FOR UPDATE;`（row lock + advisory lock 雙重保護）
        - PASS → 進 step (e)
        - FAIL → 顯示「24h cooldown active；emergency override path?」
                 ├─ Yes → 走 emergency override path（補 emergency_override_reason 後進 step (e)）
                 └─ No  → 切換 abort + 寫 audit `result='cooldown_blocked'`
    (e) Freeze state check（per §7.4）：
        - 查 governance freeze state（regime/Guardian/M3/M7 等 trigger）
        - Freeze active → 切換 abort + 寫 audit `result='freeze_active_block'`
    (f) Emergency override rate check（per FA U-FA-2 + Q4 rolling 30d machine local）：
        - 計算 `emergency_override_count / total_count` for last rolling 30d in machine local time
        - ≥ 30% → 切換 abort + emit alert「emergency override rate ≥30%; toggle frozen 24h + added to monthly PM review queue」
        - audit `result='emergency_override_rate_freeze'`
    (g) System assertion：current_level != target_level（防 no-op switch）

[6] System execute（per AV-9 single PG transaction wrap，原子性保證）：

    BEGIN;
        -- (a) PG advisory lock 已在 step 5(c) 取得（仍在同一 transaction）
        -- (b) UPDATE config row
        ...
        -- (d) PG NOTIFY for engine cache invalidation
        NOTIFY autonomy_level_changed;
    COMMIT;

    -- 任一 statement FAIL → 全 ROLLBACK（per AV-9 atomic）
    -- E.g., INSERT audit 因 disk full 失敗 → UPDATE config 也 ROLLBACK

    (e) Engine listener task SUBSCRIBE 收到 NOTIFY → reload atomic store new value（per §4.3 B4 cache invalidation；≤ 200ms latency）
    (f) Emit Slack + email + Console banner 三路通知（≤ 10s / ≤ 60s / ≤ 5s SLA per AMD v2 §Decision 3.1）
        - 三路全 fail → 進 §4.4 escalation ladder（Stage 1-4）
    (g) Console banner 顯示「Autonomy Level: Level X (Conservative/Standard) -
        switched 2026-XX-XX HH:MM UTC (local: HH:MM ±offset) by <actor>」
        - 24h persistence + BroadcastChannel cross-tab broadcast（per A3 spec）
        - 3-stage fold pattern（initial 展開 / 1h 後 collapse / 24h 後 hide）

[7] 引擎下一個 lease emit 時 honor 新 Level（cache invalidation 收斂後）
```

**Confirm button 5s delay-enabled**（per A3 anti-pattern AP-3：button initially disabled / 5s countdown enable）

**Console banner cooldown countdown**（per A3 spec）：
- 雙時間戳顯示：UTC `2026-XX-XX HH:MM` + Local `HH:MM (TZ±offset)` 
- Cooldown countdown 進度條 `XXh XXm remaining until next switch eligible`
...
### 5.4 Level 2 Toggle Disabled State（per FA U-FA-1 + F3 新章）

**設計目標**：Level 2 schema + DB row 可現在 land（V099 / `system` schema not blocked），但 **GUI toggle button to Level 2 必 disabled** until evidence baseline 達標。理由 = Level 2 啟用受 P0-EDGE-1 阻（per FA gating）。

**Disabled state 設計**：

| 元素 | 設計 |
|---|---|
| Toggle button visibility | GUI 顯示 toggle button 但 disabled（per FA U-FA-1 「顯示但 disabled」非 hide）|
| Disabled visual cue | Button greyed-out + lock icon + 不可點 |
| Tooltip on hover | 顯示具體 unmet 條件 + 達標進度（per F3 spec） |
| Tooltip 內容範例 | "Level 2 啟用受以下條件 gating（per FA U-FA-1）：<br/>1. 21d demo 穩定期：✅ DONE (23d 累積 / >21d 達標)<br/>2. 5 textbook 策略 N ≥ 30：⚠️ 4/5 達標（grid 374 / ma 167 / bb_breakout 27 / bb_reversion 4 / funding_arb dormant）<br/>3. Wilson CI 95% lower bound 正向：❌ PENDING (waiting on Phase B/C/D + A 群 alpha source)" |

**達標檢查邏輯（runtime query）**：
```sql
-- C-1：21d demo 穩定期
SELECT COUNT(DISTINCT date_trunc('day', timestamp)) FROM learning.demo_fills
WHERE timestamp >= now() - INTERVAL '30 days';  -- ≥ 21 PASS

-- C-2：5 textbook 策略各 N ≥ 30
SELECT strategy_id, COUNT(*) AS n FROM learning.edge_estimate_snapshots
WHERE engine_mode IN ('live', 'live_demo') AND created_at >= now() - INTERVAL '30 days'
GROUP BY strategy_id HAVING COUNT(*) >= 30;
-- 必含 grid / ma / bb_breakout / bb_reversion / funding_arb 全 5 條

-- C-3：Wilson CI 95% lower bound 正向
-- 每 strategy 算 winrate Wilson CI lower bound（per QC convention），全 5 條必 > 0.5
```

**達標後自動 enable**：
- Engine 定期 cron（每小時）run 達標查詢；達標 → UPDATE `system.autonomy_level_eligibility` (new table TBD or 加 column to config)
- GUI poll eligibility 狀態 → 達標時 toggle 自動 enable
- 同時 emit Slack alert「Level 2 enable 條件達成（21d + 5 策略 N≥30 + Wilson CI 正向），可考慮切換 Level 2」

### 5.5 A3 8 anti-pattern 寫入（per A3 audit）

| AP-# | 反模式 | Mitigation in spec |
|---|---|---|
| **AP-1** | Confirm button 立即可點（無 delay）→ 衝動切換 | 5s delay-enabled（per §5.2 [6] confirm button countdown）|
| **AP-2** | Typed-confirm phrase 過長過複雜（如 'CONFIRM SWITCH TO LEVEL 2 STANDARD'）→ operator copy-paste 紀律破壞 | 簡潔 `CONFIRM SWITCH`（per U1）兩方向統一 |
| **AP-3** | Warning copy 含模糊措辭（如 'this might affect ...'）→ operator 不知道實際影響 | 雙向 differential warning 明示 14 path 中 N 條變化（per §5.2 [2]）|
| **AP-4** | Reason 字段只接 dropdown 不接自由文本 → audit 無法分辨真實意圖 | Dropdown + ≥30 字元 free text 補充並存（per §5.2 [3]）|
| **AP-5** | 2FA 接 'Remember device 30d' 選項 → 紀律降級 | 禁 'Remember device'（per §5.2 [4]）|
| **AP-6** | Banner 切換後立即消失 → operator 看不到歷史變更 | 24h persistence + BroadcastChannel + 3-stage fold（per §5.2 [6](g)）|
| **AP-7** | 14 path matrix 一律 hide 折疊 → operator 無法看 13 條 path 新增的 auto 行為 | Level 1→2 升級時預設展開（per U2）|
| **AP-8** | Cooldown countdown 只顯示 UTC → operator 不易換算 local | 雙時間戳 UTC + local（per §5.2 [6](g) + §3.3 雙時間戳設計）|

### 5.6 Read-only display 設計

| 顯示元素 | 內容 |
|---|---|
| Console 全局 banner（top right corner） | "Autonomy Level: Level X" + 顏色 code（Level 1 = blue / Level 2 = amber）|
| Governance tab Autonomy Posture sub-section header | "Current Level: X (Conservative/Standard) - Last switched: YYYY-MM-DD HH:MM UTC (local HH:MM TZ±offset) by <actor> - Reason: <reason>" |
| Autonomy Posture sub-section detail panel | 14 path × 2 level 完整對照表（per §2.1）+ 5-gate 永鎖 + venue change 永鎖 marker + Level 2 eligibility progress（per §5.4） |

### 5.7 GUI handler 設計約束
...
- **Tab 位置**：tab-governance.html 新增「Autonomy Posture」sub-section（per A3 audit + §5.6 header location）
...
| **AV-9 [CRITICAL]**（per E2 BLOCK B3） | DB UPDATE + audit INSERT split-brain — UPDATE config 寫成功但 INSERT audit 失敗（disk full / CHECK violation）→ config 已改但 audit 缺 row | **PG transaction wrap atomic**：spec §5.2 step 6 必 `BEGIN; UPDATE config; INSERT audit; NOTIFY; COMMIT;` 包裹；任一 statement FAIL 全 ROLLBACK；E4 regression mock disk full 必驗證 UPDATE 也 rollback |
| **AV-10 [CRITICAL]**（per E2 BLOCK B3） | Two-operator session race — 兩 session 同時走 §5.2 flow，兩個都過 cooldown check（看到 last_switched_at = 25h ago）→ 兩個 UPDATE 重疊 race | **PG advisory lock + SELECT FOR UPDATE row lock 雙重保護**：spec §5.2 step 5(c)(d) `pg_try_advisory_xact_lock('autonomy_level_switch'::regtype::int)` + `SELECT ... FROM config FOR UPDATE`；race 中 1 win 1 fail；audit 兩 row：win = `result='success'`，lose = `result='race_lost'` |
| **AV-11 [HIGH]**（per E2 BLOCK B3） | 2FA backend down fail-mode 未明示 → 攻擊者 DoS 2FA backend 後伺隙切換 | **Fail-closed**：2FA backend timeout/unreachable → 切換 abort + audit `twofa_verify_result='FAIL', twofa_method='backend_unreachable', result='twofa_backend_down'`；E3 security review 確認 2FA backend timeout SLO ≤ 5s |

### 8.2 Mitigation 層級總結

- **層 1 — DB constraint**：CHECK / REVOKE / PRIMARY KEY / INDEX uniqueness / **PG ENUM type**（per MIT Q1）/ **PG advisory lock + FOR UPDATE row lock**（per AV-10）
- **層 2 — Atomic transaction**：UPDATE + INSERT + NOTIFY 單 PG transaction wrap（per AV-9）；任一 FAIL 全 ROLLBACK
- **層 3 — Engine 代碼 hard-coded**：§Decision 2 5 條 fail-safe + Level toggle 自身不繞 fail-safe（per §7.5）
- **層 4 — Auth gate**：Operator role + 2FA (TOTP only, fail-closed on backend down per AV-11) + Console warning + HMAC chain
- **層 5 — Audit immutability**：Append-only + ADR-0006 訂正紀律 + monthly PM review + 雙時間戳（per Q4）
- **層 6 — Notification + Escalation**：Slack + email + Console banner 三路冗餘 + 三路全 fail → 1h wait → SM-04 Defensive escalation（per E2 Q3 + B1）
- **層 7 — Time-based throttle**：24h cooldown + emergency override rolling 30d machine local（per Q4）+ 30% rate freeze 24h + monthly PM review（per FA U-FA-2）+ freeze period block
- **層 8 — Cold start fail-closed**：DB read fail → Level 1 CONSERVATIVE（per §5.3）
- **層 9 — Cache invalidation**：PG LISTEN/NOTIFY 主路徑 + polling 5s fallback（per B4 R-2）；防 hot path PG round-trip 也防 lease emit lag
- **層 10 — Eligibility gating**：Level 2 button disabled until evidence baseline 達標（per FA U-FA-1 + §5.4）
...
| **9.5** | GUI Governance tab **Autonomy Posture** sub-section toggle component | A3 design + E1 IMPL（per A3 audit 估時 21-28 hr）：Console banner real-time display + Autonomy Posture sub-section UI flow（per §5.2）+ 14 path × 2 level 對照表（升級展開/降級折疊）+ read-only display + warning modal 雙向 differential 中文 copy + typed-confirm `CONFIRM SWITCH` + 2FA TOTP-only + reason dropdown+free text ≥30 字元 + emergency override path + cooldown countdown 雙時間戳 + 三路通知 emit + 8 anti-pattern 避免 + Level 2 disabled state with progress tooltip + 5s delay-enabled button + 24h banner persistence + BroadcastChannel cross-tab；JS `node --check` sign-off | **21-28** | A3 + E1 + E3（auth path）+ E2 |
...
| ~~**9.7**~~ | ~~CLAUDE.md §四 baseline~~ | **REMOVED**（per operator Q1 amendment 並存原則 — CLAUDE.md 不動；amendment via AMD-2026-05-21-01 v2 並存即可）| 0 | — |
| **9.8**（new） | SM-04 ladder mapping「最高保護模式」對齊 + `RiskEvent::NotificationFailsafeTimeout` 新事件變體 | **PA 拍板 reuse `Defensive` + active 鎖利 hook 擴充**（不新增 ULTRA_DEFENSIVE）；E1 在 `rust/openclaw_core/src/sm/risk_gov.rs` 加 `RiskEvent::NotificationFailsafeTimeout` 變體 + Defensive `active_de_risking` hook 擴充「縮 SL 至 entry + sync to exchange conditional」；E4 regression 三路通知 fail → 1h timeout → SM-04 Defensive transition 驗證 | 6-10 | PA + E1 + E4 |
| **9.9**（new） | TODO.md §0.5 / §1.4 staging entry + docs/README.md index 補本 spec | 本 spec land 為 staging entry；docs/README.md 加 reference；TODO §1.4 D+2-D+3 entry 加「review Autonomy Level Toggle design spec」+ §0.5 加本 spec V### migration staging | 1-2 | PM + TW |

**合計 cascade patch 估時**：**46-71 hr**（v1 42-64 hr + 9.8 SM-04 6-10 hr - 9.7 removed 2-3 hr），並行 5-7 sub-agent → wall-clock **~1.5-2 working days**（含 V### PG dry-run + SM-04 Rust patch）。

**並行 wave 設計（v2 patch）**：
- **Wave 1**（並行，~2-3 hr wall-clock）：9.1 + 9.3（TW / BB 並行；不撞文件）— 9.7 CLAUDE.md 已移除
- **Wave 2**（並行，~4-6 hr wall-clock）：9.2 + 9.6（PA + R4 並行；ADR cross-ref 路徑）
- **Wave 3**（並行，~8-12 hr wall-clock）：9.4 + 9.5（MIT/E1 PG dry-run + A3/E1 GUI；不撞 PG schema layer 與 GUI layer 互斥）+ 9.8（PA/E1 SM-04 Rust patch；獨立 module 不撞 GUI/PG）
- **Wave 4**（serial，~1-2 hr wall-clock）：9.9（TODO/README index 最後 reconcile）
...
## §11 Operator Readiness Checklist for Level 2 Enablement（per FA F1 / 10 條）

Level 2 從 disabled toggle 進入 enabled 切換的 readiness gating，operator + system 雙端 SOP：

| # | Criterion | 驗證來源 | Pass 條件 |
|---|---|---|---|
| **C-1** | 5-gate operator role auth active | `auth/role_check.py` runtime probe | Operator role token valid + non-expired |
| **C-2** | 30d emergency override 使用率 ≤ 30% | `system.autonomy_level_switch_audit` query（rolling 30d machine local time，per Q4） | `emergency_override_count / total_count < 0.30` |
| **C-3** | 7d Guardian alert 數 = 0 active | `learning.guardian_alerts` query last 7d | 0 active alert（resolved/expired 不計）|
| **C-4** | 所有 Stage 3+ 策略 21d 穩定期達成 | `learning.strategy_stage_state` table query | 全 Stage ≥ 3 策略 `current_stage_since >= now() - 21 days` |
| **C-5** | §Decision 2.4 freeze trigger 6 條全 clear | M3 health domain + M7 lifecycle + Guardian + 5-gate kill + M8 anomaly + regime change 並聯查 | 6 條全 false/non-active |
| **C-6** | 24h 內無切換（per §4.2 cooldown） | `system.autonomy_level_config.last_switched_at` | `now() - last_switched_at >= INTERVAL '24 hours'` |
| **C-7** | 三路通知通道健康 | Slack / email / Console banner health probe（last 1h） | 至少 1 路 healthy；建議 ≥ 2 路 |
| **C-8** | 5 textbook 策略 evidence baseline N ≥ 30 | `learning.edge_estimate_snapshots` query（per §5.4 C-2 query） | grid + ma + bb_breakout + bb_reversion + funding_arb 全 5 條 N ≥ 30 |
| **C-9** | V### schema apply 成功 | `db/migrations` log + Linux PG `\dt system.*` verify | `system.autonomy_level_config` + `system.autonomy_level_switch_audit` 兩 table 存在 + ENUM type 存在 |
| **C-10** | Operator forgetfulness mitigation 自評 | Operator 自評問卷 / PM 月度 review record | Operator 確認可記住 14 path × 2 level 主要差異 |

**自動檢查 frequency**：每小時 cron run C-1 ~ C-9（C-10 monthly PM review）；達標時 emit Slack alert + 自動 enable Level 2 toggle（per §5.4）。

---

## §12 Acceptance Criteria for IMPL（per FA F2 / 8 條）

E1 IMPL + E4 regression 必驗 8 條 acceptance criteria：

| # | Acceptance Criterion | E4 regression test |
|---|---|---|
| **AC-1** | DB default Level 1 | Fresh PG instance apply V### migration → `SELECT current_level FROM system.autonomy_level_config WHERE id=1` 必返回 `'CONSERVATIVE'` |
| **AC-2** | 第一次 Level 1→2 audit trail 完整 | Mock operator 走完 §5.2 flow → audit row 含 actor + before/after + 2FA result + reason + 雙時間戳 + 三路通知 status |
| **AC-3** | 24h cooldown 強制 | 切換成功 1 次後立即再嘗試切換 → 必拒 + audit `result='cooldown_blocked'` |
| **AC-4** | Emergency override governance review trigger | Mock 30d rolling window 中達 30% emergency override 比率 → engine emit alert + freeze 24h + monthly PM review queue entry |
| **AC-5** | Engine cold start fail-closed default Level 1 | Mock DB row corrupted / read fail → engine 啟動讀 fail → 自動 default Level 1 + Console banner red warning |
| **AC-6** | Freeze 期間 Level toggle 拒切換 | Mock M7 lifecycle DECAY_ENFORCED active → §5.2 [5](e) freeze state check 必 FAIL → audit `result='freeze_active_block'` |
| **AC-7** | Level 2 (a)(c)(d)(f) 4 條 individual lease emit | Mock Level 2 active 期間 4 條 path 各 emit decision → 必有 4 個 independent lease row（per ADR-0034 §Decision 1，不接受 umbrella lease）|
| **AC-8** | In-flight lease 不受切換影響 | Mock 1 lease emit at Level 1（含 autonomy_level='CONSERVATIVE' snapshot）→ Level 切到 2 → lease lifecycle 走完 settled 仍按 Level 1 行為（per §4.3） |

**所有 8 條必 PASS 才能 sign-off Level toggle IMPL**。

---

## §13 M3 Health Monitor 新增 Check（per MIT integration / §6）

M3 health monitor 新增 `check_autonomy_level_switch_recent_24h()`：

- **檢查**：24h 內 Level 切換次數 ≥ 2 → CRITICAL
- **理由**：24h 內 2 次切換意味 emergency override path 被觸發 ≥ 1 次；或者 audit chain 有異常 multi-actor race；任一情況都應 emit alert
- **query 範式**：
  ```sql
  SELECT COUNT(*) FROM system.autonomy_level_switch_audit
  WHERE switched_at_utc >= now() - INTERVAL '24 hours'
    AND actor_role = 'operator';  -- 不算 system_default cold start
  ```
- **CRITICAL trigger**：count ≥ 2
- **Action**：emit Slack alert + freeze Level toggle 1h（與 emergency override 30% freeze 互斥；取較長者）+ 加入 PM monthly review queue
- **IMPL 位置**：`rust/openclaw_engine/src/health/m3_monitor.rs`（既有 module 加新 check fn）

---

## §14 PA Unresolved Design 問題（needing operator confirm）

### ~~Q1 — Emergency override path 設計範圍~~ ✅ RESOLVED by FA U-FA-2 + E2 Q4

**Operator 2026-05-22 拍板（FA U-FA-2）**：混合模式 = 30% 比率達標 → active alert + freeze 24h + monthly PM review queue（per §4.2 + §8.1 #6）；rolling 30d machine local time（per Q4）。

**結果**：採 Path C 強化版（FA 混合）；不採 Path A 純 passive 也不採 Path B 硬上限；hard-coded 進 §4.2 + §3.3 audit schema + §8.1 #6。
...
✅ **RESOLVED**：Path A 採納（§5.3 cold start fail-closed default Level 1）。

### Q3（new）— SM-04 ladder mapping「最高保護模式」reuse Defensive vs 新 ULTRA_DEFENSIVE

**背景**：§4.4 三路通知 fail escalation 進「最高保護模式」，需對齊 SM-04 ladder。

**選項**：
- **Path A**（PA 拍板）：reuse 既有 `Defensive` + active 鎖利 hook 擴充
- **Path B**：新增第 7 級 `ULTRA_DEFENSIVE`（active 鎖利 + close-only + tighten SL + freeze new position）

**PA 拍板 Path A**（per §4.4「SM-04 ladder mapping 決策」4 條理由）。
...
## §15 與 PM v2 Draft §Decision 2 5 Hard Requirements 對齊驗
...
*OpenClaw / 玄衡 Arcane Equilibrium — Autonomy Level Toggle Design Spec v2 — IMPL 設計依據 for AMD-2026-05-21-01 v2 §Decision 1 Q2 拍板*

*v2 整合 operator 2026-05-22 6 拍板（A3 U1/U2 + E2 Q3/Q4 + FA U-FA-1/-2 + MIT Q1/Q2）+ E2 BLOCK B1-B5 補丁 + FA F1-F3 補完 + A3 8 anti-pattern 寫入 + 新增 §11 readiness / §12 AC / §13 M3 health check / §14 Q3-Q4*
