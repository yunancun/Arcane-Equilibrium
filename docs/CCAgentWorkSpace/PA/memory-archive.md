# PA Memory Archive（append-only：壓實遷入原文，勿刪改）

--- 2026-06-10 壓實遷入（原 memory.md 第 1-7242 行）---
...
## 2026-06-03 — FND-2 PIT universe builder IMPL 設計（AEG-S2 critical-path 前置）
- **最 load-bearing 教訓（DB reflection 推翻契約字面）**：FND-2 contract §3 算法把 `first_seen_ts`/`last_seen_ts`（snapshot `ts` 衍生）當 `listed_at`/`delisted_at` 的 coalesce fallback + lifetime 邊界。但 Linux 親驗 `market.symbol_universe_snapshots`：snapshot `ts` **只跨 27 天**（2026-05-07→2026-06-03），而 `listed_at` 跨 **2018→2026**、`delisted_at` 跨 **2022→2026**。對 18mo 窗，`coalesce(listed_at, first_seen_ts)` = trap：會把 2024 上市 symbol 的 alive_from 錯夾到 2026-05-07，抹掉真實上市歷史。→ **`listed_at`/`delisted_at` 是唯一 lifetime 權威；ts 僅診斷 + 兩權威欄全 NULL 時標 `unknown_lifetime`（不 coalesce 到 ts）**。E1 須照 PA 修正算法（報告 §4 Step D），不照契約字面。**再次印證「文檔/契約數字須 grep+reflection 核查」**（文檔說 ~293 symbol/144k row/24d，實值 937 symbol/508k row/27d；USDT-perp 852=571 alive+281 delisted）。
- **lifecycle 一致性穩健**：latest-per-symbol 證 `is_delisted_at_asof` ⟺ `delisted_at IS NOT NULL` ⟺ `status='Closed'` 三者等價（0 矛盾），故 `seen_delisted` bool_or 形式穩健、survivor-rejection gate 可機械化。raw status 只有 Trading/Closed/PreLaunch（無 Delivering/Settled/Delisted）→ §3 那些分支現為 dead-but-forward-defensive。
- **架構決策**：放 `helper_scripts/research/fnd2_pit_universe/`（package-dir，mirror 既有 `multiday_trend_diagnostic/`+`funding_tilt_diagnostic/` sibling），CLI via harness.py。**不 lib**（有業務語義非 byte-identical helper）/**不 bin**（離線研究非 runtime collector）/**Rust-first 例外**（CLAUDE §七「unless local design says otherwise」：one-shot read-only artifact builder，下游 AEG-S2 (a)/(b)/(c) 全 Python batch+parquet，跨語言 0 收益；須 PM sign-off OQ-1）。核心切分：`builder.py` 純函數（0 DB，synthetic 可測），`data_loader.py` 只 SELECT。
- **forbidden function 代碼證據**：`full_chain_fixture.py:745-863 fetch_historical_universe_snapshot_sync` 同時違反 FND-2 §5 三禁：(1) `LIMIT max_symbols`=truncation (2) `WHERE NOT (is_delisted_at_asof AND delisted_at<start)`+cap=survivor 傾斜 (3) `ORDER BY turnover_24h DESC`+LIMIT=用 current liquidity 截斷。`replay_full_chain_routes.py:299-311` current_scanner_fallback 亦禁。FND-2 自寫 SQL：對稱 lifetime∩window inclusion（含窗內 delisted）、turnover 只排序 tier 不截斷、無 fallback。
...
## 2026-06-03 — AEG-S2 Component (b) Breadth Ladder Runner IMPL 設計（artifact-only，無 V127 依賴）

- **最 load-bearing 設計判斷（FND-2 IMPL code 推翻 naive tier 直覺）**：(b) 的 4 breadth tier 組裝**必須**用 FND-2 universe artifact 的 `cohort_ids`（array，multi-membership，`cohorts.py:93 cohort_ids_for`），**NOT** `recommended_tier`（single-pick priority，`cohorts.py:69 classify_recommended_tier`）。FND-2 `recommended_tier` 對每 symbol 只給一個 tier（core25 > scanner > top_liq > full 優先序裁決）→ BTCUSDT 只會是 `core25_pinned`，不在 `top_liquidity_40_50` 集。但 breadth ladder 語義是嵌套加寬（core25 ⊂ top_liquidity ⊂ full_survivorship），core25 成員須同時在更寬 tier。誤用 recommended_tier 會讓 tier 互斥、monotonicity 比較完全失真。**教訓：消費 sibling artifact 前必讀其 IMPL code，schema 欄名相近但語義（single-pick vs multi-membership）天差地別。**

- **第二 load-bearing（breadth ≠ n_independent，cost-wall 實證機械化）**：加寬 breadth（core25 25 sym → full 829 sym）報 `breadth_symbol_count`，但 `n_independent` 是 **time-cluster-bound**，絕不隨 symbol 數膨脹。cost-wall report（`2026-05-31--cost_wall_escape_2_multiday_trend_diagnostic.md` §51/§97-99）已實證：「binding constraint 是 independent TIME periods（weekly+ ~8 max）NOT symbol count；即使 59 long-short legs，independent time clusters = 8。」**(b) 不可沿用 multiday 的 `eff_n = pooled_flips × cluster_factor`（harness.py:337）**——該式經 pooled_flips（pooled 跨 symbol）隨 symbol 數漲，是 breadth-contaminated false-rich-sample。招牌 bite test：`n_independent(full)==n_independent(core25)`（time-cluster 不變，固定窗時間軸一致）。這是 6 週 down-beta 偽裝 edge 教訓的機械防線。

- **第三（cross-section rank PIT leak flag）**：FND-2 `top_liquidity_40_50` 的 turnover rank 來自 `data_loader._load_turnover`（latest snapshot at asof，single point）= **full-window-constant rank**。MIT b.5 要求 per-rebalance PIT rank，但 `market.market_tickers` 是 latest-only（S0 §3.1）+ index/mark persistence bug（S0 §1.7）→ **per-rebalance liquidity rank 目前無 PIT 資料源**。PA recommend：top_liquidity tier 標 `tier_quality='liquidity_source_not_pit'` + `excluded_from_promotion=true`（diagnostic-only），monotonicity 主軸用 core25/full（成員不依 liquidity，無此 leak）。除非 PM/MIT 批准 asof-constant 近似（OQ-B3）。

- **中心架構決策（candidate-agnostic）**：(b) = candidate-agnostic harness（窄 `CandidateEvaluator` protocol：`evaluate(tier, universe, alive_mask) → TierResult`），**NOT** 綁特定候選。理由：①sibling 候選 harness `run_diagnostic(panel, universe)`（multiday/harness.py:280）已 universe-parametrized，universe 換 tier symbol-set 即得 per-tier 結果，零候選改動可被 adapter 包裝；②breadth ladder 是 ADR-0047 證據基質，每個 alpha 候選晉升都要過（S0 §1.3 verdict required），綁單一候選 = 違背「可重用凍結 evidence 基質」mandate。listing-fade 是首消費者（OQ-B2），但 funding-tilt/trend/未來候選都用。survivorship mask **繼承** FND-2 `alive_from_utc`/`alive_to_utc`（MIT b.2，不重算，避開 FND-2 已修的 R-1 snapshot-ts trap）。
...
role: PA
payload_sha256: sha256:8d5bdac1b071e433203d79e0e284f93eab68b6f606c54771e439552a09c58241
payload_bytes: 129097
-->
# PA Memory — 工作記憶
...
## 2026-07-09 — IBKR B1 read-only TWS client 技術設計（design-only, E1-READY, build-now/run-G4）
- 交付 inline 回 parent（harness：不寫 report .md）。B1=connect handshake + reqCurrentTime（G4 最小首次接觸 health/server-time），**零新契約**。落點 **engine 新單檔** `rust/openclaw_engine/src/ibkr_readonly_tws_client.rs`（`#![allow(dead_code)]` 惰性慣例）。**net-free 守衛只綁 openclaw_types**（Python guard 只掃 Python 檔；engine crate 已用 tokio::net—ai_service_client/ipc_server），故 Rust 手寫 wire client 進 engine crate 架構上合規。
- **wire 協議親證（rust-ibapi source）**：framing=4-byte **big-endian** u32 長度前綴 + payload；欄位 `\0` 分隔 ASCII（如 `"49\0"`）。handshake=raw `API\0`（非長度前綴）+ framed `v{min}..{max}`；server ACK=framed [server_version, connection_time]；START_API=framed [71,2,client_id,""]；reqCurrentTime=framed [49,1]→CURRENT_TIME=framed [49,1,epoch_s]。**≥213 floor 有 protobuf(msg_id+200)**，故 MAX client ver 保守 pin 避免協商 protobuf-only。
- **關鍵 G4 風險（handshake 不需 wire creds）**：reqCurrentTime 握手**不經 wire 認證**——IB Gateway 須**帶外預登入 paper 帳號**(4002) socket 才 up，client 只送 client_id 無憑證。故 B1 真 run 的 G4 依賴=Operator 有 running+logged-in paper Gateway（IBC/手動），**非 code 需憑證**；P1 secret slot 只供 attestation 指紋欄，B1(reqCurrentTime 無帳號回傳)連指紋都非必要。
- **codec/socket 分離（測試性關鍵）**：純 codec（encode/decode frame+fields+handshake+start_api+req_current_time+decode ack/current_time；typed `CodecError` no-panic）default build 編譯+synthetic fixture 單測，**零 socket**。driver `async fn drive<S: AsyncRead+AsyncWrite+Unpin>` generic over stream—default 編譯、測試用 `tokio::io::duplex` 餵合成 server frame（無 gateway）。**唯一 concrete socket `TcpStream::connect(127.0.0.1:4002)` + G4 bin 鎖 `required-features=["ibkr_g4_contact"]`**（鏡像 replay_runner）→ default `cargo build` **物理上編不進 socket 開啟碼**（compile-gate 強於 dead_code）。
- **三層惰性 gate**：compile-gate(feature)∧runtime-gate(`phase2_immutable_pass_artifact_present()` re-verify ∧ 新 G4 owner-only approval 檔，鏡像 P2 approval A-model 但獨立 token)∧structural-gate(host/port **硬編**拒 4001/7496 非 config)。G4-run 機制=gated bin（dry-run 預設 + 顯式 --contact + env gate），Operator 顯式觸發，QA 捕 runtime 證據。
- **FA #4 裁決=不加 health row**：B1 用 engine-private `TwsHealthProbeResult{server_version,connection_time_raw,server_epoch_seconds,...}`（impl struct 非契約、無 validate、不跨 IPC/DB）+ 投影 subset 進既有 `IbkrApiSessionTopologyV1`(api_server_version_recorded/host/port/gateway_mode/deterministic_client_id_present)。**full validate-passing topology/attestation 是 P5**（AMD gate 表：attestation=P5）；B1 只產 partial 健康快照供 QA capture 不 seal/persist。加 health row=新 S3/S5 契約需 acceptance+normalizer lockstep=正是 B2/B3 延後工作。
- **E1 派工=單 E1**（codec+driver+guards+gate+mock 測試緊耦合單檔，拆分負收益；~700-800 LOC 過半測試，遠低 2000）。E2 3 審=①no-order/write 源級 grep=0（結構性無寫方法）②host/port 硬編拒 live port（非 config 可繞）③default build 無 concrete socket（feature-gate 生效）。scope：不接 IPC/dispatch/normalizer(P4)、不 auto-run、無 production caller、Bybit 不變、無 DB migration。

## 2026-07-09 — IBKR P2 WAIVER-gated T1+T2 合併技術設計（design-only, E1-READY, 單一 E1 原子改）
- 交付 inline 回 parent（harness：不寫 report .md）。**T1 IBKR-P2-SEAL-LINEAGE-FIELDS**=給 `IbkrPhase2GateArtifactV1` 加 2 必填欄 `contact_authorization_amd:String`+`approval_lineage_hash:String`（受 ticket 授權解凍凍結型別，非違 net-free：加欄無網路 I/O）。validate() 加 2 blocker：contact AMD==新常量 `IBKR_PHASE2_CONTACT_AMD="AMD-2026-07-08-01"`（放 gate.rs 常量、lib.rs export；producer 私有 `PHASE2_CONTACT_AMD` 併入單一真源）、lineage hash==is_sha256_hex（types 只驗 shape 不重算，同 raw/redacted hash 慣例）。**Default 裁決**=contact_authorization_amd 默認=常量（同 adr/amd 慣例，default blocker vec 不新增此項）、approval_lineage_hash 默認=""（→default vec 只新增 `ApprovalLineageHashInvalid` 一項，插在 RedactedSummaryHashInvalid 後）。
- **raw hash 自動覆蓋新欄**（compute_raw_artifact_hash 全 serde 序列化，加欄即入覆蓋域，0 改）；redacted_summary_value 手建 json! **須手動加 2 key** 才覆蓋。approval_lineage_hash canonical=新 `compute_approval_lineage_hash(&Phase2SealApproval)`=sha256(明確 key 序 json!{adr,amd,sorted(reviewer_roles),approved_source_commit,issued/expires}); 僅 approval_valid 時填欄，否則 ""（fail-closed，連坐既有 OperatorReviewerMissing）。
- **T2 IBKR-P2-TRIANGULATION-CROSSCHECK**=移除 build_phase2_artifact_candidate :322-323 無條件覆寫（topo.account_fingerprint_hash=secret.fp），改用 topology 自身 fp。真 equality 機制 `account_fingerprint_triangulation_ok`(:367,非空+相等) **已存在**且 seal(:606)已呼叫——覆寫是唯一遮蔽點。移除後 production：denied secret fp=""(loader:508) vs topo source_template fp="c"*64→triangulation false→BLOCKED（正確 never-seal）。**P5 殘餘**=topo fp 仍 placeholder，真獨立源待 P5 session-attestation；:317 TODO 由「移除覆寫」改寫為「P5 供真 topo fp 源」。**types validate() 不加 triangulation blocker**（producer-side + matrix envelope cross-check :215 已足；加 types 變體徒增凍結型別 churn，列 optional 給 E3/CC）。
- **breaking-change 影響全清單（T1 最大風險）**：編譯破=Default impl(artifact.rs:37)+producer 結構字面(:335) 須加 2 欄；語意破（`..default()` 過編譯但 validate 失敗）=2 acceptance 檔的 `accepted_artifact_fixture` 須設 approval_lineage_hash 合法 64-hex（artifact_acceptance.rs:14/feature_flag_acceptance.rs:21）+ artifact_acceptance `default_gate_artifact_blocks_contact` vec 加 ApprovalLineageHashInvalid。T2 破=producer 測試 build_full_green 須用 matched_topology(secret) 取代 valid_topology()（覆寫沒了）。matrix Default(:80)用 ::default() 自動吸收=0 改。**跨語言=安全**：Python normalizer(stock_etf_authorization_normalizers.py:195)是 lenient .get()+default 讀「projection」非 sealed 檔 raw schema→加欄不破；無 DB migration；IPC summary(:811)shape 不變。
- **閉合**：T1 完全閉 finding-2（sealed 檔自記 contact AMD+lineage hash，raw+redacted 覆蓋，validate enforce；:204/:341 TODO 可移）。T2 閉 finding-1 遮蔽（真 equality 於 seal enforce，pre-P5 正確 BLOCKED 非 masked-true）；殘 P5 依賴（真 topo fp 源，非 waiver 缺口）。**兩 ticket 皆可標 closed**。scope=純 source-only、不 wire seal（outcome 仍永不調 seal）、不建槽、不外部接觸、Bybit 不變。E1 派工=**單 E1 原子改**（T1/T2 同改 build_phase2_artifact_candidate 同區塊+共用 topology fixture，拆分必撞，0 並行收益）；~110 LOC 多為測試。E2 3 審=①default blocker vec 精確更新②approval_lineage canonical 決定性(sorted roles)③覆寫確實移除且 build_full_green 改 matched_topology。E4=cargo test -p openclaw_types + -p openclaw_engine ibkr_phase2。

## 2026-07-09 — IBKR P2 external-surface gate producer 技術設計（design-only, E1-READY, 現況 fail-closed BLOCKED）
- 交付回 parent（harness 要求：不寫 report .md，設計 inline 回覆）。落點 **engine 新檔** `rust/openclaw_engine/src/ibkr_phase2_gate_producer.rs`（四件套風格），消費既有 `IbkrPhase2GateArtifactV1::validate()`（不新造型別）。producer 組裝 5 腿→跑 validate()→**全過才 seal write-once immutable 檔**，否則回 BLOCKED verdict + blocker 清單，**絕不 seal 假 PASS**。
- **關鍵 gotcha**：artifact.amd 必須 == `IBKR_PHASE2_AMD` 常量 = **"AMD-2026-06-29-01"**（原 AMD，非 07-08-01；07-08-01 是 CLAUDE.md policy 層授權，artifact 契約常量仍是 06-29-01）。硬編 07-08-01 → AmdMismatch → BLOCKED。
- **validate() 只驗 shape 不驗 hash 內容**（`is_sha256_hex` 只查 64-hex，不重算）→ producer 必自帶 `verify_sealed_artifact()` 重算 raw+redacted hash 比對，才有 tamper-evidence（E4「篡改 hash→reject」靠此，非 validate()）。raw_artifact_hash=sha256(pre-seal canonical JSON: hash 欄清空+sealed=false)；redacted=sha256(redacted summary，只含 hash/bool/contract_id，禁 account_id 明文)。
- **seal 機制**：候選 artifact 記憶體內 sealed=true（構造細節非 claim），validate() 全過才落檔；write-once=`create_new` tmp→fsync→`hard_link` 到 final（存在則 Err=拒二次 seal）→chmod 0o400→gov dir 0o700。source_commit 來源=`boot_observability::BUILD_GIT_SHA`（40-hex；producer 額外拒 "unknown" 雖 validate 不擋）。created_at_ms=engine SystemTime。
- **現況 GREEN vs BLOCKED**：GREEN now（免真槽）=5 policies（真載 `ibkr_phase2_policies.toml`→`toml::from_str::<IbkrPhase2PolicyBundleV1>` 直接 deser，非 source_template）/allowlist accepted_fixture/gate 靜態 posture/source_commit/hashes/topology shape(template)。BLOCKED now=secret_slot_contract（P1 loader 現回 denied fallback，真槽 Operator-gated 未建）→連帶 gate.secret_contract_present→ExternalSurfaceGateRejected；reviewer "Operator"（sign-off 輸入機制未定）；sealed（全過才）；topology account 綁定真槽。**這是正確 fail-closed**。
- **P1 消費**：需把 P1 私有 `ibkr_secret_slot_contract()`(OnceLock accessor)+`denied_ibkr_secret_slot_contract_fallback()` 提為 pub(crate)（2 行 visibility bump），P2 復用其 base 解析+快取+fail-closed，不重造。precontact.rs 只 1 行改（`immutable_pass_artifact_present` 反映 producer on-disk sealed 態）避免重複組 gate。0 熱檔、0 DB migration、0 socket、0 flag flip、Bybit 不變。rollback=撤 mod 行+precontact 1 行+P1 visibility；tempdir fixtures 測試不落真檔。
- **開放問題（PM/E3/BB/Operator 拍板）**：Q1 immutable 檔落點（建議 `<OPENCLAW_DATA_DIR>/governance/ibkr_phase2/`，但 prod DATA_DIR 預設 /tmp=ephemeral，須指持久 owner-only 路徑）；Q2 Operator sign-off 輸入機制（建議讀 owner-only approval 檔 → 才注 reviewer_roles "Operator"，producer 不可自證；shape 待定；非 authorization.json 硬邊界範圍）；Q3 topology account_fingerprint_hash 是否綁 P1 真槽 hash（建議 yes）。E1 派工=單 E1 一波（producer 為 build+validate+seal+verify+summary 緊耦合單元，拆分負收益）。

## 2026-07-09 — IBKR P1 fingerprint-only SecretSlotLoader 技術設計（design-only, E1-READY）
- 交付 `docs/CCAgentWorkSpace/PA/workspace/reports/2026-07-09--ibkr_p1_secret_slot_loader_tech_design.md`（Operator/ 副本已放）。消費既有 `IbkrSecretSlotContractV1`（openclaw_types/ibkr_phase2_runtime.rs），**落點 engine 新檔** `rust/openclaw_engine/src/ibkr_secret_slot_loader.rs`（net-free guard 禁 openclaw_types）。同構 P0 risk-policy loader 四件套（handlers/stock_etf.rs:408-468）：純 loader 收 base dir 參數 + env wrapper + OnceLock + denied fallback。**0 熱檔改動**（僅 lib.rs +1 mod 行），P1 不接 matrix/IPC/Python/DB、不 enable flag、rollback 零下游斷裂。
- **指紋裁決**：`secret_slot_fingerprint`=sha256(**paper 槽**描述符=present+dir_mode+sorted[檔名+mode+內容sha256])，非聚合 readonly+paper（否則 P5 paper session 對不齊）；`account_fingerprint_hash`=sha256(trim+upper(paper/account_id))。**對齊充要條件=抽單一 `pub(crate)` 純函數 P1 建 P5 用**（同輸入同算法），非各腿各自實現。明文零逃逸**成立**：struct 全欄位 enum/bool/64-hex 無處容明文 + secret_content_serialized/account_id_serialized 恒常量 false + 局部讀→hash→出 scope + T7 序列化子串斷言。zeroize workspace 無（建議手動 Drop 歸零，威脅模型 LOW 非 blocker）。
- **live-absent**：live 槽永不讀內容/不算指紋，只列目錄；含材料→LivePresentDenied+false→fail-closed。**運行時 caveat**：三槽全 absent 無法區分「對目錄空」vs「錯目錄」→假 live-absent；readonly/paper 為正控證 base dir 正確才可信（P5/G4 關注）。
- **開放問題（拍板）**：Q1 trade-core `OPENCLAW_SECRETS_DIR` 實值（bybit 慣例=secret_files/bybit 根 vs AMD 字面=OPENCLAW_SECRETS_DIR/external/ibkr，不確認→loader 找錯目錄→假 live-absent）；Q2 paper/account_id 檔名+格式須==IBKR 回傳帳號原樣（否則四方 hash 對不齊）；Q3 指紋政策內容摘要 vs stat-only(mode+len)（描述符 builder 單點可切）。E1 派工=單 E1 一波（檔零重疊，拆分負收益）。

## 2026-07-08 — IBKR stock_etf real read-only backend 可行性設計（PA leg, design-only）
- 交付直接回 parent（無 report 檔，harness 要求）。read-only/zero-money/live-denied(ADR-0048)。核心裁決：**型別權威 scaffold 已 90% 建好**（`stock_etf_risk_policy.rs`/`ibkr_feature_flag_secret_auth.rs`/`ibkr_phase2_runtime.rs`/`ibkr_phase2_gate.rs` 全含 `source_template()`=真值範本 + `validate()`；缺的只是「產生真值的 runtime」）。
- **IB client 落點=Rust openclaw_engine 新 gated module**（非 Python skeleton、非 openclaw_types）：Python `broker_connectors/ibkr_connector/` 全樹 + control-api `*ibkr*/*stock_etf*` 被 `test_stock_etf_python_no_write_static_guard` AST-guard 鎖死（禁 socket/asyncio/time/os.environ/ibapi/ib_insync）；rust 端 guard 只 pin `openclaw_types/src/ibkr_*.rs` 型別檔（禁 std::net/TcpStream/tokio::net），**不掃 engine 模組**→真 TWS-API client 只能進 engine crate。baseline=IB Gateway paper headless loopback:4002（client_portal_web_api 契約禁）。
- **normalizer 是負空間 attestation gate 非 formatter**：`_account_status_contract_violations`(stock_etf_account_normalizers.py:159) 把任何真值(hash present/as_of_ms≠0/ibkr_call_performed/snapshot present)標 violation→接真數據必與 gate PASS 同步演進（violation 語意從「任何真值」翻成「無合法 lineage 的真值」）。
- **dispatch fork**：dispatch.rs:456 全 stock_etf 走 `debug_assert slot==None` 的**同步無狀態** fixture arm→真讀需新 IpcSlotRequirement(IbkrReadonlyConnector)+獨立 async arm 拿 connector handle；只 account/health/market-data 需真 backing，其餘 12 governance-status 留 fixture。
- **risk TOML 幾乎接好**：`StockEtfRiskPolicySourceConfigV1`(risk_policy.rs:273 TOML-shaped)+`from_source_config`(:169)已存，TOML 只 2 acceptance test 引用=dormant；policy_status_summary(stock_etf.rs:391)用 `..default()`(全 0)→改成載 TOML/source_template。
- 序：health(reqCurrentTime, 免 session attestation)先於 account(需 attestation)先於 market-data。全程 flag-gated，read-only 無 DB migration(ADR db_evidence_ddl source-only)，rollback=flip flag+停 Gateway 進程零 schema。
...
- **真 dormant = M12 `order_router.rs`（不同能力層）**：OrderRouter trait 6 method 全 `unimplemented!()`(Sprint 1A-δ stub, ADR-0039)，**0 生產 caller**(全 src grep 僅 lib.rs:136 mod 宣告)。它是 adaptive routing(venue_health/forecast_slippage/reverse_snipe/maker_fill_rate_30d rebate 監控)—比 basic maker/taker 高階，與現行 dispatch 正交。thesis「dormant exec層」對 M12 成立，但「naive taker」對 entry+demo-close 已假。
- **P0-prep**：①RiskConfig cap=Rust 唯一權威(Arc<ArcSwap>~5ns；order-size cap=limits.max_order_notional_usdt:435/position_size_max_pct:409/per_trade_risk_pct:471)，Python route parse→IPC patch_risk_config→handle_patch_config 深合併+validate。②**hidden-authority=CLEAN**：June P1(live RiskConfig 繞 5-gate)**已修**——dispatch.rs:127 PHASE 0 AUTH-1 live-write token chokepoint(match 前,全 LIVE_WRITE_METHODS,fail-closed+V014)，handlers_config.rs:152 舊 warn-only fail-open 已刪；Python update_per_engine_global_config(risk_routes.py:1039)+reset_drawdown_baseline(:734)live 分支過 all_five_live_gates_ok+_patch_live_with_token。小單無旁路：主 dispatch 必帶 decision_lease_id(step_4_5_dispatch.rs:166 空/畸形 fail-closed no-op)，process_gates_only_with_features(:866)在交易所 dispatch 前跑；flash_dip_buy/bounded_probe 走同 orchestrator→gate 路徑。③**outcome keying=context_id 血緣非時間窗**：decision_context_snapshots ON CONFLICT(context_id,ts)(context_writer.rs:117)、outcome_backfiller JOIN on context_id(:106)、trading.fills 帶 context_id+entry_context_id。
- **微結構 evidence loop=多數已建, slice 極小**：trading.fills 已存 fee_rate/reference_price/reference_ts_ms/reference_source/slippage_bps/liquidity_role/**maker_markout_bps**/close_maker_attempt(trading_writer.rs:499)；WS execution 已解 is_maker(bybit_private_ws.rs:437)+fee_rate；V145 split_markout_by_role(execution_fill_helpers.rs:21)把 taker→slippage_bps / maker→maker_markout_bps 互斥分流 keyed to fill lineage。缺=queue-position(需 L2 delta，我方 PG 聚合成 1m 丟 99.7%→hftbacktest Tardis 路)+markout horizon 擴充(現 reference_price 單點)。Rust-first：capture 全在 Rust fill 路徑，Python 僅離線分析。
- 教訓：①brief grep「hardcode market」必驗是否 #[cfg(test)]——orchestrator.rs:380/mode_state.rs:394 都是 fixture，字面 grep 把測試誤判生產 hot path。②「dormant exec 層」要區分哪一層——basic maker/taker(strategies+order_manager)是 live，M12 adaptive router 才 dormant，混談誤導 PM 以為連 PostOnly 都沒接。③microstructure capture 先 grep 既有 fills schema——maker_markout_bps/liquidity_role/reference_source 已 land(V145)，大半基建現成。report=口頭直交(parent 讀輸出非檔)。
...
## 2026-07-03 — event_consumer 熱檔拆分 spec（純機械搬移，E1-READY）
- 交付 `docs/CCAgentWorkSpace/PA/workspace/2026-07-03--event_consumer_split_spec.md`（HEAD 2bc69697c，event_consumer/ 乾淨已驗；read-only design，operator 指定路徑非 reports/）。修正 E1 triage #5 兩處：dispatch 抽 315 行→793 太貼線，擴為 424 行全簇（consts+enums+classify+noop/close_dup+timeout+run_dispatch_retry→dispatch_retcode.rs）；loop_handlers 兩刀留檔 842 仍紅（E1 的 ~634 行把 inline tests 誤算進 handle_tick_event）→三刀 Arm D/E/F 各自成檔（loop_pending_registration/loop_pipeline_command/loop_tick）。拆後 9 檔全 ≤800 且餘量 ≥100。
- 關鍵 grep 鐵證：①dispatch 搬移項 crate 內 0 外部 caller（notification_failsafe::DispatchOutcome 同名異物）；②event_consumer/tests/ 5 檔經 `super::super::loop_handlers::` 消費 handle_pipeline_command/pending_registration/exchange_event → loop_handlers 三 arm re-export 不可 cfg(test) gate（mod.rs 非 test 也消費），而 OPEN_NO_RETRY re-export 必須 `#[cfg(test)]`（僅留守測試用，鏡像 unattributed_emit 先例）；③同父 sibling 搬移使 body 內 `super::X` 逐字仍有效 + 除枚舉的 4 fn 升 pub(super) 外全部 visibility 自動保持。
- 教訓：①test-file split 的 fixture 歸屬決定測試搬哪邊——close_dup 測試留 dispatch_tests（與 send_exchange_zero_close 共用 close_dispatch_req_for_zero 單副本），只複製 9 行 biz()，比跨 test-module visibility 便宜；②「拆到 ≤800」必算 inline tests 與 unused-import 清理兩個隱藏項，並留未來成長餘量（36 行餘量方案被否，選三刀 250 餘量）；③pub(restricted) use 未消費會觸 unused-import 警告，re-export 集必按實際消費者收口。
...
## 2026-07-09 — 綜合盈利機會地圖 ROI 排序（read-only 研判，report+Operator 副本）
- 交付 `.../reports/2026-07-09--profit_opportunity_map_roi_ranked.md`。11 move 四模式齊備：#1 反事實管線修復（F1 dedup+成本雙軌，親跑 ledger uniq 復驗 2614×+2444×=2 distinct entry；outcome_review.py grep 0 dedup hit）前置一切 order-capable dispatch；#2 bb_reversion maker 化 fill_sim；#3 horizon arbitrage 日級 XS（klines1d 19,751/26 symbols 親驗在位）；#4 執行衛生 BBO-peg+RPI；#5 六監測 unlock 包（shadow_exits=0/label7d=14 親驗）。落選待裁：Bybit Alpha Prediction Market 需 CC 邊界裁決。
- 教訓：①五報告 reconcile 先對 definition——誤殺母集 49,388(單 reason 字串) vs 71,207(regex 全類) 是口徑差非矛盾，採全類但期望按有效 cell 數(~4-10)管理；②攻性提案的驗證前提必 cross-check 守側 FACT——AI-E O1 引 Gate-B 探針數據，QC-EXT 實測該表 0 row，依賴關係入排序；③FACT 抽樣重跑值得做——L1 timeout 728→732 顯示 drift 方向，TODO 親核防 stale 排序（F1 無 session 修復，outcome_count=5058 仍在 READY 鏈）。

## 2026-07-10 — GUI P0.2 utility 層規格 + 批次 1 edit 計劃（design-only, spec-of-record 已落）
- 交付 `docs/execution_plan/gui_redesign/design/05_utilities.md`（P0.2 八批跨批接口正本）。核心裁決：獨立 `static/oc-utilities.css`（§A 詞彙 16 family ~55 class bare 命名 + §B annex 跨文檔 JS 模板迷你組件）；**全體 `!important`**（inline 優先級的結構性重現，審計面從全 CSS 縮到該元素 JS 寫點）+ 兩鐵則：JS 軸排除（元素同屬性有 `el.style.X=` 寫點必同批 classList 化，否則 utility 壓死狀態繪製）與 **className-wipe 陷阱**（console.html 20+ 處 `el.className='mc-val …'` 每 30s 整串重寫會抹掉 utility → 此類元素禁掛 utility，走父層修飾類）。等價性引理：inline display:none 唯一來源時 `style.display=''` ≡ `classList.remove('hidden')`，''↔none 型切換 class 化證明安全。
- 實測修正：baseline 漂移 1,375→**1,469**（2026-07-10 grep；per-file ratchet 吸收）；`row/col/wrap/block` 碰撞是 `\b` regex 邊界假象（exact-token=0）；tab-settings 已有逐字相同 `.hidden{display:none!important}`；common-modals 3 條 inline 與 `ocInjectBaseCSS` 組件宣告冗餘（讀組件 CSS 後映射會縮水）；`width:80px`×23 全在 `input.oc-input` → `.oc-input--num` annex 組件修飾非 utility；ocCategoryTag 四色 hex（藍綠琥珀紫）palette 外 → 承 tokens-compat `--blue` 中性化裁決轉中性 chip（canon 1）。
- 教訓：utility 層設計的正確性瓶頸不在 CSS 而在 JS 交互面——先 grep `\.className\s*=` 與 `\.style\.<prop>=` 全寫點再定放置策略；「link 放最末」管不到運行時 append 的注入 `<style>`（ocInjectBaseCSS 永遠排最後），cascade 保證只能靠 !important+軸排除，不能靠載入順序。

## 2026-07-10 — Move 3 日級 XS horizon arbitrage 決策 dossier（read-only 綜合裁決）
- 交付 `.../reports/2026-07-10--move3_decision_dossier.md`（Operator 副本未落=dispatch 限制，PM 代複製）。裁決 **GO_WITH_CONDITIONS**：作 $0 研究管線批（置信高），作近期 PnL 線不批（置信低-中）。五條件=①FIX-1..7 進 v1.1 才凍結②敘事改寫「Stage A 不產 demo admission，demo 路徑=pooled E100⊕E_fwd」（RT1：26 名窗 P(GO)≤15%，coin-flip 需 IC 0.18-0.21=物理不可信）③engine lane 0 授權至 GO verdict（RT6：批次權重執行器+hedge overlay+per-leg SL 治理=M12 級 3-5 sprint 新範式，非「近零」）④Stage B 同週啟動（breadth=唯一功效槓桿；不批→KILL 永不可達只能 PARK）⑤價值主張=「管線+breadth 期權+forward PIT」誠實呈交（先驗均值下 net 為負；上端 ~$3-4k/yr@$100k）。
- 關鍵親驗：holding_hours_max=168h(demo toml:22)×rm.time(risk_checks.rs:389) 硬截 primary h=14=demo-cell≠GO-cell → 採 FIX-5 選項(a) operator TOML 具名 blocking 前置，拒選項(b)重凍 h=7（成本 ×2 貼 INVALID_COST 線=本末倒置）；KlineManager 無 1d buffer 但 flash_dip DB 直讀前例(bootstrap.rs:985-992)=信號面不動 KlineManager。pre-GO 足跡=~1,500-2,500 LOC Python 研究碼（Rust-first 例外：離線研究沿 residual producer/fill_sim 範式）、0 熱檔、全 additive rm -rf 可回滾。
- 教訓：①紅隊「把 prereg 自己的表格相乘」是最高價值審計動作——IC 檢定功效≠GO 閘鏈功效這一混淆若不抓，operator 會用錯誤 P(GO) 計價整條線；②decision dossier 的兩個置信度（該不該走 vs 會不會賺）必須拆開報，合併報告=結構性誤導；③demo config 與 primary cell 衝突時，改 config（operator scoped 決策）優於改 primary 遷就 config——研究設計不向基建現狀彎腰，但實作授權要等 verdict。

## 2026-07-10 — Move 2 bb_reversion maker 化 fill_sim 重放決策 dossier（read-only 綜合裁決）
- 交付 `.../reports/2026-07-10--move2_decision_dossier.md`（依任務指令不落 Operator 副本，PM 決定複製）。裁決 **GO_WITH_CONDITIONS（HIGH）限「$0 R1 重放量測儀器」形態**；三寄生子項 NO-GO=probe Rust 管道預建（bb_rev 無 `use_maker_entry` knob，親 grep registry.rs:144/242/272 僅 ma/bbb/grid；單 cell 封頂 0.77-9.0 USDT/月 vs sprint 級工程）、cell 拯救敘事（60d gross +3.66/31-60d −7.07：markout=0 下 maker 化算術已負）、label 供血/破鎖敘事（soak 07-02 已結束三腿全斷）。5 GO 條件=v1.1 納紅隊 FIX-1~5+RT-9/11/12 才凍結、**R1 ≤2026-07-19（l1_events retention=21d 滾動，親驗 ssh timescaledb jobs）**、永久放棄 +0.3bps 直接檢定（n≈4,400≈10 年）、不觸碰 maker-nogo（TODO P1-MAKER-FIRST NO_GO marker 親核在位）、regime 鐵律（OpenShort 僅 07-06 後、long-only 主判）。工作量 ≤1 sprint 純 research Python（fill_sim.py=2000-line 例外 registry #8）0 Rust 0 migration；降級路徑=07-17 未 ready 先跑最小版保 L1 切片（死線綁數據非代碼）。G6 兩案裁 worst-of 分母保留（root principle 13 唯一判定式投影）。
- 教訓：①`maker_markout_bps` 代碼註釋自稱「post-fill adverse selection」但數學是 `adverse_slippage_bps(fill_price, reference@submit)`（execution_fill_helpers.rs:26-43 親讀）——註釋 aspiration ≠ runtime fact 再驗，欄名語義必讀實際計算式；②retention 政策是重放研究的隱藏死線軸——「每週累積到 n_eff≥30」的數學前提被 21d 滾動窗直接摧毀（穩態可重放恆 ≈25），凍結斷言必錨 immutable artifact 切片非活 DB；③紅隊 FATAL 要分層處置（子項 FATAL≠線 FATAL）：descope 後 KILL 也有淨價值（M12 第一份 strategy-conditional 真 post-fill markout）才是 GO 的真理由，且對「值得做」與「會 SCREEN_PASS」拆兩個置信度報（後者先驗低：p50 持倉 48s<τ=60s 信號壽命≈隊列等待）。

<!-- /ROLE_MEMORY_COMPACTION_V1 payload_sha256=sha256:8d5bdac1b071e433203d79e0e284f93eab68b6f606c54771e439552a09c58241 -->
