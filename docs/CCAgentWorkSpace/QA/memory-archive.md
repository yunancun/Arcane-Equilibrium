# QA Memory Archive（append-only：壓實遷入原文，勿刪改）

--- 2026-06-10 壓實遷入（原 memory.md 第 1-746 行）---
# QA Memory — 工作記憶
...
role: QA
payload_sha256: sha256:27ac0b95b89c977937a7cd18e4df30c176940c8e1f0b6cfcd994828f67ba1654
payload_bytes: 50610
-->
# QA Memory — 工作記憶
...
## 2026-07-09 — IBKR stock_etf 16 read-only status endpoints runtime e2e (旧引擎 07-05)
`IBKR-STOCKETF-READONLY-16EP` runtime e2e PASS。IPC-direct（engine.sock + HMAC __auth，fresh-socket/call）打 16 個 `stock_etf.get_*_status` **全 16/16 返回、全 honest fail-closed**（ibkr_live_enabled/ibkr_call_performed/secret_slot_touched/order_routed/bybit_ipc_reused/db_apply_performed 全 false；state=blocked/accepted_no_runtime_authority/source_ready_runtime_blocked）。無假數據/假 fill/假 success。get_account_status 誠實無 snapshot：accepted=false + cash/buying_power=0 + currency="" + as_of=0 + 11 blockers（0 非「假-$0 當真餘額」，GUI 須 gate on accepted/blockers=INFO）。get_policy_status enabled=false/shadow_only=true/allow_*=false（pre-P0 硬編 false，honest，非誤導 bare-default）。get_authorization_status feature_flags 全 false。HTTP 層（Bearer=.secrets/api_token 讀取，read-only GET）lane/account/policy = 200/ok:true/is_simulated:False + no-store（no-cache,no-store,private,must-revalidate）+ degraded=False（證 route→IPC→Rust→normalizer 走真引擎非降級）；unauth GET=401；POST=403（GET-only 亦源碼證 router 只 register .get()）。
**教訓（provenance）**：binary mtime 會誤導——running `openclaw-engine` mtime=07-05 18:27 但需確認真 build commit：**嵌入的 40-hex `54d5fbf99`（07-05 18:24 commit，build 前 3 分）= 真 build 來源**；`strings | grep -c` P0(`risk_config_load_error`)/P1(`ibkr_secret_slot_loader`)/P2(`ibkr_phase2_gate_producer`)/B1(`reqCurrentTime`) 全 0 → running engine 確實早於 P0(c66338e8b)/P1/P2/B1（全 07-09 commit）。**注意**：probe 腳本 `dict.get(k)` 對缺席 key 回 None→JSON null，別把 `null` 誤讀成「引擎有輸出該欄」（我一度誤判 risk_config_load_error:null 為 P0 在場，靠 strings -c=0 + 嵌入 SHA 糾正）。rebuild-only 新欄：P0 TOML source-of-record caps+risk_config_load_error / P1 真 secret-slot 指紋 / P2 sealed gate artifact / B1 connector handshake（皆仍 flag-off dormant，operator-gated，thaw 條件見 IBKR topic 檔）。findings 直接回 PM，無 report 檔。

### post-restart（新引擎 PID 1983100，binary mtime 07-09 23:59:56，build SHA `26401fbb`，size 20227400→20453560；literals P0 `risk_config_load_error`=1/P2 `ibkr_phase2_gate_producer`=1/B1 `reqCurrentTime`=0[socket compile-excluded]）：重跑 16/16 仍全響應、cf 全 false、honest blocked。**真 diff**：僅 **P0 真 wired**——policy_status caps 由旧 default 0+accepted=false → **TOML 真值 1000/5000/10000 + 5/10 + accepted=true + config_version=1 + risk_config_load_error 鍵在=null**，穿 normalizer 到 GUI HTTP JSON（is_simulated:False/no-store/degraded=False）。**P2 只 wire 一欄** `immutable_pass_artifact_present`（precontact.rs:58 呼 `phase2_immutable_pass_artifact_present()` 磁碟 sealed re-verify→false，never-seals）。**教訓（wiring≠deployed）**：P1 loader **完全未 wire 進 16 status handler**（`grep ibkr_secret_slot_loader ipc_server/`=空；僅 P2 producer 內部引用），故 authorization/account 的 `secret_slot_contract`/`session_attestation` 仍是**靜態 default contract**（fingerprint_present=false、postures="unknown"、live_secret_absent_or_empty=false）——外觀 fail-closed 誠實，但**非 live-loader 驗證結果**；勿把「fingerprint 缺席=false」讀成「P1 loader 已反映到 GUI」。B1 dormant。→「deployed P0/P1/P2/B1」真正反映到只讀面的只有 P0（+P2 一欄），P1/B1=compiled-but-dormant scaffold。

## 2026-07-07 — WP7 Learning Effect Review Stop Loop source QA acceptance
`WP7-EFFECT-REVIEW-AND-STOP-LOOP` source-only QA acceptance PASS：`learning_effect_review_v1` builds and validates review-only effect packets solely from caller-provided `reward_ledger_v1` records, supports exactly `continue`/`rollback`/`rotate_candidate`/`stop_loss_control`/`stop_no_edge`/`stop_evidence`/`promote_review_only`, and fail-closes authority/loss-control/evidence/control/sample failures before profitable decisions. E2 MED-1 refs integrity, MED-2 authority aliases/string grants, and MED-3 malformed loss controls are closed; E4 regression PASS accepted. QA reran focused suite (`134 passed`), forbidden-surface grep PASS(no matches), diff-check PASS, and adversarial probe for rotate floor, nested execution authority string, and truthy loss breach. Runtime/loss-control, DB persistence, bounded Demo outcome ingestion, Cost Gate, order/probe, live/mainnet, model reload, serving reload, symlink, and promotion authority remain out of scope and blocked. Report `2026-07-07--wp7_learning_effect_review_stop_loop_acceptance.md`.

<!-- /ROLE_MEMORY_COMPACTION_V1 payload_sha256=sha256:27ac0b95b89c977937a7cd18e4df30c176940c8e1f0b6cfcd994828f67ba1654 -->
