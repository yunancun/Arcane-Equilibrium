# MIT Memory Archive（append-only：壓實遷入原文，勿刪改）

--- 2026-06-10 壓實遷入（原 memory.md 第 1-1315 行）---
# MIT Memory
...
## 2026-06-04 自我審計：「我們比 LLM-alpha 論文更嚴謹」claim 核查 (REFUTE-AS-OVERCLAIMED)

**Trigger**: 主會話對 operator 宣稱「我們已有 DSR/PBO/CSCV/purged-kfold+embargo/HAC/permutation/walk-forward，且比 RD-Agent(Q)/AlphaAgent/QuantaAlpha 更嚴謹」。要求 file:line 證據 confirm/refute。

**Verdict: PARTIAL-TRUE on math, OVERCLAIMED on application surface.**

**逐能力 file:line 結論**:
1. **DSR**: `learning_engine/dsr_gate.py` PSR-based Bailey-LdP(2014) E[max SR_k] Eq.8 + skew/kurt PSR — 數學正確（_compute_psr:305 variance_term γ4-1=ke+2 正確；_compute_expected_max_sharpe:254 Euler-γ 正確）。第二套 DSR 在 `governance_hub_live_candidate_review.py::_compute_bailey_ldp_sr_0:253`（bps-deflation 非機率，亦正確）。
2. **PBO/CSCV**: `learning_engine/pbo_gate.py` Bailey-Borwein-LdP-Zhu(2014) CSCV C(16,8) logit — 數學正確（_cscv_pbo:217，tie-handling:305 用 mean rank 防 logit=-inf）。
3. **CPCV purge+embargo**: `ml_training/cpcv_validator.py:171-189` purge(label window cross)+embargo(adjacent buffer) 雙向正確；per-strategy embargo map（trending 24h/grid 72h）。
4. **walk-forward**: `ml_training/edge_estimate_validation.py:116` 有 purge param 但 **purge_days 默認 0**（line 126）+ **無 embargo**。
5. **HAC/Newey-West**: 0 production hit；**僅 `helper_scripts/research/{multiday_trend,funding_tilt,aeg_breadth_ladder}`** = ONE-OFF research（recon 正確）。
6. **permutation test**: **0 hit in program_code**（recon 正確，ABSENT）。
7. **quantile_bootstrap**: `learning_engine/quantile_bootstrap.py` Politis-Romano(1994) stationary block bootstrap，serial-corr aware，正確。
8. **half_life**: `learning_engine/half_life_estimator.py` exp-fit + 14d fallback，正確但 docstring 自承 fixture-driven。

**THE CRITICAL ANSWER (TASK 2)**：DSR/PBO/CPCV 嚴謹 gate 套在 **ML-MODEL 重訓 + Demo→LivePending 候選晉升**，**不是 alpha/strategy discovery**：
- CPCV → `scorer_trainer.py:178`（LightGBM scorer 重訓），且 **非硬 gate**（line 181-186「continuing with final fit but marking reference-only」），weekly cron。
- DSR/PBO 複合 `promotion_gate.py::SelectionBiasPromotionGate` → `promotion_evidence.py:187` → `edge_estimator_scheduler.py:525` = **demo-only advisory WRITE**（寫 learning.promotion_pipeline/strategy_trial_ledger）非 alpha 拒絕。
- 真 live gate = `review_live_candidate` R1-R5（lg5_review_consumer_scheduler spawn）；R3=PSR floor、R4=DSR-deflation（reject_dsr_deflated:1414）。但 **R4 在 K<5 SKIP（informational 非 blocking, line 1385-1388）= 候選少時 DSR 實質不作用**。
- **新 strategy candidate 不經 dsr_gate.py/pbo_gate.py/cpcv 任一**。實際 alpha NO-GO（trend/funding-tilt/cascade/listing）全靠 `helper_scripts/research/*` 的 HAC/DSR-on-counterfactual ONE-OFF script 人工跑，非 pipeline gate。

**Leakage (TASK 3)**: `multiday_trend_diagnostic/data_loader.py:300` `np.quantile(finite_vols, 2/3)` = **full-sample vol-tercile leak 仍在**（fork 到 funding_tilt:418 已修 expanding/prior-365，但 trend 版未回補）。cpcv_validator purge+embargo 本身 leak-free。

**5-line verdict**:
- 真實嚴謹: DSR/PBO/CSCV/quantile-bootstrap **數學教科書級正確**（dsr_gate/pbo_gate/quantile_bootstrap），+ 兩套 DSR 真接 live 候選晉升 R4。
- OVERCLAIMED #1: 「permutation test」**ABSENT**（0 program_code hit）。
- OVERCLAIMED #2: 「HAC/Newey-West」**僅 research ONE-OFF**，0 production gate。
- OVERCLAIMED #3: 「rigorously validate ALPHAS」假 — gate 套 model-retrain + candidate-promotion，**alpha discovery 無 gate**；且 CPCV 是 advisory soft-warn、R4 在 K<5 skip。
- 真 gap: walk-forward embargo 缺（purge 默認 0）；trend data_loader:300 leak；論文的 closed search loop（自動 hypothesis→backtest→select）我們沒有 = 多人 repo 在 end-to-end 自動化勝。

**Papers 更完整處**：RD-Agent(Q)/AlphaAgent 有 full closed-loop 自動 alpha 搜索（LLM propose→formulate→backtest→select 循環）；我們 alpha 提案是人工 + ONE-OFF research script，無自動 selection loop。「比論文嚴謹」只在「單一候選的 post-hoc 統計檢定深度」成立，「end-to-end alpha 發現 pipeline 完整度」我們輸。

## 2026-06-05 Residual-alpha PRODUCER 對抗性 leakage/CV 審計 (R-1/R-2)

**Scope**: `/private/tmp/wt-residual-producer` worktree — residual_alpha_producer.py (R-1) + residual_alpha_producer_db.py (R-2) + deps (residual_alpha_gate.py / realized_edge_stats.py)。demo FIFO round-trip net_pnl_bps 候選 × BTC contained-bar factor → train-fit beta → residualize eval → PSR/DSR/PBO gate。

**Verdict: NEEDS-FIX (1 CRITICAL holding-window embargo leak)**。

**CRITICAL (must-fix)**: `_build_fit_window` (residual_alpha_producer.py:189-223) 的 embargo 只 purge `entry_ts <= eval_start - embargo_gap` 的 train obs，**忽略每筆 round-trip 的 holding DURATION**。round-trip return 在 [entry,exit] 實現；若 train obs 的 `exit_ts > eval_start`（即 holding_period > embargo_gap），其 y 與 BTC-factor x **都**用 eval 窗價格 → beta fit 吃未來 → 正是「BTC-beta 偽裝 alpha」要擋的東西反而滲漏。數值驗證：entry gap=5h embargo 下，hold=6h 的 train obs exit 已跨 eval_start。Gate 從不收 exit_ts（candidate_rows 只帶 timestamp+return，:121-123），結構上無法察覺。**Fix**: R-2 用 exit_ts 做 duration-aware purge（剔除 `exit_ts >= eval_start - embargo`），或 embargo_gap ≥ max holding period 並在 producer assert。R-1 `embargo_gap` 預設 0.0（:74）+ 全鏈無 `embargo >= max_hold` validation = 預設不安全。

**Clean (verified)**:
- Task 2 contained-bar (residual_alpha_producer_db.py:74-102) 邊界 `bts>=entry AND bts+interval<=exit` 緊且正確：跨 entry/跨 exit/partial 全排除，單筆 round-trip 內 factor 無 look-ahead（數值驗證 4 邊界 case）。
- Task 3 gate train-only beta：`_fit_factor_beta`(:619) 只吃 train_x/train_y；residual_eval 只扣 factor beta 不扣 intercept(:244-245)；`_fit_window_reasons`(:546) 強制 train_end<eval_start（hard fail）。無 full-sample beta。
- Task 5 PIT `pit_active_symbols`(:105-125) listed<=entry AND (delisted is None OR delisted>exit) 正確含已下市；但 v2 market path dead（load_symbol_lifecycles 未寫，build_strategy_residual_report:367 傳 {}，v1 BTC-only）。

**HIGH**: (1) 重疊 round-trip 序列相關 → PSR/DSR normal-approx i.i.d. 假設違反 + OLS beta SE 失真，`_normal_approx_psr`(:660) 用 std/sqrt(n) 無 HAC/Newey-West，多日 perp 長 hold 重疊嚴重時過度樂觀。(2) PBO residual peer = `raw_values - factor_prediction`(:729) 共用同一條 eval factor path，self-admitted「最小近似」，peer 殘差不獨立 → PBO 偏低。**MED**: DSR `n_trials` 鏈路 OK（R-1:105 拒 <1，gate:322 拒 <1，無預設 1 偷渡），但真實 multiple-testing count 必由 cycle orchestrator 傳「達標 cell 數 × 時程」非 row 數非 1 — 目前無 orchestrator，靠 caller 紀律，無 enforcement。

**Report**: 直接回 parent（無獨立 report 檔，本 session 為 worktree 外部模組審計）。

## 2026-06-07 Residual PART 2 hidden-OOS bridge — leak/CV/PIT audit (RETURN-TO-E1)

**Scope**: `/private/tmp/wt-residual-p2` branch `feature/residual-hidden-oos-wiring` HEAD `ae6fec2a` (c39f84e6 E1 impl + ae6fec2a MED-1/2/3 fix). Purely additive: sealer +11 flat-key, new `residual_hidden_oos_bridge.py` +392, 2 test files. 21 bridge tests + 113 full residual/gate cluster green on Mac pure-core.

**VERDICT: RETURN-TO-E1** (1 HIGH boundary-bucket leak + 1 MED design-doc gap). NOT a PART-2-plumbing blocker per se, but the HIGH is a genuine PIT defect in the residual computation that must be fixed before any real OOS promotion. Current data defers regardless (single-config → PBO N/A → honest defer at `_validate_durable_residual_report_snapshot`, verified).

**HIGH-1 (boundary-bucket second-order leak — the MED-2 fix is in the WRONG place)**: bridge derives sealed windows with OOS-bucket filter `aligned = [b for b in aligned if b+bucket_sec <= oos_start_epoch]` (bridge:263), BUT `evaluate_cell` — the function that ACTUALLY computes the sealed residual — re-buckets `rts_non_oos`+`btc_klines` independently (cycle.py:125-126) with NO such filter. Empirically reproduced (Mac probe): when `oos_start` is NON-bucket-aligned and a round-trip exits in `[bucket_floor(oos_start), oos_start)` (carve-out keeps it, exit<oos_start), the straddle bucket `bucket_floor(oos_start)` enters evaluate_cell's residual (aligned_obs 10→11, eval_obs 3→4, report dict differs). Two coupled defects: (a) `load_btc_klines` clamps on bar OPEN-time `ts<=oos_start` but a 4h bar's CLOSE embeds price thru open+4h≥oos_start → boundary factor peeks past oos_start (the MED-2 comment bridge:219-224 IDENTIFIED this vector but only fixed the window-label, not the computation); (b) sealed `candidate_window_end` (bucket-20 boundary) misrepresents the data the residual was fit on (incl bucket 100) → durable audit record not faithful (focus #5). Bound: ≤1 bucket, only non-aligned oos_start. **Fix**: filter the boundary bucket out of the residual itself — drop trips/factor buckets with `bucket_floor(exit)+bucket_sec > oos_start` BEFORE evaluate_cell (or pass an `oos_start_epoch` arg into evaluate_cell so it applies the same filter the bridge's windowing uses). Re-derive windows from the SAME filtered set evaluate_cell uses (single source of truth, not two independent derivations). T11 only asserts the sealed window, not that evaluate_cell excluded bucket 100 — add a bite test.

**MED-2 (candidate→OOS boundary embargo — focus #2 ruling)**: sealed `embargo_seconds` = INTERNAL train→eval embargo `(eb+0.5)*bucket_sec`, NOT a candidate→OOS boundary embargo. `candidate_window_end` can sit immediately adjacent to `oos_start` (zero purge gap). NO layer enforces candidate↔OOS separation: V132 windows_chk (V132:118-123) only checks each window start<end; `_extract` (registry:1014-1031) only checks within-window + `window_start==body.data_window_start`; `_load_hidden_oos_state_snapshot` (source_contract:496-506) only checks OOS window vs registry. Only the bridge's `_windows_strictly_increasing` (`nw[1]<=ow[0]`, bridge:374) enforces non-overlap, and only `<=` (adjacency allowed). **RULING: NOT a leak that blocks PART-2 plumbing, and NOT a leak into the sealed OOS hold-out** — because returns are exit-realized and carve-out is strict `exit_ts < oos_start`, no included trip's return depends on price at/after oos_start; the OOS window `[oos_start,data_end)` is genuinely never computed on (confirmed: evaluate_cell only gets rts_non_oos + klines clamped <oos_start, modulo HIGH-1). So a boundary purge BETWEEN candidate_end and oos_start is NOT required for the hold-out's validity. BUT: (b) the sealed `embargo_seconds` semantically represents internal-embargo while a reader of the durable record may reasonably assume it's the OOS-boundary embargo → document explicitly, OR when the OOS is opened for real (PART-3 future gate), add a purge band `[oos_start - embargo, oos_start)` to prevent serial-correlation straddle on the FIRST trips evaluated post-open. This is a PART-3/future-promotion gate, not a PART-2 blocker. Recommend: doc the embargo semantics in the sealer + a design note that OOS-open must purge the boundary.

**PASS items (verified)**: (1) carve-out PIT `exit_ts < oos_start` strict is correct — return realized at exit, no straddle into OOS (focus #1 first-order). entry-before/exit-after trips correctly excluded (exit>=oos_start → OOS partition). (3) beta fit on TRAIN ONLY (`_fit_factor_beta(train_y,train_x)` gate:254) applied to eval (`eval_y - eval_x@beta` gate:259) — NO full-sample beta, OOS never fits anything. `n_train=max(1,min(floor(0.7*n),n-1))` matches producer `_build_fit_window` (producer:203-204). train→eval embargo purge correct (producer:208-211 Lopez de Prado). (4) n_trials=derive_n_trials(var×sym×strat floor K>=10) = search cardinality not obs/days (cycle:65-86), flows to sealed total_candidates_k. (5) embargo_days↔seconds round-trip dam (bridge:216), residual canonical_sha256 byte-identical to source_contract/drar. honest-defer verified: gate order puts `_validate_durable_residual_report_snapshot` (source_contract:169) BEFORE hidden-oos gates → `durable_residual_alpha_report_hash_missing` PENDING_SCHEMA correct.

**Mac cannot verify (Linux/PART-3 owed)**: real V132 CHECK actual reject (constraints byte-read only), real klines/round_trips, sqlx migration apply, OPENCLAW_ENGINE_BINARY_SHA gate. V132 has Guard A/B/C present (V132:26-79 A / :174-199 B / :201-244 C) + idempotent CREATE IF NOT EXISTS — but empirical double-apply owed on Linux.

**Report**: returned inline to PM (no .md per dispatch instruction).


<!-- ROLE_MEMORY_COMPACTION_V1
role: MIT
payload_sha256: sha256:95db5b41998adec18791266c019733d14635b7009cee13f7cd7111a157b2b113
payload_bytes: 103866
-->
# MIT Memory
...
<!-- /ROLE_MEMORY_COMPACTION_V1 payload_sha256=sha256:95db5b41998adec18791266c019733d14635b7009cee13f7cd7111a157b2b113 -->
