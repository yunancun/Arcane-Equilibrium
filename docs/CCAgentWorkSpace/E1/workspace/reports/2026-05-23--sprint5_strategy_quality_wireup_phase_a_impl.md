...
### 7.6 strategy_ctx CTE `AND context_id IS NOT NULL` filter — 修 spec §3.2 line 612 latent NULL JOIN bug

E1 round 2 補記（per E2 round 1 LOW-1 finding）：

- IMPL `rust/openclaw_engine/src/main_health_emitters.rs` STRATEGY_QUALITY_BATCH_QUERY 的 `strategy_ctx` CTE 加 `AND context_id IS NOT NULL` filter；spec §3.2 line 612 原版 CTE 未含此 filter
- 修法 rationale：PG SQL NULL=NULL 評估為 UNKNOWN（非 true），但 DISTINCT 不會 reduce NULL 為單一 row（NULL 各自視為不同）；strategy_ctx 與 lease_grants CTE JOIN ON `context_id` 時，若任一端 context_id 為 NULL，JOIN 永遠 false → lease_grant_rate metric 失能 0% 命中
- 主動加 NULL filter 是 production-safe 修法，確保 strategy_ctx CTE 只回非 NULL context_id 的策略 → 對應 lease_grants 才能成功 JOIN
- 此修法不破 spec semantics（spec §3.2 line 612 是 latent bug，IMPL 主動修正屬 conservative 補強）；未來如需 spec 端同步 patch，由 PA 在 Sprint 5+ §4.3 amend round 補
