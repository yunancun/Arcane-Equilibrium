...
> 自動產出（vol_event_trigger.py）：2026-06-22T00:00:02.867809+00:00
> $0 唯讀 / OFFLINE / leak-free PIT。指標性彙總，**最終 verdict 屬 QC**。
> 不下單、不碰 production engine/risk、不改 sibling 檔。

## 彙總

- 已分析獨立 high_vol 事件：**11**（downside=4 / upside_squeeze=7）
...
| 軸（signal） | 過牆事件數 / 總事件數 |
|---|---|
| OFI@10s_long_short_ofi10s_fwd15s | 0 / 11 |
| OFI@10s_long_short_ofi10s_fwd5s | 0 / 11 |
| microprice_tilt_decile_spread | 0 / 11 |
