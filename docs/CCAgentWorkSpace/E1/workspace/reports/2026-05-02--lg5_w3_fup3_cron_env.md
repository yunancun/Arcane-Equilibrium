...
- `test_env_file_complete_exports_database_url` — mock python3 in PATH echoes DSN → wrapper log 含 `MOCK_PY3_DSN=postgresql://tradebot:secret_pw@127.0.0.1:15432/trading_ai`，驗 DSN 真 export 到下游 + HOST fallback 127.0.0.1 + PORT 15432 from env + USER/PASS/DB 正確 sourced
...
- 4 manual smoke test cases all green (sealed env via `env -i`):
  ...
  - TEST D (mock python3 echo DSN) → exit 0 + log 含 `MOCK_PY3_DSN=postgresql://tradebot:secret_pw@127.0.0.1:15432/trading_ai`
