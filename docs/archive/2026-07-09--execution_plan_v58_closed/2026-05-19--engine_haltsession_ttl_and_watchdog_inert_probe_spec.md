...
- **v0.3 (2026-05-20, PA / E1 Round 2 fold-in)**: Layer B watchdog inert probe Round 2 patch driven by E2 RETURN review
  - §4.3 example snippet line 639：`i.get("ts_ms", 0)` → `i.get("timestamp_ms", 0)`，與 Rust `pipeline_types.rs:60-67 TimestampedIntent::timestamp_ms` SoT 對齊（E2 OBSERVATION LOW-3；E1 IMPL 一直正確以 schema 為準，僅 spec 文件 drift）
  - Round 2 patch 補丁清單（code only，非 spec scope）：
    - HIGH-1：`load_inert_state` 對 type-mismatch JSON 防呆（per-engine try-except + partial recovery）
    - MEDIUM-1：`save_inert_state` transition-only write（~95% I/O 縮減 — 從 1800/h 寫盤降至僅 state 變化時寫盤）
    - LOW-1：`load_inert_probe_config` 拒收 <=0 threshold
    - LOW-2：`_emit_inert_cleared` 對 None trigger fallback `"no_trigger_recorded"` 而非裸 null
  - 測試：32 既有 + 8 新 Round 2 tests = 40/40 PASS；118/118 wider canary regression PASS
