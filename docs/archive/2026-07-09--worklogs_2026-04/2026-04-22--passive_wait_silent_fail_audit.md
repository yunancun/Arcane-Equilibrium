...
### 3.0 ATR 概念 bug 深度追查（2026-04-22 23:00 追加，升級 P0-13 scope）

原 §3.1 診斷為「builder.rs:110 unit scaling bug」（單位錯）但深度追查後發現是**尺度錯**，scope 從 4h 升級到需 ATR 語義決策：

**實證**：DB `learning.exit_features` 10 sample rows：

| symbol | peak_pnl_pct | atr_pct | giveback_atr_norm |
|---|---:|---:|---:|
| SOLUSDT | 0.148 | 0.00151 | 30.24 |
| CLUSDT | 0.171 | 0.00237 | 45.73 |
| CHIPUSDT | 0.745 | 0.00622 | 156.88 |
| METUSDT | 1.585 | 0.00435 | 154.90 |

**所有 row `atr_pct` 都是 0.001-0.006 量級**（percent-as-number，即 0.001%-0.006% per tick）。

**溯源**：
- `compute_atr_pct`（[`price_tracker.rs:105-135`](rust/openclaw_core/src/risk/price_tracker.rs:105)）用 5-min rolling window × min_samples=10 的 **連續 tick 間 abs return 平均 × 100**
- Tick 之間間隔取決於 WS event rate（ms-s 級）→ per-tick abs return 極小 → 返回 0.001% 量級 percent-as-number
- ✅ 單位正確（`* 100.0` 確實轉成 percent-as-number）
- 🔴 **尺度錯**：傳統 ATR 是 14-period 高低 true range，這是 per-tick abs return，兩者 scale 差 100-10,000x

**概念 bug**：
- `peak_pnl_pct` 是「整個持倉（~分鐘-小時）累積 peak」~1% 量級
- `atr_pct` 是「單 tick（~毫秒-秒）abs return」~0.001% 量級
- `peak / atr` = 數百 — 不是 design intent「多少個 ATRs 的 giveback」，是「這個 peak 等於多少個 tick 的 move」

**跨 consumer 影響**：

| Consumer | 假設 atr_pct | 實際 | 後果 |
|---|---|---|---|
| `compute_dynamic_stop_pct` (`stops.rs:49`) | percent-as-number ~1% | 0.001% | `atr_stop = atr × 2 = 0.002` < base=10 → 永遠 fallback base（**DYNAMIC STOP 7d 1 次證實**） |
| `build_exit_features_for_tick` / `build_exit_feature_row` | percent-as-number ~1% | 0.001% | giveback 放大 100-1000x，Gate 4a threshold 失效 |
| `exit_features/v2.rs` Gate 3 `peak_pnl_pct / atr_pct >= 0.5` | — | — | 等效檢查 `peak >= atr × 0.5`，atr 過小 → 所有倉位過 Gate 3 → 過濾 shallow peak 功能失效 |

**Fix scope 重估**：原「1 行 builder.rs fix」不成立，需 **ATR 語義決策**：

- **Option A（source fix）**：引入新 fn `compute_atr_14period` 用真 ATR（window 高低 true range，scale ~1% day level）供 Priority 6 用；保留 `compute_atr_pct` 原 per-tick 語義供其他 consumer 用（或標註 deprecated）
- **Option B（normalize fix）**：builder 把 per-tick atr 乘以「持倉期 tick 數」換算成持倉期 atr（需新 tracker 欄位 `position_tick_count`）
- **Option C（redefine fix）**：接受 atr_pct 是 per-tick，全部 gate 閾值重新 calibrate（`min_peak_atr_norm=0.5` → `500`，`giveback_base=1.0` → `1000` 等）— 語意混亂

推薦 **Option A**，但需 operator 決策 — 影響範圍包括 `compute_dynamic_stop_pct`（所有策略 dynamic stop 行為改動）、`exit_features/v2.rs` 全部 gate 閾值重 calibrate、`counterfactual_replay_audit_spec.md` 重寫。

**預估 scope 升級**：
- 原：4h code fix + 4 regression tests
- 新：2-3d work chain — ATR design decision doc → Rust 新 fn 實作 → 多個 consumer 遷移 → 閾值 recalibrate → regression + end-to-end tests → counterfactual replay spec 同步更新

**P0-13 本 session 不 autonomously fix**：等 operator 拍板 Option A/B/C；同時 sub-agent A 在做 P0-14 RCA，結果可能影響最終 fix plan（若 P0-14 修好讓 Gate 1 過 → Gate 4a 立刻 fire，unit bug 必須先修）。

---

### 3.1 `giveback_atr_norm` unit scaling bug（`builder.rs:110`）🔴 P0（已由 §3.0 深度追查 supersede 為「尺度 bug」）
