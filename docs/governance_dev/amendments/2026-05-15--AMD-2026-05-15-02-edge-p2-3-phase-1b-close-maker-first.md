...
**Runtime activation layer**（v0.5 補件 — 2026-05-18 post-E2 BLOCKER RCA）：`RuntimeKnobs.use_maker_close` field on `RiskConfig`（per `risk_config_advanced.rs` §RuntimeKnobs）。三環境 TOML 獨立配置 per `feedback_env_config_independence`：
...
TOML field absent → serde default `false`（與 Rust struct cold-boot 對齊）。Activation 路徑：`set_risk_store` → `apply_risk_snapshot` → `set_use_maker_close_runtime`，boot-time 一致 + tick-top hot-reload（ConfigStore 版本號變化下個 on_tick 生效）。

**Phase 2b live_demo 啟用**（per Rollout table）：`commands.rs:92` 當前 Demo-only guard 需另開 IMPL ticket 升級為「Demo OR live_demo endpoint」；本 AMD 不在此回 patch，留 Phase 2b 啟用前單獨補件（per `feedback_demo_loose_live_strict_policy`）。
...
3. ⏳ 三閘全過：
   ...
   - a non-tombstoned funding-related Stage 0R passed OR W-AUDIT-8b deprecated by formal tombstone amendment（funding-related alpha source evidence; 8b tombstoned per Round 2 RED_FINAL 2026-05-18 + redirect to W-AUDIT-8c/8a Phase B/C/D per fix-plan v1.1 §9.4 critical path）
...
| 2026-05-18 | v0.7 | W-AUDIT-8b Round 2 RED_FINAL post-4-agent review wording patch — §8 IMPL Prereq 條件 3 第二子閘從「W-AUDIT-8b Stage 0R passed（funding skew empirical evidence）」改為「a non-tombstoned funding-related Stage 0R passed OR W-AUDIT-8b deprecated by formal tombstone amendment（funding-related alpha source evidence; 8b tombstoned per Round 2 RED_FINAL 2026-05-18 + redirect to W-AUDIT-8c/8a Phase B/C/D per fix-plan v1.1 §9.4 critical path）」。本 patch 純 governance wording 補白 / 無 numerical / 無 AC 改 / 無 IMPL Prereq counts 改 / 無 rollout-phase 改。對齊 Round 1 RED RCA line 209 conditional amendment 觸發點 + Round 2 final sweep 8/8 cells RED + 4 empirical gates PASS + natural gate cross 7.0049d + 4-agent (BB/QC/FA/MIT) consensus APPROVE（consolidated verdict `docs/CCAgentWorkSpace/PM/workspace/reports/2026-05-18--w_audit_8b_round2_red_final_4agent_consolidated.md` ffdbc2d0）。同步 dispatch W-AUDIT-8b spec v0.3 → v0.4 tombstone amendment（獨立 spec event，非本 AMD scope）。 | PM main session per 4-agent W-AUDIT-8b RED_FINAL review |
...
**v0.5 補件後續**：Phase 1b RUNTIME ACTIVATOR BLOCKER fix E1 IMPL 在 2026-05-18 first-dispatch（4-agent batch）被 stopped due to worktree isolation 不穩定 + main tree corruption；Phase 1b activator IMPL 保留在 main tree dirty state（2972 lib tests passing per agent self-report，未進 E2 review chain），下一輪需 cleaner dispatch（單 agent + E2 review chain）。
