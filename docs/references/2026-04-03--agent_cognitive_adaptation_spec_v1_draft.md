    ...
    def update(self,
        ...
        Parameters:
            ...
            regret_data: OpportunityTracker.get_regret_summary() 的輸出
                {regret_from_overtrading, regret_from_undertrading, 
        ...
        # === confidence_floor 調製 ===
        floor = self.BASE_CONFIDENCE_FLOOR
        
        # 遺憾信號：最近因為太激進虧錢了 → 提高門檻
        #           最近因為太保守錯過機會了 → 降低門檻
        if regret_data.get("net_regret_direction") == "overtrading":
            floor += 0.05
        elif regret_data.get("net_regret_direction") == "undertrading":
            floor -= 0.03  # 降低幅度小於提高幅度（保守偏好）
        
        # 連續虧損調製：連虧越多，門檻越高
        if consecutive_losses >= 3:
            floor += 0.02 * min(consecutive_losses - 2, 5)
        ...
        self._confidence_floor = max(self.MIN_CONFIDENCE_FLOOR,
                                     min(self.MAX_CONFIDENCE_FLOOR, floor))
        
        # === qty_ceiling 調製 ===
        ceiling = self.BASE_QTY_CEILING
        
        # 連續虧損 → 縮小倉位上界
        if consecutive_losses >= 3:
            ceiling -= 0.05 * min(consecutive_losses - 2, 5)
            # 連虧 3 筆 -0.05, 連虧 5 筆 -0.15, 連虧 7 筆 -0.25（封頂）
        
        # 週度虧損 → 進一步縮小
        if weekly_net_pnl < 0:
            ceiling -= 0.1
        
        self._qty_ceiling = max(self.MIN_QTY_CEILING,
                                min(self.BASE_QTY_CEILING, ceiling))
        
        # === stoploss_multiplier 調製 ===
        ...
        self._stoploss_multiplier = max(0.8,
                                        min(self.MAX_STOPLOSS_MULTIPLIER, multiplier))
        ...
        # 週度虧損 → 加速掃描（更積極地尋找機會）
        if weekly_net_pnl < 0:
            interval = int(interval * 0.5)  # 15 分鐘
        
        # 遺憾方向為 undertrading → 加速掃描
        if regret_data.get("net_regret_direction") == "undertrading":
            interval = int(interval * 0.7)
        
        self._scan_interval_s = max(self.MIN_SCAN_INTERVAL_S,
                                    min(self.MAX_SCAN_INTERVAL_S, interval))
...
class OpportunityTracker:
    ...
    def update_virtual_pnl(self, current_prices: dict[str, float]) -> None:
        ...
        for opp in self._opportunities:
            ...
            if opp.direction == "long":
                pnl_pct = (price - opp.entry_price) / opp.entry_price * 100
            else:  # short
                pnl_pct = (opp.entry_price - price) / opp.entry_price * 100
    ...
    def get_regret_summary(self, window_days: int = 7) -> dict:
        ...
        Returns:
            {
                "regret_from_overtrading": float,   # 因衝動交易產生的虧損（$）
                "regret_from_undertrading": float,  # 因過度保守錯過的收益（$）
        ...
        # 合併活躍和已結算的機會
        relevant = [o for o in list(self._opportunities) + list(self._settled)
                    if o.ts_ms >= cutoff_ms]
        
        if not relevant:
            return {
                "regret_from_overtrading": 0.0,
        ...
        # 躲過的子彈 = 避免的虧損總額（undertrading 的好處）
        # 注意：這裡 final_pnl 是負數，取 abs
        bullets_dodged = sum(
        ...
        # 淨遺憾方向
        if regret_undertrading > bullets_dodged * 1.5:
            direction = "undertrading"   # 錯過的收益遠大於避免的虧損
        elif bullets_dodged > regret_undertrading * 1.5:
            direction = "overtrading"    # 相反（篩選器做得好，但 Agent 自身的交易在虧）
        else:
            direction = "balanced"
        ...
        # 勝率
        closed_relevant = [o for o in relevant if o.closed]
        hit_rate = (len(would_profit) / len(relevant)) if relevant else 0.0
        
        return {
            "regret_from_overtrading": round(bullets_dodged, 2),  # 注意命名：overtrading 的遺憾 = 篩選器好但自身交易虧
    ...
    def get_schema(self) -> dict:
        return {
            ...
            "schema_version": 1,
            "description": "追蹤未執行機會的虛擬表現",
            "output_fields": {
                "regret_from_overtrading": "因衝動交易的遺憾值",
                "regret_from_undertrading": "因過度保守錯過的收益",
                "net_regret_direction": "overtrading/undertrading/balanced",
    ...
    def get_alerts(self) -> list:
        alerts = []
        summary = self.get_regret_summary(window_days=7)
        if summary["regret_from_undertrading"] > 50.0:  # 7 天內錯過超過 50% 的累積虛擬收益
            alerts.append({
                "severity": "warning",
...
class DreamEngine:
    ...
    def run_cycle(self,
        ...
        for strategy_name, param_space in self.PARAM_SEARCH_SPACE.items():
            for param_name, (lo, hi) in param_space.items():
                ...
                for val in grid:
                    pnls = []
                    for _ in range(self.CYCLES_PER_BATCH // self.PARAM_GRID_SIZE):
                ...
                self._total_cycles += self.CYCLES_PER_BATCH
        ...
        return {
            "cycles_completed": self.CYCLES_PER_BATCH,
    ...
    def _simulate_single_run(self,
        ...
        # 隨機起點，截取 24-72 根 K 線
        run_length = random.randint(24, min(72, len(data) - 1))
        start = random.randint(0, len(data) - run_length - 1)
        ...
        # 隨機方向（或基於簡單趨勢判斷）
        if segment[0]["close"] > segment[0]["open"]:
            direction = "long"
        else:
            direction = "short"
    ...
    def _analyze_results(self,
        ...
        if len(best_pnls) < 5:
            ...
            positive_ratio = sum(1 for p in best_pnls if p > 0) / len(best_pnls)
            sample_factor = min(1.0, len(best_pnls) / 50)  # 50 輪以上滿分
            confidence = positive_ratio * 0.6 + sample_factor * 0.4
...
"pressure": {
    ...
    "regret_direction": "undertrading",  # OpportunityTracker 摘要
    "regret_score": 12.5,           # 累積遺憾百分比值
},
"dream": {
...
async def _main_loop(self):
    while True:
        ...
        while self._is_idle() and not self._has_pending_events():
            result = self._dream_engine.run_cycle(
            ...
            # 每批次後檢查是否需要回到工作模式
            await asyncio.sleep(0.1)  # 讓出事件循環
...
| 風險 | 描述 | 緩解措施 |
|------|------|----------|
| 參數振盪 | 連虧→提高門檻→錯過機會→遺憾顯示 undertrading→降低門檻→重複 | 降低幅度（-0.03）小於提高幅度（+0.05），自然偏向保守穩態 |
| 極端鎖死 | 極端市場下所有調製都推到上限，Agent 幾乎停止交易 | MAX_CONFIDENCE_FLOOR = 0.85 確保 Agent 始終保有行動能力 |
| 滯後反應 | 市場已經轉好但認知調製仍在高門檻 | 使用短期窗口（7 天），且遺憾追蹤會自動校正方向 |

## 8.2 遺憾追蹤的風險

| 風險 | 描述 | 緩解措施 |
|------|------|----------|
| 虛擬 PnL 不等於真實 PnL | 滑點、手續費、流動性問題在虛擬追蹤中被忽略 | 虛擬止損/止盈已預設，且遺憾數據只用於方向性判斷，不用於精確計算 |
| 倖存者偏差反轉 | 只追蹤「沒做的」，忽略「做了但虧了的」 | CognitiveModulator 同時使用 consecutive_losses 和 weekly_net_pnl |
| 數據遞歸 | 遺憾數據影響決策→決策改變→遺憾數據改變→... | 調製幅度有夾緊上下限，防止正反饋失控 |

## 8.3 夢境循環的風險

| 風險 | 描述 | 緩解措施 |
|------|------|----------|
| 過擬合 | 7 天數據的最優參數可能是噪聲 | MIN_CYCLES_FOR_CONFIDENCE = 200，且 confidence 低於 0.4 不產生建議 |
| 簡化模擬失真 | V1 的簡化模擬不含策略完整信號邏輯 | 標記為 TODO：Phase 2 接入真實策略信號函數 |
| CPU 搶佔 | 蒙特卡洛計算在高頻時佔用過多 CPU | nice +15 + 每批次後 yield 事件循環 |
