# EDGE-P3-1 · Realized Edge Predictor — 功能規格 v1.4
# Realized Edge Predictor — Functional Specification v1.4
# 狀態：v1.4 整合 Stage 0 pre-flight reality-alignment（FA/ML-MIT/AI-E 三角色裁定 2026-04-15）— Stage 0 施工中
# 日期：2026-04-15（v1.0 → v1.1 → v1.2 → v1.3 → v1.4）
# Owners：FA 主筆 · PA 架構 · ML-MIT 模型 · AI-E 接線 · CC 審查

---

## CHANGELOG v1.3 → v1.4（Stage 0 pre-flight · reality alignment）

Stage 0 開工前，主會話 pre-flight 檢查發現 v1.3 §5.1/§7.3/§8.8 存在 5 處 spec-vs-reality 衝突。FA/ML-MIT/AI-E 三角色並行裁定（2026-04-15 session），綜合投票如下。

| # | Section | 變更 | 裁定 |
|---|---|---|---|
| R1 | §5.1 / §12.3 / 附錄 | `learning.decision_context_snapshots` → `trading.decision_context_snapshots`（命名空間修正：既有表 V003:40 在 `trading.*`，無 `learning.*` 副本） | 三角色一致 |
| R2 | §5.1 / §5.3 / §12.3 | `trading.fills` 加**新列** `entry_context_id TEXT NULL`；**不**復用既有 `context_id`（V003:283）— ML-MIT + AI-E 查 `tick_pipeline/mod.rs:1025` 確認 `emit_close_fill` 現用 `make_context_id(em, symbol, ts_ms)` 合成 close-time 新 id，復用會造成 100% 靜默 label 丟失 | ML-MIT + AI-E 推翻 FA 的「復用 context_id」提議 |
| R3 | §7.3 U3 / §8.8 U1 | `audit.events` → `observability.engine_events`（既存 V014 append-only hypertable，已有 4 個 Rust writer 使用；預測器審計用 `event_type='predictor_disabled_all'` 前綴區隔）| FA + AI-E（ML-MIT 允許，加 prefix 過濾即可） |
| R4 | §5.2 | ON CONFLICT 對 hypertable 明寫複合鍵 `(context_id, ts)`（非單一 `context_id`，否則 DDL-time 失敗）| FA 發現 |
| R5 | §5.1 | `disagreed BOOLEAN GENERATED ALWAYS AS ... STORED` → 普通 BOOLEAN + Rust 寫入時計算（TimescaleDB 2.x hypertable + compression 不支援 GENERATED STORED）；刪除 ALTER 中重複的 `feature_schema_hash`（保留在 `learning.decision_features`）；`predicted_q50` 索引用 `CREATE INDEX CONCURRENTLY` 避免擋 tick hot-path | AI-E + ML-MIT |

**關鍵排序約束**（ML-MIT + AI-E 共同警告）：V017 migration **必須**與 Rust `PaperPosition.entry_context_id` + `emit_close_fill` 簽名改動同會話/同窗口落地。否則新列全部 NULL，等同 R2 Option A 的靜默 label 丟失。Stage 0 施工序：(1) spec patches → (2) V017 SQL → (3) Rust threading → (4) 回歸測試。
...
**`trading.decision_context_snapshots`**（既有表增列，C16 · QA 要求 · F3a NULL 語義修正 · R1/R5 reality-alignment）：
```sql
ALTER TABLE trading.decision_context_snapshots
  ...
  -- R5: plain BOOLEAN, Rust writer derives at INSERT time with COALESCE semantic.
  -- TimescaleDB 2.x 對 hypertable + compression 不支援 GENERATED STORED；原 F3a 的
  -- COALESCE 規則改由 Rust context_writer 在寫入前計算（兩邊都 NULL → FALSE；
  -- 任一邊 NULL → 另一邊驅動；兩邊都有 → 字串相等比較）。
  ADD COLUMN disagreed              BOOLEAN,
  ADD COLUMN predict_latency_us     INTEGER;

-- R5: feature_schema_hash 保留在 learning.decision_features（該表是 feature
-- authority），此處不重複以避免 dual-write drift。
-- 儀表板 / 告警查詢標準範式：
-- SELECT AVG(disagreed::int) FROM trading.decision_context_snapshots
-- WHERE predictor_decision IS NOT NULL AND shrinkage_decision IS NOT NULL;

-- R5: 熱路徑索引用 CONCURRENTLY 避免擋 tick-path INSERT（必須在 transaction 外執行）
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_dcs_predicted_q50
  ON trading.decision_context_snapshots (predicted_q50)
  WHERE predicted_q50 IS NOT NULL;
```

**`trading.fills`**（R2 · 新列，**不**復用既有 `context_id`）：
```sql
ALTER TABLE trading.fills ADD COLUMN entry_context_id TEXT NULL;
CREATE INDEX IF NOT EXISTS idx_fills_entry_ctx
  ON trading.fills (entry_context_id)
  WHERE entry_context_id IS NOT NULL;
```
**R2 語義**：既有 `context_id`（V003:283）是**close event 自己的 context id**（`emit_close_fill` 用 `make_context_id(em, symbol, ts_ms)` 合成，close-time 新生）；新列 `entry_context_id` 是**該 close 對應的 entry 的 context_id**（從 `PaperPosition.entry_context_id` 透傳）。兩者並存：close fill 同時攜帶自己的 context_id 與所屬 entry 的 context_id，ML 訓練 JOIN 用 `entry_context_id`。

**`learning.decision_shadow_fills`**（F4 · 新表，C13 ε-greedy 合成 fill 專用 · R5 CHECK 移入 CREATE TABLE）：
```sql
CREATE TABLE learning.decision_shadow_fills (
    shadow_id           BIGSERIAL PRIMARY KEY,
    context_id          TEXT NOT NULL,        -- 原 decision context_id
    ts                  TIMESTAMPTZ NOT NULL,
    engine_mode         TEXT NOT NULL CHECK (engine_mode = 'paper'),  -- R5: DB-level invariant (ε-greedy paper-only)
    strategy_name       TEXT NOT NULL,
    symbol              TEXT NOT NULL,
    side                SMALLINT NOT NULL,
    features_jsonb      JSONB NOT NULL,
...
CREATE INDEX idx_shadow_fills_strategy_ts
...
### 5.2 寫入時機（v1.4 · R4 明寫 ON CONFLICT 複合鍵）

- Open 接受 → Rust emit `DecisionFeatureSnapshot` → Python consumer INSERT：
  - `learning.decision_features`（非 hypertable，PK 單一 `context_id`）：`ON CONFLICT (context_id) DO NOTHING`
  - `trading.decision_context_snapshots`（hypertable，PK `(context_id, ts)` 複合，V003:91）：**必須**明寫 `ON CONFLICT (context_id, ts) DO NOTHING`（單鍵會 DDL-time 失敗：no matching unique constraint）
- Close fill → 按 `entry_context_id`（R2 新列）回填 label；split 情況合併為 qty-weighted blend 單 row
...
- **Audit**（R3）：同一 token 寫入 `observability.engine_events`（V014 既存 hypertable，4 個 Rust writer 已用；event_type 用 `predictor_*` 前綴區隔）→ 事後可追蹤觸發 operator
...
4. **Audit**（R3）：
   ...
   INSERT INTO observability.engine_events
     (ts, event_type, severity, source, details)
   VALUES
     (now(), 'predictor_disabled_all', 'warn', 'edge_predictor',
      jsonb_build_object(
        'operator_token_hash', encode(sha256(convert_to(token, 'UTF8')), 'hex'),
        'reason', reason,
        'ts_ms', (extract(epoch from now()) * 1000)::bigint
      ));
   ```
   V014 既存 hypertable；復用既有 Rust writer pattern（`ipc_server/handlers.rs:839` / `handlers_config.rs:169` / `position_reconciler:252,707` / `orphan_handler:310` 皆此模式）。檢索預測器審計 rows：`WHERE event_type LIKE 'predictor_%'`。
...
### 12.1 PA（架構）— 4 件（v1.4 · 合併入 V017 單一 migration）

1. **SQL migration V017**（單檔封裝所有 EDGE-P3-1 DDL）：
   - 新建 `learning.decision_features`（普通表，PK `context_id`，加 partial index `WHERE label_net_edge_bps IS NOT NULL`）
   - 新建 `learning.decision_shadow_fills`（BIGSERIAL PK + DB-level `CHECK (engine_mode='paper')`）
   - ALTER `trading.decision_context_snapshots` 加 7 列（R5：`feature_schema_hash` 已刪、`disagreed` 改普通 BOOLEAN + Rust 計算）
   - ALTER `trading.fills` 加 `entry_context_id TEXT NULL` + partial index（R2：**新列**，非復用）
2. **`parquet_etl.py`** 擴 Feature store 接入 + split-blend label backfill job + reconcile 7d NULL alerter
3. **Label 回填 SQL WHERE**：硬排除 `f.close_tag NOT LIKE 'orphan:%' AND f.close_tag NOT LIKE 'shadow_fill:%'` + F4 子查詢
4. **ON CONFLICT 模式**：`learning.decision_features` 用 `(context_id)`；`trading.decision_context_snapshots` 用 `(context_id, ts)` 複合（R4）
...
- **2026-04-15 v1.4**：Stage 0 pre-flight reality-alignment（FA/ML-MIT/AI-E 三角色並行裁定 5 衝突 R1-R5 + ML-MIT 發現 `emit_close_fill` close-time 合成 context_id 的靜默 label 丟失風險 → 推翻「復用 context_id」）
- **下一步**：Stage 0 施工中（spec patches ✅ → V017 SQL → Rust `PaperPosition.entry_context_id` threading → 回歸測試）
