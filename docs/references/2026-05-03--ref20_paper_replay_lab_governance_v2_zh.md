# REF-20 v2 — Paper Replay Lab 治理 Amendment（中文版）

**狀態：** REF-20 v2 治理 amendment — **supersedes v1（2026-05-02）**
**Owner：** PM（PA 共同撰寫 §3 / §6 / §11 / §12 amendment）
**日期：** 2026-05-03
**Supersedes：** `docs/references/2026-05-02--paper_replay_learning_surface_design_zh.md`（v1 product surface design，保留為 historical baseline）
...
**REF-19 governance partner：** `docs/references/2026-05-03--reality_calibrated_fast_replay_governance_v2_zh.md`（v2 中文版）
**英文版：** `docs/references/2026-05-03--ref20_paper_replay_lab_governance_v2.md`
**關聯：** REF-03、REF-04、REF-18、DOC-01 §5.1–§5.16

---

## 0. v2 與 v1 的差異（讀者導讀）

REF-20 v1（2026-05-02）為 product surface design — 描述 Paper Tab → Paper Replay Lab 升級、Learning 維持知識 cockpit、5-Agent 抽出為 Agents Monitor。**為什麼 v1 還不夠**：v1 寫的是 surface 邊界（Paper Tab 升級為 Paper Replay Lab、Learning 不變 replay runner、5-Agent 抽出），但**沒有**把 V3 工程坑（runner binary / signing / DB role / cfg gate）對應到 surface。所以 P1 frontend / P2 runner / P4 advisory / P6 handoff 之間的契約不清，多 sub-agent 並行會撞約。

v2 的職責是把 V3 工程坑反映進 governance amendment。v2 不重做 v1 的 surface IA（凍結 v1 §3–§5），僅補三項整合：
...
**為什麼是 v2 而不是直接跳 V3**：governance 文件（REF-20）與 implementation plan（V3）是兩個層級。V3 描述「怎麼做」；REF-20 描述「邊界承諾」。v2 把 V3 確立的工程坑反映進 surface 邊界，使 surface 承諾與 implementation 對齊。

---

## 2. 既有系統發現（v1 §2 沿用，要點摘錄）

### 2.1 Paper Tab → Paper Replay Lab

- 主檔：`tab-paper.html` + `app-paper.js` + `paper_trading_routes.py` + `paper_trading_metrics.py`
...
- 主檔：`tab-learning.html` + `app-learning.js` + `learning_legacy_routes.py` + `learning_ops.py` 等
...
- 主檔：`agent-tracker.js` + `agents_routes.py`
...
| Sub-tab | 用途 | Actionability | UX SoT |
|---|---|---|---|
...
**為什麼凍結 IA**：multi sub-agent 並行 frontend wave（U1-U10）撞檔率高，必須在 IA 凍結後才能拆分；否則一個 sub-agent 的 IA 變更會 invalidate 隔壁 sub-agent 的工作。
...
**為什麼要 PM 集中分配**：2026-04-24 V023 silent-noop postmortem + 2026-05-02 V028-V034 retrofit 期已多次撞號 / 衝突；多 sub-agent 同時派發時若無集中分配會造成 migration 編號混亂、idempotency 驗證失效。
...
**為什麼要雙層**：feature flag 是 compile-time guarantee（不可能呼叫到 forbidden module），runtime profile enum 是 last-line-of-defense（即使有人加 mod 沒加 cfg gate，runtime 還能 panic 阻止）。雙層保險避免單點失效。
...
**為什麼 Mac 永遠 non-actionable**：Mac 不跑 release-strip binary（aarch64-apple-darwin Mac dev 環境不跑 CI runner level 的 verification），byte-equality 未實測；engine binary sha 不可信。任何用 Mac 結果做 actionable 判斷的路徑都是不安全的。
...
## 7. P2 前置：Indicator Leak-Free Sweep（V3 §3 G6 解封 + 本 v2 新增；v1 無對應節）
...
**為什麼這道前置門很關鍵**：replay invalidates itself if strategy indicators see future bars。這不是說「目前一定有 leak」，而是「我們不能假設沒有」。所以是 precondition gate，不是 post-hoc check。
...
| Gate | Requirement |
|---|---|
| attribution completeness | `attribution_chain_ok_ratio >= 0.70` |
| strategy-window sample | `n >= 200` |
| cell sample | `n >= 30` |
| stale calibration | model age <= 72h |
| OOS embargo | `max(7d, 2 * signal_half_life)` |
| DSR | `DSR(K) > 0.95` |
| PBO | `PBO < 0.5` when K >= 10, total trades >= 320 |
| cost gate | `cost_edge_ratio >= 0.8` |

Insufficient power → verdict `defer_data`（**不是** `demo_candidate`）。

Half-life estimation: per strategy fit `PnL_t = PnL_0 * exp(-lambda * t)`；half-life = `ln(2) / lambda`；unmeasured → conservative default 14d。

### 8.2 Quantile and Shrinkage

q10 / q50 / q90: block bootstrap (Politis-Romano, 1000 iter), preserve autocorrelation, output 95% CI; n<30 fallback parametric only with `low_confidence` and blocked from handoff.

Shrinkage decision tree:
1. cell n<30: low confidence; block handoff
2. small cell + related cells: hierarchical Bayes preferred
3. cross-strategy global: James-Stein allowed
4. small-K candidate: empirical Bayes allowed
5. method declared in manifest; ad hoc forbidden
...
1. **Warmup**: First 500 fills after negative-edge regime transition cannot drive handoff（防 5-strategy negative-edge env permanent CUSUM freeze）.
2. **CUSUM**: realized edge per cell freezes actionable handoff（**不是** model itself）on +/- 3σ break.
3. **Kupiec POF**: per (strategy, symbol) cell when n >= 250；cell n < 250 skipped（**不從** PBO sample 借）.
4. **PSR(0)**: < 0.95 across 3×250 windows triggers refit + PM alert.
...
**Cognitive overload mitigation**: 4 mode badge + 5 verdict label same screen ≥10 chip 視覺擁擠 risk。緩解 = inline pill 化 + grey-tone disabled + `execution_confidence='none'` 警告色 + 灰底 + ⚠️ icon + tooltip + 卡片右上紅邊。
...
Per `ml-pipeline-maturity-audit` 5-stage × 4-dimension framework. 顯式 surface 避免「P6 deploy = production」誤讀。
...
**為什麼要 maturity label**：MIT R3 #7 audit 指出「P6 deploy ≠ Production」最易被 operator 誤讀；人類習慣「最後階段 = 生產」，但 REF-20 設計上 P6 是 demo handoff 的 Canary，不是 live handoff。Maturity label 防止這個誤讀。
...
| 角色 | 文件路徑 |
|---|---|
| v1 historical baseline（英文） | `docs/references/2026-05-02--paper_replay_learning_surface_design.md` |
| v1 historical baseline（中文） | `docs/references/2026-05-02--paper_replay_learning_surface_design_zh.md` |
| v2 英文版 | `docs/references/2026-05-03--ref20_paper_replay_lab_governance_v2.md` |
...
| REF-19 governance partner（v2，中文） | `docs/references/2026-05-03--reality_calibrated_fast_replay_governance_v2_zh.md` |
