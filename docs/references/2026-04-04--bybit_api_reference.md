...
**✅ 110009 official meaning + source fix（2026-06-18）**：Bybit V5 error table 當前列 `110009 = The number of stop orders exceeds the maximum allowable limit`，不是 PositionNotFound。Source 已在 TODO v186 修正：`BybitRetCode::StopOrderLimitExceeded = 110009`、`from_code(110009)` 對應新 enum，`dispatch.rs` 不再把 `110009` 與 `110001` 一起歸 NoOp；110009 會按 default conservative path 落 `Structural` / fail-closed。相關 tests/comments 已同步更新，focused Rust tests 通過。BB 2026-05-30 call-path trace 判定：`PositionManager::set_trading_stop` 的 SL/TP 路徑不經 dispatch classifier，會 fail-loud；因此此修正關閉的是 P2 latent misclassification，不是 P1 active SL/TP swallow。邊界：source/test/reference doc fix only；本 pass 未 deploy/rebuild/restart，running engine binary 不宣稱已載入此修法。ref：官方 Bybit V5 error table `https://bybit-exchange.github.io/docs/v5/error`；`docs/CCAgentWorkSpace/BB/workspace/reports/2026-05-30--BB--bybit_api_compatibility_audit.md` §NEW FINDINGS BB-2026-05-30-P2-1。
