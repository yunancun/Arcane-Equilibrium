...
5. **Supervisor escalation fifth**: MAG-019 is complete at `65a4279f`. Cloud remains default-disabled; any future cloud L2 call must go through one supervisor packet, explicit budget/model config, and `agent.ai_invocations` ledger reservation before provider IO.
...
| MAG-019 | AI-E/PM | P1 | DONE (MAC/LINUX POLICY CONTRACT) | Define and wire supervisor cloud escalation ledger policy after `agent.ai_invocations` exists. | `openclaw_supervisor_policy.py` landed at `65a4279f`: cloud default disabled, explicit budget/model required, supervisor packets are bounded and hashed, budget denial creates local diagnosis payload, allowed calls must first reserve an `AgentEventStore.record_ai_invocation` row with `control_plane=true`, and the policy module has no cloud/network call markers. `openclaw_routes.py` now surfaces the policy snapshot in `model_budget`. Mac/Linux targeted tests 45/0, py_compile, and node check passed; no cloud provider call, service restart, deploy, proposal/write endpoint, or trading authority change. |
...
| MAG-021 | E1 | P0 | DONE (MAC/LINUX CONTRACT) | Add `OpportunityCandidate` and `OpportunityDecay` contracts. | `scanner/types.rs` now defines serde contracts for `ScannerAuthorityMode`, `OpportunityCandidate`, `OpportunityDecayReason`, and `OpportunityDecay`; `scanner_advisory_contracts.py` mirrors them as Pydantic models. Rust/Python targeted serialization tests pass on Mac and Linux. No runtime authority behavior is wired in this task. |
| MAG-022 | E1 | P0 | DONE (MAC/LINUX EMIT CONTRACT) | Emit scanner decay when symbol weakens, is displaced, or exits top set. | `scanner/advisory.rs` builds `OpportunityDecay` for weakened/displaced/exited selected-set changes; `ScannerRunner` emits review-required decay evidence to IPC and `trading.scanner_opportunity_decays`. Open-position decay sets `position_review_required=true` and `auto_close_allowed=false`; no close/reduce dispatch path is touched. Mac/Linux targeted Rust tests pass. |
| MAG-023 | E1 | P0 | DONE (MAC/LINUX REPLAY PROOF) | Preserve active-position market data subscription independent of scanner ranking. | `ScannerRunner` now treats open-position discovery failures as fail-closed for removals, and replay runner proof `adapter_pipeline_preserves_open_position_tick_after_scanner_drop` shows a seeded BTC open position still receives its tick, emits a close fill, and realizes PnL after scanner timeline removal while unrelated inactive symbols are skipped. Mac/Linux targeted Rust tests pass. |
| MAG-024 | E1 | P0 | DONE (MAC/LINUX SHADOW WIRE) | Convert scanner hot-path new-open gate into advisory shadow comparison behind flag. | `ScannerConfig` now carries `[authority].mode` with default `legacy_gate`; TickPipeline receives the mode at bootstrap. In `advisory_shadow`/`advisory_enforced`, legacy scanner active-universe / route-mode / opportunity-canary would-block reasons are recorded in intent `details.scanner_gate` and `details.scanner` without suppressing the open path; `legacy_gate` preserves the prior reject behavior. Mac/Linux targeted Rust tests pass. |
| MAG-025 | QC | P0 | DONE (MAC/LINUX REPLAY FIXTURE) | Build replay set for scanner churn windows and wave profit/loss behavior. | Replay fixture `replay_churn_fixture_identifies_scanner_driven_wave` reconstructs a deterministic SOLUSDT -> XRPUSDT scanner wave with explicit `added`/`removed` cycles. The MAG-023 runner proof covers the matching PnL side by closing an existing position after scanner drop. Mac/Linux targeted Rust tests pass. |
| MAG-026 | E4 | P0 | DONE (MAC/LINUX REGRESSION) | Regression: scanner decay -> PositionReview, no auto close. | Regression `scanner_removal_of_open_position_is_review_input_not_close_signal` proves scanner removal of an open-position symbol emits `OpportunityDecay` review evidence with `position_review_required=true`, `position_review_input=true`, and `close_dispatch_allowed=false` / `auto_close_allowed=false`; no close dispatch path is introduced. Mac/Linux targeted Rust tests pass. |
...
| MAG-032 | E1 | P0 | DONE (MAC/LINUX DURABLE STORE) | Implement spine store for StrategistDecision, GuardianVerdict, ExecutionPlan, ExecutionReport. | `sql/migrations/V064__agent_spine_decision_store.sql` adds `agent.decision_objects`, `agent.decision_edges`, `agent.decision_state_changes`, and `agent.execution_idempotency_keys`; Rust `agent_spine::{events,store}` plus `database::agent_spine_writer` define fail-soft typed persistence for StrategySignal -> StrategistDecision -> GuardianVerdict -> ExecutionPlan/ExecutionReport lineage. Mac/Linux targeted Rust/static migration tests pass. Runtime writer startup remains unwired. |
| MAG-033 | E1a | P0 | DONE (MAC/LINUX PY CLIENT) | Add Python `agent_spine_client.py` for Strategist/Guardian/Analyst interaction. | `agent_contracts.py` mirrors Rust spine payload contracts and `agent_spine_client.py` can fail-soft publish/consume StrategySignal, StrategistDecision, GuardianVerdict, ExecutionPlan, ExecutionReport, and AnalystInsight objects plus typed edges/idempotency keys. Payloads are bounded, hashed, and redacted; default env remains disabled. Mac/Linux targeted pytest + py_compile pass. |
| MAG-034 | E2 | P0 | DONE (MAC/LINUX IDEMPOTENCY AUDIT) | Audit idempotency and double-execution prevention. | `2026-05-07--mag034_idempotency_double_execution_audit.md` confirms every execution candidate carries `decision_id`, `order_plan_id`, `idempotency_key`, and `engine_mode`; V064 dedupes by idempotency key, plan/mode, and decision/plan/mode; Rust/Python contract tests enforce lineage ids. Mac/Linux targeted checks pass. |
| MAG-035 | E4 | P0 | DONE (MAC/LINUX SHADOW REGRESSION) | Shadow integration test: legacy Rust path vs spine decisions. | Rust regression `shadow_spine_chain_is_complete_while_legacy_signal_msg_stays_unchanged` builds StrategySignal -> StrategistDecision -> GuardianVerdict -> ExecutionPlan -> ExecutionReport plus idempotency reservation while proving the legacy `TradingMsg::Signal` serialization remains unchanged. Mac/Linux targeted Rust tests pass. |
...
| MAG-041 | E1a | P0 | DONE (MAC/LINUX DECISION BUILDER) | Implement StrategistDecision open/hold/reduce/close/no_action. | `strategist_decision_v2.py` builds typed StrategistDecision V2 objects with canonical strategy selection, candidate scores, thesis, invalidation, expected net edge, portfolio impact, fact/inference/hypothesis refs, negative-net-LCB open blocking, and explicit no_action fail-closed paths. Rust/Python contracts mirror the new fields; Mac/Linux targeted tests pass. |
| MAG-042 | E1a | P0 | DONE (MAC/LINUX PY CONTRACT) | Implement PositionReview for scanner decay and regime shifts. | `position_review_v2.py` builds typed PositionReview recommendations for scanner decay/regime shift paths: hold, reduce, tighten_exit, stop_adding, close_when_net_positive, close_now_if_risk_requires, and no_action. Scanner decay remains advisory (`allow_auto_close=false`), malformed decay auto-close is ignored, reduce/close require Guardian lineage downstream, and PositionReview can become a StrategistDecision candidate. Mac/Linux targeted Python tests pass. |
| MAG-043 | E1a | P1 | DONE (MAC/LINUX PY CONTRACT) | Consume Guardian rejection stats in next-cycle decision. | `strategist_decision_v2.py` now accepts `GuardianFeedbackStats`; high Guardian reject rate raises the confidence floor for new opens, reduces open aggressiveness via proposed-quantity scaling, lowers effective risk prior in candidate scores, and preserves position-review reduce/close candidates from open-rejection feedback. Mac/Linux targeted Python tests pass. |
| MAG-044 | E1a | P1 | DONE (MAC/LINUX PY CONTRACT) | Consume AnalystInsight and TruthRegistry in strategy weights. | `strategist_decision_v2.py` now consumes AnalystInsight and TruthRegistry-style claims as bounded learning-weight adjustments; losing/negative patterns can move preference away from the affected strategy with reason codes/evidence refs persisted in candidate scores, while winning/positive patterns can boost strategy weight. Mac/Linux targeted Python tests pass. |
| MAG-045 | E4 | P0 | DONE (MAC/LINUX PY REGRESSION) | Replay test: Strategist decisions are not equivalent to scanner score sorting. | Replay-style regression `test_replay_strategist_decision_is_not_raw_scanner_rank_sorting` compares scanner rank 1 vs selected rank 2, requires Guardian/Analyst feedback reason codes, thesis/invalidation, and evidence refs. Mac/Linux targeted Python tests pass. M4 Strategist V2 closed. |
...
| MAG-052 | E1 | P0 | DONE (MAC/LINUX PY+RUST) | Add P2 risk modification output to GuardianVerdict. | Python/Rust GuardianVerdict contracts now carry structured `p2_modifications` for bounded size/leverage/stop/cooldown adjustments without symbol/direction authority. `guardian_agent.py` emits P2 modifications for leverage, correlation fallback/soft limit, and soft per-strategy risk; hard strategy drawdown/loss-streak rejects new opens, records pause/review reason codes, and does not direct-close. Mac/Linux targeted Python + Rust agent_spine tests pass. |
| MAG-053 | E1 | P1 | DONE (MAC/LINUX PY) | Consume Scout event alerts and scanner risk evidence in Guardian. | Guardian review now consumes active Scout EventAlert risk, scanner risk evidence from intent metadata/params, and stored RISK_PATTERN messages. High/soft evidence P2-modifies size/cooldown with reason codes; critical/hard evidence rejects/pause new opens and may request PositionReview evidence, without symbol/direction changes or direct order/close authority. Mac/Linux targeted Guardian pytest 74/0 + py_compile passed. |
| MAG-054 | E4 | P0 | DONE (MAC/LINUX PY+RUST REGRESSION) | Regression: Guardian verdict is mandatory before ExecutionPlan. | `ExecutionPlan.verdict_id` is non-empty by contract, `AgentSpineClient.publish_execution_plan()` fail-soft rejects plans without a prior allowing GuardianVerdict and rejects plans after a rejected verdict, while modified Guardian verdicts persist state `modified`. Rust spine envelopes also classify P2-modified GuardianVerdict as `modified`. Mac/Linux Python spine-client tests 11/0, Rust agent_spine 6/0, py_compile, and diff checks passed. M5 Guardian V2 closed. |
...
| MAG-061 | E1 | P1 | DONE (MAC/LINUX PY BUILDER) | Implement ExecutionPlan generation. | `executor_plan_v2.py` now deterministically builds an `ExecutionPlan` from an approved/modified `StrategistDecision + GuardianVerdict` lineage. Plans copy symbol/direction/strategy/engine_mode only from StrategistDecision, reject hold/no_action and rejected/mismatched verdicts, apply Guardian P2 size/stop/cooldown/leverage policy metadata, choose bounded market/post-only order shape, urgency, max slippage, maker preference, and lease request scope/TTL without submit or lease binding. Mac/Linux targeted pytest 22/0, py_compile, and diff checks passed. |
| MAG-062 | E1 | P1 | DONE (MAC/LINUX PY BUILDER) | Add Decision Lease binding to ExecutionPlan. | `executor_plan_v2.py` now binds Decision Lease IDs onto ExecutionPlan objects through `acquire_execution_plan_lease()` and enforces `prepare_execution_plan_for_submit(real_submit=True)` fail-closed behavior when a real submit has no lease, no GovernanceHub, failed acquisition, or missing lease request fields. Shadow/pre-submit plans may remain unleased for durable planning. Mac/Linux targeted pytest 28/0, py_compile, and diff checks passed. |
| MAG-063 | E1 | P1 | DONE (MAC/LINUX PY+RUST CONTRACT) | Persist ExecutionReport with quality metrics. | Python/Rust `ExecutionReport` now carries requested/filled qty, expected/average fill price, slippage bps, fees paid, fee bps, submit/fill latency, liquidity role, and quality_metrics. `executor_report_v2.py` builds Analyst-consumable reports from ExecutionPlan + fill observations, and `AgentSpineClient.publish_execution_report()` writes those metrics into the `executed_by` edge details. Mac/Linux targeted Python tests 16/0, Rust agent_spine 6/0, py_compile, cargo fmt, and diff checks passed. |
| MAG-064 | E4 | P1 | DONE (MAC/LINUX PY REGRESSION) | Regression: Executor never chooses symbol/direction. | `test_executor_plan_v2.py` now proves Executor plan generation copies symbol/direction only from the approved StrategistDecision even when decision, Guardian verdict, and Guardian P2 metadata carry decoy scope fields. `test_agent_spine_client.py` rejects non-Strategist scope sources at contract validation and refuses persisted plans whose symbol or direction diverges from the prior approved decision. Mac/Linux targeted pytest 32/0, py_compile, and diff checks passed. M6 Executor Planner is closed. |
...
| MAG-071 | E1a | P1 | DONE (MAC/LINUX PY SPINE) | Persist AnalystInsight and link to evidence refs. | `AgentSpineClient.publish_analyst_insight()` now writes the AnalystInsight object, the parent `analyzed_by` edge, and one `evidence_for` edge per unique `evidence_ref`, so insights can be traced back to round-trip IDs, strategy metric IDs, execution reports, or prior insight IDs. Edge details carry analyst tier/type/level and evidence index. Mac/Linux targeted pytest 34/0, py_compile, and diff checks passed. |
| MAG-072 | E1a | P1 | DONE (MAC/LINUX PY STRATEGIST) | Strategist consumes losing/winning patterns through typed rules. | `strategist_decision_v2.py` now records Analyst/TruthRegistry learning effects as typed rules in candidate and selected-candidate `learning_feedback`, including source, Analyst tier/type/level, insight/claim ID, polarity, reason code, and evidence refs. L2 Analyst losing-pattern tests prove strategy weight can move away from grid_trading; winning-pattern tests prove a lower-ranked bb_breakout route can be boosted, with explainable reason/evidence in the next-cycle StrategistDecision. Mac/Linux targeted pytest 16/0, py_compile, and diff checks passed. |
| MAG-073 | E1 | P1 | DONE (MAC/LINUX PY GUARDIAN) | Guardian consumes risk patterns. | `guardian_agent.py` now preserves Analyst risk-pattern insight metadata (`insight_id`, analyst tier/type/level, evidence refs) when consuming `RISK_PATTERN` messages, maps Analyst risk patterns to explicit `risk_pattern_*` reason codes, and can P2-tighten size/cooldown from a soft risk pattern without changing symbol/direction or direct close/order authority. Mac/Linux targeted Guardian pytest 45/0, py_compile, and diff checks passed. |
| MAG-074 | E4 | P1 | DONE (MAC/LINUX PY E2E REGRESSION) | End-to-end test: losing pattern -> Strategist weight change -> persisted reason. | `test_agent_spine_client.py` now persists an L2 losing-pattern AnalystInsight with evidence edges, feeds the same typed insight into StrategistDecision V2, proves the next-cycle strategy preference moves away from the losing grid route, then publishes the StrategistDecision and asserts the persisted payload carries the typed learning reason and evidence refs. Mac/Linux targeted pytest 35/0, py_compile, and diff checks passed. M7 Analyst Learning Loop is closed. |
...
| MAG-083 | QA | P0 | BLOCKED (WAITING CANARY EVIDENCE) | Final release audit. | `2026-05-07--mag083_final_release_audit_blocked.md` records the QA pre-audit: source/policy prerequisites are present, but final release audit cannot pass until an operator-approved MAG-082 24h canary evidence report proves no trade reaches execution without StrategistDecision + GuardianVerdict + ExecutionPlan + Decision Lease. Documentation only; no canary run, runtime flag, rebuild, restart, deploy, DB write, live auth, or trading authority change. |
| MAG-084 | PM | P0 | BLOCKED (WAITING MAG-083 PASS) | Operator sign-off. | `2026-05-07--mag084_operator_signoff_blocked.md` records that sign-off cannot proceed while MAG-083 is blocked. M8 remains open until an operator-approved MAG-082 canary evidence window exists, MAG-083 reruns with PASS, and only then written sign-off updates TODO/CLAUDE/MEMORY as needed. Documentation only; no canary run, runtime flag, rebuild, restart, deploy, DB write, live auth, or trading authority change. |
