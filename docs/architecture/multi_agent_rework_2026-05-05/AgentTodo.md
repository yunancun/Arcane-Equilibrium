...
| ID | Owner | Priority | Status | Task | Acceptance |
|---|---|---:|---|---|---|
| MAG-010 | E1 | P0 | SOURCE DONE / ROW PROOF PENDING | Add legacy/advisory bus trace writer for `agent.messages` without promoting `MessageBus` to authority. | `AgentEventStore.record_message` + `MessageBus(message_sink=...)` landed 2026-05-06 with fail-soft tests; final DONE still requires Linux runtime fresh rows via `[52]`. |
| MAG-011 | E1 | P0 | SOURCE DONE / ROW PROOF PENDING | Persist `agent.state_changes` from agent start/stop/degrade/heartbeat transitions. | `BaseAgent` lifecycle + `Conductor.set_agent_state` write fail-soft state rows; final DONE still requires Linux rows for five agents + Conductor. |
| MAG-012 | E1a | P0 | SOURCE DONE / ROW PROOF PENDING | Persist `agent.ai_invocations` for local L1/L1.5/L2 and supervisor cloud escalations with model, latency, cost, prompt hash, output hash. | Strategist / Guardian / Analyst local Ollama paths now write prompt hash + response hash metadata; supervisor cloud escalation rows remain MAG-019 scope; final DONE requires Linux nonzero AI rows. |
| MAG-013 | E2 | P0 | SOURCE DONE / E2 REVIEW PENDING | Audit DB sink failure modes. | Writer default-off, fail-soft on DB/serialization errors, and `[52] agent_event_store_rows` makes enabled-but-empty tables fail-visible; E2 review + Linux `[52]` proof still pending. |
| MAG-014 | E4 | P0 | TESTS PASS / RUNTIME ROW PROOF PENDING | Add Linux regression for agent schema nonzero row acceptance. | Added targeted pytest for event store, bus sink ordering/fail-soft, BaseAgent lifecycle, and `[52]`; Mac and Linux targeted tests passed 215/0 at `91379cd2`; runtime fresh-row proof still pending. |
