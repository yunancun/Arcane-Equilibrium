...
| MAG-023 | E1 | P0 | PARTIAL (MAC/LINUX RUNNER GUARD) | Preserve active-position market data subscription independent of scanner ranking. | `ScannerRunner` now treats open-position discovery failures as fail-closed for removals: reliable empty sets still allow removal, but timeout/channel failure returns `None` and defers removal for the current active universe, preventing silent unsubscribe of unknown open positions. Mac/Linux runner tests prove query failure returns `None` and active-universe removal is deferred. Replay proof remains pending. |
