...
| MAG-014 | E4 | P0 | TESTS ADDED / LINUX ROW PROOF PENDING | Add Linux regression for agent schema nonzero row acceptance. | Added targeted pytest for event store, bus sink ordering/fail-soft, BaseAgent lifecycle, and `[52]`; Linux targeted run + runtime row proof still pending. |
