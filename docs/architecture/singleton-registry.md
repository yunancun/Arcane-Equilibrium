...
| cross_task_pattern | 單一 leader worker 持有；`divergence_snapshot_flusher` 背景協程每 30s 經 executor 跑 `flush_divergence_snapshot_once`（讀 §2.5.2 `_COUNTERS` snapshot → UPSERT V128 表）。非 leader worker 不 flush。對齊 `paper_trading_wiring._RECONCILER_ALERT_LOCK_FD` flock 範式 |
...
| migration_plan | step (iv) cleanup 連同 comparator sink（§2.5.1-3）+ dual-write mirror 一起移除（soak 0 divergence 後）；屆時 DROP V128 表 |
