...
## Project context
- [BG subagent「卡死」根因三層+SOP shipped (2026-06-11)](project_2026_06_11_bg_subagent_idle_kill_rootcause.md) — ①desktop 900s idle-pause 殺光 in-flight BG agent(兩波 2 秒實證,不可復活);②output-stub/worktree mtime 判死=必誤殺,**唯一信號=subagents/agent-*.jsonl mtime**;③限額日 API 單往返 5m38s 假死。**路線 B 拍板,SOP shipped `558ded55`**:CLAUDE§八+PM.md 正本(駐留等收/TaskStop 三前置/續作棒)+E1 checkpoint+`agent-wave` workflow(resumeFromRunId 零浪費重放);Mac 四檔未提交鏡像勿 revert;本地 main ahead-1=`175afa01` 殭屍指針,收斂 reset 待批
- [L2 P4 三點裁決全清+P2p 哨兵同日 shipped (2026-06-10)](project_2026_06_10_l2_p4_ratify_p2p_shipped.md) — P4:MIT 7 項(6A/1MODIFY #3 debit 須含 dsr 渲染即扣;#2 拍板 Option B)+QC APPROVE-with-FIX(FIX-2.1b 區間語義)+FIX-3.1 ACK(value-invariance);殘=operator V137 確認;E1 契約=三件套報告。P2p 全鏈 merge `661699e5`(E4 抓 A3 loopback 拓樸假 CRITICAL);殘 owed=installer apply+probe+兩輪 all-pass。**OQ-1:告警通道全未配置=watchdog 告警一直靜默 no-op**,operator 拍板 Telegram,creds 待給
...
- [L2 Mesh P1-P3b DEPLOYED+owed 五項 DONE (2026-06-10)](project_2026_06_08_l2_d3_phase1_green.md) — **owed 五項全補**(reindex/seed×6/adapter+route/V127=7696/klines 4380)+**E2E-0 達成**(真 HTTP→雙閘擋→seam_id=1 真 row);E2 四輪對抗全真彈+QC 預註冊帶 SANE(clone β=0.99984);**E2E-1 達成**(真 l2_calls row `l2r:93166da5722f`,FULL prompt 落庫;debounce 900s trailing-edge 真驗);enable 真用前置=cloud key+M4 ollama flag(operator);**P4 設計 `b40b9481`**(α-wealth V137 append-only/sealer 鏈已存在只補觀測/tier L3 不可達=dormant 誠實/待 MIT ratify 7 項)+**P2p 設計 `e5a39342`**(6 軸 cron 哨兵 E1-READY,watchdog alert reuse);next=P2p E1(1 session)∥P4(2-3 session); 四 phase cherry-pick 重放上 main（`7b8fae45`→收口 `9e920c21`，零衝突；測試適配補遺 `bf32074d` 閉「工作樹綠≠commit 綠」盲點）+Linux 部署完成：sqlx 133→136（engine auto_migrate applied=3；**AUTO_MIGRATE consumer=engine 非 control_api**，E3 抓），3 表建成 0 rows=dormant flag-OFF；re-test 450/4xf+full 4661/8 pre-existing/0 新 fail；feature branch SUPERSEDED `1f34653c` 勿 merge/取檔；**P3b owed-before-enable**(否則 universal DEFER):int-bar-index re-index+agent.lessons seed dead-modes+conductor wiring+V127 pop+6 ex-BTC klines;owed:deployed-E2E(operator /trigger);next=P3b-enable 前置/P4(online-FDR)/P2p;SSOT=srv/L2_TODO.md
