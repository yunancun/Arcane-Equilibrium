...
## Project context
- [五 repo 借用評估:subagent+token (2026-06-11)](project_2026_06_11_five_repo_subagent_token_eval.md) — **rtk=P0 採用**(實測 −54~98%,pytest error 計數缺陷 12 行可修,exclude 過渡);superpowers 抄 4 機制不裝 plugin(SessionStart hook/description 改寫/四態協議);TencentDB 借 prompt+schema(A=memory SOP,B=L2 PG 記憶層);oh-my-pi 70% 鎖死+ToS 灰區;last30days 抄 BB 哨兵模式;license 全綠;待 operator 拍 P0
