...
| ID | Claim | Verdict | Evidence |
|---|---|---|---|
| R4-CRITICAL-1 | CLAUDE.md says "14 ADR" but actually 22 | **CONFIRMED** | `ls docs/adr/ | wc -l` returns 22. CLAUDE.md section 1 says "14 ADR". |
...
## PM SIGN-OFF -- 2026-05-16

### Overall Assessment

**PM SIGN-OFF: APPROVED-CONDITIONAL**

PA delivered a strong consolidated fix plan. The 12-agent audit was thorough, and PA's verification layer correctly caught 4 false P0 elevations (FA-P0-1, FA-P0-2, FA-P0-4, QC-P0-2) while confirming the 2 true actionable P0s (WP-01 GUI safety, WP-02 Donchian bias). The cross-audit dedup in Section 5 is clean -- 6 clusters with no double-counting. I independently verified the following from Mac source:

**Spot-check results**:
- R4-CRITICAL-1: Confirmed. 22 ADR files exist; CLAUDE.md says 14.
- R4-CRITICAL-2: Confirmed. 16 tab-*.html files exist in the static directory.
- QC-P0-1 Donchian: **Nuanced**. `donchian_prior()` already exists at trend.rs:212-223 and is tested at :325-332. The underlying `donchian()` function at :185-208 does include the current bar. However, the production path (`IndicatorEngine::compute_all`) was verified to call `donchian_prior()` since commit `75741eff` (2026-04-28), and this was previously verified by a 4-agent fact-check that retracted the "runtime contaminated" claim (see TODO P0-V2-NEW-1-DONCHIAN-LEAK-BIAS, DONE 2026-05-09). The base `donchian()` function retaining current-bar inclusion is a code hygiene issue, not a live-runtime bug.
- QC-P1-1 OU sigma: Confirmed at grid_helpers.rs:140. Uses raw RMS, not residual std from OU regression.
- E5-P-1: Confirmed. 53 `.clone()` calls in step_4_5_dispatch.rs.

### Conditions for Full Approval

1. **WP-02 must be rescoped**: PA's WP-02 description says "Wire bb_breakout to call the prior-bar variant" -- but this was already done in `75741eff` and fact-checked by 4 agents on 2026-05-09. The actual remaining work is: (a) verify no other callers of `donchian()` exist that should use `donchian_prior()`, (b) optionally deprecate or rename the base `donchian()` to prevent future misuse. This is a P1 hygiene item, not a P0 runtime fix. PA should downgrade to P1 or clearly scope the delta.

2. **WP-08 MIT-P0-2 "6/12 ML cron scripts not installed" conflicts with existing closure**: TODO P0-V3-CRON-NOT-INSTALLED says "DONE 2026-05-09" with "F-08 5 ML cron `17 3 * * *` installed and 24h fire verified" (invariant 18 closed). MIT claiming 6/12 not installed may be measuring different cron scripts or the claim is stale. PA must reconcile before dispatch: either MIT found additional crons beyond the F-08 set, or this is a false finding.

3. **WP-01 GUI safety must include the A3+E2 adversarial review mandated by memory `feedback_impl_done_adversarial_review`**: GUI is explicitly listed as a high-risk IMPL category requiring post-IMPL A3+E2 parallel verification, not just E2 review.

### Approved Wave Sequence

| Wave | WPs | Sessions | Notes |
|---|---|---|---|
| 1 | WP-01, WP-02 (rescoped), WP-05, WP-09 | 2.5 | WP-02 rescope reduces from P0 to P1 hygiene; WP-01 remains P0 BLOCKER |
| 2 | WP-03, WP-04, WP-10, WP-07 | 2.5 | All independent; max parallelism 4 E1/E1a |
| 3 | WP-08, WP-06, WP-13 | 4-5 | WP-08 PG restart requires Linux access; WP-13 needs focused session |
| 4 | WP-11, WP-12 | 3+ | Phased background; WP-12 deferred indefinitely |

**Total effort estimate**: 12-15 sessions (optimistic 10 / pessimistic 18).

### Items I Disagree With or Reprioritize

1. **WP-02 priority P0 -> P1**: The runtime path already calls `donchian_prior()`. The base function retaining current-bar semantics is a footgun for future callers, not a live P0 bug. Downgrade to P1 hygiene.

2. **WP-06 performance items should not all be in one WP**: The Rust Arc<str> migration (E5-P-1, E5-P-14) is a 2-session Rust-only refactor. The Python state_compiler deepcopy (E5-P-2) and time util dedup (E5-P-3) are independent Python work. The orjson migration (E5-P-4) is a separate incremental effort. These should be split into WP-06a (Rust hot path), WP-06b (Python perf), WP-06c (orjson). This also enables better parallel dispatch.

3. **WP-08 MIT-P0-1 PG tuning is operator-only**: PM cannot dispatch sub-agents to modify postgresql.conf on trade-core via ssh. PG tuning (shared_buffers, work_mem) is an operator manual action with a PG restart that takes down all services. Document the recommended values and hand to operator; do not assign to E1/MIT as a code WP.

4. **AI-E-F-01 daily_usd_max=100 needs operator decision, not blind fix**: The $100 vs $2 discrepancy is real, but cutting budget_config.toml to $2 could break current Ollama-based AI flows if they occasionally burst. Correct action: flag to operator, get explicit decision on target value, then apply. Do not auto-fix config that changes runtime AI behavior.

5. **R4 "CRITICAL" findings are P2 documentation drift**: CLAUDE.md saying "14 ADR" when there are 22 is a stale doc string, not a CRITICAL finding. The system works correctly regardless. These are P2 cosmetic.

### Recommended Session Splitting Strategy

**Session A (1 session, Mac)**: WP-01 GUI Safety [E1a] + WP-09 Documentation [TW] -- both are HTML/doc-only, no Rust rebuild, fast cycle. E2 review in same session.

**Session B (1 session, Mac)**: WP-02 rescoped [E1] + WP-03 OU Sigma [E1] + WP-05 Security [E1] -- all Rust/shell, different files, can parallel 3 E1. Single engine rebuild at end. E2+QC review.

**Session C (1 session, Mac)**: WP-04 AI Observability [E1-py + E1-rs] + WP-10 Bybit [E1] -- Python + Rust mix. Engine rebuild for F-09 + BB-A-1.

**Session D (1 session, Mac)**: WP-07 Dead Code [E1] -- analysis and migration planning. Low risk.

**Session E (1-2 sessions, requires trade-core ssh)**: WP-08 ML Pipeline code fixes only (walk-forward purge, training SQL). PG tuning is operator manual action. Cron reconciliation after PA resolves the MIT-P0-2 vs P0-V3-CRON-NOT-INSTALLED conflict.

**Session F (1-2 sessions, Mac)**: WP-06a Rust performance (Arc<str> migration). Large scope, needs focused attention.

**Session G (1 session, Mac)**: WP-13 Reconciler cmd_tx. High risk, needs focused session + E4 regression.

**Sessions H+ (background)**: WP-11 Test Infrastructure, phased. WP-12 deferred.

### Conflict with Existing TODO Active Work

This audit fix plan must be sequenced against the existing active dispatch queue in TODO.md:
- **EDGE-P2-3 Phase 1b pre-IMPL prep** (7 parallel tracks) is currently active. WP-02/WP-03 touch indicator/strategy code that could conflict.
- **W-AUDIT-8a C1** 24h proof is running (PID 4100789, expected complete 2026-05-16 ~19:53 UTC).
- **W-AUDIT-8b** next task is a read-only Stage 0R query/report packet.

Recommendation: Wave 1 WPs (GUI, docs, security) have zero conflict with the existing dispatch. Wave 2 indicator/strategy fixes (WP-02/WP-03) should land BEFORE the next EDGE-P2-3 Phase 1b IMPL wave to avoid merge conflicts in the same Rust files. WP-06 performance work should wait until after EDGE-P2-3 Phase 1b stabilizes.

### Next Action for Operator

1. Review and confirm this PM sign-off + the 5 reprioritization decisions above.
2. Decide on AI-E-F-01 budget_config.toml target value ($2 per DOC-08, or a justified higher cap with DOC-08 amendment).
3. Confirm PG tuning values for trade-core (shared_buffers=2GB, work_mem=64MB) and schedule the PG restart window.
4. Approve session dispatch order (A through H) or reorder based on current priorities.
5. Confirm that the EDGE-P2-3 Phase 1b 3-gate monitoring continues in parallel -- this audit fix plan does not supersede or pause the alpha-path work.
