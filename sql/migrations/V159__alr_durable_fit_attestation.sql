...
BEGIN
    ...
    IF p_mode<>'legacy' AND EXISTS(SELECT 1 FROM pg_roles generic WHERE NOT generic.rolsuper AND left(generic.rolname,3)<>'pg_' AND generic.oid NOT IN(v_writer,v_caller,v_attestor,v_attestor_caller) AND (has_table_privilege(generic.rolname,'learning.alr_challenger_fit_attestations','SELECT') OR has_table_privilege(generic.rolname,'learning.alr_challenger_fit_attestations','INSERT') OR has_table_privilege(generic.rolname,'learning.alr_challenger_training_runs','SELECT') OR has_table_privilege(generic.rolname,'learning.alr_challenger_model_artifacts','SELECT') OR has_table_privilege(generic.rolname,'learning.alr_challenger_registry','SELECT') OR has_function_privilege(generic.rolname,'learning.persist_alr_challenger_fit_attestation_v1(bytea,jsonb,text,text,text,text,text,text,text,text,text,text,text,text,text,text,timestamp with time zone,timestamp with time zone)'::regprocedure,'EXECUTE') OR has_function_privilege(generic.rolname,'learning.persist_alr_challenger_training_result_v2(text,text,text,text,text,text,text,text,text,text,integer,text,text,timestamp with time zone,timestamp with time zone,text,bigint,text,bigint,text,bigint)'::regprocedure,'EXECUTE') OR has_function_privilege(generic.rolname,'learning.read_alr_challenger_training_result_v2(text,text)'::regprocedure,'EXECUTE'))) THEN RAISE EXCEPTION 'V159 catalog FAIL: generic role reachability'; END IF;
END
...
BEGIN
    ...
    IF v_exact_v159 THEN
        IF v_public_functions=3 THEN
            FOR v_spec IN SELECT * FROM (VALUES
              ('learning.persist_alr_challenger_fit_attestation_v1(bytea,jsonb,text,text,text,text,text,text,text,text,text,text,text,text,text,text,timestamp with time zone,timestamp with time zone)','74c4d6a36e37db17c597bf171e577705','attestor','attestor_caller'),
...
CREATE TABLE IF NOT EXISTS learning.alr_challenger_fit_attestations (
    ...
    CONSTRAINT alr_fit_attestations_evidence_check CHECK (
        ...
        AND jsonb_typeof(receipt_projection->'schema_version') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection->'evidence_tier') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection->'claim_kind') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection->'authentication_status') IS DISTINCT FROM 'string' IS FALSE
        ...
        AND jsonb_typeof(receipt_projection#>'{subject,durable_receipt_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{subject,training_key_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{subject,result_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{subject,fit_capture_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{subject,candidate_attestation_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{subject,training_run_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{subject,challenger_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{subject,runner_identity_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{subject,actual_input_material_set_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{subject,ordered_artifact_set_hash}') IS DISTINCT FROM 'string' IS FALSE
        ...
        AND jsonb_typeof(receipt_projection#>'{result_observation,source_head}') IS DISTINCT FROM 'string' IS FALSE
        AND COALESCE(receipt_projection#>>'{result_observation,source_head}','')~'^[0-9a-f]{40}$'
        AND jsonb_typeof(receipt_projection#>'{result_observation,fit_started_at}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{result_observation,fit_completed_at}') IS DISTINCT FROM 'string' IS FALSE
        ...
        AND jsonb_typeof(receipt_projection#>'{result_observation,actual_inputs,dataset_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{result_observation,actual_inputs,row_ids_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{result_observation,actual_inputs,split_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{result_observation,actual_inputs,code_manifest_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{result_observation,actual_inputs,training_config_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{result_observation,actual_inputs,feature_schema_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{result_observation,actual_inputs,label_schema_hash}') IS DISTINCT FROM 'string' IS FALSE
        ...
        AND jsonb_typeof(receipt_projection#>'{result_observation,actual_inputs,training_rows}') IS DISTINCT FROM 'number' IS FALSE
        ...
        AND jsonb_typeof(receipt_projection#>'{result_observation,model,model_schema_version}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{result_observation,model,metrics_hash}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{result_observation,model,resource_usage_hash}') IS DISTINCT FROM 'string' IS FALSE
        ...
        AND jsonb_typeof(receipt_projection#>'{authentication,issuer_id}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{authentication,trust_policy_id}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{authentication,signature_key_id}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{authentication,signature_algorithm}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection#>'{authentication,signature}') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection->'verified_at') IS DISTINCT FROM 'string' IS FALSE
        AND jsonb_typeof(receipt_projection->'expires_at') IS DISTINCT FROM 'string' IS FALSE
