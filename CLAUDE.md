...
> 截至 2026-04-19：tests engine lib **1560 passed / 2 pre-existing WIP fail**（`test_exit_feature_row_*` EXIT-FEATURES-TABLE-1 進行中，非 E5-P2 regression）+ core **380** + e2e **35** + reconciler_e2e **19** + micro_profit_fix_integration **7** · pytest 全量 **2511 passed / 2 pre-existing DYNAMIC-RISK fail / 1 skipped** · **E5-P2 Refactor Wave 2 ✅**（5 P2 派發 → 2 delivered / 3 evidence-based CANCEL / 2 defer；commits **11dedbf**（P2-3 rename `multi_interval_ws → multi_interval_topics` + 零 caller `configure_multi_interval` 刪除 + 2 ordering contract tests）· **822f799**（P2-4 bb_breakout 6 literals + grid_trading cooldown_ms → config，+9 bit-exact default tests，順帶修 GridTrading cooldown_ms TOML 不可達漏洞）；CANCEL：P2-2 onnx_inference（優化已由 EDGE-P3-1 Phase B Step 7b 滿足）· P2-7 directive_handler（R6 cohesion invariant + FIX-08 已拆 fixtures）· P2-8 learning_batch_writer（無 control_api 端真實重複）；defer P2-1/P2-6（EXIT-FEATURES-TABLE-1 WIP 衝突）；1533→**1560** 含 pre-existing WIP 測試、E5-P2 貢獻 +11；**E5-P2-4b follow-up**：bb_breakout 1265 / grid_trading 1434 / strategies/mod.rs 1442 均超 §九 1200 硬上限，分檔拆解）· **E5-P1 Refactor Wave 1 ✅**（6 delivered + 2 cancel；commits ba8cd2c / 76cd793 / d6f7572 / b0dc6b6 / c220375 / 1b72f90；1498→1533） · **P1-16 HALT-SESSION CROSS-SYMBOL PRICE CORRUPTION ✅**（commit `fef688e`；上游 Rust `on_tick.rs::HaltSession` 改用 `close_position_at_symbol_market` helper；下游 Python `realized_edge_stats::_pair_round_trips` price-jump gate + 分母保護；**mean -9.02 bps vs 修前 -2214 bps（245× cleaner）**）· **E5-P0 Refactor Wave ✅** · **LIVE-GATE-BINDING-1 ✅** · **MICRO-PROFIT-FIX-1 ✅** · **P1-8 DUST-EVICTION-GAP-1 E1/E4 ✅** · **P0-10 SCANNER-GATE ✅ 部署** · **P0-5 PHANTOM-2-FUP ✅ 部署** · **P0-9 STABILITY-1 ✅ RCA** · **LIVE-GUARD-1 ✅** · **Phase 5 PAUSED** · **Live_Ready ⚠️** · **下一步**：E5-P1 Wave 1 + E5-P2 Wave 2 整合部署（`restart_all.sh --rebuild`）· E5-P2-4b 策略檔拆解排期 · P0-2 LG-1 21d demo 觀察 → P0-3 edge 重評 · P0-6 intent write / Demo 死循環打破 · **P1-7 LEARNING-PIPELINE-DORMANT-1** · P1-4 真 ETL 首個 ONNX artifact → Stage 2 shadow mode · Phase 2B Strategist 等 G-1 R-02。
