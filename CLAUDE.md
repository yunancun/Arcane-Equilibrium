...
**G-SR-1 Signal Tightening Phase A S1-S4 ✅**（2026-04-13）— 策略訊號收緊基礎設施+參數+測試完成。S1+S2：A0-a `grid_helpers.rs` 提取；A0-b `confluence.rs` 共享模組（PersistenceTracker + compute_score 4 分量 65 分制 + score_to_qty_pct 平滑插值）；A0-c 3 策略 TOML Params struct 加 confluence 字段 + serde(default) + build_confluence_config() + factory 接線 + R4-7 update_params rebuild；A1 時間制持續性過濾器（MA/BBR=120s, BBB=60s）接入 3 策略 entry path；A2 加權匯合評分（trend 25/20/12/8, reversion 15inv/30/10/10, breakout qty-only 10% 底線）；A3 Grid 趨勢冷卻（ADX 60% + Hurst 40%, 1x-6x）。S3：4 策略 param_ranges() 擴展（MA/BBR/BBB +10 confluence ranges, Grid +3 cooldown ranges）+ threshold ordering validation + grid cooldown bounds validation。S4：+41 新測試（12 confluence edge-case + 1 persistence + 28 param/validation/cooldown）。1065 lib + 33 e2e = 1098 tests pass。剩餘：A-E5 性能審查 → S5+ Phase B Agent 接線。計劃文件：`docs/references/2026-04-12--g_sr1_signal_tightening_plan_v2.5.md`。
...
> 截至 2026-04-13：tests engine lib **1065** + e2e **33** = **1098** Rust passed **0 fail** · **G-SR-1 Phase A S1-S4 完成 ✅**（S1+S2 confluence/persistence/cooldown + S3 param_ranges expansion + S4 41 new tests）· **Phase 5 PAUSED**（策略 gross 負 edge）· **Live_Ready ✅** · **下一步**：A-E5 性能審查 → S5 B0+B1 Agent 基礎設施 · LG-1 21d paper 到期（05-01）。
