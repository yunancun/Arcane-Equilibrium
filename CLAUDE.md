...
下一步：Paper Trading 21 天觀察期（延後：L5 元學習 / Regime-aware 策略 / 策略自動優化循環 / main_legacy 重構）
...
Wave B（待執行）：auth.py + state_helpers.py = ~580 行
Wave C（待執行）：control_ops.py + pnl_ops.py + learning_ops.py = ~2760 行
Wave D（待執行）：legacy_routes.py = ~1275 行 → main_legacy.py 瘦身到 ~200 行
```
...
> 截至 2026-04-01 main_legacy.py Wave A 重構完成：5265→4056 行（-1210 行），拆出 state_models.py（391 行）+ state_compiler.py（555 行）+ state_store.py（398 行），re-export 向後兼容，monkey-patch 驗證通過；§十四 代碼結構約定建立（800 行警告/1200 硬上限/依賴方向/monkey-patch 安全/singleton 管理）；系統 demo_only 模式；live_execution_allowed 仍為 false。
