...
# 最后更新：2026-03-27（含 Layer 2 AI 推理引擎设计完成 + Beta 管线 + OpenClaw 融合 + 服务化，全量历史整合版）
...
## 三、当前系统状态（截至 2026-03-27，Layer 2 设计完成 + Beta 管线 + OpenClaw 融合 + 服务化）
...
L    Learning / Self-Observability / Net PnL    ✅ 全部完成（存储 + 手动录入 + 自动学习管线 + 审核队列 + 安全加固）
     ...
     Layer 2 AI 推理引擎                        📐 设计完成（5 模块 + 9 路由 + 4 层搜索降级 + 自适应预算，待编码）
...
docs/references/
  2026-03-27--layer2_ai_reasoning_engine_implementation_plan.md
    → ★ Layer 2 完整实现计划（4 层搜索 + 模型升级 + 自适应预算 + 9 路由 + GUI）
...
已完成：
  ...
  ✅ systemd 服务化（API 服务器开机自启 + 崩溃自动重启）
  ✅ 248 个测试用例全部通过，75 条路由
  📐 Layer 2 AI 推理引擎设计完成（2026-03-27）

下一步（按优先级）：
  ★ Layer 2 AI 推理引擎编码实现
    → 实现计划：docs/references/2026-03-27--layer2_ai_reasoning_engine_implementation_plan.md
    → 5 个新模块 + 1 个测试文件 + main.py 注册
    → 依赖安装：anthropic / duckduckgo-search / beautifulsoup4 / httpx
    → 实现顺序：types → cost_tracker → tools → engine → routes → main → tests
    → 预计新增 9 条路由（75 → 84）
    → 关键特性：
      - 4 层搜索降级：Perplexity → 本地LLM+web-pilot → 本地LLM → DuckDuckGo
      - 搜索后模型升级判断（Haiku triage → Sonnet 或 Opus）
      - 自适应预算（近 7 天 AI ROI → 动态调整预算倍率 0.3x-2.0x）
      - PnL 归因回填 → 学习 AI 花费是否划算
      - 定价表 30 天自动核实提醒
      - GUI AI Budget 控制卡片
      - 每日硬上限 $15（GUI 可调）
  ...
  Telegram 告警通道
    → 接 OpenClaw channels，推送交易告警

  自动循环 cron
    → observer cycle → shadow decision → paper order → fill tick

  AI 咨询接通 H 链
    → 当前为 stub，接 H1-H5 后可真实调 AI
...
> 截至 2026-03-27：A-L 全部完成 + Paper Trading Engine Beta + OpenClaw 融合 + systemd 服务化 + **Layer 2 AI 推理引擎设计完成**（4 层搜索降级 + 模型升级判断 + 自适应预算 + PnL 归因，待编码）。248 测试全通过，75 条路由。系统全程 read_only / disabled / not_granted。下一步：Layer 2 编码实现 → 远程安全访问 → Telegram 告警 → Beta 运行数据积累 → M 章。
