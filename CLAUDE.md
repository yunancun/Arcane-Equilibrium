...
**Runtime（2026-05-01 21:51 CEST · ssh verify · G6-04 §三 drift 規則）**：code-bearing source includes `5ce777b`; the 21:51 Linux runtime sample executed before final docs sync at `4abb36a`. Rust engine runtime remains the `daab51c` scanner deploy (no rebuild/restart for the healthcheck/RFC/docs-only work). Engine PID **2364863** + API uvicorn PID **2047851** + engine_watchdog PID **3450754** + openclaw-gateway PID **3973441** alive。watchdog `engine_alive=true`，demo/live snapshots fresh，paper inactive by design。Manual passive healthcheck wrapper SUMMARY **WARN** exit 0：current notable WARNs are `[4]` / `[10]` / `[27]` / `[33]` / `[38]` / `[40]` / `[41]` / `[11]`; `[27]` is now WARN because demo has signal snapshots but no Guardian attempts, not a persisted-intent writer wedge.

**Active edge state**：Strategy Edge Repair + Strategy Edge Models + scanner market judgement / five-strategy context are deployed. Current observation gates are `[33] maker_fill_rate`, `[38] grid_trading_lifecycle_drift`, `[40] realized_edge_acceptance`, and `[41] scanner_market_gate_confirmation`; interpret rolling windows with post-deploy cutoffs because they still mix old samples. Wave 4 pre-stage RFCs for LG-2 H0 blocking verification, MLDE-6 live promotion contract, and LG-3 provider pricing binding are ready at `5ce777b`. The main live-demo issue remains fee quality / realized edge; grid lifecycle drift is now near threshold but still rolling-WARN. Primary metric remains post-fee `net_bps_after_fee`; PnL and win-rate are secondary references.
...
**Next decisions**：Continue post-cutoff edge observation from scanner/five-strategy deploys. Latest manual wrapper at 2026-05-01 21:51 CEST: `[33]` 7d maker_like 27.2% / fee_drop 22.0%; `[38]` lifetime_ratio 0.47 WARN (demo p50 7.9min vs live_demo 3.7min; live re-entry delta WARN); `[40]` 24h rows=41 avg_net -19.26bps; `[41]` gates fired but labels insufficient; `[11]` is WARN not FAIL because the 2d rolling replay shrank as old exits aged out; `[27]` only FAILs when approved verdicts exist with zero persisted intents, while current signal-only/pre-gate state remains WARN. G2-02 ma_crossover dual-track around 2026-05-03; P0-2 + G2-01 PostOnly acceptance around 2026-05-07/08; P0-3 edge decision around 2026-05-15.
