...
**Runtime（2026-05-01 22:29 CEST · ssh verify · G6-04 §三 drift 規則）**：Linux source has loaded `[22]` healthcheck calibration `b283fda`; code-bearing source also includes G8-05/LG-5 checkpoint `25d8e54` pending final docs sync. Rust engine runtime remains the `daab51c` scanner deploy (no rebuild/restart). Engine PID **2364863** + API uvicorn PID **2047851** + engine_watchdog PID **3450754** + openclaw-gateway PID **3973441** alive。watchdog `engine_alive=true`，demo/live snapshots fresh，paper inactive by design。Manual passive healthcheck wrapper SUMMARY **WARN** exit 0；`[22]` now WARNs with `working_maker_orders_1h=3` and `risk_30m=18` instead of false-FAILING maker no-fill as writer wedge.
...
**Next decisions**：Continue post-cutoff edge observation from scanner/five-strategy deploys. Latest manual wrapper at 2026-05-01 22:29 CEST: `[22]` WARN maker no-fill context; `[33]` 7d maker_like 27.2% / fee_drop 22.0%; `[38]` lifetime_ratio 0.41 WARN (demo p50 7.9min vs live_demo 3.2min); `[40]` 24h rows=39 avg_net -17.97bps; `[41]` gates fired but labels insufficient; `[11]` is WARN not FAIL because the 2d rolling replay shrank as old exits aged out; `[27]` only FAILs when approved verdicts exist with zero persisted intents, while current rejected-only risk/cost-gate state remains WARN (`verdicts_30m=15`, `approved_verdicts_30m=0`). G2-02 ma_crossover dual-track around 2026-05-03; P0-2 + G2-01 PostOnly acceptance around 2026-05-07/08; P0-3 edge decision around 2026-05-15. Rank 9 HTTPS deploy still needs explicit runtime/deploy approval; next safe non-deploy candidates are P03-PREP-1, DOC-1, TEST-1, or PRE-LIVE-1.
