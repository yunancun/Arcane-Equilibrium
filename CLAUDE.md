...
**Runtime**：`Live_Ready` ⚠️（2026-04-22 23:35 CEST `--rebuild` 部署 3 P0 fixes 後刷新）— LIVE-P0/P1/P2 代碼完整、單測 **1866 / 0 failed**（Mac + Linux release），**仍 0 真實 live 流量**。**P0-13 ATR scale + P0-14 A Gate 1 fallback + P0-14 B JS proxy cells runtime live**：engine PID **213144**（上一 158918 被 23:35 `--rebuild` 替換）· uvicorn PID **213195** · binary mtime **2026-04-22 23:35**（baseline HEAD `9710ff9`）· atr_pct 改由 `kline_manager.get_ohlcv("1m",20) + indicators::atr(14)` 提供（持倉期 Wilder's ATR，~0.05-0.5% scale），舊 per-tick `compute_atr_pct` 加 `#[deprecated]` 保留 fast_track 用 · `edge_estimates.json` 由 Python `_inject_sync_label_proxy_cells` 從 43→**135 cells**（+4 sync-label strategies × 23 symbols = 92 proxy cells）· `ExitConfig.missing_edge_fallback_bps = -10.0` runtime 熱重載可調 · Priority 6 每 tick 呼 `physical_micro_profit_lock_v2` + `ExitConfig`，cold-start 15 min kline ATR 累積中，24h 後驗 `phys_lock_*` fire 1-10/day + DB `learning.exit_features.giveback_atr_norm` avg 從 364 → ~0.3-3.0 + `atr_pct` avg 從 0.003 → ~0.05-0.5。healthcheck [7] 135/135 PASS 即時驗證。詳 `docs/worklogs/2026-04-22--passive_wait_silent_fail_audit.md` + `2026-04-22--p0_13_14_execution_resume_plan.md`。**真實 live 門控**（Rust 可驗證 4 項 / 全部 5 項，詳 §四）：(1) Python `live_reserved` global mode (2) Python Operator 角色 auth (3) `OPENCLAW_ALLOW_MAINNET=1` env（僅 Mainnet）(4) secret slot 有 api_key + api_secret (5) `authorization.json` HMAC 簽名+未過期+env_allowed 匹配。`execution_authority` 在 Rust 僅為 P0/P1 denylist 字串常量（`claude_teacher/applier.rs:226`），非真實授權邏輯。Live 縮倉監控 5min 輪詢（代碼已寫、e2e 測試綠，**從未真實觸發**）。
