...
**Runtime**：`Live_Ready` ⚠️（2026-04-16 audit 修正 · 2026-04-22 基準刷新）— LIVE-P0/P1/P2 代碼完整、單測 **1835 / 0 failed**（Linux release，2026-04-22 TRACK-P-V2-SWAP-1 退役 8 個 v1 直測後的新基準；v2 等值覆蓋在 `exit_features/v2.rs` 25 單測），2026-04-21 20:44 CEST `--rebuild` 部署後 **仍 0 真實 live 流量**（歷史 43k 條 `engine_mode="live"` 實為 LiveDemo；當前 authorization.json 未簽，live pipeline 拒啟）。**TRACK-P-V2-SWAP-1** (`306993e`, 2026-04-22) 完成但 **runtime 未部署**（operator 指示先不部署），engine PID 3954769 仍跑 v1 linear；Priority 6 v2 non-linear giveback 待下次 `restart_all.sh --rebuild` 生效。**真實 live 門控**（Rust 可驗證 4 項 / 全部 5 項，詳 §四）：(1) Python `live_reserved` global mode (2) Python Operator 角色 auth (3) `OPENCLAW_ALLOW_MAINNET=1` env（僅 Mainnet）(4) secret slot 有 api_key + api_secret (5) `authorization.json` HMAC 簽名+未過期+env_allowed 匹配。`execution_authority` 在 Rust 僅為 P0/P1 denylist 字串常量（`claude_teacher/applier.rs:226`），非真實授權邏輯。Live 縮倉監控 5min 輪詢（代碼已寫、e2e 測試綠，**從未真實觸發**）。
...
> 截至 2026-04-22：engine lib **1835 / 0 failed**（Mac debug + Linux release `cargo test -p openclaw_engine --lib`；TRACK-P-V2-SWAP-1 退役 8 個 v1 直測後的穩定基準，v2 等值覆蓋由 `exit_features/v2.rs` 25 單測承擔；TICK-PIPELINE-MOD-SPLIT-1 為純 refactor 零測試數變化）+ bin 38 · pytest 全量 **2866 / 0 fail / 14 skipped** · **Live_Ready ⚠️**（門控 5 項，Rust 可驗證 4 項）· **Phase 5 PAUSED**（demo 2w 後 P0-3 重評）· **`tick_pipeline/mod.rs` 2274 → 1012** 行（§七 1200 硬上限符合，TICK-PIPELINE-MOD-SPLIT-1 `3d67a99` sub-agent 執行；impl 拆 3 sibling child-module 檔）· **Step 0 衍生新 TODO 章節 5/5 ✅ 歸檔**（2026-04-22 `docs/archive/2026-04-22--step_0_derived_todo_batch.md`）· **TRACK-P-V2-SWAP-1 ✅ merged**（2026-04-22 `306993e`：Priority 6 v1 linear → v2 non-linear + `ExitConfig`；`RiskConfig.phys_lock` → `RiskConfig.exit`，v1 pure fn + `PhysLockConfig` + 8 v1 直測整塊退役，淨 LOC −260；**runtime 未部署**，operator 指示先不部署，engine PID 3954769 仍跑 v1；下次 `--rebuild` 生效）· **EDGE-P2-3 Phase 2+ (b) bb_breakout + ma_crossover PostOnly entry wiring ✅ merged**（2026-04-21，demo/paper=true、live=false；runtime 待 `--rebuild`）· **DUAL-TRACK-EXIT-1 Phase 1b Track P 全鏈 ✅**（v2 pure fn `aee96b9` + GATE1-REVERSAL-1 hotfix A `d0f0c21` + **T4 runtime 接線 `e95c779` 2026-04-21** + **V2-SWAP `306993e` 2026-04-22**；Priority 6 每 tick 評估，合法 Lock 唯二 `phys_lock_gate4_giveback` / `phys_lock_gate4_stale_roc_neg`；v2 runtime 待 `--rebuild` 部署）· 主路徑：P0-6 intent write → P0-7 order submit → P0-2 21d demo → P0-3 edge 重評 → Live（最早 W24 末 ~2026-05-23）。活躍細節 → §三 · commit 歷史 → `docs/CLAUDE_CHANGELOG.md` · 里程碑敘述 → §三「已完成里程碑索引」+ `docs/archive/`。
