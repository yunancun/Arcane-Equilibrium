...
- **INTENT-WRITE-GAP-1 (P0)** ✅ **2026-04-18 commit `2a36a3f` 修復**：Rust exchange 分支補 `persist_intent` 接線；2026-04-21 24h 驗證 `trading.intents` demo **144 rows** ✅；live/live_demo 0 rows 為預期（authorization.json 未簽 → Python Gate #1 live_reserved 不可達）。**不再** P0-CRITICAL。
- **ORDER-SUBMIT-GAP-1 (P0)** ✅ **ARCHIVED**：2026-04-16 RCA 證偽，P0-6 子問題；2026-04-21 24h `trading.fills` demo **123 rows** ✅，P0-6 永久修復後閉環。
...
- ~~**Track P v2 swap pending**~~ ✅ 2026-04-22 `306993e`（見 §三 milestone index）
- ~~**tick_pipeline/mod.rs 2276 行超 §七 1200 硬上限**~~ ✅ 2026-04-22 `3d67a99`（見 §三 milestone index）
...
| 2026-04-22 | **TICK-PIPELINE-MOD-SPLIT-1** ✅ commit `3d67a99`（sub-agent 機械 split — `tick_pipeline/mod.rs` 2274 → **1012** 行進 §七 1200 硬上限，降 55.5%；`impl TickPipeline` 巨塊 L906-2178 ~1272 行拆 3 sibling child-module 檔（每檔 `impl super::TickPipeline { ... }`）：`pipeline_ctor.rs` 422（ctor + 基本 setters/getters）· `pipeline_config.rs` 300（config sync risk/budget/maker_kpi/news/fee/account）· `pipeline_helpers.rs` 654（close + exit features + channel setters + misc）；mod.rs 保留 types/struct/enum-impls/free-fns（`parse_exit_tag`/`snapshot_to_input`）+ mod decls；8 個被 on_tick/ 跨檔呼的私有 fn 從 `fn` 升 `pub(super) fn`（sync_risk_config_if_changed / sync_maker_kpi_config_if_changed / current_cost_edge_max_ratio / current_min_profit_to_close_pct / close_position_at_symbol_market / emit_close_fill / should_persist_signal / derive_regime），`apply_risk_snapshot` + `build_exit_feature_row` 同檔 caller 保持私有；零邏輯改動純機械 move；Mac debug + Linux release `cargo test -p openclaw_engine --lib` 均 **1835 passed / 0 failed**（與 pre-split 同 baseline）；淨 LOC +114（+1376 新檔 −1273 mod.rs 刪 +11 mod.rs 加）；ON-TICK-SPLIT-1 衍生 backlog 清盤；`Step 0 衍生新 TODO` 整章節 5/5 ✅ 歸檔至 `docs/archive/2026-04-22--step_0_derived_todo_batch.md`） · **TRACK-P-V2-SWAP-1** ✅ commit `306993e`（Priority 6 由 v1 `risk_checks::physical_micro_profit_lock` + `PhysLockConfig`（線性 `giveback_atr_norm_threshold=0.7`）切 v2 `exit_features::physical_micro_profit_lock_v2` + `ExitConfig`（非線性 `threshold = max(base − slope × peak_atr_norm, floor)`，seed `base=1.0`/`slope=0.15`/`floor=0.3`）；`RiskConfig.phys_lock: PhysLockConfig` → `RiskConfig.exit: ExitConfig`，`#[serde(alias = "phys_lock")]` 保 TOML 相容（三環境 TOML 無此 section 全走 Default，alias 為保險）；ArcSwap 熱重載沿用既有 `Arc<ArcSwap<RiskConfig>>`；v1 pure fn + `PhysLockConfig` struct + 8 個 v1 直測整塊退役（v2 在 `exit_features/v2.rs` 有等值 25 單測）；reason-string ABI 不變（`phys_lock_gate4_giveback` / `phys_lock_gate4_stale_roc_neg`），下游 `strip_phys_lock_prefix` + `parse_exit_tag` 零改；Mac debug + Linux release `cargo test -p openclaw_engine --lib` 均 **1835 / 0 failed**（1843 baseline − 8 退役 v1 直測 = 1835，精確對帳）；淨 LOC −260；**runtime 未部署**（operator 指示先不部署），engine PID 3954769 仍跑 v1 linear；v2 non-linear giveback 在下次 `--rebuild` 後生效；memory `project_track_p_runtime_dead.md` supersede 為 `project_track_p_runtime_live.md`；報告 `.claude_reports/20260422_200623_track_p_v2_swap.md`） |
...
| 2026-04-21 | **EDGE-P2-3 Phase 2+ (b) bb_breakout + ma_crossover PostOnly entry** ✅ merges `f5f4dc2` + `8280132`（feature commits `b2d8ac5` ma / `9edc6a4` bb 於 2026-04-20；rebase 解衝突後 `--no-ff` merge 進 main — bb_breakout.rs 7 hunks + 三 env TOML 3 hunks 手動保留 EDGE-P2-2 OI + EDGE-P2-3 PostOnly 兩組正交 feature；ma_crossover 自動 merge；三環境 TOML `[bb_breakout]` + `[ma_crossover]` 加 `use_maker_entry`/`maker_price_offset_bps`/`maker_limit_timeout_ms`，demo/paper=true、live=false；fee routing 沿用 `intent_processor::fee_rate_for_intent`；engine lib 1819 → **1827** passed / 0 failed（debug，baseline +4 bb +4 ma PostOnly tests）；**runtime 未部署**，engine PID 3813984 不含此兩 commit，下次 `restart_all.sh --rebuild` 才進 runtime；`bb_breakout_postonly`/`ma_crossover_postonly` 本地分支 + `origin/feature/p1-16-h0-gate-deterministic` 遠端分支皆已可清） · **TODO 1+2 outcome_backfiller wiring fix** ✅ commit `5e2981d`（`outcome_backfiller.rs` 7 處 timeframe 字串 `'1'→'1m'`/`'5'→'5m'`/`'60'→'1h'`/`'240'→'4h'` + `engine_mode` INSERT 接線 + 3 新 regression tests；歷史回填 267k rows engine_mode + 268k rows outcome_*；engine lib baseline 1819 建立；live_demo 恢復 77.9% outcome_1m non-NULL） · **DUAL-TRACK-EXIT-1 Phase 1b Track P v2 pure fn** ✅ commit `aee96b9`（`exit_features.rs` +698：`physical_micro_profit_lock_v2` 非線性 giveback 4-Gate pure fn + `ExitConfig` 7 參數 + 31 單測；QC 反轉 Gate 1 `edge <= floor → Hold` 對齊設計意圖；engine lib 1791 → **1816**；**runtime 接線於 `e95c779`**） · **GATE1-REVERSAL-1 hotfix A** ✅ commit `d0f0c21`（v1 Priority 6 Gate 1 同步反轉 Lock → Hold；3 tests rename + assert 反轉；engine lib 仍 **1816**；**runtime 接線於 `e95c779`**） · **DECISION-OUTCOMES-DEAD-1 RCA + Linux 驗證** commit `5a2c241` + `db65458` 情境 3 doc-only **被推翻** — writer 活躍 ✓ 但 outcome_* 100% NULL 是 timeframe 字串格式不一致 (`'1'` vs `'1m'`)、engine_mode 100% 'paper' 是 INSERT 漏接線 → 拆 3 新 P1 TODO（`DECISION-OUTCOMES-ENGINE-MODE-TAG-BUG-1` / `OUTCOME-BACKFILL-JOIN-NULL-1` / `GATE1-REVERSAL-OBSERVABILITY-1`；後者 Linux audit 後降級為 doc-only 關閉） · **TRACK-P-T4-WIRING-1** ✅ commit `e95c779`（2026-04-21 晚 3 · 主軸解阻塞）— `tick_pipeline/on_tick.rs:1677` `|_| None` 替換為實際 closure：查 `paper_state.position_exit_snapshot` + `price_tracker.compute_roc(300ms)` + `intent_processor.edge_estimates().get_cell` → 新 pure fn `exit_features::build_exit_features_for_tick`（鏡像 close-time `build_exit_feature_row` 7 維衍生規則）→ 產 `Some(ExitFeatures)`；Option::None → 4-Gate 保守 Hold（fail-soft，零 regression）；engine lib 1827 → **1839**（+12 builder tests 覆蓋 happy/short-side/fresh-high/None/非正 ATR/legacy peak/clock skew/entry 0∞/end-to-end v2 Gate 4a Lock/missing edge Hold）；Mac debug + Linux release 均驗 1839 passed / 0 failed；**runtime 待 `--rebuild` 部署**，部署後 Priority 6 每 tick 評估，合法 Lock 唯二 `phys_lock_gate4_giveback` / `phys_lock_gate4_stale_roc_neg`；edge_estimates 冷啟動時 Gate 1 全 Hold（預期 fail-safe）；memory `project_track_p_runtime_dead.md` 待下次 audit 改寫為 `project_track_p_runtime_live.md` · 後續 TODO `TRACK-P-V2-SWAP-1` (P2，~1d) 把 Priority 6 從 v1 linear 切 v2 non-linear + ExitConfig 熱重載 · **memory** 新增 `project_dev_runtime_split.md`（Mac 接手三連 engine 檢查 noop） · **EXIT-FEATURES-SPLIT-1** ✅ commit `3a9b988`（1317 行 `exit_features.rs` 拆為 `exit_features/{mod,core,v2,builder}.rs` 四檔，全 ≤ 800 soft warn；re-export 保向後相容） · **AI-SERVICE-CLIENT-ENV-RACE-1** ✅ commit `580304a`（3 env-racy tests 改呼 pure fn `resolve_socket_path_from(sock, data_dir)` 零 env 動作 + 1 新優先序測試；engine lib +1 = 1840） · **ON-TICK-SPLIT-1** ✅ commit `bfedb56`（2071 行 `on_tick.rs` 經 sub-agent 並行派發拆為 `tick_pipeline/on_tick/{mod,helpers,step_0/step_0.5/step_1_2/step_3/step_4_5/step_6}.rs` 八檔；`ControlFlow<Break, Continue>` 串接 6 step + owned state threading；step_4_5 929 行超 soft warn 但 borrow-check 限制下無法再拆；T4 closure 保留 step_6 內） · **CANARY-WRITER-ENV-RACE-1** ✅ commit `d454c17`（pure fn `decide_canary_enable_from` + 3-variant enum 保留雙 disable 語意；7→10 tests；engine lib +3 = 1843）+ **TICK-PIPELINE-MOD-UNUSED-IMPORTS-1** ✅ commit `c164cb6`（`tick_pipeline/mod.rs` 刪 `RiskAction`/`Instant`/`debug` 3 unused `use`；warnings 13 → 10；step files 無改動，Option A）— 兩者並行派發 sub-agent |
...
> 截至 2026-04-23 21:13 CEST：engine lib **1910 / 0 failed**（Mac + Linux release `cargo test -p openclaw_engine --lib`，2026-04-23 session 累積 +75 vs 1835 baseline：earlier +31 + WS retire writer tests +11 + INFRA-PREBUILD-1 Part A +28 + Part B +5）+ bin 38 · pytest 全量 **2996 / 0 fail / 1 skipped**（2026-04-23 多 commit 累積：`b4e1308` 修 4 test 檔 Py 3.12 `asyncio.get_event_loop()` 相容性 +58 → 3006；`78f31eb` 刪 backfill test −12 → 2994；`eadd31d` TEST-BROADEN-1 +2 → 2996） · **Live_Ready ⚠️**（門控 5 項，Rust 可驗證 4 項）· **Phase 5 PAUSED**（demo 2w 後 P0-3 重評）· **`tick_pipeline/mod.rs` 2274 → 1012** 行（§七 1200 硬上限符合，TICK-PIPELINE-MOD-SPLIT-1 `3d67a99` sub-agent 執行；impl 拆 3 sibling child-module 檔）· **Step 0 衍生新 TODO 章節 5/5 ✅ 歸檔**（2026-04-22 `docs/archive/2026-04-22--step_0_derived_todo_batch.md`）· **TRACK-P-V2-SWAP-1 ✅ runtime live**（2026-04-22 `306993e` + 20:55 CEST `--rebuild` 部署 engine PID **158918**：Priority 6 v1 linear → v2 non-linear + `ExitConfig`；`RiskConfig.phys_lock` → `RiskConfig.exit`，v1 pure fn + `PhysLockConfig` + 8 v1 直測整塊退役，淨 LOC −260）· **EDGE-P2-3 Phase 2+ (b) bb_breakout + ma_crossover PostOnly entry wiring ✅ merged**（2026-04-21，demo/paper=true、live=false；runtime 待 `--rebuild`）· **DUAL-TRACK-EXIT-1 Phase 1b Track P 全鏈 ✅ runtime live**（v2 pure fn `aee96b9` + GATE1-REVERSAL-1 hotfix A `d0f0c21` + **T4 runtime 接線 `e95c779` 2026-04-21** + **V2-SWAP `306993e` 2026-04-22** + **2026-04-22 20:55 CEST `--rebuild` 部署 engine PID 158918**；Priority 6 每 tick 呼 v2 非線性 giveback，合法 Lock 唯二 `phys_lock_gate4_giveback` / `phys_lock_gate4_stale_roc_neg`）· **WS-RETIRE-1 ✅ runtime live**（2026-04-23 21:13 CEST `--rebuild` engine PID **764820** / uvicorn **764878** / baseline HEAD `f42face`：Rust `bybit_private_ws_status_writer.rs` 5s 週期產 `bybit_private_ws_listener_status_latest.json`（`listener_version: "rust-v1"` / `engine_mode: "demo"` / `auth_ok_count: 1` / 4 topics 活），取代已退役 Python `bybit_private_ws_listener.py` 3 檔 340 行；`spawn_private_ws_supervisor` 在 Demo/LiveDemo 條件 spawn + cancel-aware final `running=false` 寫入確保 observer 乾淨停止狀態；readonly_observer_pipeline `bybit_build_ws_runtime_facts.py` smoke 通過）· **DEDUP-PY-RUST 獨立重審全鏈閉環 A+B+C+D** ✅ 5 commits `87e3ecf`/`16acb64`/`b9b0a57`/`b5cf59e`/`f42face`（A 刪 `program_code/governance/` 7 檔 284 行 · B docs canonical path note 清理 · C Rust writer +685 + Python listener -340 · D 刪 98 legacy H/I/J/K-chain 修復 shell + shim wrapper；淨減 shell/Python ~12.8k；3 並行 Explore sub-agent 獨立核實 + 主會話修正 Agent 1 對 `local_model_tools/` 19 個 singleton 契約 stub 的誤刪判斷 + Agent C「Rust never spawned」誤判；SCRIPT_INDEX + LOGICAL_SCRIPT_CATEGORY_MAP 同步更新；2 份 `.claude_reports/20260423_*.md` 歸檔）· **INFRA-PREBUILD-1 Part A dormant + Part B dormant**（shadow_exit_writer flag OFF / model_registry empty，runtime 零影響；下輪觸發分別為 TOML flip 與 P1-7 C labels 200 滿）· 主路徑：P0-6 intent write → P0-7 order submit → P0-2 21d demo → P0-3 edge 重評 → Live（最早 W24 末 ~2026-05-23）。活躍細節 → §三 · commit 歷史 → `docs/CLAUDE_CHANGELOG.md` · 里程碑敘述 → §三「已完成里程碑索引」+ `docs/archive/`。
