...
# 最后更新：2026-04-01（Wave 7a Spot 品類啟用 + 方案A/B symbol-category 映射 · 3161 tests）
...
```
测试：2,624 passed（Wave 6 Sprint 0+1a+1b 全部完成後，17 pre-existing failures）
...
★ 品類基礎設施審計（2026-04-01）：
  ...
  Spot（634 幣對）：70% 就緒，3 個 blocker（掃描器硬編碼 + position 無 category + 保證金邏輯）
  Inverse（27 幣對）：40% 就緒，5 個 blocker（PnL 公式錯 + 掃描器 + qty 步長 + symbol 命名 + 無策略）
  計劃：Wave 7a 先啟用 Spot，Wave 7b 再做 Inverse
...
Wave 7b：📋 Inverse 品類完善 — PnL 公式(CRITICAL) + 掃描器 + qty + 風控（待辦，TODO.md）
         方案 A 補完：TradeIntent.metadata["category"] 必填（有 Registry 後安全）

下一步：Wave 7b Inverse 品類完善（INV-1 CRITICAL：Paper Engine PnL 公式修正）
...
> 截至 2026-04-01 Wave 7a + 方案 A 完成：Wave 7a 啟用 Spot 品類（SPOT-1~5 全通，+48 測試，方案 B _symbol_category_map 雙向注入）；方案 A SymbolCategoryRegistry 啟動時批量填充 symbol→category（+19 測試），消除命名猜測問題；設計決策文件已存檔；3161 tests 通過；Wave 7b Inverse 品類（INV-1 PnL 公式 CRITICAL）為下一步；系統 demo_only 模式；live_execution_allowed 仍為 false。
