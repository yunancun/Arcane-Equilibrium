...
> 截至 2026-04-21：engine lib **1843 / 0 failed**（Mac debug + Linux release `cargo test -p openclaw_engine --lib`；TRACK-P-T4-WIRING-1 +12 builder tests + AI-SERVICE-CLIENT-ENV-RACE-1 +1 precedence test + CANARY-WRITER-ENV-RACE-1 +3 decision tests；EXIT-FEATURES-SPLIT-1 / ON-TICK-SPLIT-1 / TICK-PIPELINE-MOD-UNUSED-IMPORTS-1 為純檔案/import 清理無測試數變化；build warnings 13 → 10）+ bin 38 · pytest 全量 **2866 / 0 fail / 14 skipped** · **Live_Ready ⚠️**（門控 5 項，Rust 可驗證 4 項）· **Phase 5 PAUSED**（demo 2w 後 P0-3 重評）· **EDGE-P2-3 Phase 2+ (b) bb_breakout + ma_crossover PostOnly entry wiring ✅ merged**（2026-04-21，demo/paper=true、live=false；runtime 待 `--rebuild`）· **DUAL-TRACK-EXIT-1 Phase 1b Track P 全鏈 ✅**（v2 pure fn `aee96b9` + GATE1-REVERSAL-1 hotfix A `d0f0c21` + **T4 runtime 接線 `e95c779` 2026-04-21**；Priority 6 每 tick 評估，合法 Lock 唯二 `phys_lock_gate4_giveback` / `phys_lock_gate4_stale_roc_neg`；runtime 待 `--rebuild` 部署）· 主路徑：P0-6 intent write → P0-7 order submit → P0-2 21d demo → P0-3 edge 重評 → Live（最早 W24 末 ~2026-05-23）。活躍細節 → §三 · commit 歷史 → `docs/CLAUDE_CHANGELOG.md` · 里程碑敘述 → §三「已完成里程碑索引」+ `docs/archive/`。
