...
| Engine status | Runtime binary code line remains rebuilt `7b33ab2e`; pre-v31 Mac/origin/Linux source was verified clean/synced at `d9532e17`, and TODO v31 is source/docs/probe-only without rebuild/restart. Signed live authorization is absent; true-live remains blocked. |
...
| W-AUDIT-8a / alpha candidates | Phase C0 closed; C1 24h standalone WS proof is running but liquidation revival remains blocked until BB/MIT sign-off. A4-C is archived from promotion and `P1-A4C-RCA-1` is closed no-revive; `W-AUDIT-8b` Funding Skew spec v0.1 is the active QC/MIT/BB review + Stage 0R replay design lane; `W-AUDIT-8c` Liquidation Cluster waits for C1。 |
...
- TODO.md v31 active dispatch queue + `active-plan.md` v1.8
