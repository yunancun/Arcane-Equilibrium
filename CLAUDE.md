...
| 項 | 目前事實 |
|---|---|
| Current state sources | `TODO.md` v40 + archive `2026-05-16--todo_v36_completion_cleanup_archive.md` + `2026-05-16--stage1_demo_a4c_tombstone_cleanup.md` + PM reports `2026-05-15--stage0r_preflight_verification.md` / `2026-05-15--stage0r_preflight_step5b.md` / `2026-05-15--stage0r_oi_confirmed_5m_preflight.md` / `2026-05-15--feature_baseline_restore.md` / `2026-05-15--p1_healthcheck_55_invariant.md` / `2026-05-15--p1_intent_freeze_27_post_grace_closure.md` / `2026-05-15--w_audit_8a_phase_c0_liquidation_inventory.md` / `2026-05-15--micro_profit_alpha_prework.md` / `2026-05-15--todo_v30_three_side_sync.md` / `2026-05-15--a4c_rca_final_and_c1_proof_start.md` / `2026-05-15--w_audit_8b_review_stage0r_design.md` / `2026-05-16--v35_three_side_sync_rebuild.md` / `2026-05-16--wave_3_5_linux_migration_backlog_closure.md` / `2026-05-16--p2_maintenance_hygiene_closure.md` + direct `trade-core` read-only checks below。 |
...
- A4-C BTC→Alt Lead-Lag is a tombstone only in active state: archived from promotion/no-revive for the BTC 1m return + xcorr feature shape after Step 5b and RCA remained red. Do not open `P1-A4C-REV-1`, rerun the same feature shape, or select A4-C as a Stage 1 Demo cohort source. Keep `panel.btc_lead_lag_panel` and `[57]` diagnostic-only; future reopen requires a materially new predictive variable plus fresh Stage 0R `eligible_for_demo_canary=true` evidence.
...
- W-AUDIT-8a Phase C0 is **SOURCE/DOC CLOSED 2026-05-15**：`market.liquidations` exists but has 0 rows；production topic builders are guarded against `liquidation.*` / `price-limit.*` / `adl-notice.*` / `allLiquidation*`；C1 60s smoke passed as `SMOKE_PASS_NOT_C1_PROOF`。The 24h isolated `allLiquidation.BTCUSDT` proof started on `trade-core` at `2026-05-15T19:53:09Z` and ended early at `2026-05-16T00:37:25Z` with `FAIL_CONNECTION` after `17055.2s/86400s`; it saw 15 candidate liquidation messages, 0 subscribe failures, and a connection-lost error, so it is **not C1 proof-eligible**。C1 remains blocked until a full-duration BB + MIT signed proof passes。Replay can verify local fail-closed `LiquidationCascade` behavior, but cannot prove real Bybit WS topic safety。C1 probe plan/script exists: `docs/execution_plan/2026-05-15--w_audit_8a_c1_liquidation_topic_probe_plan.md` + `helper_scripts/bybit/liquidation_topic_probe.py`。
- W-AUDIT-8b Funding Skew Directional is **CONDITIONAL STAGE 0R DESIGN APPROVED 2026-05-15**：QC/MIT/BB allow read-only Stage 0R replay design only, not strategy implementation or demo spend。Spec v0.2 locks 30m primary horizon, branch-separated crowded-long fade / crowded-short squeeze hypotheses, `K_total >= K_prior+4050`, `DSR>=0.95`, PBO fail-closed, raw panel as-of joins, funding attribution `excluded`, and BB funding interval/source-mode fields。Runtime panel freshness probe at 2026-05-15 22:13 CEST passed (`funding=PASS(20929ms)`, `oi=PASS(20969ms)`)。Next task is PA/E1 read-only query/report packet。
...
- Legacy paper promotion paths are frozen by AMD-2026-05-15-01：Stage 0R replay preflight can only emit `eligible_for_demo_canary=true/false`; Stage 1 evidence must be `Environment::Demo` micro-canary after a future green preflight. There is no active W3 paper-cohort lane.
...
| `[27]` intents counter freeze | ✅ POST-GRACE CLOSED 2026-05-15 18:12 UTC：direct narrow probe PASS (`demo stale=3.4m, 30min_n=4`; `live_demo` inactive in 30m with verdicts/DCS=0; `live` never produced an intent)。 | `P1-INTENT-FREEZE-27` cleared; Stage 1 demo remains blocked by alpha gates, not by `[27]`。 |
...
| Blocker | 狀態 |
|---|---|
| W3 Sprint | P0 W3-1 / W3-2 remain blocked on `ncyu`（no status change in this maintenance commit）；non-P0 status: W3-3 ✅ / W3-4 ✅ / W3-5 ✅ / W3-6 🔄。Stage 1 Demo micro-canary is no longer tied to a W3 paper cohort or A4-C; it remains blocked until a future non-tombstoned strategy×symbol Stage 0R packet is green。`[55]` is source-cleared by P1-HEALTHCHECK-55-INVARIANT。 |
| Runtime healthcheck | Latest hard-blocker update is the 2026-05-15 18:12 UTC direct `[27]` PASS closure; full passive wrapper previously hung after rebuild, so a later full-suite rerun is useful housekeeping rather than the active `[27]` blocker。 |
...
| Paper / Stage 0R | Paper disabled and non-promotional；Stage 1 is Demo-only after future green Stage 0R；OI-confirmed 5m is only a future Stage 0R candidate spec。 |
| W-AUDIT-8a / alpha candidates | Phase C0 closed; C1 standalone WS proof ended `FAIL_CONNECTION` after `17055.2s/86400s` and is not proof-eligible, so liquidation revival remains blocked until a full-duration BB/MIT sign-off. A4-C is archived from promotion and `P1-A4C-RCA-1` is closed no-revive; `W-AUDIT-8b` Funding Skew spec v0.2 review/design is done and waits for a read-only Stage 0R query/report packet; strategy implementation remains blocked. `W-AUDIT-8c` Liquidation Cluster waits for C1。 |
...
**Guard A/B/C 強制**（2026-04-24 V023 silent-noop postmortem 衍生 + 2026-05-02 V028-V034 retrofit 補完）：任何 `CREATE TABLE IF NOT EXISTS` 前必加 Guard A 驗欄位俱在；任何型別敏感的 `ADD COLUMN` 必加 Guard B 驗 `data_type`；hot-path 索引選用 Guard C 比對 `pg_get_indexdef()`。模板：`sql/migrations/templates/schema_guard_template.sql`。Idempotency 強制（per MIT MUST 4 AMD-2026-05-11-W6-1）：每個 migration 本地跑兩次 `psql -f V<NNN>__<desc>.sql`，第二次必須不 RAISE — i.e. **idempotent：重複執行不產生額外 side effect**（no schema corruption + no incorrect data state）（UPDATE row count 非 0 是預期，前提是 idempotent UPSERT 邏輯不破不變式）。Range：V001-V027 多數 pre-postmortem 期未加 Guard（歷史視 idempotency 風險可接受）；V023+V028~V034 已 retrofit。詳述 → `docs/archive/2026-05-02--CLAUDE-pre-trim-snapshot.md`。
...
| `LiveCmdSenderSlot` / `DemoCmdSenderSlot` | rust/openclaw_engine/src/ipc_server/engine_routing.rs | Rust 端 pipeline command sender slots。`LiveCmdSenderSlot` 跟隨 LiveAuthWatcher rotation；`DemoCmdSenderSlot` 由 WP-13 + `a7cb517f` 擴到 demo reconciler、strategist scheduler tune-target、edge-estimate reload，避免 future demo pipeline restart 後 stale boot-time sender。 |
...
| `AccountManagerSlot` | rust/openclaw_engine/src/ipc_server/slots.rs:180 | Wave 2.2 LG-2 T3 FeeSource enum + IPC route `query_fee_source`（2026-05-11）：IPC server late-inject slot pattern（與 HStateCacheSlot / CostEdgeAdvisorDbSlot 對稱），main.rs L529 取 slot handle, L573 main_instruments 後注入；IPC handler 暴露 `FeeSource::{BybitApi, DemoConservativeDefault, ColdDefault}` 給 healthcheck [45] dual-source compare。|
...
- TODO.md v36 active dispatch queue + `active-plan.md` v1.9
