...
# 最后更新：2026-04-01（Wave 7 Demo 同步修復 + Spot/Inverse 品類規劃 · demo_only 模式）
...
> 截至 2026-04-01 Wave 7 Demo 同步修復完成：Paper Engine 內部平倉路徑（risk_auto_close / tp_sl）自動同步 Demo；stop_session() 雙遍歷自動清倉（Paper 持倉 + Demo API 殘留）；RiskManager max_single_position_pct 10%→15% 對齊 deployer；品類基礎設施審計完成（linear ✅ / spot 70% / inverse 40%）；Wave 7a Spot 品類啟用進行中；系統 demo_only 模式；live_execution_allowed 仍為 false。
