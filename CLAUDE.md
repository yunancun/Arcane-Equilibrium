...
**ML/Dream Edge Unblock implementation（2026-04-29 18:16 CEST · local complete）**：正 edge 仍是 **promotion gate**，不是 training gate。本輪已把 demo-first / live-governed 路線落地：V031 新增 `learning.mlde_edge_training_rows`（valid attribution + post-fee `net_bps_after_fee` reward）與 `learning.mlde_shadow_recommendations`（advisory-only，live/live_demo applied row 必須有 `decision_lease_id`）；LinUCB trainer 改讀 V031 view，scheduler 每輪以 `demo_live_demo` 合併訓練一次避免 shared `learning.linucb_state` 被 per-mode 覆蓋；新增 ML shadow advisor 產 `rank`/`veto`；新增 DreamEngine / OpportunityTracker read-only producers 並接入 CognitiveModulator；新增 healthcheck `[35] mlde_learning_data_contract` + `[36] mlde_shadow_recommendations`。Completion report：`docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--ml_dream_edge_unblock_completion.md`。Live 自動交易或 live 參數自動放權仍必須經 **GovernanceHub + Decision Lease + 既有 5 live gates** 批准；Rust active LinUCB arm-space 仍是 `v1_15`，rich `mlde_arm_id` 先用於 shadow/advisory，future active arm-space migration 另做。
