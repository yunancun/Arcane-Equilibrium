...
**G-SR-1 Signal Tightening Phase A S1-S4 + Phase B S5 ✅**（2026-04-13）— Phase A 信號源收緊完成 + Phase B Agent 基礎設施啟動。**Phase A**（S1-S4）：A0-a `grid_helpers.rs` 提取；A0-b `confluence.rs` 共享模組（PersistenceTracker + compute_score 4 分量 65 分制 + score_to_qty_pct 平滑插值）；A0-c 3 策略 TOML Params struct 加 confluence 字段 + factory 接線；A1 時間制持續性過濾器（MA/BBR=120s, BBB=60s）；A2 加權匯合評分；A3 Grid 趨勢冷卻（1x-6x）；S3 param_ranges 擴展 + validation；S4 +41 測試；A-E5 性能審查 PASS（`.to_string()` 為 pre-existing 模式，PersistenceTracker 建議 future 加 LRU cap）。**Phase B S5**：B0 `strategist_scheduler.rs`（tokio 5-min cycle + DB metrics R4-6/R5-3/R5-4 + 指數退避 R4-2 + validate_recommendation range/delta±30%/weight=65）；B1 `ai_service_client.rs`（100ms connect timeout + per-method TTL + newline JSON-RPC + fail-closed）；B1.5 AIServiceListener 啟動接線（R4-3 startup hook + shutdown + app.state 引用）；S5-E2 10/10 PASS。1083 lib + 33 e2e = 1116 tests pass。剩餘：S6 B2+B3+B4 Agent 真實接線 → S7 PM 驗收。計劃文件：`docs/references/2026-04-12--g_sr1_signal_tightening_plan_v2.5.md`。
...
> 截至 2026-04-13：tests engine lib **1083** + e2e **33** = **1116** Rust passed **0 fail** · Python **2852** passed · **G-SR-1 Phase A ✅ + Phase B S5 ✅**（B0 StrategistScheduler + B1 AiServiceClient + B1.5 AIServiceListener 啟動接線 + A-E5 性能審查 PASS + S5-E2 10/10 PASS）· **Phase 5 PAUSED**（策略 gross 負 edge）· **Live_Ready ✅** · **下一步**：S6 B2+B3+B4 Agent 真實接線 → S7 PM 驗收 · LG-1 21d paper 到期（05-01）。
