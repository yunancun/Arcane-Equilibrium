...
# 最后更新：2026-04-01（Wave 7b Inverse 品類完善 + 動態滑點 · 3201 tests）
...
下一步：Phase 3 L3 假設與實驗管線（或 Wave 7c option 品類，視優先級）
...
> 截至 2026-04-01 Wave 7b 完成：Inverse 品類全通（INV-1~5，32 個測試）；INV-1 幣本位 PnL 公式修正；INV-2 掃描器 category-aware 過濾；INV-3 qty 精度 category 參數；INV-4 inverse 風控 auto-inject（50x）；動態滑點分級（SLIPPAGE_TIERS）；3201 tests 通過；三個品類（linear/spot/inverse）均就緒；系統 demo_only 模式；live_execution_allowed 仍為 false。
