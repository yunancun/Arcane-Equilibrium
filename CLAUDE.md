...
| `_consumer` / `_consumer_lock` | lg5_review_consumer_scheduler.py | 內部懶加載 `start_consumer_scheduler()`（LG5-W3-FUP-1 IMPL-2 consumer scheduler，每 5min cycle，drain `learning.mlde_param_applications` pending live_promotion_candidate；解 `[42] live_candidate_eval_contract` runtime FAIL）。Sibling of EdgeEstimatorScheduler；獨立 leader sentinel。`Lg5ReviewConsumer` instance 持 daemon thread。測試用 `_reset_for_tests()` 釋放（含 graceful shutdown + join）。LG5-W3-FUP-1 ROUND 2 補登（2026-05-02） |
| `_LEADER_LOCK_FD` / `_LEADER_LOCK_PATH` | lg5_review_consumer_scheduler.py | 模組級全局；`_acquire_leader_lock()` 取得 flock fd 後寫入 `$OPENCLAW_DATA_DIR/lg5_review_consumer.leader.lock`，OS 進程退出自動釋放（含 SIGKILL）。uvicorn --workers 4 下與 edge_estimator_scheduler leader sentinel **獨立**（不同檔名），一方 crash 不影響另一方選舉。`OPENCLAW_SCHEDULER_LEADER=0` 強制非 leader；`OPENCLAW_LG5_CONSUMER_ENABLED=0` 完全關閉 consumer。測試用 `_reset_for_tests()` 釋放。LG5-W3-FUP-1 ROUND 2 補登（2026-05-02） |
