...
**Runtime（2026-05-01 21:32 CEST · ssh verify · G6-04 §三 drift 規則）**：Mac/Linux source includes `2674e14`; Rust engine runtime remains the `daab51c` scanner deploy (no rebuild/restart for the healthcheck-only fix). Engine PID **2364863** + API uvicorn PID **2047851** + engine_watchdog PID **3450754** + openclaw-gateway PID **3973441** alive。watchdog `engine_alive=true`，demo/live snapshots fresh，paper inactive by design。Manual passive healthcheck wrapper SUMMARY **WARN** exit 0：current notable WARNs are `[4]` / `[10]` / `[11]` / `[33]` / `[38]` / `[40]` / `[41]`; `[27]` recovered to PASS after an 18:00 transient FAIL.

**Active edge state**：Strategy Edge Repair + Strategy Edge Models + scanner market judgement / five-strategy context are deployed. Current observation gates are `[33] maker_fill_rate`, `[38] grid_trading_lifecycle_drift`, `[40] realized_edge_acceptance`, and `[41] scanner_market_gate_confirmation`; interpret rolling windows with post-deploy cutoffs because they still mix old samples. The main live-demo issue remains fee quality / realized edge; grid lifecycle drift is now near threshold but still rolling-WARN. Primary metric remains post-fee `net_bps_after_fee`; PnL and win-rate are secondary references.
...
**Next decisions**：Continue post-cutoff edge observation from scanner/five-strategy deploys. Latest manual wrapper after `2674e14`: `[33]` 7d maker_like 27.2% / fee_drop 22.0%; `[38]` lifetime_ratio 0.43 WARN (18:00 cron briefly PASSed at 0.53); `[40]` 24h rows=37 avg_net -20.81bps; `[41]` gates fired but labels insufficient; `[11]` is WARN not FAIL because the 2d rolling replay shrank as old exits aged out. G2-02 ma_crossover dual-track around 2026-05-03; P0-2 + G2-01 PostOnly acceptance around 2026-05-07/08; P0-3 edge decision around 2026-05-15.
