...
- **Track P v2 swap pending**：`TRACK-P-V2-SWAP-1` (P2, ~1d) Priority 6 v1 linear → v2 non-linear + ExitConfig ArcSwap 熱重載；前置無，排在 LinUCB 訓練 / Track L ML 層前。
- **tick_pipeline/mod.rs 2276 行超 §七 1200 硬上限**：`TICK-PIPELINE-MOD-SPLIT-1` (P3, ~2-3d) ON-TICK-SPLIT-1 衍生 backlog，不阻主路徑。
