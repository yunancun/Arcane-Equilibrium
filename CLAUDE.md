...
| 2026-04-21 | **DUAL-TRACK-EXIT-1 Phase 1b Track P v2 pure fn** ✅（`rust/openclaw_engine/src/exit_features.rs` +698 行：`physical_micro_profit_lock_v2` 非線性 giveback 4-Gate pure fn + `ExitConfig` 7 參數 `Default`/`validate`/serde + `non_linear_giveback_fn`（linear decay + floor bound）+ 31 單測覆蓋 Gate 1-4 / 非線性單調性 / 邊界 / serde round-trip / Gate 1 Hold 後 Gate 4 端到端；**QC 反轉 Gate 1 語意**：E1 第一輪對齊 v1 `edge < floor → Lock` 被主會話 QC 打回，改為 `edge <= floor → Hold` 對齊設計文檔 §三 L108-111 + operator 明確意圖「防止剛有大於 fee 的微利就套離場」，Lock 唯一合法路徑收斂至 Gate 4 trailing；v1 `risk_checks.rs:312-323` Priority 6 仍為 Lock 語意 **未動**，TODO `GATE1-REVERSAL-1` 追蹤下一波替換時一併修 + Gate 1 `<` → `<=` / Gate 4b `>` → `>=` 符號統一；engine lib 1791 → **1816** passed / 0 failed / 0 跨平台違規 / 0 無關檔案改動；Mac dev-only session 派 1 E1 sub-agent → QC 回修 → E2+E4 → PM report）· **memory** 新增 `project_dev_runtime_split.md`（Mac 接手三連 engine 檢查 noop） |
