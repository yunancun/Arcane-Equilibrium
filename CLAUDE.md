...
**Runtime（2026-05-01 22:36 CEST · ssh verify · G6-04 §三 drift 規則）**：Linux source is synced to `d8080f9`; code-bearing source includes `[22]` calibration `b283fda` and G8-05/LG-5 checkpoint `25d8e54`. Rust engine runtime remains the `daab51c` scanner deploy (no rebuild/restart). Engine PID **2364863** + API uvicorn PID **2047851** + engine_watchdog PID **3450754** + openclaw-gateway PID **3973441** alive。watchdog `engine_alive=true`，demo/live snapshots fresh，paper inactive by design。Manual passive healthcheck wrapper SUMMARY **WARN** exit 0；`[22]` now WARNs with `working_maker_orders_1h=2` and `risk_30m=4` instead of false-FAILING maker no-fill as writer wedge. `[16]` is transient WARN: last strategist cycle 11.3min ago within the 30-min backoff window.

**Active edge state**：Strategy Edge Repair + Strategy Edge Models + scanner market judgement / five-strategy context are deployed. Current observation gates are `[22] trading_pipeline_silent_gap`, `[33] maker_fill_rate`, `[38] grid_trading_lifecycle_drift`, `[40] realized_edge_acceptance`, and `[41] scanner_market_gate_confirmation`; interpret rolling windows with post-deploy cutoffs because they still mix old samples. Wave 4 pre-stage RFCs for LG-2 H0 blocking verification, MLDE-6 live promotion contract, and LG-3 provider pricing binding are ready at `5ce777b`. `ec8f0f4` adds STRK-FUP broader RFC, LG-4 RFC, dormant G7-04 CUSUM hook, and G4-03 canary Phase B. `25d8e54` adds the G8-05 AI Cost ROI Monitor static UI and LG-5 constrained autonomous live RFC. The main live-demo issue remains fee quality / realized edge; grid lifecycle drift is rolling-WARN. Primary metric remains post-fee `net_bps_after_fee`; PnL and win-rate are secondary references.
...
**Next decisions**：Continue post-cutoff edge observation from scanner/five-strategy deploys. Latest manual wrapper at 2026-05-01 22:36 CEST: `[22]` WARN maker no-fill context; `[16]` transient strategist-cycle WARN; `[33]` 7d maker_like 27.2% / fee_drop 22.0%; `[38]` lifetime_ratio 0.41 WARN (demo p50 7.9min vs live_demo 3.2min); `[40]` 24h rows=39 avg_net -17.97bps; `[41]` gates fired but labels insufficient; `[11]` is WARN not FAIL because the 2d rolling replay shrank as old exits aged out; `[27]` only FAILs when approved verdicts exist with zero persisted intents, while current rejected-only risk/cost-gate state remains WARN (`verdicts_30m=1`, `approved_verdicts_30m=0`). G2-02 ma_crossover dual-track around 2026-05-03; P0-2 + G2-01 PostOnly acceptance around 2026-05-07/08; P0-3 edge decision around 2026-05-15. Rank 9 HTTPS deploy still needs explicit runtime/deploy approval; next safe non-deploy candidates are P03-PREP-1, DOC-1, TEST-1, or PRE-LIVE-1.
