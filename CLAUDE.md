...
```
測試：3,348 Py + 823 Rust = 4,171 tests（4 pre-existing multi_interval_ws failures from linter）
...
★★★★ EXT-1 架構設計完成（2026-04-05）：
  五路聯合研究：否決「樂觀成交+回滾」，採用「交易所即真相」模式
  trading_mode="exchange" 時 Paper 僅為交易所確認後的本地記錄
  Demo=Live 統一路徑，切換只改一行 config（BybitEnvironment）
  設計已寫入 TODO EXT-1-01~10，下一 sprint 實現
...
**★★★ 當前焦點：Phase 3b 完成 → Phase 4 (7/03) → 讀 TODO.md 開始執行**
...
> 截至 2026-04-05：4124 tests（Py 3334 + Rs 790）· **RC-10 Python 引擎禁用 · Demo=執行引擎 · Paper=測試引擎** · IPC Command Channel（pause/resume/close_all/reset）· Shadow orders default-on · GUI 全面 Rust 遷移 · WS broken topics 修復 · **EXT-1「交易所即真相」架構已設計**（Demo=Live 統一路徑）· 下一步：EXT-1 實現 → 讀 TODO.md。
