...
## 三、真實狀態全景（2026-05-06 scanner opportunity healthcheck checkpoint `90db1e1c`）

### Runtime 部署
- **Mac/Linux/origin source checkpoint**: `90db1e1c`（新增 `[51]` scanner opportunity shadow healthcheck；2026-05-06 Mac → origin/main → Linux `trade-core` 三端同步）
- **Engine binary deployed**: `74b986a0`（2026-05-06 Linux `restart_all.sh --rebuild --keep-auth`；engine PID 2025331 / API parent PID 2025412）
- **Engine 健康**: watchdog `engine_alive=true`；demo/live snapshots fresh，paper inactive by design（final check 2026-05-06 13:39 UTC；demo/live snapshot age 14.1s）；scanner runtime snapshot `2026-05-06 15:32:35.543+02:00` 10/10 candidates 帶 `strategy_judgments[*].opportunity`，API `/strategy/scanner/opportunities` sample `has_opportunity=true`
- **Live boundary**: LiveDemo 跑（Live 管線走 demo endpoint），mainnet **0 流量** by design
- **健康檢查**: SUMMARY = FAIL（2026-05-06 17:43:30 UTC；pre-existing/live reality gaps：FAIL `[42]` live_candidate_eval_contract、`[42c]` 3d attribution drift、`[50]` replay_run_state_health；WARN `[40]` realized edge 仍負；新增 `[51]` scanner opportunity shadow coverage 100% 但 labels=7<10，WARN）
...
| `[51]` scanner opportunity shadow | 3h snapshot routes 340/340、scanner intents 4/4；24h labels=7，positive_avg=27.93bps、nonpositive_avg=-55.70bps、corr=0.22 | labels≥10 後評估 calibration | row proof 完整；樣本不足，仍是 shadow-only |
