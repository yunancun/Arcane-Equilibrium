...
**Runtime**：`Live_Ready` ⚠️（2026-04-16 audit 修正 · 2026-04-22 20:55 CEST `--rebuild` 部署後刷新 · **22:30 被動等待 audit 揭露 3 個 P0 runtime silent fail**）— LIVE-P0/P1/P2 代碼完整、單測 **1835 / 0 failed**（Linux release，TRACK-P-V2-SWAP-1 退役 8 個 v1 直測後的穩定基準；v2 等值覆蓋在 `exit_features/v2.rs` 25 單測），**仍 0 真實 live 流量**（歷史 43k 條 `engine_mode="live"` 實為 LiveDemo；當前 authorization.json 未簽，live pipeline 拒啟）。**TRACK-P-V2-SWAP-1 + TICK-PIPELINE-MOD-SPLIT-1 runtime live**：engine PID **158918** · binary mtime **2026-04-22 20:55**（baseline HEAD `9fcc7d4`）· Priority 6 每 tick 呼 `physical_micro_profit_lock_v2` + `ExitConfig`（非線性 giveback `max(1.0 − 0.15 × peak_atr_norm, 0.3)`）。**⚠️ 2026-04-22 22:30 實測**：`learning.exit_features` 7d 110 rows 揭露 (a) `giveback_atr_norm` unit scaling bug（DB avg 364.85 vs 正確 ~3.0 / `builder.rs:110` gb% ÷ atr-decimal 放大 100x）、(b) `est_net_bps` 99.1% NULL（T4 closure edge_estimates.get_cell 99% miss）、(c) `phys_lock_*` 7d 0 fire；疊加 risk_checks.rs:250-264 Track P T3 deprecation 後 COST EDGE gate 註解，形成 2026-04-19 晚 → 2026-04-21 晚 **2.5 天退場層空窗**（trailing 7 + dynamic 1 全 7 天總數）— 詳 TODO §P0-13/14/15 + worklog `docs/worklogs/2026-04-22--passive_wait_silent_fail_audit.md`。**真實 live 門控**（Rust 可驗證 4 項 / 全部 5 項，詳 §四）：(1) Python `live_reserved` global mode (2) Python Operator 角色 auth (3) `OPENCLAW_ALLOW_MAINNET=1` env（僅 Mainnet）(4) secret slot 有 api_key + api_secret (5) `authorization.json` HMAC 簽名+未過期+env_allowed 匹配。`execution_authority` 在 Rust 僅為 P0/P1 denylist 字串常量（`claude_teacher/applier.rs:226`），非真實授權邏輯。Live 縮倉監控 5min 輪詢（代碼已寫、e2e 測試綠，**從未真實觸發**）。
