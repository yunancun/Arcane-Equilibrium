...
**62-finding remediation 狀態（2026-04-29 CEST）**：Batch A-F 全 62 findings 已完成修復、簽核、tracking 更新並部署到 Linux。主修復 commit `bc3fa70`，文檔同步 `6539e4e`，restart ownership hotfix `5db4e29`；Linux `trade-core` 已 fast-forward 並完成 `restart_all.sh --rebuild --keep-auth` rebuild/redeploy（cargo PATH 需顯式帶入）。當前 runtime：engine PID **161957** (`openclaw-engine`)，API master PID **162029** + 4 workers；API `:8000` 無 address-in-use，direct unauth health endpoints 回 401（auth enforced），GUI-origin API requests 200 OK。**Post-deploy healthcheck 不是綠燈**：最新 `passive_wait_healthcheck.sh --quiet` 仍 FAIL `[12] bb_breakout_post_deadlock_fix` + `[22] trading_pipeline_silent_gap`，WARN `[27] intents_counter_freeze`；啟動暫態 `[16] strategist_cycle_fresh` 已清，`[31] edge_diag_2_strategy_diversity` 在最新重跑未再出現。Live pipeline 拒絕啟動是預期 gate：authorization schema v1 vs expected v2，需 Operator 經 `/api/v1/live/auth/renew` 或 renew-review 重新簽署。Batch F sign-off：`docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--batch_f_ml_agent_autonomy_signoff.md`；剩餘 release gaps：live PG registry integration、real ONNX artifact e2e、LinUCB live boot smoke、`[22]` silent-gap / fee-rate cold-boot cost_gate fail-closed RCA。
