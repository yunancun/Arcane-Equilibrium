...
> 截至 2026-04-19：tests engine lib **1567 passed / 0 failed**（pre-existing 5 `test_exit_feature_row_*` WIP 由 EXIT-FEATURES Phase 1b FUP `c7171b2` 全綠化）+ core **380** + e2e **35** + reconciler_e2e **19** + micro_profit_fix_integration **7** · pytest 全量 **2511 passed / 2 pre-existing DYNAMIC-RISK fail / 1 skipped** · **FILL-CONTEXT-LINKAGE-1 ✅**（commit `bd45e90`）：P1-7 C ML 訓練 0 標籤根因修復 — 訊號時刻 context_id 端到端傳遞（`OrderDispatchRequest`/`PendingOrder.context_id` 新欄位 + `apply_confirmed_fill(...,signal_context_id,...)` 新參數），閉合 `decision_features.context_id` (3.36M) ↔ `trading.fills.entry_context_id` (3514) 0 JOIN overlap；+2 regression tests；P1-7 C 從「結構性斷鏈」解除為「等部署 + 累積流量」 · **EXIT-FEATURES-TABLE-1 Phase 1b FUP ✅**（commit `c7171b2`）：Phase 1b producer wiring（`6ea643e`）漏 2 個 close paths（`process_external_fill` + `ipc_close_symbol` paper 分支），抽出 `try_emit_exit_feature_row` helper + 2 paths 接線 + 3 新 tests · **E5-P2 Refactor Wave 2 ✅**（5 P2 派發 → 2 delivered / 3 evidence-based CANCEL / 2 defer；commits **11dedbf** · **822f799**；E5-P2-4b follow-up：bb_breakout 1265 / grid_trading 1434 / strategies/mod.rs 1442 均超 §九 1200 硬上限，分檔拆解）· **E5-P1 Refactor Wave 1 ✅**（6 delivered + 2 cancel；commits ba8cd2c / 76cd793 / d6f7572 / b0dc6b6 / c220375 / 1b72f90；1498→1533） · **P1-16 HALT-SESSION CROSS-SYMBOL PRICE CORRUPTION ✅**（commit `fef688e`；mean -9.02 bps vs 修前 -2214 bps，245× cleaner）· **E5-P0 Refactor Wave ✅** · **LIVE-GATE-BINDING-1 ✅** · **MICRO-PROFIT-FIX-1 ✅** · **P1-8 DUST-EVICTION-GAP-1 E1/E4 ✅** · **P0-10 SCANNER-GATE ✅ 部署** · **P0-5 PHANTOM-2-FUP ✅ 部署** · **P0-9 STABILITY-1 ✅ RCA** · **LIVE-GUARD-1 ✅** · **Phase 5 PAUSED** · **Live_Ready ⚠️** · **下一步**：FILL-CONTEXT-LINKAGE-1 + EXIT-FEATURES Phase 1b FUP + E5-P1/P2 整合部署（`restart_all.sh --rebuild`）→ 累積流量後 P1-7 C 首跑 ONNX artifact · E5-P2-4b 策略檔拆解排期 · P0-2 LG-1 21d demo 觀察 → P0-3 edge 重評 · P0-6 intent write / Demo 死循環打破 · **P1-7 LEARNING-PIPELINE-DORMANT-1** · Phase 2B Strategist 等 G-1 R-02。
