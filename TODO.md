...
Version: v47
Date: 2026-05-18
Status: v47 dispatch-state sync — TODO refresh after `2026-05-18 multi-agent dispatch race incident` recovery; the recovery is now CLOSED via single-sequential E1+E2+E4+QA chain. Wave 1 of W-AUDIT-8a Phase B/C/D (B-REM-1 dispatch snapshot contract tests `49975eeb` + B-REM-5 SourceAvailability schema `5997dd43` + ADR-0023 `1b614daf` + C1-LIQ-WRITER LiquidationPulse provider `7ab6c22d` + healthcheck `[67]` `d8938a78`) is fully merged to main via `ef0dfc6e` / `5aeae75c` / `25413e96`, with E2 APPROVE / APPROVE-CONDITIONAL + E4 PASS + QA APPROVE WITH RESERVATIONS on file. EDGE-P2-3 Phase 1b runtime activator deploy chain remains CLOSED 2026-05-17 23:54 (engine PID 1143103, `runtime.use_maker_close=true`); AC-A 24h window verification still pending statistical significance. W-AUDIT-8b Round 2 RED_FINAL tombstoned via spec v0.4 (`ef7ea6c2`) + AMD v0.7 (`71f2283b`). Wave 2 (`C2-ORDERFLOW` 5pd HIGH + `C3-SPREAD` + `D-CONTRACT-LOCK` 2pd PA-only) deferred to Sprint N+4 per PA decomposition. Previous v46 production `allLiquidation*` writer revival (`0e8a8ae8` / `bedc40c3`) remains CLOSED.
