...
**版本**：v81（v80 並存 + 2026-05-29 Packet C/V114 engine rebuild deploy 收尾 — 修正 v80「engine binary 未 rebuild」stale）
...
| `P2-M11-SMOKE-ZOMBIE-DESIGN-FIX` | 2 | **NEW 2026-05-29 per rebuild [50] discovery** — M11 wrapper smoke 做 register + **run**；register 寫 `replay.experiments` 滿足 `[48]`，但 **run dispatch 真實 replay 子進程 hang/死 → `replay.run_state` zombie → 4h 後 `[50] replay_run_state_health` FAIL**。首例 zombie `6532fc38`（E1-M11 install smoke）已 operator cancel cleanup。**設計缺陷**：每日 04:00 cron fire 會累積 zombie。E2-M11 review APPROVE-WITH-CONDITIONS 沒抓（zombie 4h 後才現）。**修法候選**：(a) smoke register-only（heartbeat 已足 keep [48]，不 dispatch run）；(b) run 加 proper timeout + 完成/失敗時寫 completed_at/exit_code；(c) zombie auto-cleanup cron（running > Nh 自動 cancel）。**Owner**: PA design ruling → E1 wrapper fix。**Trigger**: 下次 04:00 cron fire 前（避免再積 zombie）；建議優先 (a) 最小。 |
