...
| `P1-C10-SYNTHETIC-SPOT-CLOSE-PNL-FALLBACK` | 1 | **NEW 2026-05-24 per 隔壁 audit + PA review verdict HYBRID-BUG** — C10 funding_harvest `on_close_confirmed`/`on_external_close` 用 `entry_price` fallback → synthetic spot PnL≡0；spec §4.1 line 765「runtime PnL vs Stage 0R replay drift > 5% → demote」**結構性永真 → C10 永遠 demote / Stage 0R rollback 機制失效**；trait sig 人為窄化（callsite `step_4_5_dispatch.rs:1727` + `step_6_risk_checks.rs:585` 有 close fill price 但沒傳給 strategy）。**修法 Option (a)**: Strategy trait `on_close_confirmed(symbol, close_price, close_ts_ms)` + `on_external_close(...)` 簽名升級；5 既有 strategy override migrate + funding_harvest 真實 PnL + 2 callsite 傳 close fill price + 4 new test case + spec §3.4.1 amend + §4.1 line 765 drift gate 澄清。Owner: PA spec amend + E1 IMPL；工時 +5-8 hr；阻 C10 Stage 1 Demo closure + E2/E4/QA/PM Phase 3e；ref PA review 2026-05-24 verdict + `docs/CCAgentWorkSpace/PM/workspace/reports/2026-05-24--sprint_1a_1b_completion_audit.md` line 80 |
| `P1-INTENTTYPE-DIRECTION-MISMATCH` | 1 | **NEW 2026-05-24 per 隔壁 audit + PA review verdict HYBRID-PLACEHOLDER-BUG** — schema 兩 layer (is_long bool + intent_type enum) 是 by-design (PA spec §2 backward-compat)；但 Sprint 1B IMPL **32 個 caller 全寫死 `IntentType::OpenLong` 字面值占位**；short-capable strategy (funding_arb:499-516 + funding_harvest:519-530 + bb_reversion:337 + bb_breakout:870 + ma_crossover:98 + grid_trading 等 8 file) emit `is_long=false / intent_type=OpenLong` 語義矛盾；router.rs:100 寫死 "TRADE_ENTRY" 未從 intent_type 取。**修法 Phase A**: `OrderIntent::new_trade(symbol, is_long, qty, strategy)` helper 內部自動依 is_long 設 intent_type + 8 個 strategy emit site 修 + `OrderIntent::validate()` debug_assert + test 對抗測試。Owner: PA spec §2 §2.2 amend + E1 IMPL + E2 review focus 3 點；工時 +3-5 hr；**不阻 Sprint 1B closure**（既有 hot path 用 is_long 還沒漂）；**阻 Sprint 2 Wave 1 router.rs:100 wire-up + Wave 2 IntentProcessor Earn dispatch**；ref PA review 2026-05-24 + `docs/CCAgentWorkSpace/PM/workspace/reports/2026-05-24--sprint_1a_1b_completion_audit.md` line 79 |
