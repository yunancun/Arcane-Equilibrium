...
| `P1-ENGINE-BINARY-SPRINT-1A-IMPL-DEPLOY` | – | ✅ **DEPLOY-VERIFIED 2026-05-25 00:27 UTC by E1 sub-agent `a0441a9f` inline return** — Linux HEAD `bbb21c56` (含 round 1+2 C10 PnL fix + IntentType direction fix + Earn Wave C IntentProcessor IMPL); cargo build PASS 34.80s; `restart_all.sh --rebuild --keep-auth` PASS; 新 engine PID **320381** 替換舊 4105805 (deleted binary); API PID 320486; 中斷時長 ~12s; binary mtime 2026-05-25 00:27 / /proc/320381/exe 非 deleted; source grep verify earn_router.rs (9 hit) + funding_harvest/mod.rs (3 hit) Sprint 1A/1B IMPL symbols live; Linux cargo test 4143/1/5 對齊 Mac (略差 2 feature flag 不影響核心); 1 FAIL 仍是 pre-existing layer_2_fence_archive_policy_diagnostic_only; /api/v1/healthz 200 OK; engine_alive=true (demo, age 12.4s post-restart). **3 carry-over (不阻 closure)**: (1) `/tmp/openclaw/watchdog_status.json` 不寫 (pre-existing 非本次 deploy 引起，建議補 watchdog status writer) (2) live engine alive=false (`authorization.json` 缺 — operator 手動 `/api/v1/live/auth/renew` 才能恢復 live trading；demo 不受影響) (3) 7d demo observation 啟動 2026-05-25 00:27 → 2026-06-01 (C10 AC #1/#4 真實 entry sample collection 截止) |
...
| `P1-SPRINT-1B-C10-CLOSURE-GAPS` | – | ✅ **CLOSED 2026-05-25** — full closure chain: E2 round 1+2 APPROVE / E1 round 1+2 IMPL +1028 LOC / E4 round 1+2 PASS 4135 / E2 C10 closure review (a8b8077b) CLOSURE-CONDITIONAL / QA Stage 0R replay (a184e767) APPROVED-CONDITIONAL / PA round 2 spec amend (af6b06c1) §3.4.1+§4.1+§6.6 +32 LOC / PM Phase 3e SOURCE-LAYER APPROVE / Linux deploy bbb21c56 DEPLOY-VERIFIED (new PID 320381 / binary mtime 2026-05-25 00:27 / 4143/1/5 cargo); **7d demo observation 啟動 → 2026-06-01 AC #1/#4 sample**. ref `P1-ENGINE-BINARY-SPRINT-1A-IMPL-DEPLOY` |
...
| `P1-EARN-WAVE-C-FIRST-STAKE-RUNTIME` | 2 | **PARTIAL-CLOSED / OPERATOR-BLOCKED 2026-05-25** — IntentProcessor Earn branch IMPL (E1 sub-agent `afbf6555`) ✅ DONE: earn_router.rs 703 LOC + 9-gate fail-closed E-0..E-9 + EarnLeaseGuard RAII + 5-gate inheritance + +10 new test 4145 PASS; Stage 0R Earn variant spec (PA sub-agent `aef185e2`) ✅ 656 LOC / 5 AC / 8 Open Q. Linux deploy `bbb21c56` 含 EarnStake binary symbol live ✅. **仍阻於 operator 親手動作**: (a) OP-1 Bybit web UI 重發 API key 加 `asset:earn` scope (5 min) (b) 實際 stake $100-200 Flexible-only first stake (動 demo balance；走 IntentProcessor Earn branch → bybit_earn_client → learning.earn_movement_log 真實 row). **carry-over (Wave D/E)**: governance_approval_id 占位 sentinel 0 (E1 IMPL `earn_router.rs:316-322` 文檔化) — Wave D/E 補 `learning.governance_audit_log` INSERT RETURNING id chain. **8 Open Q operator 拍板** (per `2026-05-25--stage_0r_earn_variant_design_spec.md` §8) — 不阻 deploy 但需 first stake 前處理. |
| `P2-WATCHDOG-STATUS-JSON-WRITER` | 3 | **NEW 2026-05-25 per Linux deploy 揭露** — `/tmp/openclaw/watchdog_status.json` 不存在；watchdog daemon (PID 2936560) 跑著但不寫 status file。pre-existing 非本次 deploy 引起。Owner: PA + E1；ETA: ~2-4 hr (補 status writer or 刪舊 healthcheck 期待)；不阻 closure；建議 Sprint 2 Wave 3 / pre-flight |
