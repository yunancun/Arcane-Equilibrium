...
**Runtime（2026-04-30 22:56 CEST · ssh verify）**：Mac/source HEAD `d847fb8` before this cleanup commit；Linux source HEAD `d847fb8` clean；current code-bearing runtime checkpoint remains `a9fce24` (no rebuild/restart in this session). Runtime PIDs from latest verified checkpoint: engine **1529433**, API **1591455**, watchdog alive, gateway **3973441**. Latest passive healthcheck (2026-04-30 22:28 CEST) SUMMARY WARN exit 0：WARN `[4]` phys_lock_runtime, `[11]` replay JSON age, `[33]` maker_fill_rate, `[38]` grid lifecycle drift, `[40]` realized edge acceptance；PASS `[13]` edge scheduler fresh, `[35]` MLDE data contract, `[36]` advisory boundary, `[37]` demo applier, `[39]` strategy cardinality.
...
- ⚠️ [11] WARN counterfactual：sample count 已滿（latest post-P013 n=864/200），仍因 replay JSON age 16.5h 等 fresh artifact + PASS streak
...
2. **🟡 [11] counterfactual clean window freshness** — sample count 已超標（post-P013 n=864/200），目前 WARN 原因是 replay JSON age 16.5h；連續 PASS 後 EDGE-P3 才解鎖
...
- [ ] **EDGE-P3 4 條件全滿** — sample count 已滿（[11] n=864/200），但 latest [11] 因 replay JSON age 16.5h 仍 WARN；等 fresh replay + 連續 PASS
...
| **EDGE-P3** clean window ≥200 | 資料被動 | 2026-04-22 → fresh replay + 連 3d PASS | 🟢 sample count 已滿：latest [11] post-P013 n_rows **864/200 (432%)**；🟡 latest verdict 仍 WARN，原因是 replay JSON age 16.5h | [11] counterfactual_clean_window_growth | 延後 Gate 1 fallback 到 fresh replay + PASS streak |
...
- **2026-04-30 🟡**: EDGE-P3 [11] sample count **864/200 (432%)**，但 latest verdict WARN（replay JSON age 16.5h）；Gate 1 fallback 部署仍等 fresh replay artifact + PASS streak
