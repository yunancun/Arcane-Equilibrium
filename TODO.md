...
- **PM Phase 3e sign-off ✅ DONE-VERDICT-PASS WITH 8 CARRY-OVER (2026-05-23)** — Sprint 1B late §4.1.1 + Sprint 5+ §4.2.1/§4.3.1 + Sprint 4+ §4.1 carry-over item 4 全鏈 closure；7 commit chain `011fd5f9 → 2b9e1c7d → e5fb4895 → 0d4a4aeb → 4d692cd6 → e377a94e → 6ceb5814`；3 closure (V100 M4 base + B-2 WS supervisor + B-3 Track E) + 1 governance NEW (PA-DRIFT-6 TimescaleDB hypertable composite PK FK governance lesson)；8 carry-over routing (V101/V102 defer / Sprint 5+ §4.2.2-4 cascade / Sprint 5+ §4.3.2-6 M3 follow-up / production 監測 4 / NEW PA-DRIFT-6 audit P2 / NEW sandbox V100 stub cleanup P2)；**Next: 8 carry-over operator 優先級拍板 OR Sprint 1B 剩 3 章節 dispatch (C10 READY / Earn NEEDS-OPERATOR)**
- **GUI Bybit-first Demo PnL ✅ DONE / DEPLOY-SYNCED (2026-05-23, operator 1A2A3A; code commit 47eddc37)** — Q1 response-field-only no 24h cron this sprint；Q2 no `/demo/wallet-truth` this sprint；Q3 backend keeps 4 `strategy_source` values, GUI folds labels. Implemented `/api/v1/strategy/demo/closed-pnl` Bybit-first read model + read-only PG fallback/reconcile, in-process cache/in-flight dedup, cursor pagination beyond 500, `demo/live_demo` filter, Demo Profit tab exchange-confirmed closedPnl rendering, stale/source UI scoped to Profit, `restart_all.sh` engine.sock readiness gate. E2 RETURN×2 fixed; E3 security/read-only APPROVE; A3 UX APPROVE. Mac + Linux focused tests PASS 50/50; Linux `restart_all.sh --keep-auth` engine.sock gate PASS; trade-core API startup 200 and unauthenticated closed-pnl GET correctly 401. Full connector pytest 4171 PASS / 21 known-existing fail / 12 skip (static `tab-live.html` extraction drift + pre-existing stop-cancel reason mismatch).
