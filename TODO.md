...
**版本**：v93（v92 + 2026-05-31 PM 收尾：TODO 歸檔精簡 + alpha-edge 工程安排整合 + governance 更正 + 三端同步）
...
**版本增量歷史（v75–v91）+ 已 DONE phase / 舊 runtime snapshot / 已關閉 workflow/queue 明細**：→ `docs/archive/2026-05-31--todo_v92_archive.md`
...
|---|---|---|---|---|
| **P0-EDGE-1** | 🔴 ACTIVE（**THE 核心 blocker**；SSH empirical 2026-05-26 22:00 UTC: 0/3 AC paths satisfied；2026-05-26 cohort reframed 5→4 textbook per AMD-2026-05-26-01）| QC + PA | (i) **4 textbook** ≥3 demo 7d avg_net>5bps + Wilson lower>0 + n≥30（funding_arb retired per AMD-2026-05-26-01）；**OR** (ii) ≥3 alpha-bearing 達同標；**OR** (iii) portfolio 7d gross 正向 | 唯一可預期 closure path = (ii) Sprint 2 Alpha Tournament dispatch；當前 (i) **4/4 textbook** `insufficient_total_samples`（funding_arb deprecated；ma_crossover / bb_breakout / bb_reversion / grid_trading runtime_bps −11~−42）；(iii) live_demo 7d net=-1.99 USDT。**⚠️ closure path (ii) 受挫 2026-05-30~31**：A2 LCS fade QC 審結 = **REVISE/HOLD 不推進 Stage1**（avg_net −2.45bps 負 / 證據 0/3 / alpha 假設執行可達性可疑）；A2 maker-fill feasibility Linux PG 2026-05-31 verdict=`reject`（1bp 60s PostOnly touch 53/108=49.07% <50%，2bp=31.48%，5bp=14.81%；0bp top-of-book optimistic=71.30% 但 queue priority not modeled）；A1 仍卡 basis_panel 前向累積（~D+14=2026-06-13）；**A3 BTC/ETH pairs 2026-05-31 Linux PG precheck verdict=`reject`**（corr 0.5297 / ADF-like t -0.8327 / half-life 4110.8 bars / avg_net -24.28bps）。**v92 成本牆發現**：6 週 4 候選 + 4 textbook 全死於同一道成本牆（edge 1-3bps ≪ 成本 11-27bps，結構性）；alpha 重定向 = oi_delta 降 ensemble feature + R-2 重定向 listing pump-dump fade（首選 ~50-65%）+ funding-extreme directional。**2 primary + A3 draft 暫無一可即推 Stage1** → P0-EDGE-1 closure ETA 較原估更遠，next executable = listing-fade 資料可行性診斷（dispatched）、A1 basis accumulation、M4 lease schema/IPC alignment、或另找新候選。M4 Stage1 可產 exploratory DRAFT，GovernanceHub provider seam 已 source done，但 production writeback 仍被 UUID lease column vs `lease:<id>` canonical ID mismatch + active IPC lease path 阻住，不能當 promotion evidence。 |
...
**NOW — 3 並行 session（PM 2 簽後可即派）**：

| Session | Track | Owner chain | Acceptance（exit，可證偽）| Blocked-on |
|---|---|---|---|---|
| **S1-W1-S1** ⭐ | T1 critical | PM→MIT+operator→PA | retention(operator 簽 365→1095d)+**survivorship-corrected symbol 清單（含 delisted；純 survivor=FAIL）** | 無（NOW）|
| **S2-W0-S1** | T2 Gate-A(kill-gate) | PM→QC+MIT→joint;BB | maker-fill feasibility 估計（**<30%=KILL Track2**，省 collector IMPL+5-6mo）| 無（NOW）|
| **S4-W0-S1** | T4 | PM→MIT+E1→MIT | 2024 bull regime funding/price 回填（與 S1-W1 同批 infra）| 無（NOW）|

**gated 後續**（詳 plan §2）：S1-W1-S2/S3（執行回填含 delisted + 品質 verify，blocked-on retention gate）→ **S1-W2-S1/S2**（TSMOM + X-sec leak-free 回測，2 並行；net>2×成本 / OOS≥0.5×IS / PSR>0.95 / DSR>0；**OOS<0.3×IS=kill**）→ **S1-W3-S1**（CP-2 go/no-go + Track3 載體裁定）｜S2-W1+（Gate-A PASS 後 collector IMPL → 累積 ~Q4 → Gate-B 回測）｜S3-W1-S1（ensemble，blocked-on Track1 GO；**ρ>0.5=kill**）｜S4-W1-S1（funding percentile directional 回測）
