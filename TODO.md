...
|---|---|---|---|---|
| **P0-EDGE-1** | 🔴 ACTIVE（**THE 核心 blocker**；SSH empirical 2026-05-26 22:00 UTC: 0/3 AC paths satisfied；2026-05-26 cohort reframed 5→4 textbook per AMD-2026-05-26-01）| QC + PA | (i) **4 textbook** ≥3 demo 7d avg_net>5bps + Wilson lower>0 + n≥30（funding_arb retired per AMD-2026-05-26-01）；**OR** (ii) ≥3 alpha-bearing 達同標；**OR** (iii) portfolio 7d gross 正向 | 唯一可預期 closure path = (ii) Alpha-Edge 4-Track。當前 (i) **4/4 textbook** `insufficient_total_samples`（funding_arb deprecated；ma_crossover / bb_breakout / bb_reversion / grid_trading runtime_bps −11~−42）；(iii) live_demo 7d net=-1.99 USDT。**⚠️ closure path (ii) 受挫 2026-05-30~31**：A2 LCS fade QC 審結 = **REVISE/HOLD 不推進 Stage1**；A1 仍卡 basis_panel 前向累積（~D+14=2026-06-13）；**A3 BTC/ETH pairs 2026-05-31 Linux PG precheck verdict=`reject`**。**v92 成本牆發現**：6 週 4 候選 + 4 textbook 全死於同一道成本牆（edge 1-3bps ≪ 成本 11-27bps，結構性）；alpha 重定向 = oi_delta 降 ensemble feature + listing pump-dump fade + funding-extreme directional。**2026-05-31 NOW 三並行結果**：S1-W1-S1 MIT advisory **PASS**，但 S1-W1-S2 仍卡 operator 簽 `market.klines` 365→1095d、18mo、core25/full survivorship universe（797 symbols / 225 delisted）；S2-W0-S1 Gate-A **PROCEED**（maker-fill 59/67=88.1%，Wilson 78.2-93.8%，但不是 alpha proof，Gate-B 前需更長 BB PreLaunch transition probe）；S4-W0-S1 **BLOCKED_ON_RETENTION + SCRIPT**（2024-11 local PG funding/kline 0 row；Bybit API 可取；需 `market.funding_rates` 180d retention/storage 決策 + E1 idempotent DB writer）。Next executable = operator alpha-history retention/storage decision → E1 public Bybit history backfill script → MIT verify；M4 exploratory DRAFT 仍不能當 promotion evidence。 |
...
|---|---|---|---|---|
| **S1-W1-S1** ⭐ | T1 critical | PM→MIT+operator→PA | ✅ **MIT advisory PASS**：`market.klines` current retention=365d；recommend 1095d + 18mo + core25 primary / full 797-symbol survivorship backfill universe（225 delisted/Closed overlap；純 survivor-only 排除）。Report: `docs/CCAgentWorkSpace/MIT/workspace/reports/2026-05-31--s1_w1_s1_retention_symbol_universe.md`；artifact: `...--survivorship_universe_18mo_usdt_perp.csv` | **operator must sign retention/window/breadth**；S1-W1-S2 still locked |
...
| **S4-W0-S1** | T4 | PM→MIT+E1→MIT | ⛔ **BLOCKED_ON_RETENTION + SCRIPT**：local PG 2024-11 funding/kline rows=0；Bybit public API can return 2024-11 `1000PEPEUSDT` funding + daily/4h klines；existing `market.funding_rates` retention=180d and `market.klines`=365d would reap history；no production DB-writer backfill script. Report: `docs/CCAgentWorkSpace/MIT/workspace/reports/2026-05-31--s4_w0_s1_bull_regime_backfill_preflight.md` | operator storage decision must include `market.funding_rates` or separate funding-history table + E1 script |

**gated 後續**（詳 plan §2）：operator alpha-history storage decision → E1 public Bybit historical backfill writer（klines 1d/4h + funding history, idempotent + coverage report）→ S1-W1-S2/S3（執行回填含 delisted + 品質 verify，blocked-on retention gate）→ **S1-W2-S1/S2**（TSMOM + X-sec leak-free 回測，2 並行；net>2×成本 / OOS≥0.5×IS / PSR>0.95 / DSR>0；**OOS<0.3×IS=kill**）→ **S1-W3-S1**（CP-2 go/no-go + Track3 載體裁定）｜S2-W1+（Gate-A PASS；先做 BB 24h PreLaunch transition probe，再 collector IMPL → 累積 ~Q4 → Gate-B 回測）｜S3-W1-S1（ensemble，blocked-on Track1 GO；**ρ>0.5=kill**）｜S4-W1-S1（funding percentile directional 回測；blocked until funding+price history persists）
...
**紀律（plan §0，dispatch 必帶）**：① backfill 必含 delisted（倖存者偏差是 T1 唯一致命污染；SoT=`symbol_universe_snapshots`）② Gate-A maker-fill kill-gate 先於 collector IMPL ③ leak-free shift(1) 每回測 day-1 ④ ≤7 並行（NOW=3 / max=4）⑤ retention 365→1095d 是 operator gate，硬卡 backfill 執行前
...
|---|---|---|---|---|
| **NOW** | **OP-ALPHA-HISTORY**：簽 alpha history storage decision — `market.klines` retention 365→1095d（MIT recommend）+ `market.funding_rates` 180d retention/storage 決策 + window=18mo + primary breadth=core25 / full survivorship backfill universe | 15-30 min | S1/S4 reports done 2026-05-31 | 阻 S1-W1-S2 / S4-W0 DB backfill、Track1/Track4 回測 |
