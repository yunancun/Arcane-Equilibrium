...
**版本** v133 ｜ **日期** 2026-06-12 ｜ **來源實作 HEAD** main 本檔所在 HEAD（本輪 AEG-S3 execution observations producer code checkpoint `9eaad929`；runtime 詳見 §0/§6；版本敘事歸 `docs/CLAUDE_CHANGELOG.md`）。
...
| 區域 | 當前狀態 | 下一步 |
|---|---|---|
| 來源同步 | **三端同步** 本檔所在 main HEAD；本輪 AEG-S3 execution observations producer code checkpoint `9eaad929` 已 Mac→GitHub→Linux `trade-core`，Linux artifact-only smoke 已跑；Gate-B watch cron 仍在 Linux `12,42 * * * *`。 | 維持三端同步；**Mac→GitHub 直連已恢復**（2026-06-10 fetch+push 親驗，bundle 流程退役）；Mac srv checkout 駐 main，本輪未 deploy/rebuild/restart。 |
...
| `AEG-S3-CANDIDATE-DIRECT-ROWS` | 1 | 🟡 execution observations producer 已接上 / 候選仍 non-promotable / Gate-B 改事件觸發 | 2026-06-11/12 已完成 artifact-only AEG-S3 direct rows infra、V125/V127 true panel export、funding_revive + oi_delta evidence/PBO/direct rows/metrics、event breadth adapter、empirical execution adapter、Gate-B 專用 watcher cron、sidecar matrix wiring。`66a9e511` 讓 formal matrix input 可用 `--breadth-run-dir/--execution-realism-json` 引用既有 sidecar artifact；`9eaad929` 新增 `research/aeg_s3_execution_observations/`，可把 `listing_fade` candidate evidence + Gate-B run 轉成 matched `execution_observations.jsonl`，供 `aeg_s3_event_execution_realism` 使用；Gate-B producer 僅支援 listing_fade，funding_revive/oi_delta 不冒充。Mac/Linux 本輪 execution-observations focused regression **31 passed** + compileall OK + static forbidden-route search 無命中。Linux true smoke：old Gate-B run `listing_24h_20260602_1847` -> listing_fade evidence sample_count=2 -> observations count=2 -> execution realism FAIL；10 USDT 失敗腿=`sample_count_below_30` + `participation_rate_p95_above_0_05`，1 USDT 失敗腿只剩 `sample_count_below_30`。結論：producer/aggregator 已接通，但舊 run 不足 promotion，仍需 fresh Gate-B 取得 `>=30` matched samples。funding_revive 仍卡 `DSR=0.0`、PBO `0.54583333`、execution 未有 `>=30` matched empirical observations；oi_delta event breadth 仍按 basket 不支援 fail-closed，PBO `0.9125`。Gate-B watch latest `WATCH_ONLY`、23 candidates、0 alertable、0 start/schedule、唯一 live watch=`BPUSDT` old ContinuousTrading。PM reports：`2026-06-12--aeg_s3_execution_observations_producer.md`、`2026-06-12--aeg_s3_sidecar_matrix_wiring.md` 及前序 AEG-S3 reports。**下一步**：主線 Gate-B 等 `[GATE-B-WATCH]` fresh Pre-Market/PreLaunch/conversion alert 或 latest artifact 變 `ACTIONABLE_*` 後啟動 isolated 24h probe；並行在新 run 上產生 `>=30` matched execution observations，跑 `aeg_s3_event_execution_realism`，再以 `--execution-realism-json` 重跑 formal matrix。未經 E2/MIT/QC 獨立審前不得當 promotion proof。 |
