...
| 11 | `W-AUDIT-4` ML 基座 + dead schema (mount W-F-1) | E1×6 並行 + MIT + E2 + E4 | ACTIVE 2026-05-09 (~30h, 3 sessions, after W-AUDIT-1) | V075 corrected retention/compression source/test added for 5 real hypertables + dry-run plain-table prune function; V076 Guard A retrofit and V077 F-29 archive CHECK source/test added. None DB-applied. Remaining: V068-V074 migration/source set, F-08 ML cron, F-09 FUP-2 deploy. Source work may proceed; DB apply/deploy remains separate authorization. |
...
| `P1-AUDIT-ML-4` | 3 | W-AUDIT-4 ML 基座 + dead schema (mounts W-F-1) | ACTIVE: Linux read-only audit showed the planned "9 table retention" target is actually 5 hypertables + 2 plain tables + 2 views. V075 source/test therefore installs Timescale policy only for `trading.risk_verdicts`/`position_snapshots`/`signals`/`order_state_changes`/`intents`, adds a dry-run-default prune function for `learning.decision_features` and non-live `trading.decision_outcomes`, and explicitly treats `learning.scorer_training_features` + `learning.mlde_edge_training_rows` as views. V076 Guard A retrofit and V077 F-29 bounded archive CHECK also source/test added. None DB-applied. Remaining: V068-V071 drop/archive dead schema + V072 feature_baselines writer contract resolution + V073 edge_estimate_snapshots cycle writer + V074 decision_outcomes live backfill + F-08 5 ML cron + F-09 FUP-2 deploy. Heavy parallel; 3 sessions; depends on W-AUDIT-1. |
