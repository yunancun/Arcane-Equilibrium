...
**版本** v141 ｜ **日期** 2026-06-12 ｜ **來源實作 HEAD** main 本檔所在 HEAD（本輪 incident-policy dispatch trigger BB/E2 review checkpoint；runtime 詳見 §0/§6；版本敘事歸 `docs/CLAUDE_CHANGELOG.md`）。
...
| 區域 | 當前狀態 | 下一步 |
|---|---|---|
| 來源同步 | **三端同步** 本檔所在 main HEAD；本輪 P2 incident-policy dispatch trigger BB/E2 review 已核對：CORE ledger + auth invalid producer + Bybit fail-closed producer + C4 incident-policy E2E 已在 source，BB/E2 對 CORE+auth+Bybit path 0 blocker/high/medium；focused Rust tests Mac+Linux 於 source checkpoint 綠；P5-SM `[81]/[82]` selector fix checkpoint `bf673cdc` 已 Mac→GitHub→Linux `trade-core`，Gate-B watch cron 仍在 Linux `12,42 * * * *`。 | 維持三端同步；**Mac→GitHub 直連已恢復**（2026-06-10 fetch+push 親驗，bundle 流程退役）；Mac srv checkout 駐 main，本輪未 deploy/rebuild/restart。 |
...
| `P2-INCIDENT-POLICY-DISPATCH-TRIGGER` | 2 | 🟡 **CORE+auth+Bybit source-live / BB+E2 reviewed / producer coverage partial** | 2026-06-12 PM checkpoint：source 已非「待實作」。`notification_failsafe/incident_policy.rs` 已有 class-level ledger、sustained/throttle/7d cooling、push-secret gate、single armed owner、自癒 `AllSuccess` guard；`live_auth_watcher.rs` 已接 `auth_invalid` / resolved；`bybit_rest_client.rs` GET/POST retCode counter 已接 `bybit_fail_closed` / resolved；C4 E2E 覆蓋 incident_policy AllFail -> watcher timer -> in-band `NotificationFailsafeEscalate` -> Demo SM-04 Defensive。Mac+Linux focused Rust：incident_policy `15 passed`、C4 wire `4 passed`、ret_code_counter `6 passed`。2026-06-12 BB/E2 review：BB `APPROVE-WITH-CONDITIONS`、E2 `PASS-WITH-CONDITIONS`，0 blocker/high/medium；結論=CORE+auth+Bybit path 可繼續，但 `bybit_fail_closed` 僅代表 business-retCode fail-closed，不等於完整 exchange outage coverage；C4 owner handler 仍是唯一 set_trading_stop side-effect path。邊界：尚未接 `sm_halt_stuck`、`position_drift`、external `engine_dead` watchdog notify-only；未跑 E4/QA/full-chain；未 deploy/rebuild/restart。下一步：先做 remaining producer coverage，建議從 `sm_halt_stuck` arm-class 開始，再補 `position_drift`/`engine_dead` notify-only。報告 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-06-12--incident_policy_dispatch_trigger_checkpoint.md`、`docs/CCAgentWorkSpace/PM/workspace/reports/2026-06-12--incident_policy_dispatch_trigger_bb_e2_review.md`。 |
