...
**最新狀態快照**（2026-04-29 18:16 CEST · ML/Dream edge unblock implemented）：62-finding remediation Batch A-F 全部完成、push、Linux rebuild/redeploy；Items 1-6 follow-through commit `53bff07` 已推送、Linux fast-forward、release rebuild + restart，engine PID **620724** + API PID **620851**。fee freshness 改為 grouped demo/live binding task，啟動日誌已確認 `engine=demo env=Demo` + `engine=live env=LiveDemo`；post-restart risk verdicts 不再有 fee-stale self-lock，[27] PASS。`[34] intent_signal_attribution` 仍因 30min rolling window 內 pre-restart 空 signal_id 舊 rows 報 FAIL；post-restart intents 已確認 `empty_signal_id=0`，等窗口自然清。report 見 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--items-1-6-completion.md`。PM 方案 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--ml_dream_edge_unblock_plan.md` 決定：**正 edge 是 promotion gate，不是 training gate**；本輪已落地 MLDE Learning Data Contract、LinUCB post-fee reward 接線、ML shadow advisor、DreamEngine/OpportunityTracker read-only producers、scheduler 接線與 healthcheck `[35]`/`[36]`；completion report 見 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--ml_dream_edge_unblock_completion.md`。Live 自動交易或 live 參數自動放權仍必須經 GovernanceHub / Decision Lease + 既有 5 live gates 批准。
...
**測試基準（2026-04-29 ML/Dream Edge Unblock local 後）**：Mac Rust lib **2361/0** · scanner subset **61/0** · DB writer **3/0** · tick attribution helper **16/0** · maker_price **10/0** · `cargo check --bins` PASS · `cargo check openclaw_core` PASS · Python maker/attribution pytest **9/0** · MLDE targeted pytest **63/0** · `python3 -m compileall` targeted PASS · `git diff --check` PASS · healthcheck local registry now includes `[35]`/`[36]`（numbered [1]-[36] skip [17] + [Xa]/[Xb]；無 [0]）· DB migrations expected after V031
**21d demo 時鐘**：起算 2026-04-16 22:16 → 解鎖 2026-05-07
**Wave 3 healthcheck**：cron 6h 目前 runtime 跑 35 checks（numbered [1]-[34] skip [17] + [Xa]/[Xb]；含 F7 [22]-[29]、cost/execution [30]-[34]）；本地 main 下一版新增 MLDE `[35]`/`[36]`，需隨 V031 migration rebuild/deploy 後 runtime 生效。
...
1. **ML-DREAM-EDGE-UNBLOCK-2026-04-29（local complete · demo-first / live-governed）** — completion report 已寫入 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--ml_dream_edge_unblock_completion.md`。已落地：A) V031 `learning.mlde_edge_training_rows` + `learning.mlde_shadow_recommendations`；B) LinUCB trainer 改讀 valid attribution + post-fee reward，scheduler 每輪以 `demo_live_demo` 合併訓練一次，避免 shared `learning.linucb_state` 被 per-mode 覆蓋；C) ML shadow advisor 產 `rank`/`veto` advisory；D) DreamEngine / OpportunityTracker read-only producers 接入 CognitiveModulator；E) EdgeEstimatorScheduler 每小時 fail-open 執行 MLDE 任務；F) healthcheck `[35]` learning data contract + `[36]` shadow recommendation / live lease boundary。仍不可做：live/live_demo 自動交易或 live 參數自動放權，必須另走 GovernanceHub + Decision Lease + 5 live gates。後續：runtime rebuild/deploy 後觀察 `[35]`/`[36]`、advisory rows、LinUCB pulls；rich `mlde_arm_id` 已供 shadow 分析，Rust LinUCB active arm-space 仍是 `v1_15`，若要 production active arm 切到 richer shape 需單獨 arm-space migration。
