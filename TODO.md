...
| Runtime | Linux `trade-core` **engine 已 rebuild+restart 到 `b8c258b4`**（6/3 00:40，PID 2269678，release build 39.92s，本 session 親驗 watchdog engine/demo/live alive、ticks 流動）。**P5 step-i Rust IPC 已編入 running engine 但仍 flag-OFF/dormant**（soak 前置 `--rebuild` 順帶完成，**只剩 flag-on 即可起 24-48h soak**）。funding/OI engine 改動已上線（行為等價）+ **`--apply` DONE**（run `18b3c2f8`：V125 history 已回填 funding 46539 + OI 348153 rows，accepted）。 | P5 soak 只需 operator flag-on 即起；funding/OI `--apply` 已完成（V125 history 已填，run-versioned 查詢須固定 run_id）；QA post-deploy 檢查 A-1/A-2/B/A-4；paper lane restart 後翻 alive 待復查（預設 OFF，疑為 fresh-snapshot 非 pipeline 啟動）。 |
...
| `P1-BB-REVERSION-REGIME-OBSERVABILITY` | 2 | QA 建議（B 無正面證據面） / 待 PA scope | B fix 無 observability 證 fire-in-mean_reverting：`market.regime_snapshots` 全表 0 rows（producer 未跑）+ Hurst regime 未逐 intent 持久化。建議 fire 時把 Hurst regime label 持久化進 `trading.intents.details`，供未來 QA 正面驗證 + 助 `P3-BB-STRATEGIES-30D` 樣本判定。 |
