...
**版本** v148 ｜ **日期** 2026-06-12 ｜ **來源實作 HEAD** main 本檔所在 HEAD（本輪 AEG-S3 Gate-B preflight command guard + P5-SM `[82]` countdown refresh；runtime 詳見 §0/§6；版本敘事歸 `docs/CLAUDE_CHANGELOG.md`）。
...
| 區域 | 當前狀態 | 下一步 |
|---|---|---|
| 來源同步 | **三端同步** 本檔所在 main HEAD；本輪 P2 incident-policy dispatch trigger source chain 已完成並關閉；AEG-S3 Gate-B preflight v0.3 command guard checkpoint `289fcbe8` 已 Mac→GitHub→Linux `trade-core`，`WATCH_ONLY` + sample<30 時 `recommended_command_operator_recommended=false` / `HOLD_WAIT_FOR_ACTIONABLE_WATCH`；P5-SM `[81]/[82]` selector fix checkpoint `bf673cdc` 已同步，Gate-B watch cron 仍在 Linux `12,42 * * * *`。 | 維持三端同步；**Mac→GitHub 直連已恢復**（2026-06-10 fetch+push 親驗，bundle 流程退役）；Mac srv checkout 駐 main，本輪未 CI、未 deploy/rebuild/restart；runtime activation 仍 operator/deploy-gated。 |
...
| ID | P | 狀態 | 下一步 |
|---|---:|---|---|
| `P5-SM-OPTION2-CONVERGENCE` | 1 | 🟢 **48h soak RUNNING**(2026-06-11 04:00 deploy:V137 applied=137+兩 flag 已寫 env 檔+canary 已拍/heartbeat 已流;**錨點 03:59:37+02,gate 到期 ~06-13 03:59:37+02**;`[82]` 正確報 accumulating；`bf673cdc` 已修好 `--check 81 --check 82` narrow selector) | **基建鏈完成 2026-06-11**:E1×4 接力(`58ad4dba`→`0ce0874c`)→E2 RETURN(2H 假綠:crash-loop 稀釋+canary 死亡不可見,皆 probe 實證)→E1-fix(rollover 去重+30min heartbeat 四子軸)→re-E2 ACCEPT(Probe A/D 重放翻 FAIL)→E4 PASS `d7a9eacf`(兩端全套 0 新 fail;**1s 過殺壓測:真 engine IPC 240/240 ok、H0 p50/p99/p999 三相 byte-identical、RTT p99 1.1ms**)→CC **APPROVE A**(0 BLOCKER;LOW-1 flag 忘關無絆線=PM 接受〔step-(iv) 具名+SOP 三處〕;LOW-2 部署註記見⑤;INFO×3 記錄)。**soak 啟動 SOP(operator)**:①deploy(V137 經 AUTO_MIGRATE=engine;**可與 OPS-2 rebuild 合併一次 restart**)②兩 flag `OPENCLAW_LEASE_PYTHON_IPC_ENABLED=1`+`OPENCLAW_SM_IPC_CANARY_ENABLED=1` 寫 `$SECRETS_ROOT/environment_files/basic_system_services.env`→restart ③soak 期每日 6h cron log 或手跑 `--check 81 --check 82`（Linux 真 DB smoke 2026-06-12T21:00Z：`[81] PASS`, `[82] 43.0h<48h`, probes=1290；23:00+02 時約剩 5h，不可提前收）④gate=S1-S5(48h/≥500 probe/≥99%/無 15min 連段/S5 smoke N≥10)⑤結束移除兩 flag(**移 flag 後 72h 內 [82] FAIL 屬預期**=fail-closed 噪音,CC LOW-2)。owed-Linux 隨 deploy:canary 真 round-trip 一拍+`[82]` 真值+smoke 首跑。→ step-ii → 🔴step-iii CUTOVER(operator sign-off+CC/E2/BB/E4)。cadence env `OPENCLAW_SM_CANARY_INTERVAL_SECS` 默認 120。 |
...
| **S2 Gate-B 24h 真捕捉 run** | 探針已部署（R-0 zero-leak）；專用 `gate_b_watch` 已部署並安裝 Linux cron `12,42 * * * *`。2026-06-12 20:42 UTC latest artifact `/tmp/openclaw/gate_b_watch/gate_b_watch_latest.json` = `WATCH_ONLY`，23 candidates、0 alertable、0 start/schedule、唯一 live watch=`BPUSDT` old `ContinuousTrading`。`2b880f5d` 後 preflight 可直接讀此 artifact；`289fcbe8` 後 preflight v0.3 會對 full-chain command 給 operator guard。Linux smoke `aeg_s3_gate_b_preflight_command_guard_20260612T2105Z` 回 `gate_watch.operator_action=WAIT_FOR_ACTIONABLE_WATCH`、sample_count=2、pbo_status=`produced_candidate_grid`、readiness=`READY_BUT_SAMPLE_BELOW_GATE`、`recommended_command_operator_recommended=false`、status=`HOLD_WAIT_FOR_ACTIONABLE_WATCH`。報告 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-06-12--aeg_s3_gate_b_preflight_command_guard.md` / `2026-06-12--aeg_s3_gate_b_watch_preflight_bridge.md` / `2026-06-12--gate_b_autonomous_watch_impl.md`。 | 等 `[GATE-B-WATCH]` fresh Pre-Market/PreLaunch/standard-conversion alert 或 latest artifact 變 `ACTIONABLE_START_NOW` / `ACTIONABLE_SCHEDULE`；先跑 `aeg_s3_gate_b_preflight.harness --gate-watch-latest-json /tmp/openclaw/gate_b_watch/gate_b_watch_latest.json` 確認 `operator_action`，再按 `probe_command_hints` 手動跑 24h isolated capture 作 listing fade 主路第一證據；無 transition = `INCONCLUSIVE_NO_TRANSITION`，不可連 production WS/scanner/strategy/DB/order/auth；full-chain command 需 `recommended_command_operator_recommended=true` 才算 operator-recommended。 |
