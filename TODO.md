...
- [ ] **DECISION-OUTCOMES-ENGINE-MODE-TAG-BUG-1**（P1，2026-04-21 Linux 驗證新發現）：`trading.decision_outcomes` 264,800 rows engine_mode = 'paper' 佔 100%，但 context_id 前綴展示 demo_* / live_demo_* 混雜 + PAPER-DISABLE-1 (2026-04-16) 後 paper 預設關閉 → **engine_mode 欄位寫入邏輯故障**。影響所有 outcome-based per-engine 歸因（EDGE-P3-1 `engine_mode IN ('live','live_demo')` filter 永遠 0 rows / Phase 5 JS per-engine 切片 / LinUCB reward per-engine bucket）。**修復**：audit `rust/openclaw_engine/src/database/outcome_backfiller.rs:94-107` INSERT SQL，確認 engine_mode 欄位來源；fix 為讀 `context_id` 前綴判斷（`demo_*`/`live_demo_*`/`paper_*`）或從 `decision_context_snapshots.engine_mode` JOIN；考慮是否 backfill 歷史 rows（或標 `engine_mode=NULL` 表示 unknown）。
- [ ] **OUTCOME-BACKFILL-JOIN-NULL-1**（P1，2026-04-21 Linux 驗證新發現）：`trading.decision_outcomes` 264,800 rows 的 `outcome_1m / outcome_5m / outcome_horizon_minutes` **100% NULL**（不是稀疏，是全空）；抽樣符號在 `market.klines` 有資料 → 排除 klines 稀疏假設。**真根因**：`outcome_backfiller.rs:94-107` LATERAL 子查詢 JOIN / horizon-window 邏輯斷鏈。**修復**：先寫 diagnostic query 取一個 context_id 人工推演 outcome 該有值，比對實際 SQL 輸出；audit LATERAL 子查詢 ON clause + 窗口邊界計算；修好後 backfill 歷史 rows（或接受只補 forward-going）。LinUCB 訓練品質恢復的前置。
- [ ] **GATE1-REVERSAL-OBSERVABILITY-1**（P2，2026-04-21 Linux 驗證新發現）：`GATE1-REVERSAL-1 hotfix A` 原觀察計畫「2-3d 對比 demo `phys_lock_gate1_low_edge` 新 vs 舊 fills」**失效** — `trading.fills` 全時段 0 筆 `phys_lock_*` 前綴（hotfix 前也是 0，非 reversal 效果）。可能原因：(X) Priority 6 PHYS-LOCK 在 demo 從未實際觸發（ExitFeatures::None 導致 noop 或 edge/profit gates 過嚴）(Y) close reason wrapping 路徑不寫 `phys_lock_*` 到 `fills.reason`/`strategy_name`。**修復方向**：engine 內部 Prometheus counter（`phys_lock_gate1_hold_total` / `phys_lock_gate4_lock_total`）或 audit wrapping 路徑補 fills tag；hotfix A 行為驗證改走 engine metric 而非 fills 觀察。
