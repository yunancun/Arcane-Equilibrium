...
| `P1-DATA-1` | 3 | WARN cluster: `[45]`; `[14]` contextualized, `[37]` recovered-history semantics source-fixed, `[40]` combined-cell gap source-fixed | Exit-feature accumulation still warns when samples stall, but now distinguishes risk/cost gate suppression from writer-health evidence. Realized edge now catches negative demo/live_demo combined cells; pricing freshness remains a quality signal. |
