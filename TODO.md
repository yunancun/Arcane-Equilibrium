...
## ⏩ Wave 1（W17/18 · 4/24→5/08）— 基礎設施解凍【✅ 主體 + G1-03 全完成，剩 G1-04 背景 + [12] FAIL 結構性】

**狀態（2026-04-24 21:55 CEST 1-4 任務全做後）**：實際 **8/10 核心完成**（G1-01/02/03/05/06 + G6-01/02/04；剩 G1-04 P1 背景 + G6-03 部分）：
- ✅ **G1-01/02/03/05/06 全完成**：G1-03 本 session 4 subagent + 主 session 接手收尾，**所有 7 Rust 違規檔破 <1200 硬上限**（detail 在 G1-03 列）。
- ✅ **G6-01/02/04 完成**：healthcheck + cron 6h 全線。
- ✅ **G6-03 V024 DB apply**：2026-04-24 21:35 CEST `psql -f V024` 跑完，DO block guards PASS（無 RAISE），V019/V020 legacy table + indexes shape 正確；**但 binary 20:21 不含 V024 code**（binary 需重建後 auto_migrate 或純 DB apply 完成）。
- 🟠 **G1-04 fee drag / R:R baseline**：P1 tag 未動（依賴 PostOnly demo 累積，背景等待，非本 session 範圍）。
- 🔴 **Healthcheck [12] FAIL 結構性已確認**：bb_breakout 7d entries=0 — FIX-26-DEADLOCK-1 (`bcc5401`+`63957ad`) **已在 binary 20:21** 中，排除部署嫌疑。**根因 = P1-11 F1 結構性 1m bandwidth 永不達 expansion_bw=0.04**，非新 bug；屬 bb_breakout profile/timeframe 不匹配範疇（Wave 2+3 profile 調整或 5m timeframe 範圍）。本 session 不修。
- 🟡 **Binary 落後 origin/main**：binary 20:21 落後 origin/main tip（含 G1-03 後 4 檔 + V024 + 各 docs），operator `restart_all.sh --rebuild` 後即同步；V024 DB 已 apply，重啟不會 crash。
...
| **G3-11** | 🟡P2 | STRATEGIST-CYCLE-OBSERVABILITY-1 — Rust `strategist_scheduler` 加 `CycleCounters`（reject/apply/last_ts per reason）+ IPC emit `strategist_cycle_event` + Python DB sink `learning.strategist_cycle_events` + GUI `/api/v1/strategist/history/cycle_metrics` 切 DB 查詢（取代 engine.log tail-parse，解 416MB log + engine restart + rotation 觀測盲區）· **FA fallback signal**：若 STRATEGIST-PERSIST-AUDIT-GAP-COUNTER-1 久未解（LLM 仍 100% reject → 觀察空 pipeline 倒置優先級），可降級到 Backlog 等 PERSIST gap 先修 | G3-01 IPC RFC | E1+PA / E2+E4 | 2-3d | endpoint 回 DB row（非 log parse）+ reject/apply count 可 cross-validate `strategist_applied_params` + healthcheck [16] `strategist_cycle_fresh`（last_ts <10min）PASS / 來源：[FA 報告](.claude_reports/20260424_fa_eval_gap2_strategist_observability.md) + [PA 報告](.claude_reports/20260424_pa_eval_gap2_todo_placement.md) |
...
| **STRATEGIST-PERSIST-AUDIT-GAP-COUNTER-1** | 🟡 Attempted fix v1 + 觀察中（2026-04-24 晚間）| G3-02 | 🟠P1（降自 🔴，attempt v1 部署中）| **2026-04-24 GUI Live 後實證**：`learning.strategist_applied_params` 0 rows；engine log 近 15 cycle 100% propose 被 ±30% cap reject（cooldown_ms=60000 → propose 30000/150000/180000 = 50-200% delta）；root cause = Python `ai_service._handle_strategist` Ollama L1-9b LLM 不遵守 "Keep changes conservative (within ±30%)" prompt；Rust cap `strategist_scheduler/mod.rs:48 MAX_PARAM_DELTA_PCT=0.30` (safe-by-design) 永遠攔；`persist_applied_params` 從未 invoke。**Attempt v1 部署**（2026-04-24 `STRATEGIST-TUNE-CAP-ENFORCE-1`）：修改 `ai_service._build_strategist_prompt` 預算每個 adjustable param 的 `allowed_range=[current*0.7, current*1.3]`（clipped outer bounds）直接寫入 prompt，HARD RULES 明確說「violation → rejected, your suggestion wasted」+ example 引用實際違規值（30000, 150000）。預期降低 Ollama L1-9b 違規率。驗收：uvicorn restart 後觀察 ≥3 個 5-min cycle，若仍 100% reject → 升級到 (b) 升 L2 Claude 或 (c) Rust-side clamp（違背設計，非首選）。若 apply rate >0% → gap 解決，降回 P2。FA H2 源頭 Phase 5 依賴。|
