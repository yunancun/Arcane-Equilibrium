...
| `P1-EARN-WAVE-C-FIRST-STAKE-RUNTIME` | 2 | **PARTIAL-CLOSED / OPERATOR-BLOCKED 2026-05-25 PM recheck** — IntentProcessor Earn branch IMPL ✅; local `cargo test -p openclaw_engine --release --lib earn` = 80 passed; running engine has `EarnStake=2`; PG `learning.earn_movement_log` still **0 rows**. **Still blocked by operator hand action**: (a) OP-1 Bybit web UI reissue key with `asset:earn` scope (b) execute $100-200 Flexible-only first stake through Earn path. Carry-over: governance_approval_id sentinel 0 until Wave D/E inserts `learning.governance_audit_log` and returns real id. |
