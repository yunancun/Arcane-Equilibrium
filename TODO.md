...
Version: v35
Date: 2026-05-16
Status: v35 current-progress sync. Wave 1-4 source/test work is closed through origin `864f4e81`; Round 4 WP-13 leftover P1 is now source/test closed at local commit `a7cb517f` by routing strategist scheduler tuning and edge-estimate reload demo commands through `DemoCmdSenderSlot`. Mac verification: `cargo check --release -p openclaw_engine` PASS; `cargo test --release -p openclaw_engine tune_cmd_snapshot` PASS 2/2; `cargo test --release -p openclaw_engine edge_reload_tests` PASS 16/16; `cargo test --release -p openclaw_engine --lib` PASS 2908/0/1 after escalated rerun for socket tests; `cargo test --release -p openclaw_engine --bin openclaw-engine` PASS 62/0. W-AUDIT-8a C1 24h isolated `allLiquidation.BTCUSDT` proof ended early at `2026-05-16T00:37:25Z` with `FAIL_CONNECTION`, observed `17055.2s/86400s`, 15 candidate liquidation messages, 0 subscribe failures, and 1 connection-lost error; C1 is not proof-eligible and production liquidation topic revival remains blocked until a full-duration BB/MIT-approved proof. As of pre-deploy check, Mac is ahead of origin by `a7cb517f`, Linux `trade-core` fetched origin and is behind by 1 before this sync, runtime engine/API are alive, and runtime binary still reflects the prior rebuilt code line `7b33ab2e`; because v35 includes Rust runtime changes, three-side source sync plus `restart_all.sh --rebuild --keep-auth` is required. Signed live authorization remains absent and true-live remains blocked.
