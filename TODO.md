...
| 12 | `W-AUDIT-5` 性能/結構/CI/跨平台 (split 5a + 5b) | E1×6 並行 + E5 + E2 + E4 | ACTIVE 2026-05-09 (~17h+17h, 2 sessions) | F-21 source/test added `rust/Cargo.toml [profile.release] strip = "symbols"`; F-26 source/test added GitHub Actions Rust release-check matrix for `x86_64-unknown-linux-gnu` + `aarch64-apple-darwin`; no release build/restart. Remaining 5a: F-12 runner.rs split 5 sibling / F-20 damaged table dump+drop ops / F-27 dictionary drift / test_h_state split. 5b: orjson / deepcopy / ai_budget RwLock / event_consumer split. |
...
| `P1-AUDIT-PERF-5` | 3 | W-AUDIT-5a 性能/結構/CI urgent | F-21 source/test added release symbol stripping in `rust/Cargo.toml`; F-26 source/test added `.github/workflows/ci.yml` cargo release-check matrix for `x86_64-unknown-linux-gnu` + `aarch64-apple-darwin`; no release build/restart/deploy. Remaining: F-12 runner.rs 2467 LOC hard violation split 5 sibling (config/scheduler/reporter/calibrator/metrics, E1-a 6h) + F-20 DROP `trading.*_damaged_20260414_130607` 4 表 909MB (E1-b+ops 2h, NAS dump 先) + F-27 字典 4 drift L5-1..L5-4 + G9-02 章節補 (TW or BB 1.5h) + test_h_state_query_handler.py 2641 split (E4 3h). Heavy parallel; 1 session ~17h. |
