...
**版本** v117 ｜ **日期** 2026-06-05 ｜ **來源實作 HEAD** `f3d4a29e`（三端同步；本版本文檔收口提交在其後）｜ runtime 詳見 §0。
...
| 區域 | 當前狀態 | 下一步 |
|---|---|---|
| 來源同步 | **三端同步**（Mac=origin=Linux `trade-core`；2026-06-05 Alpha-Edge candidate-metrics direct rows interface checkpoint push+ff，實作 HEAD `f3d4a29e`；engine binary 仍沿用 2026-06-03 runtime build，`061d3c8c` dispatch retry boundary 為 source-only 語義等價重構，未在本輪重啟）。 | 維持三端同步；Mac 仍有既有無關 memory/docs dirty，Linux source clean；本 v117 TODO/CHANGELOG 收口提交需同樣 push+ff。 |
...
| `AEG-S2-EVIDENCE-AUTOMATION` | 1 | ✅ S2 基建波三端同步（head `f3d4a29e`）/ 缺 AEG-S3 真候選 rows | 已完成 FND-2 PIT universe（雙審 cleared）、V127 regime-labels live apply、V131/V132 registry、(a) regime runner、(b) breadth ladder、(c) robustness matrix、execution-realism builder、candidate-metrics adapter，且 `candidate_regime_metrics.csv` 已接入 robustness matrix；candidate metrics v0.2 已要求 matrix-critical 欄位，並支援 AEG-S3 harness 直接輸出 top-level `candidate_regime_metrics` block。Linux smoke/測試見 §2 checkpoint。candidate metrics 會對現有診斷缺 `net_bps` / 90/180d freshness / cluster-adjusted `n_independent` / PSR/DSR/PBO / OOS Sharpe fail-closed，matrix 不會把 aggregate breadth、`mean_daily_bps`、`n_days` 或 DSR K budget 冒充 promotion evidence。**下一步**：派 AEG-S3 候選接口（listing fade / oi_delta / funding revive）產真 direct rows；不要把 aggregate breadth、mean_daily_bps 或單一 execution PASS 當 promotion proof。 |
