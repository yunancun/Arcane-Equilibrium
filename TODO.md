...
| `AEG-S3-CANDIDATE-DIRECT-ROWS` | 1 | 🟡 PM-local infra DONE / 真候選 rows 未產 | 2026-06-11 新增 artifact-only `research/aeg_s3_candidate_rows/`：explicit sample returns evidence JSON → top-level `candidate_regime_metrics` direct report；另新增 `research/aeg_s3_listing_fade/`：Gate-B run dir 或 V130 capture-events JSONL export → `listing_fade_candidate_evidence.json`（strict PASS_CAPTURE、explicit regime、explicit cost/k_trials；PBO 不捏造，缺 grid 下游 `missing_pbo`）；再新增 `research/aeg_s3_oi_delta/`：offline OI/price panel JSONL → non-overlapping cross-sectional OI-delta rebalance samples（top/bottom tail long-short，explicit cost/net，保留 standalone 成本牆，PBO 不捏造）。設計報告 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-06-11--aeg_s3_candidate_rows_design.md` + `2026-06-11--aeg_s3_listing_fade_evidence_design.md` + `2026-06-11--aeg_s3_oi_delta_evidence_design.md`；驗證 focused regression（listing fade + oi_delta + AEG-S3 rows + candidate metrics + robustness + Gate-B probe）= **70 passed** + compileall OK；static forbidden-route search 新模組無命中（測試檔僅自身 forbidden list 命中）。**下一步**：等/導入 operator-timed Gate-B 真 transition artifact；從 V125 accepted run 匯出 OI/price/regime panel 跑 oi_delta；補 listing-fade/oi_delta candidate-grid PBO；再接 funding_revive evidence producer，最後串 `aeg_candidate_metrics` + `aeg_robustness_matrix`；未經 E2/MIT/QC 獨立審前不得當 promotion proof。 |
