...
**版本**：v83（v82 + 2026-05-29 cold-audit Wave1/Wave2 source checkpoints + SoT doc-drift patch）
...
**日期**：2026-05-29
**HEAD**：Mac/origin `main` 已含 cold-audit source checkpoints `b93d3210` + `11b9531f` + v83 doc sync；Linux runtime repo pull/deploy/rebuild pending，engine binary 未載入這兩個 source checkpoints。
...
| `P1-COLD-AUDIT-LIVE-GATE-FAKE-SUCCESS` | 1 | 🟡 **SOURCE DONE 2026-05-29 commit `b93d3210` / DEPLOY PENDING** — PkgA 實作 live-auth signing-key verifier、start/resume/grant gate→IPC→Rust readback、live_reserved 精確比對、GUI partial_failure persistent banner、manual-mark honest state、set_system_mode partial_failure + rust_synced=false。E2/A3/E4 green；Mac-only tests per commit body。**Next**: deploy/rebuild 前確認 runtime live-auth signing key/env posture；Linux smoke protected endpoints fail-closed。 |
| `P1-COLD-AUDIT-LIVE-CANCEL-AUTHORITY-DECISION` | 1 | 🟢 **SOURCE DONE + BB SPOT-CHECK APPROVE 2026-05-29 commit `b93d3210`** — live Stop cancel-all 已改 Rust IPC `cancel_all_orders` + PipelineCommand/OrderManager，Python live REST fallback disabled；demo helper保留。E2/E4 green。BB APPROVE-WITH-CONDITIONS：`/v5/order/cancel-all` body category=linear+settleCoin=USDT 正確、authority-revoke 後仍 risk-reducing、Python live REST grep 0。**Deploy-gated only**: Linux IPC end-to-end probe（不可 provoke live cancel）。 |
| `P1-COLD-AUDIT-ORDER-RETRY-POLICY` | 1 | 🟢 **SOURCE DONE + BB SPOT-CHECK APPROVE 2026-05-29 commit `b93d3210`** — OPEN create 走 empty retry schedule，timeout/parse/transport/nonzero retCode 單次嘗試 fail-closed，Decision Lease 非 Consumed；reduce-only CLOSE 保留 bounded idempotent retry作 survival exception。E2/E4 green。BB 確認對齊 Bybit V5 retCode 語義 + 無 create idempotency key（單次嘗試正確）。 |
| `P1-COLD-AUDIT-BYBIT-STOP-LIVEDEMO-SEMANTICS` | 1 | 🟢 **SOURCE DONE + BB SPOT-CHECK APPROVE 2026-05-29 commit `b93d3210`** — trading-stop side-aware tick normalize + missing spec fail-closed；LiveDemo `live` secret slot 禁 process-env credential fallback；amend 缺 instrument spec fail-closed；rate-limit preflight path/group aware。E2/E4 green。BB 6/6 PASS vs Bybit V5；P2-04 doc drift（pre_check / demo dcp / 10→20 r/s comment）已於 Wave 3 修。 |
| `P1-COLD-AUDIT-EVIDENCE-PROMOTION-GATES` | 1 | 🟢 **SOURCE DONE 2026-05-29 commit `11b9531f`+`7909ca3d`** — P1-09 stale/legacy/unvalidated positive edge：live reject、demo exploration；P1-10 paper→demo promotion frozen；P2-01 stub backtest cannot inject evidence；P1-11 reclassified（canonical = V094 `trading.fills.close_maker_*`，不建 dead table，MIT 14396 fills 100% populated 親驗）；P1-16 Alpha/M11 Stage A/B doc split + `attribution_daily promotion_evidence=false` scaffold marker（commit `7909ca3d`）。E2/E4/R4 green。**Deferred-by-design（未來 ML wave，設計已完成見 pkgE report）**: P2-06 model_performance evaluator-writer（無 migration 需求，MIT dry-run + 啟用 deploy-gated）；P2-07 Stage B cohort replay。 |
| `P1-COLD-AUDIT-AI-ML-LINEAGE` | 1 | 🟡 **SOURCE DONE 2026-05-29 commit `11b9531f` / LINUX PG EMPIRICAL PENDING BEFORE DEPLOY** — P1-12 route/policy enum normalized end-to-end；P1-13 provider-native paid call writes durable `agent.ai_invocations` + `learning.ai_usage_log` with deterministic event_ts idempotency（paid ledger failure fail-closed）；P1-14 model registry persistence required for shippable/shadow artifacts；P2-08 paid opt-in + unpriced paid call blocked。E2 condition MED-1/MED-2 cleared + E4 green。**Next**: Linux PG empirical pass for ON CONFLICT/deterministic timestamp semantics before deploy. |
| `P1-COLD-AUDIT-SOT-DOCS-DRIFT` | 2 | 🟢 **TW PATCH + R4 APPROVE 2026-05-29 commit `7909ca3d`** — P1-15 SPECIFICATION_REGISTER ADR-0036..0041 paths corrected（0 missing）；P1-16 Stage A/B doc split（SSOT + register footnote + SCRIPT_INDEX）；P2-04 bybit_api_reference source-aligned（pre_check removed / demo dcp removed）；P2-16 CLAUDE+README「Bybit-only execution; ADR-approved read-only data exceptions」；P2-17 docs/README dead M13/V116 links repointed + missing TODO archive entries。R4 APPROVE（0 dead path, no over-claim）。**PM-deferred policy**: literal-vs-generated index + Operator-mirror stub-vs-copy policy（P3-07 範疇）。 |
| `P2-COLD-AUDIT-DEFERRED-BACKLOG` | 3 | 🟢 **15/17 P2 DONE across Wave1-4 2026-05-29** — DONE: P2-01/02/03（Wave1-2 commits `b93d3210`+`11b9531f`）, P2-04/09/12/14/15/16/17（Wave3 `7909ca3d`）, P2-05/10/11/13（Wave4 `dc2a15aa`）, P2-08（Wave2）。**Design-complete impl-deferred（未來 ML wave，acceptance + 無 migration 需求 in pkgE report `2026-05-29--cold_audit_pkgE_ml_maturity_p2_05_06_07.md`）**: P2-06 model_performance evaluator-writer, P2-07 Stage B cohort replay。**P3 (7) 仍 backlog**: P3-01..07 per PA plan，PM 排程（多為 comment/低風險/index policy）。 |
