...
- `observability.feature_baselines` MAX(created_ts)=`2026-05-27 04:42 UTC` (~7.5h ago / 2074 rows) — **[78] healthcheck 今日 PASS verified 2026-05-27 by E3**（sentinel mtime=04:41:01 fresh / age=8.9h<25h threshold；2026-05-22 G1 cluster "4.75d stale" 為 crontab 未 install 期間舊樣本，現 `41 4 * * *` 已 install + 兩日連續 fire；§13 stale finding CLOSED）
...
| **P0-OPS-1..4** | 🟡 Wave 4 closure 2026-05-27 / OPS-1 round 2 ✅ (8 fix + bonus) / OPS-4 minor ✅ (4/4) / OPS-2 Phase 1 ✅ / GAP-B/D ⚠ PARTIAL operator 7 hand-action / W18-21 前 closure | OPS-1 → E2+A3 重審 → E4；OPS-4 → E4；OPS-2 → E4 + 14d soak；GAP-B/D → operator deploy | OPS-1 round 2 ✅ 8 fix (2 HIGH + 3 MED + 3 LOW) + bonus Caddyfile/$HOME；33→48 test GREEN + 6 JS node --check PASS；enforcing cutover ready；OPS-4 minor ✅ Requires=空 + verify warn/error + root guard + reset-failed；OPS-2 Phase 1 ✅ 14d soak start D+0；GAP-B 53d schema-only dump+NAS 未掛 / GAP-D 3 drafts ready 291 行 但未 install + disk strategy 待拍板；ref §6 NEW P1+P2 |
...
| `P1-OPS-4-GAP-B-PG-RESTORE-DRILL` | 1 | 🟡 **Round 1 PARTIAL 2026-05-27** — MIT Track B 1/3 deliverable land: ✅ `srv/helper_scripts/db/post_restore_validation.sql` (330 LOC / 9 query covering I1/I2/I7/I8 invariants per FA §B.1) / ❌ `docs/runbooks/pg_restore_drill_sop.md` 缺 / ❌ template 缺；**Round 2 dispatched background** MIT 補 SOP runbook (7 scenario / 9 chapter mirror replay_signing_key_rotation.md) + drill report template；阻 first-day live per FA §E #11 |
| `P1-OPS-4-GAP-D-PG-DUMP-CRON` | 1 | 🟡 **Round 1 PARTIAL 2026-05-27** — E1 Track A 4/6 deliverable land: ✅ `helper_scripts/cron/install_pg_dump_cron.sh` (96 LOC) + `trading_ai_pg_dump_cron.sh` (181 LOC EXCLUDE evaluations + governance_audit_log INSERT + retention 30d) + `verify_pg_dump.sh` (129 LOC Bash sidecar 5 check) + `sql/migrations/V113__governance_audit_log_pg_dump_event_types.sql` (V053/V098 race-free pattern + Guard A/B) / ❌ `helper_scripts/canary/healthchecks/check_pg_dump_freshness.py` 缺 / ❌ `passive_wait_healthcheck.sh` wire 缺；**Round 2 dispatched background** E1 補 Python healthcheck + passive_wait wire (per FA §E #7 阻 first-day live) |
| `P1-OPS-4-GAP-B-D-OPERATOR-DEPLOY` | 1 | **NEW 2026-05-27 (round 2 land 後 trigger)** — operator hand-action ~28hr first-day live unblock: (1) D+0 operator review v68 spec amendment + 3 cron + V113 + SQL + SOP runbook (2) `OPENCLAW_BACKUP_CRON_APPLY=0` dry-run install (3) `=1` apply (4) D+1 03:00 UTC fresh dump fire (5) check_pg_dump_freshness.py PASS (6) full data restore drill ~4 hr per MIT empirical (7) MIT row + FA × 2 row + BB Earn cross-sign per PA spec §11 |
