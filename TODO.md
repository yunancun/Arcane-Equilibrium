...
**版本**：v75（v74 + 2026-05-28 PM takeover：runbook v1.0 4-patch land + 14d soak D+1 PASS + 3 P2/P3 carry-over）
**日期**：2026-05-28
**HEAD**：previous Mac SSOT `e59e6ff1`（v74 doc sync）；本 v75 doc sync 為 branch tip pending；previous code commit `920f8299`（Wave 5 Packet C engine integration source land）；Linux runtime engine PID 仍 `a07a08c0` 時代 binary（minimal slice 不接 pipeline_ctor，per AMD §Decision 2.5）。
**Session**：D+2 PM takeover continuation；driveable non-operator-gated 工作落地：(1) `P1-OPS-2-RUNBOOK-V1.0-PATCH` 4 patches CLOSED — PA delivered v1.0 對齊 A3 對抗性核驗 4 conditional items（§4.2.1 backward-compat + §10.1.1 grep WARN=0 invariant + §10.5 cross-lang HMAC pinned hex + §13 Phase 2 cutover SOP），495→687 行（+192）；(2) `P1-OPS-2-14D-SOAK-OBSERVE` D+1 grep ssh trade-core empirical PASS — engine.log 0 / api.log 0 `ops2_secret_split_phase1_fallback`，AC (a) on track；(b) `/auth/renew` 0 次 = OP-1 待簽 operator-blocked；(3) 3 P2/P3 carry-over land — runbook §10.3 healthcheck SQL routine 未實裝 (MED) / §6.2.4 + §5.2.3 emergency revoke audit contract 缺 (LOW)。下一步維持 Operator TOTP enrollment + Operator-timed restore drill S1 + (post-Wave) Packet C pipeline_ctor wire。
...
| `P1-WAVE5-TOTP-BACKEND` | 1 | 🟡 **SOURCE DONE / VAULT DIR PREP DONE / OPERATOR SECRET ENROLLMENT PENDING 2026-05-28** — `autonomy_totp.py` file-backed fail-closed verifier + route gate + 10/10 pytest PASS already landed. ssh trade-core (2026-05-28 15:51 UTC) prep: `mkdir -p ~/BybitOpenClaw/secrets/vault && chmod 700` 已建好（drwx------ ncyu ncyu）；direct probe via Python `autonomy_totp._load_totp_config()` 確認 runtime state = `exists=False / configured=False / error=secret_file_missing`，API endpoint 401 require auth = fail-closed by design。Operator one-liner（agent 不可生成 seed）：(1) 用 authenticator app 產 base32 seed `SEED_B32`；(2) `python3 -c "import hashlib; print(hashlib.sha256(b'${SEED_B32}').hexdigest())"` 取 `FINGERPRINT`；(3) `umask 077 && cat >~/BybitOpenClaw/secrets/vault/autonomy_totp.json <<EOF\\n{"totp_seed_b32":"${SEED_B32}","fingerprint":"${FINGERPRINT}","totp_algorithm":"SHA1","totp_digits":6,"totp_interval_sec":30}\\nEOF`；(4) smoke `cd /home/ncyu/BybitOpenClaw/srv/program_code/exchange_connectors/bybit_connector/control_api_v1 && .venv/bin/python3 -c "from app.autonomy_totp import autonomy_totp_backend_configured; print(autonomy_totp_backend_configured())"` 必回 `True`；(5) `bash srv/helper_scripts/restart_all.sh --keep-auth` 重啟 API；(6) 認證 GET state 確 `totp_backend_configured=true`。 |
| `P1-WAVE5-PACKET-C-E2-E4-INTEGRATION` | 1 | 🟡 **ENGINE INTEGRATION SOURCE LAND / pipeline_ctor wire PENDING 2026-05-28** — commit `920f8299` lands `openclaw_engine::notification_failsafe` module (1099 LOC inc 14 mock tests + 5 trait seam: NotificationDispatcher / PositionSnapshotProvider / ExchangeStopSync / FailsafeAuditEmitter / FailsafeClock). 整條 chain 已接：observe AllFail → 1h timer → SM-04 `transition(Defensive, NotificationFailsafeTimeout, RiskGovernor, "auto_escalated_to_sm04_defensive")` → `active_lock_profit_per_position` → `ExchangeStopSync::sync_stop` per adj → `FailsafeAuditEmitter::emit_auto_escalated`. `DEFAULT_TIMEOUT_MS=3_600_000` compile-time hard-coded（無 TOML override 路徑，per AMD §Decision 2.5 + Q3 Path A）；`cargo test -p openclaw_engine --lib` 3482/3482 PASS = 3468 baseline + 14 new；clippy 0 hit；core 27/27 unchanged。Minimal slice 不接 pipeline_ctor / 不 spawn 長運行 task / audit 走 trait emitter（沒下一 wave 真實 3-way dispatcher 接線前 wire 進 runtime 等同假 wire，per CLAUDE.md §六 + memory feedback_no_dead_params）。 |
