...
|---|---|---|---|---|
| **P0-EDGE-1** | 🔴 ACTIVE（SSH empirical 2026-05-26 22:00 UTC: 0/3 AC paths satisfied）| QC + PA | (i) 5 textbook ≥3 demo 7d avg_net>5bps + Wilson lower>0 + n≥30；**OR** (ii) ≥3 alpha-bearing 達同標；**OR** (iii) portfolio 7d gross 正向 | 唯一可預期 closure path = (ii) Sprint 2 Alpha Tournament W12-15 dispatch；當前 (i) 5/5 textbook `insufficient_total_samples` (max n=16 funding_arb；runtime_bps −11~−42)；(iii) live_demo 7d net=-1.99 USDT |
...
| **P0-OPS-1..4** | 🟡 SPEC LAND 2026-05-26（4 sub-agent verdict）/ E1 IMPL DISPATCH PENDING / W18-21 前必 closure | PA + BB + E3 | OPS-1 HTTPS Caddy+Tailscale cert spec ✅；OPS-2 1 CRITICAL secret split + 2 HIGH runbook gap；OPS-3 0 ship-stop / 5 operator confirm；OPS-4 runbook 11 章 / 4 critical gap | OPS-1 E1 dispatch (23-33hr)；OPS-2 P1-OPS-2-SECRET-SPLIT + RUNBOOK + DRY-RUN；OPS-3 5 operator confirm (C-1 residence)；OPS-4 4 critical gap (systemd + PG backup)；ref §6 NEW P1 entries |
...
| **F** (NEW 2026-05-26) | funding_arb (D) 3C TOML deprecation cascade (operator-decided) | T2: PA deprecation spec → TW docs/README/AMD cascade + R4 cross-ref | 4-6 hr | 1 (parallel) | — D+0 可啟動 |
...
| `P1-OPS-3-OPERATOR-CONFIRM-5` | 1 | **NEW 2026-05-26 per BB OPS-3 verdict CONDITIONAL** — 5 critical operator confirm 阻 Sprint 4：C-1 residence 自證 + 非 Bybit 16 restricted jurisdictions (US/HK 2025+/SG/CA/FR/Japan 2026 phase-out/OFAC；operator residence 全 governance doc 0 hit = 最大 ship-stop) / C-2 KYC tier ≥ Standard L1 / C-3 KYC 完成日 ≥ 2026-04-09 (Earn scope key) / C-4 tax filing jurisdiction (EU DAC8 2026-01-01) / C-5 Earn Flexible APR floating + 平台 default risk = total loss 自承；ref `srv/docs/CCAgentWorkSpace/BB/workspace/reports/2026-05-26--p0-ops-3-bybit-tos-geo-kyc-audit.md` |
...
| `P1-FUNDING-ARB-DEPRECATION-CASCADE` | 2 | **NEW 2026-05-26 per operator decision (D)** — funding_arb (D) 3C TOML deprecation closure cascade；PA deprecation spec + TW docs/README/AMD cascade + R4 cross-ref；commit `a19797d` 已 land base_ratio 0.4→0.25 + funding_arb 3% override；Owner: PA spec → TW docs cascade → R4 audit；ETA: 4-6 hr；ref §9 Workflow F |
...
| **operator decision 2026-05-26 (D) funding_arb deprecation** (Workflow F NEW) | (1) PA deprecation spec (commit `a19797d` 已 land base_ratio 0.4→0.25 + funding_arb 3% override，但 strategy 程式碼 + TOML active 仍需 PA 終結) (2) TW docs/README/AMD cascade (3) R4 cross-ref audit | PA → TW → R4 | 待派 (4-6 hr 估)；ref `P1-FUNDING-ARB-DEPRECATION-CASCADE` |
