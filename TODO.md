...
| **GRID-LIFECYCLE-DRIFT** grid_trading lifecycle 漂移觀察 | 資料被動 | 2026-04-29 → 05-06（7d）| 🔴 **首跑 FAIL**（`45bbe4d` 路線 B 修配對後 2026-04-29 立刻揪出 real signal：lifetime_ratio=0.18-0.21 / re_entry_rate=0.75-0.77 / fee_burn ratio=2.11-2.20）| [38] grid_trading_lifecycle_drift（lifetime ratio < 0.5x WARN / < 0.3x FAIL；fee burn > 0.8 abs WARN / > 1.5 FAIL；same-symbol re-entry > 0.5 WARN / > 0.7 FAIL）| **不需等 7d**：1 次 FAIL 已 confirmed（規則：連 3 次 FAIL 中止被動等待）→ 派 PA RFC 評估 grid_trading live config（縮 levels / 暫停 5 robust 負 cells / live trailing 改 2.5~3.0% / 暫停 partial_tp）|
