...
**§九 governance 戰況**：⚠️ 2026-04-29 21:56 CEST 接手核對發現此快照已漂移：`rg --files ... | xargs wc -l` 顯示多個 active 檔重新超 1200（例：`strategist_scheduler/mod.rs` 1819、`event_consumer/loop_handlers.rs` 1481、`checks_strategy.py` 1239、`checks_engine.py` 1206；另有多個 tests/static 檔超線）。原「1200 hard cap active violations 0」視為 stale，需走 G5/G6 maintenance wave 重新收斂；優先見 `G5-08 E1 implementation` + `CHECKS-STRATEGY-SUBSPLIT`。
...
9. **Maintenance backlog** 等下次 wave：
   - G5-08 E1 implementation P1（`strategist_scheduler/mod.rs` 1819 行，PA split plan ready）
   - CHECKS-STRATEGY-SUBSPLIT P2（`checks_strategy.py` 1239 行，已從 warning-zone 變 hard-cap drift）
...
| **G5-08 E1 implementation** | PA design plan ready (`2063386`)；按 Method A 4-sibling 落地；2026-04-29 接手重掃 `strategist_scheduler/mod.rs` **1819** 行（比 PA plan 1770 更高，仍是 P1 hard-cap cleanup） | 立即可派（無前置）| 🟠P1 | E1 5-6.5h 全鏈（含 E2 review + E4 regression）；下次 session 啟動；engine lib baseline 2166 預期升至 2166+ |
...
| **CHECKS-STRATEGY-SUBSPLIT** | E2 batch review 原揭：`passive_wait_healthcheck/checks_strategy.py` 1048 行接近 §九 800 警告線；2026-04-29 接手重掃已 **1239** 行，越過 1200 hard cap，需拆 EDGE-P / bb_breakout / counterfactual 系 | 下個 G6-04 維護 wave | 🟡P2 | E5 1d |
