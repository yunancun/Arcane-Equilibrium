...
**日期**：2026-05-29
**HEAD**：Mac/origin `08fddabf`（含 cold-audit source checkpoints `b93d3210` + `11b9531f` + v83 doc sync）；Linux runtime repo pull/deploy/rebuild pending，engine binary 未載入這兩個 source checkpoints。
...
| `P1-COLD-AUDIT-LIVE-CANCEL-AUTHORITY-DECISION` | 1 | 🟡 **OPERATOR DECISION APPLIED + SOURCE DONE 2026-05-29 commit `b93d3210` / BB PRE-DEPLOY SPOT-CHECK PENDING** — live Stop cancel-all 已改 Rust IPC `cancel_all_orders` + PipelineCommand/OrderManager，Python live REST fallback disabled；demo helper保留。E2/E4 green。**Next**: Bybit-facing deploy 前做 BB-style spot-check：`/v5/order/cancel-all` body category=linear + settleCoin=USDT、authority revoke 後 cancel 仍可 risk-reducing 執行、no Python live REST path grep 0。 |
| `P1-COLD-AUDIT-ORDER-RETRY-POLICY` | 1 | 🟡 **SOURCE DONE 2026-05-29 commit `b93d3210` / BB PRE-DEPLOY SPOT-CHECK PENDING** — OPEN create 走 empty retry schedule，timeout/parse/transport/nonzero retCode 單次嘗試 fail-closed，Decision Lease 非 Consumed；reduce-only CLOSE 保留 bounded idempotent retry作 survival exception。E2/E4 green。**Next**: deploy 前復核 CLAUDE hard-boundary wording + Bybit retCode/no hidden create retry。 |
| `P1-COLD-AUDIT-BYBIT-STOP-LIVEDEMO-SEMANTICS` | 1 | 🟡 **SOURCE DONE 2026-05-29 commit `b93d3210` / BB PRE-DEPLOY SPOT-CHECK PENDING** — trading-stop side-aware tick normalize + missing spec fail-closed；LiveDemo `live` secret slot 禁 process-env credential fallback；amend 缺 instrument spec fail-closed；rate-limit preflight path/group aware。E2/E4 green。**Next**: BB-style spot-check against Bybit reference before runtime deploy；P2-04 doc wording still future TW cleanup if needed。 |
| `P1-COLD-AUDIT-EVIDENCE-PROMOTION-GATES` | 1 | 🟡 **PARTIAL SOURCE DONE 2026-05-29 commit `11b9531f`** — P1-09 stale/legacy/unvalidated positive edge：live reject、demo exploration；P1-10 paper→demo promotion frozen；P2-01 stub backtest cannot inject evidence；P1-11 reclassified: canonical close-maker evidence = V094 `trading.fills.close_maker_*` writer/readers/healthchecks，`learning.close_maker_audit` table missing is stale spec artifact，**不建 dead table**。E2/E4 green。**Still active**: P1-16 Alpha/M11 Stage A smoke vs Stage B promotion evidence doc split + scaffold `promotion_evidence=false` marker；P2-06 mode-scoped drift/model_performance empty-table live packet defer；P2-07 Stage B cohort replay design/impl deferred。 |
...
| `P1-COLD-AUDIT-SOT-DOCS-DRIFT` | 2 | 🟡 **PM/TW PATCH DONE 2026-05-29 / R4 VERIFY OPTIONAL BEFORE ARCHIVE** — SPECIFICATION_REGISTER ADR-0036..0041 active paths corrected；quick active path-existence check returns 0 missing；PkgB/PkgC Operator mirrors added and byte-identical to PA canonical reports (`cmp=0`)。**Next**: R4 or PM final sweep before moving from active queue；keep future Operator mirrors as pointer/stub or exact copy with `cmp` check. |
