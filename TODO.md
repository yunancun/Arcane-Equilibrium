...
**日期**：2026-05-29
**HEAD**：Mac = origin = Linux `main` = `5bf8085c`（三端同步 2026-05-29 PM）；Linux engine binary **= P1-110017 fix `caf008b6`**（atomic deploy 15:51 UTC，engine PID 113386 SHA-verified）。本輪（2026-05-29 PM 接手）= health-freeze reconcile + #1 LG-3 ARMED + #2 P2/P3 cleanup batch + **P1-110017 治本 fix DEPLOYED+VERIFIED**（demo PHYS-LOCK zero-position close-loop 已修停），詳見 §-1 首條 + §0 runtime snapshot。
...
| `P2-110017-D2-RECONCILE-QTY-GT-ZERO-DRIFT` | 2 | **NEW 2026-05-29 per BB MED-1 + E2 MED-1** — 本次 fix guard 限 `qty==0 全平 form`（安全），故 **qty>0 close-maker drift（demo `use_maker_close=true`，close-maker PostOnly+market fallback 走 qty>0）撞 drift 倉撞 110017 不會自動收斂** → 若該路徑出 drift 會「安靜迴圈」（NoOp 無 log spam 但仍 ~1.4/sec 下單）。治本 = D2 reconcile 納 exchange truth：對「連發 ≥N 次 110017 / Bybit query 確認 size=0」的 symbol 強制本地 remove + log drift（不限 qty form，但須先 position query 確認 size=0 防誤刪，per BB）。Owner: PA spec → E1 → BB → E2 → E4。 |
