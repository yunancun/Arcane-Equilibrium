...
| `P1-ENV-OPENCLAW-ALLOW-MAINNET-SET-FOR-EARN-FIRST-STAKE` | 1 | **NEW 2026-05-25 per Action 3 SSH verify (operator 拍「立刻做」)** — engine PID 350616 (post Hygiene Phase 1a restart) environ 完全無 `OPENCLAW_ALLOW_MAINNET=1`；列 OPENCLAW env: AUTO_MIGRATE=0 / CANARY_MODE=1 / ENABLE_PAPER=0 / LEASE_ROUTER_GATE_ENABLED=1 / DATA_DIR / H_STATE_GATEWAY=1 / BASE_DIR / COST_EDGE_ADVISOR=1 / IPC_SOCKET / DATABASE_URL_FILE / IPC_SECRET_FILE / AGENT_SPINE_RUNTIME_MODE=shadow (12 var 但 0 hit mainnet gate). **當下對 demo 0 影響** (demo 不需 mainnet gate)；**對 Earn first stake CRITICAL** — Earn endpoint mainnet-only (per BB) + true live 5-gate 必含此 env (per CLAUDE.md §四 Hard Boundaries). **同時 verify**: live slot `/home/ncyu/BybitOpenClaw/secrets/secret_files/bybit/live/` 存在 — api_key 18 bytes (mtime 2026-04-10 19:54) + api_secret 36 bytes + bybit_endpoint 5 bytes / owner ncyu:docker / 0600 ✅; API healthz Tailscale bind `http://100.91.109.86:8000/api/v1/healthz` 200 OK (修正 Phase 1a「localhost 空回應」是 bind 差異而非 endpoint 改名). **修法**: (1) ssh trade-core edit systemd unit / .bashrc / restart_all.sh export 加 `OPENCLAW_ALLOW_MAINNET=1` (2) restart engine via `bash helper_scripts/build_then_restart_atomic.sh` (per Hygiene Option E Phase 1b 防 race) (3) verify env 含 var + LIVE-GUARD-1 三閘 (`bybit_rest_client.rs:909`). Owner: E1 + operator；ETA: ~15 min；阻 Earn first stake；不阻 demo trading / C10 7d observation |
