...
| Runtime | Linux `trade-core` **engine 已 rebuild+restart 到 `b8c258b4`**（6/3 00:40，PID 2269678，release build 39.92s，本 session 親驗 watchdog engine/demo/live alive、ticks 流動）。**P5 step-i soak RUNNING**（flag-on `OPENCLAW_LEASE_PYTHON_IPC_ENABLED=1` operator-env + `--api-only` restart 2026-06-03，全 5 API workers flag=1 親驗）。funding/OI engine 改動已上線（行為等價）+ **`--apply` DONE**（run `18b3c2f8`：V125 history 已回填 funding 46539 + OI 348153 rows，accepted）。 | **P5 soak 監測** api.log divergence WARN（0=pass）+ `learning.lease_transitions` 成長；**soak 期間避免全量 `restart_all`**（operator-env 會 revert）；funding/OI `--apply` 已完成（run-versioned 查詢須固定 run_id）；QA A-1/A-2/B/A-4 done（2 PASS/2 INCONCLUSIVE，re-check 6/10）；paper lane restart 後翻 alive 待復查。 |
...
| `P0-EDGE-1-POST-DEPLOY-QA-A1A2BA4` | 2 | ✅ QA DONE（A-2/A-4 PASS · A-1/B INCONCLUSIVE · 0 FAIL/BLOCK） | QA 親查（`324001c3` 6/1 21:19 部署，~1.2d 證據窗）：**A-2 qty_zero skip PASS**（reject_other 6/1 21:21 後停寫、rebuild 後 0、qty_zero_skips counter live 9/0）；**A-4 runtime_bps PASS**（rb==sb 100% cells、0 old-bug signature；caveat 現無正 cell，zeroing 分支未對正 cell 觸發）；**A-1 INCONCLUSIVE + 前提證偽**（bb_breakout 本就 non-BTC 主導，BTCUSDT 35d 僅 4 intents → OI-gate fix 碼正確但非 binding；應 re-scope 為「OI-gate decouple 預防性」非「外幣恢復」）；**B INCONCLUSIVE**（gate 碼 E1/E2/E4 已驗，但 `regime_snapshots` 0 rows + Hurst label 未逐 intent 持久化 + 現僅 1/31 symbol mean_reverting → 0 post-gate fire；behavioral-consistent 非 positive-confirm）。A-1/B re-check 2026-06-10。 |
| `P1-BB-REVERSION-REGIME-OBSERVABILITY` | 2 | QA 建議（B 無正面證據面） / 待 PA scope | B fix 無 observability 證 fire-in-mean_reverting：`market.regime_snapshots` 全表 0 rows（producer 未跑）+ Hurst regime 未逐 intent 持久化。建議 fire 時把 Hurst regime label 持久化進 `trading.intents.details`，供未來 QA 正面驗證 + 助 `P3-BB-STRATEGIES-30D` 樣本判定。**（2026-06-03 評估：動 live intent 序列化路徑 → 需 PA 設計 + E1→E2→E4，非 directly-start 快修；funding/OI 2 LOW 同評為非快修：LOW#1 honest-fix 需改 `u64`→`Option` + nullable 欄位 migration，LOW#2 E2 已接受現狀。）** |
...
| `P0-EDGE-1-CAND-FUNDING-OI-BACKFILL` | 1 | ✅ code `5b80c2f7` + dry-run smoke PASS + **`--apply` DONE**（run `18b3c2f8`，accepted） | NO-GO-TREND verdict 點名 P0 基礎：現 funding 覆蓋僅 ~58 天是多日持倉硬約束。接手孤兒 Codex WIP → 全鏈（E2 對抗審查〔client byte-identity + strict-parse mutation bite〕+ E4 Mac/Linux 3757/0/1 + dry-run smoke 全 PASS）→ `--apply` 回填 V125 history（run `18b3c2f8`，20 symbol×730d）：**funding 46539 + OI 348153 rows / 各 20 symbol / ingest_run accepted / 0 rejected**；19 symbol OI 滿 730d + POLUSDT partial（15254≈636d，POL 2024-09 上市合理）。**⚠ run-versioned schema（run_id 在 PK）**：re-apply 會 append 新 run 非 0-new 冪等，查詢須 `WHERE run_id`/取最新 run；運維「刷新」須清舊 run 或固定 run_id（schema doc/cron wrapper 待補）。follow-up：(1) **E5 RCA 證偽「lingering bug」**：非 bug，6m20s 是合法序列分頁（1985 page×~178ms、ledger 均勻分佈、末頁→accepted 3ms、無背景 task）；一次性 backfill 可接受，cron 化才需 symbol 間 `buffer_unordered N=2-3`（→~2-3min，PA/BB 評 demo rate-limit 安全上限）；(2) 2 LOW（ledger request_start honesty / page 僅 retCode=0）；(3) stdout block-buffered 誤導進度判讀（cron 化改逐行 flush）。spec：`…/BB/…/2026-06-02--funding_oi_backfill_endpoint_spec.md`。 |
...
| `P5-SM-OPTION2-CONVERGENCE` | 1 | 🟡 step-i + E1b done / **soak RUNNING**（flag-on 2026-06-03） | SM 單源收斂 = **Option 2**(Rust 唯一權威、刪 Python SM transition、Python 降唯讀投影 + 留 5 live-auth 閘 + GovernanceHub)。step-i Rust IPC `a99bfa1d` + E1b `e6aa5e37`（Python 權威路由 + auth-axis divergence comparator，re-E2 mutation PASS）已編入 running engine（`b8c258b4`）。**soak STARTED 2026-06-03（operator 批准）**：`OPENCLAW_LEASE_PYTHON_IPC_ENABLED=1`（operator-env 一次性）+ `--api-only` restart，全 5 API workers flag=1 親驗 → hub lease 走 Rust IPC + Python shadow。**監測**：`grep -i divergence /tmp/openclaw/api.log`（真 mismatch 發 WARN，0=pass）+ `learning.lease_transitions` 成長。**revert caveat**：operator-env 於下次**全量 `restart_all` 自動 revert OFF**（worker respawn 不影響，master 持 env）→ soak 期間避免全量重啟（或改 persistent config）。**next**：24-48h 0-divergence（N≥數百 lease ops）→ step-ii projection → 🔴step-iii CUTOVER（operator sign-off + CC/E2/BB/E4）。設計:`…/Operator/2026-06-02--sm_option2_convergence_migration_design.md`。 |
...
| funding/OI backfill `--apply` 真回填 | ✅ **DONE**（operator 核准，run `18b3c2f8`，2026-06-03）| V125 history 已回填：funding 46539 + OI 348153 rows，20 symbol×730d，accepted，0 rejected（POL partial 合理）。run-versioned：未來刷新須清舊 run/固定 run_id。 |
...
| P5-SM step-i soak | ✅ flag-on 2026-06-03（operator 批准，operator-env + --api-only）/ **soak RUNNING** | 24-48h 0-divergence gate；**avoid 全量 `restart_all`**（會 revert operator-env flag）；gate 過後 step-ii→step-iii cutover。 |
