...
|---|---|---|---|
| `P0-PHASE-1B-PARAM-CALIBRATION-1` | ⏳ **scheduled post-12H window ~2026-05-18 11:54 UTC** | **Dispatch sequence after 12H window closes**: (1) PA spec calibration sweep design (offset_bps 0.5 → {1.0, 2.0, 3.0} × buffer_ticks 1 → {2, 3, 4} × timeout {30s→60s/90s grid, 15s→45s/60s phys_lock} per-symbol cohort) → (2) E1 replay counterfactual harness IMPL (re-simulate 4 timeout fills + recent N=50 whitelist closes at swept params; output `expected_fee_saving_bps` per cell + Wilson CI) → (3) PA verdict + recommended TOML deltas → (4) operator dispatch live-demo trial (1-2 cell pilot 24h before broader sweep) → (5) E4 regression + QA APPROVE → (6) merge + restart. **Acceptance**: ≥1 viable parameter cell with `maker_fill_rate ≥ 25%` AND `expected_fee_saving_bps ≥ 0.5` AND zero adverse selection signal (no `slippage_after_quote_move` > pre-Phase-1b baseline). **PnL impact** (operator priority lens): unlocks projected $50-$200/year fee saving (E3 empirical) that current 100% timeout-fallback freezes at $0. **Owner**: PA → E1 → E2 → E4 → QA. **Est**: ~3-5 person-day (PA spec 1d + E1 replay harness 2d + sweep + review). |
...
- AC-A verification: ⏳ post-restart 90min sample = 1 whitelist close, 0 attempts (n=1 too small); requires ~24h window for statistical significance per QA template; PM 24h audit packet template ready at `docs/CCAgentWorkSpace/PM/workspace/templates/2026-05-18--pm_24h_post_deploy_verification_audit_packet.md`.
- Phase 2a 14d observation clock t=0 = first AC-A SQL PASS UTC timestamp (NOT restart timestamp).
- Cross-wave consistency check pending (QA recommendation #3 — restart triggered W6/W7/W1 main-landed-but-not-deployed sources too).
- Next: Phase 2b LiveDemo / operator + AMD live carve-out / Phase 3 Mainnet remain future gates.
