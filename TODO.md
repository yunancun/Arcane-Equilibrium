...
**關鍵路徑**:`~~P0-0 reconciler burst fix~~ ✅ → ~~restart_all --rebuild 部署~~ ✅ → ~~P0-9 STABILITY-1 引擎崩潰 RCA~~ ✅（停電 infra 事件，非 code bug）→ ~~P0-11 LIVE-GATE-BINDING-1~~ ✅（2026-04-18，Python↔Rust 授權綁定）→ ~~P0-6 intent write 永久修復~~ ✅（commit 293a808，待 `--rebuild` 部署）→ ~~P0-7 order submit gap~~ ✅ archived（RCA 證偽，P0-6 子問題）→ **DUAL-TRACK-EXIT-1 Step 0 可行性 sprint → Phase 1 Track P + P1-7 A/B/C 並行**（W23）→ **P1-10 結構修復 + Phase 2 shadow**（W24）→ P0-3 Phase 5 edge 2w 評估 + P0-2 LG-1 21d demo → LG-4/5 → Live`(P0-1 G-2 並行驗證 funding_arb 子集,不在主路徑;~~P0-5 PHANTOM-2-FUP~~ ✅ 待 `--rebuild` 部署即生效;~~P0-8 LIVE-GUARD-1~~ ✅ Rust 端 Mainnet 三重硬鎖回補)
