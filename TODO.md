...
**版本** v142 ｜ **日期** 2026-06-12 ｜ **來源實作 HEAD** main 本檔所在 HEAD（本輪 incident-policy `sm_halt_stuck` producer coverage；runtime 詳見 §0/§6；版本敘事歸 `docs/CLAUDE_CHANGELOG.md`）。
...
| `P2-INCIDENT-POLICY-DISPATCH-TRIGGER` | 2 | 🟡 **CORE+auth+Bybit+sm_halt source-live / BB+E2 reviewed prior CORE path / producer coverage partial** | 2026-06-12 `sm_halt_stuck` producer slice 已接：新增 `event_consumer/sm_halt_incident.rs`，以 `TickPipeline.halt_kind` + `halt_set_ts_ms` 為 runtime source-of-truth（不讀已 stale 的 `[69]` healthcheck selector）；tick 後與 60s lease sweep 後觀察 active HaltSession，5s cadence feed `IncidentClass::SmHaltStuck`，halt 清除時 `report_resolved`。既有 CORE/auth/Bybit/C4 path 保持：`incident_policy` class ledger、sustained/throttle/7d cooling、push-secret gate、single armed owner、自癒 guard；`auth_invalid` / `bybit_fail_closed` producer source-live。Mac focused Rust：`sm_halt_incident` 5 passed、incident_policy 15 passed、C4 wire 4 passed、halt_ttl 20 passed、ret_code_counter 6 passed。邊界：上一輪 BB/E2 review 只覆蓋 CORE+auth+Bybit path，新 `sm_halt` slice 尚待 BB/E2/E4/QA/full-chain review；`position_drift` notify-only、external `engine_dead` watchdog notify-only 仍未接；未 CI、未 deploy/rebuild/restart、無 DB/auth/risk/trading mutation。報告 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-06-12--incident_policy_sm_halt_stuck_producer.md`。下一步：接 `position_drift` notify-only producer，或先對 `sm_halt_stuck` 做 BB/E2 focused re-review。 |
