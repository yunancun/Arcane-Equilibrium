# OpenClaw TODO — 工作清單（v9 · P0/P1/P2 三層 · 2026-05-05）

**版本**：v9（2026-05-05 §九 LOC governance change：硬上限 1500→2000 + REF-20 Sprint C accept C1+C2 split）
**v8 摘要保留**：2026-05-05 REF-20 Sprint A + B closed — 累計 12 commit chain：A 8 commit `c1ab7ea9 → ... → 2531c011` + B 4 commit `2a69addb (B1 R4+R0-T0) → c679a8b4 (R5-T1+T2) → a2f819c5 (R5-T3) → 4ffb24c4 (R5-T4+T5+T6+T7)`；plan §6.R3 acceptance 4 表 row > 0 + §6.R4 UI enabled + §6.R5 A4 + A5 hermetic acceptance PASS。
**v9 governance change**：CLAUDE.md §九 硬上限 1500→2000（operator 決定，REF-20 Sprint C 拍板，理由：runner.rs 1466 + R6 ~180 LOC 將破舊 1500 cap）。**警告線維持 800**。Sprint C 設計 accept PA push back 拆 C1（R6 fee/execution calibration, ~6.5d）+ C2（R7 MLDE/Dream advisory integration, ~6d）；總 12.5d wall。`runner.rs` R0-T0 拆檔仍按 PA 設計做（邏輯內聚清理），非 LOC 強迫。
**HEAD**: `da02230d`（Mac/Linux/origin 同步）· **Engine deployed**: `dbcf845b`（Sprint 3 Track H 仍 active；R3 round 1-9 為 Python only，無 Rust rebuild）
...
| **P1-INFRA-3p** | 🔄 **Sprint C in flight (2026-05-05 PA design + QC + MIT advisory delivered, V055 retrofit pre-task pending)** — accept PA push back 拆 C1+C2 + V055 R6-T0' pre-task（MIT P0 BLOCKER fix）。**Sprint C1 (R6, ~7d)**：**R6-T0' V055 retrofit**（NEW，operator 拍板必先做）— `CREATE OR REPLACE FUNCTION learning.verify_replay_evidence_and_insert` 修補 INSERT 漏寫 4 metadata column（`evidence_source_tier` / `replay_experiment_id` / `manifest_hash` / `expires_at`），V036 docstring 自承「PR3 補 function body」從未發生 — silent corruption 風險：R7 producer 傳 calibrated_replay 但 row body 走 V040 default `'real_outcome'`，A10 假綠。Fee/Execution Calibration — grid_trading + ma_crossover pilot；apply_fill 真 maker/taker fee + slippage model；Rust CalibrationLabelProducer 採 QC spec（n=200/30 + freshness 7d/14d + MAD 3/8 bps + IQR 8/20 bps 四維度 AND filter；MAD-based σ 不用 sample std；empirical percentile CI 不用 naive bootstrap；'limited' tier='calibrated_replay' + 短 TTL 3d；regime detection DEFER to Sprint D）；Python simulated_fills_writer.py payload extension；experiment_registry.py execution_confidence write；R6-T7 順帶 unblock LG-3 healthcheck (RFC closure 0%→70%)。**R0-T0 拆檔 (Sprint B B1 模式)**：抽 `IsolatedPipeline::execute` → `replay/isolated_pipeline.rs`（新檔 ~400 LOC）+ 抽 `apply_fill` → `replay/apply_fill.rs`（新檔 ~250 LOC），runner.rs 1466→~750 內聚清理（非強迫 LOC，governance 已 2000 cap）。**Sprint C2 (R7, ~6d, C1 closed 後啟)**：MLDE/Dream Advisory Integration — dream_engine.persist_dream_insights + opportunity_tracker.persist_regret_summary 升級 evidence_source_tier='calibrated_replay'（依 R6 deliver 的 execution_confidence label，**caller 端傳 7d TTL** per operator 拍板，不採 V036 default 30d）；linucb caller 驗（MIT 確認 0 verify_replay caller，R7-T4 likely no-op 加註釋）；mlde_demo_applier_evidence_filter Block B integration test；4 producer FK chain audit；R7-T7 加 1-line INFO log dump capability+block mode（observability for LG-3 healthcheck）。**1 V### migration**（V055 retrofit）。**Sprint C acceptance**：A6 fee-aware PnL + A7 confidence honesty + A10 ML/Dream advisory boundary（hardcoded "real_outcome" 0 hit / paired CHECK enforce / TTL hard check enforce）。**Pre-DAG advisory ✅ DONE**：QC `2026-05-05--ref20_r6_calibration_label_spec.md` + MIT `2026-05-05--ref20_r6_r7_capability_risk.md`。**Post-MIT operator decisions**：D1=V055 R6-T0' accept / D2=7d TTL / D3=PM 自接 補 task DAG。設計報告：`docs/CCAgentWorkSpace/PA/workspace/reports/2026-05-05--ref20_sprint_c_task_dag.md`（§13 PM-appended post-advisory updates 待加）。 | Sprint B R4+R5 acceptance ✅，QC+MIT advisory ✅，V055 dispatch pending |
