...
| `P0-NEW-VULN-2` | SOURCE-CLOSED 2026-05-09 / DEPLOY PENDING | lease audit runtime 0 emit (HIGH) | `e97a333b` emits one synthetic `BYPASS` audit row for Validation/Exploration facade bypass without creating SM objects; V078 widens `learning.lease_transitions.to_state` for the facade state. Remaining runtime action: deploy/rebuild/restart, apply V078, then verify lease_transitions row count > 0. |
