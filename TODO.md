...
| 12 | `EDGE-P2-3 Phase 1b` Close-Maker-First Refactor | alpha-impact-adjacent execution-quality | PA → E1 → E2 → E4 → QA → PM | 🟡 **DEPLOY DONE BUT RUNTIME ACTIVATOR BLOCKER 2026-05-18** | Worktree B + V094 + engine deploy DONE 2026-05-17 (`ea4ceca6` + `b867e452`), but post-deploy verification (4h `trading.fills` sample) shows **0% maker_attempt rate** due to cold-default `use_maker_close=false` + ZERO production callers for `set_use_maker_close_runtime` (E2 RCA `docs/CCAgentWorkSpace/E2/workspace/reports/2026-05-18--phase_1b_0_attempt_rca.md`). PA designing Option A TOML activator (H0Gate shadow_mode pattern); E1 IMPL ~40 LOC chain pending. Phase 2a 14d observation must reset post-fix deploy. AMD §3.1 wording patch required to make activation layer explicit. |
