...
| 11 | `W-AUDIT-4` ML 基座 + dead schema (mount W-F-1) | E1×6 並行 + MIT + E2 + E4 | ACTIVE 2026-05-09 (~30h, 3 sessions, after W-AUDIT-1) | V069 corrected observability cleanup source/test drops only empty `observability.scorer_predictions` with RESTRICT; V073 edge snapshot cycle wrapper/contract, V074 decision_outcomes live-lane backfill helper/wrapper/index guard, V075 corrected retention/compression, V076 Guard A retrofit, V077 F-29 archive CHECK, and F-08 ML training maintenance cron source/test added. None DB-applied; cron not installed. Remaining: V068/V070/V071 require code-reference-safe reclassification, V072 feature_baselines contract resolution, and F-09 FUP-2 deploy. Source work may proceed; DB apply/deploy remains separate authorization. |
...
| `P1-AUDIT-ML-4` | 3 | W-AUDIT-4 ML 基座 + dead schema (mounts W-F-1) | ACTIVE: V069 source/test corrects the observability cleanup to drop only empty `observability.scorer_predictions`; `model_performance` is retained because `canary_promoter.py` reads it, and `feature_baselines`/`drift_events` are retained for V072/drift-detector resolution. V073 source/test adds a V059 edge snapshot contract guard and a cron wrapper around `ref21_backfill_v058_v059.py --skip-instruments --skip-freeze-log --apply`; cron is not installed. V074 source/test adds live/live_demo `decision_outcomes` backfill helper + cron wrapper + pending-scan index guard; cron is not installed and migration is not DB-applied. V075 source/test corrects the "9 table retention" plan to 5 hypertables + 2 plain tables + 2 views. V076 Guard A retrofit and V077 F-29 bounded archive CHECK also source/test added. F-08 source/test adds `ml_training_maintenance_cron.sh` + runner covering `linucb_trainer`, `mlde_shadow_advisor`, `mlde_demo_applier`, `scorer_trainer`, and `quantile_trainer`; cron is not installed. None DB-applied. Remaining: V068/V070/V071 require code-reference-safe reclassification + V072 feature_baselines writer contract resolution + F-09 FUP-2 deploy. Heavy parallel; 3 sessions; depends on W-AUDIT-1. |
