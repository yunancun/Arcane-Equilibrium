...
**當前主線**：`P0-EDGE-1` Alpha-Edge 體制證據治理（§1）。候選 2 多日 trend = 🔴 **NO-GO-TREND**（關閉）→ 主路 = **listing fade**（Gate-B 探針已部署，待 24h 真捕捉）+ **funding/OI history backfill**（✅ code + `--apply` DONE，V125 history 已填 730d）。活躍工程佇列 §5；操作員行動 §6；排程 §7。**前進主線（2026-06-03 設計完成，待 IMPL scope）**：`AEG-S2` 證據自動化（MIT 設計，含 V002→V127 schema 衝突發現 + FND-2 critical-path）＋ `funding-tilt 診斷`（QC 協議，逃逸路②，cheap-first DATA TASK 先行）；詳 §5。
...
| 區域 | 當前狀態 | 下一步 |
|---|---|---|
| 來源同步 | **三端同步**（Mac=origin=Linux `trade-core`=`ab4d6e40`，本 session 親驗）。本 TODO cleanup commit 後再同步一次。 | 維持三端同步；工作樹三端 0-dirty。 |
| Runtime | Linux engine `b8c258b4` rebuild+restart（6/3 00:40，PID 2269678，watchdog engine/demo/live alive、ticks 流動，本 session 親驗）。**P5 step-i soak RUNNING**（flag-on `OPENCLAW_LEASE_PYTHON_IPC_ENABLED=1` operator-env + `--api-only` 2026-06-03，全 5 API workers flag=1 親驗）。funding/OI `--apply` DONE（V125 history funding 46539 + OI 348153 rows，accepted）。 | **P5 soak 監測**（2026-06-03 親驗：flag 真 ON〔worker env〕、`SM_DIVERGENCE` WARN=0〔注意：簽名是 `SM_DIVERGENCE` 不是 "divergence"〕、`lease_transitions` 24h +408k）。**⚠ soak gate 是 `divergences==0 AND total>=N`〔`governance_divergence.py:33`〕——只查 WARN=0 不充分**：comparator(`record_divergence`)只在 Python `GovernanceHub.acquire/release/get`(hub.py:933/1053/1179)內觸發，該 hub 主 caller 是 `executor_agent.py:554`(**shadow 默認**)非 Rust 權威熱路徑(那 408k 是 Rust event_consumer 寫)→ **若 Python shadow-hub 無 organic 流量,total≈0,「0 divergence」是空轉(silent no-op 偽 pass)**。**收口前置**：讀 `total` counter(auth'd POST `/api/v1/governance/health-check` → `lease_ipc_divergence.total`,API 綁 tailnet IP 非 localhost);若 total≈0,soak 需驅動 lease ops 過 Python 路徑或重審 gate 設計。**soak 期間避免全量 `restart_all`**(operator-env 會 revert)。QA A-1/A-2/B/A-4 done(re-check 6/10)。 |
...
| `AEG-S2-EVIDENCE-AUTOMATION` | 1 | 🟢 設計完成（MIT，2026-06-03）/ 待 IMPL scope | §2.4 路線圖下一步＝證據自動化 3 組件：(a) regime label runner (b) breadth ladder runner〔a/b 並行〕 (c) robustness matrix builder。設計 `…/MIT/…/2026-06-03--aeg_s2_evidence_automation_design.md`。**BLOCKING 發現**：regime labels **不可重用** V002 `market.regime_snapshots`（intraday/無版本 vs AEG daily-anchor/版本化衝突）→ 新建 `research.aeg_regime_labels` **V127**（建檔前重確 Linux `_sqlx_migrations` head）。(b)/(c) 為 artifact-only（S2 無新表，DB 表 defer S3+）。**critical-path 前置＝FND-2 PIT universe builder IMPL（契約完成但未 scope，PM 須先 scope）**。(a) 可在 V127 批准後即開（只依 live daily-kline 14505）。next：PM scope FND-2 IMPL + V127 migration design（MIT→E2/E4 dry-run→operator gate）→ (a)∥(b)→(c)。**並修**：Phase-1 `compute_rule_based_regime` data_loader.py:300 full-sample vol-tercile cross-section leak（MIT+QC 獨立交叉印證）不可繼承。 |
| `P0-EDGE-1-CAND-FUNDING-TILT-DIAGNOSTIC` | 2 | 🟡 cheap DATA TASK DONE（MIT，2026-06-03）→ **PROCEED-TO-HARNESS（conditional/narrow）** | 成本牆逃逸路②的 funding 維度。協議 `…/QC/…/2026-06-03--funding_tilt_carry_diagnostic_protocol.md`；cheap pre-check `…/MIT/…/2026-06-03--funding_tilt_data_task_precheck.md`（canonical run `18b3c2f8`）。**#0 覆蓋 PASS**（18/20 精確 2190 列/730d 均勻、零 gap、coverage 96.1%≥0.95、非近期密集）。**#1b 橫截面 tilt spread median 1.436bps/結算**（跨兩半段穩定 1.41/1.47）→ 7d/21結算=30bps gross < 42bps taker RT（net −12），只 ≥14d 或 maker 才轉正。**★ 決定性 caveat：~68% carry 來自 short-top leg（reliable +1.0bps）、long-bottom leg 弱（+0.47bps，40.5% 時間「最負 tertile」其實正 funding=長腿付費）→ bull 窗（72.4% 正 funding）下退化成「做空最擁擠多單 alt」=賣 short-squeeze 保險偽裝 carry（§4b NO-GO 分支）**。MIT 修正 QC 預判：死法非「量級不足」（marginal 非 dead）而是「短腿擠壓+price-side 侵蝕」，**需全 harness + klines 才能定論**（DB pass 給不了 price side），**進場預期 NO-GO/regime-bet 非 GO**。**next**：PM 決定是否 scope E1 建 harness（復用 trend 骨架；須分報 long/short leg funding_pnl + carry_share by leg、按腿標、interval 從間距推 TON/POL=4h、cohort 標 bull-heavy）。紅線：demo 無 spot lending（perp-only directional）/ cap SSOT=upperFundingRate / funding 雙面會計（§3.0）。 |
...
| v92 V### 對帳 | SQL head 仍為 V115；V125（AEG storage）+ V126（DB hygiene）已 apply 但編號保留 V116-124 規劃槽 | TW 可更新文檔註記，不觸碰已套用 SQL。 |
