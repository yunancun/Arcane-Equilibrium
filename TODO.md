...
| 行動 | 觸發 | 影響 |
|---|---|---|
| **V127 regime-labels migration apply** | E1 寫 + E2 PASS + E1/E2 雙 sandbox 冪等驗（`85bf8170` 就緒）；待 operator 批准真 apply | apply 到 live `trading_ai`（head 126→127），E4 跑正式 double-apply；解鎖 AEG-S2 (a) regime runner。AUTO_MIGRATE=0 故須明確批准（psql -f 或 rebuild+AUTO_MIGRATE=1）。 |
