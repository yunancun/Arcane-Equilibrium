...
**版本** v143 ｜ **日期** 2026-06-12 ｜ **來源實作 HEAD** main 本檔所在 HEAD（本輪 incident-policy `position_drift` notify-only producer coverage；runtime 詳見 §0/§6；版本敘事歸 `docs/CLAUDE_CHANGELOG.md`）。
...
| 區域 | 當前狀態 | 下一步 |
|---|---|---|
| 來源同步 | **三端同步** 本檔所在 main HEAD；本輪 P2 incident-policy dispatch trigger 已在 source 覆蓋 CORE ledger + auth invalid + Bybit fail-closed + `sm_halt_stuck` + `position_drift` notify-only producer；BB/E2 對 CORE+auth+Bybit path 0 blocker/high/medium，新 producer slices 尚待 review；focused Rust tests Mac+Linux source checkpoint 綠；P5-SM `[81]/[82]` selector fix checkpoint `bf673cdc` 已 Mac→GitHub→Linux `trade-core`，Gate-B watch cron 仍在 Linux `12,42 * * * *`。 | 維持三端同步；**Mac→GitHub 直連已恢復**（2026-06-10 fetch+push 親驗，bundle 流程退役）；Mac srv checkout 駐 main，本輪未 deploy/rebuild/restart。 |
...
| `P2-INCIDENT-POLICY-DISPATCH-TRIGGER` | 2 | 🟡 **CORE+auth+Bybit+sm_halt+position_drift source-live / BB+E2 reviewed prior CORE path / producer coverage partial** | 2026-06-12 `position_drift` notify-only producer slice 已接：新增 `position_reconciler/incident.rs`，在 successful reconcile cycle 分類後、orphan/ghost 處理後、baseline 更新前觀察剩餘 actionable drifts（MajorDrift/SideFlip/Orphan/Ghost；MinorDrift 忽略），沿用 `PERSISTENT_DRIFT_CYCLES=3` 與 `STARTUP_GRACE_MS=5min`，連續 3 cycle 未收斂才 feed `IncidentClass::PositionDrift`；該 class 在 incident_policy 預設 `NotifyOnly`，不餵 C4 AllFail。此前 `sm_halt_stuck` producer 也已接（`event_consumer/sm_halt_incident.rs`；`TickPipeline.halt_kind` + `halt_set_ts_ms`；5s cadence；halt 清除時 resolved）。既有 CORE/auth/Bybit/C4 path 保持：class ledger、sustained/throttle/7d cooling、push-secret gate、single armed owner、自癒 guard；`auth_invalid` / `bybit_fail_closed` producer source-live。Mac+Linux focused Rust：position incident 6 passed、position_reconciler 94 passed、incident_policy 15 passed；上一輪 `sm_halt` slice測試：sm_halt_incident 5、C4 wire 4、halt_ttl 20、ret_code_counter 6 passed。邊界：上一輪 BB/E2 review 只覆蓋 CORE+auth+Bybit path，新 `sm_halt` + `position_drift` slices 尚待 BB/E2/E4/QA/full-chain review；external `engine_dead` watchdog notify-only 仍未接；未 CI、未 deploy/rebuild/restart、無 DB/auth/risk/trading mutation。報告 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-06-12--incident_policy_position_drift_producer.md`、`docs/CCAgentWorkSpace/PM/workspace/reports/2026-06-12--incident_policy_sm_halt_stuck_producer.md`。下一步：接 external `engine_dead` watchdog notify-only，或先對新增 producer slices 做 BB/E2 focused re-review。 |
