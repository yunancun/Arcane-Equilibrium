...
- [ ] **CANARY-WRITER-ENV-RACE-1**（P4，2026-04-21 新增，ON-TICK-SPLIT-1 sub-agent flag）：`canary_writer::tests::spawn_honours_disable_dump` 並行跑時偶發 flake，與 AI-SERVICE-CLIENT-ENV-RACE-1 同型態 env-var race，應套同一 fix pattern（抽 pure fn + tests 不動 env）。工程量 ~30min。不阻塞，候補時清。
