...
- **GUI Bybit-first Demo PnL ✅ SOURCE-DONE / REVIEW-APPROVED / DEPLOY-SYNC-PENDING (2026-05-23, operator 1A2A3A)** — Q1 response-field-only no 24h cron this sprint；Q2 no `/demo/wallet-truth` this sprint；Q3 backend keeps 4 `strategy_source` values, GUI folds labels. Implemented `/api/v1/strategy/demo/closed-pnl` Bybit-first read model + read-only PG fallback/reconcile, in-process cache/in-flight dedup, cursor pagination beyond 500, `demo/live_demo` filter, Demo Profit tab exchange-confirmed closedPnl rendering, stale/source UI scoped to Profit, `restart_all.sh` engine.sock readiness gate. E2 RETURN×2 fixed; E3 security/read-only APPROVE; A3 UX APPROVE. Local focused tests PASS 50/50; full connector pytest 4171 PASS / 21 known-existing fail / 12 skip (static `tab-live.html` extraction drift + pre-existing stop-cancel reason mismatch).
