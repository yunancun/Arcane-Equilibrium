...
- [ ] **RUST-DOUBLE-PREFIX-1** — 2026-04-23 EDGE-DIAG-1 實驗揭露：Rust 端 phys_lock fill 的 `strategy_name` 實寫為 `"risk_close:risk_close:phys_lock_gate4_giveback"`（雙重 `risk_close:` 前綴），log 顯示 `reason=risk_close:phys_lock_gate4_giveback` + `strategy=risk_close:risk_close:phys_lock_gate4_giveback` → 某處 emit 時把已含 prefix 的 reason 再 format `"risk_close:{reason}"`。**COST EDGE + TRAILING 單前綴正常，只 phys_lock 路徑中招**。**根因已定位**：v2.rs Lock string `"phys_lock_gate4_giveback"` 不含前綴 → `risk_checks.rs:242` 加第 1 層 `format!("risk_close:{}", reason)` → `RiskAction::ClosePosition("risk_close:phys_lock_*")` → `step_6_risk_checks.rs:246` 對所有 RiskAction 統一加第 2 層 `format!("risk_close:{reason}")` → 雙重 prefix。COST EDGE/TRAILING reason 沒第 1 層 prefix 故只加一次。影響：healthcheck.py [4] 原 pattern `'risk_close:phys_lock_%'` 0 match 假 FAIL；已先 patch `'risk_close:%phys_lock_%'` 容錯（commit `21e3d5e`）。根治修法（二選一）：(A) `risk_checks.rs:242` 不加前綴，讓 step_6 統一加 — 簡單但需 verify 沒人下游靠 RiskAction reason 帶 prefix；(B) `step_6:246` 檢查 reason 是否已含 `risk_close:` 前綴，已含則不加 — 防禦性。修後把 healthcheck pattern 收回 single-prefix 形式。~0.5d 含回歸測試。
- [ ] **STRATEGIST-SCHED-CHANNEL-PAPER-ORPHAN-1** — 2026-04-23 EDGE-DIAG-1 RCA byproduct (commit `d9cecda` 修了第一層後浮出第二層)。**現象**：engine.log 每 5 min 噴 ~3 行 `WARN strategist_scheduler: fetch_current_params failed strategy=grid_trading|ma_crossover|funding_arb error=channel closed`，即便 strategy 是合法 entry name 也失敗。**根因**：`main.rs:884` `StrategistScheduler::new(ai_client, paper_cmd_tx.clone(), ...)` 接 paper engine 的 cmd channel；PAPER-DISABLE-1 (2026-04-16, `project_paper_pipeline_disabled_by_default.md`) 後 paper engine 預設不 spawn → `paper_cmd_tx` 對應 receiver drop → channel 永久 closed → scheduler 對任何 strategy 呼叫 `PipelineCommand::GetStrategyParams` 都失敗。**修法選項**：(A) `paper_cmd_tx` → `demo_cmd_tx`（簡單但 hardcode 假設 demo always alive；live 路徑無法調參）(B) 三引擎各 spawn 自己的 scheduler（per-engine 自治，與 3E-ARCH 對齊；要解決 AI service IPC 競爭）(C) scheduler cmd_tx 改成 multiplex 按 engine_mode 路由（最 robust，要新 enum + dispatcher）。**影響**：scheduler 完全 dead，自我調參能力 = 0；但不阻 Live（strategist 自我調參在 Phase 5+ 才啟用）；不阻 EDGE-DIAG-1。**附加價值**：fix 後真正失敗（AI service down 等）才能在 log 顯露，被前期 channel-closed spam 蓋住。~1-2d 視選方案。
