...
**版本** v132 ｜ **日期** 2026-06-12 ｜ **來源實作 HEAD** main 本檔所在 HEAD（本輪 AEG-S3 sidecar matrix wiring code checkpoint `66a9e511`；runtime 詳見 §0/§6；版本敘事歸 `docs/CLAUDE_CHANGELOG.md`）。
...
| 區域 | 當前狀態 | 下一步 |
|---|---|---|
| 來源同步 | **三端同步** 本檔所在 main HEAD；本輪 AEG-S3 sidecar matrix wiring code checkpoint `66a9e511` 已 Mac→GitHub→Linux `trade-core`，Linux artifact-only smoke 已跑；Gate-B watch cron 仍在 Linux `12,42 * * * *`。 | 維持三端同步；**Mac→GitHub 直連已恢復**（2026-06-10 fetch+push 親驗，bundle 流程退役）；Mac srv checkout 駐 main，本輪未 deploy/rebuild/restart。 |
...
| `AEG-S3-CANDIDATE-DIRECT-ROWS` | 1 | 🟡 sidecar matrix wiring 已接上 / 候選仍 non-promotable / Gate-B 改事件觸發 | 2026-06-11/12 已完成 artifact-only AEG-S3 direct rows infra、V125/V127 true panel export、funding_revive + oi_delta evidence/PBO/direct rows/metrics、event breadth adapter、empirical execution adapter、Gate-B 專用 watcher cron。`66a9e511` 新增 `aeg_s3_matrix_inputs --breadth-run-dir/--execution-realism-json`，formal matrix input 可直接引用既有 sidecar artifact；缺 sidecar 時仍 fail-closed placeholder，provided artifact mismatch 直接 fail-closed。Mac/Linux 本輪 sidecar focused regression **24 passed** + compileall OK + static forbidden-route search 無命中；Linux true smoke：`breadth_input_mode=provided_breadth_artifact` 消費 `aeg_s3_funding_revive_event_breadth_v125_20260611T200033Z_oos20260301_pbo18`，matrix `aeg_s3_funding_revive_robustness_sidecar_inputs_v125_20260611T200033Z_oos20260301_pbo18` row_count=24、coverage PASS、survivorship `pit_fnd2_delisted_proof`、execution `unverified_missing_missing`、final labels 16 insufficient / 8 kill。funding_revive 仍卡 `DSR=0.0`、PBO `0.54583333`、execution 未有 `>=30` matched empirical observations；oi_delta event breadth 仍按 basket 不支援 fail-closed，PBO `0.9125`。Gate-B watch latest `WATCH_ONLY`、23 candidates、0 alertable、0 start/schedule、唯一 live watch=`BPUSDT` old ContinuousTrading。PM reports：`2026-06-12--aeg_s3_sidecar_matrix_wiring.md` 及前序 AEG-S3 reports。**下一步**：主線 Gate-B 等 `[GATE-B-WATCH]` fresh Pre-Market/PreLaunch/conversion alert 或 latest artifact 變 `ACTIONABLE_*` 後啟動 isolated 24h probe；並行收集 `>=30` matched empirical execution observations 後以 `--execution-realism-json` 重跑 formal matrix。未經 E2/MIT/QC 獨立審前不得當 promotion proof。 |
