...
| `P0-ALPHA-SWITCH-8B-8C` | 🔵 ACTIVE | PM → QC/MIT/BB → PA → FA → PM | Switch alpha focus after A4-C RCA closure | `W-AUDIT-8b` proceeds to QC/MIT/BB review + Stage 0R replay design; `W-AUDIT-8a C1` 24h isolated proof is running; `W-AUDIT-8c` waits for C1 proof + MIT schema sign-off. |
...
| 2026-05-17..23 | Sprint N+1 ALPHA SURFACE PANEL WIRING | finish 8a C1 24h proof review + 8b review/replay design; A4-C diagnostic-only after no-revive RCA closure; Stage 1 demo only after future green Stage 0R (`[55]` source-cleared) |
