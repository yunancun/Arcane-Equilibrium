...
# 最後更新：2026-04-01（PA 實況檢查 + Wave 8 排程）
...
```
3475 passed / 21 failed / 17 errors（pre-existing failures 不影響本工作）
...
### [ ] A1：bybit_demo_sync.py 零測試覆蓋（E4 P1）
...
### [ ] A2：grafana_data_writer.py 零測試覆蓋（E4 P2）
...
### [ ] A4：CORS allow_credentials=True 安全加固（E3 HIGH，部分修復）
...
### [ ] A5：executor_agent.py 動態異常字串洩漏內部信息（FA P2）
...
### [ ] A6：symbol 欄位格式驗證不統一（E3 LOW，部分修復）
...
### [ ] A7：cost_tracker 方法名不一致（AI-E P2）
...
### [ ] A8：MessageBus send() lock 內同步執行 subscriber callback（CC 確認）
...
### [ ] B1：now_ms() 11 處定義 + 712 處 inline（E5 #17，最嚴重重複）
...
### [ ] B6：compile_state 每次讀寫 3x O(n) list scan（E5 #28）
- **檔案**：`app/state_compiler.py:528-552`
- **修復**：加入快取機制（dirty flag + memoize），寫入時標記 dirty
- **工時**：2h
- **E1 指派**：E1-Beta

### [ ] B7：trading.html 完全獨立 auth/API 實現（E5 #37）
...
### [ ] B8：:root CSS 變量 4 處重複定義（E5 #35）
- **修復**：統一至 styles.css，其他文件移除重複 :root
- **工時**：1h
- **E1 指派**：E1a-Alpha

### [ ] B9：console.html getToken() 重複 stub（E5 #36，部分修復）
- **修復**：刪除 deprecated stub，統一由 common.js 處理
- **工時**：30m
- **E1 指派**：E1a-Alpha

### [ ] B10：phase2_strategy_routes 30% 覆蓋率（E4 P2）
- **檔案**：`app/phase2_strategy_routes.py`（1625 LOC）
- **修復**：增加 40+ route 測試，覆蓋 error path + state transition
- **工時**：4h
- **E1 指派**：E4

### [ ] B11：WS 斷線 + stop-loss 交互未測試（E4 P1）
- **修復**：新建整合測試模擬 WS 斷線場景
- **工時**：3h
- **E1 指派**：E4

### [ ] B12：tab-governance innerHTML 審查（E3 MEDIUM，部分修復）
- **現狀**：ocEsc() 已廣泛使用（52+ 處），剩餘 innerHTML 為靜態字串
- **修復**：逐一確認剩餘 innerHTML 確實為靜態
- **工時**：1h
- **E1 指派**：E1a-Beta

### [ ] B13：Evolution→Deploy 自動化循環斷開（FA P2-FA-3）
...
### [ ] B14：_ollama_stats lazy init 無觀測性（FA FA-10）
- **檔案**：`app/layer2_cost_tracker.py`
- **修復**：初始化時 emit log + 暴露 /api/v1/ai/stats 端點
- **工時**：1h
- **E1 指派**：E1-Delta

### [ ] B15：ollama_client.is_available() 仍同步 1s（AI-E P2，部分修復）
...
### [ ] C1：strategist_agent.py 1068 行 God-class（AI-E P3）
- **修復**：拆為 strategist_core.py + h1_thought_gate.py + h4_validator.py + l2_evaluator.py
- **工時**：6h
- **E1 指派**：E1-Alpha

### [ ] C2：H3 ModelRouter 非獨立模組（FA P2-FA-5）
...
### [ ] D1：COMPREHENSIVE_SPEC_REQUIREMENTS.md 100% 重複（TW）
...
### [ ] D5：decisions/ .docx 索引不完整（R4，部分修復）
