...
| `P0-EDGE-1-CAND-FUNDING-TILT-DIAGNOSTIC` | 2 | 🟢 協議完成（QC，2026-06-03）/ 待 cheap-first DATA TASK | 成本牆逃逸路②的 funding 維度（trend NO-GO 後唯一未測逃逸象限）。協議 `…/QC/…/2026-06-03--funding_tilt_carry_diagnostic_protocol.md`（對標 trend 診斷 5 重嚴謹；信號＝cross-sectional funding-tilt long-short + time-series funding-extreme；K=8）。**QC 誠實預判：清成本牆 ~20-25%，最可能失敗＝carry 量級不足（median |F|≈IR floor → NO-GO-C）**。**next＝MIT 先跑 cheap DATA TASK #0+#1（canonical run 覆蓋驗證 + funding median |F| 量級），~1 天可能就 NO-GO-C，省下全 harness 建置**；過 cheap gate 才 E1 建 harness（復用 trend 骨架）。紅線：demo 無 spot lending（perp-only directional）/ cap SSOT=upperFundingRate 禁樣本窗 max / funding 雙面會計（§3.0 防雙重計入）。 |
