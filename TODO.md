...
| `P2-110017-D2-RECONCILE-QTY-GT-ZERO-DRIFT` | – | ✅ **SOURCE DONE 2026-05-29 commit `a5e1ded1` / DEPLOY BATCHED（per operator）** — D2 = position_reconciler `DriftVerdict::Ghost`（本地 mirror 有倉 / Bybit flat）週期收斂：新 `PipelineCommand::ConvergeExchangeZero` → 復用 D1 `converge_exchange_zero_close`（0 改動）+ audit→`engine_events`。**全鏈綠** PA→E1(r1+r2)→**BB(RETURN 1 CRITICAL→re-APPROVE)**→E2 APPROVE-WITH-CONDITIONS→E4 PASS(lib 3619/0)。**BB CRITICAL 攔截**：主 fetch `get_positions(None)` 無分頁（Bybit limit=20）→ 持倉>20 時 page2+ 真倉誤判 Ghost→誤刪真倉含 live money；E1 加 **S-6 單-symbol point-query gate**（`get_positions(Linear,Some(sym))` 不受截斷，確認 size==0 才收斂；StillHasPosition→keep+log pagination_false_ghost；QueryFailed→fail-closed）修畢。**runtime-live → deploy pending（operator 批次：等 C4/下個 Rust rebuild 一起 deploy）**。2 follow-up：`P2-RECONCILER-GET-POSITIONS-PAGINATION`（修法 B）+ audit removed_position 語意（MED observability）。ref D2 spec + BB report `2026-05-29--d2_reconcile_ghost_exchange_truth.md`。 |
