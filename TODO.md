...
Version: v16
Date: 2026-05-09
Status: PM sync after W-AUDIT-6 bb_breakout 5m RFC/IMPL source/test checkpoint; next W-AUDIT-6 work is VaR/CVaR/EVT
...
- **W-AUDIT-1..7 verification verdict**:
  ...
  - W-AUDIT-6 策略: ⚠️ source/test materially advanced (`bb_breakout`
    ...
    cleanup are closed; ma_crossover R:R trailing/TP source/test is closed;
    remaining immediate work is bb_breakout 5m RFC/IMPL, with VaR/CVaR/EVT
    queued as W-AUDIT-6c)
...
- **W-AUDIT-6 queue ordering after source/test cleanup**：QC stand-alone fixes
  through `bb_breakout` 5m are closed. Current active order is portfolio
  VaR/CVaR/EVT as W-AUDIT-6c. Keep the
  2026-05-16 `funding_arb` 14d audit as verification/history only, not
...
| 11 | `W-AUDIT-4` ML 基座 + dead schema (mount W-F-1) | E1×6 並行 + MIT + E2 + E4 | ACTIVE 2026-05-09 (~30h, 3 sessions, after W-AUDIT-1) | V068/V070/V071 source/test reclassify the dead-schema plan as metadata-only retention/review guards after code-reference audit; V069 corrected observability cleanup source/test drops only empty `observability.scorer_predictions` with RESTRICT; V072 contract guard source/test locks `feature_baselines` to Rust drift-detector 34-dim `features.online_latest` semantics and prevents accidental 17-dim `decision_features` seeding; V072 follow-through source/test adds a dry-run-default Rust writer that rebuilds baselines from `trading.decision_context_snapshots.indicators_snapshot + last_price` through the same 34-dim `FeatureSnapshot::to_feature_vector()` path; V073 edge snapshot cycle wrapper/contract, V074 decision_outcomes live-lane backfill helper/wrapper/index guard, V075 corrected retention/compression, V076 Guard A retrofit, V077 F-29 archive CHECK with Timescale columnstore trigger fallback, and F-08 ML training maintenance cron source/test added. V068-V076 were auto-applied during the 2026-05-09 authorized rebuild/restart; V077 initially hit Timescale columnstore CHECK limitation and was hotfixed before final engine restart. F-09 FUP-2 runtime is verified: Linux crontab has `edge_label_backfill_cron.sh`, logs show 30m demo/live_demo passes, and `[43]` PASSed. Remaining: separately authorized V072 writer apply/install if ops wants runtime `feature_baselines` rows. |
...
| `P1-AUDIT-STRATEGY-6` | 3 | W-AUDIT-6 策略 verdict + DSR/PBO promotion gate (mounts P0-EDGE-1) | UNBLOCKED by AMD-2026-05-09-02: bb_breakout cooldown 600k vs 300k source/test DONE (`DEFAULT_COOLDOWN_MS=300_000` shared by params default + constructor, runtime cooldown regression covered). Kelly tier 8/6/4 source/test DONE: `RiskConfig.kelly.{young,mature,established}_fraction` drives `compute_kelly_qty()` with defaults `1/8`, `1/6`, `1/4`; all risk TOMLs expose behavior-preserving defaults. fast_track 15%/5%+3σ source/test DONE: `RiskConfig.fast_track.{extreme_drop_pct,moderate_drop_pct,outlier_sigma_threshold}` drives Step 0 fast_track decisions, scoped-reduce classification, and sigma-scaled cooldown with defaults `15/5/3`. F-13 source/test DONE: `learning_engine/promotion_gate.py` composes DSR(K)+PBO/CSCV and `promotion_pipeline.py` blocks Demo→LivePending without a passing `demo_selection_bias_report`. per_trade_risk_pct SSOT source/test DONE: shared bounds `0.001..=0.20`, `KellyConfig::from_risk_config()`, replay runner, and risk hot-reload all derive from `RiskConfig.limits.per_trade_risk_pct`. funding_arb active RiskConfig cleanup DONE: four `risk_config*.toml` files no longer carry funding_arb per-strategy schema, while real `strategy_params_*` keep `funding_arb.active=false`; lib-test warnings are clean. ma_crossover R:R trailing/TP source/test DONE: `StrategyOverride.take_profit_enforced_override` enables MA-only TP enforcement and four risk TOMLs bind MA exits to SL 2.5% / TP 8.0% / trailing 0.6%+0.4%. bb_breakout 5m RFC/IMPL source/test DONE at `6d3ea046`: real 5m indicator snapshot, 5m kline bootstrap, `signal_timeframe`, demo-only 5m activation, no 1m fallback. Remaining QC heavy: VaR/CVaR/EVT as W-AUDIT-6c (3d later). 3 sessions; depends on W-AUDIT-1 + W-AUDIT-3. |
| `P1-AUDIT-AI-UX-7` | 3 | W-AUDIT-7 AI + GUI/UX 收口 | F-30 source/test/browser-smoke DONE: `common.js` now exposes shared `openPromptModal()`, learning experiment completion uses textarea + confidence select modal, governance audit/live-auth renewal/review flows use custom modal prompts and tier select pickers, and static guard prevents native `prompt()` from returning in those target files. F-system-mode-confirm source/test/browser-smoke DONE: `tab-system.html` `live_reserved` mode confirmation now disables the confirm button for 5s, rejects single-click confirmation after countdown, and submits only after a 1.2s hold-to-confirm. F-strategy-confirm source/test/browser-smoke DONE: `common.js` now defines shared action risk zones, `tab-strategy.html` visually separates Pause/Stop/Delete, `tab-paper.html` separates run/pause/stop/dual-stop and replaces `sessionStopAll()` native confirm with `openConfirmModal()`, and `tab-live.html` groups Stop/Emergency plus marks close-all/row-close actions as destructive with custom modal confirms. Remaining: F-07 operator GUI ANTHROPIC_API_KEY + Layer2 manual trigger 觀察 7d (operator 5min) + F-cea-env `OPENCLAW_COST_EDGE_ADVISOR=1` env + restart (ops 0.5h) + F-strategist-cap RiskConfig strategist max_param_delta_pct 30%→50% (E1 1h). W-AUDIT-7b: F-28 ContextDistiller IMPL (PA+E1 8h, 推遲到 LG-2 IMPL 之後). W-AUDIT-7c autonomous loop is sunset by ADR-0020; keep manual/supervisor trigger work only. 2 sessions urgent; can parallel with W-AUDIT-3..6. |
