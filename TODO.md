...
**最新狀態快照**（2026-04-29 20:21 CEST · post-MLDE follow-through deployed + W1-T1 scaffolding synced）：62-finding remediation Batch A-F 全部完成、push、Linux rebuild/redeploy；Items 1-6 follow-through commit `53bff07` 已推送、Linux fast-forward、release rebuild + restart，engine PID **620724** + API PID **620851**。本輪 follow-through commit `a3659d7` 已推送、Linux fast-forward、`restart_all.sh --rebuild --keep-auth` 成功，engine PID **691042** + API PID **691117**。PM 方案 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--ml_dream_edge_unblock_plan.md` 決定：**正 edge 是 promotion gate，不是 training gate**；已落地 MLDE Learning Data Contract、LinUCB post-fee reward 接線、ML shadow advisor、DreamEngine/OpportunityTracker producers、scheduler 接線與 healthcheck `[35]`/`[36]`/`[37]`。本輪 follow-through：`[37] mlde_demo_applier` 補 no-eligible no-op audit，手動跑一次 applier 後 [37] 已清；`[33] maker_fill_rate` 補 intent/order TIF/PostOnly audit persistence，post-deploy `postonly_order_rows` 已從 0 增至 6；`[12] bb_breakout` 已跑 5m 14d sweep，最佳 fwd6 raw 約 +8bps 但 t-stat 0.62、無顯著性，暫不切 timeframe；G8-01 W2/W3 測試已存在並綠；`VERIFY-IPC-TOKEN-EMPTY-SECRET` 補 fail-closed guard + test。MLDE gap fix `67b1160` 已推送並部署：demo-only Shadow/Dream min_samples 預設降為 3、live_demo/live 保持 5，且 LinUCB trainer 已回寫 `learning.linucb_state.cumulative_reward`；Linux 手動重訓後 `trending__ma_crossover` / `mean_reverting__grid_trading` 已出現非零 cumulative_reward。最新 runtime code commit 為 `45bbe4d`（W1-T1 schema attribution scaffolding + helpers split + [38] grid lifecycle dead-pairing fix）；最新 repo/doc sync commit 為 `e01a929`，Mac 與 Linux 已同步。Post-deploy healthcheck 剩 WARN `[12]` + `[33]` + `[11]`，SUMMARY WARN。Live 自動交易或 live 參數自動放權仍必須經 GovernanceHub / Decision Lease + 既有 5 live gates 批准。
...
- **~05-06**: GRID-LIFECYCLE-DRIFT 7d 觀察期 → [38] grid_trading_lifecycle_drift 結論（lifetime / fee burn / re-entry 三指標）；觸發 = GUI Learning tab 24h LiveDemo 157 vs Demo 63（2.5x），物理基礎 = `risk_config_live` trailing 2.0% / partial_tp=true vs demo 3.5% / partial_tp=false；MIT in-message audit 2026-04-29 設計（healthcheck 已落地 `checks_execution.py`）
...
| **GRID-LIFECYCLE-DRIFT** grid_trading lifecycle 漂移觀察 | 資料被動 | 2026-04-29 → 05-06（7d）| ⬜ 起算（MIT [38] healthcheck 落地）| [38] grid_trading_lifecycle_drift（lifetime ratio < 0.5x WARN / < 0.3x FAIL；fee burn > 0.8 abs WARN / > 1.5 FAIL；same-symbol re-entry > 0.5 WARN / > 0.7 FAIL）| 連 3 次 FAIL → grid live_demo 重評（fee 失控 vs 物理配置漂移）|
