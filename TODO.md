...
**Engine**（採集 2026-04-25 00:25 CEST · 多輪 restart + unhalt 嘗試後）：engine PID **1402148**（00:16 start 由 `restart_all.sh --rebuild`，binary 2026-04-25 00:16 含 G7-09）· HEAD `3eccfb5` · engine process **alive**（scanner / DB writers / WS subscribe 14 topics 均成功）· demo session via IPC `resume_paper {engine:demo}` 回 `session_state: active` ✅；paper session 同樣 resume OK · **但 pipeline_snapshot*.json 從 23:09（前 engine）起至今（25 分+）未更新任何一檔**（所有 per-engine snapshot 含 compat path 都停在 paper 21:58 / demo 23:09 / live 04-19）· **新 engine 從未 force_write 過 snapshot**，意味 tick_pipeline 即使 resume 後仍未收到 tick events（WS connected + subscribed 但 market data 未 flow to pipeline）· 這不是 news halt（新 engine 從未 log `guardian halt fired`），而是**更深的 tick 路徑問題**待 RCA · **G7-09 binary 已 live 但尚未觸發過 fill fee path**（無 tick = 無 fill）· 先前「crashloop #6/12h」RCA 部分正確但需擴：不只 news halt，還有 tick/snapshot 問題（同一 pattern 根源可能互相）
...
| **G6-FUP-TICK-PIPELINE-DEAD-1** | 🔴P0 | tick pipeline 未接收 WS ticks — **2026-04-25 00:25 CEST 發現**：`restart_all.sh --rebuild` 後 engine PID 1402148 alive、scanner 更新 symbol 集、public WS subscribed topics=14 batches=2、private WS Demo auth OK 訂 ["order","execution","position","wallet"]、所有 DB writer started；但 `pipeline_snapshot*.json`（demo/paper/live + compat）無一檔自新 engine 啟動後被 force_write。IPC `resume_paper {engine:demo}` 與 `resume_paper {engine:paper}` 都回 `session_state: active`，pipeline `session_halted: false` / `paper_paused: false`，邏輯上應該 tick；但實際 25 分 + 未寫任何 snapshot · **推測**：tick 事件未從 WS client 送達 tick_pipeline 通道，或 primary pipeline 指定失效（paper OPENCLAW_ENABLE_PAPER!=1 disabled，demo 是否成為 primary 待確認）· **範圍**：(a) 確認 which pipeline_kind `is_primary=true` 在此部署（paper disabled 時 demo 應接 primary flag）(b) 加 `tick_events_received_counter` debug log 定位 WS→pipeline 訊息斷點 (c) engine 啟動時若 market data 10s 內未流入 tick_pipeline → log WARN + 健康度降級 (d) healthcheck 加「snapshot_mtime 於 engine startup 30s 內 bump」判斷 · **阻塞**：blocks 所有 G7-09 real-world fill validation + G1-04 cutoff + G7-05 data accumulation · **同層**：與 G6-FUP-NEWS-HALT-DEDUP-1 可能互相糾纏，需一併 RCA | G6-FUP-NEWS-HALT-DEDUP-1 | E1+E2 / QA | 4-8h | engine fresh boot 後 30s 內至少一個 snapshot 被 force_write（與 session-active status 一致）|
