...
| `P2-RECONCILER-GET-POSITIONS-PAGINATION` | 3 | **NEW 2026-05-29 per BB（D2 修法 B）** — `position_manager::get_positions(Linear, None)` 主 fetch 單頁無 cursor 迴圈（Bybit default limit=20）+ `parse_position_list` 丟 `nextPageCursor` → 持倉>20 時 reconciler Orphan 偵測 + baseline 完整性盲區（漏報 page2+，**非誤刪**故非 ship-stop；D2 收斂已由 S-6 point-query 解耦此截斷）。修法 B = 加 nextPageCursor 分頁迴圈 + 顯式 limit=200。順帶字典 §get_positions 補 limit/cursor 警告。Owner: PA→E1→BB→E2→E4。 |
