...
| `P2-WP05-FUP-1` | 🟡 **PARTIAL 23/32 DONE 2026-05-18 commit `428f1505`**：23 sites（22 audit + 1 strategist_promote）跨 8 py file（edge_estimator_scheduler / layer2_routes / layer2_tools / live_session_routes / live_trust_routes / paper_trading_routes / strategist_promote_routes / strategy_ai_routes）`str(exc)` → 穩定 reason_code + logger.warning；response payload 不再洩漏 exception detail。**9 sites in risk_routes.py HIT signature blocker**：`_ipc_failure(detail: str)` 把 str 直接餵 HTTPException.detail，無 reason_code/log_detail split kwarg。> 3 sites 觸發 dispatch §「STOP and write a design memo」→ memo `docs/CCAgentWorkSpace/E1/workspace/reports/2026-05-18--p2_wp05_fup1_signature_blocker.md`（Option A signature 升級 / B caller inline / C P3 defer），E1 推薦 A，等 PA Round 2 決策。1 site live_session_routes:591 marker compare 保留（dispatch 明確分類為非 leak）。pytest 213/3 baseline 0 new fail。 |
