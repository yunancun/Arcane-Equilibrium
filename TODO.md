...
| `AEG-S3-CANDIDATE-DIRECT-ROWS` | 1 | 🟡 PM-local infra DONE / 真候選 rows 未產 | 2026-06-11 新增 artifact-only `research/aeg_s3_candidate_rows/`：explicit sample returns evidence JSON → top-level `candidate_regime_metrics` direct report；新增 `research/aeg_s3_panel_export/`：V125/V127 read-only storage → `oi_delta_panel.jsonl` + `funding_revive_panel.jsonl`（daily OI resample、funding settlement 保真、缺 price/regime fail-closed）；另新增 `research/aeg_s3_listing_fade/`：Gate-B run dir 或 V130 capture-events JSONL export → `listing_fade_candidate_evidence.json`（strict PASS_CAPTURE、explicit regime、explicit cost/k_trials；PBO 不捏造，缺 grid 下游 `missing_pbo`）；新增 `research/aeg_s3_oi_delta/`：offline OI/price panel JSONL → non-overlapping cross-sectional OI-delta rebalance samples（top/bottom tail long-short，explicit cost/net，保留 standalone 成本牆，PBO 不捏造）；再新增 `research/aeg_s3_funding_revive/`：offline funding/price panel JSONL → single-symbol funding stress unwind event windows（負 funding stress revive 做 long、正 funding stress unwind 做 short，explicit funding_pnl+cost/net；不是重開 funding-tilt/carry）。設計報告 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-06-11--aeg_s3_candidate_rows_design.md` + `2026-06-11--aeg_s3_listing_fade_evidence_design.md` + `2026-06-11--aeg_s3_oi_delta_evidence_design.md` + `2026-06-11--aeg_s3_funding_revive_evidence_design.md`；實作報告 `2026-06-11--aeg_s3_candidate_rows_impl.md` + `2026-06-11--aeg_s3_listing_fade_evidence_impl.md` + `2026-06-11--aeg_s3_oi_delta_evidence_impl.md` + `2026-06-11--aeg_s3_funding_revive_evidence_impl.md` + `2026-06-11--aeg_s3_panel_export_impl.md`；驗證 focused regression（panel export + funding_revive + listing fade + oi_delta + AEG-S3 rows + candidate metrics + robustness + Gate-B probe）= **81 passed** + compileall OK；static forbidden-route search 新模組無命中（測試檔僅自身 forbidden list 命中）。**下一步**：Linux 上用 accepted V125 run 跑 `aeg_s3_panel_export` 產真 OI/funding panels，再跑 oi_delta/funding_revive evidence；等/導入 operator-timed Gate-B 真 transition artifact；補 listing-fade/oi_delta/funding_revive candidate-grid PBO；最後串 `aeg_candidate_metrics` + `aeg_robustness_matrix`；未經 E2/MIT/QC 獨立審前不得當 promotion proof。 |
