...
| `P1-CRON-INSTALL-WAVE-1` | 2 | ✅ **SOURCE/TEST DONE 2026-05-18 commit `7bb994c3`** / ⏳ **crontab install operator-gated**：5 wrapper（panel_aggregator_health / wave9_replay_no_live_mutation_watch / replay_key_rotation_check / feature_baseline_writer / blocked_symbols_30d_unblock_check）加 `touch sentinel at start`；新 `checks_cron_heartbeat.py` 5 check [75..79] + `runner.py` register + 42/42 PASS；install recipe `docs/execution_plan/2026-05-18--p1_cron_install_wave_1_install_recipe.md` 提供 paste-safe one-liner，待 operator 在 trade-core 上 crontab -l 加 5 cron 條目。 |
...
| `P2-DEAD-SCHEMA-DROP-1` | ✅ **SOURCE/TEST DONE 2026-05-18 commit `428f1505`** / ⏳ **Linux PG apply operator-gated**：`sql/migrations/V096__drop_dead_learning_tables.sql` DROP `learning.rl_transitions` + `learning.symbol_clusters` RESTRICT + Guard A/B（非空 + pg_depend）+ idempotent DO $$ guard；deviation 從 dispatch 字面 CASCADE → RESTRICT 因 0 dependency grep + V069 sprint sibling pattern + 對 unexpected leftover dependent fail-loud 較安全。companion test `test_v096_drop_dead_learning_tables.py` 22/22 PASS。Linux apply 由 operator 觸發。 |
