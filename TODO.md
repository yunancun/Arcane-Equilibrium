...
**最後更新：2026-04-18**（**P0-7 ARCHIVED**：RCA 證偽獨立 order-submit gap，P0-6 方案 A 部署後 live_demo 1073 fills/24h、`order dispatched ↔ WS fill` 114:114 100% 落地；§P0-7 → 2 行 stub，完整條目併入 `docs/archive/2026-04-17--completed_todo_p0_scanner_phantom_live_guard.md` · **慢性虧損全週期 audit**：entry_context_id 配對 demo 24h → grid_trading net −$43.36 / ma_crossover −$11.90，fees 佔 grid gross loss 74%；bb_reversion 66 signals 0 fills，bb_breakout 0 signals；MICRO-PROFIT-FIX-1 narrow band 勝率實為 **99.39%**（491/494，非 100%）且為 sampling bias 非獨立 edge · 新增 **P1-9 ~ P1-12** 記錄成因鏈 · **P0 歸檔整理**：P0-5 / P0-7 / P0-8 / P0-9 / P0-10 五條完整內容移至歸檔，TODO 內僅保留 stub · **P0-6 INTENT-WRITE-GAP-1 永久修復 ✅ commit 293a808**（待 `--rebuild` 部署）· P1-7 LEARNING-PIPELINE-DORMANT-1 🟡）
...
**關鍵路徑**:`~~P0-0 reconciler burst fix~~ ✅ → ~~restart_all --rebuild 部署~~ ✅ → ~~P0-9 STABILITY-1 引擎崩潰 RCA~~ ✅（停電 infra 事件，非 code bug）→ ~~P0-11 LIVE-GATE-BINDING-1~~ ✅（2026-04-18，Python↔Rust 授權綁定）→ ~~P0-6 intent write 永久修復~~ ✅（commit 293a808，待 `--rebuild` 部署）→ ~~P0-7 order submit gap~~ ✅ archived（RCA 證偽，P0-6 子問題）→ P0-3 Phase 5 edge 2w 評估 + P0-2 LG-1 21d demo → **P1-7 LEARNING-PIPELINE-DORMANT-1** → LG-4/5 → Live`(P0-1 G-2 並行驗證 funding_arb 子集,不在主路徑;~~P0-5 PHANTOM-2-FUP~~ ✅ 待 `--rebuild` 部署即生效;~~P0-8 LIVE-GUARD-1~~ ✅ Rust 端 Mainnet 三重硬鎖回補)
...
1. Phase 0 可行性 sprint 任一紅 → 重整設計
2. Phase 1 Track P replay 證明 net edge < 0 → 不上線，改 Tier B（per-strategy native TP）
...
5. 任何階段 hold-out control edge 顯著 > ML-active → ML 存在 feedback bias，暫停訓練查資料洩漏
