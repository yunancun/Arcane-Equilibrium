...
Version: v21
Date: 2026-05-15
Status: PM freeze + AMD-2026-05-15-01 canary rebase applied. Step 3 runtime smoke + A4-C rebase completed 2026-05-15; Step 4 fill-lineage gate remains WARN_REAL_FILL_PROPAGATION_PARTIAL. Prior v19/v20 queue remains the historical base, but W3 Stage 1 paper cohort and A4-C D+12 paper-edge promotion are frozen; replacement path is Stage 0R replay preflight + Stage 1 demo micro-canary.
...
| 18 | `W-AUDIT-8d` A4-C BTC→Alt Lead-Lag 新策略 | alpha-bearing | PA spec → E1 IMPL + QC review | ✅ **DONE Sprint N+1 W1** (Spec v1.2+v1.3 D+0; IMPL `3d0ea347` btc_lead_lag producer + `58970d24` IPC slot + `31dba487` strategy consumers + `4b267dff` P0 stale fix + `1f0354cf` E2 review) / ✅ **v1.4 rebase DONE 2026-05-15** / **paper promotion FROZEN** | BTC 1m ≥1.5σ lead signal; 7-alt cohort; grid_trading/ma_crossover shadow log; bb_reversion CrossAsset filter。Legacy D+12 paper-edge promotion path frozen; spec + W2 report CLI now emit Stage 0R diagnostic `eligible_for_demo_canary=true/false` only, with Stage 1 demo as sole promotion gate. Spec: `docs/execution_plan/2026-05-10--a4c_btc_alt_lead_lag_spec.md`. |
