...
| ~~**AGENT-HEARTBEAT-SCOUT-WIRE**~~ | ✅ 完成 2026-04-30：`strategy_wiring_scanner._scan_and_produce_intel()` 在 no-opportunity 與 successful intel 兩條 completed scan path 都呼叫 `ScoutAgent.record_scan()`，讓 production ScoutWorker 30-min cycle 刷新 `_last_heartbeat_ms` / `scans_completed` / h_state invalidation。新增 `test_strategy_wiring_scanner.py` 2 個 hermetic tests（fake MarketScanner / ScoutWorker / ScoutAgent，無 thread / 無 exchange）。驗證：new pytest 2/0、`test_agent_heartbeat_contract.py` 36/0、targeted `py_compile` PASS。報告 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-30--dust_edge_scout_followthrough.md`。 | 完成 | ✅ | n/a |
