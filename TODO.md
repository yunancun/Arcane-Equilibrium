...
| ID | P | 狀態 | 下一步 |
|---|---:|---|---|
| `P5-SM-OPTION2-CONVERGENCE` | 1 | 🟡 step-i+E1b done / soak **ENDED-INVALID**（06-03+06-07 restart 兩殺）/ **監測重設計 DONE 2026-06-10** | **狀態校正（PA 2026-06-10 親驗代碼，v121 本 row 原文已過期）**：06-03 已落一輪 gate rework（`b904125f`→`87047e32`→`b847ae28`：V129 PG 投影+30s flusher+`[81]` P-LIVE healthcheck；operator 已拍板 (b)+(b-i)＝comparator 降為觀測信號非 gate；EQUIV sampler 建後同日棄；`governance_divergence.py:33` gate 公式=stale docstring）。**真缺口=Python→IPC→Rust 生產管線零 runtime 曝險**（P-LIVE 不經 IPC arm、4a 是離線 parity；shadow 默認 SHADOW_BYPASS〔hub.py:990-1001〕短路→organic 流量對 comparator 貢獻=0）。**重設計**（PA 報告 `docs/CCAgentWorkSpace/PA/workspace/reports/2026-06-10--p5sm_soak_observability_redesign.md`）：唯讀 IPC canary（300s probe `is_authorized`+`get_status`，leader-elected，零 mutation）+ epoch/flag 事件帳本（小 migration）+ `[82]` soak-window check + flag 持久化 SOP（`restart_all.sh:717` 機制已在，前兩次 soak 未用）。**新 gate=S1-S5**（S1 4a CI 綠＋S2 `[81]` P-LIVE＋S3 IPC canary 連續 48h/跨 epoch ≥500 probe/結構成功率 ≥99%/無 ≥15min 失敗連段＋S4 flag/epoch 完整性〔違反=soak invalid 自動重置〕＋S5 operator 收口 mutating smoke N≥10）正式取代「0 divergence + N ops」。**設計鐵則（雙邊比對死因）**：Rust per-pipeline 自動授權 vs Python hub-level 授權=兩個獨立狀態實例→雙邊 divergence 結構性不可達，canary 驅動亦永久卡死。**operator 已批 2026-06-10**;PM 時間參數定案(`docs/CCAgentWorkSpace/PM/workspace/reports/2026-06-10--p5sm_soak_cadence_decision.md`):cadence **300s→120s**(env 配置+jitter±10%,500-floor 餘量 1.15×→5.76×,gate 退化為純 48h 限定=最快路徑;15min 連段解析度 3→7-8 樣本)+五條 fire-機率硬防護(single-flight/2s timeout/不立即重試/連敗退頻/kill-switch 默認 OFF)+E4 對抗證明(1s 極端頻率下 tick SLA 不退步);48h 窗不縮(step-iii 單向 cutover 證據底線)。**E1 進度(2026-06-10,兩次中斷)**:Wave1(A+C)✅ commit `58ad4dba`(V137 soak 事件帳本+唯讀 IPC canary 120s/五條 fire-防護);Wave2(B+D)+1456 行 **WIP 快照** `feat/p5sm-soak-observability` 已 push origin(**未測試未審查,續作須先審+補測,勿直接信任**);Wave3(E)未始;**V137 Linux dry-run 未做**。🔴 **BLOCKER=Claude 月度花費上限**(第二個 E1 中途切斷;首個被 kill)→ 續作 gated on operator 提額(claude.ai/settings/usage)或月度重置。**續作序**:新 E1 接 worktree `/tmp/wt-p5sm-soak`(不在則從 origin branch 重建)→審 Wave2 WIP→補測+V137 dry-run→Wave3→E2→E4→CC→S-gate 落地→48h soak→step-ii→🔴step-iii CUTOVER(operator sign-off+CC/E2/BB/E4)。 |
...
| `P1-BB-REVERSION-REGIME-OBSERVABILITY` | 2 | QA 建議 / 待 PA scope | B fix 無正面證據面：`regime_snapshots` 0 rows + Hurst regime 未逐 intent 持久化。建議 fire 時持久化 Hurst label 進 `trading.intents.details`。評估：動 live intent 序列化路徑 → 需 PA→E1→E2→E4（非快修）。**2026-06-10 證據**：hurst key=0（all-time）、`regime_snapshots`=0、`research.aeg_regime_labels`=0（V127 表在未 populate）；bb_reversion 7d 12 intents＝持久化一落地樣本即流。 |
...
| **OPS-2 Phase-2 cutover deploy** | 鏈全綠;**merge 已完成**(`3018c7a3`,sync_land session;Linux 源碼已含 cutover) | 剩:①C-C 外部 alert 3 字串+親簽 ②`--rebuild` ③C-B 手動 `/auth/renew` 留證 ④§13.6 D+15-44 起算+首次 rotation 2026-09-08。**Linux regression(E4-owed)= ✅ 全綠**:Py 67f/4545p(0 cutover 相關失敗,4 點名檔 61/61)+Rust 43 targets **4153/0** EXIT=0 → **rebuild GO**(C-C 後即可按)。 |
