...
**版本** v135 ｜ **日期** 2026-06-12 ｜ **來源實作 HEAD** main 本檔所在 HEAD（本輪 AEG-S3 listing_fade PBO grid wiring code checkpoint `3d03698c`；runtime 詳見 §0/§6；版本敘事歸 `docs/CLAUDE_CHANGELOG.md`）。
...
| 區域 | 當前狀態 | 下一步 |
|---|---|---|
| 來源同步 | **三端同步** 本檔所在 main HEAD；本輪 AEG-S3 listing_fade PBO grid wiring code checkpoint `3d03698c` 已 Mac→GitHub→Linux `trade-core`，Linux artifact-only PBO smoke 已跑；Gate-B watch cron 仍在 Linux `12,42 * * * *`。 | 維持三端同步；**Mac→GitHub 直連已恢復**（2026-06-10 fetch+push 親驗，bundle 流程退役）；Mac srv checkout 駐 main，本輪未 deploy/rebuild/restart。 |
...
| `AEG-S3-CANDIDATE-DIRECT-ROWS` | 1 | 🟡 Gate-B chain + listing_fade PBO grid 已接上 / 候選仍 non-promotable / Gate-B 改事件觸發 | 2026-06-11/12 已完成 artifact-only AEG-S3 direct rows infra、V125/V127 true panel export、funding_revive + oi_delta evidence/PBO/direct rows/metrics、event breadth adapter、empirical execution adapter、Gate-B 專用 watcher cron、sidecar matrix wiring、Gate-B chain wrapper、listing_fade explicit PBO grid wiring。`66a9e511` 讓 formal matrix input 可用 `--breadth-run-dir/--execution-realism-json` 引用既有 sidecar artifact；`9eaad929` 新增 `research/aeg_s3_execution_observations/`，把 `listing_fade` candidate evidence + Gate-B run 轉成 matched `execution_observations.jsonl`；`75ed19c8` 新增 `research/aeg_s3_gate_b_chain/`，fresh Gate-B run 後可一鍵編排 listing evidence -> candidate rows -> candidate metrics -> execution observations -> event execution realism，若同時給 `--fnd2-run-dir --regime-run-dir` 再接 event breadth + formal robustness matrix；`3d03698c` 讓 listing_fade PBO candidate grid 明確 opt-in（`--include-default-pbo-grid` / `--pbo-grid-json`）並透過 Gate-B chain pass-through，默認仍誠實報 `not_produced_missing_candidate_grid`，不足 10 cells 報 `insufficient_candidate_grid`。Mac/Linux 本輪 focused regression **54 passed** + compileall OK；Mac `rg` / Linux `grep -R -E` forbidden-route search 無命中。Linux true PBO smoke `aeg_s3_gate_b_chain_listing_pbo_smoke_20260612` 用 old Gate-B run `listing_24h_20260602_1847`：listing_sample_count=2、execution_observation_count=2、listing_pbo_status=`produced_candidate_grid`、chain_status=`COMPLETE_EXECUTION_REALISM_FAIL`、reject=`sample_count_below_30`。結論：一鍵鏈 + PBO pass-through 已接通，但舊 run 不足 promotion，fresh Gate-B 仍需 `>=30` matched samples；Gate-B v0.1 execution 來源仍是 publicTrade prints，不宣稱 orderbook-depth fill realism。funding_revive 仍卡 `DSR=0.0`、PBO `0.54583333`、execution 未有 `>=30` matched empirical observations；oi_delta event breadth 仍按 basket 不支援 fail-closed，PBO `0.9125`。Gate-B watch latest `WATCH_ONLY`、23 candidates、0 alertable、0 start/schedule、唯一 live watch=`BPUSDT` old ContinuousTrading。PM reports：`2026-06-12--aeg_s3_listing_fade_pbo_grid_wiring.md`、`2026-06-12--aeg_s3_gate_b_chain_wrapper.md`、`2026-06-12--aeg_s3_execution_observations_producer.md`、`2026-06-12--aeg_s3_sidecar_matrix_wiring.md` 及前序 AEG-S3 reports。**下一步**：主線 Gate-B 等 `[GATE-B-WATCH]` fresh Pre-Market/PreLaunch/conversion alert 或 latest artifact 變 `ACTIONABLE_*` 後啟動 isolated 24h probe；跑 `aeg_s3_gate_b_chain.harness --include-default-pbo-grid`，產生 `>=30` matched observations 後審 execution realism + formal matrix。未經 E2/MIT/QC 獨立審前不得當 promotion proof。 |
