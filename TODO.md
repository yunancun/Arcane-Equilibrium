...
| `P1-ENGINE-BINARY-SPRINT-1A-IMPL-DEPLOY` | 2 | ⚠️ **FEATURE-LIVE / PROC-EXE-DRIFT 2026-05-25 PM recheck** — Linux HEAD `70cac13f`; running engine PID 320381 includes C10/Earn strings (`funding_harvest=6`, `EarnStake=2`) and demo watchdog alive, so previous C10/Earn binary-0 blocker is cleared. But the clean deploy claim is false: `/proc/320381/exe -> ...openclaw-engine (deleted)`, running SHA `b005bb...` != on-disk SHA `c88f82...`, process mtime 00:27:47 vs path mtime 00:29:08. API healthz is 200 on Tailscale bind `100.91.109.86:8000`; localhost 127.0.0.1 is not a valid bind check. **Next**: schedule safe no-live restart/rebuild window, then verify `/proc/$pid/exe` not deleted, proc/path SHA aligned or explainable, watchdog/API/PG still green. Do not call runtime deploy hygiene fully closed until this passes. |
