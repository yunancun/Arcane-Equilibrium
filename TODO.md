...
| ~~**3c**~~ | ✅ **FA-PHANTOM-2 fix**（2026-04-15 發現＋修復）— G-2 daemon 7h 0/20 fills 調查揭露第二類 fast_track false-positive：`max_drop_pct()` 掃全部 25+ 觀察幣種，任一小幣 5min 內抖 5% 就觸發 CloseAll，demo FA 8 次開倉 **4-7 秒內全被 risk_close:fast_track 秒殺**，0 次走 strategy_close 自然出口。修復 3 處：(1) `PriceHistoryTracker::worst_drop_for_held(held_symbols)` + sigma 新方法、(2) `evaluate_fast_track` 新簽名 `(level, held_drop_pct, held_drop_sigma, margin_util)` + 分級升級規則（15% 無條件 CloseAll、5%+3σ+Defensive+ CloseAll、5%+3σ+<Defensive ReduceToHalf、其他按舊風控梯度）、(3) `on_tick.rs` 改傳 held 信號 + WARN 攜帶診斷字段。Spec: `docs/references/2026-04-15--fa_phantom_2_fix_spec.md`。engine lib 1309→1318 (+9) · core 372→380 (+8)。deploy 透過 `restart_all.sh --rebuild` | done | — | G-2 daemon 解除 PHANTOM-2 阻塞 |
| **4** | 🧪 **G-2 FundingArb 驗證** — Step 4.1 ✅ 路徑全鏈路驗證（strategy_exit + ipc_close 都帶 realized_pnl 入 DB）。Step 4.3 ⏳ **後台 daemon PID 598572 監控中**（`/tmp/openclaw/g2_monitor.{py,log,pid,progress.json}`），FA-PHANTOM-2 修復前 7h 0/20 fills（全被 fast_track 秒殺），**修復部署後重新開始計時**。達 demo ≥20 strategy_exit fills 自動寫 `docs/audits/2026-04-15--g2_funding_arb_clean_edge.md`。**operator/Claude 接手先 `cat /tmp/openclaw/g2_monitor.progress.json`** | 修復後重評 ETA | ~~PHANTOM-2~~ ✅ | Phase 5 歸因量化確認 · LG-1 觀察期起點 |
...
- [ ] **EDGE-P3-1** 🧠 **Realized Edge Predictor** — shrunk_bps 退役 · 單筆單子 ex-ante edge 預測
  ...
  - **分工（Stage 0 Phase A COMPLETE，Phase B + #25 可並行）**：
    ...
    - **AI-E Phase B** (#27 後半)：
      ...
      - ✅ **#4 RNG seeding** — bootstrap wire `gate::seed_for_engine(startup_nanos, pipeline_kind) → IntentProcessor::predictor_rng`（`event_consumer/mod.rs` 與 7a/7c 同區塊）· 配套修復 `TickPipeline::with_kind` 未 forward kind 給 IntentProcessor 的潛伏 bug（原本 demo/live gate 仍以 Paper 視角跑 ε-greedy，只靠 writer R5 + DB CHECK 兜底）· 新 `TickPipeline::set_predictor_rng_seed` wrapper + `IntentProcessor::pipeline_kind()` accessor + `predictor_rng_lock_for_tests`（`#[cfg(test)]`）· +2 tests（`with_kind_forwards_kind` · `set_predictor_rng_seed_changes_draw_stream`）· lib 1307→1309 · 符合 spec §7.3 F9 完整語意
    ...
    - **AI-E Step 7 餘項**（IPC 全套，非 blocking，可獨立前推）：
      ...
      - 🟡 **Step 7b** `ReloadEdgePredictor{engine, strategy, path}` plumbing-only — `PipelineCommand::ReloadEdgePredictor` variant（`engine`+`strategy`+`path`+`response_tx`）· `edge_predictor::load_predictor_from_path` 存根（恆 `Err("onnx_loader_not_wired: awaiting ML-MIT #26")`，path 不存在先行早退 err shape 亦拱出）· `event_consumer::handlers::handle_reload_edge_predictor` 白名單驗證（paper/demo/live，trim 防 Python proxy 殘留換行）→ store 未接線 early-err → 呼叫存根 loader → 成功才 `store.swap()` · `handle_paper_command` match arm 透過 oneshot 回應 · capability flag `reload_edge_predictor` 保持 `False` 但註解改寫為「protocol wired / waiting for ML-MIT #26」· +4 tests（rejects_unknown_engine · requires_store · stub_loader_errs with 實存檔確保走完整路徑 · trims_engine_name）。lib 1303→1307。**Python route 暫不加**（flag 仍 False，無 client 會呼叫；待 #26 交付 ONNX artifact 後 Python route + flag flip + 實作替換 stub 一併落盤）。
      - ✅ **Step 7c** `EmitShadowFill` → `learning.decision_shadow_fills` writer — Option-B (Rust-direct writer + passthrough IPC，與 7a 對稱)：`ShadowFillMsg` struct (`database/mod.rs`)，+`side: i8` 於 `PipelineCommand::EmitShadowFill` variant 與 `ShadowFillPayload`（gate 建構時從 `FeatureVectorV1::side` 取值）；`database/shadow_fill_writer.rs` async drain+dedup+DB-RUN-6 epoch-0 reject+R5 `engine_mode=="paper"` 第二道防線+malformed JSONB warn+skip，`synthetic_*`+`close_tag` 欄位由 V017 DDL 預設（writer 不顯式 bind，避免 DDL 漂移）；`spawn_db_writers` 5→6 tuple；3 `EventConsumerDeps` sites (paper/demo/live) 皆注入（demo/live 用於深度防禦日誌）；`TickPipeline::set_shadow_fill_db_tx`+`shadow_fill_db_tx()`（`debug_assert!` 防雙注入）；handler 從 Stage-0 stub 轉為 `try_send` off hot path，Full/Closed 丟棄+warn；`engine_capabilities_routes._EDGE_P3_IPC_SUPPORT.emit_shadow_fill: False→True`。lib 1296→1303 (+7：dedup/epoch0/non-paper 拒絕/malformed JSONB/valid JSONB/i16 side/SQL 欄位鎖)，engine-capabilities 6 tests 繼續通過。
      ...
      - ✅ **Step 7e** `DisableEdgePredictorAll` 完整兩階段 commit + V014 audit — 骨架 commit `97777d5`（擴 `operator_token`/`reason` 欄位 + `persist_path()`/`risk_store()` getter + 標準入口函式）；本 step 填完 impl：Stage 1 `write_toml_atomic_fsynced(&next, persist_path)` disk-first fail-abort（寫盤失敗立即 reject，不改記憶體）→ Stage 2 `ConfigStore::apply_patch(Operator, mutate, validate)` ArcSwap（failures 由於 Stage 1 已落 authoritative 副本，重啟時磁碟為準）→ Stage 3 `EdgePredictorStore::clear_all()` 清 slot；`risk_store` 未接線時降級為 memory-only（cleared N slots (memory-only)）。V014 `predictor_disabled_all` audit fire-and-forget `tokio::spawn`（`audit_pool=None` skip，test 無需 tokio runtime）：`event_type='predictor_disabled_all'` / `source='operator'` / `config_name='risk_config'`，payload `{operator_token_hash(sha256), reason, cleared_slots, persisted, engine_mode, stage2_error}`；raw token 永不落盤。+3 tests：`test_handle_disable_edge_predictor_all_rejects_short_token`（<32 字符 → Err，slot 不觸動）· `test_handle_disable_edge_predictor_all_memory_only_when_store_unwired`（無 risk_store → memory-only 訊息 + slot 清空）· `test_handle_disable_edge_predictor_all_writes_toml_stage1`（wire `ConfigStore::new(...).with_toml_persist(tempdir)` + 種 use=true → 驗 TOML 含 `use_edge_predictor = false` + in-mem 翻 + slot 清空 + 訊息含 `persisted=false`）。lib 1293→1296 (+3)。
      - ✅ **Step 7f** `GET /api/v1/engine/capabilities` endpoint — backward-compat probe (EDGE-P3-1 §12.3 item 7)。新 `engine_capabilities_routes.py`（`/api/v1/engine/capabilities`）回傳三段：靜態 `feature_schema`（v1 / dim=17 / names 從 `parquet_etl.EDGE_P3_FEATURE_NAMES` 匯入避免鏡像漂移）· 本 build EDGE-P3-1 Step 7 IPC 支援宣告 `ipc_methods`（7a/7d/7e=true，7b/7c/SetEdgePredictorShadow=false，同 PR 落地時翻旗）· per-engine edge_predictor 窄視圖（use / shadow / quantile_k / require_q10_pos_adds / exploration_rate / fallback_on_error，經 `get_risk_config` IPC）。Fail-closed：IPC 不可用仍 200 + `degraded=true` + `reason=ipc_unavailable`，靜態部分永遠可用。Auth：`Depends(base.current_actor)` viewer 即可（純讀取探針）。新 `tests/test_engine_capabilities_routes.py` +6 tests（200 without IPC · degraded=true reason set · static schema/ipc_methods 保留 · happy-path per-engine routing · envelope shape · 401 without auth override）。main.py 註冊於 `live_trust_router` 下方。
