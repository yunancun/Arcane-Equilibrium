...
| **P1-INFRA-3p** | 🔄 **Sprint C in flight (2026-05-05 PA design accepted, advisory wave dispatch pending)** — accept PA push back 拆 C1+C2。**Sprint C1 (R6, ~6.5d)**：Fee/Execution Calibration — grid_trading + ma_crossover pilot；apply_fill 真 maker/taker fee + slippage model；Rust CalibrationLabelProducer (sample count + freshness + CI bound + regime)；Python simulated_fills_writer.py payload extension；experiment_registry.py execution_confidence write；R6-T7 順帶 unblock LG-3 healthcheck (RFC closure 0%→70%)。**R0-T0 拆檔 (Sprint B B1 模式)**：抽 `IsolatedPipeline::execute` → `replay/isolated_pipeline.rs`（新檔 ~400 LOC）+ 抽 `apply_fill` → `replay/apply_fill.rs`（新檔 ~250 LOC），runner.rs 1466→~750 內聚清理（非強迫 LOC，governance 已 2000 cap）。**Sprint C2 (R7, ~6d, C1 closed 後啟)**：MLDE/Dream Advisory Integration — dream_engine.persist_replay_candidate_recommendations + opportunity_tracker.persist_replay_outcome 升級 evidence_source_tier='calibrated_replay'（依 R6 deliver 的 execution_confidence label）；linucb caller 驗（grep 後決定）；mlde_demo_applier_evidence_filter Block B integration test；4 producer FK chain audit。**0 V### migration 需求**（V036 + V050 + V051 既 land）。**Sprint C acceptance**：A6 fee-aware PnL + A7 confidence honesty + A10 ML/Dream advisory boundary（hardcoded "real_outcome" 0 hit / paired CHECK enforce / TTL hard check enforce）。**Pre-DAG advisory wave**：QC（confidence label 數學 spec）+ MIT（capability risk assessment）並行 1d；C2 啟前 AI-E（advisory chain spec）1d。設計報告：`docs/CCAgentWorkSpace/PA/workspace/reports/2026-05-05--ref20_sprint_c_task_dag.md`。 | Sprint B R4+R5 acceptance ✅，advisory dispatch 已立 |
