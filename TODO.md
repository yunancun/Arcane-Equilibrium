...
**版本** v144 ｜ **日期** 2026-06-12 ｜ **來源實作 HEAD** main 本檔所在 HEAD（本輪 incident-policy external `engine_dead` watchdog notify-only producer coverage；runtime 詳見 §0/§6；版本敘事歸 `docs/CLAUDE_CHANGELOG.md`）。
...
| `P2-INCIDENT-POLICY-DISPATCH-TRIGGER` | 2 | 🟡 **planned producers source-live / BB+E2 reviewed prior CORE path / new producer review pending** | 2026-06-12 external `engine_dead` watchdog notify-only producer 已接：新增 `helper_scripts/canary/engine_dead_incident.py`，由 `engine_watchdog.on_engine_crash` 在 snapshot stale ≥30s 且 respawn failed ≥1（`consecutive_failures>=1`）後發 `ENGINE_DEAD_NOTIFY_ONLY` + 既有 engine-down alert；同 down episode 去重，`on_engine_recovery` 發 `ENGINE_DEAD_RESOLVED` 並清 marker；明確不餵 Rust C4 `AllFail`、不武裝 Defensive、無 auth/order/DB/risk/trading mutation，符合 PA §2.6 自指悖論決策。此前 `position_drift` notify-only producer 已接（`position_reconciler/incident.rs`，連續 3 reconcile cycle 未收斂才 feed `IncidentClass::PositionDrift`，class policy NotifyOnly），`sm_halt_stuck` producer 已接（`event_consumer/sm_halt_incident.rs`；`TickPipeline.halt_kind` + `halt_set_ts_ms`；halt 清除 resolved）。既有 CORE/auth/Bybit/C4 path 保持：class ledger、sustained/throttle/7d cooling、push-secret gate、single armed owner、自癒 guard；`auth_invalid` / `bybit_fail_closed` producer source-live。Mac focused tests：`py_compile` watchdog files OK；`test_canary.py` 87 passed + 9 subtests；`test_engine_watchdog.py` 40 passed；`test_watchdog_alert.py` 41 passed；`rustfmt --edition 2021 --check main_boot_tasks.rs` + `git diff --check` OK。Mac+Linux focused Rust 前序：position incident 6、position_reconciler 94、incident_policy 15；`sm_halt` slice：sm_halt_incident 5、C4 wire 4、halt_ttl 20、ret_code_counter 6 passed。邊界：上一輪 BB/E2 review 只覆蓋 CORE+auth+Bybit path；`sm_halt` + `position_drift` + external `engine_dead` slices 尚待 BB/E2/E4/QA/full-chain review；未 CI、未 deploy/rebuild/restart。報告 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-06-12--incident_policy_engine_dead_watchdog_producer.md`、`2026-06-12--incident_policy_position_drift_producer.md`、`2026-06-12--incident_policy_sm_halt_stuck_producer.md`。下一步：對新增 producer slices 做 BB/E2 focused re-review，通過後再派 E4/QA/full-chain；不需要再等 producer coverage。 |
