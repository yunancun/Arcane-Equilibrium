...
| 10 | `W-AUDIT-3` ExecutorAgent fake-live | alpha-neutral | E1 + E1a + E2 + E4 + PA + PM | 🟡 **SOURCE/SMOKE CLOSED** 2026-05-15; demo launch still waits on `[55]` PASS | F-17 ✅ / F-15 ⚠️ / SM-05 Option A / F-01 source/test closed. **`W-AUDIT-3b` Sprint N+1 W4 RouterLeaseGuard Drop test 已 land** (commit `22efd9de`) and runtime smoke passed on `trade-core`: RouterLeaseGuard Drop Rust PASS + fail-closed pytest PASS + `[55] chains_with_lease=89`. Remaining blocker is Step 4 `[55] WARN_REAL_FILL_PROPAGATION_PARTIAL` (24/138 real-fill reports). |
...
- ⚠️ **`[55]` fill-lineage WARN**: `chains_with_real_fill_report=24/138`;
...
| `P1-HEALTHCHECK-55-INVARIANT` | ⏳ demo-canary blocker | Redesign / clear [55] WARN gate as invariant test (QC S3) | Current 2026-05-15 `[55]`: `chains_with_real_fill_report=24/138` → `WARN_REAL_FILL_PROPAGATION_PARTIAL`; demo canary launch blocked until PASS or explicit PM/operator waiver. Future hardening: 改 ratio threshold → 1:1 mapping invariant (per trading.fills.fill_id → 1 real-fill ER) |
