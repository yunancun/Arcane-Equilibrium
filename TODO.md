...
- [ ] **STRATEGIST-PERSIST-TIE-BREAK-1（P2，FA H1，Phase 5+ 前置）** — V019 `applied_at_ms DESC` tie-break 在並發 writer（Strategist 自動 + manual_promote）同 ms 下非確定。DISTINCT ON tie 時 PG 用 physical row order 隨機挑。**修**：index `idx_strategist_applied_engine_strategy_ts` 末加 `, id DESC`，SQL 加 `, id DESC` tie-break；現 Demo 單 writer 0 觸發，**Phase 5+ STRATEGIST-PROMOTE-TRIGGER-1 上線前必修**（manual_promote 與自動 cycle 並發會活化）。~0.5h SQL + 測試。
...
- [ ] **STRATEGIST-PERSIST-TEST-BROADEN-1（P3，QC M2）** — 現 2 test 只覆蓋 `pool=None` 路徑。補 (a) SQL schema 不存在（relation does not exist）error path 測試 (b) `DISTINCT ON` + `ORDER BY applied_at_ms DESC` 多 row 排序正確性測試 (c) multi-row round-trip 測試。~0.5d。
