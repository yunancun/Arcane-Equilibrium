...
**最後更新**：2026-04-22（TRACK-P-V2-SWAP-1 完成 · 未部署）— 當日完成 `306993e` Priority 6 v1 linear → v2 non-linear + ExitConfig swap；runtime 待 `restart_all.sh --rebuild`（operator 指示先不部署）。2026-04-21 批次歸檔至 `docs/archive/2026-04-21--completed_todo_batch.md`（14 項，含 DECISION-OUTCOMES-* 三連、TRACK-P-T4-WIRING-1、DUAL-TRACK-EXIT-1 Phase 1b Track P v2、GATE1-REVERSAL-1 hotfix A、EDGE-P2-3 Phase 2+ (b) PostOnly 擴展、EXIT-FEATURES-SPLIT-1 / ON-TICK-SPLIT-1、3 個 env-race pure-fn 模板化、TICK-PIPELINE-MOD-UNUSED-IMPORTS-1、20:44 CEST `--rebuild` 部署）。
...
**測試基準線**：Rust engine lib **1835**（2026-04-22 TRACK-P-V2-SWAP-1 退役 8 個 v1 直測後；Mac debug + Linux release `cargo test -p openclaw_engine --lib` 均驗；v2 等值覆蓋在 `exit_features/v2.rs` 25 單測）/ bin 38 / core 392 / e2e 35 / reconciler_e2e 19 · Python **2866** passed + audit 4 + ml_training 238 · build warnings 13 → **10**（TICK-PIPELINE-MOD-UNUSED-IMPORTS-1 清 3）· **0 pre-existing fail**
...
- ✅ **TRACK-P-V2-SWAP-1** 2026-04-22 commit `306993e`：Priority 6 由 v1 `risk_checks::physical_micro_profit_lock` + `PhysLockConfig`（線性 giveback）切 v2 `exit_features::physical_micro_profit_lock_v2` + `ExitConfig`（非線性 `base - slope × peak_atr_norm`，floor 保底）；`RiskConfig.phys_lock: PhysLockConfig` → `RiskConfig.exit: ExitConfig`（`#[serde(alias = "phys_lock")]` 保 TOML 相容，當前三環境 TOML 無此 section 全走 Default）；ArcSwap 熱重載沿用既有 `Arc<ArcSwap<RiskConfig>>`；v1 pure fn + `PhysLockConfig` struct + 8 個 v1 直測整塊退役（v2 有等值 25 單測於 `exit_features/v2.rs`）。淨 LOC −260。Mac debug + Linux release `cargo test -p openclaw_engine --lib` 均 **1835 / 0 failed**（1843 − 8 退役 v1 直測）。Runtime 部署待 `restart_all.sh --rebuild`（operator 指示先不部署），部署前 engine PID 3954769 仍跑 v1 linear。報告 `.claude_reports/20260422_200623_track_p_v2_swap.md`。
