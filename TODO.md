...
**最新狀態快照**（2026-04-29 17:54 CEST · Items 1-6 local complete · Linux deploy pending）：62-finding remediation Batch A-F 全部完成、push、Linux rebuild/redeploy；Strategy Edge Repair commit `ece31b6` 已推送且 Linux repo 已 fast-forward pull，但 runtime 尚未 rebuild。本輪 items 1-6 已補完 fee-refresh/intents-freeze follow-through：fee freshness 改為 grouped demo/live binding task，demo/LiveDemo 已有 conservative cached fees 時不再因 demo fee endpoint 不支援而 cost_gate timestamp self-lock；report 見 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--items-1-6-completion.md`。新 PM 方案 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--ml_dream_edge_unblock_plan.md` 決定：**正 edge 是 promotion gate，不是 training gate**；Demo 可先用 ML/LinUCB/DreamEngine/OpportunityTracker 做 read-only、shadow、counterfactual、demo 小探索來修 edge；Live 自動交易或 live 參數自動放權必須經 GovernanceHub / Decision Lease + 既有 5 live gates 批准。
...
4. **Fee-refresh RCA follow-through** — `[22]` cleared after `bdd3177` deploy；items 1-6 follow-through 已修 demo only-stale 問題：fee refresh task 改 grouped binding，啟動需看到 `engine=demo env=Demo` + `engine=live env=LiveDemo` 註冊；下一個自然驗證點是首個 1h periodic refresh log 對每個 demo endpoint binding 都出現 `conservative defaults re-seeded`，且 >2h 不再出現 fee-rate staleness cost_gate self-lock。
