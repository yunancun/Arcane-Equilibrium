...
**版本** v146 ｜ **日期** 2026-06-12 ｜ **來源實作 HEAD** main 本檔所在 HEAD（本輪 incident-policy dispatch trigger E4 source-focused regression closure；runtime 詳見 §0/§6；版本敘事歸 `docs/CLAUDE_CHANGELOG.md`）。
...
| 區域 | 當前狀態 | 下一步 |
|---|---|---|
| 來源同步 | **三端同步** 本檔所在 main HEAD；本輪 P2 incident-policy dispatch trigger 已在 source 覆蓋 CORE ledger + auth invalid + Bybit fail-closed + `sm_halt_stuck` + `position_drift` notify-only + external `engine_dead` watchdog notify-only producers；BB/E2 focused review 通過（0 blocker/high/medium；E2 新 slices 0 low），E4 source-focused regression/full-chain review 通過（Mac+Linux focused Rust/Python matrix green）；QA acceptance pending；Linux `trade-core` source fast-forward 同步完成；P5-SM `[81]/[82]` selector fix checkpoint `bf673cdc` 已 Mac→GitHub→Linux `trade-core`，Gate-B watch cron 仍在 Linux `12,42 * * * *`。 | 維持三端同步；**Mac→GitHub 直連已恢復**（2026-06-10 fetch+push 親驗，bundle 流程退役）；Mac srv checkout 駐 main，本輪未 CI、未 deploy/rebuild/restart。 |
...
| `P2-INCIDENT-POLICY-DISPATCH-TRIGGER` | 2 | 🟡 **planned producers source-live / BB+E2+E4 source-focused passed / QA pending** | Source coverage now includes CORE ledger/C4 arm path, `auth_invalid`, Bybit business-retCode fail-closed, `sm_halt_stuck`, `position_drift` notify-only, and external watchdog `engine_dead` notify-only. Producer boundaries unchanged: `engine_dead` emits `ENGINE_DEAD_NOTIFY_ONLY`/`ENGINE_DEAD_RESOLVED` from watchdog only and never feeds Rust C4 `AllFail`; `position_drift` remains NotifyOnly after persistent unresolved reconciler drift; `sm_halt_stuck` observes `TickPipeline.halt_kind/halt_set_ts_ms` and uses existing incident_policy sustained/throttle/cooling. BB/E2 reports remain 0 blocker/high/medium and E2 0 low; E4 source-focused report `docs/CCAgentWorkSpace/E4/workspace/reports/2026-06-12--incident_policy_dispatch_trigger_e4_regression.md` is PASS_WITH_CONDITIONS. Mac matrix: incident_policy 15×2, C4 wire 4×2, sm_halt 5×2, position_drift 6×2, retcode 6×2, auth invalid 2+1, notification_failsafe 124, position_reconciler 94, halt_ttl 29, canary/watchdog py_compile + `test_canary` 87+9 subtests + `test_watchdog_alert` 41 + `test_engine_watchdog` 40. Linux source matrix: incident_policy 15, C4 wire 4, sm_halt 5, position_drift 6, retcode 6, auth invalid 2+1, targeted engine_dead canary 5. Boundary: no CI, no deploy/service rebuild/service restart, no DB/auth/risk/order/trading mutation; next step QA acceptance then PM closure. |
