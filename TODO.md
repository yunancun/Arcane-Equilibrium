...
**版本**：v88（v87 + 2026-05-31 M4 Stage 1 production runner main sync）
...
**日期**：2026-05-31
**HEAD**：Mac/origin/Linux source = `main` v88 sync（M4 code checkpoint `b25f9048` + TODO sync commit on top）；Linux runtime binary still from v87 deploy，engine PID `968350`，binary SHA `30adb40cc4dfbc9649c7b338a785932b30bd6c50cc5cfe28fd700a32e5a939a1`。**V115 applied**；V104 runtime state now `_sqlx_migrations version104=true` + `learning.supervised_live_audit` exists；AUTO_MIGRATE remains disabled at boot (normal posture).
...
| **v5.8 §4 業務 Sprint 2（Alpha Tournament）** | Alpha Tournament + M4 stage 1 + M10 Tier A + M8 read-only — per v5.8 §4 + SSOT `docs/execution_plan/2026-05-26--alpha_tournament_ssot_spec.md` | 🟡 **SSOT SPEC-FINAL / PARTIAL IMPL STARTED**（M4 Stage 1 runner source on main；未 deploy） |
...
**三端 sync**：2026-05-31 v88 current = Mac / origin/main / Linux trade-core all on `main` after M4 Stage 1 source sync（M4 code checkpoint `b25f9048` + TODO sync commit）；Linux runtime **not rebuilt** for this source-only sync, still running v87 binary PID `968350` / SHA `30adb40cc4dfbc9649c7b338a785932b30bd6c50cc5cfe28fd700a32e5a939a1`；API healthz 200；watchdog engine/demo/live fresh，paper archived/stale by design.
