# OpenClaw TODO — 工作清單（v6 · P0/P1/P2 三層 · 2026-05-04）

**版本**：v6（2026-05-04 REF-20 Gap Closure Plan V1 — Sprint A 啟動，label 從 "P6 PRODUCTION CLOSED" 改 "closed-with-known-gap (Sprint A in flight)"。新增 P1-INFRA-3n/3o/3p/3q 對應 Sprint A/B/C/D。Codex review 揭 4 P0/P1 gap 進入 forward stabilization）
**HEAD**: `a4ea3571`（Mac/Linux/origin 同步：plan 文件已 land）· **Engine deployed**: `dbcf845b`（Sprint 3 Track H + Track I Phase E restart_all `--rebuild` 已跑，Engine PID 4128610 + API PID 4128681 alive）
...
| **P1-INFRA-3** | ⚠️ **REF-20 Paper Replay Lab — closed-with-known-gap (Sprint A in flight)** (Sprint 4 closure commit `0ad79f67` + 2026-05-04 Codex review)。**Label 變更原因**：Sprint 4 P6 PRODUCTION CLOSED 後 Codex review 揭 4 個 P0/P1 gap → Gap Closure Plan V1 (commit `a4ea3571`) 切 Sprint A-D。**Sprint A scope** = R1 (runtime usability：binary path + /api/v1/replay/health + audit script) + R2 (manifest registry + /run FK guarantee + /manifest/verify production path) + R3 (first real E2E evidence on Linux)。**Sprint B-D pending**：B=R4+R5 (UI + decision/risk replay) / C=R6+R7 (calibration + MLDE/Dream advisory) / D=R8+R9 (maintenance + reality-calibrated sign-off)。**完成條件**：Wave R9 acceptance (≥5 successful runs / ≥1 parameter-change replay / ≥1 fee-aware report / 0 live mutation / UI usable / MLDE/Dream advisory only / confidence labels match calibration)。**仍適用 Sprint 4 base 接線**：Sprint 1+2+3 chain 三端同步；24/25 V3 §12 acceptance binding GREEN（schema/route/UI shell 仍是有效 foundation，但不能用作 strategy/risk 真實 evidence）。Plan 文件：`docs/execution_plan/2026-05-04--ref20_gap_closure_reality_backtest_plan_v1.md`。 | Sprint A→D 推進 |
...
| **P1-INFRA-3n** | 🟡 **Sprint A IN FLIGHT (2026-05-04)** — Gap Closure Plan V1 R1+R2+R3。**R1 Runtime Usability**：fix `route_helpers.py:138/143` binary path（`rust/openclaw_engine/target/...` → `rust/target/release/...`，加 fallback chain `OPENCLAW_REPLAY_RUNNER_BIN > workspace target > debug target > legacy nested fallback`）+ fix `helper_scripts/ci/replay_runner_symbol_audit.sh:91` 同 path bug + ensure API process 取得 `OPENCLAW_BASE_DIR`/`OPENCLAW_DATA_DIR` + add `/api/v1/replay/health` route。**R2 Manifest Registry**：production manifest registration path + atomic `experiments` row 確保 `/run` 不創 dangling FK + SQL/archive-backed `/manifest/verify` 不依賴 `OPENCLAW_REPLAY_VERIFY_TEST_KEY` + idempotency key。**R3 First Real E2E**：authenticated minimal replay on Linux + persist run_state/report_artifacts/simulated_fills + 4 個 SQL 查詢驗 row count > 0。**Sprint A acceptance**：A1 API spawn runner + A2 DB lineage exists + A3 no dangling FK。Plan: `docs/execution_plan/2026-05-04--ref20_gap_closure_reality_backtest_plan_v1.md` | E2 + E4 + QA + commit |
