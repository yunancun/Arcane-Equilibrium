...
**版本**：v78（v77 + 2026-05-28 PM session — Sprint 2 Wave 1 4-agent dispatch 完成 + **4 層 drift discovery** corrected + 真實 Sprint 2 殘餘工作收口）
**日期**：2026-05-28
**HEAD**：previous = `d18b54d0`（v77 doc sync）；本 v78 doc sync 為 branch tip pending；engine binary 仍 16:18 UTC rebuild 產物。
...
| `P1-PACKET-C-3WAY-DISPATCHER-WIRE-DISPATCH` | 1 | **NEW 2026-05-28 per PA C background agent spec land (v78)** — design spec at `docs/execution_plan/specs/2026-05-28--packet_c_3way_dispatcher_wire_spec.md` (33.5K)。**Q5 Sprint 2 scope 重新拍板候選**：(operator Q5 拍板) 路線 2 完整 wire C1+C2+C3+C4+C5 進 Sprint 2 ≈ 44-58 sub-agent hr / 16-22 wall clock hr / +14-23% Sprint 2 overhead；(PA C 推薦) hybrid C1+C2+C3 進 Sprint 2 / C4+C5 拉 Sprint 3 ≈ scope creep 降 22%→10%，pipeline_ctor wire + GUI banner 留 Sprint 3 Level 2 promotion 同步做。**10 operator 必答決策**（IMPL 不可繞）：Q1.1 Slack workspace + channel / Q1.2 Slack auth Webhook vs Bot Token（PA 推薦 Webhook 拒 Bot）/ Q1.3 訊息格式 plaintext vs blocks markdown / Q2.1 Email backend Gmail SMTP App Password（PA 推薦零外部成本）vs SendGrid/SES/postfix / Q2.2 cloud@ncyu.me 收/發確認 / Q3.1 Banner 持久化（PA 推薦「直到 operator ack 不 auto-clear」）/ Q3.2 GUI ack 是否要 V099-style typed confirm / Q3.3 Banner GUI 位置 tab-governance 頂部 vs 新 widget / Q4.1 Watcher 結構 single shared (PA 推薦) vs per-pipeline / Q5.1 Audit table `observability.notification_failsafe_events` (PA 推薦) / Q5.2 V114 migration 編號確認與 parallel sprint 無衝突 / Q5.3 `acked_at_utc` UPDATE trading_admin role grant 認可 / **Q6.1 scope hybrid vs 完整**。**5 commit 切片** C1(dispatchers+tests) / C2(V114+audit emitter) / C3(runtime providers) / C4(pipeline_ctor wire) / C5(GUI banner+ack)；C1/C2/C3 三 E1 真並行；C4 依 C1+C2+C3；C5 依 C4。**E1 IMPL** 25-34 sub-agent hr / 12-16 wall clock hr (3-4 sub-E1)；**E2 review** 6-8 hr；**E4 regression+integration** 8-10 hr；**QA** 3-4 hr；**PM** 2 hr；**總 44-58 sub-agent hr / 16-22 wall clock hr**。**Owner**: operator 拍板 → PM → 3-4 E1 並行 → E2 → E4 → QA → PM。**Trigger**: operator 回 10 Qs + scope decision 後派 IMPL chain。**Ref**: PA workspace report `docs/CCAgentWorkSpace/PA/workspace/reports/2026-05-28--packet_c_3way_dispatcher_wire_spec.md` + Operator mirror `docs/CCAgentWorkSpace/Operator/2026-05-28--packet_c_3way_dispatcher_wire_spec.md`。 |
