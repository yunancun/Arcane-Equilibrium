...
**日期**：2026-05-29
**HEAD**：Mac = origin = Linux `main` = `44990d13`（三端同步 2026-05-29 PM）；Linux engine binary **= batched deploy `26bbc241` SHA，PID 191366**（atomic rebuild+restart ~晚間，含 A failsafe-fix + C D2-reconcile + C4 wire + D-hygiene btc_lead_lag）。**部署後驗證**：healthz 200 / demo+live alive / 0 panic / **C4 fail-safe watcher dormant（0 escalation = 安全，incident-trigger 未接）** / 110017 loop 仍 fixed（TRXUSDT 0/60s）。**Source ahead**：B A1/A2 Stage 0R runner IMPL 在 worktree `wt-b-runner`（branch `feature/a1a2-stage0r-runner`）**uncommitted，pending QC(hard gate)→A3+E2→E4→commit**（A2 functional / A1 stub draft_only basis-blocked）。本輪（2026-05-29 PM 接手）= health-freeze + LG-3 ARMED + P2/P3 cleanup + P1-110017 D1 DEPLOYED + **4-track 加速衝刺全 deploy（A/C/C4/D committed+deployed；B runner IMPL done pending review）**，詳見 §-1。
...
- ✅ **Engine/API/watchdog alive（2026-05-29 晚間 batched deploy = CURRENT）** — engine **PID 191366**（`build_then_restart_atomic.sh` flock atomic，`/proc/exe SHA == post-build SHA == `26bbc241`...`，含 4-track Rust：A failsafe-fix `3423f0f7` + C D2-reconcile `a5e1ded1` + C4 wire `a8ba146c` + D-hygiene btc_lead_lag `af92e2ca`）；demo+live pipeline alive；`/api/v1/healthz` **200**；watchdog active + linger=yes；**0 panic / C4 fail-safe dormant（0 escalation，incident-trigger 未接）/ 110017 loop 仍 fixed（TRXUSDT 0/60s）/ reconciler live**。FA gap audit 2026-05-29 確認 deployed binary = 當前 main Rust（deploy 後僅 docs+B-runner Python commit，0 Rust drift）。**PID lineage**：27582（13:57 cold-audit）→ 113386（15:51 110017 D1）→ **191366（晚間 4-track batched）**。 |
...
| `P3-SESSION-CLEANUP-FA-AUDIT-FOLLOWUPS` | 4 | **NEW 2026-05-29 per FA session closure gap audit** — 3 個 LOW 結構債（FA 防孤兒蒸發）：(G1) `risk.rs` 822 行 >800 review 門檻（C4 推過閾值），須評估 split（按 escalate-handler vs 其他 risk handler 拆）；(G2) `main.rs` btc_lead_lag tracing var `btc_lead_lag_paper_enabled_env` near-dead（D-hygiene 修 source 後僅 log 用），順手清；(G3-source) `single_watcher.rs:78` 註解「無集中登記表」已 stale（§2.4 已登記），順手改。皆 touched-when-next-edit 級，不單獨 deploy。Owner: E1（觸及相關檔時順手）。 |
...
| `P2-BASIS-PANEL-INFRA` | 2 | **NEW 2026-05-29（A1 前置阻）** — A1 funding-short candidate 需 basis<0.3% entry gate，但 **PG 完全無 basis 資料源**（2026-05-29 ssh probe：`%basis%` table/column 0 hit；panel schema 只 btc_lead_lag/funding_rates/oi_delta）→ A1 cohort 邏輯不可建（dead code）。本 ticket = 建 basis data pipeline（perp price vs spot/index basis）+ `panel.basis_panel` + writer。**A1 Stage 0R runner cohort 待此 land 才可建**（當前 runner A1 = draft_only stub）。Owner: PA spec → E1 → MIT（schema/V###）→ E2 → E4。Trigger: Sprint 2 A1 真實評估前。 |
