...
| `P1-AUDIT-STRATEGY-6` | 3 | W-AUDIT-6 策略 verdict + DSR/PBO promotion gate (mounts P0-EDGE-1) | UNBLOCKED by AMD-2026-05-09-02: bb_breakout cooldown 600k vs 300k source/test DONE (`DEFAULT_COOLDOWN_MS=300_000` shared by params default + constructor, runtime cooldown regression covered). Kelly tier 8/6/4 source/test DONE: `RiskConfig.kelly.{young,mature,established}_fraction` drives `compute_kelly_qty()` with defaults `1/8`, `1/6`, `1/4`; all risk TOMLs expose behavior-preserving defaults. Remaining: F-13 `learning_engine/promotion_gate.py` DSR/PBO/CPCV 強制 promotion gate (QC+E1-a 8h, mounts LG-2 IMPL) + per_trade_risk_pct 雙 SSOT 統一 0.1% (kelly_sizer.rs:109) + fast_track 15%/5%+3σ → RiskConfig (E1-c 1h) + funding_arb active RiskConfig cleanup (E1-d+QC 1h). Plus QC heavy: bb_breakout 1m→5m RFC (4h spec + 1d IMPL) + ma_crossover R:R trailing/TP 重寫 (1d). VaR/CVaR/EVT 排 W-AUDIT-6c (3d 後期). 3 sessions; depends on W-AUDIT-1 + W-AUDIT-3. |
