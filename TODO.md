...
**版本** v131 ｜ **日期** 2026-06-12 ｜ **來源實作 HEAD** main 本檔所在 HEAD（本輪 Gate-B watch code checkpoint `3675f651`；runtime 詳見 §0/§6；版本敘事歸 `docs/CLAUDE_CHANGELOG.md`）。
...
| 區域 | 當前狀態 | 下一步 |
|---|---|---|
| 來源同步 | **三端同步** 本檔所在 main HEAD；本輪 Gate-B watch code checkpoint `3675f651` 已 Mac→GitHub→Linux `trade-core`，Linux cron 已安裝。 | 維持三端同步；**Mac→GitHub 直連已恢復**（2026-06-10 fetch+push 親驗，bundle 流程退役）；Mac srv checkout 駐 main，本輪未 deploy/rebuild/restart。 |
...
| `AEG-S3-CANDIDATE-DIRECT-ROWS` | 1 | 🟡 event breadth + empirical execution adapter 已接上 / formal matrices 仍 fail-closed / Gate-B 改事件觸發 | 2026-06-11/12 已完成 artifact-only AEG-S3 direct rows infra、V125/V127 true panel export、funding_revive + oi_delta evidence/PBO/direct rows/metrics；Mac focused regression 最新 = **88 passed**（event execution realism + event breadth + S3/matrix/breadth focused set）+ compileall OK + static forbidden-route search無命中；Linux event-execution smoke 4 passed、event-breadth smoke 3 passed。funding_revive PBO run `aeg_s3_funding_revive_v125_20260611T200033Z_oos20260301_pbo18`（18/18 grid，938 samples）→ metrics 5/5 PASS 但 all regimes `DSR=0.0`、PBO `0.54583333`；2026-06-12 `research/aeg_s3_matrix_inputs/`（`25ec85dd`）先用 candidate-specific placeholder matrix 解 smoke breadth lineage；同日 `research/aeg_s3_event_breadth/`（`8fed7073`）升級 funding_revive 到真 FND-2 PIT event breadth：`aeg_s3_funding_revive_event_breadth_v125_20260611T200033Z_oos20260301_pbo18` row_count=4，full_survivorship breadth=829/delisted=255/n_independent=261，survivorship healthcheck PASS，`oi_delta` real evidence 以 `unsupported_candidate_for_event_breadth:oi_delta` fail-closed；`research/aeg_s3_event_execution_realism/`（`c35f8425`）新增 matched observation JSONL → canonical `execution_realism.json` 接線，`listing_fade`/`funding_revive` 可用，`oi_delta` basket fail-closed，真 PASS 仍需 `>=30` matched empirical observations。formal matrix `aeg_s3_funding_revive_robustness_event_breadth_v125_20260611T200033Z_oos20260301_pbo18` row_count=24，coverage PASS、survivorship `pit_fnd2_delisted_proof`、execution `unverified_missing_missing`、final labels 16 insufficient / 8 kill。oi_delta PBO run `aeg_s3_oi_delta_v125_20260611T200033Z_oos20260301_pbo18`（18/18 grid，294 samples）→ metrics 3 PASS / 2 FAIL，all regimes `DSR=0.0`、PBO `0.9125`，formal matrix `aeg_s3_oi_delta_robustness_v125_20260611T200033Z_oos20260301_pbo18` final labels 3 insufficient / 2 kill / 1 stale-data。2026-06-12 Gate-B 專用 watcher `3675f651` 已接 Linux cron `12,42 * * * *`，latest artifact `WATCH_ONLY`、23 candidates、0 alertable、0 start/schedule、唯一 live watch=`BPUSDT` old ContinuousTrading；PM reports：`2026-06-11--aeg_s3_true_panel_first_candidate_rows.md`、`2026-06-12--aeg_s3_funding_revive_pbo_grid.md`、`2026-06-12--aeg_s3_oi_delta_pbo_grid.md`、`2026-06-12--aeg_s3_candidate_specific_matrix_inputs.md`、`2026-06-12--aeg_s3_event_breadth_funding_matrix.md`、`2026-06-12--aeg_s3_event_execution_realism_gate_b_watch.md`、`2026-06-12--gate_b_autonomous_watch_impl.md`。**下一步**：主線 Gate-B 等 `[GATE-B-WATCH]` fresh Pre-Market/PreLaunch/conversion alert 或 latest artifact 變 `ACTIONABLE_*` 後啟動 isolated 24h probe；並行收集 `>=30` matched empirical execution observations 後跑 execution-realism artifact。未經 E2/MIT/QC 獨立審前不得當 promotion proof。 |
...
| **S2 Gate-B 24h 真捕捉 run** | 探針已部署（R-0 zero-leak）；專用 `gate_b_watch` 已部署並安裝 Linux cron `12,42 * * * *`。2026-06-12 09:43 UTC formal run：latest artifact `/tmp/openclaw/gate_b_watch/gate_b_watch_latest.json` = `WATCH_ONLY`，23 candidates、0 alertable、0 start/schedule、唯一 live watch=`BPUSDT` old `ContinuousTrading`；報告 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-06-12--gate_b_autonomous_watch_impl.md`。 | 等 `[GATE-B-WATCH]` fresh Pre-Market/PreLaunch/standard-conversion alert 或 latest artifact 變 `ACTIONABLE_START_NOW` / `ACTIONABLE_SCHEDULE`，再手動跑 24h isolated capture 作 listing fade 主路第一證據；無 transition = `INCONCLUSIVE_NO_TRANSITION`，不可連 production WS/scanner/strategy/DB/order/auth。 |
