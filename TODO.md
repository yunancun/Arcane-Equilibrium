...
最後更新：2026-04-06（Session 11 · R2 完成）
...
- [ ] **PNL-1**（P0）qty=0 幽靈倉禁止開倉 — `intent_processor` 在 `final_qty==0` 時 reject（BTC/ETH $1k 餘額取整為 0）
- [ ] **PNL-2**（P0）H0Gate 從未觸發（`h0_checks=0`）— 排查為何代碼路徑未執行
- [ ] **PNL-3**（P1）引擎重啟冷卻期 — 重啟後等 N tick 建立 ATR/indicator 再開倉（當前立即開 5 倉）
- [ ] **PNL-4**（P1）regime 動態化 — 接入 Hurst/ADX 替代硬編碼 `"ranging"`
- [ ] **PNL-5**（P1）Cost Gate 小帳戶收緊 — k=1.5 對 $20 notional 幾乎不攔截
- [ ] **PNL-6**（P2）止損 RR 失衡 — 虧 $0.26~0.42 vs 賺 $0.01~0.06
- [ ] **PNL-7**（P2）`dynamic_stop` base=0.6/cap=0.8 → 提取為可配置參數

### 數據庫運行治理（Session 10 · 12hr 觀察 · DB 19 GB · signals 15.2M）

- [ ] **DB-RUN-1**（P0）signals 寫入降頻 — 352 行/秒 → 1 天 30M 行 / 12 GB；改為每 bar close 或 batch，預期降 95%+
- [ ] **DB-RUN-2**（P0）`decision_context_snapshots` 治理 — 5.3M/12hr = 13 GB；改為每 intent 或 30s/1min 快照
- [ ] **DB-RUN-3**（P1）`realized_pnl` 全為 0 — RRC-1 已修復，需確認引擎用新 binary
- [ ] **DB-RUN-4**（P1）`feature_writer` 無歷史積累 — 排查 history 表是否被寫入
- [ ] **DB-RUN-5**（P2）47/57 表空（agent/learning/observability/risk 全空）— 確認各 writer 啟動狀態
- [ ] **DB-RUN-6**（P2）5 條 `decision_context` ts=1970（epoch 0 遺留）清理
- [ ] **DB-RUN-7**（P3）signals hypertable 5.7 GB 但 pg_stat 顯示 40 KB — ANALYZE / compression 驗證
...
- [ ] **#5 drift_events writer** — producer 端確認
- [ ] **#6 quality_events writer** — producer 端確認

### 測試覆蓋

- [ ] **WP-E4/T-P1-1 殘餘** event_consumer 完整事件循環整合測試（fixture harness，獨立 sprint）
- [ ] **I-22 殘留** event_consumer mod.rs 785 → <700 進一步精簡（loop state 結構化，可選）
