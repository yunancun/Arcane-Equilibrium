...
**版本**：v84（v83 + 2026-05-29 cold-audit Wave3/Wave4/P3 全閉環 + 歸檔 + Linux rebuild/deploy）
...
| `P2-INCIDENT-POLICY-DISPATCH-TRIGGER` | 2 | **NEW 2026-05-29 Sprint 3 — C4 全 live 的最後一塊** — C4 wire 了 fail-safe 機制但 **incident trigger（真正觸發 3-way dispatch 的源）未接**：`FAILSAFE_FEED_SENDERS` OnceLock 留 `outcome_tx` seam 但 0 production producer → `outcome_rx` 永空 → timer 永不武裝 → escalate dormant。本 ticket = 接 incident policy（什麼事件/條件觸發 3-way notification dispatch）→ 餵 `observe_dispatch` → 武裝 timer → fail-safe 全 live。**接上後 BB mandatory re-review**（set_trading_stop 真觸發 + incident 頻率 + live slot respawn cmd_tx 不 stale）+ QA full incident E2E。Owner: PA spec → E1 → BB → E2 → E4 → QA。Trigger: Sprint 3。 |
