...
| **OPS-2 Phase-2 cutover deploy** | 鏈全綠;**merge 已完成**(`3018c7a3`,sync_land session;Linux 源碼已含 cutover) | 剩:①C-C 外部 alert 3 字串+親簽 ②`--rebuild` ③C-B 手動 `/auth/renew` 留證 ④§13.6 D+15-44 起算+首次 rotation 2026-09-08。**Linux regression(E4-owed)**:Py 全套 67f/4545p(vs Mac 66f=+1 env-lane 噪音,**0 cutover 相關失敗**,4 點名檔 61/61 綠);Rust 重跑中(首輪 cargo PATH 陷阱)。 |
