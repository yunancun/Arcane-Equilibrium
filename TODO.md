...
| `P1-INTENT-FREEZE-27` | 🟡 SOURCE/TEST CLOSED; RUNTIME PENDING 2026-05-15 | Full passive healthcheck hard FAIL `[27] intents_counter_freeze` | RCA found BTCUSDT exchange precision rounding (`final_qty <= 0`) after approved risk verdict, not a whole `trading_writer` outage. Source fix records this path as rejected qty=0 audit intent/verdict and defers Approved verdict persistence until a dispatchable `final_qty > 0`. Runtime deploy/rebuild and post-grace `[27]` PASS still required before canary/promotion-sensitive runtime action. |
