...
# 最後更新：2026-04-03（全路線圖定稿 · 代碼 72% 業務 52% 工時 31% · 關鍵路徑 20 週 102d）
...
```
3703 passed / 24 failed / 17 errors（pre-existing）
...
### [ ] 0A-1：學習反饋閉環（原 U-01 · FA P0-GAP-1）
- **檔案**：`app/strategist_agent.py`
- **修復**：`_apply_pattern_insight()` 接入 `_evaluate_signal()` 決策路徑
- **工時**：4h · **E1**：E1-Alpha

### [ ] 0A-2：進化參數自動重部署（原 U-02 · FA P0-GAP-2）
- **檔案**：`evolution_engine.py` + `strategy_auto_deployer.py`
- **修復**：EvolutionEngine best_params → Deployer；paper/demo 免確認
- **工時**：4h · **E1**：E1-Beta

### [ ] 0A-3：H0 Gate shadow 觀察（原 U-06 · FA P1-GAP-3）
- **修復**：shadow 記錄 would-have-blocked 但不攔截，觀察 1 週後切 blocking
- **工時**：1h · **E1**：E1-Gamma

### [ ] 0A-4：Scanner→Deployer 自動接通（原 U-07 · FA P1-GAP-5）
- **工時**：2h · **E1**：E1-Delta

### [ ] 0A-5：Backtest 生產環境啟用（原 U-08 · FA P1-GAP-6）
- **工時**：2h · **E1**：E1-Epsilon

### [ ] 0A-6：L2 觸發門檻降低 50→20（原 U-15 · FA P2-GAP-7）
- **工時**：1h · **E1**：E1-Gamma
...
### [ ] 0B-1：FundingRateArb 完整成本模型精算（原 U-10 · QC S3）
- **修復**：精算手續費+滑點+funding rate+basis risk+持倉天數
- **工時**：6h · **E1**：E1-Alpha · **依賴**：0A-5

### [ ] 0B-2：交易所條件單 SL/TP（原 U-11 · FA P1-GAP-4）
- **修復**：Bybit Demo 側掛 SL/TP 條件單（原則 9 雙重防線）
- **工時**：6h · **E1**：E1-Beta · **需 E3 安全審查**

### [ ] 0B-3：Kelly fraction + GUI + Agent 自動資本分配（原 U-14 · QC S4）
- **修復**：Kelly 在 tab-ai.html 顯示；Agent 根據 Kelly 分配資本
- **工時**：3h · **E1**：E1-Gamma
...
### [ ] 1-6：CognitiveModulator — L0 決策門檻調製（認知 SPEC §2）
- **新建**：`local_model_tools/cognitive_modulator.py`（~120 行）
- **數學**：[Q1] max 單因子 + [Q6] EMA α=0.3 + [R1-5] 連虧忽略負向
- **降級**：regret_data={}, dream_data={} 傳空 dict
- **工時**：0.5d · **SPEC**：認知自適應 V1.1+R1

### [ ] 1-7：OpportunityTracker — 虛擬 PnL 追蹤（認知 SPEC §3）
- **新建**：`local_model_tools/opportunity_tracker.py`（~250 行）
- **數學**：[Q2] 扣 2x fee + [Q3] 歸一化 + [R1-8] ≥5 樣本
- **工時**：1d · **SPEC**：同上

### [ ] 1-8：DreamEngine — 閒置蒙特卡洛模擬（認知 SPEC §4）
- **新建**：`local_model_tools/dream_engine.py`（~350 行）
- **數學**：[Q4] ≥30 輪/參數 + [Q5] binomial test
- **暫不接入** CognitiveModulator（Phase 2 啟用）
- **工時**：2d · **SPEC**：同上

### 並行組 C

### [ ] 1-9：LocalLLMClient 抽象 — Ollama + LM Studio 兼容（報告 §4.5）
- **工時**：0.5d

### [ ] 1-10：影子決策追蹤 — 四階段退出條件數據（報告 §2）
- **工時**：0.5d · **依賴**：1-2, 1-7
