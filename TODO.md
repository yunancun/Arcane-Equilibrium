...
**最後更新：2026-04-18**（**🎯 DUAL-TRACK-EXIT-1 設計定稿**：三輪對話後 operator 定向「物理層最優 + ML 持續優化並行」；Track P = 物理啟發式 7 維度（可解釋下界）；Track L = per-strategy ML exit policy（同特徵集監督學習上界）；Combine Layer 保證系統永遠 ≥ Track P。P1-9 ⛔ superseded；P1-4 併入 Phase 1 Track L Prereq C；P1-7 A/B/C 併入 Phase 1（1 週解阻塞）；P1-10 並行 W24。設計日誌 `docs/worklogs/2026-04-18--dual_track_exit_design.md`。**下一動作：W23 Step 0 可行性 sprint 3 天** · **P0-7 ARCHIVED**：RCA 證偽獨立 order-submit gap，P0-6 方案 A 部署後 live_demo 1073 fills/24h、`order dispatched ↔ WS fill` 114:114 100% 落地 · **慢性虧損全週期 audit**：grid_trading net −$43.36 / ma_crossover −$11.90，fees 佔 grid gross loss 74%；bb_reversion 66 signals 0 fills · **P0 歸檔**：P0-5/7/8/9/10 內容移至歸檔 · **P0-6 INTENT-WRITE-GAP-1 永久修復 ✅ commit 293a808**（待 `--rebuild` 部署）· P1-7 🟡）
...
- **初步指示**：gross edge 大概率仍負 → P0-3 判斷結果可能是「策略重做」路徑；推遲到 `DUAL-TRACK-EXIT-1` Track P 上線 + P1-10 結構修復後才重評，才能辨別「策略本身沒 edge」vs「退場規則把微利丟掉造成假負 edge」
...
**死循環結構**（Live_Demo 已解，Demo 殘留）：
- Live_Demo：`cost_gate_live` fail-closed → 0 fills → 0 edge data → `edge_estimates.json` 永遠是 `{}` → `cost_gate_live` 永遠 fail-closed ✅ 方案 A 解除
- Demo：correlated exposure 超限 → 0 new opens → 0 fills → bybit_sync seeded positions 無活躍策略 emit Close → exposure 不降 → 永遠超限（由 P1-8 FUP `retriage_synthetic_owner` + P1-6 BYBIT-SYNC-ORPHAN 負責接管，非 order-submit gap）
...
- [ ] **Demo 死循環打破**：P1-8 FUP `retriage_synthetic_owner` 已 tick-level 執行（Agent 自主），等一週觀察是否消化 6 個 bybit_sync orphan；若不消化再轉方案 B 臨時調 `correlated_exposure_max_pct` 或方案 C 由 P1-6 BYBIT-SYNC-ORPHAN 補 orphan close path
...
**阻塞**：Live gate（需真實 fill data → edge 估計 → cost gate 通過。方案 A 部署後 Live_Demo 死循環已解，order submit path 健康（§P0-7 歸檔驗證），但 Live+Mainnet 仍靠 edge 累積 → P1-7 LEARNING-PIPELINE-DORMANT-1 共同阻塞真 live）

### P0-7 · ORDER-SUBMIT-GAP-1 ✅ ARCHIVED 2026-04-18（已歸檔）
RCA 證偽原 hypothesis — 不存在獨立 order submit gap；P0-6 方案 A 部署後 live_demo 1073 fills / 24h、`order dispatched` ↔ `WS fill` 114:114 1:1 落地。Demo 死循環殘留併入 §P0-6「Demo 死循環打破」+ P1-8。詳見歸檔 `docs/archive/2026-04-17--completed_todo_p0_scanner_phantom_live_guard.md` §P0-7。
...
**關鍵路徑**:`~~P0-0 reconciler burst fix~~ ✅ → ~~restart_all --rebuild 部署~~ ✅ → ~~P0-9 STABILITY-1 引擎崩潰 RCA~~ ✅（停電 infra 事件，非 code bug）→ ~~P0-11 LIVE-GATE-BINDING-1~~ ✅（2026-04-18，Python↔Rust 授權綁定）→ ~~P0-12 LIVE-GATE-FALLBACK-1~~ ✅（2026-04-18，平倉 REST 降級）→ ~~P0-6 intent write 永久修復~~ ✅（commit 293a808，待 `--rebuild` 部署）→ ~~P0-7 order submit gap~~ ✅ archived（RCA 證偽，P0-6 子問題）→ **DUAL-TRACK-EXIT-1 Step 0 可行性 sprint → Phase 1 Track P + P1-7 A/B/C 並行**（W23）→ **P1-10 結構修復 + Phase 2 shadow**（W24）→ P0-3 Phase 5 edge 2w 評估 + P0-2 LG-1 21d demo → LG-4/5 → Live`(P0-1 G-2 並行驗證 funding_arb 子集,不在主路徑;~~P0-5 PHANTOM-2-FUP~~ ✅ 待 `--rebuild` 部署即生效;~~P0-8 LIVE-GUARD-1~~ ✅ Rust 端 Mainnet 三重硬鎖回補)
**最早 Live 日期**:**W24 末（～2026-05-23）** — 若 DUAL-TRACK-EXIT-1 Step 0 可行性 sprint 任一紅需重整設計，可能延後 1-2 週

### P0-12 · LIVE-GATE-FALLBACK-1 — 平倉路徑 REST 降級 ✅ 2026-04-18
**根因**：LIVE-GATE-BINDING-1 部署後，`authorization.json` 缺失時 Rust 不 spawn Live pipeline → `channels.live=None` → Python GUI 按「Stop Live Session」或「Close All Positions」→ IPC 回 `-32603: paper command channel not configured` → 多筆 error 回給 GUI。但交易所實際有 7 個 live orphan 持倉（LIVE-GATE-BINDING-1 部署前舊 session 遺留：ADAUSDT / ENAUSDT / FARTCOINUSDT / MOVRUSDT / ETHUSDT / IPUSDT 等），operator 無法透過 GUI 平掉。違反根原則 #6「失敗默認收縮」— 授權失效應拒絕開新倉但**仍必須允許平倉**。

**修復（純 Python，不需 `--rebuild`）**：`app/live_session_routes.py` 新增 REST-only reduce_only 降級路徑：
- `_CHANNEL_NOT_CONFIGURED_MARKER` + `_is_live_channel_unavailable_error()` 錯誤分類器（**嚴格**：只看 `"paper command channel not configured"` 字串）
- `_rest_close_position_reduce_only(rc, symbol, qty, is_long)` 直接 `BybitClient.place_order(Market, reduce_only=True)`（含 `round_qty` 對齊，失敗 fallback raw qty）
- `_sweep_live_orphan_positions()`：IPC channel-not-configured → REST 降級 + `rest_fallback/swept_via_ipc/swept_via_rest` 統計
- `post_live_close_all_positions` / `post_live_session_stop` / `post_live_close_position(symbol)`：三處 route 降級邏輯 + response 標記 `rest_fallback=true, reason="live_pipeline_not_authorized", source="rust_engine_with_rest_fallback"`

**觸發嚴格**：只認 channel-not-configured 字串；timeout / retCode / connection refused / stepSize 等錯誤**原樣暴露**不降級，避免遮蔽真實問題。

**測試**：Python `tests/test_live_gate_fallback.py` **11/11 passed** — 錯誤分類器（2）· REST 平倉 side/reduce_only/round_qty（4）· orphan sweep 降級/不降級/REST 失敗/空持倉/混合路徑（5）。既有 `test_live_authorization_signing` **10/10 無回歸**（合計 21/21）。

**部署**：純 Python → **不需 `--rebuild`**。直接 `bash helper_scripts/restart_all.sh` 重啟 uvicorn 即可。

**部署後驗證**：
1. GUI 按 Close All Positions → response 應含 `rest_fallback: true, errors: null`
2. 引擎 log 出現 `LIVE-GATE-FALLBACK-1: IPC close_all_positions channel unavailable ...` 和 `(REST fallback — live pipeline not authorized)`
3. `python3 -c "from openclaw_core import BybitClient; print([(p['symbol'], p.get('size')) for p in BybitClient(environment='live_demo').get_positions('linear') if float(p.get('size') or 0) > 0])"` 應為空列表

**工程日誌**：`docs/worklogs/2026-04-18--live_gate_fallback_1_implementation.md`

**不變量**：LIVE-GATE-BINDING-1 啟動門控不變（不會鬆開開倉路徑）· 降級**只平倉不開倉**（`reduce_only=True` 保障）· LiveDemo 與 Mainnet 同等對待 · `round_qty` 失敗不阻擋平倉 · IPC 其他錯誤原樣記錄。
...
### P1-4 · 在真 ETL 資料跑首個 ONNX export 🔗 併入 DUAL-TRACK-EXIT-1 Phase 1 Track L Prereq C
**狀態**：`learning.decision_features` 1.65M rows 已累積 ✅；P1-4 的「首個 ONNX」工作即 `DUAL-TRACK-EXIT-1` Phase 1 Track L **Prereq C**（`run_training_pipeline` 首跑 + 排程）。
**解鎖**：整個 EDGE-P3-1 Stage 2 shadow mode + Track L ML 退場 policy 起步。
**追蹤**：從此處退追蹤，由 DUAL-TRACK-EXIT-1 單點推進，見設計日誌 `docs/worklogs/2026-04-18--dual_track_exit_design.md`。
...
### P1-9 · MICRO-PROFIT-FIX-1 語意重構 ⛔ 2026-04-18 撤回（superseded by DUAL-TRACK-EXIT-1）
**撤回原因**：operator 二輪 push back 否決「降門檻抓微利」方向 — < 0.30% 微利會被 fee 吃掉 + P1-10 過度交易放大；三輪重新定義命題為「**放大單筆收益 + 風控最大化套現**」。由下方 `🎯 DUAL-TRACK-EXIT-1` 大章節承接（物理層 7 維度 + ML per-strategy exit policy 雙軌並行）。
**設計日誌**：`docs/worklogs/2026-04-18--dual_track_exit_design.md`
...
**下一步**：
  ...
  3. [ ] 與 `DUAL-TRACK-EXIT-1`（下方大章節）並行 — Track P 物理層上線後重新評估不對稱倍數；兩者修好後 P0-3 重評才乾淨
...
## 🎯 DUAL-TRACK-EXIT-1 — 物理最優 + ML 持續優化雙軌退場設計 🟢 NEW 2026-04-18

> **Supersedes P1-9**。三輪對話後 operator 定向：**「物理層最優 + ML 持續優化並行」**。Track P = 物理啟發式規則（7 維度、可解釋下界）；Track L = per-strategy ML exit policy（同特徵集的監督學習上界）；Combine Layer = 決策融合 + 安全網，保證系統永遠 ≥ Track P。
>
> **設計日誌**（完整論證、架構圖、Phase 規劃、QA 守衛）：`docs/worklogs/2026-04-18--dual_track_exit_design.md`

### 🚦 Step 0 — W23 可行性驗證 Sprint（Day 1-3，實質工作前必做）

**大前提**：以下 4 項不確定必須全部驗證，否則後續所有設計都是猜測。任一紅則 Phase 1 延後並重整設計。

- [ ] **不確定 1**：`james_stein_estimator.py` 實測能跑通嗎？
  - 行動：E1 執行 `python -m program_code.ml_training.james_stein_estimator` + 查是否寫入 `settings/edge_estimates.json`
  - 成功：產出 non-empty JSON + `test_james_stein.py`（若存在）綠
  - 失敗回退：先修 Python 端缺件；或降級為手工 backfill 並把 P1-7 B 列為獨立阻塞

- [ ] **不確定 2**：`decision_features` schema 與 Track P 7 維度對齊？
  - 行動：E1 讀 `program_code/ml_training/parquet_etl.py` + `decision_features` 表結構，對照 7 維度（`est_net_bps / peak_pnl_pct / atr_pct / giveback_atr_norm / time_since_peak_ms / price_roc_short / entry_age_secs`）
  - 特別：`time_since_peak_ms` 和 `price_roc_short` 若歷史資料缺，需先加 feature logging（backward-incompat，慎重）
  - 成功：至少 5/7 可直接 derive，剩下有明確 backfill 計劃
  - 失敗回退：Track P 暫用可得維度，Track L 訓練 feature 集縮減

- [ ] **不確定 3**：per-strategy 樣本平衡度
  - 行動：`SELECT strategy, COUNT(*) FROM learning.decision_features WHERE engine_mode IN ('demo','live_demo') GROUP BY strategy`
  - 成功：至少 2 策略 ≥10k rows
  - 失敗回退：單策略 PoC（grid_trading 起步），其餘樣本累積後再訓

- [ ] **不確定 4**：Counterfactual replay 可行？
  - 行動：查 tick-level price history 保留時長 + 粒度（`price_tracker.rs` snapshots、`/tmp/openclaw/` tick dumps、`trading.risk_verdicts` 時序）
  - 成功：可重放 7d demo 倉位 tick-level PnL 軌跡
  - 失敗回退：改「事後歸因 audit」— 對已關倉 positions 分析 peak-to-exit 軌跡，不做逐 tick replay

**Sprint 產出**：`docs/worklogs/2026-04-18-N--dual_track_exit_feasibility.md`（N=1/2/3 per day）→ 4 項全綠後才推進 Phase 1

---

### 📐 Phase 1（W23 Day 4-7）— Track P 實作 + P1-7 解阻塞 A+B+C（並行）

**並行軌道 1：Track P 物理層 7 維度規則**
- [ ] E1: 加 `peak_reached_ts_ms` 欄位到 `PaperPosition`（含 legacy migration）
- [ ] E1: `price_tracker` 加 `compute_roc(symbol, lookback_ms)`
- [ ] E1: 7 維度規則實作 in `risk_checks.rs`（Priority 6 位置替換現有 COST EDGE，重命名為 `PHYS-LOCK`）+ ConfigStore hot-reload 綁定
- [ ] E1: Combine Layer 骨架（Track L 缺失時等同 Track P-only）
- [ ] E2 + E4: counterfactual replay audit（demo 7d）+ ≥18 單測（含 spike-wick 不誤觸、長期 winner 不誤砍、波動率歸一化邊界、ConfigStore hot-reload、早期寬容、ML 缺席退化）
- [ ] E5: rebuild + 灰度部署（預設閾值保守，需 24h 無明顯 fee 惡化才逐步收緊）

**並行軌道 2：P1-7 解阻塞 A + B + C**
- [ ] **A** Rust 接 `trading.intents` 持久化（定位 DEDUP-PY-RUST Tier A stub 點 + 補 Rust 寫入 + 單測）
- [ ] **B** james_stein_estimator scheduler 啟用（每小時 cron + IPC hot-trigger + 寫 `edge_estimates.json`）
- [ ] **C** `run_training_pipeline.py` 首跑 grid_trading → 產 `models/demo/grid_trading_exit_policy_v20260425.onnx` + 排程化

**Phase 1 完成標準**：
1. Track P 灰度部署 ≥48h，`exit_source=Physical` 審計正常
2. `settings/edge_estimates.json` non-empty 且每小時刷新
3. 第一個 ONNX artifact 落地，Rust `ort` loader 可成功載入（推理暫不參與決策）
4. `trading.intents` live/live_demo 開始有 rows（P0-6 尾 gap 也順便收尾）

---

### 🔬 Phase 2（W24）— Track L shadow mode + P1-10 並行

- [ ] Combine Layer 啟用，`ml_override_high = 2.0`（不可達），只寫 `learning.decision_shadow_fills`
- [ ] 每日對比 Track P 決策 vs Track L 推理一致性（target ≥60% 同意率）→ 校準 `ml_confirm_threshold / ml_override_high / ml_veto_low` 初始值
- [ ] 每筆 `trading.fills` 寫入 `exit_source` 欄位（Physical / Hybrid-shadow / ML-shadow）
- [ ] **並行 P1-10** grid 過度交易 + ma_crossover R:R 不對稱修復（比 ML 重要 5 倍）

**Phase 2 完成標準**：shadow 一致性 ≥60% + P1-10 兩策略結構問題收斂（fee 佔比 <50%、不對稱倍數 ≤1.5×）

---

### 🚀 Phase 3（W25-W26）— Track L 灰度開啟

- [ ] `ml_override_high` 從 0.95 起步（極保守）→ 每 1-2 週降一階：0.95 → 0.85 → 0.75
- [ ] 每週 per-strategy 對比 P-only vs Hybrid vs ML-only 的 net edge
- [ ] 灰度進階條件：該階段 Hybrid net edge 顯著 > P-only（p-value < 0.1, n ≥ 200）才下調閾值
- [ ] Hold-out control group：5-10% 倉位永遠跑 P-only，作 ML feedback bias 檢測對照

**Phase 3 完成標準**：`ml_override_high = 0.75` 穩定運行 ≥1 週 + 累積 net edge 正向貢獻

---

### 🔁 Phase 4（W27+）— 持續優化常態

- [ ] 每週 retraining cron（最新 7-14d 數據）+ model registry 自動版本化 + canary deployment
- [ ] 每月 CPCV（Combinatorial Purged Cross-Validation）驗證 + drift 檢測
- [ ] 整合 **G-7 Teacher directive**（Phase 2B Strategist 工作 → 融入 feature 或約束）
- [ ] 整合 **Symbol Embedding**（若 Autoencoder 就緒）
- [ ] 整合 **Regime Detection LSTM**（若 DL 層就緒）

---

### 🛡️ QA 守衛（避免 ML 變賭博）

- [ ] **每策略樣本下限**：< 1000 rows 不訓練、永遠 Track P-only（bb_breakout 首當其衝）
- [ ] **CPCV 時序驗證**：嚴禁 random split
- [ ] **Hold-out control group**：5-10% 永不受 ML 影響
- [ ] **Calibration 監控**：每日 Brier score，超閾值自動降級
- [ ] **Feature drift 警報**：7 個 feature 分佈每日對 baseline，shift > 2σ 報警
- [ ] **Rollback 機制**：IPC 一條命令把 `ml_override_high = 2.0` → 等同卸載 Track L
- [ ] **ML feedback loop 隔離**：hold-out 永不受 ML 影響

---

### 🔗 與其他 TODO 的關係（去重複）

| TODO | 關係 | 本章節 vs 原 TODO 分工 |
|---|---|---|
| **P1-9 原案** | 完全取代 | 上方保留 1 行 stub，本章節承接 |
| **P1-4 首個 ONNX export** | 併入 Phase 1 Prereq C | P1-4 退追蹤，從此只在本章節推進 |
| **P1-7 LEARNING-PIPELINE-DORMANT-1 A+B+C** | 併入 Phase 1 | P1-7 保留 Teacher / LinUCB / Bayesian / RL 等大項；A+B+C 在本章節追蹤 |
| **P1-10 STRATEGY-ASYMMETRY-1** | W24 並行，比 ML 重要 | P1-10 保留獨立，本章節不動其工作項 |
| **P0-3 Phase 5 edge 2w 重評** | 推遲到 P1-10 + Track P 都上線後 | P0-3 判斷 timing 依本章節進度 |
| **G-7 Teacher** | Phase 4 整合 | G-7 正常排 W23，不阻本章節前三 Phase |
| **G-6 ML edge 噪音** | P1-7 B 解阻塞後自然解 | 歸檔或等 B 完成 |

---

### ⚠️ 風險與退路

1. **Step 0 可行性 sprint 任一紅** → 重整設計（此項為 TODO 層新增，worklog 無對應）
...
5. **Per-strategy ML 過擬合 grid_trading** → 每策略獨立訓練；小樣本策略（bb_breakout 0、bb_reversion 66/d）強制 Track P-only，不進 Track L 訓練池
6. **Counterfactual replay 完全失敗**（W23 不確定 4 紅）→ 轉向「事後歸因 audit」— 對已關倉 positions 分析 peak-to-exit 軌跡形態，不做逐 tick replay；設計風險上升、Phase 1 replay 驗收標準放寬
7. **hold-out control edge 顯著 > ML-active** → ML 存在 feedback bias / 資料洩漏，暫停訓練並查因；由 QA 守衛 #3/#7 hold-out 5-10% 常態監控
...
| W23 | 05-12~16 | **DUAL-TRACK-EXIT-1 Step 0 可行性 sprint + Phase 1**（Track P 實作 + P1-7 解阻塞 A/B/C）· P0-2 LG-1 21d demo 觀察起點 · G-7 Teacher · G-10 Calibration · LG-2/3 | ⬜ |
| W24 | 05-19~23 | **DUAL-TRACK-EXIT-1 Phase 2**（Track L shadow mode）· **P1-10 並行**（grid 頻率 + ma_crossover R:R）· LG-4/5 Live Gate · SEC-21 · QoL-2 | ⬜ |
| W25-W26 | 05-26~06-06 | **DUAL-TRACK-EXIT-1 Phase 3**（Track L 灰度）· EDGE-P3-1 產線化 · Phase 5 補強或重做（P0-3 judgment 後） | ⬜ |
| W27+ | 06-09+ | **DUAL-TRACK-EXIT-1 Phase 4**（retraining / Teacher / Embedding / Regime LSTM 整合）· G-1 R-06 全 5 agent | ⬜ |
...
| G-12 | 微利退場規則語意重構 → 🎯 DUAL-TRACK-EXIT-1（物理 7 維度 + ML per-strategy policy 雙軌） | W23 Step 0 可行性 + Phase 1 ~ W27+ Phase 4 | 🟢 NEW 2026-04-18 |
