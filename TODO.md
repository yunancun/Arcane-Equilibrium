...
- [ ] **~~ATTEMPT-LOG-NOT-DEAD-1 P3 doc-only~~ ❌ 推翻** — Linux 驗證後升級為 2 個 P1 fix（見下 3 新條目）。原 RCA `docs/worklogs/2026-04-21--decision_outcomes_rca.md` §七 amendment 記錄認知更正：writer 活躍 ✓ 但 NULL 不是 klines 稀疏（是 JOIN 斷鏈）+ engine_mode='paper' 100% 是 tagging bug（context_id 前綴混雜 demo/live_demo）+ `phys_lock_*` fills 全時段 0 導致 GATE1-REVERSAL-1 觀察計畫失效。情境 3 reframe 推翻，本條目標 `[ ]` 暫留引用，待下 3 新 P1/P2 結案後一併關閉。
- [ ] **DECISION-OUTCOMES-ENGINE-MODE-TAG-BUG-1**（P1，2026-04-21 Linux audit 完成 / 待 implement）：根因 = `outcome_backfiller.rs:94-107` INSERT 欄位列**省略 `engine_mode`**，schema `V015__engine_mode_separation.sql:64-68` DEFAULT='paper' 補上，與實際 context_id 前綴（demo 132k / live 89k / live_demo 44k）不符。對照組 `decision_context_snapshots.engine_mode` 正確（前綴與 stored_mode 對應），純粹 writer 漏接線。**Fix spec**：(1) 代碼：`outcome_backfiller.rs` L42-51 pending CTE `SELECT` 加 `engine_mode`、L91 outcomes CTE 沿用、L94-96 INSERT 欄位加 `, engine_mode`、L97-107 SELECT 加 `o.engine_mode`（總改動 ~5 行）；(2) 歷史 backfill：one-shot `UPDATE trading.decision_outcomes o SET engine_mode = s.engine_mode FROM decision_context_snapshots s WHERE o.context_id = s.context_id AND o.engine_mode = 'paper' AND s.engine_mode != 'paper'`（265k rows <30s，純列值，無 index 重建）。風險：極低。可與 OUTCOME-BACKFILL-JOIN-NULL-1 同 PR。
- [ ] **OUTCOME-BACKFILL-JOIN-NULL-1**（P1，2026-04-21 Linux audit 完成 / 待 implement）：根因 = **timeframe 字串格式不一致**。`outcome_backfiller.rs` LATERAL 子查詢用 Bybit API 原始 interval 碼 `'1'/'5'/'60'/'240'`，但 `market.klines.timeframe` 實際儲存 `'1m'/'5m'/'15m'/'1h'/'4h'`（writer 端做過正規化）。Linux CC side-by-side 手測：相同 context_id + ts，`timeframe='1m'` 回 2353.99 / `timeframe='1'` 回 NULL。損害面 266,800 rows × 7 個 outcome 欄位 = 100% NULL 全面。**Fix spec**：(1) 代碼：`outcome_backfiller.rs` L58/83/88 `'1'→'1m'` / L63 `'5'→'5m'` / L68 `'60'→'1h'` / L73/78 `'240'→'4h'`（共 5 string 替換）；(2) 歷史 backfill 方案 C 推薦：一次性 UPDATE SQL 用同樣 LATERAL 子查詢（繞過 5min loop，<10 min）；方案 A loop 需 4.6 天太慢 / 方案 B tmp BATCH_SIZE=10k 手動觸發 ~1h。風險：低-中（hypertable chunk-level lock 短暫）。LinUCB 訓練品質恢復前置。可與 ENGINE-MODE-TAG-BUG-1 同 PR。
- [ ] **GATE1-REVERSAL-OBSERVABILITY-1**（P2 → 重新分類為 **`[x] 關閉 with acknowledgement`** 2026-04-21 Linux audit 完成）：根因 = 假說 (X) 勝出 — **Priority 6 PHYS-LOCK 從未在生產觸發**，因為 `exit_features: Option<&ExitFeatures>` 永遠是 `None`。證據鏈：`tick_pipeline/on_tick.rs:1670-1679` 硬編碼 `|_| None`（T4 接線點）；`position_risk_evaluator.rs:153` docstring 明文「當前傳 `|_| None`」；engine log `grep phys_lock` 全時段 0 matches；歷史 fills 有 `risk_close:COST EDGE` 550 rows (2026-04-16 前舊管線) + `risk_close:fast_track` 1030 + 其他前綴 → wrapping 路徑結構完全 OK，若 PHYS-LOCK 被 fire 會寫 `risk_close:phys_lock_gate4_*`，實際 0 筆。**結論**：hotfix A (`d0f0c21`) 實際 runtime 行為變化 = 0（代碼對齊設計，但 Priority 6 inert 中）。**處置**：doc-only 關閉本 TODO（不加 Prometheus counter，upstream 仍 None 會永遠 0 無意義）；`phys_lock_*` 真正觀察等 TRACK-P-T4-WIRING-1 落地後自動生效。
- [ ] **TRACK-P-T4-WIRING-1**（**P1 主軸阻斷**，2026-04-21 新增 — Phase 1b 軌道 1 的 missing link）：DUAL-TRACK-EXIT-1 Track P 物理層完成了 v1 `risk_checks::physical_micro_profit_lock` (MICRO-PROFIT-FIX-1 2026-04-17) + v2 `exit_features::physical_micro_profit_lock_v2` pure fn (`aee96b9`) + Gate 1 Lock→Hold 反轉 (`d0f0c21`) + Priority 6 callsite (`risk_checks.rs:244-250`)，**但 T4 builder 從未接線** — `tick_pipeline/on_tick.rs:1677` 硬編碼 `|_| None`，Priority 6 inert。設計文件 `docs/worklogs/2026-04-18--dual_track_exit_design.md` §七 Phase 1 軌道 1 L271-276 遺漏「builder 實作」為獨立交付項。**修復**：替換 `|_| None` 為真實 `ExitFeatures` snapshot builder，從 `paper_state` + `price_tracker` + ATR state 讀取 7 維：`est_net_bps`（JS estimator 或 pnl_pct - 2×fee_rate）/ `peak_pnl_pct`（`best_price` 反算）/ `atr_pct`（`compute_atr_pct`）/ `giveback_atr_norm`（peak 相對現價除 atr）/ `time_since_peak_ms`（`peak_reached_ts_ms` 已在 2026-04-19 加）/ `price_roc_short`（`compute_roc` 已在 2026-04-19 加）/ `entry_age_secs`（`entry_ts_ms`）。估工程量 **~3 天**（~500 LOC builder + 20 integration tests + 灰度部署）。前置：無（所有 feature 源都就緒）。影響：開啟 Priority 6 → GATE1-REVERSAL-1 hotfix A + v2 閾值 + 未來 Track L ML 接線都真正開始產生 runtime 效果。**直到此 TODO 完成**：所有 Track P 工作（含本輪 3 commits）runtime 影響 = 0。
