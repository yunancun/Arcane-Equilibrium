...
- **2026-05-25 v64 TODO 消歧義 + Day -1 派工 + EA-4 P0-EDGE-1 AC amend**：(1) 「Sprint 2」一詞含義二義性 closure — 「支撐 Sprint 2 pre-readiness/M3 emitter」改名為「Sub-IMPL: M3 ...」明示與 v5.8 §4 業務 Sprint 2（Alpha Tournament + M4 + M10 + M8 read-only）區分；§-2 新增判讀規則 (2) E5 audit 揭露 H-1 FD 200 flock leak (CRITICAL，阻所有 atomic restart 此刻 active，PID 374287 持鎖) / H-2 13 cron 集體 disabled (HIGH，evidence half) / H-3 edge_estimates path mismatch (HIGH，治理 noise) + proc-exe drift 已 resolved (user prompt stale) (3) QC verdict update：operator 5/21 拍 (a) 後 5d 0% sweep 執行；維持 (a) 加 5 條 EA-1~EA-5；揭露 Sprint 2 Alpha Tournament vs P0-EDGE-1 雞蛋論 (4) **EA-4 PM 自做 amend P0-EDGE-1 AC-A**（5 textbook 達標 → OR ≥3 alpha-bearing 含 Sprint 2 新 source 達標 OR portfolio-level edge attribution）解雞蛋論 (5) 派 3 sub-agent Day -1 並行：E1 (H-1 + H-3 IMPL) / PA (H-2 cron restoration spec + M-4 sub-agent hygiene SOP + EA-3 funding_arb SL gate spec) (6) operator 手做：QC EA-2 edge_estimate_snapshots crontab 補裝 (2 min) + H-2 cron 復原 (operator action 等 PA spec)
