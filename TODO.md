...
**最新狀態快照**（2026-04-29 18:45 CEST · ML/Dream demo autonomy implemented）：62-finding remediation Batch A-F 全部完成、push、Linux rebuild/redeploy；Items 1-6 follow-through commit `53bff07` 已推送、Linux fast-forward、release rebuild + restart，engine PID **620724** + API PID **620851**。fee freshness 改為 grouped demo/live binding task，啟動日誌已確認 `engine=demo env=Demo` + `engine=live env=LiveDemo`；post-restart risk verdicts 不再有 fee-stale self-lock，[27] PASS。`[34] intent_signal_attribution` 仍因 30min rolling window 內 pre-restart 空 signal_id 舊 rows 報 FAIL；post-restart intents 已確認 `empty_signal_id=0`，等窗口自然清。report 見 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--items-1-6-completion.md`。PM 方案 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--ml_dream_edge_unblock_plan.md` 決定：**正 edge 是 promotion gate，不是 training gate**；已落地 MLDE Learning Data Contract、LinUCB post-fee reward 接線、ML shadow advisor、DreamEngine/OpportunityTracker producers、scheduler 接線與 healthcheck `[35]`/`[36]`。本輪新增 V032 `learning.mlde_param_applications` + demo-only bounded `mlde_demo_applier`：ML/Dream/其他 ML 可在 demo 用 `get_param_ranges`/`patch_risk_config(engine=demo)` 自主調 strategy params、risk params、leverage 等可調參數；strong demo evidence 只產生 live `experiment_plan` candidate。Live 自動交易或 live 參數自動放權仍必須經 GovernanceHub / Decision Lease + 既有 5 live gates 批准。Completion report 見 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--ml_dream_edge_unblock_completion.md`；demo autonomy report 見 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--mlde_demo_autonomous_applier.md`。
