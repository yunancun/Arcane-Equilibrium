...
**版本**：v77（v76 + 2026-05-28 PM session — Sprint 2 grill-me + PA cross-verify 雙路收口 + 5 lock decision + hybrid 方案 C ratified）
**日期**：2026-05-28
**HEAD**：previous = `066167c8`（v76 doc sync）；本 v77 doc sync 為 branch tip pending；engine binary 仍 16:18 UTC `restart_all --rebuild --keep-auth` 產物（含 notification_failsafe 編譯產物 dead-code module）。
**Session (v75)**：parallel session — runbook v1.0 4-patch + 14d soak D+1 0 fallback log + 3 P2/P3 carry-over（保留）。
**Session (v76)**：本 session 上半 — Wave 5 Packet C SOURCE LAND + 三端同步 + Linux rebuild + A 級 ssh sweep + operator menu [1]-[5]。
**Session (v77)**：本 session 下半 — **Sprint 2 entry checklist 雙路驗證收口**：
  - **grill-me 5 lock decision**（operator 拍板）：Q1=改名 Sprint 2 為 Stage 0R Replay Preflight Sprint（廢 Tournament 業務 framing 但**保留 Tournament activation spec future use** per Q2 雙軌）/ Q2=N=5 strategy pool 入口 + M=15 per-strategy n / Q3=平行收 + Stage 0R green 只 evidence accept + demo canary 另開 sprint / Q4→C hybrid（A1+A2 主投 + grid+ma 對照組 + 30 天 catch-up bb_breakout+bb_reversion）/ Q5=路線 2 Packet C 完整 wire 進 Sprint 2 範圍。
  - **PA agent 獨立 verdict**：CONDITIONAL GO 15 entry gate（6 Reject / 10 Ratify / 5 Conditional）；A1 funding_short_v2 + A2 liquidation_cascade_fade 為主 IMPL slot；5 textbook 全 B0 baseline 不 IMPL 新 candidate。
  - **Cross-verify 結果**：4 共識可調和 + 1 重大分歧（Q4 策略選擇）→ operator 拍 hybrid 方案 C 解決。
  - **Lock-in deliverables**（Sprint 2 dispatch ready）：(1) Sprint 2 改名 + Stage 0R direct path 內部執行；(2) A1+A2 W2-B E1 IMPL dispatch ready per W2-A §11.3 12 action checklist；(3) grid+ma 對照軌 Stage 0R baseline；(4) bb_breakout/bb_reversion 30 天 catch-up clock（D+30 = 2026-06-27 評估 retire vs continue demo accumulation）；(5) Tournament activation spec（N=5/M=15）入 PA backlog；(6) Wave 5 Packet C pipeline_ctor 完整 wire（3-way dispatcher + audit emitter）拉進 Sprint 2 並行軌；(7) M11 PA proposal pending（per v76 [4]=b ticket）。
**下一步**：commit v77 + push + Linux pull 三端同步；Sprint 2 Wave 1 dispatch operator GO 後派 W2-B IMPL 鏈。
...
| `P2-M11-REPLAY-RUNNER-SCHEDULE-PROPOSAL` | 3 | **NEW 2026-05-28 per operator decision [4]=b** — Operator 拍板：M11 replay_runner 排程 = governance call，agent 不自啟，派 PA 出 proposal。**任務**：PA 評估 `replay_runner` binary 啟動排程（hourly / 6h / daily / on-demand），輸出 cost/benefit 矩陣（PG IO 持續成本 vs M11 evidence 累積速率 vs Sprint 1A-ζ M1 LAL skeleton 階段對 M11 的真實需求），交由 operator 拍板週期。**Owner**: PA → operator。**Input**: M11 spec + ADR-0044 M7 decay + Sprint 1A-ζ Phase 2 完成度 + `replay.experiments` 23 row last_age=407h 現狀 + 既有 `wave9_replay_no_live_mutation_watch.sh` + `replay_key_rotation_check.sh` cron（無 runner 本體）。**Output**: proposal `.md` 入 `docs/CCAgentWorkSpace/PA/workspace/reports/`；至少 3 cadence option + 預估 PG storage growth + cross-ref M7 + 對 `[48]` healthcheck 影響評估。**Trigger**: 本 ticket land 後派 PA。**ETA**: 2-4 hr。 |
| `P1-SPRINT2-STAGE0R-REPLAY-PREFLIGHT-DISPATCH` | 1 | **NEW 2026-05-28 per Sprint 2 grill-me + PA cross-verify lock (v77)** — Sprint 2 業務性 framing 改名 `Stage 0R Replay Preflight Sprint`（廢 Tournament 排名運作，保 activation spec future use）；內部執行走 Stage 0R direct path（per AMD-2026-05-15-01 §3.3）。**Q4 hybrid 方案 C**：(主軌) A1 funding_short_v2 + A2 liquidation_cascade_fade Rust struct + TOML default `active=false` 30-40 hr IMPL per W2-A §11.3 12 action checklist；(對照軌) grid + ma 進 Stage 0R 作 B0 baseline 控制組驗 QC 5 textbook 結構性 alpha-deficient verdict（PA 預測 reject = 確認 verdict / positive = 推翻 verdict）；(catch-up 軌) bb_breakout + bb_reversion 維持 demo 累積，D+30 ~2026-06-27 評估是否進 M7 decay_signals RETIRED。**Q3 平行 + 出口分 lane**：W2-B IMPL Wave 1+2 不阻於 P0-EDGE/LG-3/OPS gate；Wave 3 出口分 stage0_ready（需 6 Reject gate 收齊：Stage 0R 6 sanity + M11 runner cron + W2-E E2 18 grep + W2-F MIT attribution_chain_ok 100% + AC-S2-A-3 ≥1 candidate avg_net>5bps + W3-C TW+PM sign-off）vs draft_only（V103 EXTEND DRAFT writeback 等 Sprint 3+）；demo canary 升 Stage 1 另開 sprint。**Q5 Packet C 完整 wire 進 Sprint 2 並行軌**：pipeline_ctor wire + 3-way dispatcher (Slack/Email/Console banner) + audit emitter PG INSERT (需新 schema `observability.notification_failsafe_events` 或重用 `observability.engine_events`)；Sprint 2 內 land 為 Sprint 3+ Level 2 promotion 預備。**Owner**: PM (Sprint 2 conductor) → PA (W2-A dispatch packet) → E1 (W2-B IMPL) → E2/MIT/A3/E4/QA Wave 3 sign-off。**Trigger**: operator GO 後 PM 發 Wave 1 dispatch（per PA verdict CONDITIONAL GO Wave 1+2 不阻；只 Wave 3 stage0_ready 出口 conditional）。**ETA**: Sprint 2 W12-W14.5 wall-clock ~2.5 week / 248-351 hr 7 並行（per dispatch packet §1.5）。**Ref**: PA report `docs/CCAgentWorkSpace/PA/workspace/reports/2026-05-28--sprint2_alpha_tournament_entry_checklist.md` + PM sign-off `docs/CCAgentWorkSpace/PM/workspace/reports/2026-05-28--sprint2_grill_pa_cross_verify_lock.md`。 |
| `P2-ALPHA-TOURNAMENT-ACTIVATION-SPEC` | 4 | **NEW 2026-05-28 per Sprint 2 Q2 lock (v77)** — Operator 拍 N=5 strategy 池入口 + M=15 per-strategy n（Q2 雙軌：保 Tournament framing 但不寫 runner code）；派 PA 寫 `docs/execution_plan/specs/2026-05-28--alpha_tournament_activation_protocol.md`。**任務**：spec 列 (1) activation 觸發 = settled 策略池 ≥ N=5 個 + 每策略 n_settled ≥ M=15；(2) 退出 = 池 < N-1 時自動凍結回 Stage 0R direct path；(3) 排名 metric slot 預留（不指定公式，等資料夠迭代）；(4) 晉級規則 slot 預留；(5) 淘汰規則 slot 預留；(6) Re-entry path = 被淘汰策略走 Stage 0R replay preflight 重評；(7) Per-Sprint 觸發評估 cron / dashboard 入口（不寫 runner code）。**禁線**：spec 不寫 tournament runner code（per CLAUDE.md operating-style #2 no speculative implementation + feedback_no_dead_params dead code 禁線）；only spec governance ladder。**Owner**: PA；**ETA**: 2-3 hr。**Trigger**: 本 ticket land 後派 PA。 |
