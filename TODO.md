...
| **S2 Gate-B 24h 真捕捉 run** | 探針已部署（R-0 zero-leak）；2026-06-12 官方公告/API 檢查：最近 `new_crypto` derivatives 為 2026-06-09 已 open，live PreLaunch 僅老 `BPUSDT` ContinuousTrading；等待條件改為公告哨兵/手動 API 命中 fresh Pre-Market/PreLaunch/conversion 窗口 | 一旦有真窗口，跑 24h isolated capture 作 listing fade 主路第一證據；無 transition = `INCONCLUSIVE_NO_TRANSITION`，不可連 production WS/scanner/strategy/DB/order/auth。 |
