...
| `P1-OP1-IP-WHITELIST-CORRECTION` | 1 | **NEW 2026-05-25 per OP-1 pre-verify** — BB sub-agent `ab6c5309` SOP §2 Step 5 推薦「IP whitelist = `100.91.109.86` Tailscale IPv4」**錯誤**；trade-core outbound `curl ifconfig.me` = **`86.127.234.45`** (ISP IP)；Bybit 端看到的是 ISP IP 不是 Tailscale。**正確選項**: (a) IP whitelist = `86.127.234.45` (但 ISP IP 可能 dynamic — operator 需驗 ISP 是否 static 或 NAT/PAT) (b) Bybit "no IP restriction" (less secure but stable，per `bybit-policy-compliance` skill 注意 broker rebate eligibility 不受 IP restriction 影響) (c) Tailscale exit node mode 路由 outbound 走 Tailscale (需額外配置，trade-core sysctl + tailscale up --exit-node)。**修法**: operator 在 Bybit web UI OP-1 期間採選項 (a) 或 (b) 或 (c) 之一. Owner: operator；ETA: 含 OP-1；阻 Earn first stake 若 IP whitelist 設錯 |
