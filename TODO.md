...
**版本** v147 ｜ **日期** 2026-06-12 ｜ **來源實作 HEAD** main 本檔所在 HEAD（本輪 incident-policy dispatch trigger source-chain QA+PM closure；runtime 詳見 §0/§6；版本敘事歸 `docs/CLAUDE_CHANGELOG.md`）。
...
| 區域 | 當前狀態 | 下一步 |
|---|---|---|
| 來源同步 | **三端同步** 本檔所在 main HEAD；本輪 P2 incident-policy dispatch trigger source chain 已完成：CORE ledger + auth invalid + Bybit fail-closed + `sm_halt_stuck` + `position_drift` notify-only + external `engine_dead` watchdog notify-only producers；BB/E2 focused review 通過（0 blocker/high/medium；E2 新 slices 0 low），E4 source-focused regression 通過，QA source acceptance 通過，PM source closure 完成；Linux `trade-core` source fast-forward 同步完成；P5-SM `[81]/[82]` selector fix checkpoint `bf673cdc` 已 Mac→GitHub→Linux `trade-core`，Gate-B watch cron 仍在 Linux `12,42 * * * *`。 | 維持三端同步；**Mac→GitHub 直連已恢復**（2026-06-10 fetch+push 親驗，bundle 流程退役）；Mac srv checkout 駐 main，本輪未 CI、未 deploy/rebuild/restart；runtime activation 仍 operator/deploy-gated。 |
