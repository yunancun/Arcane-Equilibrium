...
**版本** v145 ｜ **日期** 2026-06-12 ｜ **來源實作 HEAD** main 本檔所在 HEAD（本輪 incident-policy producer slices BB/E2 focused re-review closure；runtime 詳見 §0/§6；版本敘事歸 `docs/CLAUDE_CHANGELOG.md`）。
...
**當前主線**：`P0-EDGE-1` Alpha-Edge 體制證據治理（§1）。候選 2 多日 trend = 🔴 **NO-GO-TREND**（關閉）；逃逸路② funding-tilt = 🔴 **NO-GO-C**（關閉）；主路 = **listing fade**（Gate-B 探針 + 專用 watcher/cron 已部署，等 fresh window 告警後跑 24h 真捕捉）+ 已完成的 AEG-S2 證據自動化 runner 基建（§2/§5）。活躍工程佇列 §5；操作員行動 §6；排程 §7。
...
| 區域 | 當前狀態 | 下一步 |
|---|---|---|
| 來源同步 | **三端同步** 本檔所在 main HEAD；本輪 P2 incident-policy dispatch trigger 已在 source 覆蓋 CORE ledger + auth invalid + Bybit fail-closed + `sm_halt_stuck` + `position_drift` notify-only + external `engine_dead` watchdog notify-only producers；BB/E2 對 CORE+auth+Bybit path 與新增 producer slices 均已 focused review 通過（0 blocker/high/medium；E2 新 slices 0 low），E4/QA/full-chain pending；focused Rust/Python tests Mac source checkpoint 綠；Linux `trade-core` source fast-forward 同步完成；P5-SM `[81]/[82]` selector fix checkpoint `bf673cdc` 已 Mac→GitHub→Linux `trade-core`，Gate-B watch cron 仍在 Linux `12,42 * * * *`。 | 維持三端同步；**Mac→GitHub 直連已恢復**（2026-06-10 fetch+push 親驗，bundle 流程退役）；Mac srv checkout 駐 main，本輪未 deploy/rebuild/restart。 |
...
| ID | P | 狀態 | 下一步 |
|---|---:|---|---|
| `P5-SM-OPTION2-CONVERGENCE` | 1 | 🟢 **48h soak RUNNING**(2026-06-11 04:00 deploy:V137 applied=137+兩 flag 已寫 env 檔+canary 已拍/heartbeat 已流;**錨點 03:59:37+02,gate 到期 ~06-13 04:00**;`[82]` 正確報 accumulating；`bf673cdc` 已修好 `--check 81 --check 82` narrow selector) | **基建鏈完成 2026-06-11**:E1×4 接力(`58ad4dba`→`0ce0874c`)→E2 RETURN(2H 假綠:crash-loop 稀釋+canary 死亡不可見,皆 probe 實證)→E1-fix(rollover 去重+30min heartbeat 四子軸)→re-E2 ACCEPT(Probe A/D 重放翻 FAIL)→E4 PASS `d7a9eacf`(兩端全套 0 新 fail;**1s 過殺壓測:真 engine IPC 240/240 ok、H0 p50/p99/p999 三相 byte-identical、RTT p99 1.1ms**)→CC **APPROVE A**(0 BLOCKER;LOW-1 flag 忘關無絆線=PM 接受〔step-(iv) 具名+SOP 三處〕;LOW-2 部署註記見⑤;INFO×3 記錄)。**soak 啟動 SOP(operator)**:①deploy(V137 經 AUTO_MIGRATE=engine;**可與 OPS-2 rebuild 合併一次 restart**)②兩 flag `OPENCLAW_LEASE_PYTHON_IPC_ENABLED=1`+`OPENCLAW_SM_IPC_CANARY_ENABLED=1` 寫 `$SECRETS_ROOT/environment_files/basic_system_services.env`→restart ③soak 期每日 6h cron log 或手跑 `--check 81 --check 82`（Linux 真 DB smoke 2026-06-12T16:38Z：`[81] PASS`, `[82] 38.7h<48h`, probes=1160）④gate=S1-S5(48h/≥500 probe/≥99%/無 15min 連段/S5 smoke N≥10)⑤結束移除兩 flag(**移 flag 後 72h 內 [82] FAIL 屬預期**=fail-closed 噪音,CC LOW-2)。owed-Linux 隨 deploy:canary 真 round-trip 一拍+`[82]` 真值+smoke 首跑。→ step-ii → 🔴step-iii CUTOVER(operator sign-off+CC/E2/BB/E4)。cadence env `OPENCLAW_SM_CANARY_INTERVAL_SECS` 默認 120。 |
...
| `AEG-S3-CANDIDATE-DIRECT-ROWS` | 1 | 🟡 Gate-B full matrix + preflight/watch bridge 已接上 / 候選仍 non-promotable / Gate-B 改事件觸發 | 2026-06-11/12 已完成 artifact-only AEG-S3 direct rows infra、V125/V127 true panel export、funding_revive + oi_delta evidence/PBO/direct rows/metrics、event breadth adapter、empirical execution adapter、Gate-B 專用 watcher cron、sidecar matrix wiring、Gate-B chain wrapper、listing_fade explicit PBO grid wiring、Gate-B full-matrix PBO regression guard、Gate-B preflight locator、Gate-B watch → preflight bridge。`75ed19c8` 新增 `research/aeg_s3_gate_b_chain/`；`3d03698c` 讓 listing_fade PBO candidate grid 明確 opt-in 並透過 Gate-B chain pass-through；`235858f4` 固化 full formal matrix 分支必須攜帶 listing PBO 至 candidate rows；`44a30afa`/`f4a58b3c` 新增並收緊 `research/aeg_s3_gate_b_preflight/`；`2b880f5d` 讓 preflight 讀 local `gate_b_watch_latest.json`，輸出 `gate_watch.operator_action`、`probe_command_hints`，對 stale/malformed/source failure fail-closed，`WATCH_ONLY` 明確為 wait-only。Mac/Linux 本輪 focused regression **62 passed** + compileall OK；Mac `rg` / Linux `grep -R -E` forbidden-route search 無命中。Linux watch-bridge smoke `aeg_s3_gate_b_preflight_watch_bridge_smoke_allow_slow_20260612`：old Gate-B + auto FND2/regime + `/tmp/openclaw/gate_b_watch/gate_b_watch_latest.json`，`gate_watch.artifact_status=WATCH_ONLY`、candidate_counts total=23/alertable=0/start_now=0/schedule=0/watch_only=1、operator_action=`WAIT_FOR_ACTIONABLE_WATCH`、listing sample_count=2、pbo_status=`produced_candidate_grid`、readiness=`READY_BUT_SAMPLE_BELOW_GATE`、command generated。結論：fresh Gate-B 到來後 preflight 同時告訴 operator 是否該啟動 isolated probe、用哪個 probe hint、以及 full-chain command；舊 run 不足 promotion，fresh Gate-B 仍需 `>=30` matched samples；Gate-B v0.1 execution 來源仍是 publicTrade prints，不宣稱 orderbook-depth fill realism。funding_revive 仍卡 `DSR=0.0`、PBO `0.54583333`、execution 未有 `>=30` matched empirical observations；oi_delta event breadth 仍按 basket 不支援 fail-closed，PBO `0.9125`。PM reports：`2026-06-12--aeg_s3_gate_b_watch_preflight_bridge.md`、`2026-06-12--aeg_s3_gate_b_preflight_locator.md`、`2026-06-12--aeg_s3_gate_b_full_matrix_pbo_readiness.md` 及前序 AEG-S3 reports。**下一步**：主線 Gate-B 等 `[GATE-B-WATCH]` fresh Pre-Market/PreLaunch/conversion alert 或 latest artifact 變 `ACTIONABLE_*`；先跑 `aeg_s3_gate_b_preflight.harness --run-id <RUN_ID> --gate-watch-latest-json /tmp/openclaw/gate_b_watch/gate_b_watch_latest.json`，若 `gate_watch.operator_action` 是 START/SCHEDULE 再啟動 isolated 24h probe；probe 完後 preflight sample_count >=30 才跑 generated full-chain command；未經 E2/MIT/QC 獨立審前不得當 promotion proof。 |
...
| `P2-INCIDENT-POLICY-DISPATCH-TRIGGER` | 2 | 🟡 **planned producers source-live / BB+E2 focused re-review passed / E4+QA pending** | 2026-06-12 external `engine_dead` watchdog notify-only producer 已接：新增 `helper_scripts/canary/engine_dead_incident.py`，由 `engine_watchdog.on_engine_crash` 在 snapshot stale ≥30s 且 respawn failed ≥1（`consecutive_failures>=1`）後發 `ENGINE_DEAD_NOTIFY_ONLY` + 既有 engine-down alert；同 down episode 去重，`on_engine_recovery` 發 `ENGINE_DEAD_RESOLVED` 並清 marker；明確不餵 Rust C4 `AllFail`、不武裝 Defensive、無 auth/order/DB/risk/trading mutation，符合 PA §2.6 自指悖論決策。此前 `position_drift` notify-only producer 已接（`position_reconciler/incident.rs`，連續 3 reconcile cycle 未收斂才 feed `IncidentClass::PositionDrift`，class policy NotifyOnly），`sm_halt_stuck` producer 已接（`event_consumer/sm_halt_incident.rs`；`TickPipeline.halt_kind` + `halt_set_ts_ms`；halt 清除 resolved）。既有 CORE/auth/Bybit/C4 path 保持：class ledger、sustained/throttle/7d cooling、push-secret gate、single armed owner、自癒 guard；`auth_invalid` / `bybit_fail_closed` producer source-live。BB/E2 focused review 已覆蓋 prior CORE/auth/Bybit path + 新增 `sm_halt`/`position_drift`/`engine_dead` slices：BB 0 blocker/high/medium，E2 0 blocker/high/medium/low；報告 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-06-12--incident_policy_producer_slices_bb_e2_closure.md`、`docs/CCAgentWorkSpace/E2/workspace/reports/2026-06-12--incident_policy_producer_slices_re_review.md`、`docs/CCAgentWorkSpace/BB/workspace/reports/2026-06-12--incident_policy_producer_slices_bb_re_review.md`，前序 producer reports `2026-06-12--incident_policy_engine_dead_watchdog_producer.md`、`2026-06-12--incident_policy_position_drift_producer.md`、`2026-06-12--incident_policy_sm_halt_stuck_producer.md`。Mac focused tests：`sm_halt_incident` 5 passed、`position_reconciler::incident` 6 passed、`incident_policy` 15 passed、C4 wire 4 passed、watchdog `py_compile` OK、`test_canary.py -k 'engine_dead or WatchdogAlertWiring'` 5 passed；前序 full watchdog/canary checkpoint 仍綠。邊界：未 CI、未 deploy/rebuild/restart；source coverage + BB/E2 已不再阻塞；下一步派 E4 focused regression/full-chain review，通過後 QA acceptance。 |
...
| **S2 Gate-B 24h 真捕捉 run** | 探針已部署（R-0 zero-leak）；專用 `gate_b_watch` 已部署並安裝 Linux cron `12,42 * * * *`。2026-06-12 16:12 UTC latest artifact `/tmp/openclaw/gate_b_watch/gate_b_watch_latest.json` = `WATCH_ONLY`，23 candidates、0 alertable、0 start/schedule、唯一 live watch=`BPUSDT` old `ContinuousTrading`。`2b880f5d` 後 preflight 可直接讀此 artifact；Linux smoke `aeg_s3_gate_b_preflight_watch_bridge_smoke_allow_slow_20260612` 回 `gate_watch.operator_action=WAIT_FOR_ACTIONABLE_WATCH`、sample_count=2、pbo_status=`produced_candidate_grid`、readiness=`READY_BUT_SAMPLE_BELOW_GATE`。報告 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-06-12--aeg_s3_gate_b_watch_preflight_bridge.md` / `2026-06-12--gate_b_autonomous_watch_impl.md`。 | 等 `[GATE-B-WATCH]` fresh Pre-Market/PreLaunch/standard-conversion alert 或 latest artifact 變 `ACTIONABLE_START_NOW` / `ACTIONABLE_SCHEDULE`；先跑 `aeg_s3_gate_b_preflight.harness --gate-watch-latest-json /tmp/openclaw/gate_b_watch/gate_b_watch_latest.json` 確認 `operator_action`，再按 `probe_command_hints` 手動跑 24h isolated capture 作 listing fade 主路第一證據；無 transition = `INCONCLUSIVE_NO_TRANSITION`，不可連 production WS/scanner/strategy/DB/order/auth。 |
