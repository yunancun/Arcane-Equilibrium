...
| `P3-BTCLEADLAG-FENCE-TEST-DRIFT` | 4 | **NEW 2026-05-29 per E4** — `btc_lead_lag_panel_fence_integration::layer_2_fence_archive_policy_diagnostic_only` 確定性 FAIL（單跑/單緒亦 fail，與本 fix byte-identical 無關，pre-existing）：test L300 斷言「ENABLE_PAPER=1 不 spawn」vs source `should_spawn_btc_lead_lag_producer`（btc_lead_lag.rs:67-68）回 true → test 過期 or source 退化，須釐清。Owner: E1 + FA。 |
...
| `P3-OPS-2-CI-FLAKINESS-BIN-CRATE-LOCK` | 4 | **NEW 2026-05-29 per E2 follow-up** — bin crate（main.rs `mod`）的 `main_boot_tasks.rs:644` + `live_auth_watcher_tests.rs:210` 各自獨立 `ENV_GUARD`；與 lib 是獨立 test process **不互 race**，但 bin 內部彼此並行仍是 process-global env 操作。若要統一須在 bin crate 另開 bin-level 共用鎖。低 race 風險（已知 24/24 PASS），低價值，defer。Owner: E1；Trigger: bin test 出現 flake 或順手 wave。 |
...
| `P2-PACKET-C-C4-PIPELINE-WIRE` | 2 | 🟡 **前置(1) HIGH-1 ✅ 解鎖 2026-05-29 / PA C4 spec IN PROGRESS** — C4 = `tasks.rs::spawn_notification_failsafe_watcher` + pipeline_ctor wire + 注入 ThreeWayDispatcher/RestPositionProvider/BybitExchangeStopSync/PgAuditEmitter/WallClock 替 mock。**殘前置**：(2) **ATR 注入**（operator Q-B=BB）— Bybit REST 不回 ATR → `active_lock_profit_per_position` 跳過 → C4 須從 strategies ATR cache 注入 PositionSnapshot.atr；(3) C3 PC3.Q4 `dispatch_and_observe` vs mpsc outcome 路徑；(4) paper engine_mode noop。**operator 2026-05-29 拍板推進**：PA 出 C4 spec → E1 IMPL → E2 → E4 → QA。C4 deploy 將批次帶 C（D2）一起 rebuild。**Owner**: PA spec → E1 → E2 → E4 → QA。 |
