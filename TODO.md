...
- ✅ **Engine/API/watchdog alive（2026-05-29 15:51 UTC 後 = 110017 fix deploy）** — engine **PID 113386**（`build_then_restart_atomic.sh` flock atomic，`/proc/exe SHA == post-build SHA == a0eb67c1…`，含 P1-110017 fix `caf008b6`）；demo+live pipeline alive snapshot fresh；uvicorn ×4 bind `100.91.109.86:8000`；`/api/v1/healthz` **200**；watchdog active + linger=yes；ollama PID 1081。**前一 binary = PID 27582**（cold-audit deploy 13:57，已被 110017 fix 取代）。sqlx clean（無新 migration）。 |
