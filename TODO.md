...
| `P1-EARN-WAVE-C-FIRST-STAKE-RUNTIME` | 2 | **PARTIAL-CLOSED / OPERATOR-BLOCKED 2026-05-25 PM recheck + 2026-05-25 BB+Action-3 verify update** — IntentProcessor Earn branch IMPL ✅; local `cargo test -p openclaw_engine --release --lib earn` = 80 passed; running engine has `EarnStake=2`; PG `learning.earn_movement_log` still **0 rows**. **CRITICAL 政策訂正 (BB sub-agent `ab6c5309`)**: Bybit demo 不支援 `/v5/earn/*` (per WebFetch Bybit Demo API doc) — OP-1 必須 **live slot mainnet key**, demo key 加 `asset:earn` 技術上做不到. **Still blocked by 4 operator hand actions**: (a) OP-1 Bybit web UI mainnet key 重發 with Read+Trade+Earn scope + 永絕 Withdraw + IP whitelist `100.91.109.86` (~27 min, 24h 冷卻 risk) (b) `OPENCLAW_ALLOW_MAINNET=1` env var (per 2026-05-25 SSH Action 3 verify: engine PID 350616 environ 12 var 但 **完全無 mainnet gate** — 必加 + restart engine, ref `P1-ENV-OPENCLAW-ALLOW-MAINNET-SET-FOR-EARN-FIRST-STAKE`) (c) Stage 0R Earn variant spec §8 8 OQ 拍板 (d) 實際 stake $100-200 Flexible-only first stake (動 mainnet balance；走 IntentProcessor Earn branch → bybit_earn_client → learning.earn_movement_log 真實 row). Carry-over: governance_approval_id sentinel 0 until Wave D/E inserts `learning.governance_audit_log` and returns real id. ref `docs/CCAgentWorkSpace/E1/workspace/reports/2026-05-25--sprint1b_earn_wave_c_intent_processor_dispatch.md` + `docs/execution_plan/2026-05-25--stage_0r_earn_variant_design_spec.md` §8 |
