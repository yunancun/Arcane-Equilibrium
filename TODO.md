...
**版本** v137 ｜ **日期** 2026-06-12 ｜ **來源實作 HEAD** main 本檔所在 HEAD（本輪 AEG-S3 Gate-B preflight locator checkpoints `44a30afa`/`f4a58b3c`；runtime 詳見 §0/§6；版本敘事歸 `docs/CLAUDE_CHANGELOG.md`）。
...
| 區域 | 當前狀態 | 下一步 |
|---|---|---|
| 來源同步 | **三端同步** 本檔所在 main HEAD；本輪 AEG-S3 Gate-B preflight locator checkpoints `44a30afa`/`f4a58b3c` 已 Mac→GitHub→Linux `trade-core`，Linux explicit/auto preflight smoke 已跑；Gate-B watch cron 仍在 Linux `12,42 * * * *`。 | 維持三端同步；**Mac→GitHub 直連已恢復**（2026-06-10 fetch+push 親驗，bundle 流程退役）；Mac srv checkout 駐 main，本輪未 deploy/rebuild/restart。 |
...
| `AEG-S3-CANDIDATE-DIRECT-ROWS` | 1 | 🟡 Gate-B full matrix + preflight locator 已接上 / 候選仍 non-promotable / Gate-B 改事件觸發 | 2026-06-11/12 已完成 artifact-only AEG-S3 direct rows infra、V125/V127 true panel export、funding_revive + oi_delta evidence/PBO/direct rows/metrics、event breadth adapter、empirical execution adapter、Gate-B 專用 watcher cron、sidecar matrix wiring、Gate-B chain wrapper、listing_fade explicit PBO grid wiring、Gate-B full-matrix PBO regression guard、Gate-B preflight locator。`75ed19c8` 新增 `research/aeg_s3_gate_b_chain/`；`3d03698c` 讓 listing_fade PBO candidate grid 明確 opt-in 並透過 Gate-B chain pass-through；`235858f4` 固化 full formal matrix 分支必須攜帶 listing PBO 至 candidate rows；`44a30afa` 新增 `research/aeg_s3_gate_b_preflight/`，掃本地 Gate-B/FND2/regime artifacts、preview listing sample/PBO 狀態並輸出 full-chain command；`f4a58b3c` 收緊 auto locator 語義驗證（FND2 summary + regime classifier/healthcheck）。Mac/Linux 本輪 focused regression **58 passed** + compileall OK；Mac `rg` / Linux `grep -R -E` forbidden-route search 無命中。Linux preflight explicit smoke `aeg_s3_gate_b_preflight_explicit_final_20260612`：old Gate-B + `fnd2_18mo_real_20260603` + `aeg_regime_smoke_20260605`，status=`READY_BUT_SAMPLE_BELOW_GATE`、sample_count=2、pbo_status=`produced_candidate_grid`、command generated。Linux auto-locator smoke `aeg_s3_gate_b_preflight_auto_final_20260612`：自動選 `listing_24h_20260602_1847` / `fnd2_18mo_real_20260603` / 最新有效 regime `l2_owed_v127_pop_20260610`，同樣 status=`READY_BUT_SAMPLE_BELOW_GATE`。結論：fresh Gate-B 到來後可先跑 preflight，再複製其 recommended command 跑 execution + event breadth + formal matrix + PBO 全鏈；舊 run 不足 promotion，fresh Gate-B 仍需 `>=30` matched samples；Gate-B v0.1 execution 來源仍是 publicTrade prints，不宣稱 orderbook-depth fill realism。funding_revive 仍卡 `DSR=0.0`、PBO `0.54583333`、execution 未有 `>=30` matched empirical observations；oi_delta event breadth 仍按 basket 不支援 fail-closed，PBO `0.9125`。Gate-B watch latest `WATCH_ONLY`、23 candidates、0 alertable、0 start/schedule、唯一 live watch=`BPUSDT` old ContinuousTrading。PM reports：`2026-06-12--aeg_s3_gate_b_preflight_locator.md`、`2026-06-12--aeg_s3_gate_b_full_matrix_pbo_readiness.md`、`2026-06-12--aeg_s3_listing_fade_pbo_grid_wiring.md`、`2026-06-12--aeg_s3_gate_b_chain_wrapper.md` 及前序 AEG-S3 reports。**下一步**：主線 Gate-B 等 `[GATE-B-WATCH]` fresh Pre-Market/PreLaunch/conversion alert 或 latest artifact 變 `ACTIONABLE_*` 後啟動 isolated 24h probe；先跑 `aeg_s3_gate_b_preflight.harness --run-id <RUN_ID>`，若 status 不是 blocked 且 sample_count >=30，再執行 recommended full-chain command；未經 E2/MIT/QC 獨立審前不得當 promotion proof。 |
