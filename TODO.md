...
**最新狀態快照**（2026-04-29 18:02 CEST · Items 1-6 deployed · residual rolling-window [34]）：62-finding remediation Batch A-F 全部完成、push、Linux rebuild/redeploy；Items 1-6 follow-through commit `53bff07` 已推送、Linux fast-forward、release rebuild + restart，engine PID **620724** + API PID **620851**。fee freshness 改為 grouped demo/live binding task，啟動日誌已確認 `engine=demo env=Demo` + `engine=live env=LiveDemo`；post-restart risk verdicts 不再有 fee-stale self-lock，[27] PASS。`[34] intent_signal_attribution` 仍因 30min rolling window 內 pre-restart 空 signal_id 舊 rows 報 FAIL；post-restart intents 已確認 `empty_signal_id=0`，等窗口自然清。report 見 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--items-1-6-completion.md`。新 PM 方案 `docs/CCAgentWorkSpace/PM/workspace/reports/2026-04-29--ml_dream_edge_unblock_plan.md` 決定：**正 edge 是 promotion gate，不是 training gate**；Demo 可先用 ML/LinUCB/DreamEngine/OpportunityTracker 做 read-only、shadow、counterfactual、demo 小探索來修 edge；Live 自動交易或 live 參數自動放權必須經 GovernanceHub / Decision Lease + 既有 5 live gates 批准。
