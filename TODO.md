...
| `P1-OPS-4-GAP-B-PG-RESTORE-DRILL` | 1 | ⚠️ **PARTIAL 2026-05-27 by MIT** — 53d 前 189KB schema-only dump only / 無 active pg_dump cron / NAS 未掛 vs spec §7.2 drift；restore drill empirical PASS 但結構過時不可當 readiness；ref `docs/CCAgentWorkSpace/MIT/workspace/reports/2026-05-27--ops_4_gap_b_d_pg_backup_drill.md`；待 operator 7 hand-action ~28hr unblock |
| `P1-OPS-4-GAP-D-PG-DUMP-CRON` | 1 | ⚠️ **DRAFTS READY 2026-05-27 by MIT** — 3 drafts (install_pg_dump_cron.sh 74 + trading_ai_pg_dump_cron.sh 101 + verify_pg_dump.sh 116) at `docs/CCAgentWorkSpace/MIT/workspace/drafts/ops4_gap_b_d/`；0 硬編碼 + Linux-only + idempotent + default DRY-RUN；但未 install；disk 226GB×15d≈841G ceiling 需 disk strategy 拍板 |
| `P1-OPS-4-GAP-B-D-OPERATOR-DEPLOY` | 1 | **NEW 2026-05-27 per MIT** — 7 operator hand-action ~28hr first-day live unblock: (1) 審 3 drafts (2) 拍板 disk strategy (NAS / 7d retention / local 15d) (3) land 至 helper_scripts/cron/ (4) dry-run + install cron (5) D+1 03:00 UTC fresh dump 後 verify_pg_dump.sh PASS (6) fresh 226GB dump 跑 full data restore drill (7) verify_pg_dump 接 passive_wait_healthcheck.sh 新 check |
