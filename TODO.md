...
| **OPS-2 Phase-2 cutover deploy + P5-SM 活化(可合併一次 restart)** | OPS-2 鏈全綠 merge 完成(`3018c7a3`);P5-SM 基建 shipped(`ca80d084`,dormant) | 剩:①C-C 外部 alert 3 字串+親簽 ②`--rebuild` ③C-B 手動 `/auth/renew` 留證 ④§13.6 D+15-44 起算+首次 rotation 2026-09-08。**Linux regression(E4-owed)= ✅ 全綠**:Py 67f/4545p(0 cutover 相關失敗,4 點名檔 61/61)+Rust 43 targets **4153/0** EXIT=0 → **rebuild GO**(C-C 後即可按)。 |
