"""P4 online-FDR α-wealth / pre-registration / V132 哨兵 — `[83]`-`[87]`。

MODULE_NOTE
模塊用途：L2 Mesh P4 的五軸帳本完整性哨兵（V138 research.alpha_wealth_ledger
  + research.pre_registered_hypotheses + V132 hidden_oos_state_registry）：
    [83] alpha_wealth_family_cardinality — family 數 vs MIT 4a 量化門檻
         N_fam ≤ 1/γ = 10（超限 = 全域 mFDR ≤ α_target 詮釋不再成立）。
    [84] alpha_wealth_orphan_refund — refund 無對應 debit（MIT N-3
         wealth-inflation 向量；DB CHECK 無法跨 row，本哨兵補位）。
    [85] alpha_wealth_refund_amount_mismatch — refund.amount ≠ φ·|debit.amount|
         （MIT N-3 第二臂；φ=1.0 M1 ENDORSED）。
    [86] pre_reg_cross_family_duplicate_spec — 同 spec_sha256 跨 family 重註冊
         計數（MIT 4b：帳務 sound、觀測級即可，WARN 不 FAIL）。
    [87] hidden_oos_state_regression — V132 sealed 列被回寫指紋
         （QC QN-1：consumed/opened→sealed 回退偵測，best-effort）。
主要函數：check_83_* ~ check_87_*（`(cur) -> (status, msg)` 與既有 checks_*
  同契約）。
依賴：psycopg2 cursor（runner 注入）；0 其他依賴。
硬邊界：全部唯讀 SELECT；V138 表不存在 → 一律 PASS-skip 不 FAIL
...
# [85] 的 SQL 等式（r.amount + d.amount = 0 ⟺ refund = 1.0·|debit|）須同步。
...
_SKIP_MSG = "SKIP (V138 not deployed): research FDR tables absent; check is dormant"


def check_83_alpha_wealth_family_cardinality(cur) -> tuple[str, str]:
    """[83] family 基數 vs MIT 4a 門檻（N_fam ≤ 10）。
...
def check_84_alpha_wealth_orphan_refund(cur) -> tuple[str, str]:
    """[84] orphan refund（refund 列無對應 debit 列）。

    為什麼是 FAIL：refund 是 +amount 事件，無對應 debit = 憑空灌 wealth
    （MIT N-3 wealth-inflation 向量）。V138 CHECK 無法跨 row 驗，append-only
...
def check_85_alpha_wealth_refund_amount_mismatch(cur) -> tuple[str, str]:
    """[85] refund.amount ≠ φ·|debit.amount|（φ=1.0）。
...
def check_86_pre_reg_cross_family_duplicate_spec(cur) -> tuple[str, str]:
    """[86] 跨 family 重複 spec_sha256（觀測級，MIT 4b）。
...
def check_87_hidden_oos_state_regression(cur) -> tuple[str, str]:
    """[87] V132 state 回退指紋（consumed/opened → sealed，QC QN-1）。
    ...
    V132 表缺 → PASS-skip（與 V138 同款部署順序容忍）。
    """
    _rollback_quietly(cur)
    cur.execute(
