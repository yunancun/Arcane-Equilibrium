"""AEG-S3 listing fade evidence artifact writer。"""
...
def write_all(
    *,
    ...
    evidence_path = write_json(run_dir / "listing_fade_candidate_evidence.json", evidence)
    summary_path = write_json(run_dir / "listing_fade_evidence_summary.json", summary)
    artifacts = [
        _index_entry("listing_fade_candidate_evidence.json", evidence_path, EVIDENCE_SCHEMA_VERSION),
        _index_entry("listing_fade_evidence_summary.json", summary_path, summary.get("schema_version", "unknown")),
    ]
    prov = _git_provenance(repo_root)
    manifest = {
        "schema_version": MANIFEST_SCHEMA_VERSION,
        "run_id": run_id,
        "program": "AEG-S3-listing-fade-evidence",
    ...
    index_path = write_json(
        ...
        {"schema_version": "aeg.s3_listing_fade_artifact_index.v0.1", "run_id": run_id, "artifacts": artifacts},
    )
    return {
        "run_dir": str(run_dir),
