#!/usr/bin/env python3
"""AEG-S3 funding revive evidence producer CLI."""
...
try:
    ...
    from aeg_s3_funding_revive import artifact as artifact_mod  # type: ignore
    from aeg_s3_funding_revive import builder as builder_mod  # type: ignore
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="aeg_s3_funding_revive.harness",
        description="Offline funding/price panel JSONL to AEG-S3 funding revive candidate evidence JSON",
    )
    p.add_argument("--panel-jsonl", required=True, dest="panel_jsonl")
    p.add_argument("--run-id", required=True, dest="run_id")
    p.add_argument("--lookback-points", required=True, type=int, dest="lookback_points")
    p.add_argument("--horizon-hours", required=True, type=float, dest="horizon_hours")
    p.add_argument("--stress-z", required=True, type=float, dest="stress_z")
    p.add_argument("--exit-z", required=True, type=float, dest="exit_z")
    ...
    p.add_argument("--candidate-id", default="funding_revive", dest="candidate_id")
