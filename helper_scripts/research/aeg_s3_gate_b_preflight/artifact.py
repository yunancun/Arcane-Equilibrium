"""Artifact writer for AEG-S3 Gate-B preflight summaries."""
...
def write_all(
    *,
    ...
    summary_path = write_json(run_dir / "gate_b_preflight_summary.json", summary)
    artifacts = [
        _index_entry("gate_b_preflight_summary.json", summary_path, SUMMARY_SCHEMA_VERSION),
    ]
    prov = _git_provenance(repo_root)
    manifest = {
        "schema_version": MANIFEST_SCHEMA_VERSION,
        "run_id": run_id,
        "program": "AEG-S3-gate-b-preflight",
        ...
        "gate_b_run_dir": summary.get("selected_artifacts", {}).get("gate_b_run_dir"),
        "fnd2_run_dir": summary.get("selected_artifacts", {}).get("fnd2_run_dir"),
        "regime_run_dir": summary.get("selected_artifacts", {}).get("regime_run_dir"),
        "policy": "inspect_local_artifacts_only_no_collector_no_runtime_mutation",
        "artifacts": artifacts,
    }
    ...
    index_path = write_json(
        ...
        {
            "schema_version": "aeg.s3_gate_b_preflight_artifact_index.v0.1",
            "run_id": run_id,
            "artifacts": artifacts,
        },
    )
