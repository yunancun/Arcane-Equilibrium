...
def classify_pre_state(
    *, observed: Any, desired: Any, ownership_evidence: Any
    ...
    if observed == desired:
        if isinstance(ownership_evidence, dict) and ownership_evidence.get(
            "s2_4_receipt_digest"
        ) and ownership_evidence.get("journal_digest"):
...
def _build_postcheck(
    *,
    ...
    try:
        ...
        return None, [f"independent {component_effect_class} postcheck failed: {error}"]
...
def _build_rollback(
    *,
    ...
    if recovery_required:
        ...
        status = "COMPENSATED_EXACT" if compensated else "NOT_COMPENSATED"
    rollback = {
        ...
        "exact_pre_state_restored": bool(compensated),
...
def _finish_row(
    *,
    ...
    if postcheck is None or postcheck["status"] != "PASS":
        compensated = bool(compensate()) if mutation_performed else True
        rollback = _build_rollback(
            ...
            compensated=compensated, ownership_aware=True, clock=clock,
        ...
        try:
            ...
            postcheck_reasons = postcheck_reasons + [f"compensation journal failed: {error}"]
            compensated = False
        final = (
            COMPONENT_STATUS_POSTCHECK_ROLLED_BACK if compensated
    ...
    rollback = _build_rollback(
        component_effect_class=component_effect_class, install_plan_digest=plan_digest,
        pre_state_digest=pre_state_digest, post_state_digest=post_state_digest,
        compensated=False, ownership_aware=True, clock=clock,
    )
    ...
    return _verdict(
        status, reasons, component_effect_class=component_effect_class, row_abi=row_abi,
...
def _compensating_failure(
    *,
    ...
    try:
        ...
        reason = f"{reason}; compensation journal failed: {error}"
        compensated = False
    return _verdict(
        COMPONENT_STATUS_FAILED if compensated else COMPONENT_STATUS_RECOVERY_REQUIRED,
        [
            reason,
            (
                "the task-owned delta was compensated in reverse order"
                if compensated
                else "exact compensation was impossible; RECOVERY_REQUIRED blocks S2.5"
            ),
        ],
        component_effect_class=component_effect_class, row_abi=row_abi,
