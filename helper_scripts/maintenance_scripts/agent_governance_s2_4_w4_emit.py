#!/usr/bin/env python3
"""S2.4(WP4·W4)wave-exit 正式發射葉(鏡 ``agent_governance_s2_4_w3_emit``;install 上層 re-export)。

四段鏈路(§10.3 W4 row):

1. 讀持久化的歷史 W3 receipts 作 lineage 記錄(其 source_head 屬歷史世代,**不**直接進
   derivation 鏈;self_digest 竄改即硬拒——屬停機調查事件);
2. 以同一批 builder 於記憶體內重發「當前世代」W0 admission + W0/W1/W2/W3 wave-exit 並逐段
   導出 ADMITTED/PASS;
3. 構建 W4 wave-exit 綁定該鏈,由中央 validator 以
   ``predecessor_wave_receipt=<W3>`` + ``predecessor_wave_chain=(W0, W1, W2)`` 遞迴導出 PASS;
...
W3_RECEIPT_DIR = (
    REPO_ROOT / "docs" / "execution_plan" / "ai_ml_landing" / "receipts" / W3_RECEIPT_DIRNAME
)
W4_RECEIPT_DIRNAME = "S2.4-WP4-W4"
W4_WAVE_EXIT_FILENAME = "S2.4-WP4-W4-wave-exit-receipt-v1.json"
W4_REGENERATED_W0_ADMISSION_FILENAME = (
    "S2.4-WP4-W4-regenerated-W0-source-admission-receipt-v1.json"
)
W4_REGENERATED_W0_WAVE_EXIT_FILENAME = "S2.4-WP4-W4-regenerated-W0-wave-exit-receipt-v1.json"
W4_REGENERATED_W1_WAVE_EXIT_FILENAME = "S2.4-WP4-W4-regenerated-W1-wave-exit-receipt-v1.json"
W4_REGENERATED_W2_WAVE_EXIT_FILENAME = "S2.4-WP4-W4-regenerated-W2-wave-exit-receipt-v1.json"
W4_REGENERATED_W3_WAVE_EXIT_FILENAME = "S2.4-WP4-W4-regenerated-W3-wave-exit-receipt-v1.json"
W4_DERIVATION_RECORD_FILENAME = "S2.4-WP4-W4-derivation-record.json"
# W3 側持久化物的檔名(只讀 lineage;與 w3_emit 的常量同值,發射器不改寫歷史 receipt)。
_W3_WAVE_EXIT_FILENAME = "S2.4-WP4-W3-wave-exit-receipt-v1.json"
_W3_REGENERATED_W0_ADMISSION_FILENAME = (
    "S2.4-WP4-W3-regenerated-W0-source-admission-receipt-v1.json"
)
_W3_REGENERATED_W0_WAVE_EXIT_FILENAME = "S2.4-WP4-W3-regenerated-W0-wave-exit-receipt-v1.json"
_W3_REGENERATED_W1_WAVE_EXIT_FILENAME = "S2.4-WP4-W3-regenerated-W1-wave-exit-receipt-v1.json"
_W3_REGENERATED_W2_WAVE_EXIT_FILENAME = "S2.4-WP4-W3-regenerated-W2-wave-exit-receipt-v1.json"


def build_w4_wave_exit_receipt(
    ...
    *,
    ...
    """綁定當前世代 W0/W1/W2/W3 鏈與真實 test/capture/review 證據 digest 的 W4 wave-exit(無 status)。

    W4 面(owned-path/diff/exported-ABI)全由中央 validator 的 code-owned W4 投影於活 repo
    重算(exported-ABI 折入 permit/lock/journal/budget/compensation 五組活裁決);predecessor
    綁 W3 的 self_digest。
    """
    ...
    receipt: dict[str, Any] = {
        ...
        "owned_path_manifest_digest": central_validator.canonical_digest(
            sorted(central_validator._W4_OWNED_PATHS)
        ),
        "owned_path_diff_digest": central_validator.w4_owned_path_diff_digest(),
        "exported_abi_digest": central_validator.canonical_digest(
            central_validator.w4_exported_abi_projection()
        ),
...
def load_persisted_w3_receipts(w3_receipt_dir: Path) -> dict[str, Any] | None:
    """讀持久化的歷史 W3 receipts(lineage 記錄用;讀不到/畸形回 None → 發射 fail-closed)。"""

    names = (
        _W3_WAVE_EXIT_FILENAME,
        _W3_REGENERATED_W0_ADMISSION_FILENAME,
        _W3_REGENERATED_W0_WAVE_EXIT_FILENAME,
        _W3_REGENERATED_W1_WAVE_EXIT_FILENAME,
        _W3_REGENERATED_W2_WAVE_EXIT_FILENAME,
    )
    try:
        loaded = [
            json.loads((w3_receipt_dir / name).read_text(encoding="utf-8")) for name in names
        ]
    except (OSError, json.JSONDecodeError):
        return None
    if not all(isinstance(item, dict) and item.get("self_digest") for item in loaded):
        return None
    w3_wave_exit, w0_admission, w0_wave_exit, w1_wave_exit, w2_wave_exit = loaded
    integrity = (
        "VERIFIED"
        if all(
    ...
    return {
        "persisted_dir": str(w3_receipt_dir),
        ...
        "historical_source_head": w3_wave_exit.get("source_head"),
        "historical_w3_integrity": integrity,
    }


def emit_w4_receipts(
    *,
    repo_root: Path = REPO_ROOT,
    out_dir: Path,
    test_evidence: dict[str, Any],
    review_provenance: list[dict[str, Any]],
    w3_receipt_dir: Path = W3_RECEIPT_DIR,
    allow_overwrite: bool = False,
) -> dict[str, Any]:
    """發射並持久化正式 W4 wave-exit receipt;任一導出非 ADMITTED/PASS 即 fail-closed 不寫檔。"""

    import agent_governance_s2_4_install as _install

    _install._validate_emit_evidence(test_evidence, review_provenance)

    historical_w3 = load_persisted_w3_receipts(w3_receipt_dir)
    if historical_w3 is None:
        return {
            "status": "W4_EMIT_REFUSED",
            "stage": "historical_w3_receipts",
            "reasons": [
                f"persisted W3 receipts are unreadable or malformed under {w3_receipt_dir}"
            ],
        }
    if historical_w3.get("historical_w3_integrity") != "VERIFIED":
        return {
            "status": "W4_EMIT_REFUSED",
            "stage": "historical_w3_receipts",
            "reasons": [
                "persisted W3 receipts fail self-digest verification "
                "(tampered governance history; investigate before any W4 emission)"
            ],
        }
    ...
    w4_wave_exit = build_w4_wave_exit_receipt(admission, w3_wave_exit, **evidence)
    ...
    derivation_record = {
        "schema_version": "s2_4_w4_derivation_record_v1_informal",
        "source_head": admission["source_head"],
        ...
        "w4_wave_exit_derivation": w4_result,
        ...
        "historical_persisted_w3": historical_w3,
        "test_evidence": test_evidence,
        "review_provenance": review_provenance,
        "replay_note": (
            "W4 re-derivation binds source_head == checkout HEAD; to replay, checkout "
            "source_head, rebuild the in-memory W0/W1/W2/W3 chain with the same builders, then "
            "run derive_wave_exit_status(w4, source_admission_receipt=admission, "
            "predecessor_wave_receipt=w3_wave_exit, "
            "predecessor_wave_chain=(w0_wave_exit, w1_wave_exit, w2_wave_exit))"
        ),
    }
    existing = _persist_emit_artifacts(
        out_dir,
        (
            (W4_WAVE_EXIT_FILENAME, w4_wave_exit),
            (W4_REGENERATED_W0_ADMISSION_FILENAME, admission),
            (W4_REGENERATED_W0_WAVE_EXIT_FILENAME, w0_wave_exit),
            (W4_REGENERATED_W1_WAVE_EXIT_FILENAME, w1_wave_exit),
            (W4_REGENERATED_W2_WAVE_EXIT_FILENAME, w2_wave_exit),
            (W4_REGENERATED_W3_WAVE_EXIT_FILENAME, w3_wave_exit),
            (W4_DERIVATION_RECORD_FILENAME, derivation_record),
        ),
        allow_overwrite=allow_overwrite,
    )
    if existing is not None:
        return _emit_collision_refusal("W4_EMIT_REFUSED", existing)
    return {
        "status": "W4_RECEIPTS_EMITTED",
        "out_dir": str(out_dir),
        "source_head": admission["source_head"],
        "admission_self_digest": admission["self_digest"],
        "regenerated_w0_wave_exit_self_digest": w0_wave_exit["self_digest"],
