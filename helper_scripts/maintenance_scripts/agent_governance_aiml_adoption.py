...
TERMINAL_RECEIPT_SINK_CONTRACT = {
    "status": "declared_disposable_worm_emulation_implemented",
    "owner_session": "S1.2",
    "authority": (
        "PM/operator approved terminal_receipt_append_intent_v1 for disposable "
        "local WORM emulation only; no external WORM destination and no "
        "production terminal append"
    ),
    "invariant": (
        "each append commits at most one immutable content-addressed record "
        "(chmod 0o444); idempotent dedup never rewrites a committed record; "
        "external_worm targets fail closed; readback ACK requires a distinct "
        "actor; no route_task node or closure effect binding is injected before "
        "S8.6"
    ),
    ...
    "implementation_paths": [
        "helper_scripts/maintenance_scripts/agent_governance_terminal_receipt_sink.py"
    ],
    "component_paths": [],
    "intent_schema_path": (
        f"{AIML_GATE_SCHEMA_ROOT}/terminal_receipt_append_intent_v1.schema.json"
    ),
    "receipt_schema_path": (
        f"{AIML_GATE_SCHEMA_ROOT}/terminal_receipt_append_result_v1.schema.json"
    ),
    "readback_ack_schema_path": (
        f"{AIML_GATE_SCHEMA_ROOT}/terminal_receipt_readback_ack_v1.schema.json"
    ),
}
