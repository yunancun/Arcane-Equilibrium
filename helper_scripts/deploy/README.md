...
OpenClaw 运行一个 HTTP 服务与一个独立 Rust 引擎：

| 服务 / Service | 说明 / Description | 默认端口 / Default Port |
|---|---|---|
| **Trading API** | FastAPI + Uvicorn，交易控制核心 | 8000 |
| **OpenClaw Engine** | Rust 交易运行时（本地 IPC，不开放 HTTP 端口） | N/A |

外部 Node.js Gateway 與 `/openclaw/*` HTTP 反向代理已于 2026-07-16
退役并移除。Trading API 仍保留受認證的本地 `/api/v1/openclaw/*`
只讀控制/监控路由。
...
该服务注册为 **user-level systemd unit**（`systemctl --user`），不需要 root 权限。
...
倉庫提供 3 個 launchd plist 範本（對應 Linux systemd 單元）：
...
for plist in com.openclaw.trading-api com.openclaw.engine \
             com.openclaw.engine-watchdog; do
...
4. 載入服務（依賴順序：engine → watchdog → api）：
...
```bash
umask 077
printf 'postgresql://trading_admin:%s@127.0.0.1:5432/trading_ai\n' \
...
Trading API 與 Rust engine 透過既有資料庫模組連接 PostgreSQL。
...
> Tailscale Serve / reverse proxy remains preferred for HTTPS.
...
3. OpenClaw Engine     — Rust 交易运行时就绪
4. Trading API         — 主服务启动
...
systemd `After=` 僅等待 network-online；engine 與 API 的實際健康由既有
runtime readiness / watchdog 檢查判定。launchd 無原生依賴排序。
