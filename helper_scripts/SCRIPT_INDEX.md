...
| `db/passive_wait_healthcheck/checks_alpha_wealth_fdr.py` | `[82]`-`[86]` L2 P4 online-FDR 五軸哨兵（`(cur)->(status,msg)` 同契約，全唯讀）：`[82]` family 基數 vs MIT 4a 門檻 N_fam≤1/γ=10（>10 FAIL／==10 WARN）；`[83]` orphan refund（refund 無對應 debit = wealth-inflation 向量，MIT N-3，FAIL）；`[84]` refund.amount ≠ φ·\|debit.amount\|（φ=1.0 M1 ENDORSED；NUMERIC 精確等式 `r.amount+d.amount<>0`，FAIL）；`[85]` 跨 family 重複 spec_sha256（MIT 4b：帳務 sound，觀測級 **WARN 不 FAIL**）；`[86]` V132 sealed 列回寫指紋（QC QN-1：`state='sealed' AND updated_at>created_at+1s`；蓄意連 updated_at 偽造的多欄重寫無痕=已知盲點，docstring 誠實標界）。**V137/V132 表缺 → 一律 PASS-skip**（pre-deploy 不 false-FAIL，graceful-absent 慣例）。配對測試 `test_checks_alpha_wealth_fdr.py`（fake cursor，0 連線）。 |
| `db/passive_wait_healthcheck/runner.py`（P4 E1-C delta） | 新增 `[82]`-`[86]` 註冊（import + cursor 區塊內 append，純 SQL check）。 |
| `cron/ml_training_maintenance.py`（P4 E1-C delta） | 新 OPTIONAL job `alpha_wealth_reconciler`（不在 DEFAULT_JOBS）：wrapper `_run_alpha_wealth_reconciler` flag gate `OPENCLAW_ALPHA_WEALTH_RECONCILER`（預設 OFF → skipped）→ 獨立 psycopg2 conn（statement_timeout=30s，非 api pool）→ `program_code/ml_training/alpha_wealth_refund_reconciler.run_alpha_wealth_refund_reconciler`（dry_run 跟 cron `--dry-run`）。三重 OFF：flag + 不在 DEFAULT_JOBS + V137 0 rows = 部署即行為中性。配對測試 `cron/tests/test_alpha_wealth_reconciler_cron.py`。 |
