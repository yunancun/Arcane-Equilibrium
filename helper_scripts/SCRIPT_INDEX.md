...
最新補充（2026-06-20 FillSim refresh cron）：新增 `cron/fill_sim_refresh_cron.sh`，以 bounded read-only 方式刷新 MM verdict 依賴的 `research/fillsim/fillsim_report.json`；預設只在報告 age >60h 或 `OPENCLAW_FILL_SIM_FORCE=1` 時跑 `python -m program_code.research.microstructure.fill_sim --hours 2 --out ...`，避免無界全掃隨 L1 累積膨脹；PG 層 `PGOPTIONS=-c default_transaction_read_only=on` + loader `set_session(readonly=True)`，寫入面僅 artifact/log/status/heartbeat/alert。建議 daily 06:05 UTC 跑在 `recorder_mm_verdict_cron.sh` 06:41 UTC 前。正式條目見 `## 2026-06-20 FillSim refresh cron` section。
...
| 腳本 / 模塊 | 用途 |
|---|---|
| `cron/fill_sim_refresh_cron.sh` | MM verdict 的 fill-sim artifact refresh wrapper。預設 `OPENCLAW_FILL_SIM_HOURS=2`、`OPENCLAW_FILL_SIM_MAX_AGE_H=60`、`OPENCLAW_FILL_SIM_STALE_ALERT_H=72`：報告新鮮時只寫 `logs/fill_sim_refresh.log` status line 並 skip；過期或 `OPENCLAW_FILL_SIM_FORCE=1` 時執行 `python -m program_code.research.microstructure.fill_sim --hours N --out <DATA>/research/fillsim/fillsim_report.json`。PG creds 只讀 `basic_system_services.env`，export `PGOPTIONS=-c default_transaction_read_only=on`，再由 `data_loader.connect().set_session(readonly=True)` 二次保護；writes 僅 local JSON/CSV artifact、`logs/fill_sim_refresh_cron.log`、status log、`cron_heartbeat/fill_sim_refresh.last_fire`、失敗/stale 時 `alerts/alerts.jsonl`。建議 Linux cron：`5 6 * * * ... fill_sim_refresh_cron.sh`（在 `recorder_mm_verdict_cron.sh` 06:41 UTC 前）。不啟 strategy/probe，不重啟，不下單，不碰 auth/risk/order/trading 表。 |
