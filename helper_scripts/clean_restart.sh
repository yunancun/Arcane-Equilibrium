...
# OPS-2 SECRET-SPLIT — clean_restart 不 seed（保留 state；seed 由 restart_all 主路徑做）；
# spawn 點注入 env 讓 engine + API 接通新 path。
...
if [ -d "$DATA_DIR" ]; then
    ...
    # canary + audit;engine.log 必須在 spawn 前 mv 歸檔:watchdog
    # classify_engine_failure 依賴「重啟後 active log 是新檔」invariant(殘留舊
    # 尾行會誤導崩潰分類),歸檔同時保留死亡日誌(OPS-F1 2026-07-15)。
    for f in "$DATA_DIR"/engine_results.jsonl "$DATA_DIR"/demo_audit.jsonl "$DATA_DIR"/watchdog.log \
...
# 空轉(機制詳見 restart_all.sh 引擎啟動行注釋)。上一輪 engine.log 已在歸檔段
# mv 進 $ARCHIVE_ROOT/canary/,spawn 時檔案必不存在,run 中大小由 logrotate
# 1G size cap 兜底。
