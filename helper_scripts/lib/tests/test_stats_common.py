...
def test_dsn_builds_from_discrete_env(monkeypatch):
    ...
    assert PG.resolve_report_dsn() == "postgresql://svc:pw@10.0.0.5:6543/trade"
...
def test_dsn_host_port_defaults(monkeypatch):
    ...
    assert PG.resolve_report_dsn() == "postgresql://u:p@127.0.0.1:5432/d"
...
def test_dsn_loads_missing_password_from_secrets_env(monkeypatch, tmp_path):
    ...
    assert PG.resolve_report_dsn() == "postgresql://u:secret=pw@127.0.0.1:5432/d"
...
def test_dsn_does_not_override_existing_password(monkeypatch, tmp_path):
    ...
    assert PG.resolve_report_dsn() == "postgresql://u:env_pw@127.0.0.1:5432/d"
