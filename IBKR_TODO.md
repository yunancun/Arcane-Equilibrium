...
- **現況一句話(2026-07-16 R8 校準)**:治理全解鎖(AMD-2026-07-11-01 Accepted);**W0/W1/W2(DONE_SOURCE_SECURED_HARDENED,PR#28)/W-CI(DONE_LANDED_FIRST_GREEN,PR#21)/W3(DONE_SOURCE_SECURED,S1-S4,PR#32/#33/#35/#38)/W4(DONE_SOURCE_SECURED,W4-1,PR#40)全收口且經 R8 對抗審計 CONFIRMED**;型別契約陣 34 modules/37 acceptance 檔;**W5(S0-S4)已於 2026-07-17 loop R9-R13 全包收口 DONE_SOURCE_SECURED**(PR#48/#49/#57/#60/#61;收口四腿 E2/E3/QA/CC 全過,INV-1 HOLDS;三腿指紋三角測量第三腿顯式 re-scope→W6 必修同票);**W8a+W9a 已於 2026-07-17 loop R14∥R15 收口 DONE_SOURCE_SECURED**(PR#63/#62;E3-F1 首階段 CLOSED、TOFU pin 供應鏈紀律、四腿全過)——**D2 觸發:EA1(憑證+D3 username)/EA2(Gateway 安裝窗)已可開,operator 已獲通知;EA3 mini-wiring 已收口(07-17 R16,PR#66:Gate 5=envelope 驗證+nonce 原子消費於 connect 前;IB-NOTE-1 CLOSED)——**EA1-EA3 全部零工程等待,剩 operator 動作+runbook**(RB-1~RB-5:①TWS/Gateway API 時間設 UTC 模式;②dry-run 先看 G4GateStatus 含 envelope/epochs 姿態位;③envelope JSON+epochs TOML 簽發程序文件待 authoring;④nonce 已燒後 connect 失敗=換新 envelope,成功接觸後歸檔 envelope 檔或 bump epoch 防重啟 replay;⑤澄清 #3 apply 前批准檔位元組歸檔);**W6 進行中:S0(R17 PR#69)+S1(R18 PR#70)+S3a(R19 PR#76:mktData 紅線 E2-F1 注入修+regulatorySnapshot FUNDS-LOCKED+quote/provenance 契約+OUT builders,E2 APPROVE/E3/IB PASS_WITH_NOTES)已收口**;**S3b(R20 PR#77)已收口→W6-S3 mktData lane 完整**;**W6(S0-S4)全收口 DONE_SOURCE_SECURED(R22 PR#79)**;**W7 設計輪 R23 PR#80(INV-ORDER transport-gating)+W7-S0 收口 R24 PR#81(transport-gating 恆拒 stub 地基:OrderFrame newtype+OrderEffectPermit mint cfg(test)+單一 send_order_framed+EffectEnvelopeRequiredStub 恆拒+machine-check+4 structure guard;engine ibkr 414/final-binary DCE 零 seam;E2 APPROVE_WITH_NOTES/E3 PASS INV-ORDER+INV-1 HOLDS/IB PASS/CC PASS A 級)**;**S1 blocking discipline(E2-NOTE-1/E3-NOTE-1):S0 承重=無 encoder+0 caller+DCE 非型別屏障→S1 encoder 落地必強制 order bytes 唯一經 gated send_order_framed(新 G5 守衛禁直達 driver::send_framed,或委 S4 runtime gate);sealed WireBytes+binary DCE 守衛=S1 評估**;**W7-S1 收口 R25 PR#82**(訂單狀態機 driver+intent journal 不送出=14-態+唯一 mutator reduce-only fail-closed+journal+nextValidId+idempotency_key join+ApiPending transient 分流+restart recovery+**NOTE-1 (a) sealed WireBytes** 型別阻斷 order bytes 退化+place/cancel encoder NO send+encode ceiling;engine ibkr 443;初審四腿→**IB 揪 2 CONFIRMED DIVERGENT wire 缺陷**→rework:placeOrder 全欄序重導 byte-golden 對 pinned client.py 升 **CONFIRMED-vs-9.81.1 ≤157**、whatIf tail decode fail-close **PreviewDecodeBlockedPendingFullSequence**(OrderState mid-message 非尾,全序列解碼 EA3-gated)→複審 E2 APPROVE+IB PASS DIVERGENT CLOSED);**新 EA3 前 blocking BLOCK-ORDER-BAND-4(placeOrder 158-176 sv-gated re-pin)/BAND-5(whatIf 全序列解碼 blocked-until),BAND-3 修訂(≤157 定位缺陷+band 增長)**;S3 非阻擋 carry=task_03abee1a;**W7-S2 收口 R26 PR#83**(cash 約束引擎 deterministic pre-submit gate:7 gate settled-funds T+1(SEC 15c6-1(a))/GFV/no-short 硬邊界/RTH/order-type 白名單/fractional/LULD,i128 定點,CashAccountRules 官方數值全 illustrative 待 IB/EA 现勘;engine ibkr 482;初審 E2/IB现勘 PASS 5 gate CONFIRMED→rework 修 3:MKT buffer 補唯一 fail-open+GFV 語義收緊 unsettled_funded 修誤殺+LULD venue-flag 權威本地 approximate-only→複審 E2 APPROVE+IB PASS 2 DIVERGENT CLOSED);**EA 校準 7 項**(§3.3:offset/LULD tier/結算曆等價/MKT buffer 真機制/unsettled-funded 資料源/halt 資料源/GFV 硬拒 vs strike 姿態)=EA-gated;**W7-S3 收口 R27 PR#85**(三向對賏引擎 P0 核心幻影倉防線:broker 真值×intent journal×本地態,idempotency_key join,P0-A/B/C,**持久凍結 store sticky-until-adjudication**+終態量對賏 i128;engine ibkr 511;全 P0 panel E2/E4/IB/CC 揪 2 真 fail-closed 缺陷閉合=凍結非持久自動解凍+終態量分歧漏判;複審 CC PASS/E2 APPROVE);**S4 blocking acceptance carry**=symbol-binding 保真/order-emit 查 frozen store 當閘/orderRef=idempotency_key wire+Execution.orderRef 捕捉/permId fallback tier/ledger 禁加總 exec;**W7-S4a 收口 R28 PR#88**(option B HMAC 授權核心=`ibkr_effect_activation.rs` HMAC 簽名新軸+金鑰 custody+`check_effect_contact` 放行臂 gate 序 sig 先於 nonce/mint+Paper scope;**production 恆拒維持** stub-only+零 caller DCE+無真金鑰真 envelope=EA5;engine ibkr 532;全授權 panel E2/E3/IB/CC 三腿收斂 signature-coverage gap→canonical payload 15→24 欄補簽 session_fingerprint+risk_hash+3 額度+3 lineage+operator_identity version bump→複審 E3 PASS collision 反證+E2 APPROVE);**W8/EA5 arming 前 blocking**:值綁定 posture 現值比對(簽名軸補齊值綁定歸 W8)/nonce durable ledger/permit 綁 signed limits/OPS nm 捕獲/operator_identity pipe-free 硬 blocker;下一=R29 W7-S4b(IPC+Python thin relay+fake-TWS 全矩陣+S3 全 carry+PaperOrderPreview+10147/10148 分碼+收口,+QA E2E,W7 收口);**D5 已裁(operator 2026-07-17)=KEEP 176**(不降 157)→W5-S2/S3+W6-S1/S3 pinned decoder(≤157 band)EA3 前必對 10.x band(sv158-176)重現勘+重 pin(license-gated 官方源或 EA 捕捉真 wire,blocking);E2-F1 出站注入面已於 S3a 修**;W7-W11 QUEUED。runtime 側教科書式 dormant(07-16 復核):lane 旗標全 false、IB Gateway 未安裝、零 ibkr secret slot、production 從未 seal——**從未發生任何真實接觸**。
...
| **EA3 G4 首次接觸(readonly)** | 前置:AMD-07-08-01 澄清 #3 Operator acknowledgement **已落(2026-07-16)**;production seal(W2 路徑,option A 批准檔;**每次 apply 前歸檔該次批准檔精確位元組——AMD-07-08-01 澄清 #3 第 4 點,審計重算閉合必要條件**)→ readonly envelope + 活化紀錄 → 首次 **health/serverTime** 真讀(嚴格對齊 AMD-07-08 G4 讀集;accountSummary 等歸 EA4 逐項納入) | **一次性顯式批准**(AMD-07-08 G4 語義)+ 活化紀錄 | QA runtime 證據;`session-ready` 達成 |
