...
**版本** v2.2 | **日期** 2026-09-09 | **正本** repo 根目錄 `IBKR_TODO.md`(main 分支;外層 `~/Projects/TradeBot/IBKR_TODO.md` 為鏡像副本,以 main 為準)
**2026-09-09 現況校準**：main 的 W0–W6／W8a／W9a 歷史 source closure 保留；W7 只到 S3，S4 等 fresh source admission。GitHub PR88 已 CLOSED、未 merge，head `36e829cb160fa6bcc357c315824f9cc315ab7efc`；撤銷舊 OPEN／in-flight／自動續 loop 的交接指示。W8–W11 仍依 predecessor 等待。7 月 dormant／零接觸觀察屬歷史，本輪未取得新 runtime／broker attestation。S4 acceptance carry、D8 protocol-band 義務與獨立 activation envelope 全部保留；詳根 TODO 與 [freshness audit](docs/references/2026-09-09--todo-workflow-freshness-audit.md)。

以下 7 月 lineage／逐輪數字是歷史設計與驗收紀錄，不是當前 queue 或跨 session 執行權。
**v2.1 校準 lineage**:2026-07-20 R29 operator 指令校準(比對計劃書與 R9-R27 實況)——補齊 W6 收口/W7 S0-S3 的 S6 回寫欠帳(§4.4/§5 節狀態行/TODO W6 行)、§0 重寫重釘 @ `b486c0718`、§5.5 輪號重排(實績 19 輪/2.5 天)、**07-17 serverVersion 裁決(KEEP 176)因與 §9 D5 撞號改編 D8**(§9 新行,BLOCK-ORDER-BAND-1~5 綁定);R28=W7-S4a in-flight(遠端分支+PR#88 CLOSED_UNMERGED（2026-09-09 已核對）,質量鏈未走完,接棒須 current-source fresh admission；不繼承舊 loop)。
...
- **歷史摘要(2026-07-20 R29 校準,main @ `b486c0718`)**:治理全解鎖(AMD-2026-07-11-01);**已收口九包:W0/W1/W2/W-CI/W3/W4(R8 審計 CONFIRMED)+W5(R9-R13,PR#48/#49/#57/#60/#61)+W8a(R14,PR#63)+W9a(R15,PR#62)+EA3 mini-wiring(R16,PR#66)+W6(R17-R22,PR#69/#70/#76/#77/#78/#79)**,歷輪 INV-1(+R24 起 INV-ORDER)全 HOLDS;**W7 進行中 4/5**:S0 transport-gating 恆拒地基(R24 PR#81)+S1 訂單狀態機/intent journal(R25 PR#82,byte-golden vs pinned 9.81.1 ≤157)+S2 cash 約束 7-gate(R26 PR#83)+S3 三向對賬幻影倉防線(R27 PR#85,engine ibkr 511)已收,**S4=option B HMAC+check_effect_contact 放行臂+IPC+fake-TWS 全矩陣+收口(R28 S4a in-flight 待收割+S4b)**,細節與逐輪 findings 見 §5 W7 節+帳本;W8-W11 QUEUED。**EA1-EA3 零工程等待**(operator 動作+RB-1~RB-5 見 §6;RB-3 簽發程序文件待 authoring);**EA3 前 blocking=D8(KEEP 176,§9):BLOCK-ORDER-BAND-1~5——W5-S2/S3+W6-S1/S3+W7-S1 的 pinned 佈局(byte-verified ≤157)須對 10.x band(sv158-176)重現勘+重 pin**;EA5 前 blocking=W7+W8 全綠+option B+S4 acceptance carry+EA 校準 7 項(§5 W7);W8 吸收票=durable nonce/lineage 現值比對/attestation fingerprint 正本/authenticity 申報(§5 W8)。runtime 教科書式 dormant(07-17 復核:4001/4002/7496/7497 零 listener、~/Jts 缺席、零 ibkr slot、零 seal、lane 旗標全 false)——**從未發生任何真實接觸**。
- **路徑一句話**:剩 W7-S4+W8→W11 四包半把引擎從「全帳戶/行情/訂單狀態機骨架」補成「全生命週期 live-capable 但 default-inactive」;之後 EA1→EA8 活化跑道(全 Operator-gated)按 readonly→paper→evidence clock→tiny-live→live 逐級點火。W8a/W9a 為純 no-contact 開發(AMD-07-11 授權內),先行落地使 EA1-EA3 在 Operator 就緒時零等待點火;EA 時序決策(D2)不因此預決。
...
| account/positions/open orders/executions/commissions + session attestation | ~~缺口~~ **全補(2026-07-17 R11-R13:S2/S3 消化+S4 attestation producer source-landed)** | W5 ✅ |
| contract details/market data/calendar/entitlement | **DONE(2026-07-17,R17-R22)**:S0 必修同票硬化+S1 contract details lane+S3 mktData(delayed-only v1、regulatorySnapshot 源碼封死 FUNDS-LOCKED)+S2 trading calendar(DST gap/fold 鎖死)+S4 provenance calendar_hash 真綁定;三角測量第三腿(W5 carve)閉合 | W6 ✅ |
| order lifecycle(preview/place/cancel/replace/fills/reconcile) | **S0-S3 landed(2026-07-17~18,R24-R27)**:transport-gating 恆拒(INV-ORDER)+狀態機 driver/intent journal(byte-golden ≤157)+cash 7-gate+三向對賬(P0 幻影倉防線,持久凍結 store);**剩 S4**(option B HMAC+放行臂+IPC+fake-TWS 矩陣+收口;S4a in-flight) | W7 🔄 |
...
**進度(2026-07-17 loop R13)**:**W5 全包收口 DONE_SOURCE_SECURED**——S4=PR#61(head `10819d48b`):attestation producer(managed_accounts_inspect 指紋+DU 白名單保守形+SessionAttestationPosture 結構性禁自填+Blocked-until-verified);四腿 E2 APPROVE_WITH_NOTES/E3 PASS(INV-1 HOLDS)/QA ACCEPT_WITH_NOTES/CC PASS_WITH_NOTES(16/16,lineage HOLDS);**re-scope:三角測量第三腿(auth-matrix 接 loader 真值+attested 真消費)移 W6 必修同票**;W6 同票全集見 TODO W5 行;帳本 PROGRESS R13。前史:**S0-S3 DONE(source-landed)**——S3=PR#60(head `6f141b655`):`ibkr_tws_order_exec_data.rs`(exec/commission/orderStatus/openOrder-minimal;哨兵雙判別→None;exec_time UTC grammar config;sv floor 145/ceiling 157 雙界=DIVERGENT-1 裁 (a),10.x band 補證歸 operator);**W6 IPC 投影前必修同票**:Invalidated 恢復/staleness 視圖/reject telemetry/join 緩存 cap;**ApiPending 須 W7 前解**;**EA 前置追加**:TWS API 時間=UTC 模式 operator 欄+exec_time 首批樣本 grammar attestation。前史:**S0+S1+S2 DONE(source-landed)**——S2=PR#57(head `d7d1cec10`):`ibkr_tws_account_data.rs` 消化模組(四 outbound builder/IN 61-64 decode/訂閱狀態機/typed 六態 staleness/G1-G4 fail-closed)+AvailableFunds 契約翻轉(IB DIVERGENT:=EWL−InitMargin 恒先轉負)+fake-TWS 場景+ci.yml ready_for_review types。**W6 IPC 投影前必修**:Invalidated 恢復政策+staleness 綁定視圖+reject telemetry(E2 F3-F5/E3 MED-01);**EA 哨兵校準清單**:exchange 空欄(F9)/G2 哨兵 21 位/3 分鐘節拍。原 R9∥R10 進度:**S0+S1 DONE(source-landed)**——S0=PR#48(head `b7925fdb6`):Layer 3 per-field 窮舉+三枚舉封閉域+parity 反向 superset+enum 共源 lockstep+CI 守衛鏈三洞收口(E2 APPROVE_WITH_NOTES;E3 PASS,W4 移交 E3-F1/F2 雙 CLOSED);S1=PR#49(head `728752a7f`):四 typed row contracts+四配對守衛檔+coverage 守衛 rel-path 修(E2 APPROVE_WITH_NOTES;IB 現勘 9 項全 CONFIRMED)。**S2 前置注意**:AvailableFunds/ExcessLiquidity 符號不對稱入 IB 現勘清單;emitter 側 reconnect_budget_exhausted 終態形態會被 Layer3 攔。**S3 前置 blocking**:max-double 哨兵→None 映射、exec_time 格式×TWS timezone 配置聯 pin(IB 2026-07-16,官方出典見 PROGRESS R10)。
...
### W6 — contract details/market data/calendar/entitlement ✅ **DONE_SOURCE_SECURED 2026-07-17(loop R17-R22,S0-S4)**

**收口紀錄(R29 校準補記)**:S0 IPC 投影前必修同票硬化六項(R17 PR#69;E3 四 finding 家族 CLOSED,三角測量第三腿=W5 re-scope carve 閉合)· S1 contract details lane(R18 PR#70;CONTRACT_DATA message-version==8 硬 pin、contractDetails 最小消化裁決 (b) head-prefix+secIdList bounded skip)· S3a mktData 紅線+契約(R19 PR#76;E2-F1 出站注入修、**regulatorySnapshot 源碼封死+機器守衛(0.01 USD paper 也計費=FUNDS-LOCKED)**、quote/provenance 契約+OUT builders)· S3b engine digest(R20 PR#77;mktData lane 完整)· S2 trading calendar(R21 PR#78;tradingHours/liquidHours 雙 grammar、DST gap/fold 臂鎖死測試)· S4 收口(R22 PR#79;provenance calendar_hash 綁真 identity-row calendar+兩 IPC loose ends)。delayed-only v1 語義成立(降級 opt-in)。**殘項移交**:pinned decoder ≤157 band 受 D8(KEEP 176)約束——EA3 前 10.x band 重現勘+重 pin(BLOCK-ORDER-BAND 家族,§9 D8);W3 移交 F1 historical ordering 已於 S3 消化;EA 哨兵校準面見帳本 R18-R22。

**目標(原始)**:工具識別 + 行情 + 交易日曆 + 權限狀態,全部 point-in-time、可雜湊、可審計。
...
### W7 — order lifecycle(preview/place/cancel/replace + fills + reconciliation;fake-TWS)🔄 **IN_PROGRESS 4/5(R23 設計+R24-R27 S0-S3 收口;剩 S4)**

**進度(2026-07-20 R29 校準)**:設計輪 R23 PR#80(INV-ORDER transport-gating 不變量)→ **S0** 恆拒 stub 地基(R24 PR#81:OrderFrame newtype+OrderEffectPermit mint `#[cfg(test)]`+單一 send_order_framed+恆拒 stub+4 structure guard;E2/E3/IB/CC 四腿過,INV-ORDER+INV-1 HOLDS)→ **S1** 訂單狀態機 driver+intent journal 不送出(R25 PR#82:14-態+唯一 mutator reduce-only fail-closed+idempotency_key join+ApiPending transient 分流+sealed WireBytes;**IB 揪 2 CONFIRMED DIVERGENT wire 缺陷 rework 閉合,placeOrder byte-golden vs pinned 9.81.1 ≤157**)→ **S2** cash 約束引擎(R26 PR#83:7-gate settled-funds T+1/GFV/no-short/RTH/白名單/fractional/LULD,i128 定點;官方數值 illustrative,**EA 校準 7 項**=EA-gated)→ **S3** 三向對賬引擎(R27 PR#85:broker 真值×intent journal×本地態,P0-A/B/C,**持久凍結 store sticky-until-adjudication**+終態量對賬 i128;P0 panel 揪 2 真 fail-closed 缺陷閉合)。**S4 blocking acceptance carry**:symbol-binding 保真/order-emit 查持久 frozen store 當閘/orderRef=idempotency_key wire+Execution.orderRef 捕捉/permId fallback tier(orderRef>permId 非零>order-id)/ledger 禁加總 exec shares。**R28 S4a 歷史候選(option B HMAC authority 核心),當前 WAITING_FRESH_ADMISSION:遠端分支 `agent/ibkr-w7-s4a-option-b-hmac`(head `36e829cb1`)+PR#88 CLOSED_UNMERGED（2026-09-09 已核對）,R28 記帳 commit 已在支線但質量鏈未走完——fresh admission 後先核對 source 差異與 cargo 測試再續審(E2/E3/CC/IB 必須走完),禁憑半成品自報合入;S4b=check_effect_contact 放行臂+IPC preview/submit/cancel/replace+fake-TWS 全矩陣+W7 收口。**

**目標(原始)**:完整訂單狀態機與對賬,paper/live capable、default-inactive,全部用 fake-TWS 驗證。**本包是效果面核心,also 是 HMAC 升級觸發點。**
...
**定位紀律**:純 no-contact 開發,在 AMD-07-11 development 授權內;**D2 已裁採用(2026-07-16)**——本切片收口即為 EA1-EA3 點火前置之一,活化仍逐步 Operator-gated;W8 全包落地時**吸收**本切片(共用同一驗證代碼路徑,禁兩套語義漂移——同 W2「caller 與 consume 共路徑」原則)。落地本切片的唯一目的=讓 EA1-EA3 在 Operator 就緒時零工程等待。
...
### 5.5 剩餘工程日誌安排(R29 重排 2026-07-20;live-ready 為止;R8 原表見 git 歷史)

```
[✅W2 ✅W-CI ✅W3 ✅W4 ✅W5 ✅W8a ✅W9a ✅EA3-wiring ✅W6]
W7(S0-S3 source closed → S4a WAITING_FRESH_ADMISSION → S4b 未收口)── W8 ── W9 ── W10 ── W11
```

| 輪 | 切片 | 注記 |
|---|---|---|
| R9-R27 | 已消化:W5(R9-R13)/W8a∥W9a(R14/R15)/EA3 mini-wiring(R16,插片)/W6(R17-R22,S0 為必修同票插片)/W7 設計+S0-S3(R23-R27) | 實績 19 輪/約 2.5 天;R8 原排 R16-R18=W6、R19-R22=W7,插片致輪號 +4~6,日曆反而領先 |
| R28 | **W7-S4a** option B HMAC authority 歷史候選(PR88 closed unmerged) | 接棒驗屍→補全質量鏈→合入;其 R28 帳本行隨其 PR 落帳 |
| R29 | 本校準輪(docs-only,operator 指令) | §7 白名單輪,不計零代碼計數 |
| R30± | **W7-S4b**(check_effect_contact 放行臂+IPC 方法矩陣+fake-TWS 全矩陣)→ **W7 收口** | S4 blocking acceptance carry 全清;E3+CC 必審 |
| R31-R33 | **W8**(envelope 全量 承 W8a/option B 活化紀錄/風控/kill switch/audit)→ W8 收口 | 吸收票全清(durable nonce/lineage 比對/attestation fingerprint/authenticity)+reactivation 重置+permit audit 第二階段(TCP factory 面) |
| R34-R37 | **W9**(DB migration **D1 裁決**/FastAPI/GUI Phase 4/service/observability/rollback)→ W9 收口 | XL;GUI 面承 W4 defer;D1 撞到即 WAITING+通知 |
| R38 | **W10** Mac 全量+fake-TWS E2E+Linux inactive deploy+QA+三端 | operator 開 deploy 窗 |
| R39 | **W11** 對抗性全域 rescan → 終態宣告 | gap matrix 只許外部殘項 |

- **工期估計(R29 修訂)**:剩餘 ~11 輪;實證 cadence R9-R27=19 輪/約 2.5 天(含 XL 切片與全 P0 panel 審)⇒ **工程側 live-ready ≈ 2-5 個有效工程日**(變異源:reviewer/E4 帶寬、agent 配額死亡、operator 決策窗 D1/deploy 窗、CI spending;W8/W9 的 authority/runtime 面審查密度高於均值,輪數或膨脹)。EA 疊加軸不變:**paper/shadow evidence clock(6-8 週)仍是最大 wall-clock 項**,W7+W8+option B 齊備即申請 EA5 開窗。
- **EA 疊加時間軸(D2 已採用 2026-07-16;各週標記按樂觀端 ~2 輪/天,悲觀端順延 ×1.6)**:W5+W8a+W9a 齊(≈第 1-1.5 週末)→ EA1-EA3 點火(全 Operator 動作,屆時逐步活化);EA4 readonly soak 7-14 天與 W6/W7 開發平行;W7+W8+option B 齊(≈第 3 週)→ 申請 EA5 開 paper 窗 → evidence clock 6-8 週起跑;工程終態宣告(W11)與 clock 平行,tiny-live(EA7)最早 ≈ clock 開窗後 6-8 週(受 OPEN-GOV-1/D7 保守讀法約束)。
...
| **EA3 G4 首次接觸(readonly)** | 前置:AMD-07-08-01 澄清 #3 Operator acknowledgement **已落(2026-07-16)**;**D8 blocking(R29 校準入表):BLOCK-ORDER-BAND-1~5——W5-S2/S3+W6-S1/S3+W7-S1 的 pinned 佈局(byte-verified ≤157)須對 10.x band(sv158-176)重現勘+重 pin——**EA3 前唯一可完成路徑=license-gated 官方 ibapi 10.x 源導入;EA 窗真 wire 捕捉屬 EA3 活化後的顯式授權校準步(捕捉本身=接觸),不得倒置為 EA3 前置的滿足手段**(見 §9 D8)**;RB-1~RB-5 runbook(RB-3 envelope/epochs 簽發程序文件待 authoring);production seal(W2 路徑,option A 批准檔;**每次 apply 前歸檔該次批准檔精確位元組——AMD-07-08-01 澄清 #3 第 4 點,審計重算閉合必要條件**)→ readonly envelope + 活化紀錄 → 首次 **health/serverTime** 真讀(嚴格對齊 AMD-07-08 G4 讀集;accountSummary 等歸 EA4 逐項納入) | **一次性顯式批准**(AMD-07-08 G4 語義)+ 活化紀錄 | QA runtime 證據;`session-ready` 達成 |
...
**平行化建議(R8 修訂,以本段為準)**:W8a/W9a 兩個 carve 件已入 §5 排程(R14/R15);**D2 已裁採用(2026-07-16,Operator)**——W5+W8a+W9a 齊備即開 EA1-EA4,每步仍逐一活化。**EA1-EA3 前置 = W3-W5 + W8a + W9a**;W8a/W9a 未齊則 EA1-EA3 前置回落完整 W8/W9。EA 各步 W 前置總表:EA5 = W7+W8 全綠 + option B(建議含 W9);EA7+ = W7-W11 全綠。
...
| D2 | 唯讀跑道提前並行(EA1-EA4 提前點火) | **已裁:採用(2026-07-16,Operator)** | W5+W8a+W9a 齊備即開 EA1-EA4,與 W6/W7 平行;**每步仍逐一 Operator 活化**(EA1 憑證/EA2 Gateway 登入/EA3 一次性批准/EA4 soak 每日活化);EA4 負擔重時可開 OPEN-GOV-2 裁排程性重連授權;D3(專用 paper username)於 EA1 前一併定,建議拆 |
...
| D8 | serverVersion 協商上限:KEEP 176 vs 降 157 | **已裁:KEEP 176(2026-07-17,Operator)** | 後果=W5-S2/S3、W6-S1/S3、W7-S1 的 pinned wire 佈局(byte-verified ≤157)須於 **EA3 前**對 10.x band(sv158-176)重現勘+重 pin(**BLOCK-ORDER-BAND-1~5**;證據路徑:①license-gated 官方 ibapi 10.x 源導入=**EA3 前唯一可完成路徑**;②EA 窗真 wire 捕捉=**EA3 活化後**於 envelope 顯式授權 scope 內的校準步——捕捉本身即接觸,不得倒置為 EA3 前置滿足手段;完成前受影響 158-176 decode 面按 fail-closed 不得依賴,readonly 首讀集是否受影響由 IB 於 EA3 設計時裁定)。**注**:本裁決於 R25-R27 帳面曾誤記為「D5」,與本表 D5(策略 universe)撞號,R29 校準統一改編 D8;帳本歷史行按 append-only 不改寫,以本行為準 |
