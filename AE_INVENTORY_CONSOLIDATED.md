# 玄衡 · Arcane Equilibrium 系統盤點統合報告（5-Session Consolidated Inventory）
...
> **2026-06-10 校準**：上表為 2026-04-25 快照值。2026-06-10 實測：`find rust -name "*.rs" -not -path "*/target/*"` = **558 檔 / 271,701 LOC**；`find program_code -name "*.py"`（排 `__pycache__`/`venv`）= **783 檔 / 311,686 LOC**。增幅來自 ~6 週重構拆檔（ipc_server / tick_pipeline / passive_wait_healthcheck 等拆分）與 L2 / AEG / ML 新模組；注意口徑不同——快照「~357 個」為人工篩選的**產品向模組**數（Python 面含 helper_scripts 等、不逐檔計），實測為原始檔案數，兩者不可直接相減。
...
> **2026-06-10 校準**：檔案級事實重驗仍成立（`change_audit_log.py` + `audit_persistence.py` 兩檔 0 個 `INSERT INTO`），但 agent 平面其後新增 durable PG 落點 `agent_event_store.py`（2026-05-06，commit 91379cd2）寫 `agent.messages` / `agent.state_changes` / `agent.ai_invocations`；詳見紅色 #3 注記。
...
> **2026-06-10 校準**：本速覽中行數與「startup-only」均為快照值——edge_estimates 熱重載已於 2026-04-26 落地（F6 PH5-WIRE-1 RELOAD：1h 週期 reload daemon + `reload_edge_estimates` IPC）；各檔行數現值見 Part 4 複雜度熱點下的校準注記。
...
> **2026-06-10 校準**：**已閉環（hardcode 面）**——`_shadow_mode=True` 寫死已移除，改為 G3-03 Phase B（2026-04-25/26）`ExecutorConfigCache` + `shadow_mode_provider()` 於執行時讀 Rust `RiskConfig.executor` 權威（executor_agent.py 註明 "`_shadow_mode: bool = True` has been removed"，並有 `_read_canary_stage()` 分級）。Executor 真實下單權限仍未開啟，屬治理決策非 bug（README 2026-05-15/23 口徑：`OPENCLAW_LEASE_ROUTER_GATE_ENABLED=1` ≠ Executor order authority）。
...
> **2026-06-10 校準**：**已閉環**——F6 PH5-WIRE-1 RELOAD（2026-04-26）落地雙路徑熱重載：1h 週期 reload daemon（`main_boot_tasks::spawn_edge_estimates_reloader_if_enabled`）+ 手動 `reload_edge_estimates` IPC（`event_consumer/handlers/edge_estimates.rs`）。另 cold audit P1-09（2026-05-29，commit 11b9531f）加 freshness gate：edge 估計須 fresh（≤48h TTL）+ runtime-derived，live 過期 fail-closed reject。
...
> **2026-06-10 校準**：**部分處理**——`change_audit_log.py` + `audit_persistence.py` 本體 2026-06-10 重 grep 仍 0 個 `INSERT INTO`，建議的 `governance.agent_audit_log`（V025）未建；但 agent 平面已另有 durable PG 落點：`agent_event_store.py`（2026-05-06 接線）寫 `agent.messages` / `agent.state_changes` / `agent.ai_invocations`，且 cold audit P1-13（2026-05-29）令 paid AI call 寫 durable ledger fail-closed。殘餘缺口縮小為 change_audit_log 自身仍 in-memory + JSONL。
...
> **2026-06-10 校準**：**已閉環（原反向已不存在）**——舊 PostOnly 鍵已被 EDGE-P2-3 重構為 per-strategy `use_maker_entry`（entry-only PostOnly Limit）：現值 demo 3 處全 true；live ma_crossover/grid_trading=true、bb_breakout=false 且 TOML 注釋明文「live stays on Market (root principle #6)，demo 驗證正 edge 後再開」——demo 先驗證、live 保守，方向正確非反向。樞紐治理面：cold audit P2-09（2026-05-29）已把 Guardian scoring 權重/門檻抽為 invariant named const + lock tests；原建議的 TOML pre-commit invariant linter 未見對應實作。
...
> **2026-06-10 校準**：**大部分閉環**——FA-PHANTOM-1 修後 `margin_utilization_pct` 已 leverage-aware（fast_track.rs 注釋明載）；FA-PHANTOM-2（microcap 噪音誤觸）追加修復並有回歸測試 `test_fa_phantom_2_regression_microcap_noise_no_action`；drop 閾值已抽為可配置（commit 8df29e9e "risk: expose fast track drop thresholds"）並以 sigma 區分真離群與 microcap 常態噪音。設計性 Guardian 旁路按設計保留；原建議的「單次平倉 ratio 上限超出退回 Guardian」未按原樣實作。
...
> **2026-06-10 校準**：本工作量/排程估算僅作史料——5 項已於 2026-04-25 ~ 2026-05-29 間分別閉環或部分處理（見上方各條注記），未按本表排程執行。
...
> **2026-06-10 校準**：本分批計劃已被實際進度取代：紅 #2（F6 RELOAD 2026-04-26）、紅 #1（G3-03 Phase B 2026-04-25/26）、紅 #4（EDGE-P2-3 `use_maker_entry` 重構）、紅 #5（FA-PHANTOM-2 + 閾值配置化）已落地；紅 #3 部分處理（`agent_event_store.py` PG 落點已有，change_audit_log 本體仍 JSONL）。逐條證據見 §2 注記。
...
> **2026-06-10 校準**：本節對 CLAUDE.md §二/§三 的「改寫位置」已 stale——CLAUDE.md 其後全面重構：§三 現為 Active State Routing（active state 移 TODO.md），16 條根原則保留於 §二。建議 1 的觸發案例（edge_estimates 熱重載）已修；建議 2 對應缺口仍部分開放（見紅 #3 注記）；治理修訂現行載體為 `docs/adr/` + `docs/governance_dev/amendments/`（如 AMD-2026-05-15-01 凍結 paper promotion、AMD-2026-05-31-01 / ADR-0047 alpha 證據治理）。
...
> **2026-06-10 校準**：表中多列其後已演進——ExecutorAgent shadow 已 G3-03 config 化、PostOnly 已重構為 `use_maker_entry`、edge_estimates 已熱重載（各見 §2 注記）；`step_4_5_dispatch.rs` 現路徑為 `tick_pipeline/on_tick/step_4_5_dispatch.rs`（cold audit P2-11 已拆出 inline tests，2020→1803 行；2026-06-10 實測 1813 行）。
...
> **2026-06-10 校準**：多項其後已解：**#5** change_audit_log 至 2026-06-10 重 grep 仍 0 INSERT INTO（紅 #3 注記）；**#6** `engine_maintenance.flag` CREATE 點已定位——`helper_scripts/clean_restart.sh:238` 與 `fresh_start.sh:230` 的 `touch "$MAINT_FLAG"`，`restart_all.sh:488` 負責清除；**#9** strategist 自動 tune 邊界已成文——`strategist_scheduler/mod.rs` 檔頭 R3-4：param_ranges bounds + weight sum=65±0.1 + delta clamp（默認 ±50%，`RiskConfig.strategist.max_param_delta_pct`）；**#10** edge_estimates 熱重載已落地（F6 PH5-WIRE-1 RELOAD，2026-04-26）；**#14/#15** 其後兩輪 cold audit（2026-05-29 closure；2026-05-30 PM final：0 P0、0 P1）及 P2-11 記錄的 3599 tests 已取代本節對測試與工作量的不確定性。
...
> **2026-06-10 校準**：上行「~180 檔 / 131,120 LOC」為 2026-04-25 快照值。2026-06-10 實測：`find rust -name "*.rs" -not -path "*/target/*"` = **558 檔**，`wc -l` 合計 **271,701 LOC**（口徑：含測試/註釋/空行，不含 target/）。crate 級：openclaw_types 9 檔、openclaw_core 44 檔、openclaw_engine 505 檔，另有 schemas/（無 .rs）——**3-crate workspace + schemas/ 結構與本節敘述一致，未變**。抽查 8 個 Rust 條目：`h0_gate.rs` 1067→1243（+16%，OK）、`main.rs` 2062→1667（−19%，OK）、`bybit_rest_client.rs` 1725→1591（−8%，OK）、`tick_pipeline/on_tick/step_4_5_dispatch.rs` 935→1813（+94%）、`intent_processor/mod.rs` 1100→1968（+79%）、`core/src/dream.rs` 已刪除、`ipc_server/mod.rs` 1192→155（已拆分）、`strategist_scheduler/mod.rs` 1166→495（已拆分）。本節 357 模組明細表為 2026-04-25 快照，**不逐條更新**；新增/變更模組以 `docs/audit/inventory_summary.md` 與代碼為準。另注意：表內「§九 1200 硬上限」為快照期口徑，現行 CLAUDE.md §九 為 800 行需注意 / 2000 行硬上限。
...
> **2026-06-10 校準**：本批模組集已變動。已自 openclaw_core 移除：`dream.rs`、`attention.rs`、`attribution.rs`、`cognitive.rs`（types/src/cognitive.rs 仍在）、`message_bus.rs`、`opportunity.rs`（engine/scanner 下另有同名新檔）、`order_match.rs`——repo 內無同名遷移檔，屬刪除。新增：`alpha_surface.rs`、`governance_emit.rs`、`lease_scope.rs`、`hot_path_metrics/`、`m4_miner/`。openclaw_core 現為 44 個 .rs 檔；下表相應行視為快照。
...
> **2026-06-10 校準**：`ipc_server/mod.rs` 已完成快照中預告的拆分：1192→155 行，新增 `server.rs`/`connection.rs`/`dispatch.rs`/`protocol.rs`/`method_registry.rs`/`engine_routing.rs`/`slots.rs` 等兄弟模組。`on_tick/step_4_5_dispatch.rs` 935→1813 行（快照「不可再拆」注記下持續增長）。下表行數為 2026-04-25 快照值。
...
> **2026-06-10 校準**：`intent_processor/mod.rs` 1100→1968 行（快照已標「需拆」，現逼近現行 §九 2000 硬上限）。下表行數為 2026-04-25 快照值。
...
> **2026-06-10 校準**：`strategist_scheduler/mod.rs` 1166→495 行，已拆出 `evaluate.rs`/`cycle_counters.rs`（`persist.rs` 原列）。`bybit_rest_client.rs` 1725→1591 行（−8%）。下表行數為 2026-04-25 快照值。
...
> **2026-06-10 校準**：同口徑重測為 **271,701 LOC / 558 檔**（2026-04-25 快照值 131,120 / ~180 檔保留如上）。test/prod 拆分比未重測。
...
> **2026-06-10 校準**：上行為 2026-04-25 快照值（且僅計被盤點子集，如 helper_scripts 只取 17 個非一次性檔）。2026-06-10 全目錄實測（`find <dir> -name "*.py"` 排除 `__pycache__`/venv，wc -l 含註釋/空行）：app/ **219 檔 109,957 LOC**、ml_training/ **87 檔 35,193**、local_model_tools/ **36 檔 6,748**、helper_scripts/ **308 檔 109,597**、readonly_observer_pipeline/ **17 檔 2,700**、io_and_persistence/ **15 檔 2,372**、audit/ **4 檔 1,199**，同範圍合計 **≈267,766 LOC**；兩者口徑不同（子集 vs 全目錄），不可直接相減。program_code 全樹另含 ai_agents/learning_engine/market_data_processor/research 等快照外目錄，總計 783 檔 311,686 LOC。抽查 7 個 Python 條目：`james_stein_estimator.py` 555→642（+16%，OK）、`local_model_tools/backtest_engine.py` 87→87（OK）、`risk_manager.py` 52→52（OK）、`governance_hub.py` 1014→1483（+46%）、`live_session_routes.py` 1449→720（已拆分）、`ai_service.py` 1258→250（已拆分）、`passive_wait_healthcheck.py` 1822→36（已 package 化）。本節明細表為 2026-04-25 快照，**不逐條更新**；新增/變更模組以 `docs/audit/inventory_summary.md` 與代碼為準。
...
> **2026-06-10 校準**：舊獨立 Python `governance/`、`risk_control`（H0）、`trade_executor`（Decision Lease）目錄已遷移至 Rust（openclaw_core: `governance_core.rs`・`h0_gate.rs`・`sm/risk_gov.rs`）且 Python 目錄已刪除（README「项目结构」段聲明；2026-06-10 `ls program_code/governance|risk_control|trade_executor` 確認 GONE）。本批所列 `app/*.py` 治理檔案（governance_hub / 3 SM / h0_gate 等）**仍存在**，屬控制面側 Python 鏡像，Rust 為權威；`governance_hub.py` 1014→1483 行（+46%），3 個 SM 檔與 `h0_gate.py` 行數 ±5% 內。
...
> **2026-06-10 校準**：`live_session_routes.py` 已完成快照中「超 1200 硬上限須拆分」的拆分：1449→720 行，新增 `live_session_endpoints.py`、`live_session_account_routes.py` 兄弟檔。下表行數為 2026-04-25 快照值。
...
> **2026-06-10 校準**：`multi_agent_framework.py` 1137→535 行（已拆分/縮減）；`executor_agent.py` 630→1031 行（+64%，shadow→live 接線演進後增長，現況以代碼為準）。下表行數為 2026-04-25 快照值。
...
> **2026-06-10 校準**：`ai_service.py` 已完成快照中「超 1200 硬上限須拆分」的拆分：1258→250 行，新增 `ai_service_dispatch.py`、`ai_service_guardian.py`、`ai_service_listener.py` 兄弟檔（`ai_service_feedback.py` 原列）。下表行數為 2026-04-25 快照值。
...
> **2026-06-10 校準**：README「项目结构」現將整包定位為「策略工具包 stub-shim（计算已收编 Rust，仅保留 import 表面）」。實測 2026-06-10：36 檔 6,748 LOC（快照 27 檔 ≈3,090）；表列真實計算檔 `cognitive_modulator.py`（197）/`evolution_engine.py`（567）/`ewma_vol_estimator.py`（159）/`hurst_exponent.py`（164）仍含實際計算代碼、行數與快照一致，stub 判定以各檔內容為準。
...
> **2026-06-10 校準**：`passive_wait_healthcheck.py` 已完成快照中「遠超硬上限須拆分」的拆分：1822→36 行薄入口，主體移入 `helper_scripts/db/passive_wait_healthcheck/` package（`checks_*.py` 分模組）。`counterfactual_exit_replay.py` 1216→1119（−8%）。helper_scripts 全目錄現有 308 個 .py（快照僅盤點 17 個非一次性檔）。下表行數為 2026-04-25 快照值。
...
> **2026-06-10 校準**：同範圍目錄全量重測 ≈267,766 LOC（口徑差異見本節開頭注記）。上方「§九 超 1200 硬上限」清單為快照期狀態：`live_session_routes.py`（現 720）、`ai_service.py`（現 250）、`passive_wait_healthcheck.py`（現 36）均已拆分達標；`multi_agent_framework.py` 現 535、`strategist_agent.py`/`governance_hub.py` 等以現行 §九 口徑（800 注意 / 2000 硬上限）重新評估，以代碼為準。
...
> **2026-06-10 校準**：Paper pipeline 已硬性封存（2026-05-23 `c4e1411d`，承 05-15 canary freeze）：`main_pipelines.rs:221-230` 無條件 `paper_enabled=false`，`OPENCLAW_ENABLE_PAPER=1` 被忽略並 warn「paper pipeline is archived; use Replay Stage 0R + Demo micro-canary」。本流現役引擎=Demo/LiveDemo（Live 受 5 重門控）；Step 5 Gate 5 ε-greedy（PAPER ONLY）隨之結構性休眠。Paper 僅餘 Archive/replay 診斷用途（CLAUDE.md §四）。
...
1. **WS 接收與解析**
   - 位置：`srv/rust/openclaw_engine/src/ws_client/`（單檔已拆為模組：connection.rs / parsers.rs / run_loop.rs / dispatch.rs / stats.rs；解析入口在 parsers.rs）
   - 輸入：Bybit V5 WS JSON
...
4. **TickPipeline 7 step 編排**
   ...
   - 子步驟：
     ...
     - **Step 4+5**：[step_4_5_dispatch.rs:1-935](srv/rust/openclaw_engine/src/tick_pipeline/on_tick/step_4_5_dispatch.rs)（現 ~1813 行，tests 已拆至 step_4_5_dispatch_tests.rs；2026-04-25「935 行」快照已失效）每策略 `Strategy::on_tick()` → `StrategyAction` enum → 構造 `OrderIntent` → 呼 `IntentProcessor::process_with_features()` → 收集 `Vec<OrderIntent>`（已風控通過）
...
7. **OrderManager 驗證 + REST 提交**
   - 位置：[srv/rust/openclaw_engine/src/order_manager.rs](srv/rust/openclaw_engine/src/order_manager.rs)（已瘦身至 ~997 行，回到 §九 上限內；tests 拆至 order_manager_tests.rs）
...
> **2026-06-10 校準**：① hardcode 已移除——G3-03 Phase B 改為 `canary_stage_provider`/`shadow_mode_provider`（Rust IPC 快取 `RiskConfig.executor.shadow_mode` 為 SoT，無 provider 時 fail-closed shadow=True；`strategy_wiring.py:530-534`）。運行默認仍 shadow，「Rust hot-path=唯一活躍下單路徑」結論不變。② 補強（2026-06-05 事故結構真相）：demo/live close-all 與 orphan-sweep 100% 走 Rust IPC（`close_all_positions`/逐倉 `close_position`；live REST 兜底被 P1-03 永久禁）→ **引擎掛=GUI 零平倉路徑**，端點仍回 HTTP 200。
...
> **2026-06-10 校準（PHANTOM-FILL-FIX-1，2026-06-08 已部署）**：TON 幻影倉事故揭露 PositionUpdate/Fill 無序雙寫競態。修法：① `apply_fill` 成為倉位**唯一 mutating 源**，新入口 `apply_fill_with_close_semantics(is_close)`（fill_engine.rs:315）——reduce-only/close 在本地無倉時 fail-closed no-op，真翻倉用餘量建反向倉；② WS `PositionUpdate` 降為 advisory 只讀比對（loop_exchange.rs:534-）；③ `position_reconciler` 新增幻影偵測軸（presence+side，只告警不收斂，寫 `observability.engine_events` type=`reconcile_phantom_local`+canary）。commit `74b2e264` → main `bdf15e4f`，trade-core 運行中。
...
> **2026-06-10 校準**：註冊點已可直接定位——`startup/private_ws.rs:167` per-pipeline supervisor 內 `listener.set_on_fill(...)`，fill 經 event consumer → `tick_pipeline/commands.rs`（apply_fill 前捕獲 entry_context_id/snapshot）→ `fill_engine.apply_fill_with_close_semantics`。「未驗」標記可移除。
...
- (B) **Layer 2 手動觸發**：Python `app/layer2_routes.py` POST `/api/v1/paper/layer2/trigger`（router prefix=`/api/v1/paper/layer2`；「paper」僅為歷史命名，paper promotion lane 已凍結不影響此 L2 推理入口） → `Layer2Engine` 自主深度推理（不經 Rust）。
...
> **2026-06-10 校準（G5-04 拆分，2026-04-24）**：`AIService` 本體已搬出——`ai_service.py` 現為 ~250 行 facade（常數/LLM 單例/socket 解析/factory，re-export 向後相容）；dispatch 與 5 handler 在 `ai_service_dispatch.py`（registry :111-115：strategist_evaluate/analyst_evaluate/conductor_evaluate/scout_scan/guardian_check；`dispatch()` :130；`_handle_strategist` :193），listener 在 `ai_service_listener.py`。本步原列 ai_service.py 行號全部失效。
...
9. **成本記錄 → IPC `record_ai_usage` ↕ IPC edge**
   ...
   - IPC method：`record_ai_usage`（呼叫點已抽至 [layer2_cost_recording.py:230](srv/program_code/exchange_connectors/bybit_connector/control_api_v1/app/layer2_cost_recording.py)；ipc_client.py 不再直接持有此 method 名）
...
7. **Rust 啟動時讀（一次性，**無 hot reload**）↕ IPC edge（檔）**
   ...
   - **⚠️ 關鍵限制（2026-06-10 更新）**：hot-reload 機制已建（F6 PH5-WIRE-1 RELOAD，commit `ccd7d266`）——週期 reload daemon（預設 1h，下限 60s）+ 手動 IPC `reload_edge_estimates`（ipc_server/dispatch.rs:473），fan-out `PipelineCommand::ReloadEdgeEstimates` 到各 pipeline，各引擎讀自己模式對應 JSON（模式隔離：paper→`edge_estimates_paper.json`，demo/live→`edge_estimates.json`，`load_for_mode()`）。**但嚴格 env-gate `OPENCLAW_EDGE_RELOAD=="1"` DEFAULT-OFF**——flag 未開時行為仍是 startup-only 讀一次。原 `set_edge_estimates()` 函數名已不存在（現 `load_from_file`/`load_for_mode`）。
...
> **2026-06-10 校準**：本流（小時級 JS edge 估計 → cost_gate）之外，learning 面新增一條**平行車道**：residual alpha producer（2026-06-07 上 main 部署，daily ML cron 03:17 帶 `OPENCLAW_RESIDUAL_ALPHA_PRODUCER=1` 啟用——engine/API 進程內 OFF，僅 cron 環境 ON），attach residual/signal_spec/hidden_oos 證據到 mlde shadow recommendations（06-08 cron log：attach 7/7→9/9）。它不寫 `edge_estimates.json`、不進 cost_gate，與本流無耦合；PART 4 gap-closure 已部署但 flag-OFF。另：ML training cron 為 hybrid——5 個 training job DAILY + 5 個 audit job 每日 fire 但 weekday=6 gate（非「全 weekly」）。
...
> **2026-06-10 校準（OPS-2 SECRET-SPLIT）**：簽名 key 已從共用 `OPENCLAW_IPC_SECRET` 切到專用 `OPENCLAW_LIVE_AUTH_SIGNING_KEY`（Python `live_trust_routes.py:71-89` primary + IPC_SECRET 為 Phase 1 fallback；Rust `live_authorization.rs` 同序解析）。D+14 證據已達成，Phase 2 cutover（移除 fallback）in-flight（branch `fix/ops2-phase2-cutover`）。另一 runtime 注記：LiveDemo pipeline 自 2026-06-06 起因缺 `authorization.json` 拒啟（既有狀態，恢復需 POST /api/v1/live/auth/renew）；demo 不受影響。
...
> **2026-06-10 校準**：`_shadow_mode=True` hardcode 已由 G3-03 Phase B 移除——ExecutorAgent 改 `canary_stage_provider`（W-AUDIT-9 T3 SoT）/`shadow_mode_provider`（Rust IPC 快取 `RiskConfig.executor.shadow_mode`），無 provider 時 fail-closed 回 shadow=True；StrategistAgent 已 shadow=False（Sprint 5a，`strategy_wiring.py:253`）。運行默認 Executor 仍 shadow，故本斷鏈點的「症狀」描述仍適用，但成因從「hardcode」改為「Rust 權威配置默認 shadow」。
...
> **2026-06-10 校準**：此斷鏈點已閉合——repo 現有 `helper_scripts/systemd/openclaw-watchdog.service` + `openclaw-engine.service`（另 tls-renew/listing-collector 等），Linux runtime 實跑 `openclaw-watchdog.service`（systemd --user）。遺留 companion：unit 模板漂移（缺 Restart=always）待重裝（2026-06-05 事故 follow-up）。
...
> **2026-06-10 校準（2026-06-05 bind-host 宕機事故後，commit `072b8e20`+`92cdcc41` 均在 main）**：① watchdog 自愈由 edge-trigger 改 **level-trigger**（restart 持續失敗時每 poll 經 `should_restart()` 重試，修 first-detection deadlock）；② `trigger_restart` subprocess env sanitize——`OPENCLAW_BIND_HOST` 落在 DANGEROUS_BIND_HOSTS（0.0.0.0/::）強制改 `auto`（engine_watchdog.py:642-657），restart_all.sh 同樣經 `lib/api_bind_host.sh` 守衛拒 0.0.0.0；③ 「canary_events.jsonl 無告警消費者」缺口已 code-閉合：GUI 可配置 Telegram/Webhook 告警（`app/alert_config.py`+watchdog best-effort sender，no-op until operator 填 creds）。本流行號已大幅漂移（classify_engine_failure 現 :345、run_watchdog :1429、maintenance flag 清除 restart_all.sh:488），鏈路結構不變。
...
3. Score 5 fitness fns ([scanner/scorer.rs](srv/rust/openclaw_engine/src/scanner/scorer.rs)) — F_ma / F_grid / F_bbrv / F_bkout / F_funding_arb + edge bonus + correlation filter（注：funding_arb 策略本身 2026-05-02 已走棄用路徑，fitness fn 仍在 scorer 內；週期由 config `scan_interval_secs` 控制，預設 1800s=30min）
...
4. **Pipeline orchestrate**：news/pipeline.rs → DB 寫入 `market.news_signals`（pipeline.rs INSERT 已驗，原「推測 news.events」不成立）
...
> **2026-06-10 校準（全段適用）**：本 Part 為 2026-04-25 快照。CLAUDE.md 已於 2026-05-31 大幅重構：16 條根原則仍在 §二（編號 1-16 不變）但措辭改為精簡英文版（如 #2「mostly read-only」、#7「Learning must not rewrite live state directly」、#12「evolve from evidence, not anecdotes」、#13 刪去 cost_edge_ratio≥0.8 字面、#15 增「Conductor is orchestration, not a sixth trading agent」）；以下各條「原文」均為 2026-04-25 引用版，現行措辭以 CLAUDE.md §二 為準、硬邊界以 §四 為準（5-gate true-live 口徑：Python `live_reserved`＋Operator role auth＋`OPENCLAW_ALLOW_MAINNET=1`＋valid secret slot＋signed 未過期 `authorization.json` 環境匹配）。系統總稱現為「玄衡 · Arcane Equilibrium」（2026-05-06 軟更名），OpenClaw 僅指控制平面服務族，運行面名稱不變。另：2026-05-30 cold audit re-run 結論 **P0=0/P1=0**——安全關鍵面（5-gate live 邊界/authority chain/GUI write-through-Rust/fail-closed retCode/ML deploy-stage gating）全過對抗審計（根目錄 `2026-05-30--cold_audit_pm_final.md`），本 Part 各「缺口」應在該結論下重讀。
...
> **2026-06-10 校準**：(1) ExecutorAgent shadow 已非 hardcoded——AMD-2026-05-09-03 graduated canary＋SM-05 後改由 `canary_stage_provider`（CanaryStage enum，executor_config_cache）stage-aware 驅動，runtime 仍 shadow 默認（TODO §0）。(2) 寫入口唯一性曾在 fill 記帳層真實失守：2026-06-08 demo TON 幽靈倉根因＝PositionUpdate/Fill 無序雙寫競態，修復後 `apply_fill` 為 paper_state 唯一 mutator＋reduce-only fail-closed＋reconciler 幻影偵測軸（commit `74b2e264`，已部署）。(3) close-all 平倉路徑 2026-06-05 後全走 Rust IPC（引擎掛＝GUI 零平倉路徑，SPOF 注記）。(4) SM 單源收斂已拍板 Option 2（Rust 唯一權威），step-i Rust IPC `a99bfa1d` 已落，cutover operator-gated。
...
> **2026-06-10 校準**：PostOnly demo/live 反向已修——EDGE-P2-3（2026-04-20 起多 phase）demo 與 live TOML 均改入場 PostOnly maker-entry 路線（`use_maker_entry` per-strategy 配置，demo 先驗證、live 同步啟用），原「demo=false/live=true」二元反向已不存在。另現行 CLAUDE.md §二 #2 措辭為「research, GUI, and learning are **mostly** read-only」；SM 收斂 Option 2 將 Python SM 降為唯讀投影（soak 進行中，cutover operator-gated）。
...
> **2026-06-10 校準（本條核心缺口已失效）**：(1)「Lease 概念在 Rust hot path 缺席」已失效——Decision Lease 路徑 A retrofit 已 land：`intent_processor/router.rs` 內建 LeaseGuard（Gate 1.4；成功路徑 `consume()` 交 fill consumer 於交易所 ack 後 release(Consumed)，拒絕路徑 Drop 釋放防 leak），且 Rust event_consumer 已是 lease 主權威路徑（累計 ~408k lease ops，TODO §0）。惟 `OPENCLAW_LEASE_ROUTER_GATE_ENABLED=1` 僅為 shadow/evidence 語義，**不等於**真實 live 授權或 Executor order authority（README 關鍵里程碑＋CLAUDE.md §四）。(2) 雙源 drift：已拍板 SM 收斂 Option 2（Rust 唯一權威、刪 Python SM transition、Python 降唯讀投影＋留 5 live-auth 閘）；E1b auth-axis divergence comparator `e6aa5e37` 已部署；P5-SM soak 監測 2026-06-10 重設計（真缺口=IPC 管線零曝險，新 gate S1-S5；「雙邊 divergence 結構性不可達」鐵則）。
...
> **2026-06-10 校準**：2026-05-30 cold audit re-run（對抗式，含 authority chain/fail-closed 面）結論 P0=0/P1=0——「策略繞過風控」軸未再出新洞。fast_track.rs 設計性旁路仍存在（檔案仍在，本條 claim 維持有效），FA-PHANTOM-1 修復後無復發紀錄；guardian_agent 雙源同受 SM Option 2「Rust 唯一權威」收斂方向約束。
...
> **2026-06-10 校準**：watchdog 防線經歷真實壓力測試並暴露雙 bug——2026-06-05 引擎自愈宕機事故：`OPENCLAW_BIND_HOST=0.0.0.0` 卡住 watchdog 重啟，引擎掛 20h；且 close-all 全走 Rust IPC，引擎掛＝GUI 零平倉路徑。watchdog 自愈雙 bug 已修（`072b8e20`），restart bind host 改 auto 解析 Tailscale IPv4 否則 loopback、禁 0.0.0.0；仍開放：canary_events.jsonl 無告警消費者。本條「✅ Watchdog 2s 週期 + 3 strike」運行時驗證須加此事故注腳——存活防線本身亦需防線。
...
> **2026-06-10 校準**：(1) PostOnly 反向已修（見原則 #2 校準）。(2)「edge_estimates startup-only」已過期——PH5-WIRE-1 RELOAD daemon 已實作（`spawn_edge_estimates_reloader_if_enabled`：週期＋手動 IPC trigger 雙路徑，per-pipeline fan-out `ReloadEdgeEstimates`），惟 env-gate `OPENCLAW_EDGE_RELOAD=1` DEFAULT-OFF，未開啟時行為仍等同 startup-only。(3) 2026-06-01 runtime RCA 推翻「gate 棘輪」歸因：cost_gate 拒 90.5% 經查全為真負（0 誤殺），收縮機制工作正常，真問題=入場 alpha 缺失——本原則的「收縮」面比快照時更被證實，失效的是上游 alpha。
...
> **2026-06-10 校準**：(1) AMD-2026-05-15-01（2026-05-15）凍結 paper promotion：「Paper is no longer an admissible promotion evidence lane」，promotion_pipeline 的 PAPER_SHADOW→DEMO_ACTIVE 段已標 [FROZEN]（enum 與審計保留，promote() 該段 aspirational）；Stage 1 改為 green Stage 0R replay preflight 後的 Demo micro-canary。(2) V5.8 alpha pivot（AMD-2026-05-31-01）：M1-M13 自主/學習模組全 active-IMPL 凍結，解凍 gate=首個 net+ 帶 alpha stage0_ready 候選（M7 decay 例外）。(3) residual alpha producer 2026-06-07 flag-on（daily cron 03:17）——研究報告產出、不改寫 live 狀態，與本原則一致。
...
> **2026-06-10 校準**：(1) 可重建性曾於 fill 記帳層真實失守後修復：2026-06-08 demo TON 幽靈倉＝PositionUpdate/Fill 無序雙寫競態（記帳與真實持倉脫鉤＝直接違反本原則），修=apply_fill 唯一 mutator＋reduce-only fail-closed＋reconciler 幻影偵測軸（`74b2e264`，已部署）；殘留=幻影告警僅 DB 可查。(2) 治理盲區 A（Python agent audit 無 PG）2026-06-10 重驗**仍成立**：change_audit_log.py 與 audit_persistence.py 仍 0 個 INSERT INTO。(3) 離線可解釋工具新增：P2 #7 `signal_postmortem` 8-taxonomy 失敗分類器 2026-06-07 生效（0 caller/0 DB，純離線消費 vetted gate report）。
...
> **2026-06-10 校準（本 bullet 已失效）**：TRACK-P-V2 swap 早已生效——T4＋V2 SWAP 完成且 runtime Priority 6 呼叫 `physical_micro_profit_lock_v2`（memory project_track_p_runtime_live，2026-04-21/22 起），「v1 仍在當前 PID」過期。另 2026-06-05 事故補強本原則的現實依據：close-all 全走 Rust IPC，引擎掛 20h 期間 GUI 零平倉路徑——交易所端條件單成為僅存防線，「本地＋交易所雙重防線」非冗餘而是必要。
...
> **2026-06-10 校準**：認知誠實已在 alpha 證據域制度化——ADR-0047（2026-05-31 Accepted）＋AMD-2026-05-31-01：promotion evidence **math-primary**；bull-heavy/rally-only/2024-dominated/stale-year-dominated 證據強制標籤（未標籤的 bull 正面結果=regime-bet/learning-only 非晉升證明）；News/X/Reddit 僅旁證、不可覆蓋 quantitative gate；Bybit market API 是 raw state input 非 prediction oracle，trend/regime 標籤須本地 leak-free 時點計算；每個候選 verdict 須含 regime/breadth/freshness/survivorship/execution-realism 證據矩陣。CLAUDE.md 已新增獨立「Alpha Evidence Governance」節。本條「DataQualityLevel hot path 無治理力」觀察維持，但證據域已有強制力落地。
...
> **2026-06-10 校準**：自主性「擴張」現處全面凍結——V5.8 alpha pivot（2026-05-31 operator 拍板，AMD-2026-05-31-01）：M1-M13 自主模組 active-IMPL 凍結，解凍 gate=首個 net+ 帶 alpha stage0_ready 候選（M7 例外）；Layered Autonomy with Fail-Safe（AMD-2026-05-21-01 v2：Autonomy Toggle＋三路通知 fail→SM-04＋7d cooling）設計完成、Wave 5 IMPL 同凍。邊界內自主（scanner universe/strategist auto-tune）照常運行；「自主能力進化」按優先級序（autonomy evolution 最末位）自然沉底，與本條「Live 階段才有壓力測試」判斷一致。
...
> **2026-06-10 校準（原文已重寫）**：(1) 現行 CLAUDE.md §二 #12=「System behavior should evolve from evidence, not anecdotes」；舊文「Paper 驗證→參數進化」段已隨 AMD-2026-05-15-01 paper promotion 凍結失效（paper=Archive/replay diagnostics）。(2)「沒在學」根因已查明（2026-06-01 runtime RCA）：cost_gate 拒 90.5% 全真負、非 gate 棘輪；真問題=已實現 edge 普遍負（入場 alpha/exit-policy/成本），主攻轉 AEG alpha 證據治理（ADR-0047，P0-EDGE-1）。(3) ml_training cron 已 hybrid 化（5 training DAILY＋5 audit weekday-gated），但 MIN_SAMPLES=200 下 4/5 策略不過；Sprint N+0 修復 attribution_chain_ok 0.5%→100%。Phase 5 PAUSED 框架由 V5.8 alpha pivot 接替。
...
> **2026-06-10 校準**：現行 CLAUDE.md §二 #13 已泛化為「AI calls have cost and must justify expected edge」，刪去「cost_edge_ratio ≥ 0.8 → 建議關倉」字面——本條「0.8 關倉實作未驗」缺口隨原則改寫降級為歷史問題。相關 operator 意圖另見 MICRO-PROFIT-FIX-1：「有微利就套（net>0）」非 cost_edge_ratio gate。三層預算雙計結構與 Rust BudgetTracker SSOT 結論維持有效。
...
> **2026-06-10 校準**：(1) 現行 CLAUDE.md §二 #15 增訂「Conductor is orchestration, **not a sixth trading agent**」。(2) ExecutorAgent shadow 已非 hardcoded：AMD-2026-05-09-03 graduated canary default＋SM-05 polling 設計後由 `canary_stage_provider`（CanaryStage enum）stage-aware 驅動，runtime 仍 shadow 默認——「Executor 不 IPC 發單」現狀不變，但機制從寫死改為 staged 治理。(3) 未來 News/X/Reddit 類 agent 依 ADR-0047 僅可旁證，不得成為主信號或直接驅動交易。
...
> **2026-06-10 校準**：組合級風險治理新增「beta 中性化鐵則」——2026-06-03 雙 NO-GO RCA（被攔信號反事實＋cascade fade）確認兩線同根因 down-beta 偽裝 alpha：BTC 弱勢 regime（17d −13.9%）下任何短 bias=趨勢 beta，強制 beta 中性化；funding-tilt（第 5 個結構性候選）同根因 NO-GO-C（aggregate net +9.12bps 但 82% 為裸價格 down-beta）。L2 P3b 已落 beta_neutral_check（B1）＋altcap producer（`24d049fc`，pre-deploy green、enable owed）。「策略重疊持倉缺實證 case」已由此系列 RCA 補上實證：重疊曝險的真實形態是共同 down-beta 因子，而非名義持倉重疊。
...
> **2026-06-10 校準**：本盲區已部分落地——PH5-WIRE-1 RELOAD daemon（`spawn_edge_estimates_reloader_if_enabled`，main_boot_tasks.rs）提供週期＋手動 IPC 雙路徑熱重載，per-pipeline fan-out `ReloadEdgeEstimates`、各引擎讀自身模式 JSON（結構性模式隔離）；惟 env-gate `OPENCLAW_EDGE_RELOAD=1` DEFAULT-OFF，未開啟時仍等同 startup-only。前文「多條原則共用的單點」表中 `edge_estimates.json startup-only` row 同此校準。
...
> **2026-06-10 校準**：治理已採納方向且更進一步——SM 雙源不走「自動對齊驗證」而走「單源化」：Option 2 收斂（Rust 唯一權威、刪 Python SM transition、Python 降唯讀投影＋留 5 live-auth 閘）已拍板；過渡期 SM contract test＋E1b auth-axis divergence comparator（`e6aa5e37`）已部署；P5-SM soak 監測 2026-06-10 重設計（真缺口=IPC 管線零曝險、新 gate S1-S5、「雙邊 divergence 結構性不可達」鐵則）。cutover（step-iii）operator sign-off gated。
...
> **2026-06-10 校準（結論摘要修訂）**：「最弱原則」名單收斂——#3 的 hot-path lease 缺席已由 Decision Lease 路徑 A retrofit（router.rs LeaseGuard）＋Rust event_consumer lease 權威化（~408k lease ops）實質補上（flag 仍 shadow/evidence 語義，非 live 授權）；#15 Executor 由 hardcoded shadow 轉 graduated canary staged（仍 shadow 默認）。2026-05-30 cold audit re-run P0=0/P1=0：安全面全過對抗審計，殘餘 P2/P3。「最大治理盲區」**維持有效**：Python agent audit 仍 0 INSERT INTO PG（2026-06-10 重驗）。新增盲區候選：幽靈倉位告警僅 DB 可查（2026-06-08 修復殘留）、canary_events.jsonl 無告警消費者（2026-06-05 事故殘留）。
...
> **2026-06-10 校準**：本表行數全為 2026-04-25 快照值，其後大規模拆檔。2026-06-10 實測：main.rs **1667**、instrument_info.rs **1050**、bybit_rest_client.rs **1591**、order_manager.rs **997**、ipc_server/mod.rs **155**（已拆 dispatch.rs / server.rs / slots.rs）、tick_pipeline/mod.rs **1450**、intent_processor/mod.rs **1968**、passive_wait_healthcheck.py **36**（殼層，主體拆入 `passive_wait_healthcheck/` 套件）、live_session_routes.py **720**。另現行 CLAUDE.md §九 口徑為 **800 行 review attention / 2000 行硬上限**，非本表引用的 1200。
...
> **2026-06-10 校準**：#1 change_audit 缺 PG 部分成立（本體仍 0 `INSERT INTO`；agent 平面另有 `agent_event_store.py` durable PG 落點，見紅 #3 注記）；#2 edge_estimates startup-only 已修（F6 RELOAD 2026-04-26）；#3 `engine_maintenance.flag` CREATE 點已定位（clean_restart.sh:238 / fresh_start.sh:230 `touch`）。
...
> **2026-06-10 校準**：本節五條與 §2 同文，核實結果相同：紅 #1 **已閉環**（G3-03 Phase B config 化；Executor 下單權限為治理性凍結）；紅 #2 **已閉環**（F6 RELOAD 2026-04-26 + P1-09 freshness fail-closed）；紅 #3 **部分處理**（`agent_event_store.py` PG 落點已有，change_audit_log 本體仍 JSONL）；紅 #4 **已閉環**（EDGE-P2-3 `use_maker_entry`：demo 先驗證、live 保守 Market，反向不存在）；紅 #5 **大部分閉環**（FA-PHANTOM-2 回歸測試 + 閾值配置化 8df29e9e）。逐條證據見 §2 各注記。
...
> **2026-06-10 校準**：本附錄為歷史快照原文，2026-06-10 校準不修改本附錄；狀態演進一律見正文各節注記。
