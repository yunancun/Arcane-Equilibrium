...
fn stress_fast_track_flash_crash_closes_all_positions() {
    ...
    pipeline.paper_state.apply_fill("BTCUSDT", true, 0.01, 65000.0, 3.575, 1000);
    pipeline.paper_state.apply_fill("ETHUSDT", true, 0.1, 2000.0, 0.11, 1000);
    pipeline.paper_state.apply_fill("SOLUSDT", true, 1.0, 150.0, 0.0825, 1000);
    pipeline.paper_state.apply_fill("XRPUSDT", true, 100.0, 0.60, 0.033, 1000);
    pipeline.paper_state.apply_fill("DOGEUSDT", false, 1000.0, 0.15, 0.0825, 1000);
...
fn stress_fast_track_boundary_exactly_5pct_drop() {
    ...
    assert_eq!(evaluate_fast_track(RiskLevel::Normal, 5.0, 0.0), FastTrackAction::CloseAll);
    // 4.99% should NOT trigger
    assert_eq!(evaluate_fast_track(RiskLevel::Normal, 4.99, 0.0), FastTrackAction::NoAction);
}

#[test]
fn stress_fast_track_boundary_exactly_90pct_margin() {
    assert_eq!(evaluate_fast_track(RiskLevel::Normal, 0.0, 90.0), FastTrackAction::CloseAll);
    assert_eq!(evaluate_fast_track(RiskLevel::Normal, 0.0, 89.99), FastTrackAction::NoAction);
}
...
fn stress_multi_symbol_5_coins_simultaneous_ticks() {
    ...
    pipeline.orchestrator.register(Box::new(GridTrading::new(60000.0, 80000.0)));
...
fn stress_multi_symbol_rapid_alternating_ticks() {
    ...
    for i in 0..1000 {
        if i % 2 == 0 {
            pipeline.on_tick(&make_event("BTCUSDT", 67000.0 + (i as f64).sin() * 100.0, i));
        } else {
...
fn stress_ma_crossover_whipsaw_rapid_reversals() {
    ...
    for i in 0..20 {
        ...
        let ctx = make_ctx("BTCUSDT", 50000.0, ts,
    ...
    assert!(fills_throttled < 15, "cooldown not throttling: {} trades in 20 ticks", fills_throttled);
    ...
    assert!(fills_unthrottled > fills_throttled,
...
fn stress_bb_reversion_extreme_oversold_bounce() {
    ...
    let ctx1 = make_ctx("ETHUSDT", 2000.0, 0,
    ...
    let ctx2 = make_ctx("ETHUSDT", 2050.0, 700_000,
...
fn stress_bb_breakout_valid_squeeze_with_volume() {
    ...
    let ctx2 = make_ctx("BTCUSDT", 68000.0, 700_000,
...
fn stress_grid_trading_wide_range_traversal() {
    ...
    assert!(total_intents > 5, "grid should trade on range traversal: got {}", total_intents);
}
...
fn stress_guardian_rejects_on_high_drawdown() {
    ...
    let intent = OrderIntent {
        symbol: "BTCUSDT".into(), is_long: true, qty: 0.01, confidence: 0.8,
    ...
    assert!(result.rejected_reason.unwrap().contains("guardian_rejected"));
}
...
fn stress_guardian_rejects_direction_conflict() {
    ...
    let intent = OrderIntent {
        symbol: "BTCUSDT".into(), is_long: false, qty: 0.01, confidence: 0.8,
        strategy: "test".into(), order_type: "market".into(), limit_price: None,
    };
    let result = proc.process(&intent, &gov, &state);
...
fn stress_guardian_rejects_position_count_limit() {
    ...
    let result = proc.process(&intent, &gov, &state);
    assert!(!result.submitted, "should reject 4th same-direction position");
}
...
fn stress_hard_stop_triggers_on_5pct_drop() {
    ...
    assert!(triggers[0].1.reason.contains("hard"), "reason: {}", triggers[0].1.reason);
}
...
fn stress_tick_latency_benchmark() {
    ...
    for i in 100..1100 {
        pipeline.on_tick(&make_event("BTCUSDT", 67000.0 + (i as f64).sin() * 100.0, i * 60_000));
    }
    let elapsed = start.elapsed();
    let per_tick_us = elapsed.as_micros() as f64 / 1000.0;

    // Release mode target: <100μs. Debug mode ~10x slower, allow 1000μs.
    let threshold = if cfg!(debug_assertions) { 1000.0 } else { 100.0 };
    assert!(per_tick_us < threshold,
        "tick avg should be <{:.0}μs, got {:.1}μs", threshold, per_tick_us);
...
fn stress_full_pipeline_volatile_market_simulation() {
    ...
    pipeline.orchestrator.register(Box::new(BbReversion::new()));
    pipeline.orchestrator.register(Box::new(BbBreakout::new()));
    pipeline.orchestrator.register(Box::new(GridTrading::new(63000.0, 71000.0)));
    ...
    assert!(status.balance > 0.0, "balance went to zero/negative: {}", status.balance);
}
...
fn stress_full_pipeline_extreme_prices() {
    ...
    // BTC at 200k
    for i in 0..50 {
        pipeline.on_tick(&make_event("BTCUSDT", 200_000.0 + i as f64 * 10.0, i * 60_000));
    }
    // DOGE at 0.001 (very small)
    for i in 50..100 {
        pipeline.on_tick(&make_event("DOGEUSDT", 0.001 + (i as f64 * 0.0001).sin() * 0.0001, i * 60_000));
    }
