...
//!
//! Default mode is read-only dry-run. `--apply` requires an explicit
//! `--i-understand-this-modifies-db` acknowledgement because it closes active
//! baseline rows and upserts new rows.
...
fn parse_args() -> Result<Args, String> {
    ...
    while let Some(arg) = it.next() {
        match arg.as_str() {
            "--dry-run" | "--verify" => args.mode = Mode::DryRun,
    ...
    if args.mode == Mode::Apply && !args.i_understand {
        return Err("--apply requires --i-understand-this-modifies-db".to_string());
    }
