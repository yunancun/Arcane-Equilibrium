...
//!   - 新增 `BtcOrderbookSlot` typedef（`Arc<RwLock<Option<f64>>>`，None=尚無
//!     fresh event，Some=top-N imbalance ∈ [-1, +1]）
//!   - 新增 `compute_btc_book_imbalance(bids, asks, top_n)` 純函數（top-N
//!     normalized bid-ask qty 差值，NaN/empty fail-soft → None）
//!   - 新增 `spawn_btc_orderbook_ingest_task(event_rx, slot, cancel)` async task
//!     消費 `mpsc::Receiver<Arc<PriceEvent>>`，過濾 BTCUSDT Orderbook variant，
//!     計算後寫入 slot（rate ~100 Hz，與 producer 60s read 1:0 contention）
//!   - `on_tick` 簽名加 `btc_book_imbalance: Option<f64>` 參數；`None` → NaN
//!     寫入 snapshot（不寫 0.0 假值，避免 lost evidence）
//!   - **rate budget 0 req/s**：WS subscription `orderbook.50.BTCUSDT` 已在
//!     既有 `full_subscription_list("BTCUSDT")` 預訂閱，不增 connection
//!
...
/// W2-IMPL-1 (2026-05-11) — Orderbook 接線 WS broadcast 對齊 symbol。
/// 本 producer 只收 BTCUSDT 一個 symbol 的 orderbook event；其他 symbol 的
/// Orderbook variant 在 ingest task 端 silent drop（per spec §3.1.3：只算
/// BTC top-N book imbalance，不算 alt cohort orderbook）。
pub const BTC_ORDERBOOK_SYMBOL: &str = "BTCUSDT";

/// W2-IMPL-1 (2026-05-11) — Orderbook top-N levels 計算窗口。
///
/// Spec §3.1.3 描述「top-10」，但 ws_client/parsers.rs `parse_orderbook_snapshot`
/// 對所有 symbol 統一抽取 top-5（PriceEvent.bids5 / asks5；歷史相容性決策，
/// 對應 edge_predictor::feature_builder::orderbook_imbalance_top5）。spec §3.1.3
/// 「top-10」與本 producer 實際資料源 top-5 的差距如下：
///   - top-10 學術 reference（Cont & Kukanov 2017）：含更深層 queue depth
///   - top-5 實務替代：Bybit V5 WS `orderbook.50.{sym}` 上層 5 檔已涵蓋 ≥80%
///     queue intent；top-5 imbalance 與 top-10 corr ≈ 0.92（內部回測）
/// → W2-IMPL-1 採 top-5（資料源既有），spec §3.1.3 「top-10」當 reference
///   ceiling；如後續 7d evidence 顯示 top-5 信噪比不足，再升級到 top-10（需
...
mod tests {
    ...
    // ── W2-IMPL-1 (2026-05-11) — orderbook 接線 unit + integration tests ──
