...
//!   命名空間。本模組統一 host:
//!   - W1 sub-task 1 (本 PR)：funding_curve aggregator + V085 writer
//!     + PanelAggregator skeleton (broadcast core)
//!   - W1 sub-task 2 (E1-β)：oi_delta_panel aggregator + V087 writer
//!   - W1 sub-task 3 (E1-γ)：WS subscription + main.rs broadcast migration
//!     + step_4_5 surface dispatch wire
...
pub struct PanelAggregator {
    ...
    /// 預留供未來 sub-aggregator（oi_delta / btc_lead_lag）共用 pool 句柄。
    #[allow(dead_code)]
    db_pool: Arc<DbPool>,
...
impl PanelAggregator {
    ...
    ///   會 clone 一份給 funding_curve aggregator。後續 oi_delta aggregator 同 cohort。
    ...
    pub fn new(
        ...
        let funding_curve_aggregator =
            FundingCurveAggregator::new(db_pool.clone(), cohort_symbols);
        Self {
    ...
    ///
    /// 預期最終 shape（per W1 spec §2.3 line 169-191）：
    /// ```ignore
    /// pub async fn run(
    ///     mut self,
    ...
    /// ```
    ///
    /// 本 sub-task 1 階段 placeholder：log 進入 + 等 cancel 即退出（不訂閱 WS、
    /// 不 flush）。允許整合 test 觀測 PanelAggregator 構造 + cancel 路徑通暢。
    pub async fn run_placeholder(self) {
        info!(
            target: "panel_aggregator",
            cohort_size = self.funding_curve_aggregator.cohort_size(),
            "PanelAggregator placeholder start (sub-task 1 skeleton; \
...
mod tests {
    ...
    async fn test_panel_aggregator_constructs_with_cohort() {
        // PASS：PanelAggregator 建構 + funding_curve aggregator cohort 大小對齊
