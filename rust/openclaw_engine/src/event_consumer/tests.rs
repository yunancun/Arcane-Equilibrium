...
fn make_market_pending(elapsed_ms_is_zero: bool) -> super::PendingOrder {
...
fn make_postonly_pending(maker_timeout_ms: Option<u64>) -> super::PendingOrder {
    super::PendingOrder {
...
fn test_classify_market_under_5s_keeps_order() {
    ...
    assert_eq!(
        super::classify_pending_sweep(&po, 0),
        super::PendingSweepAction::Keep
    );
    // 5s exact: boundary — legacy condition is `elapsed > 5000`, so 5000 still keeps.
    assert_eq!(
        super::classify_pending_sweep(&po, 5_000),
        super::PendingSweepAction::Keep
    );
}
...
fn test_classify_market_between_5s_and_60s_soft_warns() {
    ...
    assert_eq!(
        super::classify_pending_sweep(&po, 5_001),
        super::PendingSweepAction::LegacySoftWarn
    );
    // 30s mid-range.
    assert_eq!(
        super::classify_pending_sweep(&po, 30_000),
        super::PendingSweepAction::LegacySoftWarn
    );
    // 60s exact: boundary — legacy condition is `elapsed > 60000`, so 60000 still warns.
    assert_eq!(
        super::classify_pending_sweep(&po, 60_000),
        super::PendingSweepAction::LegacySoftWarn
    );
}
...
fn test_classify_market_over_60s_hard_removes() {
    ...
    assert_eq!(
        super::classify_pending_sweep(&po, 60_001),
        super::PendingSweepAction::LegacyHardRemove
    );
    assert_eq!(
        super::classify_pending_sweep(&po, 90_000),
        super::PendingSweepAction::LegacyHardRemove
    );
}
...
fn test_classify_postonly_under_deadline_keeps_order() {
    ...
    assert_eq!(
        super::classify_pending_sweep(&po_default, 0),
        super::PendingSweepAction::Keep
    );
    // Under default 45s → keep, even above Market's 5s soft-warn band.
    // 未達 45s → Keep，不應誤用 Market 的 5s 軟警告路徑。
    assert_eq!(
        super::classify_pending_sweep(&po_default, 30_000),
        super::PendingSweepAction::Keep
    );
    assert_eq!(
        super::classify_pending_sweep(&po_default, 44_999),
        super::PendingSweepAction::Keep
    );
    ...
    assert_eq!(
        super::classify_pending_sweep(&po_fast, 14_999),
        super::PendingSweepAction::Keep
    );
}
...
fn test_classify_postonly_at_or_past_deadline_cancels() {
    ...
    let po_default = make_postonly_pending(None);
    assert_eq!(
        super::classify_pending_sweep(&po_default, 45_000),
        super::PendingSweepAction::MakerTimeoutCancel
    );
    assert_eq!(
        super::classify_pending_sweep(&po_default, 300_000),
        super::PendingSweepAction::MakerTimeoutCancel
    );
    ...
    assert_eq!(
        super::classify_pending_sweep(&po_60s, 60_000),
        super::PendingSweepAction::MakerTimeoutCancel
    );
    ...
    assert_eq!(
        super::classify_pending_sweep(&po_60s, 120_000),
        super::PendingSweepAction::MakerTimeoutCancel
    );
}
...
fn test_classify_postonly_with_zero_deadline_cancels_immediately() {
    ...
    assert_eq!(
        super::classify_pending_sweep(&po_zero, 0),
        super::PendingSweepAction::MakerTimeoutCancel
    );
    assert_eq!(
        super::classify_pending_sweep(&po_zero, 1),
        super::PendingSweepAction::MakerTimeoutCancel
    );
}
...
fn test_classify_market_vs_postonly_isolation() {
    ...
    assert_eq!(
        super::classify_pending_sweep(&m, elapsed),
        super::PendingSweepAction::LegacySoftWarn
    );
    assert_eq!(
        super::classify_pending_sweep(&p, elapsed),
        super::PendingSweepAction::Keep
    );
}
