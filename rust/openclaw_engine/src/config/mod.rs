...
pub use risk_config::{
    per_strategy_concurrency_rejection, per_strategy_new_entry_rejection, CostEdgeConfig,
    FastTrackConfig, HurstConfig, RiskConfig, SlippageConfig, SlippageTier, StrategyOverride,
    DEFAULT_PER_TRADE_RISK_PCT, MAX_PER_TRADE_RISK_PCT, MIN_PER_TRADE_RISK_PCT,
};
...
pub use store::{write_toml_atomic_fsynced, ConfigStore, PatchOutcome, PatchSource};
...
pub struct EngineBootstrap {
    ...
    /// Enable extended WS subscriptions (kline/tickers/orderbook/publicTrade/allLiquidation).
    /// Legacy liquidation/price-limit/adl-notice topics remain disabled.
    /// 啟用擴展 WS 訂閱（kline/tickers/orderbook/publicTrade/allLiquidation）；
    /// legacy liquidation/price-limit/adl-notice topic 仍禁用。
    #[serde(default = "default_true")]
