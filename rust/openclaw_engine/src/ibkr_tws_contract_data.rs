...
//!   `sha2`/`hex`（identity_hash 鑄造;preimage 單一定義點在 types 契約）、
...
use openclaw_types::{
    is_normalized_symbol, IbkrInstrumentIdentityRowBlocker, IbkrInstrumentIdentityRowV1,
    IbkrSecTypeV1, IbkrStockTypeV1, StockEtfCurrency, IBKR_INSTRUMENT_IDENTITY_ROW_CONTRACT_ID,
};

use crate::ibkr_tws_account_data::SnapshotStaleness;
use crate::ibkr_tws_wire::{
    decode_fields, encode_fields, encode_frame, CodecError, IN_BOND_CONTRACT_DATA_MSG_ID,
    IN_CONTRACT_DATA_END_MSG_ID, IN_CONTRACT_DATA_MSG_ID,
};

// ===========================================================================
// (a) OUT 常數 + 全限定查詢 + v8 builder（IB 現勘 2026-07-17 pinned;官方 ibapi
// 9.81.1.post1 sdist）
...
// ===========================================================================
// (b) config + per-field sv 門控表（全 config 化;參數禁假功能——每項必真實生效）
// ===========================================================================
...
const SV_GATE_MD_SIZE_MULTIPLIER: i32 = 110;
/// aggGroup 欄門檻（secIdList 後位 31）。
const SV_GATE_AGG_GROUP: i32 = 121;
/// underSymbol/underSecType 欄門檻（位 32-33）。
const SV_GATE_UNDER_SYMBOL_SECTYPE: i32 = 122;
/// marketRuleIds 欄門檻（位 34）。
const SV_GATE_MARKET_RULE_IDS: i32 = 126;
/// realExpirationDate 欄門檻（位 35）。
const SV_GATE_REAL_EXPIRATION_DATE: i32 = 134;
/// stockType 欄門檻（位 36;ETF|COMMON 判別源——sv<152 缺席=UnknownDenied 契約拒）。
const SV_GATE_STOCK_TYPE: i32 = 152;
/// longName unicode-escape 解碼門檻（sv≥153 起 server 以 `\uXXXX` 轉義送非 ASCII——
/// 漏解=靜默 mojibake）。
const SV_GATE_LONG_NAME_UNICODE_ESCAPE: i32 = 153;

/// per-field 門控判據（協商 sv ≥ 門檻 → 該欄在 wire 上出現/該行為生效）。
fn sv_gate(server_version: i32, gate: i32) -> bool {
    server_version >= gate
}
...
impl<'a> FieldCursor<'a> {
    fn new(fields: &'a [String], start: usize) -> Self {
        Self { fields, idx: start }
    }

    fn take(&mut self) -> Result<&'a str, ContractDataReject> {
...
fn decode_unicode_escape_minimal(raw: &str) -> String {
...
fn expect_msg_id(raw: &str, expected: i64) -> Result<(), ContractDataReject> {
...
/// 同構 W5-S2/S3）。
fn parse_i64(raw: &str, field: &'static str) -> Result<i64, ContractDataReject> {
    raw.parse::<i64>()
