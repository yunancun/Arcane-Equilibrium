...
fn market_block_reason(
    ...
    match strategy {
        ...
        "funding_arb" => {
            ...
            {
                Some(format!(
                    "funding_momentum_gate:trend={:.2} dir={:.2}% funding={:.2}bps",
    ...
    for strategy in [
        ...
        if let Some(reason) = market_block_reason(strategy, mc, market_cfg) {
            ...
            edge.route_reason = format!(
                "immature_negative_quarantine:n={} bps={:.2}",
                edge.n,
...
mod tests {
    ...
    fn test_market_judgment_quarantines_immature_negative_edge() {
        ...
        assert!(
            grid.route_reason.contains("immature_negative_quarantine"),
            "{}",
            grid.route_reason
        );
    }
    ...
    #[test]
    fn test_funding_momentum_gate_hard_blocks_demo_learning() {
        ...
        assert_eq!(funding.market_status, "blocked");
        assert_eq!(funding.route_mode, "market_gate");
        assert!(
            funding.route_reason.contains("funding_momentum_gate"),
            "{}",
