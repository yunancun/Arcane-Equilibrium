...
fn lookup_rule(from: OrderState, to: OrderState) -> Option<TransitionRule> {
    ...
    match (from, to) {
        // Pre-execution
        (Created, Pending) => Some(TransitionRule {
            event: OrderEvent::SubmitForApproval,
            allowed: AGENTS,
        }),
        (Pending, Approved) => Some(TransitionRule {
            event: OrderEvent::Approve,
            allowed: AUTH,
        }),
        (Pending, Rejected) => Some(TransitionRule {
            event: OrderEvent::RejectAuthorization,
            allowed: AUTH_RISK,
        }),
        (Pending, Canceled) => Some(TransitionRule {
            event: OrderEvent::Cancel,
            allowed: AGENTS,
        }),
        // Execution
        (Approved, Submitted) => Some(TransitionRule {
            event: OrderEvent::SendToVenue,
            allowed: EXEC_SYS,
        }),
        (Approved, Canceled) => Some(TransitionRule {
            event: OrderEvent::Cancel,
            allowed: CANCEL_ALL,
        }),
        (Submitted, Working) => Some(TransitionRule {
            event: OrderEvent::Acknowledge,
            allowed: VENUE,
        }),
        (Submitted, Rejected) => Some(TransitionRule {
            event: OrderEvent::RejectByVenue,
            allowed: VENUE,
        }),
        (Working, PartiallyFilled) => Some(TransitionRule {
            event: OrderEvent::PartialFill,
            allowed: VENUE,
        }),
        (Working, Filled) => Some(TransitionRule {
            event: OrderEvent::Fill,
            allowed: VENUE,
        }),
        (Working, Canceled) => Some(TransitionRule {
            event: OrderEvent::Cancel,
            allowed: CANCEL_ALL,
        }),
        (PartiallyFilled, Filled) => Some(TransitionRule {
            event: OrderEvent::Fill,
            allowed: VENUE,
        }),
        (PartiallyFilled, Canceled) => Some(TransitionRule {
            event: OrderEvent::Cancel,
            allowed: CANCEL_ALL,
        }),
        // Post-execution
        (Filled, Reconciling) => Some(TransitionRule {
            event: OrderEvent::BeginReconciliation,
            allowed: RECON,
        }),
        (Reconciling, Completed) => Some(TransitionRule {
            event: OrderEvent::ReconciliationPass,
            allowed: RECON,
        }),
        (Reconciling, Rejected) => Some(TransitionRule {
            event: OrderEvent::ReconciliationFail,
            allowed: RECON_OP,
        }),
        _ => None,
    }
...
impl OmsStateMachine {
    ...
    pub fn transition(
        ...
        let rule = lookup_rule(from, target).ok_or_else(|| SmError::InvalidTransition {
            from: from.to_string(),
            to: target.to_string(),
        })?;
        ...
        // Track metadata
        if target == OrderState::Approved {
            order.approved_by = initiator.as_str().to_string();
        }
        if target == OrderState::Completed {
            order.reconciliation_result = "PASS".to_string();
        }
    ...
    pub fn begin_reconciliation(&mut self, idx: usize) -> Result<(), SmError> {
        self.transition(
            ...
            OrderInitiator::ReconciliationEngine,
            "begin_recon",
        )
    }
...
mod tests {
    ...
    fn make_filled_order() -> (OmsStateMachine, usize) {
        ...
        sm.submit_for_approval(idx, OrderInitiator::AiAgent)
            .unwrap();
    ...
    fn test_cannot_skip_authorization() {
        ...
        let err = sm
            .transition(idx, OrderState::Submitted, OrderInitiator::System, "")
            .unwrap_err();
    ...
    fn test_cannot_skip_reconciliation() {
        let (mut sm, idx) = make_filled_order();
        let err = sm
            .transition(idx, OrderState::Completed, OrderInitiator::System, "")
            .unwrap_err();
        assert!(matches!(err, SmError::Forbidden { .. }));
    }
    ...
    fn test_terminal_cannot_transition() {
        ...
        sm.reconciliation_pass(idx).unwrap();
        let err = sm
            .submit_for_approval(idx, OrderInitiator::AiAgent)
            .unwrap_err();
