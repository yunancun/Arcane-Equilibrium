...
//!
//! MODULE_NOTE (中): 使用 ATR 縮放基礎值、傳入的 regime 止損乘數和反聚集偏移
//!   計算動態止損價。每 tick 為 StopManager 提供自適應止損水平。
//!   regime 止損乘數由 engine caller 從權威可調的 `RiskConfig.regime` 解析後傳入；
//!   core 本身不持有任何 regime 乘數表（core 不依賴 engine config）。
//!   上下限來自 RiskConfig.dynamic_stop。
...
///
/// # Logic / 邏輯
/// 1. Apply the passed-in regime stop multiplier to base stop / 對基礎止損套用傳入的 regime 止損乘數
...
/// - `regime_stop_mult`: Regime 對應的止損乘數，由 engine 從權威 `RiskConfig.regime` 解析後傳入 / Regime stop multiplier resolved by the engine from the authoritative `RiskConfig.regime`
...
pub fn compute_dynamic_stop_pct(
    ...
    // regime stop 乘數由 engine 端從 RiskConfig.regime 解析後傳入（core 不依賴 engine config）。
    let base = base_stop_pct * regime_stop_mult;
...
mod tests {
    ...
    fn test_dynamic_stop_no_atr() {
        // No ATR → use base × regime stop mult / 無 ATR → 使用基礎 × regime 止損乘數
        let stop = compute_dynamic_stop_pct(2.0, None, "BTCUSDT", 1000, 1.0, 5.0, 0.8, 1.5);
        // regime stop mult 直接傳入（trending 預設 = 1.0），base = 2.0
    ...
    fn test_dynamic_stop_with_atr() {
        // regime stop mult = 1.0 (trending 預設), base = 1.5%, cap = 5.0 * 0.8 = 4.0
        // atr_stop = 3.0 * 1.5 = 4.5, capped to 4.0
        // effective = max(1.5, 4.0) = 4.0
        let stop =
            compute_dynamic_stop_pct(1.5, Some(3.0), "BTCUSDT", 1000, 1.0, 5.0, 0.8, 1.5);
    ...
    fn test_dynamic_stop_volatile_regime() {
        // regime stop mult = 1.5 直接傳入（volatile 預設）, base = 2.0 → 3.0
        let stop = compute_dynamic_stop_pct(2.0, None, "BTCUSDT", 1000, 1.5, 5.0, 0.8, 1.5);
        assert!(stop > 2.0 && stop < 4.0, "stop={stop}, expected ~3.0");
    }

    #[test]
    fn test_dynamic_stop_floor() {
        // Very small base + squeeze regime stop mult (0.6×, 直接傳入) should still be >= 0.1
        let stop = compute_dynamic_stop_pct(0.05, None, "BTCUSDT", 1000, 0.6, 5.0, 0.8, 1.5);
    ...
    fn test_dynamic_stop_atr_below_base() {
        ...
        // regime stop mult = 1.0 (trending 預設), base=3.0, atr=0.5, mult=1.5 → atr_stop=0.75, effective=max(3.0, 0.75)=3.0
        let stop =
            compute_dynamic_stop_pct(3.0, Some(0.5), "BTCUSDT", 1000, 1.0, 5.0, 0.8, 1.5);
        assert!(stop > 2.0 && stop < 4.0, "stop={stop}, expected ~3.0");
    }
    ...
    fn test_dynamic_stop_atr_mult_widens_stop() {
        ...
        // regime stop mult = 1.0 直接傳入（trending 預設）, ATR=1.5%, base=3.0%, cap=4.0%
        //   mult=1.0: atr_stop=1.5 < base → effective=3.0
        //   mult=2.5: atr_stop=3.75 > base → effective=3.75
        let stop_tight =
            compute_dynamic_stop_pct(3.0, Some(1.5), "BTCUSDT", 1000, 1.0, 5.0, 0.8, 1.0);
        let stop_wide =
            compute_dynamic_stop_pct(3.0, Some(1.5), "BTCUSDT", 1000, 1.0, 5.0, 0.8, 2.5);
        assert!(
