...
Purpose:
- preserve the high-detail 2026-04-29 Codex memory notes that were removed from `.codex/MEMORY.md` during compaction
- keep `.codex/MEMORY.md` concise without losing potentially useful context

These details remain useful as historical context, but they are too session-specific to stay in the primary operating memory file.
...
- Not full green: latest `passive_wait_healthcheck.sh --quiet` still fails `[12] bb_breakout_post_deadlock_fix` and `[22] trading_pipeline_silent_gap`, and warns `[27] intents_counter_freeze`. Earlier transient `[16] strategist_cycle_fresh` cleared after the first 5-minute cycle; `[31] edge_diag_2_strategy_diversity` no longer appeared in the latest rerun.
