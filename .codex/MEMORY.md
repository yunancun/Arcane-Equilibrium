...
- AgentTodo is the handoff source for the next multi-agent phase. MAG-015 contract addendum is complete at `docs/architecture/multi_agent_rework_2026-05-05/2026-05-06--mag015_sprint_a_contract_addendum.md`. MAG-010..012 durable event-store source wiring and `[52] agent_event_store_rows` are now implemented and Linux targeted tests passed at `91379cd2`, but final DONE still waits for Linux runtime fresh rows in `agent.messages`, `agent.state_changes`, and `agent.ai_invocations`; next gate is `[52]` row proof, then MAG-016..019 read-only OpenClaw status/self-state and Agent Control foundation. Proposal/approval/channel relay waits until Linux agent schema row proof exists
