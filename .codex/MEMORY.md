...
- TODO v32 is the active dispatch queue as of 2026-05-15. Paper promotion is frozen by AMD-2026-05-15-01; `[55]` is source-cleared by the fully-filled plan invariant; `[67]` feature baselines are restored to 646 active rows / 19 symbols / 34 features; `[27] intents_counter_freeze` is post-grace closed by direct PASS; V079 is applied on `trade-core` and `learning.strategy_trial_ledger` has 16,212 rows. A4-C Stage 0R remains GATE-RED and is archived from active promotion; `P1-A4C-RCA-1` is closed no-revive after QC/MIT review (`avg_net_bps=-1.0013`, `PSR(0)=0.1904`, `DSR=0`, R2(120)=0; finite X=5/Y=0.20 probe only +1.4739 bps), so `P1-A4C-REV-1` is not opened and A4-C stays diagnostic-only. `bb_breakout_oi_confirmed_5m` is spec-only and not execution evidence. W-AUDIT-8a C1 has a standalone `allLiquidation.{symbol}` probe plan/script; 60s smoke passed as `SMOKE_PASS_NOT_C1_PROOF`, and the 24h proof is running on `trade-core` as PID `4100789` since `2026-05-15T19:53:09Z`, but C1 remains blocked until full BB proof + MIT schema sign-off. W-AUDIT-8b Funding Skew spec v0.2 review/design is conditionally approved for Stage 0R replay design only: 30m primary horizon, branch-separated hypotheses, `K_total >= K_prior+4050`, `DSR>=0.95`, PBO fail-closed, raw panel as-of joins, funding attribution `excluded`, and BB funding interval/source-mode fields; next task is read-only query/report packet, not strategy implementation. Pre-v32 Mac/origin/Linux source was verified clean/synced at `86a5d707`; v32 is source/docs/probe-only and runtime binary remains `7b33ab2e`. Do not start proposal/mobile/second-GUI/Stage 3/4/true-live work from older AgentTodo/TODO/MEMORY text.
