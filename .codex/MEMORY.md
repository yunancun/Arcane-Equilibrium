...
- AgentTodo is the handoff source for the next multi-agent phase. MAG-015 contract addendum is complete at `docs/architecture/multi_agent_rework_2026-05-05/2026-05-06--mag015_sprint_a_contract_addendum.md`. MAG-010..014 durable event-store source + controlled Linux row proof is closed: strict `[52]` went from FAIL `0/0/0` to PASS `messages=2 state_changes=11 ai_invocations=2` after smoke rows through real `AgentEventStore`/`MessageBus`/`BaseAgent`/`Conductor`. No service restart or production continuous flag was applied; supervisor cloud rows remain MAG-019. Next gate is MAG-016/017 read-only OpenClaw status/self-state and authority lockdown, then MAG-018/019. Proposal/approval/channel relay waits until read-only foundation exists
