...
- TODO v13 is the active dispatch queue as of 2026-05-08. It supersedes TODO v12's history-ledger layout and archives stale/closed context at `docs/archive/2026-05-07--todo_v12_agent_openclaw_replan_archive.md`. Current order: W-B runtime decision-spine lineage wiring -> new MAG-082 Stage 2 evidence -> MAG-083/MAG-084 -> residual edge/data quality and supervised-live foundations. Do not start proposal/mobile/second-GUI work from stale AgentTodo/TODO text.
- P1-REPLAY-2 source cleanup is complete: V066 adds explicit `replay_report` artifact_type + `byte_size >= 0` CHECK, finalize registers `replay_report`, and report SQL prefers `replay_report` while retaining legacy `pnl_summary` fallback. This is a source migration only until an operator authorizes applying it to runtime DB.
...
- Current P1/W-B order: `P1-FAKE-1`, `P1-OPENCLAW-3`, `P1-OPENCLAW-6/7`, and source-level `P1-AGENT-OBS-1` are closed. W-B runtime decision-spine lineage source/test wiring is source-ready as of 2026-05-08: Rust startup wires the durable Agent Spine writer behind `OPENCLAW_AGENT_SPINE_RUNTIME_MODE`, approved demo/live_demo open intents emit shadow-only typed StrategySignal -> StrategistDecision -> GuardianVerdict -> ExecutionPlan -> ExecutionReport objects plus edges/idempotency keys, and `[55]` treats runtime mode as enabled. Runtime row proof still needs explicit operator approval for rebuild/restart/env flip. P1-DATA-1 source fixes are in place: `[14]` distinguishes risk/cost gate suppression from writer-health evidence while still warning on stalled sample accumulation; `[37]` treats historical failures as recovered when later non-failed rows exist and no recent failures remain; `[40]` catches negative strategy/symbol cells combined across demo/live_demo; `LABUSDT` is source-configured in grid_trading.blocked_symbols across paper/demo/live to block new grid entries only; `[45]` accepts recent AccountManager fee-use proof during rejected-only demo/live_demo no-fill windows. Runtime effect for the LABUSDT block still awaits explicit reload/rebuild authorization. P1-DATA-2 source fix is in place: `[42b]`/`[42c]` now render low-sample attribution strategies as `LOW_SAMPLE(n, need)` sample-maturity watch instead of misleading `0.000` ratio drift while preserving hard FAIL bands for mature ratio regressions. P1-DATA-3 source fix is in place: `[51]` now requires mature `opportunity_positive` samples before PASS, reports `MATURE/LOW_SAMPLE(n, need)`, and keeps scanner opportunity shadow-only for exploration-only or immature calibrated samples. P1-EDGE-1 source fix is in place: runtime 7d ma_crossover combined demo/live_demo was negative mainly from `LABUSDT` (`n=6 avg=-244.54bps`), so `LABUSDT` is source-blocked for ma_crossover new entries in risk configs; bb_breakout remains demo-only/live-disabled with low negative sample pending more evidence.
