...
- Boundary after W-C activation: engine/API were rebuilt/restarted with `--keep-auth` for shadow Agent Spine, Decision Lease router-gate evidence, and scanner advisory-shadow mode only. No live auth mutation, real live API enablement, strategy/risk parameter change, scanner hard authority, executor order authority, Stage 3/4 approval, or MAG-083/MAG-084 approval was performed.
