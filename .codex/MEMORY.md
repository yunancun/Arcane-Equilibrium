...
- runtime/documentation recalibration `2026-04-30 21:10 CEST`: Mac/Linux source is at `5ba9b1c`; Linux runtime has been rebuilt from current source. Healthcheck is `FAIL` because `[38] grid_trading_lifecycle_drift` is a real strategy drift signal; `[33]` maker fill quality and `[40]` realized edge remain observation gates. Dust residual prevention is loaded in runtime but still needs one real Demo/Live full-close proof before exchange-side effectiveness is declared. LiveDemo/live pipeline is authorized and running under live-grade gates, but true live autonomy remains gated by GovernanceHub, Decision Lease, and the 5 live gates.
