...
- TODO v17 is the active dispatch queue as of 2026-05-09. It supersedes TODO v12/v13 historical layouts and mounts the 2026-05-08 12-agent full audit plan into W-AUDIT-1..7. Current order: keep W-C MAG-082 Stage 2 evidence collection running; W-AUDIT-1/2 are source-closed, W-AUDIT-3 F-01 is source/test closed, W-AUDIT-4 is partial: V072 has a dry-run-default 34-dim `feature_baseline_writer` source/test checkpoint, and F-08 source scope now covers the original audit five (`thompson_sampling`, `optuna_optimizer`, `cpcv_validator`, `dl3_foundation`, `weekly_report_generator`) plus operational MLDE jobs, but cron install/runtime rows remain pending operator action. W-AUDIT-6 source/test cleanup is closed through funding_arb RiskConfig retirement, ma_crossover R:R trailing/TP, bb_breakout 5m RFC/IMPL, W-AUDIT-6c portfolio VaR/CVaR/EVT promotion evidence, P0-V2 Donchian leak-bias regression, and P0-V2 Strategist wide-adjustment skill. W-AUDIT-7 source/test is closed through F-30 prompt modal, system-mode hold confirm, strategy action confirm isolation, F-strategist-cap 30%→50% source/TOML/fallback alignment plus `wide_parameter_adjustment` prompt skill, and F-28 ContextDistiller prompt-compaction IMPL/callsite. F-09 FUP-2 runtime was verified on Linux: `edge_label_backfill_cron.sh` is installed, logs are current, and `[43]` PASSed. Remaining W-AUDIT-4 work is operator-authorized F-08/V073/V074 cron install, separately authorized V072 writer apply/install if runtime rows are wanted, and true producer/drop decisions for retained empty tables that still lack production callers. Remaining W-AUDIT-7 source/ops work is F-07 and F-cea-env; no runtime reload/provider traffic was done for F-strategist-cap, wide-adjustment skill, F-28, or F-08. Do not start proposal/mobile/second-GUI/Stage 3/4/true-live work from stale AgentTodo/TODO text.
