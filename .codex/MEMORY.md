...
- runtime/documentation recalibration `2026-04-30 22:04 CEST`: Mac/Linux source is at `67791bd`; Linux engine remains from the prior rebuild, and an API-only reload loaded the Scout heartbeat Python wiring. Healthcheck is `FAIL` because `[38] grid_trading_lifecycle_drift` is a real strategy drift signal; `[33]` maker fill quality and `[40]` realized edge remain observation gates. Dust residual prevention is loaded and now has runtime proof: 8 Demo/LiveDemo `qty=0` close orders joined to nonzero fills after the 21:10 CEST rebuild, including Demo `APEUSDT` and LiveDemo `XAGUSDT` `orphan_frozen` residues. LiveDemo/live pipeline is authorized and running under live-grade gates, but true live autonomy remains gated by GovernanceHub, Decision Lease, and the 5 live gates.
