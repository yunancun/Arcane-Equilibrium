...
- TODO v30 is the active dispatch queue as of 2026-05-15. Paper promotion is frozen by AMD-2026-05-15-01; `[55]` is source-cleared by the fully-filled plan invariant; `[67]` feature baselines are restored to 646 active rows / 19 symbols / 34 features; `[27] intents_counter_freeze` is post-grace closed by direct PASS; V079 is applied on `trade-core` and `learning.strategy_trial_ledger` has 16,212 rows. A4-C Stage 0R remains GATE-RED and is archived from active promotion except for `P1-A4C-RCA-1`, a read-only bounded RCA path; RCA start also stayed below revive/promotion bands (`avg_net_bps=-1.0013`, `PSR(0)=0.1904`, `DSR=0`, R2(120)=0; finite X=5/Y=0.20 probe only +1.4739 bps). Otherwise A4-C is diagnostic-only. `bb_breakout_oi_confirmed_5m` is spec-only and not execution evidence. W-AUDIT-8a C1 has a standalone `allLiquidation.{symbol}` probe plan/script but remains blocked until 24h BB proof + MIT schema sign-off. W-AUDIT-8b Funding Skew spec v0.1 exists and needs QC/MIT/BB review + Stage 0R replay design. Pre-v30 Mac/origin/Linux source was verified clean/synced at `9a72d054`; v30 is source/docs only and runtime binary remains `7b33ab2e`. Do not start proposal/mobile/second-GUI/Stage 3/4/true-live work from older AgentTodo/TODO/MEMORY text.
