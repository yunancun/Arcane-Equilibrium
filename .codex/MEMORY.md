...
- AgentTodo is the handoff source for the next multi-agent phase. MAG-015 contract addendum is complete at `docs/architecture/multi_agent_rework_2026-05-05/2026-05-06--mag015_sprint_a_contract_addendum.md`; next order is MAG-010..014 durable event store, then MAG-016..019 read-only OpenClaw status/self-state and Agent Control foundation; proposal/approval/channel relay waits until Linux agent schema row proof exists
