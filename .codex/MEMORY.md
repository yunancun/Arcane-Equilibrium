...
- TODO v36 is the active dispatch queue as of 2026-05-16. Paper promotion remains frozen by AMD-2026-05-15-01; `[55]`, `[67]`, and `[27]` remain closed from the 2026-05-15 source/runtime checks; V079 is applied on `trade-core` and `learning.strategy_trial_ledger` had 16,212 rows at last check. 12-agent audit Wave 1-4 source/test work is closed through origin `864f4e81`, and Round 4 WP-13 leftover P1 is source/test closed at `a7cb517f`: demo reconciler, strategist scheduler tune-target, and edge-estimate reload now read current `DemoCmdSenderSlot` snapshots instead of stale boot-time senders. Mac verification passed `cargo check --release -p openclaw_engine`, `tune_cmd_snapshot` 2/2, `edge_reload_tests` 16/16, `cargo test --release -p openclaw_engine --lib` 2908/0/1 after socket-permission escalated rerun, and `--bin openclaw-engine` 62/0. Runtime/code-bearing v35 head `5f6f3edf` was synced across Mac/origin/Linux before rebuild; post-rebuild docs-only sync may advance repository HEAD without another rebuild. `trade-core` rebuilt/restarted via `restart_all.sh --rebuild --keep-auth` on 2026-05-16, engine PID `69581`, API PID `69674`, watchdog `engine_alive=true`, demo fresh. Engine env has `OPENCLAW_ENABLE_PAPER=0`; engine log says paper pipeline disabled and `paper_state.disabled=true`, so Paper is not active even if watchdog sees a fresh disabled-state marker. Live is inactive because signed live authorization is absent, and `--keep-auth` did not renew auth. A4-C remains diagnostic-only/no-revive; `P1-A4C-REV-1` is not opened; `bb_breakout_oi_confirmed_5m` remains non-promotional. W-AUDIT-8a C1 is blocked again: the standalone `allLiquidation.BTCUSDT` 24h proof that started `2026-05-15T19:53:09Z` ended early at `2026-05-16T00:37:25Z` with `FAIL_CONNECTION` after `17055.2s/86400s`; it saw 15 candidate messages and 0 subscribe failures but is not proof-eligible. W-AUDIT-8b Funding Skew v0.2 remains approved only for a read-only Stage 0R query/report packet. Signed live authorization is absent and true-live remains blocked.
- TODO v36 cleanup archive is `docs/archive/2026-05-16--todo_v36_completion_cleanup_archive.md`. Completed rows were moved out only after commit/report cross-check; active TODO intentionally retains `P1-BBMF3-WIRE-1` because E2/BB verified close-cooldown plumbing landed but production reject callback wiring is still Phase 1b scope.
