...
- TODO v40 is the active dispatch queue as of 2026-05-16. Paper promotion is not an active lane: Stage 1 evidence is Demo-only after a future green Stage 0R replay preflight. There is no active W3 paper cohort. A4-C is an active-doc tombstone only: diagnostic panel / `[57]` healthcheck may remain, but A4-C must not be selected as a Stage 0R promotion candidate or Stage 1 Demo cohort source unless a materially new predictive variable is preregistered and passes a fresh strategy×symbol Stage 0R packet. `[55]`, `[67]`, and `[27]` remain closed from the 2026-05-15 source/runtime checks; true-live remains blocked by edge/LG/ops gates. `P1-WAVE-3-5-LINUX-MIGRATION-BACKLOG` is closed on `trade-core`: V092 continuous aggregates exist, V091/V092/V093 `_sqlx_migrations` rows are recorded, and checksum verify returns drift_count=0. The v40 P2 maintenance hygiene batch also closes `P2-H0-DISPLAY-LABEL-1`, `P2-START-LOCAL-HELPER`, `P2-PA-CALLPATH-GREP-RULE`, and `P2-CROSSTAB-I18N`, with `P2-PORTFOLIO-RESTING-58-HEALTHCHECK` corrected as already done under `[68]`.
- TODO v36 cleanup archive is `docs/archive/2026-05-16--todo_v36_completion_cleanup_archive.md`; the v37 tombstone cleanup archive is `docs/archive/2026-05-16--stage1_demo_a4c_tombstone_cleanup.md`; the v39 Wave 3.5 closure report is `docs/CCAgentWorkSpace/PM/workspace/reports/2026-05-16--wave_3_5_linux_migration_backlog_closure.md`; the v40 P2 maintenance report is `docs/CCAgentWorkSpace/PM/workspace/reports/2026-05-16--p2_maintenance_hygiene_closure.md`. Active TODO intentionally retains `P1-BBMF3-WIRE-1` because E2/BB verified close-cooldown plumbing landed but production reject callback wiring is still Phase 1b scope.
